#region Local Var
	var uLocal_0 = 0;
	var uLocal_1 = 0;
	int iLocal_2 = 0;
	int iLocal_3 = 0;
	int iLocal_4 = 0;
	int iLocal_5 = 0;
	int iLocal_6 = 0;
	int iLocal_7 = 0;
	int iLocal_8 = 0;
	int iLocal_9 = 0;
	int iLocal_10 = 0;
	int iLocal_11 = 0;
	var uLocal_12 = 0;
	var uLocal_13 = 0;
	int iLocal_14 = 0;
	float fLocal_15 = 0f;
	var uLocal_16 = 0;
	var uLocal_17 = 0;
	char* sLocal_18 = NULL;
	var uLocal_19 = 0;
	var uLocal_20 = 0;
	var uLocal_21 = 0;
	var uLocal_22 = 0;
	float fLocal_23 = 0f;
	float fLocal_24 = 0f;
	float fLocal_25 = 0f;
	var uLocal_26 = 0;
	var uLocal_27 = 0;
	float fLocal_28 = 0f;
	var uLocal_29 = 0;
	var uLocal_30 = 0;
	var uLocal_31 = 0;
	float fLocal_32 = 0f;
	float fLocal_33 = 0f;
	var uLocal_34 = 0;
	var uLocal_35 = 0;
	int iLocal_36 = 0;
	var uLocal_37 = 0;
	var uLocal_38 = 0;
	var uLocal_39 = 0;
	int iLocal_40 = 0;
	int iLocal_41 = 0;
	int iLocal_42 = 0;
	int iLocal_43 = 0;
	var uLocal_44 = 0;
	var uLocal_45 = 0;
	var uLocal_46 = 0;
	var uLocal_47 = 0;
	var uLocal_48 = 0;
	var uLocal_49 = 0;
	var uLocal_50 = 0;
	var uLocal_51 = 0;
	var uLocal_52 = 0;
	var uLocal_53 = 0;
	var uLocal_54 = 0;
	var uLocal_55 = 0;
	var uLocal_56 = 0;
	var uLocal_57 = 0;
	var uLocal_58 = 0;
	var uLocal_59 = 0;
	var uLocal_60 = 0;
	var uLocal_61 = 0;
	float fLocal_62 = 0f;
	var uLocal_63 = 0;
	var uLocal_64 = 0;
	var uLocal_65 = 0;
	var uLocal_66 = 0;
	var uLocal_67 = 0;
	var uLocal_68 = -1;
	var uLocal_69 = 0;
	var uLocal_70 = 0;
	var uLocal_71 = 0;
	var uLocal_72 = 0;
	var uLocal_73 = 0;
	var uLocal_74 = 0;
	var uLocal_75 = 0;
	float fLocal_76 = 0f;
	float fLocal_77 = 0f;
	float fLocal_78 = 0f;
	float fLocal_79 = 0f;
	var uLocal_80 = 0;
	var uLocal_81 = 0;
	var uLocal_82 = 0;
	var uLocal_83 = 0;
	var uLocal_84 = 0;
	var uLocal_85 = 0;
	var uLocal_86 = 0;
	var uLocal_87 = 0;
	var uLocal_88 = 0;
	var uLocal_89 = 0;
	var uLocal_90 = 0;
	var uLocal_91 = 0;
	var uLocal_92 = 0;
	var uLocal_93 = 0;
	var uLocal_94 = 0;
	var uLocal_95 = 0;
	int iLocal_96 = 0;
	var uLocal_97 = 0;
	float fLocal_98 = 0f;
	var uLocal_99 = 0;
	var uLocal_100 = 0;
	var uLocal_101 = 0;
	var uLocal_102 = 0;
	var uLocal_103 = 0;
	var uLocal_104 = 0;
	var uLocal_105 = 0;
	var uLocal_106 = 0;
	var uLocal_107 = 0;
	var uLocal_108 = 0;
	var uLocal_109 = 0;
	var uLocal_110 = 0;
	var uLocal_111 = 0;
	var uLocal_112 = 0;
	var uLocal_113 = 0;
	var uLocal_114 = 0;
	var uLocal_115 = 0;
	var uLocal_116 = 0;
	var uLocal_117 = 0;
	var uLocal_118 = 0;
	var uLocal_119 = 0;
	var uLocal_120 = 0;
	var uLocal_121 = 0;
	var uLocal_122 = 0;
	var uLocal_123 = 0;
	var uLocal_124 = 0;
	var uLocal_125 = 0;
	var uLocal_126 = 0;
	var uLocal_127 = 0;
	var uLocal_128 = 0;
	var uLocal_129 = 0;
	var uLocal_130 = 0;
	var uLocal_131 = 0;
	var uLocal_132 = 0;
	var uLocal_133 = 0;
	var uLocal_134 = 0;
	var uLocal_135 = 0;
	var uLocal_136 = 0;
	var uLocal_137 = 0;
	var uLocal_138 = 0;
	var uLocal_139 = 0;
	var uLocal_140 = 0;
	var uLocal_141 = 0;
	var uLocal_142 = 0;
	var uLocal_143 = 0;
	var uLocal_144 = 0;
	var uLocal_145 = 17;
	var uLocal_146 = 0;
	var uLocal_147 = 0;
	var uLocal_148 = 0;
	var uLocal_149 = 0;
	var uLocal_150 = 0;
	var uLocal_151 = 0;
	var uLocal_152 = 0;
	var uLocal_153 = 0;
	var uLocal_154 = 0;
	var uLocal_155 = 0;
	var uLocal_156 = 0;
	var uLocal_157 = 0;
	var uLocal_158 = 0;
	var uLocal_159 = 0;
	var uLocal_160 = 0;
	var uLocal_161 = 0;
	var uLocal_162 = 0;
	var uLocal_163 = 17;
	var uLocal_164 = 0;
	var uLocal_165 = 0;
	var uLocal_166 = 0;
	var uLocal_167 = 0;
	var uLocal_168 = 0;
	var uLocal_169 = 0;
	var uLocal_170 = 0;
	var uLocal_171 = 0;
	var uLocal_172 = 0;
	var uLocal_173 = 0;
	var uLocal_174 = 0;
	var uLocal_175 = 0;
	var uLocal_176 = 0;
	var uLocal_177 = 0;
	var uLocal_178 = 0;
	var uLocal_179 = 0;
	var uLocal_180 = 0;
	var uLocal_181 = 0;
	var uLocal_182 = 0;
	var uLocal_183 = 0;
	var uLocal_184 = 0;
	var uLocal_185 = 0;
	var uLocal_186 = 0;
	var uLocal_187 = 12;
	var uLocal_188 = 0;
	var uLocal_189 = 0;
	var uLocal_190 = 0;
	var uLocal_191 = 0;
	var uLocal_192 = 0;
	var uLocal_193 = 0;
	var uLocal_194 = 0;
	var uLocal_195 = 0;
	var uLocal_196 = 0;
	var uLocal_197 = 0;
	var uLocal_198 = 0;
	var uLocal_199 = 0;
	var uLocal_200 = 12;
	var uLocal_201 = 0;
	var uLocal_202 = 0;
	var uLocal_203 = 0;
	var uLocal_204 = 0;
	var uLocal_205 = 0;
	var uLocal_206 = 0;
	var uLocal_207 = 0;
	var uLocal_208 = 0;
	var uLocal_209 = 0;
	var uLocal_210 = 0;
	var uLocal_211 = 0;
	var uLocal_212 = 0;
	var uLocal_213 = 12;
	var uLocal_214 = 0;
	var uLocal_215 = 0;
	var uLocal_216 = 0;
	var uLocal_217 = 0;
	var uLocal_218 = 0;
	var uLocal_219 = 0;
	var uLocal_220 = 0;
	var uLocal_221 = 0;
	var uLocal_222 = 0;
	var uLocal_223 = 0;
	var uLocal_224 = 0;
	var uLocal_225 = 0;
	var uLocal_226 = 9;
	var uLocal_227 = 0;
	var uLocal_228 = 0;
	var uLocal_229 = 0;
	var uLocal_230 = 0;
	var uLocal_231 = 0;
	var uLocal_232 = 0;
	var uLocal_233 = 0;
	var uLocal_234 = 0;
	var uLocal_235 = 0;
	var uLocal_236 = 9;
	var uLocal_237 = 0;
	var uLocal_238 = 0;
	var uLocal_239 = 0;
	var uLocal_240 = 0;
	var uLocal_241 = 0;
	var uLocal_242 = 0;
	var uLocal_243 = 0;
	var uLocal_244 = 0;
	var uLocal_245 = 0;
	var uLocal_246 = 0;
	var uLocal_247 = 0;
	var uLocal_248 = 0;
	var uLocal_249 = 0;
	var uLocal_250 = 0;
	var uLocal_251 = 0;
	var uLocal_252 = 0;
	var uLocal_253 = 0;
	var uLocal_254 = 0;
	int iLocal_255 = 0;
	int iLocal_256 = 0;
	var uLocal_257 = 0;
	var uLocal_258 = 0;
#endregion

void __EntryFunction__()
{
	iLocal_2 = 1;
	iLocal_3 = 134;
	iLocal_4 = 134;
	iLocal_5 = 1;
	iLocal_6 = 1;
	iLocal_7 = 1;
	iLocal_8 = 134;
	iLocal_9 = 1;
	iLocal_10 = 12;
	iLocal_11 = 12;
	iLocal_14 = -1;
	fLocal_15 = 0.001f;
	sLocal_18 = "NULL";
	fLocal_23 = 80f;
	fLocal_24 = 140f;
	fLocal_25 = 180f;
	fLocal_28 = 0f;
	fLocal_32 = -0.0375f;
	fLocal_33 = 0.17f;
	iLocal_36 = 3;
	iLocal_40 = 1;
	iLocal_41 = 65;
	iLocal_42 = 49;
	iLocal_43 = 64;
	fLocal_62 = ((0.05f + 0.275f) - 0.01f);
	fLocal_76 = 3f;
	fLocal_77 = 0f;
	fLocal_78 = 2f;
	fLocal_79 = 100f;
	iLocal_96 = -1;
	fLocal_98 = 0.5f;
	if (!func_1236(&Global_1697306))
	{
		func_1233();
	}
	while (true)
	{
		unk_0x4EDE34FBADD967A6(0);
		func_1(&Global_1697306);
	}
}

void func_1(var uParam0)
{
	func_1229(uParam0);
	switch (func_1228())
	{
		case 0:
			if (func_1217(uParam0))
			{
				func_1216(1);
			}
			break;
		
		case 1:
			func_3(uParam0);
			break;
		
		case 2:
			func_1233();
			break;
	}
	func_2();
}

void func_2()
{
	Global_1697301 = 0;
}

void func_3(var uParam0)
{
	func_1099(uParam0);
	func_922(uParam0);
	func_536(uParam0);
	func_199(uParam0);
	func_188(uParam0);
	func_63(uParam0);
	func_23(uParam0);
	func_20(uParam0);
	func_4(uParam0);
}

void func_4(var uParam0)
{
	int iVar0;
	
	if (unk_0x460153A63B9477BC() != uParam0->f_4356)
	{
		return;
	}
	iVar0 = 0;
	while (iVar0 < 16)
	{
		if (func_19(uParam0, iVar0) && !func_18(uParam0, iVar0))
		{
			if (func_12(uParam0, iVar0))
			{
				func_10(func_11(iVar0), -1);
				func_5(uParam0, iVar0);
			}
		}
		iVar0++;
	}
}

void func_5(var uParam0, int iParam1)
{
	int iVar0;
	
	if (!unk_0xCE990E643CD9D0E5(uParam0->f_4353.f_1, iParam1))
	{
		unk_0xBE20AB8238688965(&(uParam0->f_4353.f_1), iParam1);
	}
	iVar0 = func_9(8723, -1, 0);
	if (!unk_0xCE990E643CD9D0E5(iVar0, iParam1))
	{
		unk_0xBE20AB8238688965(&iVar0, iParam1);
		func_6(8723, iVar0, -1, 1, 0);
	}
}

void func_6(int iParam0, int iParam1, int iParam2, int iParam3, bool bParam4)
{
	int iVar0;
	
	if (bParam4)
	{
	}
	iVar0 = Global_2552060[iParam0 /*3*/][func_7(iParam2)];
	if (iVar0 != 0)
	{
		unk_0x6000E4684CB4330B(iVar0, iParam1, iParam3);
	}
}

int func_7(var uParam0)
{
	int iVar0;
	int iVar1;
	
	iVar0 = uParam0;
	if (iVar0 == -1)
	{
		iVar1 = func_8();
		if (iVar1 > -1)
		{
			Global_2551772 = 0;
			iVar0 = iVar1;
		}
		else
		{
			iVar0 = 0;
			Global_2551772 = 1;
		}
	}
	return iVar0;
}

int func_8()
{
	return Global_1312763;
}

int func_9(int iParam0, int iParam1, int iParam2)
{
	var uVar0;
	var uVar1;
	
	if (iParam0 != 11771)
	{
		if (iParam2 == 0)
		{
		}
		uVar0 = Global_2552060[iParam0 /*3*/][func_7(iParam1)];
		if (unk_0xD2F202166691EDB2(uVar0, &uVar1, -1))
		{
			return uVar1;
		}
	}
	return 0;
}

void func_10(char* sParam0, int iParam1)
{
	unk_0x17751E107423AFE9(sParam0);
	unk_0x1A169149B3D18FCB(0, 0, 1, iParam1);
}

char* func_11(int iParam0)
{
	switch (iParam0)
	{
		case 0:
			return "CH_PLAN_HLP_0";
		
		case 1:
			return "CH_PLAN_HLP_1";
		
		case 2:
			return "CH_PLAN_HLP_2";
		
		case 3:
			return "CH_PLAN_HLP_3";
		
		case 4:
			return "CH_PLAN_HLP_4";
		
		case 5:
			return "CH_PLAN_HLP_5";
		
		case 6:
			return "CH_PLAN_HLP_6";
		
		case 7:
			return "CH_PLAN_HLP_7";
		
		case 8:
			return "CH_PLAN_HLP_8";
		
		case 9:
			return "CH_PLAN_HLP_9";
		
		case 10:
			return "CH_PLAN_HLP_10";
		
		case 11:
			return "CH_PLAN_HLP_11";
		
		case 12:
			return "CH_PLAN_HLP_12";
		
		case 13:
			return "CH_PLAN_HLP_13";
		
		case 14:
			return "CH_PLAN_HLP_14";
		
		case 15:
			return "CH_PLAN_HLP_15";
		
		default:
	}
	return "";
}

bool func_12(var uParam0, int iParam1)
{
	if (((func_17(uParam0, 1) || uParam0->f_2604) || func_16(uParam0)) || (func_15() && func_14() != 33))
	{
		return 0;
	}
	if (func_13(iParam1))
	{
		return 1;
	}
	return !unk_0xDABD547F0DF2906C();
}

int func_13(int iParam0)
{
	switch (iParam0)
	{
		case 1:
		case 6:
			return 0;
		
		default:
	}
	return 1;
}

int func_14()
{
	return Global_1590682[unk_0x460153A63B9477BC() /*883*/].f_196;
}

bool func_15()
{
	return Global_1590682[unk_0x460153A63B9477BC() /*883*/].f_196 != 0;
}

int func_16(var uParam0)
{
	if (uParam0->f_4173.f_8 != -1)
	{
		return 1;
	}
	return 0;
}

bool func_17(var uParam0, int iParam1)
{
	return unk_0xCE990E643CD9D0E5(uParam0->f_4357, iParam1);
}

bool func_18(var uParam0, int iParam1)
{
	if (unk_0xCE990E643CD9D0E5(func_9(8723, -1, 0), iParam1))
	{
		return 1;
	}
	return unk_0xCE990E643CD9D0E5(uParam0->f_4353.f_1, iParam1);
}

bool func_19(var uParam0, int iParam1)
{
	return unk_0xCE990E643CD9D0E5(uParam0->f_4353, iParam1);
}

void func_20(var uParam0)
{
	bool bVar0;
	
	bVar0 = unk_0x1037B9D45CE6B788(unk_0x9B0761B4C3EB8BC7(), 2714.045f, -365.8374f, -55.7809f, 2714.005f, -373.1754f, -53.7809f, 5.25f, 0, 1, 0);
	if (!unk_0xCE990E643CD9D0E5(uParam0->f_4357, 8))
	{
		if (bVar0)
		{
			if (func_22())
			{
				unk_0xBE20AB8238688965(&(uParam0->f_4357), 8);
			}
		}
	}
	else if (!bVar0)
	{
		func_21();
		unk_0xD2459066EA58CE43(&(uParam0->f_4357), 8);
	}
}

void func_21()
{
	unk_0x9128BA5F998AA176("DLC_MPHEIST/HEIST_PLANNING_BOARD");
}

bool func_22()
{
	return unk_0x8B917081F9836EDD("DLC_MPHEIST/HEIST_PLANNING_BOARD", 0, -1);
}

void func_23(var uParam0)
{
	int iVar0;
	int iVar1;
	struct<6> Var2;
	struct<6> Var8;
	
	if (func_14() == 31)
	{
		return;
	}
	iVar1 = -1;
	iVar0 = 0;
	while (iVar0 < 77)
	{
		if (func_62(uParam0, iVar0))
		{
			if (func_33(uParam0, iVar0))
			{
				iVar1 = iVar0;
			}
			else
			{
				Jump @123; //curOff = 59
				if (func_31(uParam0, iVar0))
				{
					Var2 = { func_30() };
					Var8 = { func_28(uParam0, iVar0) };
					if (unk_0xF005CCA4263DF67F(&Var2, &Var8) && !unk_0x7BCC91F3C1CF24E8(&Var2))
					{
						iVar1 = iVar0;
					}
					else
					{
						iVar0++;
					}
					if (uParam0->f_4173.f_8 != iVar1)
					{
						if (uParam0->f_4173.f_8 == 76 && iVar1 == -1)
						{
							func_25(uParam0, 76);
							func_24(uParam0, 76);
						}
						uParam0->f_4173.f_8 = iVar1;
					}
				}

void func_24(var uParam0, int iParam1)
{
	if (iParam1 == -1 || iParam1 == 77)
	{
		return;
	}
	if (unk_0xCE990E643CD9D0E5(uParam0->f_4173[(iParam1 / 32)], (iParam1 % 32)))
	{
		unk_0xD2459066EA58CE43(&(uParam0->f_4173[(iParam1 / 32)]), (iParam1 % 32));
	}
}

void func_25(var uParam0, int iParam1)
{
	if (func_31(uParam0, iParam1))
	{
		unk_0xD2459066EA58CE43(&(uParam0->f_4173.f_4[(iParam1 / 32)]), (iParam1 % 32));
		func_26(iParam1);
	}
}

void func_26(int iParam0)
{
	int iVar0;
	
	if (func_27(iParam0) != 11771)
	{
		iVar0 = func_9(func_27(iParam0), -1, 0);
		unk_0xD2459066EA58CE43(&iVar0, (iParam0 % 32));
		func_6(func_27(iParam0), iVar0, -1, 1, 0);
	}
}

int func_27(int iParam0)
{
	switch ((iParam0 / 32))
	{
		case 0:
			return 8829;
		
		case 1:
			return 8830;
		
		case 2:
			return 8831;
		
		default:
	}
	return 11771;
}

struct<6> func_28(var uParam0, int iParam1)
{
	struct<6> Var0;
	
	StringCopy(&Var0, func_29(uParam0, iParam1), 24);
	return Var0;
}

char* func_29(var uParam0, int iParam1)
{
	switch (iParam1)
	{
		case 0:
			return "HS3LE_SB_1";
		
		case 1:
			return "HS3LE_SB_2";
		
		case 2:
			return "HS3LE_SB_3";
		
		case 3:
			return "HS3LE_SB_4";
		
		case 4:
			return "HS3LE_SB_5";
		
		case 5:
			return "HS3LE_SB_7";
		
		case 6:
			return "HS3LE_SB_9";
		
		case 7:
			return "HS3LE_SB_6";
		
		case 8:
			return "HS3LE_SB_8";
		
		case 9:
			return "HS3LE_SB_10";
		
		case 10:
			return "HS3LE_PB_1";
		
		case 11:
			if (uParam0->f_4356 != -1)
			{
				if (unk_0xCE990E643CD9D0E5(Global_1701669[uParam0->f_4356 /*68*/].f_42, 6))
				{
					return "HS3LE_PB_2B";
				}
			}
			return "HS3LE_PB_2";
		
		case 12:
			return "HS3LE_PB_3";
		
		case 13:
			return "HS3LE_PB_19";
		
		case 14:
			return "HS3LE_PB_6";
		
		case 15:
			return "HS3LE_PB_5";
		
		case 16:
			return "HS3LE_PB_4";
		
		case 17:
			return "HS3LE_PB_7";
		
		case 18:
			return "HS3LE_PB_8";
		
		case 19:
			return "HS3LE_PB_9";
		
		case 20:
			return "HS3LE_PB_10";
		
		case 21:
			return "HS3LE_PB_10B";
		
		case 22:
			return "HS3LE_PB_10C";
		
		case 23:
			return "HS3LE_PB_11";
		
		case 24:
			return "HS3LE_PB_11B";
		
		case 25:
			return "HS3LE_PB_12";
		
		case 26:
			return "HS3LE_PB_13";
		
		case 27:
			return "HS3LE_PB_14";
		
		case 28:
			return "HS3LE_PB_15";
		
		case 29:
			return "HS3LE_PB_16";
		
		case 30:
			return "HS3LE_PB_17";
		
		case 31:
			return "HS3LE_PB_18";
		
		case 32:
			return "HS3LE_GC_1";
		
		case 33:
			return "HS3LE_GC_2";
		
		case 34:
			return "HS3LE_GC_3";
		
		case 35:
			return "HS3LE_GC_4";
		
		case 36:
			return "HS3LE_GC_5";
		
		case 37:
			return "HS3LE_DC_1";
		
		case 38:
			return "HS3LE_DC_2";
		
		case 39:
			return "HS3LE_DC_3";
		
		case 40:
			return "HS3LE_DC_4";
		
		case 41:
			return "HS3LE_DC_5";
		
		case 42:
			return "HS3LE_HC_1";
		
		case 43:
			return "HS3LE_HC_2";
		
		case 44:
			return "HS3LE_HC_3";
		
		case 45:
			return "HS3LE_HC_4";
		
		case 46:
			return "HS3LE_HC_5";
		
		case 47:
			return "HS3LE_FB_1";
		
		case 48:
			return "HS3LE_FB_2";
		
		case 49:
			return "HS3LE_FB_3";
		
		case 50:
			return "HS3LE_FB_4";
		
		case 51:
			return "HS3LE_FB_5";
		
		case 52:
			return "HS3LE_FB_6";
		
		case 53:
			return "HS3LE_FB_8";
		
		case 54:
			return "HS3LE_FB_7A";
		
		case 55:
			return "HS3LE_FB_9";
		
		case 56:
			return "HS3LE_FB_10";
		
		case 57:
			return "HS3LE_FB_11";
		
		case 58:
			return "HS3LE_FB_12";
		
		case 59:
			return "HS3LE_FB_13";
		
		case 60:
			return "HS3LE_FB_14";
		
		case 61:
			return "HS3LE_SR_1";
		
		case 62:
			return "HS3LE_SR_2";
		
		case 63:
			return "HS3LE_SR_3";
		
		case 64:
			return "HS3LE_SR_4";
		
		case 65:
			return "HS3LE_SR_5";
		
		case 66:
			return "HS3LE_SR_6";
		
		case 67:
			return "HS3LE_SR_7";
		
		case 68:
			return "HS3LE_PR_1";
		
		case 69:
			return "HS3LE_PR_2";
		
		case 70:
			return "HS3LE_FR_1";
		
		case 71:
			return "HS3LE_FR_2";
		
		case 72:
			return "HS3LE_FR_3";
		
		case 73:
			return "HS3LE_FR_4";
		
		case 74:
			return "HS3LE_FR_5";
		
		case 75:
			return "HS3PR_GN_8";
		
		case 76:
			return "HS3PR_GN_9";
		
		default:
	}
	return "";
}

struct<6> func_30()
{
	struct<6> Var0;
	
	StringCopy(&Var0, "NULL", 24);
	if (Global_21005 == 4)
	{
		return Global_20624;
	}
	return Var0;
}

int func_31(var uParam0, int iParam1)
{
	if (func_32(iParam1) || unk_0xCE990E643CD9D0E5(uParam0->f_4173.f_4[(iParam1 / 32)], (iParam1 % 32)))
	{
		return 1;
	}
	return 0;
}

int func_32(int iParam0)
{
	if (func_27(iParam0) != 11771)
	{
		return unk_0xCE990E643CD9D0E5(func_9(func_27(iParam0), -1, 0), (iParam0 % 32));
	}
	return 0;
}

int func_33(var uParam0, int iParam1)
{
	char* sVar0;
	
	if (func_31(uParam0, iParam1))
	{
		return 0;
	}
	if (!func_61())
	{
		MemCopy(&sVar0, {func_28(uParam0, iParam1)}, 4);
		if (!unk_0x7BCC91F3C1CF24E8(&sVar0))
		{
			func_60(&(uParam0->f_4173.f_9), 2, 0, "LESTER", 0, 1);
			if (func_36(&(uParam0->f_4173.f_9), func_59(&sVar0), &sVar0, 9, 0, 0, 0))
			{
				func_34(uParam0, iParam1);
				return 1;
			}
		}
	}
	return 0;
}

void func_34(var uParam0, int iParam1)
{
	if (!func_31(uParam0, iParam1))
	{
		unk_0xBE20AB8238688965(&(uParam0->f_4173.f_4[(iParam1 / 32)]), (iParam1 % 32));
		func_35(iParam1);
	}
}

void func_35(int iParam0)
{
	int iVar0;
	
	if (func_27(iParam0) != 11771)
	{
		iVar0 = func_9(func_27(iParam0), -1, 0);
		unk_0xBE20AB8238688965(&iVar0, (iParam0 % 32));
		func_6(func_27(iParam0), iVar0, -1, 1, 0);
	}
}

bool func_36(var uParam0, char* sParam1, char* sParam2, int iParam3, int iParam4, int iParam5, int iParam6)
{
	func_58(uParam0, 145, sParam1, iParam4, iParam5, iParam6);
	if (iParam3 > 7)
	{
		if (iParam3 < 12)
		{
			iParam3 = 7;
		}
	}
	Global_21012 = 0;
	Global_21014 = 0;
	Global_21019 = 0;
	Global_21996 = 0;
	Global_21998 = 0;
	Global_22002 = 0;
	Global_2621441 = 0;
	return func_37(sParam2, iParam3, 0);
}

int func_37(char* sParam0, int iParam1, bool bParam2)
{
	Global_21006 = 0;
	if (Global_21005 == 0 || Global_21007 == 2)
	{
		if (Global_21005 != 0)
		{
			if (iParam1 > Global_21007)
			{
				if (Global_21012 == 0)
				{
					unk_0xBE4122AC23440E7D(0);
					Global_19681.f_1 = 3;
					Global_21005 = 0;
					Global_21006 = 1;
					Global_21058 = 0;
					Global_21001 = 0;
					Global_21002 = 0;
					Global_21016 = 0;
					Global_21015 = 0;
					Global_19680 = 0;
				}
				else
				{
					func_57();
					return 0;
				}
			}
			else
			{
				return 0;
			}
		}
		if (unk_0x6A5CC4A2C9D9ED8D())
		{
			return 0;
		}
		if (func_56(8, -1))
		{
			return 0;
		}
		Global_21081 = { Global_21075 };
		func_55();
		Global_20294 = { Global_20459 };
		Global_21011 = Global_21012;
		Global_21018 = Global_21019;
		Global_2621442 = Global_2621441;
		Global_21020 = { Global_21036 };
		Global_21013 = Global_21014;
		Global_21995 = Global_21996;
		Global_22003 = { Global_22009 };
		Global_21997 = Global_21998;
		Global_21999 = Global_22000;
		Global_22001 = Global_22002;
		Global_20624.f_370 = Global_21994;
		Global_20624.f_368 = Global_21992;
		Global_20624.f_369 = Global_21993;
		Global_21001 = Global_21002;
		if (Global_21011)
		{
			unk_0xD2459066EA58CE43(&Global_7551, 20);
			unk_0xD2459066EA58CE43(&Global_7552, 17);
			unk_0xD2459066EA58CE43(&Global_7553, 0);
			if (bParam2)
			{
				func_47();
				if (Global_8356[Global_19681 /*2811*/][0 /*281*/].f_259 == 2)
				{
					if (iParam1 == 13)
					{
					}
					else
					{
						return 0;
					}
				}
				if (Global_19681.f_1 > 3)
				{
					return 0;
				}
			}
			if (Global_19647 == 1)
			{
				return 0;
			}
			if (unk_0x5A228A6A51E757C8(unk_0x460153A63B9477BC()))
			{
				if (unk_0x42A04E707DE50816(unk_0x9B0761B4C3EB8BC7()))
				{
					return 0;
				}
				if (func_46())
				{
					return 0;
				}
				if (unk_0x7587087519CF4F83(unk_0x9B0761B4C3EB8BC7()))
				{
					return 0;
				}
				if (unk_0xCED6B5226244381C(unk_0x9B0761B4C3EB8BC7()))
				{
					return 0;
				}
				if (unk_0xAACBFCE60AF94F2D(unk_0x9B0761B4C3EB8BC7()))
				{
					return 0;
				}
				if (unk_0x02D1369035663875(unk_0x9B0761B4C3EB8BC7(), joaat("gadget_parachute")))
				{
					return 0;
				}
				if (!Global_76833)
				{
					if (unk_0xE72C9284B5143451(unk_0x9B0761B4C3EB8BC7()))
					{
						return 0;
					}
					if (unk_0x5197F8CD4A72E68F(unk_0x460153A63B9477BC()))
					{
						return 0;
					}
					if (unk_0x887D491E579FA61E(unk_0x9B0761B4C3EB8BC7()))
					{
						return 0;
					}
					if (unk_0x4C9EC9A10C274889(unk_0x460153A63B9477BC(), 0))
					{
						return 0;
					}
				}
			}
			if (func_45())
			{
				return 0;
			}
			else
			{
				switch (Global_19681.f_1)
				{
					case 7:
						return 0;
						break;
					
					case 8:
						return 0;
						break;
					
					case 9:
						break;
					
					case 10:
						break;
					
					default:
						break;
				}
				if (unk_0xCE990E643CD9D0E5(Global_7551, 9))
				{
					return 0;
				}
			}
			func_44();
			Global_21015 = bParam2;
		}
		Global_21007 = iParam1;
		StringCopy(&Global_20624, sParam0, 24);
		Global_19871 = 0;
		func_43();
		func_38();
		return 1;
	}
	if (Global_21005 == 5)
	{
		return 0;
	}
	if (iParam1 < Global_21007 || iParam1 == Global_21007)
	{
		return 0;
	}
	if (iParam1 == 2)
	{
	}
	else
	{
		func_57();
	}
	return 0;
}

void func_38()
{
	if (!func_39())
	{
		return;
	}
	if (Global_21011)
	{
		MemCopy(&(Global_1703981.f_1), {Global_20624}, 4);
		Global_1703981 = Global_6866;
		Global_1703981.f_6 = Global_21015;
	}
}

int func_39()
{
	if (!Global_262145.f_28161)
	{
		return 0;
	}
	if (!Global_76833)
	{
		return 0;
	}
	if (unk_0x460153A63B9477BC() == func_42())
	{
		return 0;
	}
	if (func_40(unk_0x460153A63B9477BC()))
	{
		return 0;
	}
	if (unk_0xCE990E643CD9D0E5(Global_1630317[unk_0x460153A63B9477BC() /*595*/].f_1, 7))
	{
		return 0;
	}
	if (unk_0xFC559366953F62B3())
	{
		return 0;
	}
	return 1;
}

bool func_40(int iParam0)
{
	return func_41(iParam0, 20);
}

bool func_41(int iParam0, int iParam1)
{
	return unk_0xCE990E643CD9D0E5(Global_1630317[iParam0 /*595*/].f_11.f_4, iParam1);
}

int func_42()
{
	return -1;
}

void func_43()
{
	int iVar0;
	
	iVar0 = 0;
	while (iVar0 <= 69)
	{
		StringCopy(&(Global_19873[iVar0 /*6*/]), "", 24);
		iVar0++;
	}
	unk_0xBE4122AC23440E7D(0);
	Global_21005 = 1;
}

void func_44()
{
	Global_21058 = Global_21057;
	Global_21052 = Global_21053;
	Global_21099 = { Global_21087 };
	Global_21105 = { Global_21093 };
	Global_21060 = Global_21059;
	Global_21129 = { Global_21111 };
	Global_21135 = { Global_21117 };
	Global_21141 = { Global_21123 };
	Global_21147 = { Global_21153 };
	Global_6866 = Global_6867;
	Global_6868 = Global_6869;
	Global_21016 = Global_21017;
	Global_21018 = Global_21019;
	Global_21020 = { Global_21036 };
	Global_21009 = Global_21010;
	Global_22021 = 0;
	Global_21054 = 0;
	Global_21055 = 0;
	unk_0xD2459066EA58CE43(&Global_7552, 16);
}

int func_45()
{
	if (Global_19681.f_1 == 1 || Global_19681.f_1 == 0)
	{
		return 1;
	}
	return 0;
}

int func_46()
{
	int iVar0;
	int iVar1;
	
	if (Global_76833)
	{
		iVar0 = 0;
		unk_0x1386E5E658CE3032(unk_0x9B0761B4C3EB8BC7(), &iVar1, 1);
		if (unk_0x5A228A6A51E757C8(unk_0x460153A63B9477BC()))
		{
			if ((iVar1 == joaat("weapon_sniperrifle") || iVar1 == joaat("weapon_heavysniper")) || iVar1 == joaat("weapon_remotesniper"))
			{
				iVar0 = 1;
			}
		}
		if (unk_0x5BFE1F305D18843B() && iVar0 == 1)
		{
			return 1;
		}
		else
		{
			return 0;
		}
	}
	if (unk_0x5A228A6A51E757C8(unk_0x460153A63B9477BC()))
	{
		if (unk_0xD15202C6B03220B8(unk_0x9B0761B4C3EB8BC7(), 78, 1))
		{
			return 1;
		}
		else
		{
			return 0;
		}
	}
	return 1;
}

void func_47()
{
	if (func_54(14))
	{
		if (!unk_0xA59F96B50B97E63C(unk_0x9B0761B4C3EB8BC7(), 0))
		{
			if (unk_0x36FE6D3220816ADA(unk_0x9B0761B4C3EB8BC7()) == Global_111858.f_28046[0 /*29*/])
			{
				Global_19681 = 0;
			}
			else if (unk_0x36FE6D3220816ADA(unk_0x9B0761B4C3EB8BC7()) == Global_111858.f_28046[1 /*29*/])
			{
				Global_19681 = 1;
			}
			else if (unk_0x36FE6D3220816ADA(unk_0x9B0761B4C3EB8BC7()) == Global_111858.f_28046[2 /*29*/])
			{
				Global_19681 = 2;
			}
			else
			{
				Global_19681 = 0;
			}
		}
	}
	else
	{
		Global_19681 = func_48();
		if (Global_19681 == 145)
		{
			Global_19681 = 3;
		}
		if (Global_76833)
		{
			Global_19681 = 3;
		}
		if (Global_19681 > 3)
		{
			Global_19681 = 3;
		}
	}
}

var func_48()
{
	func_49();
	return Global_111858.f_2359.f_539.f_4321;
}

void func_49()
{
	int iVar0;
	
	if (unk_0xAE06B9E39EBDE049(unk_0x9B0761B4C3EB8BC7()))
	{
		if (func_52(Global_111858.f_2359.f_539.f_4321) != unk_0x36FE6D3220816ADA(unk_0x9B0761B4C3EB8BC7()))
		{
			iVar0 = func_51(unk_0x9B0761B4C3EB8BC7());
			if (func_50(iVar0) && (!func_54(14) || Global_110809))
			{
				if (Global_111858.f_2359.f_539.f_4321 != iVar0 && func_50(Global_111858.f_2359.f_539.f_4321))
				{
					Global_111858.f_2359.f_539.f_4322 = Global_111858.f_2359.f_539.f_4321;
				}
				Global_111858.f_2359.f_539.f_4323 = iVar0;
				Global_111858.f_2359.f_539.f_4321 = iVar0;
				return;
			}
		}
		else
		{
			if (Global_111858.f_2359.f_539.f_4321 != 145)
			{
				Global_111858.f_2359.f_539.f_4323 = Global_111858.f_2359.f_539.f_4321;
			}
			return;
		}
	}
	Global_111858.f_2359.f_539.f_4321 = 145;
}

bool func_50(int iParam0)
{
	return iParam0 < 3;
}

int func_51(var uParam0)
{
	int iVar0;
	int iVar1;
	
	if (unk_0xAE06B9E39EBDE049(uParam0))
	{
		iVar1 = unk_0x36FE6D3220816ADA(uParam0);
		iVar0 = 0;
		while (iVar0 <= 2)
		{
			if (func_52(iVar0) == iVar1)
			{
				return iVar0;
			}
			iVar0++;
		}
	}
	return 145;
}

int func_52(int iParam0)
{
	if (func_50(iParam0))
	{
		return func_53(iParam0);
	}
	else if (iParam0 != 145)
	{
	}
	return 0;
}

var func_53(int iParam0)
{
	return Global_1848[iParam0 /*29*/];
}

bool func_54(int iParam0)
{
	return Global_41631 == iParam0;
}

void func_55()
{
	int iVar0;
	
	iVar0 = 0;
	while (iVar0 <= 15)
	{
		Global_20294[iVar0 /*10*/] = 0;
		StringCopy(&(Global_20294[iVar0 /*10*/].f_1), "", 24);
		Global_20294[iVar0 /*10*/].f_7 = 0;
		Global_20294[iVar0 /*10*/].f_8 = 0;
		iVar0++;
	}
	Global_20294.f_161 = -99;
	Global_20294.f_162 = { 0f, 0f, 0f };
}

bool func_56(int iParam0, int iParam1)
{
	switch (iParam0)
	{
		case 5:
			if (iParam1 > -1)
			{
				return Global_1377236.f_203[iParam1];
			}
			break;
	}
	return unk_0xCE990E643CD9D0E5(Global_1377236.f_1048, iParam0);
}

void func_57()
{
	unk_0x62B2C9892E3E142D();
	Global_22016 = 0;
	if ((unk_0x6B12213471F330A3() || Global_19681.f_1 == 9) || Global_19680 == 1)
	{
		unk_0xBE4122AC23440E7D(0);
		Global_21005 = 6;
		Global_19681.f_1 = 3;
		return;
	}
	if (unk_0x6A5CC4A2C9D9ED8D())
	{
		unk_0xBE4122AC23440E7D(1);
		Global_21005 = 6;
		return;
	}
}

void func_58(var uParam0, int iParam1, char* sParam2, int iParam3, int iParam4, var uParam5)
{
	Global_20459 = { *uParam0 };
	Global_6867 = iParam1;
	StringCopy(&Global_21075, sParam2, 24);
	Global_21994 = uParam5;
	if (iParam3 == 0)
	{
		Global_21992 = 1;
		Global_21990 = 0;
	}
	else
	{
		Global_21992 = 0;
		Global_21990 = 1;
	}
	if (iParam4 == 0)
	{
		Global_21993 = 1;
		Global_21991 = 0;
	}
	else
	{
		Global_21993 = 0;
		Global_21991 = 1;
	}
}

char* func_59(char* sParam0)
{
	if (unk_0xF005CCA4263DF67F(sParam0, "HS3PR_GN_8") || unk_0xF005CCA4263DF67F(sParam0, "HS3PR_GN_9"))
	{
		return "HS3PRAU";
	}
	return "HS3LEAU";
}

void func_60(var uParam0, int iParam1, int iParam2, char* sParam3, int iParam4, int iParam5)
{
	if ((uParam0[iParam1 /*10*/])->f_7 == 1)
	{
	}
	(*uParam0)[iParam1 /*10*/] = iParam2;
	StringCopy(&((uParam0[iParam1 /*10*/])->f_1), sParam3, 24);
	(uParam0[iParam1 /*10*/])->f_7 = 1;
	(uParam0[iParam1 /*10*/])->f_8 = iParam4;
	(uParam0[iParam1 /*10*/])->f_9 = iParam5;
	if (!Global_76833)
	{
		if (!unk_0x5AEB5DDFFAD43CA5(iParam2))
		{
			if ((uParam0[iParam1 /*10*/])->f_8 == 0)
			{
				unk_0x83BBFE5E96C64FBA(iParam2, 0);
			}
			else
			{
				unk_0x83BBFE5E96C64FBA(iParam2, 1);
			}
		}
		if (!unk_0x5AEB5DDFFAD43CA5(iParam2))
		{
			if ((uParam0[iParam1 /*10*/])->f_9 == 0)
			{
				unk_0xE61672E3FEF081AC(iParam2, 0);
			}
			else
			{
				unk_0xE61672E3FEF081AC(iParam2, 1);
			}
		}
	}
}

int func_61()
{
	if (Global_21005 != 0 || unk_0x6A5CC4A2C9D9ED8D())
	{
		return 1;
	}
	return 0;
}

bool func_62(var uParam0, int iParam1)
{
	if (func_31(uParam0, iParam1))
	{
		if (uParam0->f_4173.f_178 && iParam1 == 51)
		{
			uParam0->f_4173.f_178 = 0;
		}
		return 0;
	}
	if (uParam0->f_4173.f_178 && iParam1 == 51)
	{
		unk_0xBE20AB8238688965(&(uParam0->f_4173[(iParam1 / 32)]), (iParam1 % 32));
		uParam0->f_4173.f_178 = 0;
	}
	return unk_0xCE990E643CD9D0E5(uParam0->f_4173[(iParam1 / 32)], (iParam1 % 32));
}

void func_63(var uParam0)
{
	if (uParam0->f_4361)
	{
		unk_0x494D5FF88119CDC0("MPHeist3Planning", 0);
		if (func_186(0, -1, 0) && unk_0xC7F5A2E99D75A65C("MPHeist3Planning"))
		{
			if (!uParam0->f_4362)
			{
				unk_0xC4CC25B68A91D144(-1, "INFO", "HUD_FRONTEND_DEFAULT_SOUNDSET", 1);
				func_185(0, 0);
				func_183(1, "MPHeist3Planning", "HeistDetailsPannel");
				if (func_182(uParam0))
				{
					func_168(uParam0);
				}
				else if (func_167(uParam0))
				{
					func_135(uParam0);
				}
				else if (func_134(uParam0))
				{
					func_130(uParam0);
				}
				else if (func_129(uParam0))
				{
					func_123(uParam0);
				}
				else if (func_122(uParam0))
				{
					func_118(uParam0);
				}
				else if (func_117(uParam0))
				{
					func_108(uParam0);
				}
				uParam0->f_4362 = 1;
			}
			func_67(1, -1, 1, 0, 1, -1082130432, 0, 0, -1);
		}
	}
	else if (uParam0->f_4362)
	{
		func_64(1, -1);
		unk_0x2DDA2C702A92866E("MPHeist3Planning");
		uParam0->f_4362 = 0;
	}
}

void func_64(bool bParam0, int iParam1)
{
	int iVar0;
	
	if (!func_66(&iVar0, 0, iParam1))
	{
		return;
	}
	if (Global_22550.f_8417)
	{
		unk_0x610B4083B1F95914(15);
		Global_22550.f_8417 = 0;
	}
	unk_0x530DC1525CA60B00(0f);
	if (Global_22550.f_5628[iVar0])
	{
		unk_0x5D52AA94798846BB(9, 0);
		Global_22550.f_5628[iVar0] = 0;
	}
	if (Global_22550.f_5614[iVar0])
	{
		unk_0x2DDA2C702A92866E("CommonMenu");
		Global_22550.f_5614[iVar0] = 0;
	}
	if (Global_22550.f_5621[iVar0])
	{
		unk_0x2DDA2C702A92866E("MPShopSale");
		Global_22550.f_5621[iVar0] = 0;
	}
	if (bParam0)
	{
		func_65(&(Global_22550.f_5660[iVar0 /*10*/]));
		Global_22550.f_5721[iVar0] = 0;
	}
	else
	{
		Global_22550.f_5721[iVar0] = 0;
	}
}

void func_65(var uParam0)
{
	if (uParam0->f_9 != 0)
	{
		if (unk_0x95EF219D38B20CFF(*uParam0))
		{
			unk_0x2E352DDBBF674246(uParam0);
		}
		*uParam0 = 0;
		uParam0->f_9 = 0;
	}
}

int func_66(var uParam0, bool bParam1, int iParam2)
{
	char cVar0[64];
	int iVar16;
	int iVar17;
	int iVar18;
	
	if (iParam2 == -1)
	{
		if (unk_0x25DDB354A40FFCDB() && unk_0x82CC259B7B4F4AD9())
		{
			iParam2 = unk_0x1D39C24C852E6DFA();
		}
	}
	StringCopy(&cVar0, unk_0x6C5C6B207AA78253(), 64);
	StringIntConCat(&cVar0, iParam2, 64);
	iVar16 = unk_0x856D5567211886A2(&cVar0);
	iVar18 = -1;
	iVar17 = 0;
	while (iVar17 < 6)
	{
		if (Global_22550.f_5721[iVar17] == iVar16)
		{
			*uParam0 = iVar17;
			return 1;
		}
		else if (Global_22550.f_5721[iVar17] == 0)
		{
			iVar18 = iVar17;
		}
		iVar17++;
	}
	if (bParam1)
	{
		if (iVar18 != -1)
		{
			Global_22550.f_5721[iVar18] = iVar16;
			*uParam0 = iVar18;
			return 1;
		}
	}
	return 0;
}

void func_67(bool bParam0, int iParam1, bool bParam2, bool bParam3, bool bParam4, float fParam5, bool bParam6, bool bParam7, int iParam8)
{
	int iVar0;
	int iVar1;
	int iVar2;
	int iVar3;
	int iVar4;
	int iVar5;
	int iVar6;
	int iVar7;
	int iVar8;
	int iVar9;
	int iVar10;
	int iVar11;
	int iVar12;
	int iVar13;
	int iVar14;
	int iVar15;
	int iVar16;
	int iVar17;
	int iVar18;
	int iVar19;
	int iVar20;
	int iVar21;
	int iVar22;
	int iVar23;
	int iVar24;
	int iVar25;
	int iVar26;
	int iVar27;
	int iVar28;
	int iVar29;
	int iVar30;
	int iVar31;
	bool bVar32;
	bool bVar33;
	float fVar34;
	float fVar35;
	float fVar36;
	float fVar37;
	struct<3> Var38;
	bool bVar41;
	float fVar42;
	float fVar43;
	float fVar44;
	float fVar45;
	float fVar46;
	int iVar47;
	int iVar48;
	int iVar49;
	int iVar50;
	float fVar51;
	bool bVar52;
	bool bVar53;
	bool bVar54;
	bool bVar55;
	float fVar56;
	float fVar57;
	float fVar58;
	float fVar59;
	int iVar60;
	int iVar61;
	float fVar62;
	float fVar63;
	float fVar64;
	char cVar65[64];
	char cVar81[64];
	float fVar97;
	int iVar98;
	float fVar99;
	float fVar100;
	int iVar101;
	int iVar102;
	int iVar103;
	int iVar104;
	int iVar105;
	int iVar106;
	char cVar107[16];
	float fVar111;
	float fVar112;
	float fVar113;
	float fVar114;
	float fVar115;
	
	if (!func_66(&iVar0, 0, iParam1))
	{
		return;
	}
	if (iVar0 == -1)
	{
	}
	if (!func_106(0, bParam6))
	{
		return;
	}
	unk_0xFCC7EAA298CE7AC2(76, 84);
	unk_0xFFA39C4568552522(-0.05f, -0.05f, 0f, 0f);
	fVar57 = 0f;
	fVar58 = 0f;
	if (Global_22550)
	{
		if (func_104(29, 1, 1, &fVar36, &fVar37, bParam7))
		{
			fVar57 = fVar37;
			fVar58 = fVar36;
			fVar57 = (fVar57 + 0f);
		}
		else
		{
			Global_22550 = 0;
		}
	}
	if (unk_0x856D5567211886A2(&(Global_22550.f_1)) == unk_0x856D5567211886A2("HIDE"))
	{
		fVar59 = Global_22548;
	}
	else
	{
		fVar59 = (((Global_22548 + fVar57) + 0.034722f) + 0f);
	}
	if (fParam5 == -1f)
	{
		fParam5 = Global_22549;
	}
	fVar62 = 1f;
	if (bParam7)
	{
		unk_0x9DD549AAA043F83A(&iVar60, &iVar61);
		fVar63 = unk_0x8940B46482DD0A7D(0);
		if (func_103())
		{
			iVar60 = unk_0xF2DB717A73826179((unk_0xBBDA792448DB5A89(iVar61) * fVar63));
		}
		fVar64 = (unk_0xBBDA792448DB5A89(iVar60) / unk_0xBBDA792448DB5A89(iVar61));
		fVar62 = (fVar64 / fVar63);
		if (func_103())
		{
			fVar62 = 1f;
		}
		iVar60 = unk_0xF2DB717A73826179((unk_0xBBDA792448DB5A89(iVar60) / fVar62));
		iVar61 = unk_0xF2DB717A73826179((unk_0xBBDA792448DB5A89(iVar61) / fVar62));
	}
	else
	{
		unk_0x8BAB7B3A2C7777A5(&iVar60, &iVar61);
	}
	if (bParam3)
	{
		if (Global_22550.f_5218 <= 1)
		{
			func_99(Global_22550.f_5218 + 1, "DFLT_MNU_OPT", 0, 1, 0, 0);
			Global_22550.f_6014 = 1;
		}
	}
	iVar5 = 0;
	while (iVar5 < 2)
	{
		if (iVar5 == 1 && Global_22550.f_5728)
		{
			if (unk_0x856D5567211886A2(&(Global_22550.f_1)) == unk_0x856D5567211886A2("HIDE"))
			{
				fVar51 = Global_22548;
			}
			else
			{
				if (Global_22550)
				{
					StringCopy(&cVar65, func_98(29), 64);
					StringCopy(&cVar81, func_95(29, 1), 64);
					if (unk_0x856D5567211886A2(&(Global_22550.f_7029[29 /*16*/])) == joaat("CREW_LOGO"))
					{
						func_94(Global_22547, Global_22548, fParam5, (fVar57 - 0f), 0, 0, 0, 255);
						unk_0x177AA645FB4B3A38(&cVar65, &cVar81, (Global_22547 + (fParam5 * 0.5f)), (Global_22548 + ((fVar57 - 0f) * 0.5f)), fVar58, (fVar57 - 0f), 0f, 255, 255, 255, 255, 0);
					}
					else
					{
						unk_0x177AA645FB4B3A38(&cVar65, &cVar81, (Global_22547 + (fParam5 * 0.5f)), (Global_22548 + ((fVar57 - 0f) * 0.5f)), fParam5, (fVar57 - 0f), 0f, 255, 255, 255, 255, 0);
					}
				}
				if (Global_22550.f_8387)
				{
					iVar1 = Global_22550.f_8383;
					iVar2 = Global_22550.f_8384;
					iVar3 = Global_22550.f_8385;
					iVar4 = Global_22550.f_8386;
				}
				else
				{
					iVar1 = 0;
					iVar2 = 0;
					iVar3 = 0;
					iVar4 = 255;
				}
				func_94(Global_22547, (Global_22548 + fVar57), fParam5, 0.034722f, iVar1, iVar2, iVar3, iVar4);
				fVar51 = (((Global_22548 + fVar57) + 0.034722f) + 0f);
				if (unk_0x856D5567211886A2(&(Global_22550.f_1)) != 0)
				{
					func_93();
					unk_0x40A75E32D30C605F(&(Global_22550.f_1));
					iVar15 = 0;
					iVar16 = 0;
					iVar17 = 0;
					iVar18 = 0;
					iVar14 = 0;
					while (iVar14 < Global_22550.f_68)
					{
						if (Global_22550.f_5[iVar14] == 2)
						{
							unk_0x42710E9E08FA375A(Global_22550.f_10[iVar15]);
							iVar15++;
						}
						else if (Global_22550.f_5[iVar14] == 3)
						{
							unk_0x5BC8B357341A4FEE(Global_22550.f_14[iVar16], Global_22550.f_18[iVar16]);
							iVar16++;
						}
						else if (Global_22550.f_5[iVar14] == 1)
						{
							unk_0x6A826E35A3096ED0(&(Global_22550.f_22[iVar17 /*4*/]));
							iVar17++;
						}
						else if (Global_22550.f_5[iVar14] == 8)
						{
							unk_0x6A826E35A3096ED0(&(Global_22550.f_22[iVar17 /*4*/]));
							iVar17++;
						}
						else if (Global_22550.f_5[iVar14] == 5)
						{
							unk_0x9EFD301E3BE8324E(&(Global_22550.f_35[iVar18 /*16*/]));
							iVar18++;
						}
						else if (Global_22550.f_5[iVar14] == 6)
						{
							unk_0x6A826E35A3096ED0(&(Global_22550.f_35[iVar18 /*16*/]));
							iVar18++;
						}
						else if (Global_22550.f_5[iVar14] == 7)
						{
							unk_0x9EFD301E3BE8324E(&(Global_22550.f_35[iVar18 /*16*/]));
							iVar18++;
						}
						else if (Global_22550.f_5[iVar14] == 9)
						{
							unk_0x9EFD301E3BE8324E(&(Global_22550.f_35[iVar18 /*16*/]));
							iVar18++;
						}
						iVar14++;
					}
					unk_0xA434CC17B5517E4E((Global_22547 + 0.00390625f), ((Global_22548 + fVar57) + 0.00416664f), 0);
				}
				if (Global_22550.f_5735)
				{
					func_93();
					func_91((((Global_22547 + fParam5) - 0.00390625f) - func_92("CM_ITEM_COUNT", Global_22550.f_5736, Global_22550.f_5737)), ((Global_22548 + fVar57) + 0.00416664f), "CM_ITEM_COUNT", Global_22550.f_5736, Global_22550.f_5737);
				}
				else if (Global_22550.f_5731 > Global_22550.f_5225)
				{
					if (Global_22550.f_5734 != 0)
					{
						func_93();
						func_91((((Global_22547 + fParam5) - 0.00390625f) - func_92("CM_ITEM_COUNT", Global_22550.f_5734, Global_22550.f_5733)), ((Global_22548 + fVar57) + 0.00416664f), "CM_ITEM_COUNT", Global_22550.f_5734, Global_22550.f_5733);
					}
				}
			}
			iVar6 = Global_22550.f_5738;
			iVar9 = 0;
			fVar97 = fVar51;
			if (Global_22550.f_8397)
			{
				iVar1 = Global_22550.f_8393;
				iVar2 = Global_22550.f_8394;
				iVar3 = Global_22550.f_8395;
				iVar4 = Global_22550.f_8396;
			}
			else
			{
				unk_0x28DD484BAEAF8437(140, &iVar1, &iVar2, &iVar3, &iVar4);
			}
			while (iVar9 < Global_22550.f_5225 && iVar6 <= Global_22550.f_5218)
			{
				if (iVar6 >= 0)
				{
					if (Global_22550.f_5485[iVar6])
					{
						if (Global_22550.f_5356[iVar6] && iVar6 != Global_22550.f_5738)
						{
							fVar51 = (fVar51 + 0.00277776f);
						}
						fVar56 = 0.034722f;
						if (Global_22550.f_5745[iVar6] != 0f)
						{
							fVar56 = Global_22550.f_5745[iVar6];
						}
						fVar51 = (fVar51 + fVar56);
						iVar9++;
					}
				}
				iVar6++;
			}
			unk_0x177AA645FB4B3A38("CommonMenu", "Gradient_Bgd", (Global_22547 + (fParam5 * 0.5f)), ((fVar97 + ((fVar51 - fVar97) * 0.5f)) - 0.00138888f), fParam5, (fVar51 - fVar97), 0f, 255, 255, 255, 255, 0);
			if (Global_22550.f_5731 > Global_22550.f_5225)
			{
				if (Global_22550.f_8402)
				{
					iVar1 = Global_22550.f_8398;
					iVar2 = Global_22550.f_8399;
					iVar3 = Global_22550.f_8400;
					iVar4 = Global_22550.f_8401;
				}
				else
				{
					iVar1 = 0;
					iVar2 = 0;
					iVar3 = 0;
					iVar4 = 204;
				}
				func_94(Global_22547, (fVar51 + 0f), fParam5, 0.034722f, iVar1, iVar2, iVar3, iVar4);
				Var38 = { unk_0x282B86763E4DCFC4("CommonMenu", "shop_arrows_upANDdown") };
				Var38.f_0 = (Var38.f_0 * (0.5f / fVar62));
				Var38.f_1 = (Var38.f_1 * (0.5f / fVar62));
				if (Global_22550.f_8415)
				{
					iVar1 = 0;
					iVar2 = 0;
					iVar3 = 0;
					iVar4 = 255;
				}
				else
				{
					unk_0x28DD484BAEAF8437(1, &iVar1, &iVar2, &iVar3, &iVar4);
				}
				unk_0x177AA645FB4B3A38("CommonMenu", "shop_arrows_upANDdown", (Global_22547 + (fParam5 * 0.5f)), ((fVar51 + 0f) + (0.034722f * 0.5f)), ((Var38.f_0 / 1280f) * fVar62), ((Var38.f_1 / 720f) * fVar62), 0f, iVar1, iVar2, iVar3, iVar4, 0);
				fVar51 = (fVar51 + (0f + 0.034722f));
			}
			if (unk_0x856D5567211886A2(&(Global_22550.f_4690)) != 0 && Global_22550.f_4766 != -1)
			{
				fVar51 = (fVar51 + (0.00277776f * 2f));
				fVar42 = (Global_22547 + 0.0046875f);
				if (Global_22550.f_4768 != 0)
				{
					func_104(Global_22550.f_4768, 1, 1, &fVar36, &fVar37, bParam7);
					fVar42 = (((Global_22547 + fVar36) + (0.00078125f * 4f)) - (0.00078125f * 1f));
				}
				func_90(fVar42);
				unk_0x989558F5DCFEFA52(&(Global_22550.f_4690));
				iVar15 = 0;
				iVar16 = 0;
				iVar17 = 0;
				iVar14 = 0;
				while (iVar14 < Global_22550.f_4762)
				{
					if (Global_22550.f_4696[iVar14] == 2)
					{
						unk_0x42710E9E08FA375A(Global_22550.f_4701[iVar15]);
						iVar15++;
					}
					else if (Global_22550.f_4696[iVar14] == 3)
					{
						unk_0x5BC8B357341A4FEE(Global_22550.f_4705[iVar16], Global_22550.f_4709[iVar16]);
						iVar16++;
					}
					else if (Global_22550.f_4696[iVar14] == 1)
					{
						unk_0x6A826E35A3096ED0(&(Global_22550.f_4713[iVar17 /*16*/]));
						iVar17++;
					}
					else if (Global_22550.f_4696[iVar14] == 5)
					{
						unk_0x9EFD301E3BE8324E(&(Global_22550.f_4713[iVar17 /*16*/]));
						iVar17++;
					}
					else if (Global_22550.f_4696[iVar14] == 6)
					{
						unk_0x6A826E35A3096ED0(&(Global_22550.f_4713[iVar17 /*16*/]));
						iVar17++;
					}
					else if (Global_22550.f_4696[iVar14] == 7)
					{
						unk_0x9EFD301E3BE8324E(&(Global_22550.f_4713[iVar17 /*16*/]));
						iVar17++;
					}
					else if (Global_22550.f_4696[iVar14] == 9)
					{
						unk_0x9EFD301E3BE8324E(&(Global_22550.f_4713[iVar17 /*16*/]));
						iVar17++;
					}
					iVar14++;
				}
				iVar6 = unk_0x76C71C9B09716523(fVar42, (fVar51 + 0.00277776f));
				unk_0x28DD484BAEAF8437(2, &iVar1, &iVar2, &iVar3, &iVar4);
				func_94(Global_22547, (fVar51 - 0.00277776f), fParam5, 0.00277776f, iVar1, iVar2, iVar3, iVar4);
				if (Global_22550.f_8407)
				{
					iVar1 = Global_22550.f_8403;
					iVar2 = Global_22550.f_8404;
					iVar3 = Global_22550.f_8405;
					iVar4 = Global_22550.f_8406;
				}
				else
				{
					unk_0x28DD484BAEAF8437(140, &iVar1, &iVar2, &iVar3, &iVar4);
				}
				unk_0x177AA645FB4B3A38("CommonMenu", "Gradient_Bgd", (Global_22547 + (fParam5 * 0.5f)), ((fVar51 + ((((unk_0xBA269EB4693A8C47(0.35f, 0) * IntToFloat(iVar6)) + (0.00138888f * 13f)) + ((0.00138888f * 5f) * IntToFloat((iVar6 - 1)))) * 0.5f)) - 0.00138888f), fParam5, (((unk_0xBA269EB4693A8C47(0.35f, 0) * IntToFloat(iVar6)) + (0.00138888f * 13f)) + ((0.00138888f * 5f) * IntToFloat((iVar6 - 1)))), 0f, iVar1, iVar2, iVar3, iVar4, 0);
				func_90(fVar42);
				unk_0x40A75E32D30C605F(&(Global_22550.f_4690));
				iVar15 = 0;
				iVar16 = 0;
				iVar17 = 0;
				iVar14 = 0;
				while (iVar14 < Global_22550.f_4762)
				{
					if (Global_22550.f_4696[iVar14] == 2)
					{
						unk_0x42710E9E08FA375A(Global_22550.f_4701[iVar15]);
						iVar15++;
					}
					else if (Global_22550.f_4696[iVar14] == 3)
					{
						unk_0x5BC8B357341A4FEE(Global_22550.f_4705[iVar16], Global_22550.f_4709[iVar16]);
						iVar16++;
					}
					else if (Global_22550.f_4696[iVar14] == 1)
					{
						unk_0x6A826E35A3096ED0(&(Global_22550.f_4713[iVar17 /*16*/]));
						iVar17++;
					}
					else if (Global_22550.f_4696[iVar14] == 5)
					{
						unk_0x9EFD301E3BE8324E(&(Global_22550.f_4713[iVar17 /*16*/]));
						iVar17++;
					}
					else if (Global_22550.f_4696[iVar14] == 6)
					{
						unk_0x6A826E35A3096ED0(&(Global_22550.f_4713[iVar17 /*16*/]));
						iVar17++;
					}
					else if (Global_22550.f_4696[iVar14] == 7)
					{
						unk_0x9EFD301E3BE8324E(&(Global_22550.f_4713[iVar17 /*16*/]));
						iVar17++;
					}
					else if (Global_22550.f_4696[iVar14] == 9)
					{
						unk_0x9EFD301E3BE8324E(&(Global_22550.f_4713[iVar17 /*16*/]));
						iVar17++;
					}
					else if (Global_22550.f_4696[iVar14] == 8)
					{
						unk_0x6A826E35A3096ED0(&(Global_22550.f_4713[iVar17 /*16*/]));
						iVar17++;
					}
					iVar14++;
				}
				unk_0xA434CC17B5517E4E(fVar42, (fVar51 + 0.00277776f), 0);
				if (Global_22550.f_4768 != 0)
				{
					func_104(Global_22550.f_4768, 1, 1, &fVar36, &fVar37, bParam7);
					func_89(Global_22550.f_4768, 1, &iVar47, &iVar48, &iVar49, &iVar50);
					unk_0x177AA645FB4B3A38(func_98(Global_22550.f_4768), func_95(Global_22550.f_4768, 1), ((Global_22547 + (fVar36 * 0.5f)) + (0.00078125f * 2f)), ((fVar51 + (fVar37 * 0.5f)) - (0.00138888f * 4f)), fVar36, fVar37, 0f, iVar47, iVar48, iVar49, iVar50, 0);
				}
				fVar51 = (fVar51 + (((unk_0xBA269EB4693A8C47(0.35f, 0) * IntToFloat(iVar6)) + (0.00138888f * 13f)) + ((0.00138888f * 5f) * IntToFloat((iVar6 - 1)))));
				if (Global_22550.f_4766 > 0)
				{
					if ((unk_0x578C4EF320340AF7() - Global_22550.f_4767) > Global_22550.f_4766)
					{
						StringCopy(&(Global_22550.f_4690), "", 24);
						Global_22550.f_4766 = -1;
					}
				}
			}
			if (unk_0x856D5567211886A2(&(Global_4269680.f_21)) != 0 && Global_4269680.f_65 != -1)
			{
				fVar51 = (fVar51 + (0.00277776f * 2f));
				fVar42 = (Global_22547 + 0.0046875f);
				if (Global_4269680.f_67 != 0)
				{
					func_104(Global_4269680.f_67, 1, 1, &fVar36, &fVar37, bParam7);
					fVar42 = (((Global_22547 + fVar36) + (0.00078125f * 4f)) - (0.00078125f * 1f));
				}
				func_90(fVar42);
				unk_0x989558F5DCFEFA52(&(Global_4269680.f_21));
				iVar15 = 0;
				iVar16 = 0;
				iVar17 = 0;
				iVar14 = 0;
				while (iVar14 < Global_4269680.f_61)
				{
					if (Global_4269680.f_25[iVar14] == 2)
					{
						unk_0x42710E9E08FA375A(Global_4269680.f_30[iVar15]);
						iVar15++;
					}
					else if (Global_4269680.f_25[iVar14] == 3)
					{
						unk_0x5BC8B357341A4FEE(Global_4269680.f_34[iVar16], Global_4269680.f_38[iVar16]);
						iVar16++;
					}
					else if (Global_4269680.f_25[iVar14] == 1)
					{
						unk_0x6A826E35A3096ED0(&(Global_4269680.f_42[iVar17 /*6*/]));
						iVar17++;
					}
					else if (Global_4269680.f_25[iVar14] == 5)
					{
						unk_0x9EFD301E3BE8324E(&(Global_4269680.f_42[iVar17 /*6*/]));
						iVar17++;
					}
					else if (Global_4269680.f_25[iVar14] == 6)
					{
						unk_0x6A826E35A3096ED0(&(Global_4269680.f_42[iVar17 /*6*/]));
						iVar17++;
					}
					else if (Global_4269680.f_25[iVar14] == 7)
					{
						unk_0x9EFD301E3BE8324E(&(Global_4269680.f_42[iVar17 /*6*/]));
						iVar17++;
					}
					else if (Global_4269680.f_25[iVar14] == 9)
					{
						unk_0x9EFD301E3BE8324E(&(Global_4269680.f_42[iVar17 /*6*/]));
						iVar17++;
					}
					else if (Global_4269680.f_25[iVar14] == 8)
					{
						unk_0x6A826E35A3096ED0(&(Global_4269680.f_42[iVar17 /*6*/]));
						iVar17++;
					}
					iVar14++;
				}
				iVar6 = unk_0x76C71C9B09716523(fVar42, (fVar51 + 0.00277776f));
				unk_0x28DD484BAEAF8437(2, &iVar1, &iVar2, &iVar3, &iVar4);
				func_94(Global_22547, (fVar51 - 0.00277776f), fParam5, 0.00277776f, iVar1, iVar2, iVar3, iVar4);
				if (Global_22550.f_8407)
				{
					iVar1 = Global_22550.f_8403;
					iVar2 = Global_22550.f_8404;
					iVar3 = Global_22550.f_8405;
					iVar4 = Global_22550.f_8406;
				}
				else
				{
					unk_0x28DD484BAEAF8437(140, &iVar1, &iVar2, &iVar3, &iVar4);
				}
				unk_0x177AA645FB4B3A38("CommonMenu", "Gradient_Bgd", (Global_22547 + (fParam5 * 0.5f)), ((fVar51 + ((((unk_0xBA269EB4693A8C47(0.35f, 0) * IntToFloat(iVar6)) + (0.00138888f * 13f)) + ((0.00138888f * 5f) * IntToFloat((iVar6 - 1)))) * 0.5f)) - 0.00138888f), fParam5, (((unk_0xBA269EB4693A8C47(0.35f, 0) * IntToFloat(iVar6)) + (0.00138888f * 13f)) + ((0.00138888f * 5f) * IntToFloat((iVar6 - 1)))), 0f, iVar1, iVar2, iVar3, iVar4, 0);
				func_90(fVar42);
				unk_0x40A75E32D30C605F(&(Global_4269680.f_21));
				iVar15 = 0;
				iVar16 = 0;
				iVar17 = 0;
				iVar14 = 0;
				while (iVar14 < Global_4269680.f_61)
				{
					if (Global_4269680.f_25[iVar14] == 2)
					{
						unk_0x42710E9E08FA375A(Global_4269680.f_30[iVar15]);
						iVar15++;
					}
					else if (Global_4269680.f_25[iVar14] == 3)
					{
						unk_0x5BC8B357341A4FEE(Global_4269680.f_34[iVar16], Global_4269680.f_38[iVar16]);
						iVar16++;
					}
					else if (Global_4269680.f_25[iVar14] == 1)
					{
						unk_0x6A826E35A3096ED0(&(Global_4269680.f_42[iVar17 /*6*/]));
						iVar17++;
					}
					else if (Global_4269680.f_25[iVar14] == 8)
					{
						unk_0x6A826E35A3096ED0(&(Global_4269680.f_42[iVar17 /*6*/]));
						iVar17++;
					}
					else if (Global_4269680.f_25[iVar14] == 5)
					{
						unk_0x9EFD301E3BE8324E(&(Global_4269680.f_42[iVar17 /*6*/]));
						iVar17++;
					}
					else if (Global_4269680.f_25[iVar14] == 6)
					{
						unk_0x6A826E35A3096ED0(&(Global_4269680.f_42[iVar17 /*6*/]));
						iVar17++;
					}
					else if (Global_4269680.f_25[iVar14] == 7)
					{
						unk_0x9EFD301E3BE8324E(&(Global_4269680.f_42[iVar17 /*6*/]));
						iVar17++;
					}
					else if (Global_4269680.f_25[iVar14] == 9)
					{
						unk_0x9EFD301E3BE8324E(&(Global_4269680.f_42[iVar17 /*6*/]));
						iVar17++;
					}
					iVar14++;
				}
				unk_0xA434CC17B5517E4E(fVar42, (fVar51 + 0.00277776f), 0);
				if (Global_4269680.f_67 != 0)
				{
					func_104(Global_4269680.f_67, 1, 1, &fVar36, &fVar37, bParam7);
					func_89(Global_4269680.f_67, 1, &iVar47, &iVar48, &iVar49, &iVar50);
					unk_0x177AA645FB4B3A38(func_98(Global_4269680.f_67), func_95(Global_4269680.f_67, 1), ((Global_22547 + (fVar36 * 0.5f)) + (0.00078125f * 2f)), ((fVar51 + (fVar37 * 0.5f)) - (0.00138888f * 4f)), fVar36, fVar37, 0f, iVar47, iVar48, iVar49, iVar50, 0);
				}
				fVar51 = (fVar51 + (((unk_0xBA269EB4693A8C47(0.35f, 0) * IntToFloat(iVar6)) + (0.00138888f * 13f)) + ((0.00138888f * 5f) * IntToFloat((iVar6 - 1)))));
				if (Global_4269680.f_65 > 0)
				{
					if ((unk_0x578C4EF320340AF7() - Global_4269680.f_66) > Global_4269680.f_65)
					{
						StringCopy(&(Global_4269680.f_21), "", 16);
						Global_4269680.f_65 = -1;
					}
				}
			}
			func_82(iVar60, iParam1, iParam8, 0, 0, 0, bParam4, 1, 0);
			unk_0xFCC7EAA298CE7AC2(76, 84);
			unk_0xFFA39C4568552522(-0.05f, -0.05f, 0f, 0f);
		}
		if (iVar5 == 1 || !Global_22550.f_5728)
		{
			iVar19 = 0;
			iVar23 = 0;
			iVar20 = 0;
			iVar21 = 0;
			iVar22 = 0;
			iVar9 = 0;
			iVar10 = 0;
			iVar11 = 0;
			iVar12 = 0;
			iVar13 = 0;
			iVar98 = Global_22550.f_5218;
			if (Global_22550.f_5729)
			{
				iVar98 = (Global_22550.f_5732 - 1);
			}
			fVar99 = 0f;
			fVar100 = 0f;
			iVar7 = 0;
			while (iVar7 <= iVar98)
			{
				fVar56 = 0.034722f;
				if (Global_22550.f_5745[iVar6] != 0f)
				{
					fVar56 = Global_22550.f_5745[iVar6];
				}
				if (Global_22550.f_5729)
				{
					iVar6 = Global_22550.f_8038[iVar7];
				}
				else
				{
					iVar6 = iVar7;
				}
				iVar12 = iVar13;
				bVar33 = false;
				if (iVar6 >= Global_22550.f_5738 && iVar9 < Global_22550.f_5225)
				{
					bVar33 = true;
					if (Global_22550.f_5739 == iVar6)
					{
						fVar100 = fVar99;
					}
					if (Global_22550.f_5356[iVar6])
					{
						iVar12++;
					}
					fVar35 = (((fVar59 + fVar99) + (0.00277776f * IntToFloat(iVar12))) + 0.00277776f);
				}
				Global_22550.f_5879[iVar6] = fVar35;
				fVar34 = (Global_22547 + 0.0046875f);
				bVar41 = false;
				bVar32 = Global_22550.f_5739 == iVar6;
				if ((bVar32 && iVar5 == 1) && bVar33)
				{
					iVar102 = 255;
					iVar103 = 255;
					iVar104 = 255;
					iVar105 = 255;
					if (Global_22550.f_8409)
					{
						unk_0x28DD484BAEAF8437(Global_22550.f_8408, &iVar102, &iVar103, &iVar104, &iVar105);
					}
					else
					{
						unk_0x28DD484BAEAF8437(1, &iVar102, &iVar103, &iVar104, &iVar105);
					}
					unk_0x177AA645FB4B3A38("CommonMenu", "Gradient_Nav", (Global_22547 + (fParam5 * 0.5f)), (((fVar59 + fVar100) + (0.00277776f * IntToFloat(iVar12))) + (fVar56 * 0.5f)), fParam5, fVar56, 0f, iVar102, iVar103, iVar104, iVar105, 0);
					Global_22550.f_5877 = fVar35;
				}
				iVar8 = 0;
				while (iVar8 < Global_22550.f_5226)
				{
					if (unk_0xCE990E643CD9D0E5(Global_22550.f_5089[iVar6], iVar8) || Global_22550.f_5056[iVar8] == 5)
					{
						if (Global_22550.f_5729)
						{
							iVar19 = Global_22550.f_8054[((iVar9 * Global_22550.f_5226) + iVar8)];
							iVar20 = Global_22550.f_8095[((iVar9 * Global_22550.f_5226) + iVar8)];
							iVar21 = Global_22550.f_8136[((iVar9 * Global_22550.f_5226) + iVar8)];
							iVar22 = Global_22550.f_8177[((iVar9 * Global_22550.f_5226) + iVar8)];
							iVar23 = Global_22550.f_8218[((iVar9 * Global_22550.f_5226) + iVar8)];
						}
						else
						{
							Global_22550.f_8054[((iVar9 * Global_22550.f_5226) + iVar8)] = iVar19;
							Global_22550.f_8095[((iVar9 * Global_22550.f_5226) + iVar8)] = iVar20;
							Global_22550.f_8136[((iVar9 * Global_22550.f_5226) + iVar8)] = iVar21;
							Global_22550.f_8177[((iVar9 * Global_22550.f_5226) + iVar8)] = iVar22;
							Global_22550.f_8218[((iVar9 * Global_22550.f_5226) + iVar8)] = iVar23;
						}
						iVar106 = 0;
						bVar55 = false;
						if (Global_22550.f_6011[0] != -1)
						{
							if ((iVar6 * 5 + iVar8) == Global_22550.f_6008[0])
							{
								bVar55 = true;
								iVar106 = 0;
							}
						}
						if (Global_22550.f_6011[1] != -1)
						{
							if ((iVar6 * 5 + iVar8) == Global_22550.f_6008[1])
							{
								bVar55 = true;
								iVar106 = 1;
							}
						}
						if (Global_22550.f_5062[iVar8] != -1f)
						{
							fVar34 = ((Global_22547 + 0.0046875f) + Global_22550.f_5062[iVar8]);
						}
						if ((iVar8 < 4 && Global_22550.f_5062[iVar8 + 1] != -1f) && fVar34 < Global_22550.f_5062[iVar8 + 1])
						{
							fVar46 = (Global_22550.f_5062[iVar8 + 1] - fVar34);
						}
						else
						{
							fVar46 = (((Global_22547 + Global_22549) - 0.0046875f) - fVar34);
						}
						if ((Global_22550.f_5075[iVar8] && Global_22550.f_5874) && bVar32)
						{
							bVar54 = true;
						}
						else
						{
							bVar54 = false;
						}
						switch (Global_22550.f_5056[iVar8])
						{
							case 0:
								break;
							
							case 1:
								iVar24 = iVar19;
								if (bVar33)
								{
									if (!Global_22550.f_5729)
									{
										fVar43 = 0f;
										fVar44 = 0f;
										iVar25 = 0;
										iVar26 = 0;
										iVar27 = 0;
										iVar28 = 0;
										iVar29 = 0;
										if (unk_0x856D5567211886A2(&(Global_22550.f_73[iVar24 /*6*/])) != 0)
										{
											bVar52 = false;
											bVar53 = false;
											iVar14 = 0;
											while (iVar14 < 4)
											{
												if (Global_22550.f_2124[iVar24 /*5*/][iVar14] == 5 || Global_22550.f_2124[iVar24 /*5*/][iVar14] == 8)
												{
													bVar53 = true;
												}
												else if (Global_22550.f_2124[iVar24 /*5*/][iVar14] == 9)
												{
													bVar52 = true;
												}
												iVar14++;
											}
											func_80(bVar32, Global_22550.f_1610[iVar24], Global_22550.f_1867[iVar24], bVar55, iVar106, bVar53, bVar52);
											unk_0x456829A212A690D7(&(Global_22550.f_73[iVar24 /*6*/]));
										}
										iVar14 = 0;
										while (iVar14 < 4)
										{
											if (Global_22550.f_2124[iVar24 /*5*/][iVar14] == 1)
											{
												iVar25++;
												if (unk_0x856D5567211886A2(&(Global_22550.f_73[iVar24 /*6*/])) != 0)
												{
													unk_0x6A826E35A3096ED0(&(Global_22550.f_73[(iVar24 + iVar25) /*6*/]));
												}
											}
											else if (Global_22550.f_2124[iVar24 /*5*/][iVar14] == 8)
											{
												iVar25++;
												if (unk_0x856D5567211886A2(&(Global_22550.f_73[iVar24 /*6*/])) != 0)
												{
													unk_0x6A826E35A3096ED0(&(Global_22550.f_73[(iVar24 + iVar25) /*6*/]));
												}
											}
											else if (Global_22550.f_2124[iVar24 /*5*/][iVar14] == 5)
											{
												if (unk_0x856D5567211886A2(&(Global_22550.f_73[iVar24 /*6*/])) != 0)
												{
													unk_0x9EFD301E3BE8324E(&(Global_2463019[(iVar23 + iVar29) /*16*/]));
												}
												iVar29++;
											}
											else if (Global_22550.f_2124[iVar24 /*5*/][iVar14] == 6)
											{
												if (unk_0x856D5567211886A2(&(Global_22550.f_73[iVar24 /*6*/])) != 0)
												{
													unk_0x6A826E35A3096ED0(&(Global_2463019[(iVar23 + iVar29) /*16*/]));
												}
												iVar29++;
											}
											else if (Global_22550.f_2124[iVar24 /*5*/][iVar14] == 7)
											{
												if (unk_0x856D5567211886A2(&(Global_22550.f_73[iVar24 /*6*/])) != 0)
												{
													unk_0x9EFD301E3BE8324E(&(Global_2463019[(iVar23 + iVar29) /*16*/]));
												}
												iVar29++;
											}
											else if (Global_22550.f_2124[iVar24 /*5*/][iVar14] == 9)
											{
												if (unk_0x856D5567211886A2(&(Global_22550.f_73[iVar24 /*6*/])) != 0)
												{
													unk_0x9EFD301E3BE8324E(&(Global_2463019[(iVar23 + iVar29) /*16*/]));
												}
												iVar29++;
											}
											else if (Global_22550.f_2124[iVar24 /*5*/][iVar14] == 2)
											{
												if (unk_0x856D5567211886A2(&(Global_22550.f_73[iVar24 /*6*/])) != 0)
												{
													unk_0x42710E9E08FA375A(Global_22550.f_3918[(iVar20 + iVar26)]);
												}
												iVar26++;
											}
											else if (Global_22550.f_2124[iVar24 /*5*/][iVar14] == 3)
											{
												if (unk_0x856D5567211886A2(&(Global_22550.f_73[iVar24 /*6*/])) != 0)
												{
													unk_0x5BC8B357341A4FEE(Global_22550.f_4175[(iVar21 + iVar27)], Global_22550.f_4304[(iVar21 + iVar27)]);
												}
												iVar27++;
											}
											else if (Global_22550.f_2124[iVar24 /*5*/][iVar14] == 4)
											{
												iVar28++;
											}
											iVar14++;
										}
										if (unk_0x856D5567211886A2(&(Global_22550.f_73[iVar24 /*6*/])) != 0)
										{
											fVar43 = unk_0x842CB5919E31EB1F(1);
										}
										if (iVar28 > 0)
										{
											iVar14 = 0;
											while (iVar14 < iVar28)
											{
												if (func_104(Global_22550.f_4433[(iVar22 + iVar14)], bVar32, 0, &fVar36, &fVar37, bParam7))
												{
													fVar44 = (fVar44 + fVar36);
													if (iVar14 > 0)
													{
														fVar44 = (fVar44 - (0.00078125f * 4f));
													}
													if ((Global_22550.f_4433[(iVar22 + iVar14)] == 2 || Global_22550.f_4433[(iVar22 + iVar14)] == 51) || Global_22550.f_4433[(iVar22 + iVar14)] == 61)
													{
														fVar44 = (fVar44 - (0.00078125f * 5f));
													}
												}
												iVar14++;
											}
										}
										fVar42 = 0f;
										if (Global_22550.f_5083[iVar8] == 2)
										{
											fVar42 = (fVar42 + ((fVar46 - (fVar43 + fVar44)) + (0.00078125f * 1f)));
										}
										else if (Global_22550.f_5083[iVar8] == 0)
										{
											fVar42 = (fVar42 + (((fVar46 - fVar34) * 0.5f) - ((fVar43 + fVar44) * 0.5f)));
										}
										Global_22550.f_8259[((iVar9 * Global_22550.f_5226) + iVar8)] = fVar42;
										Global_22550.f_8300[((iVar9 * Global_22550.f_5226) + iVar8)] = fVar43;
										Global_22550.f_8341[((iVar9 * Global_22550.f_5226) + iVar8)] = fVar44;
										if (Global_22550.f_5083[iVar8] == 2)
										{
											iVar101 = (iVar8 - 1);
											while (iVar101 >= 0)
											{
												if (Global_22550.f_5083[iVar101] == 2)
												{
													Global_22550.f_8259[((iVar9 * Global_22550.f_5226) + iVar101)] = (Global_22550.f_8259[((iVar9 * Global_22550.f_5226) + iVar101)] - Global_22550.f_5068[iVar8]);
												}
												iVar101 = (iVar101 + -1);
											}
										}
									}
									else
									{
										fVar42 = Global_22550.f_8259[((iVar9 * Global_22550.f_5226) + iVar8)];
										fVar43 = Global_22550.f_8300[((iVar9 * Global_22550.f_5226) + iVar8)];
										fVar44 = Global_22550.f_8341[((iVar9 * Global_22550.f_5226) + iVar8)];
									}
									if (bVar54)
									{
										if (func_104(26, 1, 0, &fVar36, &fVar37, bParam7))
										{
											if (Global_22550.f_5083[iVar8] == 2)
											{
												fVar42 = (fVar42 - (fVar36 * 2f));
											}
											fVar45 = (fVar36 * 0.5f);
											if (func_104(26, 1, 1, &fVar36, &fVar37, bParam7))
											{
												func_89(26, 1, &iVar47, &iVar48, &iVar49, &iVar50);
												if (iVar5 == 1)
												{
													unk_0x177AA645FB4B3A38(func_98(26), func_95(26, 1), ((fVar34 + fVar42) + fVar45), ((fVar35 - 0.00277776f) + (fVar56 * 0.5f)), fVar36, fVar37, 0f, iVar47, iVar48, iVar49, iVar50, 0);
												}
											}
										}
										if (func_104(27, 1, 0, &fVar36, &fVar37, bParam7))
										{
											fVar42 = (fVar42 + fVar36);
											fVar45 = (fVar36 * 0.5f);
											if (func_104(27, 1, 1, &fVar36, &fVar37, bParam7))
											{
												func_89(27, 1, &iVar47, &iVar48, &iVar49, &iVar50);
												if (iVar5 == 1)
												{
													unk_0x177AA645FB4B3A38(func_98(27), func_95(27, 1), (((fVar34 + fVar42) + fVar45) + (fVar43 + fVar44)), ((fVar35 - 0.00277776f) + (fVar56 * 0.5f)), fVar36, fVar37, 0f, iVar47, iVar48, iVar49, iVar50, 0);
												}
											}
										}
									}
									iVar25 = 0;
									iVar26 = 0;
									iVar27 = 0;
									iVar28 = 0;
									iVar29 = 0;
									iVar31 = 0;
									if (unk_0x856D5567211886A2(&(Global_22550.f_73[iVar24 /*6*/])) != 0 && iVar5 == 1)
									{
										bVar52 = false;
										bVar53 = false;
										iVar14 = 0;
										while (iVar14 < 4)
										{
											if (Global_22550.f_2124[iVar24 /*5*/][iVar14] == 5 || Global_22550.f_2124[iVar24 /*5*/][iVar14] == 8)
											{
												bVar53 = true;
											}
											else if (Global_22550.f_2124[iVar24 /*5*/][iVar14] == 9)
											{
												bVar52 = true;
											}
											iVar14++;
										}
										func_80(bVar32, Global_22550.f_1610[iVar24], Global_22550.f_1867[iVar24], bVar55, 0, bVar53, bVar52);
										if (Global_22550.f_8413 && Global_22550.f_8414 == iVar6)
										{
											func_79(bVar32);
										}
										unk_0x40A75E32D30C605F(&(Global_22550.f_73[iVar24 /*6*/]));
									}
									iVar14 = 0;
									while (iVar14 < 4)
									{
										if (Global_22550.f_2124[iVar24 /*5*/][iVar14] == 1)
										{
											iVar25++;
											if (unk_0x856D5567211886A2(&(Global_22550.f_73[iVar24 /*6*/])) != 0 && iVar5 == 1)
											{
												unk_0x6A826E35A3096ED0(&(Global_22550.f_73[(iVar24 + iVar25) /*6*/]));
											}
											iVar31 = 1;
										}
										else if (Global_22550.f_2124[iVar24 /*5*/][iVar14] == 8)
										{
											iVar25++;
											if (unk_0x856D5567211886A2(&(Global_22550.f_73[iVar24 /*6*/])) != 0 && iVar5 == 1)
											{
												unk_0x6A826E35A3096ED0(&(Global_22550.f_73[(iVar24 + iVar25) /*6*/]));
											}
											iVar31 = 8;
										}
										else if (Global_22550.f_2124[iVar24 /*5*/][iVar14] == 5)
										{
											if (unk_0x856D5567211886A2(&(Global_22550.f_73[iVar24 /*6*/])) != 0 && iVar5 == 1)
											{
												unk_0x9EFD301E3BE8324E(&(Global_2463019[(iVar23 + iVar29) /*16*/]));
											}
											iVar29++;
											iVar31 = 5;
										}
										else if (Global_22550.f_2124[iVar24 /*5*/][iVar14] == 6)
										{
											if (unk_0x856D5567211886A2(&(Global_22550.f_73[iVar24 /*6*/])) != 0 && iVar5 == 1)
											{
												unk_0x6A826E35A3096ED0(&(Global_2463019[(iVar23 + iVar29) /*16*/]));
											}
											iVar29++;
											iVar31 = 6;
										}
										else if (Global_22550.f_2124[iVar24 /*5*/][iVar14] == 7)
										{
											if (unk_0x856D5567211886A2(&(Global_22550.f_73[iVar24 /*6*/])) != 0 && iVar5 == 1)
											{
												unk_0x9EFD301E3BE8324E(&(Global_2463019[(iVar23 + iVar29) /*16*/]));
											}
											iVar29++;
											iVar31 = 7;
										}
										else if (Global_22550.f_2124[iVar24 /*5*/][iVar14] == 9)
										{
											if (unk_0x856D5567211886A2(&(Global_22550.f_73[iVar24 /*6*/])) != 0 && iVar5 == 1)
											{
												unk_0x9EFD301E3BE8324E(&(Global_2463019[(iVar23 + iVar29) /*16*/]));
											}
											iVar29++;
											iVar31 = 9;
										}
										else if (Global_22550.f_2124[iVar24 /*5*/][iVar14] == 2)
										{
											if (unk_0x856D5567211886A2(&(Global_22550.f_73[iVar24 /*6*/])) != 0 && iVar5 == 1)
											{
												unk_0x42710E9E08FA375A(Global_22550.f_3918[(iVar20 + iVar26)]);
											}
											iVar26++;
											iVar31 = 2;
										}
										else if (Global_22550.f_2124[iVar24 /*5*/][iVar14] == 3)
										{
											if (unk_0x856D5567211886A2(&(Global_22550.f_73[iVar24 /*6*/])) != 0 && iVar5 == 1)
											{
												unk_0x5BC8B357341A4FEE(Global_22550.f_4175[(iVar21 + iVar27)], Global_22550.f_4304[(iVar21 + iVar27)]);
											}
											iVar27++;
											iVar31 = 3;
										}
										else if (Global_22550.f_2124[iVar24 /*5*/][iVar14] == 4)
										{
											if ((Global_22550.f_4433[(iVar22 + iVar28)] == 2 || Global_22550.f_4433[(iVar22 + iVar28)] == 51) || Global_22550.f_4433[(iVar22 + iVar28)] == 61)
											{
												if (func_104(Global_22550.f_4433[(iVar22 + iVar28)], bVar32, 0, &fVar36, &fVar37, bParam7))
												{
													fVar42 = (fVar42 + (fVar36 * 0.5f));
													if (func_104(Global_22550.f_4433[(iVar22 + iVar28)], bVar32, 1, &fVar36, &fVar37, bParam7))
													{
														func_89(Global_22550.f_4433[(iVar22 + iVar28)], bVar32, &iVar47, &iVar48, &iVar49, &iVar50);
														if (iVar5 == 1)
														{
															if (Global_22550.f_5083[iVar8] == 2)
															{
																unk_0x177AA645FB4B3A38(func_98(Global_22550.f_4433[(iVar22 + iVar28)]), func_95(Global_22550.f_4433[(iVar22 + iVar28)], bVar32), (((fVar34 + fVar42) - (0.00078125f * 8f)) + (0.00078125f * 4f)), ((fVar35 - 0.00277776f) + (fVar56 * 0.5f)), fVar36, fVar37, 0f, iVar47, iVar48, iVar49, iVar50, 0);
															}
															else
															{
																unk_0x177AA645FB4B3A38(func_98(Global_22550.f_4433[(iVar22 + iVar28)]), func_95(Global_22550.f_4433[(iVar22 + iVar28)], bVar32), ((fVar34 + fVar42) - (0.00078125f * 8f)), ((fVar35 - 0.00277776f) + (fVar56 * 0.5f)), fVar36, fVar37, 0f, iVar47, iVar48, iVar49, iVar50, 0);
															}
														}
														fVar42 = (fVar42 + (0.00078125f * 3f));
													}
												}
											}
											iVar28++;
											iVar31 = 4;
										}
										iVar14++;
									}
									if (unk_0x856D5567211886A2(&(Global_22550.f_73[iVar24 /*6*/])) != 0 && iVar5 == 1)
									{
										if (iVar31 == 4 && Global_22550.f_5083[iVar8] == 2)
										{
											unk_0xA434CC17B5517E4E(((fVar34 + fVar42) + (0.00078125f * 7f)), fVar35, 0);
										}
										else
										{
											unk_0xA434CC17B5517E4E((fVar34 + fVar42), fVar35, 0);
											if (func_78() && unk_0x7B70881748D166CD(joaat("fm_mission_creator")) > 0)
											{
												if (iVar8 == 0)
												{
													func_80(0, Global_22550.f_1610[iVar24], Global_22550.f_1867[iVar24], bVar55, 0, bVar53, bVar52);
													StringCopy(&cVar107, "TEST_LABEL", 16);
													fVar111 = 0f;
													fVar112 = 55f;
													fVar113 = 0.0185f;
													fVar114 = 0.004f;
													fVar115 = 0.02f;
													unk_0x151E2F6EE4A34CFE(0f, (0.35f * 0.7f));
													unk_0xA11D279671F2289C(255, 255, 255, 150);
													unk_0x8DDE075026DB7EFD((fVar34 - (fVar115 * 0.6f)), (fVar35 + (fVar113 * 0.75f)), 0.0175f, 0.035f, unk_0xF34EE736CF047844(fVar111), unk_0xF34EE736CF047844(fVar111), unk_0xF34EE736CF047844(fVar111), unk_0xF34EE736CF047844(fVar112), 0);
													unk_0x40A75E32D30C605F(&cVar107);
													unk_0x42710E9E08FA375A((Global_22550.f_5738 + iVar30));
													unk_0xA434CC17B5517E4E((fVar34 - fVar115), (fVar35 + fVar114), 0);
													iVar30++;
												}
											}
										}
									}
									if (iVar28 > 0)
									{
										fVar42 = (fVar42 + (6f * 0.00078125f));
										iVar14 = 0;
										while (iVar14 < iVar28)
										{
											if ((Global_22550.f_4433[(iVar22 + iVar14)] != 2 && Global_22550.f_4433[(iVar22 + iVar14)] != 51) && Global_22550.f_4433[(iVar22 + iVar14)] != 61)
											{
												if (func_104(Global_22550.f_4433[(iVar22 + iVar14)], bVar32, 0, &fVar36, &fVar37, bParam7))
												{
													fVar42 = (fVar42 + (fVar36 * 0.5f));
													if (iVar5 == 1)
													{
														if (func_104(Global_22550.f_4433[(iVar22 + iVar14)], bVar32, 1, &fVar36, &fVar37, bParam7))
														{
															func_89(Global_22550.f_4433[(iVar22 + iVar14)], bVar32, &iVar47, &iVar48, &iVar49, &iVar50);
															if (Global_22550.f_4433[(iVar22 + iVar14)] == 30)
															{
																unk_0x177AA645FB4B3A38(func_98(Global_22550.f_4433[(iVar22 + iVar14)]), func_95(Global_22550.f_4433[(iVar22 + iVar14)], bVar32), (Global_22547 + (fVar36 * 0.5f)), (((fVar35 + 0.00277776f) + (fVar37 * 0.5f)) - (0.00078125f * 11f)), fVar36, fVar37, 0f, iVar47, iVar48, iVar49, iVar50, 0);
															}
															else if (Global_22550.f_5083[iVar8] == 2)
															{
																unk_0x177AA645FB4B3A38(func_98(Global_22550.f_4433[(iVar22 + iVar14)]), func_95(Global_22550.f_4433[(iVar22 + iVar14)], bVar32), ((((fVar34 + fVar42) + fVar43) - (0.00078125f * 8f)) + (0.00078125f * 4f)), ((fVar35 - 0.00277776f) + (fVar56 * 0.5f)), fVar36, fVar37, 0f, iVar47, iVar48, iVar49, iVar50, 0);
															}
															else
															{
																unk_0x177AA645FB4B3A38(func_98(Global_22550.f_4433[(iVar22 + iVar14)]), func_95(Global_22550.f_4433[(iVar22 + iVar14)], bVar32), (((fVar34 + fVar42) + fVar43) - (0.00078125f * 12f)), ((fVar35 - 0.00277776f) + (fVar56 * 0.5f)), fVar36, fVar37, 0f, iVar47, iVar48, iVar49, iVar50, 0);
															}
														}
													}
													fVar42 = (fVar42 + (12f * 0.00078125f));
												}
											}
											iVar14++;
										}
									}
								}
								bVar41 = true;
								iVar19++;
								iVar14 = 0;
								while (iVar14 < 4)
								{
									if (Global_22550.f_2124[iVar24 /*5*/][iVar14] == 1)
									{
										iVar19++;
									}
									else if (Global_22550.f_2124[iVar24 /*5*/][iVar14] == 8)
									{
										iVar19++;
									}
									else if (Global_22550.f_2124[iVar24 /*5*/][iVar14] == 2)
									{
										iVar20++;
									}
									else if (Global_22550.f_2124[iVar24 /*5*/][iVar14] == 3)
									{
										iVar21++;
									}
									else if (Global_22550.f_2124[iVar24 /*5*/][iVar14] == 4)
									{
										iVar22++;
									}
									else if (Global_22550.f_2124[iVar24 /*5*/][iVar14] == 5)
									{
										iVar23++;
									}
									else if (Global_22550.f_2124[iVar24 /*5*/][iVar14] == 6)
									{
										iVar23++;
									}
									else if (Global_22550.f_2124[iVar24 /*5*/][iVar14] == 7)
									{
										iVar23++;
									}
									else if (Global_22550.f_2124[iVar24 /*5*/][iVar14] == 9)
									{
										iVar23++;
									}
									iVar14++;
								}
								break;
							
							case 2:
								if (bVar33)
								{
									if (!Global_22550.f_5729)
									{
										func_80(bVar32, Global_22550.f_1610[iVar24], Global_22550.f_1867[iVar24], bVar55, 0, 0, 0);
										if (Global_22550.f_8413 && Global_22550.f_8414 == iVar6)
										{
											func_79(bVar32);
										}
										unk_0x456829A212A690D7("NUMBER");
										unk_0x42710E9E08FA375A(Global_22550.f_3918[iVar20]);
										fVar43 = unk_0x842CB5919E31EB1F(1);
										fVar42 = 0f;
										if (Global_22550.f_5083[iVar8] == 2)
										{
											fVar42 = (fVar42 + ((fVar46 - fVar43) + (0.00078125f * 1f)));
										}
										else if (Global_22550.f_5083[iVar8] == 0)
										{
											fVar42 = (fVar42 + (((fVar46 - fVar34) * 0.5f) - (fVar43 * 0.5f)));
										}
										Global_22550.f_8259[((iVar9 * Global_22550.f_5226) + iVar8)] = fVar42;
										Global_22550.f_8300[((iVar9 * Global_22550.f_5226) + iVar8)] = fVar43;
									}
									else
									{
										fVar42 = Global_22550.f_8259[((iVar9 * Global_22550.f_5226) + iVar8)];
										fVar43 = Global_22550.f_8300[((iVar9 * Global_22550.f_5226) + iVar8)];
									}
									if (bVar54)
									{
										if (func_104(26, 1, 0, &fVar36, &fVar37, bParam7))
										{
											if (Global_22550.f_5083[iVar8] == 2)
											{
												fVar42 = (fVar42 - (fVar36 * 2f));
											}
											fVar45 = (fVar36 * 0.5f);
											if (func_104(26, 1, 1, &fVar36, &fVar37, bParam7))
											{
												func_89(26, 1, &iVar47, &iVar48, &iVar49, &iVar50);
												if (iVar5 == 1)
												{
													unk_0x177AA645FB4B3A38(func_98(26), func_95(26, 1), ((fVar34 + fVar42) + fVar45), ((fVar35 - 0.00277776f) + (fVar56 * 0.5f)), fVar36, fVar37, 0f, iVar47, iVar48, iVar49, iVar50, 0);
												}
											}
										}
										if (func_104(27, 1, 0, &fVar36, &fVar37, bParam7))
										{
											fVar42 = (fVar42 + fVar36);
											fVar45 = (fVar36 * 0.5f);
											if (func_104(27, 1, 1, &fVar36, &fVar37, bParam7))
											{
												func_89(27, 1, &iVar47, &iVar48, &iVar49, &iVar50);
												if (iVar5 == 1)
												{
													unk_0x177AA645FB4B3A38(func_98(27), func_95(27, 1), (((fVar34 + fVar42) + fVar45) + (fVar43 + fVar44)), ((fVar35 - 0.00277776f) + (fVar56 * 0.5f)), fVar36, fVar37, 0f, iVar47, iVar48, iVar49, iVar50, 0);
												}
											}
										}
									}
									if (iVar5 == 1)
									{
										func_80(bVar32, Global_22550.f_1610[iVar24], Global_22550.f_1867[iVar24], bVar55, 0, 0, 0);
										func_77((fVar34 + fVar42), fVar35, "NUMBER", Global_22550.f_3918[iVar20], 0);
									}
								}
								bVar41 = true;
								iVar20++;
								break;
							
							case 3:
								if (bVar33)
								{
									if (!Global_22550.f_5729)
									{
										func_80(bVar32, Global_22550.f_1610[iVar24], Global_22550.f_1867[iVar24], bVar55, 0, 0, 0);
										if (Global_22550.f_8413 && Global_22550.f_8414 == iVar6)
										{
											func_79(bVar32);
										}
										unk_0x456829A212A690D7("NUMBER");
										unk_0x5BC8B357341A4FEE(Global_22550.f_4175[iVar21], Global_22550.f_4304[iVar21]);
										fVar43 = unk_0x842CB5919E31EB1F(1);
										fVar42 = 0f;
										if (Global_22550.f_5083[iVar8] == 2)
										{
											fVar42 = (fVar42 + ((fVar46 - fVar43) + (0.00078125f * 1f)));
										}
										else if (Global_22550.f_5083[iVar8] == 0)
										{
											fVar42 = (fVar42 + (((fVar46 - fVar34) * 0.5f) - (fVar43 * 0.5f)));
										}
										Global_22550.f_8259[((iVar9 * Global_22550.f_5226) + iVar8)] = fVar42;
										Global_22550.f_8300[((iVar9 * Global_22550.f_5226) + iVar8)] = fVar43;
									}
									else
									{
										fVar42 = Global_22550.f_8259[((iVar9 * Global_22550.f_5226) + iVar8)];
										fVar43 = Global_22550.f_8300[((iVar9 * Global_22550.f_5226) + iVar8)];
									}
									if (bVar54)
									{
										if (func_104(26, 1, 0, &fVar36, &fVar37, 0))
										{
											if (Global_22550.f_5083[iVar8] == 2)
											{
												fVar42 = (fVar42 - (fVar36 * 2f));
											}
											fVar45 = (fVar36 * 0.5f);
											if (func_104(26, 1, 1, &fVar36, &fVar37, bParam7))
											{
												func_89(26, 1, &iVar47, &iVar48, &iVar49, &iVar50);
												if (iVar5 == 1)
												{
													unk_0x177AA645FB4B3A38(func_98(26), func_95(26, 1), ((fVar34 + fVar42) + fVar45), ((fVar35 - 0.00277776f) + (fVar56 * 0.5f)), fVar36, fVar37, 0f, iVar47, iVar48, iVar49, iVar50, 0);
												}
											}
										}
										if (func_104(27, 1, 0, &fVar36, &fVar37, bParam7))
										{
											fVar42 = (fVar42 + fVar36);
											fVar45 = (fVar36 * 0.5f);
											if (func_104(27, 1, 1, &fVar36, &fVar37, bParam7))
											{
												func_89(27, 1, &iVar47, &iVar48, &iVar49, &iVar50);
												if (iVar5 == 1)
												{
													unk_0x177AA645FB4B3A38(func_98(27), func_95(27, 1), (((fVar34 + fVar42) + fVar45) + (fVar43 + fVar44)), ((fVar35 - 0.00277776f) + (fVar56 * 0.5f)), fVar36, fVar37, 0f, iVar47, iVar48, iVar49, iVar50, 0);
												}
											}
										}
									}
									func_80(bVar32, Global_22550.f_1610[iVar24], Global_22550.f_1867[iVar24], bVar55, 0, 0, 0);
									func_76((fVar34 + fVar42), fVar35, "NUMBER", Global_22550.f_4175[iVar21], Global_22550.f_4304[iVar21]);
								}
								bVar41 = true;
								iVar21++;
								break;
							
							case 4:
								if (bVar33)
								{
									if (func_104(Global_22550.f_4433[iVar22], bVar32, 0, &fVar36, &fVar37, bParam7))
									{
										if (!Global_22550.f_5729)
										{
											fVar44 = fVar36;
											fVar42 = 0f;
											if (Global_22550.f_5083[iVar8] == 2)
											{
												fVar42 = (fVar42 + ((fVar46 - fVar44) + (0.00078125f * 1f)));
											}
											else if (Global_22550.f_5083[iVar8] == 0)
											{
												fVar42 = (fVar42 + (((fVar46 - fVar34) * 0.5f) - (fVar44 * 0.5f)));
											}
											Global_22550.f_8259[((iVar9 * Global_22550.f_5226) + iVar8)] = fVar42;
											Global_22550.f_8341[((iVar9 * Global_22550.f_5226) + iVar8)] = fVar44;
										}
										else
										{
											fVar42 = Global_22550.f_8259[((iVar9 * Global_22550.f_5226) + iVar8)];
											fVar44 = Global_22550.f_8341[((iVar9 * Global_22550.f_5226) + iVar8)];
										}
										if (bVar54)
										{
											if (func_104(26, 1, 0, &fVar36, &fVar37, bParam7))
											{
												if (Global_22550.f_5083[iVar8] == 2)
												{
													fVar42 = (fVar42 - (fVar36 * 2f));
												}
												fVar45 = (fVar36 * 0.5f);
												if (func_104(26, 1, 1, &fVar36, &fVar37, bParam7))
												{
													func_89(26, 1, &iVar47, &iVar48, &iVar49, &iVar50);
													if (iVar5 == 1)
													{
														unk_0x177AA645FB4B3A38(func_98(26), func_95(26, 1), ((fVar34 + fVar42) + fVar45), ((fVar35 - 0.00277776f) + (fVar56 * 0.5f)), fVar36, fVar37, 0f, iVar47, iVar48, iVar49, iVar50, 0);
													}
												}
											}
											if (func_104(27, 1, 0, &fVar36, &fVar37, bParam7))
											{
												fVar42 = (fVar42 + fVar36);
												fVar45 = (fVar36 * 0.5f);
												if (func_104(27, 1, 1, &fVar36, &fVar37, bParam7))
												{
													func_89(27, 1, &iVar47, &iVar48, &iVar49, &iVar50);
													if (iVar5 == 1)
													{
														unk_0x177AA645FB4B3A38(func_98(27), func_95(27, 1), (((fVar34 + fVar42) + fVar45) + (fVar43 + fVar44)), ((fVar35 - 0.00277776f) + (fVar56 * 0.5f)), fVar36, fVar37, 0f, iVar47, iVar48, iVar49, iVar50, 0);
													}
												}
											}
										}
										if (iVar5 == 1)
										{
											if (func_104(Global_22550.f_4433[iVar22], bVar32, 1, &fVar36, &fVar37, bParam7))
											{
												func_89(Global_22550.f_4433[iVar22], bVar32, &iVar47, &iVar48, &iVar49, &iVar50);
												unk_0x177AA645FB4B3A38(func_98(Global_22550.f_4433[iVar22]), func_95(Global_22550.f_4433[iVar22], bVar32), ((fVar34 + fVar42) + (fVar36 * 0.5f)), ((fVar35 - 0.00277776f) + (fVar56 * 0.5f)), (fVar36 * func_75(Global_22550.f_4433[iVar22])), (fVar37 * func_75(Global_22550.f_4433[iVar22])), 0f, iVar47, iVar48, iVar49, iVar50, 0);
											}
										}
									}
								}
								bVar41 = true;
								iVar22++;
								break;
							
							case 5:
								bVar41 = true;
								break;
						}
						if (Global_22550.f_5056[iVar8] == 5)
						{
							if (Global_22550.f_5068[iVar8] > 0.05f)
							{
								fVar34 = (fVar34 + Global_22550.f_5068[iVar8]);
							}
							else
							{
								fVar34 = (fVar34 + 0.05f);
							}
						}
						else
						{
							fVar34 = (fVar34 + Global_22550.f_5068[iVar8]);
							if (Global_22550.f_5075[iVar8])
							{
								if (func_104(26, 1, 1, &fVar36, &fVar37, bParam7))
								{
									fVar34 = (fVar34 - fVar36);
								}
							}
						}
					}
					else
					{
						fVar34 = (fVar34 + Global_22550.f_5068[iVar8]);
					}
					iVar8++;
				}
				if (bVar41)
				{
					if (bVar33)
					{
						Global_22550.f_8038[iVar9] = iVar6;
						Global_22550.f_5740 = iVar6;
						iVar9++;
						if (Global_22550.f_5356[iVar6])
						{
							iVar13++;
						}
						if (Global_22550.f_5745[iVar6] != 0f)
						{
							fVar99 = (fVar99 + Global_22550.f_5745[iVar6]);
						}
						else
						{
							fVar99 = (fVar99 + 0.034722f);
						}
					}
					if (!Global_22550.f_5728)
					{
						Global_22550.f_5485[iVar6] = 1;
						if (Global_22550.f_5227[iVar6])
						{
							if (bVar32)
							{
								Global_22550.f_5734 = 0;
							}
						}
						else
						{
							iVar11++;
							if (bVar32)
							{
								Global_22550.f_5734 = iVar11;
							}
						}
						iVar10++;
					}
				}
				iVar7++;
			}
			if (!Global_22550.f_5728)
			{
				Global_22550.f_5730 = ((fVar59 + fVar99) + (0.00277776f * IntToFloat(iVar12)));
				Global_22550.f_5733 = iVar11;
				Global_22550.f_5731 = iVar10;
				Global_22550.f_5728 = 1;
			}
		}
		if (!Global_22550.f_5729)
		{
			Global_22550.f_5732 = iVar9;
			Global_22550.f_5729 = 1;
		}
		iVar5++;
	}
	Global_22550.f_5876 = fVar51;
	Global_22550.f_5878 = unk_0x578C4EF320340AF7();
	unk_0x530DC1525CA60B00(Global_22550.f_5876);
	if (!Global_22550.f_8382)
	{
		func_69(0);
	}
	Global_22550.f_8382 = 0;
	if (bParam2)
	{
		unk_0x4CB4237D8858ADA6(10);
	}
	unk_0x4CB4237D8858ADA6(6);
	unk_0x4CB4237D8858ADA6(7);
	unk_0x4CB4237D8858ADA6(9);
	unk_0x4CB4237D8858ADA6(8);
	if (bParam0)
	{
		func_68(1);
	}
	unk_0x5C64FD27DB62AF7A();
}

void func_68(int iParam0)
{
	Global_1378744.f_1121 = iParam0;
}

void func_69(int iParam0)
{
	if (func_74())
	{
		return;
	}
	if (!Global_19681.f_1 == 1)
	{
		if (func_73(0))
		{
			func_70(iParam0);
		}
		unk_0xBE20AB8238688965(&Global_7552, 2);
	}
}

void func_70(int iParam0)
{
	if (func_74())
	{
		return;
	}
	if (Global_19864)
	{
		if (func_72())
		{
			func_71(1, 1);
		}
		else
		{
			func_71(0, 0);
		}
	}
	if (Global_19681.f_1 == 10 || Global_19681.f_1 == 9)
	{
		unk_0xBE20AB8238688965(&Global_7552, 16);
	}
	if (unk_0x6B12213471F330A3())
	{
		unk_0xBE4122AC23440E7D(0);
	}
	Global_21005 = 5;
	if (iParam0 == 1)
	{
		unk_0xBE20AB8238688965(&Global_7551, 30);
	}
	else
	{
		unk_0xD2459066EA58CE43(&Global_7551, 30);
	}
	if (!func_45())
	{
		Global_19681.f_1 = 3;
	}
}

void func_71(bool bParam0, bool bParam1)
{
	if (bParam0)
	{
		if (func_73(0))
		{
			Global_19864 = 1;
			if (bParam1)
			{
				unk_0x1F1B1269BB6217F7(&Global_19618);
			}
			Global_19609 = { Global_19627[Global_19626 /*3*/] };
			unk_0x241AFEE3D5B4FF70(Global_19609);
		}
	}
	else if (Global_19864 == 1)
	{
		Global_19864 = 0;
		Global_19609 = { Global_19634[Global_19626 /*3*/] };
		if (bParam1)
		{
			unk_0x241AFEE3D5B4FF70(Global_19618);
		}
		else
		{
			unk_0x241AFEE3D5B4FF70(Global_19609);
		}
	}
}

bool func_72()
{
	return unk_0xCE990E643CD9D0E5(Global_1689733, 5);
}

int func_73(int iParam0)
{
	if (iParam0 == 1)
	{
		if (Global_19681.f_1 > 3)
		{
			if (unk_0xCE990E643CD9D0E5(Global_7551, 14))
			{
				return 1;
			}
			else
			{
				return 0;
			}
		}
		else
		{
			return 0;
		}
	}
	if (unk_0x7B70881748D166CD(joaat("cellphone_flashhand")) > 0)
	{
		return 1;
	}
	if (Global_19681.f_1 > 3)
	{
		return 1;
	}
	return 0;
}

bool func_74()
{
	return unk_0xCE990E643CD9D0E5(Global_1689733, 19);
}

float func_75(int iParam0)
{
	switch (iParam0)
	{
		case 35:
		case 34:
		case 48:
		case 47:
		case 42:
		case 36:
		case 37:
		case 39:
		case 40:
		case 38:
		case 53:
		case 44:
		case 45:
		case 46:
			return 0.85f;
			break;
	}
	return 1f;
}

void func_76(float fParam0, float fParam1, char* sParam2, var uParam3, var uParam4)
{
	unk_0x40A75E32D30C605F(sParam2);
	unk_0x5BC8B357341A4FEE(uParam3, uParam4);
	unk_0xA434CC17B5517E4E(fParam0, fParam1, 0);
}

void func_77(float fParam0, float fParam1, char* sParam2, int iParam3, int iParam4)
{
	unk_0x40A75E32D30C605F(sParam2);
	unk_0x42710E9E08FA375A(iParam3);
	unk_0xA434CC17B5517E4E(fParam0, fParam1, iParam4);
}

var func_78()
{
	return unk_0x95C5B896CB42B20E(-1762644250);
}

void func_79(bool bParam0)
{
	int iVar0;
	int iVar1;
	int iVar2;
	var uVar3;
	
	if (bParam0)
	{
		unk_0x28DD484BAEAF8437(Global_22550.f_8410[0], &iVar0, &iVar1, &iVar2, &uVar3);
	}
	else
	{
		unk_0x28DD484BAEAF8437(Global_22550.f_8410[1], &iVar0, &iVar1, &iVar2, &uVar3);
	}
	unk_0xA11D279671F2289C(iVar0, iVar1, iVar2, 255);
}

void func_80(bool bParam0, bool bParam1, bool bParam2, bool bParam3, int iParam4, bool bParam5, bool bParam6)
{
	int iVar0;
	int iVar1;
	int iVar2;
	int iVar3;
	
	if (bParam2)
	{
		if (bParam3)
		{
			func_81(Global_22550.f_6011[iParam4], &iVar0, &iVar1, &iVar2);
			if ((iVar0 < 20 && iVar1 < 20) && iVar2 < 20)
			{
				if (bParam0 == 0)
				{
					unk_0x28DD484BAEAF8437(1, &iVar0, &iVar1, &iVar2, &iVar3);
				}
			}
			else if ((iVar0 > 230 && iVar1 > 230) && iVar2 > 230)
			{
				if (bParam0)
				{
					iVar0 = 0;
					iVar1 = 0;
					iVar2 = 0;
				}
			}
			unk_0xA11D279671F2289C(iVar0, iVar1, iVar2, 255);
		}
		else if (bParam1)
		{
			if (bParam0)
			{
				unk_0x28DD484BAEAF8437(14, &iVar0, &iVar1, &iVar2, &iVar3);
				unk_0xA11D279671F2289C(iVar0, iVar1, iVar2, 255);
			}
			else
			{
				unk_0x28DD484BAEAF8437(12, &iVar0, &iVar1, &iVar2, &iVar3);
				unk_0xA11D279671F2289C(iVar0, iVar1, iVar2, 255);
			}
		}
		else if (bParam0)
		{
			unk_0xA11D279671F2289C(155, 155, 155, 255);
		}
		else
		{
			unk_0xA11D279671F2289C(155, 155, 155, 255);
		}
	}
	else if (bParam1)
	{
		if (bParam0)
		{
			unk_0xA11D279671F2289C(0, 0, 0, unk_0xF34EE736CF047844((255f * 0.8f)));
		}
		else
		{
			unk_0x28DD484BAEAF8437(1, &iVar0, &iVar1, &iVar2, &iVar3);
			unk_0xA11D279671F2289C(iVar0, iVar1, iVar2, iVar3);
		}
	}
	else if (bParam0)
	{
		unk_0xA11D279671F2289C(155, 155, 155, 255);
	}
	else
	{
		unk_0xA11D279671F2289C(155, 155, 155, 255);
	}
	unk_0x151E2F6EE4A34CFE(0f, 0.35f);
	unk_0xBC2BD09DB91678C5(1);
	if (bParam5)
	{
		unk_0x151E2F6EE4A34CFE(0f, 0.425f);
		unk_0xE9FEE5ECCAD48813(4);
	}
	else if (bParam6)
	{
		unk_0x151E2F6EE4A34CFE(0f, 0.425f);
		unk_0xE9FEE5ECCAD48813(6);
	}
	else
	{
		unk_0xE9FEE5ECCAD48813(0);
	}
	unk_0x86520D0C56F68D3F(0f, 1f);
	unk_0x9CB19F83A6C8E47F(0);
	unk_0xACADD4647896AEBE(0, 0, 0, 0, 0);
	unk_0xA5C56390CC2323FE(0, 0, 0, 0, 0);
}

void func_81(int iParam0, var uParam1, var uParam2, var uParam3)
{
	switch (iParam0)
	{
		case 0:
			*uParam1 = 8;
			*uParam2 = 8;
			*uParam3 = 8;
			break;
		
		case 1:
			*uParam1 = 37;
			*uParam2 = 37;
			*uParam3 = 39;
			break;
		
		case 22:
			*uParam1 = 140;
			*uParam2 = 146;
			*uParam3 = 154;
			break;
		
		case 23:
			*uParam1 = 91;
			*uParam2 = 93;
			*uParam3 = 94;
			break;
		
		case 6:
			*uParam1 = 81;
			*uParam2 = 84;
			*uParam3 = 89;
			break;
		
		case 111:
			*uParam1 = 240;
			*uParam2 = 240;
			*uParam3 = 240;
			break;
		
		case 28:
			*uParam1 = 150;
			*uParam2 = 8;
			*uParam3 = 0;
			break;
		
		case 34:
			*uParam1 = 38;
			*uParam2 = 3;
			*uParam3 = 6;
			break;
		
		case 88:
			*uParam1 = 245;
			*uParam2 = 137;
			*uParam3 = 15;
			break;
		
		case 45:
			*uParam1 = 74;
			*uParam2 = 22;
			*uParam3 = 7;
			break;
		
		case 56:
			*uParam1 = 45;
			*uParam2 = 58;
			*uParam3 = 53;
			break;
		
		case 58:
			*uParam1 = 71;
			*uParam2 = 120;
			*uParam3 = 60;
			break;
		
		case 54:
			*uParam1 = 77;
			*uParam2 = 98;
			*uParam3 = 104;
			break;
		
		case 73:
			*uParam1 = 14;
			*uParam2 = 49;
			*uParam3 = 109;
			break;
		
		case 68:
			*uParam1 = 22;
			*uParam2 = 34;
			*uParam3 = 72;
			break;
		
		case 140:
			*uParam1 = 0;
			*uParam2 = 174;
			*uParam3 = 239;
			break;
		
		case 131:
			*uParam1 = 255;
			*uParam2 = 183;
			*uParam3 = 0;
			break;
		
		case 90:
			*uParam1 = 142;
			*uParam2 = 140;
			*uParam3 = 70;
			break;
		
		case 97:
			*uParam1 = 156;
			*uParam2 = 141;
			*uParam3 = 113;
			break;
		
		case 89:
			*uParam1 = 145;
			*uParam2 = 115;
			*uParam3 = 71;
			break;
		
		case 105:
			*uParam1 = 98;
			*uParam2 = 68;
			*uParam3 = 40;
			break;
		
		case 100:
			*uParam1 = 124;
			*uParam2 = 27;
			*uParam3 = 68;
			break;
		
		case 99:
			*uParam1 = 114;
			*uParam2 = 42;
			*uParam3 = 63;
			break;
		
		case 136:
			*uParam1 = 246;
			*uParam2 = 151;
			*uParam3 = 153;
			break;
		
		case 49:
			*uParam1 = 32;
			*uParam2 = 32;
			*uParam3 = 44;
			break;
		
		case 146:
			*uParam1 = 26;
			*uParam2 = 1;
			*uParam3 = 23;
			break;
		
		default:
			*uParam1 = 255;
			*uParam2 = 255;
			*uParam3 = 255;
			break;
	}
}

void func_82(int iParam0, int iParam1, int iParam2, int iParam3, bool bParam4, bool bParam5, bool bParam6, int iParam7, bool bParam8)
{
	int iVar0;
	int iVar1;
	int iVar2;
	int iVar3;
	int iVar4;
	
	if (!func_66(&iVar0, 0, iParam1))
	{
		return;
	}
	iParam0 = iParam0;
	if (iParam3 && !func_106(bParam4, bParam8))
	{
		return;
	}
	if (func_87())
	{
		return;
	}
	if (unk_0xDFAB5E5CBE16E8B1())
	{
		return;
	}
	if (iParam7 == 0)
	{
		if (func_85(unk_0x460153A63B9477BC(), 0))
		{
			return;
		}
	}
	if (unk_0xF8EDFF98A9C94C74())
	{
		if (unk_0x913D69ADE739309E() == 0 || unk_0xDFAB5E5CBE16E8B1())
		{
			return;
		}
	}
	if (Global_22550.f_4769 != 0)
	{
		if (unk_0x628AF533B14EF310(2))
		{
			iVar1 = 0;
			while (iVar1 < Global_22550.f_4769)
			{
				if (Global_22550.f_5026[iVar1] != 361)
				{
					StringCopy(&(Global_22550.f_4771[iVar1 /*16*/]), unk_0xED026A379B794F61(2, Global_22550.f_5026[iVar1], 1), 64);
				}
				else if (Global_22550.f_5039[iVar1] != 32)
				{
					StringCopy(&(Global_22550.f_4771[iVar1 /*16*/]), unk_0x32F8D6499042A233(2, Global_22550.f_5039[iVar1], 1), 64);
				}
				iVar1++;
			}
			Global_22550.f_4770 = 0;
		}
		if (!Global_22550.f_4770)
		{
			unk_0x830F007E19C63E14(Global_22550.f_5660[iVar0 /*10*/], "CLEAR_ALL");
			unk_0xE1FDD153F5858534();
			unk_0x830F007E19C63E14(Global_22550.f_5660[iVar0 /*10*/], "SET_MAX_WIDTH");
			unk_0x49B9B8101B158AEE((1f - (Global_22550.f_5081 / 100f)));
			unk_0xE1FDD153F5858534();
			if (unk_0xF8EDFF98A9C94C74())
			{
				unk_0x830F007E19C63E14(Global_22550.f_5660[iVar0 /*10*/], "TOGGLE_MOUSE_BUTTONS");
				unk_0xF1B28F753235CE2A(1);
				unk_0xE1FDD153F5858534();
			}
			iVar1 = 0;
			while (iVar1 < Global_22550.f_4769)
			{
				if (unk_0x856D5567211886A2(&(Global_22550.f_4964[iVar1 /*4*/])) != unk_0x856D5567211886A2("PREV"))
				{
					unk_0x830F007E19C63E14(Global_22550.f_5660[iVar0 /*10*/], "SET_DATA_SLOT");
					unk_0xAD291B8F75D737AD(iVar1);
					func_84(&(Global_22550.f_4771[iVar1 /*16*/]));
					iVar2 = iVar1 + 1;
					while (iVar2 < 12 && unk_0x856D5567211886A2(&(Global_22550.f_4964[iVar2 /*4*/])) == unk_0x856D5567211886A2("PREV"))
					{
						func_84(&(Global_22550.f_4771[iVar2 /*16*/]));
						iVar2++;
					}
					if (Global_22550.f_5013[iVar1] == -1)
					{
						func_83(&(Global_22550.f_4964[iVar1 /*4*/]));
					}
					else
					{
						iVar3 = Global_22550.f_5013[iVar1];
						if (iParam2 >= 0)
						{
							iVar3 = iParam2;
						}
						unk_0xB23270F3D5E62FDE(&(Global_22550.f_4964[iVar1 /*4*/]));
						if (bParam5)
						{
							unk_0x7899A951E85F4B28(iVar3, 70);
						}
						else
						{
							unk_0x42710E9E08FA375A(iVar3);
						}
						unk_0x2042E9CA4306F725();
					}
					if (unk_0xF8EDFF98A9C94C74())
					{
						if (Global_22550.f_5026[iVar1] != 361 && unk_0xCE990E643CD9D0E5(Global_22550.f_5052, iVar1))
						{
							unk_0xF1B28F753235CE2A(1);
							unk_0xAD291B8F75D737AD(Global_22550.f_5026[iVar1]);
						}
						else
						{
							unk_0xF1B28F753235CE2A(0);
							unk_0xAD291B8F75D737AD(361);
						}
					}
					unk_0xE1FDD153F5858534();
				}
				iVar1++;
			}
			if (unk_0x856D5567211886A2(&(Global_4269680.f_16)) != unk_0x856D5567211886A2(""))
			{
				unk_0x830F007E19C63E14(Global_22550.f_5660[iVar0 /*10*/], "SET_DATA_SLOT");
				unk_0xAD291B8F75D737AD(Global_22550.f_4769);
				func_84(&Global_4269680);
				if (Global_4269680.f_20 == -1)
				{
					func_83(&(Global_4269680.f_16));
				}
				else
				{
					iVar4 = Global_22550.f_5013[iVar1];
					if (iParam2 >= 0)
					{
						iVar4 = iParam2;
					}
					unk_0xB23270F3D5E62FDE(&(Global_4269680.f_16));
					if (bParam5)
					{
						unk_0x7899A951E85F4B28(iVar4, 70);
					}
					else
					{
						unk_0x42710E9E08FA375A(iVar4);
					}
					unk_0x2042E9CA4306F725();
				}
				unk_0xE1FDD153F5858534();
			}
			unk_0x830F007E19C63E14(Global_22550.f_5660[iVar0 /*10*/], "SET_BACKGROUND_COLOUR");
			unk_0xAD291B8F75D737AD(0);
			unk_0xAD291B8F75D737AD(0);
			unk_0xAD291B8F75D737AD(0);
			unk_0xAD291B8F75D737AD(80);
			unk_0xE1FDD153F5858534();
			unk_0x830F007E19C63E14(Global_22550.f_5660[iVar0 /*10*/], "DRAW_INSTRUCTIONAL_BUTTONS");
			if (Global_22550.f_5082)
			{
				unk_0xAD291B8F75D737AD(1);
			}
			else
			{
				unk_0xAD291B8F75D737AD(0);
			}
			unk_0xE1FDD153F5858534();
			Global_22550.f_4770 = 1;
		}
		iVar1 = 0;
		while (iVar1 < Global_22550.f_4769)
		{
			if (Global_22550.f_5013[iVar1] != -1)
			{
				if (iParam2 > 0)
				{
					unk_0x830F007E19C63E14(Global_22550.f_5660[iVar0 /*10*/], "OVERRIDE_RESPAWN_TEXT");
					unk_0xAD291B8F75D737AD(iVar1);
					unk_0xB23270F3D5E62FDE(&(Global_22550.f_4964[iVar1 /*4*/]));
					if (bParam5)
					{
						unk_0x7899A951E85F4B28(iParam2, 70);
					}
					else
					{
						unk_0x42710E9E08FA375A(iParam2);
					}
					unk_0x2042E9CA4306F725();
					unk_0xE1FDD153F5858534();
				}
			}
			iVar1++;
		}
		if (Global_4269680.f_20 != -1)
		{
			if (iParam2 > 0)
			{
				unk_0x830F007E19C63E14(Global_22550.f_5660[iVar0 /*10*/], "OVERRIDE_RESPAWN_TEXT");
				unk_0xAD291B8F75D737AD(iVar1);
				unk_0xB23270F3D5E62FDE(&(Global_4269680.f_16));
				if (bParam5)
				{
					unk_0x7899A951E85F4B28(iParam2, 70);
				}
				else
				{
					unk_0x42710E9E08FA375A(iParam2);
				}
				unk_0x2042E9CA4306F725();
				unk_0xE1FDD153F5858534();
			}
		}
		unk_0xFCC7EAA298CE7AC2(76, 66);
		unk_0xFFA39C4568552522(0f, 0f, 0f, 0f);
		if (bParam6)
		{
			if (!Global_22550.f_8417)
			{
				unk_0x36FA9C099416853E(15, 0f, -0.0375f);
				Global_22550.f_8417 = 1;
			}
		}
		else if (Global_22550.f_8417)
		{
			unk_0x610B4083B1F95914(15);
			Global_22550.f_8417 = 0;
		}
		unk_0x5C64FD27DB62AF7A();
		if (Global_22550.f_5055)
		{
			unk_0xFCC7EAA298CE7AC2(82, 66);
			unk_0xFFA39C4568552522(0f, 0f, 0f, 0f);
			unk_0xE6A03BA84586E67B(Global_22550.f_5660[iVar0 /*10*/], Global_22550.f_5053, Global_22550.f_5054, 1f, 1f, 255, 255, 255, 255, 0);
			unk_0x5C64FD27DB62AF7A();
		}
		else
		{
			unk_0x37FC7E88C7659D33(Global_22550.f_5660[iVar0 /*10*/], 255, 255, 255, 255, 0);
		}
	}
}

void func_83(var uParam0)
{
	unk_0xB23270F3D5E62FDE(uParam0);
	unk_0x2042E9CA4306F725();
}

void func_84(var uParam0)
{
	unk_0x54AED56A452DCB4F(uParam0);
}

bool func_85(int iParam0, int iParam1)
{
	bool bVar0;
	
	if (iParam0 == unk_0x460153A63B9477BC())
	{
		bVar0 = func_86(-1, 0) == 8;
	}
	else
	{
		bVar0 = Global_1590682[iParam0 /*883*/].f_211 == 8;
	}
	if (iParam1 == 1)
	{
		if (unk_0x166E920FB00B2DBB(iParam0))
		{
			bVar0 = unk_0x6CC163E30ECE0790(iParam0) == 8;
		}
	}
	return bVar0;
}

int func_86(int iParam0, bool bParam1)
{
	int iVar0;
	int iVar1;
	
	iVar1 = iParam0;
	if (iVar1 == -1)
	{
		iVar1 = func_8();
	}
	if (Global_1312876[iVar1] == 1)
	{
		if (bParam1)
		{
		}
		iVar0 = 8;
	}
	else
	{
		iVar0 = Global_1312757[iVar1];
		if (bParam1)
		{
		}
	}
	return iVar0;
}

int func_87()
{
	struct<3> Var0;
	
	if (Global_19681.f_1 > 3)
	{
		return 1;
	}
	if (func_88())
	{
		Var0 = { 0f, -500f, 0f };
		unk_0x1F1B1269BB6217F7(&Var0);
		if (Global_19626 == 0)
		{
			if (Var0.f_1 > -119f)
			{
				return 1;
			}
			else
			{
				return 0;
			}
		}
		else if (Var0.f_1 > -101f)
		{
			return 1;
		}
		else
		{
			return 0;
		}
	}
	return 0;
}

int func_88()
{
	if (unk_0x7B70881748D166CD(joaat("cellphone_flashhand")) > 0)
	{
		return 1;
	}
	return 0;
}

void func_89(int iParam0, bool bParam1, int iParam2, int iParam3, int iParam4, int iParam5)
{
	unk_0x28DD484BAEAF8437(1, iParam2, iParam3, iParam4, iParam5);
	*iParam5 = 255;
	switch (iParam0)
	{
		case 28:
			*iParam2 = 194;
			*iParam3 = 80;
			*iParam4 = 80;
			break;
		
		case 15:
		case 4:
		case 16:
		case 26:
		case 27:
		case 35:
		case 34:
		case 48:
		case 47:
		case 42:
		case 36:
		case 37:
		case 53:
		case 39:
		case 40:
		case 38:
		case 44:
		case 45:
		case 46:
		case 52:
		case 56:
		case 57:
		case 58:
		case 59:
			if (bParam1)
			{
				*iParam2 = 0;
				*iParam3 = 0;
				*iParam4 = 0;
			}
			break;
		
		case 54:
			*iParam5 = 100;
			break;
		
		case 62:
			*iParam5 = 100;
			break;
		
		case 55:
			unk_0x28DD484BAEAF8437(24, iParam2, iParam3, iParam4, iParam5);
			*iParam5 = 255;
			break;
	}
}

void func_90(float fParam0)
{
	int iVar0;
	int iVar1;
	int iVar2;
	int iVar3;
	
	unk_0x28DD484BAEAF8437(1, &iVar0, &iVar1, &iVar2, &iVar3);
	unk_0xE9FEE5ECCAD48813(0);
	unk_0x151E2F6EE4A34CFE(0f, 0.35f);
	unk_0x7508E89F04871147(2);
	unk_0xA11D279671F2289C(iVar0, iVar1, iVar2, iVar3);
	unk_0x86520D0C56F68D3F(fParam0, ((Global_22547 + Global_22549) - 0.0046875f));
	unk_0x9CB19F83A6C8E47F(0);
	unk_0xACADD4647896AEBE(0, 0, 0, 0, 0);
	unk_0xA5C56390CC2323FE(0, 0, 0, 0, 0);
}

void func_91(float fParam0, float fParam1, char* sParam2, int iParam3, int iParam4)
{
	unk_0x40A75E32D30C605F(sParam2);
	unk_0x42710E9E08FA375A(uParam3);
	unk_0x42710E9E08FA375A(uParam4);
	unk_0xA434CC17B5517E4E(fParam0, fParam1, 0);
}

float func_92(char* sParam0, int iParam1, int iParam2)
{
	if (!unk_0x211E6DB3335430B4(sParam0))
	{
		if (unk_0x856D5567211886A2(sParam0) == 0)
		{
			return 0f;
		}
	}
	else
	{
		return 0f;
	}
	func_93();
	unk_0x456829A212A690D7(sParam0);
	unk_0x42710E9E08FA375A(uParam1);
	unk_0x42710E9E08FA375A(uParam2);
	return unk_0x842CB5919E31EB1F(1);
}

void func_93()
{
	int iVar0;
	int iVar1;
	int iVar2;
	int iVar3;
	
	unk_0x28DD484BAEAF8437(1, &iVar0, &iVar1, &iVar2, &iVar3);
	if (Global_22550.f_8392)
	{
		iVar0 = Global_22550.f_8388;
		iVar1 = Global_22550.f_8389;
		iVar2 = Global_22550.f_8390;
		iVar3 = Global_22550.f_8391;
	}
	unk_0xE9FEE5ECCAD48813(0);
	unk_0x151E2F6EE4A34CFE(0f, 0.35f);
	unk_0xA11D279671F2289C(iVar0, iVar1, iVar2, iVar3);
	unk_0x86520D0C56F68D3F((Global_22547 + 0.0046875f), ((Global_22547 + Global_22549) - 0.0046875f));
	unk_0x9CB19F83A6C8E47F(0);
	unk_0xACADD4647896AEBE(0, 0, 0, 0, 0);
	unk_0xA5C56390CC2323FE(0, 0, 0, 0, 0);
}

void func_94(float fParam0, float fParam1, float fParam2, float fParam3, int iParam4, int iParam5, int iParam6, int iParam7)
{
	unk_0x8DDE075026DB7EFD((fParam0 + (fParam2 * 0.5f)), (fParam1 + (fParam3 * 0.5f)), fParam2, fParam3, iParam4, iParam5, iParam6, iParam7, 0);
}

var func_95(int iParam0, bool bParam1)
{
	char* sVar0[2];
	var uVar3;
	struct<13> Var19;
	
	if (!unk_0x7BCC91F3C1CF24E8(&(Global_22550.f_7029[iParam0 /*16*/])))
	{
		if (unk_0x856D5567211886A2(&(Global_22550.f_7029[iParam0 /*16*/])) == joaat("CREW_LOGO"))
		{
			Var19 = { func_97(unk_0x460153A63B9477BC()) };
			if (unk_0x716812D32AF6C4C0(&Var19, &uVar3))
			{
				return func_96(&uVar3);
			}
		}
		else
		{
			return func_96(&(Global_22550.f_7029[iParam0 /*16*/]));
		}
	}
	switch (iParam0)
	{
		case 3:
			sVar0[0] = "MP_hostCrown";
			sVar0[1] = "MP_hostCrown";
			break;
		
		case 21:
			sVar0[0] = "MP_SpecItem_Coke";
			sVar0[1] = "MP_SpecItem_Coke";
			break;
		
		case 22:
			sVar0[0] = "MP_SpecItem_Heroin";
			sVar0[1] = "MP_SpecItem_Heroin";
			break;
		
		case 23:
			sVar0[0] = "MP_SpecItem_Weed";
			sVar0[1] = "MP_SpecItem_Weed";
			break;
		
		case 24:
			sVar0[0] = "MP_SpecItem_Meth";
			sVar0[1] = "MP_SpecItem_Meth";
			break;
		
		case 25:
			sVar0[0] = "MP_SpecItem_Cash";
			sVar0[1] = "MP_SpecItem_Cash";
			break;
		
		case 1:
			sVar0[0] = "shop_NEW_Star";
			sVar0[1] = "shop_NEW_Star";
			break;
		
		case 2:
			sVar0[0] = "shop_NEW_Star";
			sVar0[1] = "shop_NEW_Star";
			break;
		
		case 4:
			sVar0[0] = "Shop_Tick_Icon";
			sVar0[1] = "Shop_Tick_Icon";
			break;
		
		case 6:
			sVar0[0] = "Shop_Box_CrossB";
			sVar0[1] = "Shop_Box_Cross";
			break;
		
		case 7:
			sVar0[0] = "Shop_Box_BlankB";
			sVar0[1] = "Shop_Box_Blank";
			break;
		
		case 5:
			sVar0[0] = "Shop_Box_TickB";
			sVar0[1] = "Shop_Box_Tick";
			break;
		
		case 8:
			sVar0[0] = "shop_NEW_Star";
			sVar0[1] = "shop_NEW_Star";
			break;
		
		case 9:
			sVar0[0] = "Shop_Clothing_Icon_B";
			sVar0[1] = "Shop_Clothing_Icon_A";
			break;
		
		case 10:
			sVar0[0] = "Shop_GunClub_Icon_B";
			sVar0[1] = "Shop_GunClub_Icon_A";
			break;
		
		case 17:
			sVar0[0] = "Shop_Ammo_Icon_B";
			sVar0[1] = "Shop_Ammo_Icon_A";
			break;
		
		case 18:
			sVar0[0] = "Shop_Armour_Icon_B";
			sVar0[1] = "Shop_Armour_Icon_A";
			break;
		
		case 19:
			sVar0[0] = "Shop_Health_Icon_B";
			sVar0[1] = "Shop_Health_Icon_A";
			break;
		
		case 20:
			sVar0[0] = "Shop_MakeUp_Icon_B";
			sVar0[1] = "Shop_MakeUp_Icon_A";
			break;
		
		case 11:
			sVar0[0] = "Shop_Tattoos_Icon_B";
			sVar0[1] = "Shop_Tattoos_Icon_A";
			break;
		
		case 12:
			sVar0[0] = "Shop_Garage_Icon_B";
			sVar0[1] = "Shop_Garage_Icon_A";
			break;
		
		case 13:
			sVar0[0] = "Shop_Garage_Bike_Icon_B";
			sVar0[1] = "Shop_Garage_Bike_Icon_A";
			break;
		
		case 14:
			sVar0[0] = "Shop_Barber_Icon_B";
			sVar0[1] = "Shop_Barber_Icon_A";
			break;
		
		case 15:
			sVar0[0] = "shop_Lock";
			sVar0[1] = "shop_Lock";
			break;
		
		case 16:
			sVar0[0] = "Shop_Tick_Icon";
			sVar0[1] = "Shop_Tick_Icon";
			break;
		
		case 26:
			sVar0[0] = "arrowleft";
			sVar0[1] = "arrowleft";
			break;
		
		case 27:
			sVar0[0] = "arrowright";
			sVar0[1] = "arrowright";
			break;
		
		case 28:
			sVar0[0] = "MP_AlertTriangle";
			sVar0[1] = "MP_AlertTriangle";
			break;
		
		case 29:
			sVar0[0] = "shop_NEW_Star";
			sVar0[1] = "shop_NEW_Star";
			break;
		
		case 31:
			sVar0[0] = "Shop_Michael_Icon_B";
			sVar0[1] = "Shop_Michael_Icon_A";
			break;
		
		case 32:
			sVar0[0] = "Shop_Franklin_Icon_B";
			sVar0[1] = "Shop_Franklin_Icon_A";
			break;
		
		case 33:
			sVar0[0] = "Shop_Trevor_Icon_B";
			sVar0[1] = "Shop_Trevor_Icon_A";
			break;
		
		case 51:
			sVar0[0] = "SaleIcon";
			sVar0[1] = "SaleIcon";
			break;
		
		case 52:
			sVar0[0] = "Shop_Tick_Icon";
			sVar0[1] = "Shop_Tick_Icon";
			break;
		
		case 54:
			sVar0[0] = "shop_NEW_Star";
			sVar0[1] = "shop_NEW_Star";
			break;
		
		case 55:
			sVar0[0] = "Shop_Lock_Arena";
			sVar0[1] = "Shop_Lock_Arena";
			break;
		
		case 56:
			sVar0[0] = "Card_Suit_Clubs";
			sVar0[1] = "Card_Suit_Clubs";
			break;
		
		case 57:
			sVar0[0] = "Card_Suit_Hearts";
			sVar0[1] = "Card_Suit_Hearts";
			break;
		
		case 58:
			sVar0[0] = "Card_Suit_Spades";
			sVar0[1] = "Card_Suit_Spades";
			break;
		
		case 59:
			sVar0[0] = "Card_Suit_Diamonds";
			sVar0[1] = "Card_Suit_Diamonds";
			break;
		
		case 60:
			sVar0[0] = "Shop_Art_Icon_B";
			sVar0[1] = "Shop_Art_Icon_A";
			break;
		
		case 62:
			sVar0[0] = "Shop_Art_Icon_B";
			sVar0[1] = "Shop_Art_Icon_A";
			break;
		
		case 61:
			sVar0[0] = "Shop_Chips_A";
			sVar0[1] = "Shop_Chips_B";
			break;
		
		case 0:
			sVar0[0] = "";
			sVar0[1] = "";
			break;
	}
	if (bParam1)
	{
		return sVar0[0];
	}
	return sVar0[1];
}

var func_96(var uParam0)
{
	return uParam0;
}

struct<13> func_97(int iParam0)
{
	struct<13> Var0;
	
	unk_0x2BDD80E79441BD36(iParam0, &Var0, 13);
	return Var0;
}

char* func_98(int iParam0)
{
	var uVar0;
	struct<13> Var16;
	
	if (!unk_0x7BCC91F3C1CF24E8(&(Global_22550.f_6020[iParam0 /*16*/])))
	{
		if (unk_0x856D5567211886A2(&(Global_22550.f_6020[iParam0 /*16*/])) == joaat("CREW_LOGO"))
		{
			Var16 = { func_97(unk_0x460153A63B9477BC()) };
			unk_0x716812D32AF6C4C0(&Var16, &uVar0);
			return func_96(&uVar0);
		}
		else
		{
			return func_96(&(Global_22550.f_6020[iParam0 /*16*/]));
		}
	}
	if (iParam0 == 51)
	{
		return "MPShopSale";
	}
	return "CommonMenu";
}

void func_99(int iParam0, char* sParam1, int iParam2, bool bParam3, int iParam4, bool bParam5)
{
	int iVar0;
	float fVar1;
	float fVar2;
	var uVar3;
	float fVar4;
	
	if (Global_22550.f_5218 > iParam0)
	{
		return;
	}
	if (Global_22550.f_5218 >= 128)
	{
		return;
	}
	if (Global_22550.f_5220 >= 256)
	{
		return;
	}
	if (Global_22550.f_5743 < Global_22550.f_5741)
	{
		return;
	}
	if (Global_22550.f_5218 != iParam0)
	{
		Global_22550.f_5218 = iParam0;
		Global_22550.f_5219 = 0;
	}
	iVar0 = Global_22550.f_5056[Global_22550.f_5219];
	if (iVar0 != 1)
	{
		while (Global_22550.f_5219 < 4 && iVar0 != 1)
		{
			Global_22550.f_5219++;
			iVar0 = Global_22550.f_5056[Global_22550.f_5219];
		}
		if (iVar0 != 1)
		{
			return;
		}
	}
	StringCopy(&(Global_22550.f_73[Global_22550.f_5220 /*6*/]), sParam1, 24);
	if (!unk_0x7BCC91F3C1CF24E8(sParam1) && !unk_0x3030AE9FCF1BC243(sParam1))
	{
	}
	Global_22550.f_1610[Global_22550.f_5220] = bParam3;
	Global_22550.f_1867[Global_22550.f_5220] = iParam4;
	Global_22550.f_5220++;
	if (!bParam3)
	{
		func_102(Global_22550.f_5218, 1);
	}
	else
	{
		func_102(Global_22550.f_5218, 0);
	}
	if (iParam2 == 0)
	{
		fVar1 = func_101(&(Global_22550.f_73[Global_22550.f_5220 /*6*/]));
		if (Global_22550.f_5075[Global_22550.f_5219])
		{
			func_104(26, 1, 0, &fVar2, &uVar3, 0);
			fVar1 = (fVar1 + (fVar2 * 2f));
		}
		if (fVar1 > Global_22550.f_5068[Global_22550.f_5219])
		{
			Global_22550.f_5068[Global_22550.f_5219] = fVar1;
		}
	}
	if (bParam5)
	{
		if (iParam2 == 0)
		{
			fVar4 = func_100(&(Global_22550.f_73[Global_22550.f_5220 /*6*/]));
			if (fVar4 > Global_22550.f_5745[iParam0])
			{
				Global_22550.f_5745[iParam0] = fVar4;
			}
		}
	}
	unk_0xBE20AB8238688965(&(Global_22550.f_5089[iParam0]), Global_22550.f_5219);
	Global_22550.f_5219++;
	Global_22550.f_5744 = 1;
	Global_22550.f_5742 = (Global_22550.f_5220 - 1);
	Global_22550.f_5743 = 0;
	Global_22550.f_5741 = iParam2;
}

float func_100(char* sParam0)
{
	if (!unk_0x3030AE9FCF1BC243(sParam0))
	{
	}
	return unk_0xBA269EB4693A8C47(0.35f, 0);
}

float func_101(char* sParam0)
{
	if (!unk_0x211E6DB3335430B4(sParam0))
	{
		if (unk_0x856D5567211886A2(sParam0) == 0)
		{
			return 0f;
		}
	}
	else
	{
		return 0f;
	}
	func_80(0, 1, 0, 0, 0, 0, 0);
	unk_0x456829A212A690D7(sParam0);
	return unk_0x842CB5919E31EB1F(1);
}

void func_102(int iParam0, bool bParam1)
{
	int iVar0;
	
	iVar0 = unk_0xF34EE736CF047844((unk_0xBBDA792448DB5A89(iParam0) / 32f));
	if (bParam1)
	{
		unk_0xBE20AB8238688965(&(Global_22550.f_6015[iVar0]), (iParam0 - iVar0 * 32));
	}
	else
	{
		unk_0xD2459066EA58CE43(&(Global_22550.f_6015[iVar0]), (iParam0 - iVar0 * 32));
	}
}

int func_103()
{
	int iVar0;
	int iVar1;
	float fVar2;
	
	unk_0x9DD549AAA043F83A(&iVar0, &iVar1);
	fVar2 = (unk_0xBBDA792448DB5A89(iVar0) / unk_0xBBDA792448DB5A89(iVar1));
	if (fVar2 > 3.5f)
	{
		return 1;
	}
	return 0;
}

int func_104(int iParam0, bool bParam1, bool bParam2, float fParam3, float fParam4, bool bParam5)
{
	char cVar0[64];
	char cVar16[64];
	int iVar32;
	int iVar33;
	float fVar34;
	float fVar35;
	float fVar36;
	struct<3> Var37;
	
	StringCopy(&cVar0, func_98(iParam0), 64);
	StringCopy(&cVar16, func_95(iParam0, bParam1), 64);
	if (unk_0x856D5567211886A2(&cVar16) != 0)
	{
		fVar34 = 1f;
		if (bParam5)
		{
			unk_0x9DD549AAA043F83A(&iVar32, &iVar33);
			fVar35 = unk_0x8940B46482DD0A7D(0);
			if (func_103())
			{
				iVar32 = unk_0xF2DB717A73826179((unk_0xBBDA792448DB5A89(iVar33) * fVar35));
			}
			fVar36 = (unk_0xBBDA792448DB5A89(iVar32) / unk_0xBBDA792448DB5A89(iVar33));
			fVar34 = (fVar36 / fVar35);
			if (func_103())
			{
				fVar34 = 1f;
			}
			if (unk_0x7B70881748D166CD(joaat("director_mode")) > 0)
			{
				unk_0x8BAB7B3A2C7777A5(&iVar32, &iVar33);
			}
			iVar32 = unk_0xF2DB717A73826179((unk_0xBBDA792448DB5A89(iVar32) / fVar34));
			iVar33 = unk_0xF2DB717A73826179((unk_0xBBDA792448DB5A89(iVar33) / fVar34));
		}
		else
		{
			unk_0x8BAB7B3A2C7777A5(&iVar32, &iVar33);
		}
		Var37 = { unk_0x282B86763E4DCFC4(&cVar0, &cVar16) };
		Var37.f_0 = (Var37.f_0 * (func_105(iParam0) / fVar34));
		Var37.f_1 = (Var37.f_1 * (func_105(iParam0) / fVar34));
		if (!bParam2)
		{
			Var37.f_0 = (Var37.f_0 - 2f);
			Var37.f_1 = (Var37.f_1 - 2f);
		}
		if (iParam0 == 30)
		{
			Var37.f_0 = 288f;
			Var37.f_1 = 106f;
		}
		if (iParam0 == 29 && unk_0x856D5567211886A2(&(Global_22550.f_7029[29 /*16*/])) == joaat("CREW_LOGO"))
		{
			Var37.f_0 = 106f;
			Var37.f_1 = 106f;
		}
		*fParam3 = ((Var37.f_0 / IntToFloat(iVar32)) * IntToFloat((iVar32 / iVar33)));
		*fParam4 = (((Var37.f_1 / IntToFloat(iVar33)) / (Var37.f_0 / IntToFloat(iVar32))) * *fParam3);
		if (!bParam5)
		{
			if (!unk_0xE5EF21F14D0F9D25() && iParam0 != 30)
			{
				*fParam3 = (*fParam3 * 1.33f);
			}
		}
		if (iParam0 == 29)
		{
			if (*fParam3 > Global_22549)
			{
				*fParam4 = (*fParam4 * (Global_22549 / *fParam3));
				*fParam3 = Global_22549;
			}
		}
		return 1;
	}
	return 0;
}

float func_105(int iParam0)
{
	switch (iParam0)
	{
		case 33:
		case 4:
		case 11:
		case 31:
		case 20:
		case 15:
		case 10:
		case 12:
		case 13:
		case 32:
		case 9:
		case 5:
		case 6:
		case 7:
		case 14:
		case 18:
		case 19:
		case 17:
		case 28:
		case 26:
		case 27:
		case 52:
		case 56:
		case 57:
		case 58:
		case 59:
		case 60:
		case 62:
			return 0.5f;
			break;
		
		case 61:
			return 0.8f;
			break;
	}
	return 1f;
}

int func_106(bool bParam0, bool bParam1)
{
	if (Global_2440277.f_2005.f_701 != 0)
	{
		return 1;
	}
	if ((((((((!unk_0x87F0CB19476706C3() || (func_56(8, -1) && func_107() != 65)) || (unk_0xCFB49FF4012D7C68() != 0 && !bParam1)) || (unk_0x9CC4D06F5338CA6B() && !bParam0)) || unk_0x954BAB46FBB90036()) || Global_77093) || Global_22550.f_8416) || unk_0x06EBE4F22EC30D45()) || Global_99007.f_1425)
	{
		return 0;
	}
	return 1;
}

int func_107()
{
	return Global_1312830;
}

void func_108(var uParam0)
{
	func_115(func_116(uParam0));
	func_114(-1, 1, 1);
	if (uParam0->f_2593 == 6 || uParam0->f_2593 == 7)
	{
		func_111(func_112(uParam0), 0, 0);
		func_109(func_110(uParam0->f_2593));
	}
	else
	{
		func_111(func_112(uParam0), 0, 0);
	}
}

void func_109(int iParam0)
{
	if (Global_22550.f_4763 >= 3 || Global_22550.f_4762 >= 4)
	{
		return;
	}
	Global_22550.f_4696[Global_22550.f_4762] = 2;
	Global_22550.f_4762++;
	Global_22550.f_4701[Global_22550.f_4763] = iParam0;
	Global_22550.f_4763++;
}

int func_110(int iParam0)
{
	switch (iParam0)
	{
		case 6:
			return Global_262145.f_28116;
		
		case 7:
			return Global_262145.f_28115;
		
		default:
	}
	return -1;
}

void func_111(char* sParam0, int iParam1, int iParam2)
{
	int iVar0;
	
	StringCopy(&(Global_22550.f_4690), sParam0, 24);
	Global_22550.f_4762 = 0;
	Global_22550.f_4763 = 0;
	Global_22550.f_4764 = 0;
	Global_22550.f_4765 = 0;
	Global_22550.f_4766 = iParam1;
	Global_22550.f_4767 = unk_0x578C4EF320340AF7();
	Global_22550.f_4768 = iParam2;
	iVar0 = 0;
	while (iVar0 < 4)
	{
		Global_22550.f_4696[iVar0] = 0;
		iVar0++;
	}
}

char* func_112(var uParam0)
{
	switch (uParam0->f_4355)
	{
		case 2:
			switch (uParam0->f_2593)
			{
				case 2:
					return "CH_DP_FB_D_0";
				
				case 3:
					return "CH_DP_FB_D_1";
				
				case 4:
					return "CH_DP_FB_D_2";
				
				case 13:
					return "CH_DP_FB_D_3";
				
				case 14:
					return "CH_DP_FB_D_4";
				
				case 15:
					if (func_113(uParam0->f_4356) == 2 && uParam0->f_1497.f_1020 != 4)
					{
						return "CH_DP_FB_D_5b";
					}
					return "CH_DP_FB_D_5";
				
				case 5:
					return "CH_DP_FB_D_6";
				
				case 16:
					return "CH_DP_FB_D_7";
				
				case 17:
					switch (func_113(uParam0->f_4356))
					{
						case 1:
							return "CH_DP_FB_D_8a";
						
						case 2:
							return "CH_DP_FB_D_8b";
						
						case 3:
							return "CH_DP_FB_D_8c";
						
						default:
					}
					break;
				
				case 6:
					return "CH_DP_FB_D_9";
				
				case 7:
					return "CH_DP_FB_D_10";
			}
			break;
	}
	return "";
}

int func_113(int iParam0)
{
	if (iParam0 != -1)
	{
		return Global_1701669[iParam0 /*68*/].f_18.f_4;
	}
	return 0;
}

void func_114(int iParam0, bool bParam1, int iParam2)
{
	int iVar0;
	int iVar1;
	
	Global_22550.f_5739 = iParam0;
	Global_22550.f_5874 = iParam2;
	if (Global_22550.f_5739 < Global_22550.f_5738)
	{
		Global_22550.f_5738 = Global_22550.f_5739;
	}
	else if ((Global_22550.f_5729 && Global_22550.f_5739 > Global_22550.f_5740) || (!Global_22550.f_5729 && Global_22550.f_5739 >= (Global_22550.f_5738 + Global_22550.f_5225)))
	{
		iVar0 = Global_22550.f_5738;
		while (iVar0 <= Global_22550.f_5739)
		{
			if (iVar0 >= 0 && iVar0 < 127)
			{
				if (Global_22550.f_5089[iVar0] != 0)
				{
					iVar1++;
				}
			}
			iVar0++;
		}
		while (iVar1 > Global_22550.f_5225 && Global_22550.f_5738 < 128)
		{
			Global_22550.f_5738++;
			iVar1 = 0;
			iVar0 = Global_22550.f_5738;
			while (iVar0 <= Global_22550.f_5739)
			{
				if (iVar0 >= 0 && iVar0 < 127)
				{
					if (Global_22550.f_5089[iVar0] != 0)
					{
						iVar1++;
					}
				}
				iVar0++;
			}
		}
	}
	Global_22550.f_5728 = 0;
	Global_22550.f_5729 = 0;
	if (bParam1)
	{
		StringCopy(&(Global_22550.f_4690), "", 24);
		StringCopy(&(Global_4269680.f_21), "", 16);
	}
}

void func_115(char* sParam0)
{
	int iVar0;
	
	StringCopy(&(Global_22550.f_1), sParam0, 16);
	Global_22550.f_68 = 0;
	Global_22550.f_69 = 0;
	Global_22550.f_70 = 0;
	Global_22550.f_71 = 0;
	Global_22550.f_72 = 0;
	iVar0 = 0;
	while (iVar0 < 4)
	{
		Global_22550.f_5[iVar0] = 0;
		iVar0++;
	}
}

char* func_116(var uParam0)
{
	switch (uParam0->f_4355)
	{
		case 2:
			switch (uParam0->f_2593)
			{
				case 2:
					return "CH_DP_FB_T_0";
				
				case 3:
					return "CH_DP_FB_T_1";
				
				case 4:
					return "CH_DP_FB_T_2";
				
				case 13:
					return "CH_DP_FB_T_3";
				
				case 14:
					return "CH_DP_FB_T_4";
				
				case 15:
					return "CH_DP_FB_T_5";
				
				case 5:
					return "CH_DP_FB_T_6";
				
				case 16:
					return "CH_DP_FB_T_7";
				
				case 17:
					return "CH_DP_FB_T_8";
				
				case 6:
					return "CH_DP_FB_T_9";
				
				case 7:
					return "CH_DP_FB_T_10";
				
				default:
			}
			break;
	}
	return "";
}

int func_117(var uParam0)
{
	switch (uParam0->f_4355)
	{
		case 2:
			switch (uParam0->f_2593)
			{
				case 2:
				case 3:
				case 4:
				case 5:
				case 6:
				case 7:
				case 13:
				case 14:
				case 15:
				case 16:
				case 17:
					return 1;
				
				default:
			}
			break;
	}
	return 0;
}

void func_118(var uParam0)
{
	func_115("CH_DP_TARGET");
	func_114(-1, 1, 1);
	func_111("CH_DETAILSh", 0, 0);
	func_119(func_120(func_121(uParam0->f_4356), 0));
}

void func_119(char* sParam0)
{
	if (Global_22550.f_4765 >= 3 || Global_22550.f_4762 >= 4)
	{
		return;
	}
	Global_22550.f_4696[Global_22550.f_4762] = 1;
	Global_22550.f_4762++;
	StringCopy(&(Global_22550.f_4713[Global_22550.f_4765 /*16*/]), sParam0, 64);
	Global_22550.f_4765++;
}

char* func_120(int iParam0, bool bParam1)
{
	switch (iParam0)
	{
		case 0:
			if (!bParam1)
			{
				return "CH_TARGET_0b";
			}
			return "CH_TARGET_0";
		
		case 1:
			if (!bParam1)
			{
				return "CH_TARGET_1b";
			}
			return "CH_TARGET_1";
		
		case 2:
			if (!bParam1)
			{
				return "CH_TARGET_2b";
			}
			return "CH_TARGET_2";
		
		case 3:
			if (!bParam1)
			{
				return "CH_TARGET_3b";
			}
			return "CH_TARGET_3";
		
		default:
	}
	return "";
}

int func_121(int iParam0)
{
	if (iParam0 != -1)
	{
		return Global_1701669[iParam0 /*68*/].f_18.f_7;
	}
	return 0;
}

int func_122(var uParam0)
{
	switch (uParam0->f_4355)
	{
		case 0:
			switch (uParam0->f_2593)
			{
				case 8:
					return 1;
				
				default:
			}
			break;
	}
	return 0;
}

void func_123(var uParam0)
{
	func_115(func_128(uParam0, uParam0->f_2593));
	func_114(-1, 1, 1);
	if (uParam0->f_2593 == 2)
	{
		func_111("CH_DETAILSf", 0, 0);
		func_119(func_127(uParam0, uParam0->f_2593));
		func_109(func_125(uParam0, uParam0->f_2593));
	}
	else
	{
		func_111("CH_DETAILSg", 0, 0);
		func_119(func_127(uParam0, uParam0->f_2593));
		func_109(func_125(uParam0, uParam0->f_2593));
		func_119(func_124(uParam0, uParam0->f_2593));
	}
}

char* func_124(var uParam0, int iParam1)
{
	switch (iParam1)
	{
		case 2:
			return "";
		
		case 4:
			return "CH_DP_PUR_PN_1";
		
		case 3:
			return "CH_DP_PUR_PN_2";
		
		default:
	}
	return "";
}

int func_125(var uParam0, int iParam1)
{
	return func_126(iParam1);
}

int func_126(int iParam0)
{
	switch (iParam0)
	{
		case 2:
			return Global_262145.f_28097;
		
		case 4:
			return Global_262145.f_28099;
		
		case 3:
			return Global_262145.f_28098;
		
		default:
	}
	return -1;
}

char* func_127(var uParam0, int iParam1)
{
	switch (iParam1)
	{
		case 2:
			return "CH_DP_PUR_D_0";
		
		case 4:
			return "CH_DP_PUR_D_1";
		
		case 3:
			return "CH_DP_PUR_D_2";
		
		default:
	}
	return "";
}

char* func_128(var uParam0, int iParam1)
{
	switch (iParam1)
	{
		case 2:
			return "CH_DP_PUR_T_0";
		
		case 4:
			return "CH_DP_PUR_T_1";
		
		case 3:
			return "CH_DP_PUR_T_2";
		
		default:
	}
	return "";
}

int func_129(var uParam0)
{
	switch (uParam0->f_4355)
	{
		case 0:
			switch (uParam0->f_2593)
			{
				case 2:
				case 4:
				case 3:
					return 1;
				
				default:
			}
			break;
	}
	return 0;
}

void func_130(var uParam0)
{
	func_115(func_132(uParam0, uParam0->f_2593));
	func_114(-1, 1, 1);
	func_111("CH_DETAILSe", 0, 0);
	func_119(func_131(uParam0, uParam0->f_2593));
}

char* func_131(var uParam0, int iParam1)
{
	switch (iParam1)
	{
		case 5:
			return "CH_DP_APR_0";
		
		case 6:
			return "CH_DP_APR_1";
		
		case 7:
			return "CH_DP_APR_2";
		
		default:
	}
	return "";
}

char* func_132(var uParam0, int iParam1)
{
	switch (iParam1)
	{
		case 5:
			return func_133(1, 1);
		
		case 6:
			return func_133(2, 1);
		
		case 7:
			return func_133(3, 1);
		
		default:
	}
	return "";
}

char* func_133(int iParam0, bool bParam1)
{
	switch (iParam0)
	{
		case 1:
			if (!bParam1)
			{
				return "CH_APPROACH_0b";
			}
			return "CH_APPROACH_0";
		
		case 2:
			if (!bParam1)
			{
				return "CH_APPROACH_1b";
			}
			return "CH_APPROACH_1";
		
		case 3:
			if (!bParam1)
			{
				return "CH_APPROACH_2b";
			}
			return "CH_APPROACH_2";
		
		default:
	}
	return "";
}

int func_134(var uParam0)
{
	switch (uParam0->f_4355)
	{
		case 0:
			switch (uParam0->f_2593)
			{
				case 5:
				case 6:
				case 7:
					return 1;
				
				default:
			}
			break;
	}
	return 0;
}

void func_135(var uParam0)
{
	func_115(func_166(uParam0, uParam0->f_2593));
	func_114(-1, 1, 1);
	if (func_162(uParam0, uParam0->f_2593))
	{
		if (func_158(uParam0, uParam0->f_2593) != -1)
		{
			func_111("CH_DETAILSb", 0, 0);
			func_119(func_157(uParam0, uParam0->f_2593));
			func_119(func_156(uParam0, uParam0->f_2593));
			func_109(func_158(uParam0, uParam0->f_2593));
		}
		else
		{
			func_111("CH_DETAILSc", 0, 0);
			func_119(func_157(uParam0, uParam0->f_2593));
			func_119(func_156(uParam0, uParam0->f_2593));
			func_119("CH_LIST_BLANK");
		}
	}
	else
	{
		func_111("CH_DETAILSd", 0, 0);
		func_119(func_136(uParam0, uParam0->f_2593));
	}
}

char* func_136(var uParam0, int iParam1)
{
	switch (iParam1)
	{
		case 10:
			if (func_143(uParam0))
			{
				return "CH_LIST_BLANK";
			}
			else
			{
				switch (uParam0->f_744.f_738)
				{
					case 3:
						return "CH_DP_UR_WE_0";
					
					case 4:
						return "CH_DP_UR_WE_1";
					}
				
				default:
			}
			break;
		
		case 11:
			switch (uParam0->f_744.f_740)
			{
				case 4:
					return "CH_DP_UR_DR_0";
				
				case 5:
					return "CH_DP_UR_WE_1";
				
				default:
			}
			break;
		
		case 12:
			if (func_137(uParam0))
			{
				return "CH_LIST_BLANK";
			}
			else
			{
				switch (uParam0->f_744.f_743)
				{
					case 3:
						return "CH_DP_UR_HR_0";
					
					case 5:
						return "CH_DP_UR_HR_1";
					}
				
				default:
			}
			break;
	}
	return "";
}

int func_137(var uParam0)
{
	if (uParam0->f_744.f_743 == 4 && !func_138(uParam0->f_4356, uParam0->f_744.f_743))
	{
		return 1;
	}
	return 0;
}

int func_138(int iParam0, int iParam1)
{
	if (iParam0 == -1)
	{
		return 0;
	}
	switch (iParam1)
	{
		case 1:
			return 1;
		
		case 2:
			return 1;
		
		case 3:
			if (func_142(iParam0) && func_140(iParam0))
			{
				return 1;
			}
			break;
		
		case 4:
			if (unk_0xCE990E643CD9D0E5(Global_1701669[iParam0 /*68*/].f_40, 15))
			{
				return 1;
			}
			break;
		
		case 5:
			if (func_139(iParam0) && unk_0xCE990E643CD9D0E5(Global_1701669[iParam0 /*68*/].f_42, 4))
			{
				return 1;
			}
			break;
	}
	return 0;
}

int func_139(int iParam0)
{
	if (iParam0 != -1)
	{
		return unk_0xCE990E643CD9D0E5(Global_1590682[iParam0 /*883*/].f_274.f_351, 31);
	}
	return 0;
}

int func_140(int iParam0)
{
	if (iParam0 == func_42())
	{
		return 0;
	}
	return ((func_141(iParam0, 0) && func_141(iParam0, 1)) && func_141(iParam0, 4));
}

int func_141(int iParam0, int iParam1)
{
	if (iParam0 == func_42())
	{
		return 0;
	}
	switch (iParam1)
	{
		case 0:
			return unk_0xCE990E643CD9D0E5(Global_1590682[iParam0 /*883*/].f_274.f_281.f_7, 0);
			break;
		
		case 1:
			return unk_0xCE990E643CD9D0E5(Global_1590682[iParam0 /*883*/].f_274.f_281.f_7, 1);
			break;
		
		case 4:
			return unk_0xCE990E643CD9D0E5(Global_1590682[iParam0 /*883*/].f_274.f_281.f_7, 2);
			break;
		
		case 2:
			return unk_0xCE990E643CD9D0E5(Global_1590682[iParam0 /*883*/].f_274.f_281.f_7, 3);
			break;
		
		case 3:
			return unk_0xCE990E643CD9D0E5(Global_1590682[iParam0 /*883*/].f_274.f_281.f_7, 4);
			break;
		
		case 5:
			return unk_0xCE990E643CD9D0E5(Global_1590682[iParam0 /*883*/].f_274.f_281.f_7, 5);
			break;
	}
	return 0;
}

int func_142(int iParam0)
{
	if (iParam0 != func_42())
	{
		return Global_1590682[iParam0 /*883*/].f_274.f_322 != 0;
	}
	return 0;
}

int func_143(var uParam0)
{
	if (uParam0->f_744.f_738 == 5 && !func_144(uParam0->f_4356, uParam0->f_744.f_738))
	{
		return 1;
	}
	return 0;
}

int func_144(int iParam0, int iParam1)
{
	if (iParam0 == -1)
	{
		return 0;
	}
	switch (iParam1)
	{
		case 1:
			return 1;
		
		case 2:
			return 1;
		
		case 3:
			if (func_155(iParam0) && func_154(iParam0))
			{
				return 1;
			}
			break;
		
		case 4:
			if (func_153(iParam0, 0) == 5)
			{
				return 0;
			}
			if (func_152(iParam0) && func_147(iParam0, func_151(iParam0)))
			{
				return 1;
			}
			if (func_146(iParam0) && func_145(iParam0))
			{
				return 1;
			}
			break;
		
		case 5:
			if (unk_0xCE990E643CD9D0E5(Global_1701669[iParam0 /*68*/].f_40, 9))
			{
				return 1;
			}
			break;
	}
	return 0;
}

int func_145(int iParam0)
{
	if (iParam0 != func_42())
	{
		return unk_0xCE990E643CD9D0E5(Global_1590682[iParam0 /*883*/].f_274.f_339.f_2, 13);
	}
	return 0;
}

int func_146(int iParam0)
{
	if (iParam0 != func_42())
	{
		if (Global_1590682[iParam0 /*883*/].f_274.f_339 != 0)
		{
			return 1;
		}
	}
	return 0;
}

int func_147(int iParam0, int iParam1)
{
	int iVar0;
	
	if (func_150(iParam0, iParam1))
	{
		iVar0 = func_148(iParam0, iParam1);
		if (Global_1590682[iParam0 /*883*/].f_274.f_183[iVar0 /*12*/].f_4 > 0 && Global_1590682[iParam0 /*883*/].f_274.f_183[iVar0 /*12*/].f_7)
		{
			return 1;
		}
	}
	return 0;
}

int func_148(int iParam0, int iParam1)
{
	int iVar0;
	
	if (func_149(iParam1) && iParam0 != func_42())
	{
		iVar0 = 0;
		while (iVar0 <= 5)
		{
			if (Global_1590682[iParam0 /*883*/].f_274.f_183[iVar0 /*12*/] == iParam1)
			{
				return iVar0;
			}
			iVar0++;
		}
	}
	return -1;
}

int func_149(int iParam0)
{
	if (iParam0 == 32 || iParam0 == 0)
	{
		return 0;
	}
	return 1;
}

int func_150(int iParam0, int iParam1)
{
	int iVar0;
	
	if (func_149(iParam1) && iParam0 != func_42())
	{
		iVar0 = 0;
		while (iVar0 <= 5)
		{
			if (Global_1590682[iParam0 /*883*/].f_274.f_183[iVar0 /*12*/] == iParam1)
			{
				return 1;
			}
			iVar0++;
		}
	}
	return 0;
}

int func_151(int iParam0)
{
	if (iParam0 == func_42())
	{
		return 0;
	}
	return Global_1590682[iParam0 /*883*/].f_274.f_183[5 /*12*/];
}

bool func_152(int iParam0)
{
	return func_151(iParam0) != 0;
}

int func_153(int iParam0, bool bParam1)
{
	if (iParam0 != -1)
	{
		if (Global_1701669[iParam0 /*68*/].f_18.f_12 != 0 || !bParam1)
		{
			return Global_1701669[iParam0 /*68*/].f_18.f_12;
		}
	}
	if (bParam1)
	{
		return Global_1697272.f_15;
	}
	return 0;
}

int func_154(int iParam0)
{
	if (iParam0 == func_42())
	{
		return 0;
	}
	return Global_1590682[iParam0 /*883*/].f_274.f_264.f_5;
}

int func_155(int iParam0)
{
	if (iParam0 != func_42())
	{
		return Global_1590682[iParam0 /*883*/].f_274.f_264 != 0;
	}
	return 0;
}

char* func_156(var uParam0, int iParam1)
{
	switch (iParam1)
	{
		case 10:
			if (func_143(uParam0))
			{
				return "CH_LIST_BLANK";
			}
			else
			{
				switch (uParam0->f_744.f_738)
				{
					case 1:
						return "CH_SKILL_POOR";
					
					case 2:
						return "CH_SKILL_EXPR";
					
					case 3:
						return "CH_SKILL_GOOD";
					
					case 4:
						return "CH_SKILL_EXPR";
					
					case 5:
						return "CH_SKILL_EXPR";
					}
				
				default:
			}
			break;
		
		case 11:
			switch (uParam0->f_744.f_740)
			{
				case 1:
					return "CH_SKILL_POOR";
				
				case 2:
					return "CH_SKILL_GOOD";
				
				case 3:
					return "CH_SKILL_EXPR";
				
				case 4:
					return "CH_SKILL_GOOD";
				
				case 5:
					return "CH_SKILL_EXPR";
				
				default:
			}
			break;
		
		case 12:
			if (func_137(uParam0))
			{
				return "CH_LIST_BLANK";
			}
			else
			{
				switch (uParam0->f_744.f_743)
				{
					case 1:
						return "CH_SKILL_POOR";
					
					case 2:
						return "CH_SKILL_GOOD";
					
					case 3:
						return "CH_SKILL_GOOD";
					
					case 4:
						return "CH_SKILL_EXPR";
					
					case 5:
						return "CH_SKILL_EXPR";
					}
				
				default:
			}
			break;
	}
	return "";
}

char* func_157(var uParam0, int iParam1)
{
	switch (iParam1)
	{
		case 10:
			return "CH_WE_EXPR";
		
		case 11:
			return "CH_DR_EXPR";
		
		case 12:
			return "CH_HR_EXPR";
		
		default:
	}
	return "";
}

int func_158(var uParam0, int iParam1)
{
	switch (iParam1)
	{
		case 10:
			if (func_143(uParam0))
			{
				return 0;
			}
			else
			{
				return func_161(uParam0->f_744.f_738);
			}
			break;
		
		case 11:
			return func_160(uParam0->f_744.f_740);
			break;
		
		case 12:
			if (func_137(uParam0))
			{
				return 0;
			}
			else
			{
				return func_159(uParam0->f_744.f_743);
			}
			break;
	}
	return 0;
}

int func_159(int iParam0)
{
	switch (iParam0)
	{
		case 1:
			return Global_262145.f_28110;
		
		case 2:
			return Global_262145.f_28111;
		
		case 3:
			return Global_262145.f_28112;
		
		case 4:
			return Global_262145.f_28113;
		
		case 5:
			return Global_262145.f_28114;
		
		default:
	}
	return 0;
}

int func_160(int iParam0)
{
	switch (iParam0)
	{
		case 1:
			return Global_262145.f_28105;
		
		case 2:
			return Global_262145.f_28106;
		
		case 3:
			return Global_262145.f_28107;
		
		case 4:
			return Global_262145.f_28108;
		
		case 5:
			return Global_262145.f_28109;
		
		default:
	}
	return 0;
}

int func_161(int iParam0)
{
	switch (iParam0)
	{
		case 1:
			return Global_262145.f_28100;
		
		case 2:
			return Global_262145.f_28101;
		
		case 3:
			return Global_262145.f_28102;
		
		case 4:
			return Global_262145.f_28103;
		
		case 5:
			return Global_262145.f_28104;
		
		default:
	}
	return 0;
}

int func_162(var uParam0, int iParam1)
{
	switch (iParam1)
	{
		case 10:
			return func_144(uParam0->f_4356, uParam0->f_744.f_738);
		
		case 11:
			return func_163(uParam0->f_4356, uParam0->f_744.f_740);
		
		case 12:
			return func_138(uParam0->f_4356, uParam0->f_744.f_743);
		
		default:
	}
	return 0;
}

int func_163(int iParam0, int iParam1)
{
	if (iParam0 == -1)
	{
		return 0;
	}
	switch (iParam1)
	{
		case 1:
			return 1;
		
		case 2:
			return 1;
		
		case 3:
			return 1;
		
		case 4:
			if (func_165(iParam0) && unk_0xCE990E643CD9D0E5(Global_1701669[iParam0 /*68*/].f_42, 5))
			{
				return 1;
			}
			break;
		
		case 5:
			if (func_164(iParam0, 0) == 4)
			{
				return 0;
			}
			if (func_152(iParam0) && func_147(iParam0, func_151(iParam0)))
			{
				return 1;
			}
			if (func_146(iParam0) && func_145(iParam0))
			{
				return 1;
			}
			break;
	}
	return 0;
}

int func_164(int iParam0, bool bParam1)
{
	if (iParam0 != -1)
	{
		if (Global_1701669[iParam0 /*68*/].f_18.f_10 != 0 || !bParam1)
		{
			return Global_1701669[iParam0 /*68*/].f_18.f_10;
		}
	}
	if (bParam1)
	{
		return Global_1697272.f_13;
	}
	return 0;
}

bool func_165(int iParam0)
{
	if (iParam0 == -1)
	{
		return 0;
	}
	return unk_0xCE990E643CD9D0E5(Global_1590682[iParam0 /*883*/].f_274.f_123, 14);
}

char* func_166(var uParam0, int iParam1)
{
	switch (iParam1)
	{
		case 10:
			if (func_143(uParam0))
			{
				return "CH_LIST_BLANK";
			}
			else
			{
				switch (uParam0->f_744.f_738)
				{
					case 1:
						return "CH_WE_NAME_0";
					
					case 2:
						return "CH_WE_NAME_1";
					
					case 3:
						return "CH_WE_NAME_2";
					
					case 4:
						return "CH_WE_NAME_3";
					
					case 5:
						return "CH_WE_NAME_4";
					}
				
				default:
			}
			break;
		
		case 11:
			switch (uParam0->f_744.f_740)
			{
				case 1:
					return "CH_DR_NAME_0";
				
				case 2:
					return "CH_DR_NAME_1";
				
				case 3:
					return "CH_DR_NAME_2";
				
				case 4:
					return "CH_DR_NAME_3";
				
				case 5:
					return "CH_DR_NAME_4";
				
				default:
			}
			break;
		
		case 12:
			if (func_137(uParam0))
			{
				return "CH_LIST_BLANK";
			}
			else
			{
				switch (uParam0->f_744.f_743)
				{
					case 1:
						return "CH_HR_NAME_0";
					
					case 2:
						return "CH_HR_NAME_1";
					
					case 3:
						return "CH_HR_NAME_2";
					
					case 4:
						return "CH_HR_NAME_3";
					
					case 5:
						return "CH_HR_NAME_4";
					}
				
				default:
			}
			break;
	}
	return "";
}

int func_167(var uParam0)
{
	switch (uParam0->f_4355)
	{
		case 1:
			switch (uParam0->f_2593)
			{
				case 10:
				case 11:
				case 12:
					return 1;
				
				default:
			}
			break;
	}
	return 0;
}

void func_168(var uParam0)
{
	func_115(func_181(uParam0, uParam0->f_4355, uParam0->f_2593));
	func_114(-1, 1, 1);
	if (func_176(func_177(uParam0, uParam0->f_4355, uParam0->f_2593)))
	{
		func_111("CH_DETAILSd", 0, 0);
		func_119(func_177(uParam0, uParam0->f_4355, uParam0->f_2593));
	}
	else
	{
		func_111("CH_DETAILS", 0, 0);
		func_119(func_177(uParam0, uParam0->f_4355, uParam0->f_2593));
		func_119(func_175(uParam0, uParam0->f_4355, uParam0->f_2593));
		func_119(func_171(uParam0, uParam0->f_4355, uParam0->f_2593));
		func_109(func_169(uParam0, uParam0->f_4355, uParam0->f_2593));
	}
}

int func_169(var uParam0, int iParam1, int iParam2)
{
	switch (iParam1)
	{
		case 0:
			switch (iParam2)
			{
				case 9:
					return 1;
				
				case 10:
					return 1;
				
				default:
			}
			break;
		
		case 1:
			switch (iParam2)
			{
				case 2:
					return 1;
				
				case 3:
					return 1;
				
				case 4:
					return 2;
				
				case 5:
					return 2;
				
				case 6:
					if (func_153(uParam0->f_4356, 0) == 4)
					{
						return 4;
					}
					return 2;
				
				case 7:
					return 2;
				
				case 9:
					return 2;
				
				case 13:
					return 1;
				
				case 14:
					switch (func_113(uParam0->f_4356))
					{
						case 1:
							return 1;
						
						case 2:
							switch (uParam0->f_744.f_744)
							{
								case 1:
									return 1;
								
								case 2:
									if (func_170(uParam0->f_4356, 2))
									{
										return 1;
									}
									return 2;
								
								case 3:
									if (func_170(uParam0->f_4356, 3))
									{
										return 2;
									}
									return 1;
								
								default:
							}
							break;
						
						case 3:
							return 2;
					}
					break;
				
				case 15:
					switch (func_113(uParam0->f_4356))
					{
						case 1:
							return 2;
						
						case 2:
							return 2;
						
						case 3:
							return 2;
						
						default:
					}
					break;
				
				case 16:
					switch (func_113(uParam0->f_4356))
					{
						case 1:
							return 1;
						
						case 2:
							if (func_170(uParam0->f_4356, 4))
							{
								return 2;
							}
							return 1;
						
						case 3:
							return 2;
						
						default:
					}
					break;
				
				case 17:
					switch (func_113(uParam0->f_4356))
					{
						case 1:
							return 2;
						
						case 2:
							switch (uParam0->f_744.f_745)
							{
								case 6:
									return 2;
								
								case 7:
									return 1;
								
								default:
							}
							break;
						
						case 3:
							return 1;
					}
					break;
				
				case 8:
					return 2;
				
				case 18:
					return 1;
			}
			break;
	}
	return 1;
}

int func_170(int iParam0, int iParam1)
{
	if (iParam0 != -1)
	{
		switch (iParam1)
		{
			case 1:
				return unk_0xCE990E643CD9D0E5(Global_1701669[iParam0 /*68*/].f_18, 8);
			
			case 2:
				return unk_0xCE990E643CD9D0E5(Global_1701669[iParam0 /*68*/].f_18, 10);
			
			case 3:
				return unk_0xCE990E643CD9D0E5(Global_1701669[iParam0 /*68*/].f_18, 12);
			
			case 4:
				return unk_0xCE990E643CD9D0E5(Global_1701669[iParam0 /*68*/].f_18, 14);
			}
		
		default:
	}
	return 0;
}

char* func_171(var uParam0, int iParam1, int iParam2)
{
	switch (iParam1)
	{
		case 0:
			switch (iParam2)
			{
				case 9:
					return "CH_DP_MAND";
				
				case 10:
					return "CH_DP_MAND";
				
				default:
			}
			break;
		
		case 1:
			switch (iParam2)
			{
				case 2:
					return "CH_DP_OPT";
				
				case 3:
					return "CH_DP_OPT";
				
				case 4:
					return "CH_DP_OPT";
				
				case 5:
					return "CH_DP_MAND";
				
				case 6:
					return "CH_DP_MAND";
				
				case 7:
					return "CH_DP_MAND";
				
				case 9:
					return "CH_DP_MAND";
				
				case 13:
					return "CH_DP_OPT";
				
				case 14:
					switch (func_113(uParam0->f_4356))
					{
						case 2:
							if (func_172(uParam0->f_4356))
							{
								return "CH_DP_OPT";
							}
							break;
					}
					return "CH_DP_MAND";
				
				case 15:
					return "CH_DP_MAND";
				
				case 16:
					return "CH_DP_OPT";
				
				case 17:
					return "CH_DP_OPT";
				
				case 8:
					return "CH_DP_OPT";
				
				case 18:
					return "CH_DP_OPT";
				
				default:
			}
			break;
	}
	return "";
}

int func_172(int iParam0)
{
	if (((func_173(iParam0, 1) || func_173(iParam0, 2)) || func_173(iParam0, 3)) || func_173(iParam0, 4))
	{
		return 1;
	}
	return 0;
}

int func_173(int iParam0, int iParam1)
{
	if (iParam0 != -1)
	{
		switch (iParam1)
		{
			case 1:
			case 2:
			case 3:
			case 4:
				if (func_170(iParam0, iParam1) && func_174(iParam0, iParam1))
				{
					return 1;
				}
				break;
			
			case 6:
				return unk_0xCE990E643CD9D0E5(Global_1701669[iParam0 /*68*/].f_18, 16);
			
			case 7:
				return unk_0xCE990E643CD9D0E5(Global_1701669[iParam0 /*68*/].f_18, 17);
			
			case 8:
				return unk_0xCE990E643CD9D0E5(Global_1701669[iParam0 /*68*/].f_18, 18);
			}
	}
	return 0;
}

int func_174(int iParam0, int iParam1)
{
	if (iParam0 != -1)
	{
		switch (iParam1)
		{
			case 1:
				return unk_0xCE990E643CD9D0E5(Global_1701669[iParam0 /*68*/].f_18, 9);
			
			case 2:
				return unk_0xCE990E643CD9D0E5(Global_1701669[iParam0 /*68*/].f_18, 11);
			
			case 3:
				return unk_0xCE990E643CD9D0E5(Global_1701669[iParam0 /*68*/].f_18, 13);
			
			case 4:
				return unk_0xCE990E643CD9D0E5(Global_1701669[iParam0 /*68*/].f_18, 15);
			}
		
		default:
	}
	return 0;
}

char* func_175(var uParam0, int iParam1, int iParam2)
{
	switch (iParam1)
	{
		case 0:
			switch (iParam2)
			{
				case 9:
					return "CH_DP_I_S_0";
				
				case 10:
					return "CH_DP_I_S_1";
				
				default:
			}
			break;
		
		case 1:
			switch (iParam2)
			{
				case 2:
					return "CH_DP_I_P_0";
				
				case 3:
					return "CH_DP_I_P_1";
				
				case 4:
					return "CH_DP_I_P_2";
				
				case 5:
					switch (func_164(uParam0->f_4356, 0))
					{
						case 1:
							switch (func_113(uParam0->f_4356))
							{
								case 1:
									switch (uParam0->f_744.f_739)
									{
										case 0:
											return "CH_D_I_P_3_0_aa";
										
										case 1:
											return "CH_D_I_P_3_0_ab";
										
										default:
									}
									break;
								
								case 2:
									switch (uParam0->f_744.f_739)
									{
										case 0:
											return "CH_D_I_P_3_0_ba";
										
										case 1:
											return "CH_D_I_P_3_0_bb";
										
										default:
									}
									break;
								
								case 3:
									switch (uParam0->f_744.f_739)
									{
										case 0:
											return "CH_D_I_P_3_0_ca";
										
										case 1:
											return "CH_D_I_P_3_0_cb";
										
										default:
									}
									break;
							}
							break;
						
						case 2:
							switch (func_113(uParam0->f_4356))
							{
								case 1:
									switch (uParam0->f_744.f_739)
									{
										case 0:
											return "CH_D_I_P_3_1_aa";
										
										case 1:
											return "CH_D_I_P_3_1_ab";
										
										default:
									}
									break;
								
								case 2:
									switch (uParam0->f_744.f_739)
									{
										case 0:
											return "CH_D_I_P_3_1_ba";
										
										case 1:
											return "CH_D_I_P_3_1_bb";
										
										default:
									}
									break;
								
								case 3:
									switch (uParam0->f_744.f_739)
									{
										case 0:
											return "CH_D_I_P_3_1_ca";
										
										case 1:
											return "CH_D_I_P_3_1_cb";
										
										default:
									}
									break;
							}
							break;
						
						case 3:
							switch (func_113(uParam0->f_4356))
							{
								case 1:
									switch (uParam0->f_744.f_739)
									{
										case 0:
											return "CH_D_I_P_3_2_aa";
										
										case 1:
											return "CH_D_I_P_3_2_ab";
										
										default:
									}
									break;
								
								case 2:
									switch (uParam0->f_744.f_739)
									{
										case 0:
											return "CH_D_I_P_3_2_ba";
										
										case 1:
											return "CH_D_I_P_3_2_bb";
										
										default:
									}
									break;
								
								case 3:
									switch (uParam0->f_744.f_739)
									{
										case 0:
											return "CH_D_I_P_3_2_ca";
										
										case 1:
											return "CH_D_I_P_3_2_cb";
										
										default:
									}
									break;
							}
							break;
						
						case 4:
							switch (func_113(uParam0->f_4356))
							{
								case 1:
									switch (uParam0->f_744.f_739)
									{
										case 0:
											return "CH_D_I_P_3_3_aa";
										
										case 1:
											return "CH_D_I_P_3_3_ab";
										
										default:
									}
									break;
								
								case 2:
									switch (uParam0->f_744.f_739)
									{
										case 0:
											return "CH_D_I_P_3_3_ba";
										
										case 1:
											return "CH_D_I_P_3_3_bb";
										
										default:
									}
									break;
								
								case 3:
									switch (uParam0->f_744.f_739)
									{
										case 0:
											return "CH_D_I_P_3_3_ca";
										
										case 1:
											return "CH_D_I_P_3_3_cb";
										
										default:
									}
									break;
							}
							break;
						
						case 5:
							switch (func_113(uParam0->f_4356))
							{
								case 1:
									switch (uParam0->f_744.f_739)
									{
										case 0:
											return "CH_D_I_P_3_4_aa";
										
										case 1:
											return "CH_D_I_P_3_4_ab";
										
										default:
									}
									break;
								
								case 2:
									switch (uParam0->f_744.f_739)
									{
										case 0:
											return "CH_D_I_P_3_4_ba";
										
										case 1:
											return "CH_D_I_P_3_4_bb";
										
										default:
									}
									break;
								
								case 3:
									switch (uParam0->f_744.f_739)
									{
										case 0:
											return "CH_D_I_P_3_4_ca";
										
										case 1:
											return "CH_D_I_P_3_4_cb";
										
										default:
									}
									break;
							}
							break;
					}
					break;
				
				case 6:
					return "CH_DP_I_P_4";
				
				case 7:
					return "CH_DP_I_P_5";
				
				case 9:
					return "CH_DP_I_P_6";
				
				case 13:
					switch (uParam0->f_744.f_742)
					{
						case 1:
							return "CH_DP_I_P_7a";
						
						case 2:
							return "CH_DP_I_P_7b";
						
						default:
					}
					break;
				
				case 14:
					switch (func_113(uParam0->f_4356))
					{
						case 1:
							return "CH_DP_I_P_8a";
						
						case 2:
							switch (uParam0->f_744.f_744)
							{
								case 1:
									return "CH_DP_I_P_8b_a";
								
								case 2:
									return "CH_DP_I_P_8b_b";
								
								case 3:
									return "CH_DP_I_P_8b_c";
								
								default:
							}
							break;
						
						case 3:
							return "CH_DP_I_P_8c";
					}
					break;
				
				case 15:
					switch (func_113(uParam0->f_4356))
					{
						case 1:
							return "CH_DP_I_P_9a";
						
						case 2:
							return "CH_DP_I_P_9b";
						
						case 3:
							return "CH_DP_I_P_9c";
						
						default:
					}
					break;
				
				case 16:
					switch (func_113(uParam0->f_4356))
					{
						case 1:
							return "CH_DP_I_P_10a";
						
						case 2:
							return "CH_DP_I_P_10b";
						
						case 3:
							return "CH_DP_I_P_10c";
						
						default:
					}
					break;
				
				case 17:
					switch (func_113(uParam0->f_4356))
					{
						case 1:
							return "CH_DP_I_P_11a";
						
						case 2:
							switch (uParam0->f_744.f_745)
							{
								case 6:
									return "CH_DP_I_P_11ba";
								
								case 7:
									return "CH_DP_I_P_11bb";
								
								default:
							}
							break;
						
						case 3:
							return "CH_DP_I_P_11c";
					}
					break;
				
				case 8:
					return "CH_DP_I_P_12";
				
				case 18:
					switch (func_113(uParam0->f_4356))
					{
						case 1:
						case 2:
							return "CH_DP_I_P_13";
						
						case 3:
							return "CH_DP_I_P_13b";
						
						default:
					}
					break;
			}
			break;
	}
	return "";
}

int func_176(char* sParam0)
{
	if ((unk_0xF005CCA4263DF67F(sParam0, "CH_DP_D_P_0b") || unk_0xF005CCA4263DF67F(sParam0, "CH_DP_D_P_8b_ac")) || unk_0xF005CCA4263DF67F(sParam0, "CH_DP_D_P_8b_bc"))
	{
		return 1;
	}
	return 0;
}

char* func_177(var uParam0, int iParam1, int iParam2)
{
	switch (iParam1)
	{
		case 0:
			switch (iParam2)
			{
				case 9:
					return "CH_DP_D_S_0";
				
				case 10:
					return "CH_DP_D_S_1";
				
				default:
			}
			break;
		
		case 1:
			switch (iParam2)
			{
				case 2:
					if (!func_180(uParam0->f_4356))
					{
						return "CH_DP_D_P_0b";
					}
					return "CH_DP_D_P_0";
				
				case 3:
					return "CH_DP_D_P_1";
				
				case 4:
					return "CH_DP_D_P_2";
				
				case 5:
					return "CH_DP_D_P_3";
				
				case 6:
					return "CH_DP_D_P_4";
				
				case 7:
					return "CH_DP_D_P_5";
				
				case 9:
					return "CH_DP_D_P_6";
				
				case 13:
					switch (uParam0->f_744.f_742)
					{
						case 1:
							return "CH_DP_D_P_7a";
						
						case 2:
							return "CH_DP_D_P_7b";
						
						default:
					}
					break;
				
				case 14:
					switch (func_113(uParam0->f_4356))
					{
						case 1:
							return "CH_DP_D_P_8a";
						
						case 2:
							switch (uParam0->f_744.f_744)
							{
								case 1:
									if (!func_179(uParam0->f_4356, 1))
									{
										return "CH_DP_D_P_8b_ac";
									}
									if (!func_170(uParam0->f_4356, 1))
									{
										return "CH_DP_D_P_8b_aa";
									}
									return "CH_DP_D_P_8b_ab";
								
								case 2:
									if (!func_179(uParam0->f_4356, 2))
									{
										return "CH_DP_D_P_8b_bc";
									}
									if (!func_170(uParam0->f_4356, 2))
									{
										return "CH_DP_D_P_8b_ba";
									}
									return "CH_DP_D_P_8b_bb";
								
								case 3:
									if (!func_170(uParam0->f_4356, 3))
									{
										return "CH_DP_D_P_8b_ca";
									}
									return "CH_DP_D_P_8b_cb";
								
								default:
							}
							break;
						
						case 3:
							if (func_178(uParam0->f_4356))
							{
								return "CH_DP_D_P_8cb";
							}
							return "CH_DP_D_P_8c";
					}
					break;
				
				case 15:
					switch (func_113(uParam0->f_4356))
					{
						case 1:
							return "CH_DP_D_P_9a";
						
						case 2:
							return "CH_DP_D_P_9b";
						
						case 3:
							return "CH_DP_D_P_9c";
						
						default:
					}
					break;
				
				case 16:
					switch (func_113(uParam0->f_4356))
					{
						case 1:
							return "CH_DP_D_P_10a";
						
						case 2:
							if (!func_170(uParam0->f_4356, 4))
							{
								return "CH_DP_D_P_10ba";
							}
							return "CH_DP_D_P_10bb";
						
						case 3:
							return "CH_DP_D_P_10c";
						
						default:
					}
					break;
				
				case 17:
					switch (func_113(uParam0->f_4356))
					{
						case 1:
							return "CH_DP_D_P_11a";
						
						case 2:
							switch (uParam0->f_744.f_745)
							{
								case 6:
									return "CH_DP_D_P_11b_a";
								
								case 7:
									return "CH_DP_D_P_11b_b";
								
								default:
							}
							break;
						
						case 3:
							return "CH_DP_D_P_11c";
					}
					break;
				
				case 8:
					return "CH_DP_D_P_12";
				
				case 18:
					return "CH_DP_D_P_13";
			}
			break;
	}
	return "";
}

int func_178(int iParam0)
{
	if (iParam0 != -1)
	{
		return unk_0xCE990E643CD9D0E5(Global_1701669[iParam0 /*68*/].f_18, 21);
	}
	return 0;
}

int func_179(int iParam0, int iParam1)
{
	if (iParam0 != -1)
	{
		return unk_0xCE990E643CD9D0E5(Global_1701669[iParam0 /*68*/].f_18.f_2, iParam1);
	}
	return 0;
}

int func_180(var uParam0)
{
	int iVar0;
	
	iVar0 = uParam0;
	if (iVar0 != -1)
	{
		if (((((unk_0xCE990E643CD9D0E5(Global_1701669[iVar0 /*68*/].f_61, 6) && unk_0xCE990E643CD9D0E5(Global_1701669[iVar0 /*68*/].f_61, 7)) && unk_0xCE990E643CD9D0E5(Global_1701669[iVar0 /*68*/].f_61, 8)) && unk_0xCE990E643CD9D0E5(Global_1701669[iVar0 /*68*/].f_61, 9)) && unk_0xCE990E643CD9D0E5(Global_1701669[iVar0 /*68*/].f_61, 10)) && unk_0xCE990E643CD9D0E5(Global_1701669[iVar0 /*68*/].f_61, 11))
		{
			return 1;
		}
	}
	return 0;
}

char* func_181(var uParam0, int iParam1, int iParam2)
{
	switch (iParam1)
	{
		case 0:
			switch (iParam2)
			{
				case 9:
					return "CH_DP_T_S_0";
				
				case 10:
					return "CH_DP_T_S_1";
				
				default:
			}
			break;
		
		case 1:
			switch (iParam2)
			{
				case 2:
					return "CH_DP_T_P_0";
				
				case 3:
					return "CH_DP_T_P_1";
				
				case 4:
					return "CH_DP_T_P_2";
				
				case 5:
					return "CH_DP_T_P_3";
				
				case 6:
					return "CH_DP_T_P_4";
				
				case 7:
					return "CH_DP_T_P_5";
				
				case 9:
					return "CH_DP_T_P_6";
				
				case 13:
					switch (uParam0->f_744.f_742)
					{
						case 1:
							return "CH_DP_T_P_7a";
						
						case 2:
							return "CH_DP_T_P_7b";
						
						default:
					}
					break;
				
				case 14:
					switch (func_113(uParam0->f_4356))
					{
						case 1:
							return "CH_DP_T_P_8a";
						
						case 2:
							switch (uParam0->f_744.f_744)
							{
								case 1:
									if (!func_170(uParam0->f_4356, 1))
									{
										return "CH_DP_T_P_8b_aa";
									}
									return "CH_DP_T_P_8b_ab";
								
								case 2:
									if (!func_170(uParam0->f_4356, 2))
									{
										return "CH_DP_T_P_8b_ba";
									}
									return "CH_DP_T_P_8b_bb";
								
								case 3:
									if (!func_170(uParam0->f_4356, 3))
									{
										return "CH_DP_T_P_8b_ca";
									}
									return "CH_DP_T_P_8b_cb";
								
								default:
							}
							break;
						
						case 3:
							if (func_178(uParam0->f_4356))
							{
								return "CH_DP_T_P_8cb";
							}
							return "CH_DP_T_P_8c";
					}
					break;
				
				case 15:
					switch (func_113(uParam0->f_4356))
					{
						case 1:
							return "CH_DP_T_P_9a";
						
						case 2:
							return "CH_DP_T_P_9b";
						
						case 3:
							return "CH_DP_T_P_9c";
						
						default:
					}
					break;
				
				case 16:
					switch (func_113(uParam0->f_4356))
					{
						case 1:
							return "CH_DP_T_P_10a";
						
						case 2:
							if (!func_170(uParam0->f_4356, 4))
							{
								return "CH_DP_T_P_10ba";
							}
							return "CH_DP_T_P_10bb";
						
						case 3:
							return "CH_DP_T_P_10c";
						
						default:
					}
					break;
				
				case 17:
					switch (func_113(uParam0->f_4356))
					{
						case 1:
							return "CH_DP_T_P_11a";
						
						case 2:
							switch (uParam0->f_744.f_745)
							{
								case 6:
									return "CH_DP_T_P_11b_a";
								
								case 7:
									return "CH_DP_T_P_11b_b";
								
								default:
							}
							break;
						
						case 3:
							return "CH_DP_T_P_11c";
					}
					break;
				
				case 8:
					return "CH_DP_T_P_12";
				
				case 18:
					return "CH_DP_T_P_13";
			}
			break;
	}
	return "";
}

int func_182(var uParam0)
{
	switch (uParam0->f_4355)
	{
		case 0:
			switch (uParam0->f_2593)
			{
				case 9:
				case 10:
					return 1;
				
				default:
			}
			break;
		
		case 1:
			switch (uParam0->f_2593)
			{
				case 2:
				case 3:
				case 4:
				case 5:
				case 6:
				case 7:
				case 9:
				case 13:
				case 14:
				case 15:
				case 16:
				case 17:
				case 18:
				case 8:
					return 1;
				
				default:
			}
			break;
	}
	return 0;
}

void func_183(int iParam0, char* sParam1, char* sParam2)
{
	Global_22550 = iParam0;
	func_184(29, sParam1, sParam2);
}

void func_184(int iParam0, char* sParam1, char* sParam2)
{
	StringCopy(&(Global_22550.f_6020[iParam0 /*16*/]), sParam1, 64);
	StringCopy(&(Global_22550.f_7029[iParam0 /*16*/]), sParam2, 64);
}

void func_185(bool bParam0, bool bParam1)
{
	int iVar0;
	int iVar1;
	float fVar2;
	
	iVar0 = 0;
	while (iVar0 < 256)
	{
		StringCopy(&(Global_22550.f_73[iVar0 /*6*/]), "", 24);
		iVar1 = 0;
		while (iVar1 < 4)
		{
			Global_22550.f_2124[iVar0 /*5*/][iVar1] = 0;
			iVar1++;
		}
		iVar0++;
	}
	iVar0 = 0;
	while (iVar0 < 32)
	{
		StringCopy(&(Global_2463019[iVar0 /*16*/]), "", 64);
		iVar0++;
	}
	iVar0 = 0;
	while (iVar0 < 256)
	{
		Global_22550.f_3918[iVar0] = 0;
		iVar0++;
	}
	iVar0 = 0;
	while (iVar0 < 128)
	{
		Global_22550.f_4175[iVar0] = 0f;
		iVar0++;
	}
	iVar0 = 0;
	while (iVar0 < 256)
	{
		Global_22550.f_4433[iVar0] = 0;
		iVar0++;
	}
	iVar0 = 0;
	while (iVar0 < 128)
	{
		Global_22550.f_5089[iVar0] = 0;
		Global_22550.f_5227[iVar0] = 0;
		Global_22550.f_5356[iVar0] = 0;
		Global_22550.f_5879[iVar0] = 0f;
		Global_22550.f_5485[iVar0] = 0;
		Global_22550.f_5745[iVar0] = 0f;
		iVar0++;
	}
	iVar0 = 0;
	while (iVar0 < 5)
	{
		Global_22550.f_5056[iVar0] = 0;
		Global_22550.f_5068[iVar0] = 0f;
		Global_22550.f_5062[iVar0] = -1f;
		Global_22550.f_5075[iVar0] = 0;
		Global_22550.f_5083[iVar0] = 1;
		iVar0++;
	}
	iVar0 = 0;
	while (iVar0 < 12)
	{
		StringCopy(&(Global_22550.f_4964[iVar0 /*4*/]), "", 16);
		Global_22550.f_5013[iVar0] = -1;
		Global_22550.f_5026[iVar0] = 361;
		Global_22550.f_5039[iVar0] = 32;
		iVar0++;
	}
	iVar0 = 0;
	while (iVar0 < 63)
	{
		StringCopy(&(Global_22550.f_6020[iVar0 /*16*/]), "", 64);
		StringCopy(&(Global_22550.f_7029[iVar0 /*16*/]), "", 64);
		iVar0++;
	}
	if (bParam1)
	{
		iVar0 = 0;
		while (iVar0 < 256)
		{
			Global_22550.f_1610[iVar0] = 0;
			iVar0++;
		}
	}
	StringCopy(&(Global_4269680.f_16), "", 16);
	Global_4269680.f_20 = -1;
	Global_22550 = 0;
	Global_22550.f_5218 = 0;
	Global_22550.f_5219 = 0;
	Global_22550.f_5220 = 0;
	Global_22550.f_5222 = 0;
	Global_22550.f_5223 = 0;
	Global_22550.f_5224 = 0;
	Global_22550.f_5221 = 0;
	Global_22550.f_5874 = 0;
	Global_22550.f_6014 = 0;
	Global_22550.f_5739 = 0;
	Global_22550.f_5738 = 0;
	Global_22550.f_5740 = 0;
	StringCopy(&(Global_22550.f_4690), "", 24);
	Global_22550.f_4762 = 0;
	Global_22550.f_4763 = 0;
	Global_22550.f_4764 = 0;
	Global_22550.f_4765 = 0;
	Global_22550.f_4766 = 0;
	Global_22550.f_4767 = 0;
	iVar0 = 0;
	while (iVar0 < 4)
	{
		Global_22550.f_4696[iVar0] = 0;
		iVar0++;
	}
	Global_22550.f_4768 = 0;
	StringCopy(&(Global_4269680.f_21), "", 16);
	Global_4269680.f_61 = 0;
	Global_4269680.f_62 = 0;
	Global_4269680.f_63 = 0;
	Global_4269680.f_64 = 0;
	Global_4269680.f_65 = 0;
	Global_4269680.f_66 = 0;
	iVar0 = 0;
	while (iVar0 < 4)
	{
		Global_4269680.f_25[iVar0] = 0;
		iVar0++;
	}
	Global_4269680.f_67 = 0;
	StringCopy(&(Global_22550.f_1), "", 16);
	Global_22550.f_5074 = 0f;
	Global_22550.f_68 = 0;
	Global_22550.f_69 = 0;
	Global_22550.f_70 = 0;
	Global_22550.f_71 = 0;
	Global_22550.f_72 = 0;
	iVar0 = 0;
	while (iVar0 < 4)
	{
		Global_22550.f_5[iVar0] = 0;
		iVar0++;
	}
	Global_22550.f_5744 = 0;
	Global_22550.f_5743 = 0;
	Global_22550.f_5741 = 0;
	Global_22550.f_5742 = 0;
	Global_22550.f_4769 = 0;
	Global_22550.f_4770 = 0;
	Global_22550.f_5225 = 10;
	Global_22550.f_5226 = 0;
	Global_22550.f_5876 = 0f;
	Global_22550.f_5877 = 0f;
	Global_22550.f_5728 = 0;
	Global_22550.f_5729 = 0;
	Global_22550.f_5730 = 0f;
	Global_22550.f_5731 = 0;
	Global_22550.f_5733 = 0;
	Global_22550.f_5732 = 0;
	Global_22550.f_5734 = 0;
	Global_22550.f_5735 = 0;
	Global_22550.f_5736 = 0;
	Global_22550.f_5737 = 0;
	Global_22550.f_8413 = 0;
	iVar0 = 0;
	while (iVar0 < 2)
	{
		Global_22550.f_6008[iVar0] = -1;
		Global_22550.f_6011[iVar0] = -1;
		iVar0++;
	}
	Global_22550.f_5081 = 0f;
	Global_22550.f_5052 = 0;
	Global_22550.f_5082 = 0;
	iVar0 = 0;
	while (iVar0 < Global_22550.f_6015)
	{
		Global_22550.f_6015[iVar0] = 0;
		iVar0++;
	}
	Global_22550.f_8392 = 0;
	Global_22550.f_8387 = 0;
	Global_22550.f_8397 = 0;
	Global_22550.f_8402 = 0;
	Global_22550.f_8407 = 0;
	Global_22550.f_8409 = 0;
	Global_22550.f_8415 = 0;
	Global_22547 = 0.05f;
	Global_22548 = 0.05f;
	Global_22549 = 0.225f;
	fVar2 = unk_0x8940B46482DD0A7D(0);
	if (bParam0)
	{
		Global_22549 = (0.225f * ((16f / 9f) / fVar2));
	}
	else if (fVar2 < 1.77777f)
	{
		Global_22549 = (0.225f * ((16f / 9f) / fVar2));
	}
	else
	{
		Global_22549 = 0.225f;
	}
}

bool func_186(char* sParam0, int iParam1, bool bParam2)
{
	int iVar0;
	bool bVar1;
	bool bVar2;
	
	if (!func_66(&iVar0, 1, iParam1))
	{
		return 0;
	}
	bVar1 = true;
	StringCopy(&(Global_22550.f_5635[iVar0 /*4*/]), sParam0, 16);
	if (!unk_0x7BCC91F3C1CF24E8(&(Global_22550.f_5635[iVar0 /*4*/])))
	{
		unk_0xBAA4D421B8B8A744(&(Global_22550.f_5635[iVar0 /*4*/]), 9);
		Global_22550.f_5628[iVar0] = 1;
		if (!unk_0xD77C5A1792C02DA1(&(Global_22550.f_5635[iVar0 /*4*/]), 9))
		{
			bVar1 = false;
		}
	}
	unk_0x494D5FF88119CDC0("CommonMenu", 0);
	Global_22550.f_5614[iVar0] = 1;
	if (!unk_0xC7F5A2E99D75A65C("CommonMenu"))
	{
		bVar1 = false;
	}
	if (bParam2)
	{
		unk_0x494D5FF88119CDC0("MPShopSale", 0);
		Global_22550.f_5621[iVar0] = 1;
		if (!unk_0xC7F5A2E99D75A65C("MPShopSale"))
		{
			bVar1 = false;
		}
	}
	bVar2 = false;
	StringCopy(&(Global_22550.f_5660[iVar0 /*10*/].f_1), "instructional_buttons", 24);
	bVar2 = func_187(&(Global_22550.f_5660[iVar0 /*10*/]));
	if (!bVar1 || !bVar2)
	{
	}
	return (bVar1 && bVar2);
}

bool func_187(var uParam0)
{
	switch (uParam0->f_9)
	{
		case 0:
			if (!unk_0x95EF219D38B20CFF(*uParam0))
			{
				*uParam0 = unk_0x5DEA4192B46CB99B(&(uParam0->f_1));
				uParam0->f_9 = 1;
				if (uParam0->f_7)
				{
					if (unk_0x95EF219D38B20CFF(*uParam0))
					{
						uParam0->f_8 = unk_0x578C4EF320340AF7();
						uParam0->f_9 = 2;
					}
				}
			}
			else
			{
				uParam0->f_8 = unk_0x578C4EF320340AF7();
				uParam0->f_9 = 2;
			}
			break;
		
		case 1:
			if (unk_0x95EF219D38B20CFF(*uParam0))
			{
				uParam0->f_8 = unk_0x578C4EF320340AF7();
				uParam0->f_9 = 2;
			}
			break;
		
		case 2:
			if (!unk_0x95EF219D38B20CFF(*uParam0))
			{
				uParam0->f_9 = 0;
			}
			break;
	}
	return uParam0->f_9 == 2;
}

void func_188(var uParam0)
{
	int iVar0;
	
	if (func_198(uParam0))
	{
		iVar0 = -1;
		switch (uParam0->f_4355)
		{
			case 0:
				func_197(uParam0, &iVar0);
				break;
			
			case 1:
				func_197(&(uParam0->f_744), &iVar0);
				break;
			
			case 2:
				func_197(&(uParam0->f_1497), &iVar0);
				break;
		}
		if (iVar0 != -1)
		{
			uParam0->f_2593.f_6 = uParam0->f_2593;
			uParam0->f_2593 = iVar0;
			func_196(uParam0, 0);
			if (uParam0->f_4356 == unk_0x460153A63B9477BC())
			{
				func_191(uParam0, func_192(uParam0));
			}
			switch (uParam0->f_4355)
			{
				case 0:
					func_190(uParam0, 1);
					break;
				
				case 1:
					func_190(&(uParam0->f_744), 1);
					if (uParam0->f_4356 == unk_0x460153A63B9477BC() && (((uParam0->f_2593 == 14 || uParam0->f_2593 == 15) || uParam0->f_2593 == 16) || uParam0->f_2593 == 17))
					{
						func_189(uParam0, 5);
						if (uParam0->f_2593 == 14 || uParam0->f_2593 == 15)
						{
							func_189(uParam0, 6);
						}
					}
					break;
				
				case 2:
					func_190(&(uParam0->f_1497), 1);
					break;
				}
			}
	}
}

void func_189(var uParam0, int iParam1)
{
	if (uParam0->f_4356 != unk_0x460153A63B9477BC())
	{
		return;
	}
	if (!unk_0xCE990E643CD9D0E5(uParam0->f_4353, iParam1))
	{
		unk_0xBE20AB8238688965(&(uParam0->f_4353), iParam1);
	}
}

void func_190(var uParam0, int iParam1)
{
	uParam0->f_706 = iParam1;
}

void func_191(var uParam0, int iParam1)
{
	if (iParam1 == -1 || iParam1 == 77)
	{
		return;
	}
	if (!func_31(uParam0, iParam1) && !unk_0xCE990E643CD9D0E5(uParam0->f_4173[(iParam1 / 32)], (iParam1 % 32)))
	{
		unk_0xBE20AB8238688965(&(uParam0->f_4173[(iParam1 / 32)]), (iParam1 % 32));
	}
}

int func_192(var uParam0)
{
	switch (uParam0->f_4355)
	{
		case 0:
			switch (uParam0->f_2593)
			{
				case 5:
					if (func_195(uParam0->f_4356) == 1)
					{
						return 63;
					}
					else if (func_194(uParam0->f_4356) == 1)
					{
						return 64;
					}
					return 4;
				
				case 6:
					if (func_195(uParam0->f_4356) == 2)
					{
						return 63;
					}
					else if (func_194(uParam0->f_4356) == 2)
					{
						return 64;
					}
					return 5;
				
				case 7:
					if (func_195(uParam0->f_4356) == 3)
					{
						return 63;
					}
					else if (func_194(uParam0->f_4356) == 3)
					{
						return 64;
					}
					return 6;
				
				case 10:
					return 1;
				
				default:
			}
			break;
		
		case 1:
			switch (uParam0->f_2593)
			{
				case 2:
					if (func_180(uParam0->f_4356))
					{
						return 26;
					}
					break;
			}
			break;
		
		case 2:
			if (unk_0xFC559366953F62B3())
			{
				if (func_193(uParam0->f_4356, 0))
				{
					switch (uParam0->f_2593)
					{
						case 12:
							return 74;
						
						default:
					}
				}
				else
				{
					switch (uParam0->f_2593)
					{
						case 8:
							return 59;
						
						case 9:
							return 59;
						
						case 10:
							return 59;
						
						case 11:
							return 59;
						
						case 12:
							return 60;
						
						case 15:
							switch (func_113(uParam0->f_4356))
							{
								case 2:
									if (uParam0->f_1497.f_1020 != 4)
									{
										return 54;
									}
									break;
							}
							break;
						}
				}
			}
			else
			{
				switch (uParam0->f_2593)
				{
					case 12:
						if (func_193(uParam0->f_4356, 0))
						{
							return 73;
						}
						return 58;
					}
				
				default:
			}
			break;
	}
	return -1;
}

int func_193(int iParam0, int iParam1)
{
	if (iParam0 != -1)
	{
		switch (iParam1)
		{
			case 0:
				return unk_0xCE990E643CD9D0E5(Global_1701669[iParam0 /*68*/].f_40, 10);
			
			case 1:
				return unk_0xCE990E643CD9D0E5(Global_1701669[iParam0 /*68*/].f_40, 11);
			
			case 2:
				return unk_0xCE990E643CD9D0E5(Global_1701669[iParam0 /*68*/].f_40, 12);
			
			case 3:
				return unk_0xCE990E643CD9D0E5(Global_1701669[iParam0 /*68*/].f_40, 13);
			}
		
		default:
	}
	return 0;
}

int func_194(int iParam0)
{
	if (iParam0 != -1)
	{
		return Global_1701669[iParam0 /*68*/].f_18.f_6;
	}
	return 0;
}

int func_195(int iParam0)
{
	if (iParam0 != -1)
	{
		return Global_1701669[iParam0 /*68*/].f_18.f_5;
	}
	return 0;
}

void func_196(var uParam0, int iParam1)
{
	uParam0->f_2593.f_9 = iParam1;
}

int func_197(var uParam0, int iParam1)
{
	if (uParam0->f_708 == 0)
	{
		unk_0x830F007E19C63E14(uParam0->f_709, "GET_CURRENT_SELECTION");
		uParam0->f_711 = unk_0xF4712E16A4C164FE();
		uParam0->f_708 = 1;
	}
	else if (unk_0x654BC7395AE9BF77(uParam0->f_711))
	{
		*iParam1 = unk_0x14E3913C604B1108(uParam0->f_711);
		uParam0->f_708 = 0;
		return 1;
	}
	*iParam1 = -1;
	return 0;
}

bool func_198(var uParam0)
{
	return uParam0->f_2593.f_9;
}

void func_199(var uParam0)
{
	func_221(uParam0);
	func_219(uParam0);
	func_204(uParam0);
	func_200(uParam0);
}

void func_200(var uParam0)
{
	if ((func_17(uParam0, 2) || (uParam0->f_4356 != func_42() && Global_1701669[uParam0->f_4356 /*68*/].f_67)) || func_202(unk_0x460153A63B9477BC()))
	{
		if (unk_0x03DB5C6AABF8DA46() && !unk_0x5DA78AD6801A0523())
		{
			if ((uParam0->f_724.f_5 || uParam0->f_744.f_724.f_5) || uParam0->f_1497.f_724.f_5)
			{
				func_201(uParam0, 0);
			}
		}
	}
}

void func_201(var uParam0, int iParam1)
{
	switch (uParam0->f_4355)
	{
		case 0:
			uParam0->f_724.f_5 = iParam1;
			uParam0->f_744.f_724.f_5 = 0;
			uParam0->f_1497.f_724.f_5 = 0;
			uParam0->f_4361 = 0;
			break;
		
		case 1:
			uParam0->f_724.f_5 = 0;
			uParam0->f_744.f_724.f_5 = iParam1;
			uParam0->f_1497.f_724.f_5 = 0;
			uParam0->f_4361 = 0;
			break;
		
		case 2:
			uParam0->f_724.f_5 = 0;
			uParam0->f_744.f_724.f_5 = 0;
			uParam0->f_1497.f_724.f_5 = iParam1;
			uParam0->f_4361 = 0;
			break;
	}
}

int func_202(int iParam0)
{
	if (iParam0 != func_42() && func_203(iParam0, 1, 1))
	{
		return unk_0xCE990E643CD9D0E5(Global_2426097[iParam0 /*443*/].f_314, 3);
	}
	return 0;
}

int func_203(int iParam0, bool bParam1, bool bParam2)
{
	int iVar0;
	
	iVar0 = iParam0;
	if (iVar0 != -1)
	{
		if (unk_0x166E920FB00B2DBB(iParam0))
		{
			if (bParam1)
			{
				if (!unk_0x5A228A6A51E757C8(iParam0))
				{
					return 0;
				}
			}
			if (bParam2)
			{
				if (!Global_2440277.f_3[iVar0])
				{
					return 0;
				}
			}
			return 1;
		}
	}
	return 0;
}

void func_204(var uParam0)
{
	if (unk_0xCE990E643CD9D0E5(uParam0->f_1497.f_1015, 0))
	{
		if (unk_0xCE990E643CD9D0E5(uParam0->f_1497.f_1015, 1))
		{
			func_206(&(Global_2452015.f_778), -1, 0, 1, 1);
			func_205(uParam0, 5);
			func_205(uParam0, 6);
			func_205(uParam0, 3);
			func_205(uParam0, 4);
			unk_0xD2459066EA58CE43(&(uParam0->f_1497.f_1015), 1);
		}
	}
	else if (unk_0xCE990E643CD9D0E5(uParam0->f_1497.f_1015, 1))
	{
		unk_0xD2459066EA58CE43(&(uParam0->f_1497.f_1015), 1);
	}
}

void func_205(var uParam0, int iParam1)
{
	unk_0xD2459066EA58CE43(&(uParam0->f_4357), iParam1);
}

void func_206(var uParam0, int iParam1, int iParam2, int iParam3, bool bParam4)
{
	if ((iParam1 != 0 && iParam1 != 3) || iParam2 == 1)
	{
		if (unk_0x40397A9A17EEC1C5(*uParam0))
		{
			unk_0x856549C07003344B(*uParam0, 0);
			if (bParam4)
			{
				unk_0x3ED9330214992278(0, 0, 3000, 1, 1, 0);
			}
			unk_0xB8B0F7C8C1548C5B(*uParam0, 0);
			*uParam0 = 0;
			func_218();
			if (func_216())
			{
				unk_0x701364DD9F501826();
			}
			unk_0xDF93DB0848FA3B5E(0);
			if (Global_1312390 == 0 && iParam3 == 1)
			{
				func_208(1, 0);
				func_208(1, 0);
			}
			if (!func_207())
			{
				if (unk_0xC6C08C02733D02C8() || unk_0xDCA0815D900F27F8())
				{
					if (!Global_2460706.f_7)
					{
						unk_0xEE377857147BD117();
					}
				}
				else
				{
					unk_0xEE377857147BD117();
				}
			}
		}
	}
}

bool func_207()
{
	return unk_0xCE990E643CD9D0E5(Global_1590682[unk_0x460153A63B9477BC() /*883*/].f_142, 0);
}

void func_208(bool bParam0, int iParam1)
{
	if (bParam0)
	{
		func_213(1, 0, 1);
	}
	else
	{
		func_209(iParam1);
	}
}

void func_209(int iParam0)
{
	func_212();
	if (iParam0 == 0)
	{
		if (unk_0x896F732C949E6864())
		{
			return;
		}
	}
	if (func_211() == 0 || unk_0xF0C12886E5C1B20E())
	{
		func_210(1);
		unk_0x882C03972C3A1AE2(0);
		unk_0x882C03972C3A1AE2(0);
	}
}

void func_210(int iParam0)
{
	if (Global_1312485.f_20 != iParam0)
	{
		Global_1312485.f_20 = iParam0;
	}
}

int func_211()
{
	return Global_1312485.f_20;
}

void func_212()
{
	Global_2464025 = 1;
}

void func_213(int iParam0, bool bParam1, bool bParam2)
{
	if (func_214())
	{
		return;
	}
	if ((func_211() == 1 || unk_0xF0C12886E5C1B20E()) || iParam0)
	{
		func_210(0);
		unk_0x882C03972C3A1AE2(1);
		if (!bParam1)
		{
			unk_0x882C03972C3A1AE2(1);
		}
		if (bParam2)
		{
			unk_0x9AFDC48A84E7D53D();
		}
	}
}

int func_214()
{
	if (func_215())
	{
		return Global_2462388;
	}
	return 0;
}

int func_215()
{
	if (Global_2462381)
	{
		return Global_2462380;
	}
	return 0;
}

int func_216()
{
	if (func_85(unk_0x460153A63B9477BC(), 0))
	{
		if (Global_1590682[unk_0x460153A63B9477BC() /*883*/].f_747 != func_42())
		{
			if (func_203(Global_1590682[unk_0x460153A63B9477BC() /*883*/].f_747, 0, 1))
			{
				if (func_217(Global_1590682[unk_0x460153A63B9477BC() /*883*/].f_747, 1, 0))
				{
					return 0;
				}
			}
		}
	}
	return 1;
}

int func_217(int iParam0, bool bParam1, bool bParam2)
{
	if (iParam0 == func_42())
	{
		return 0;
	}
	if (unk_0xCE990E643CD9D0E5(Global_1590682[iParam0 /*883*/].f_274.f_26, 0))
	{
		return 1;
	}
	if (bParam1)
	{
		if (unk_0xCE990E643CD9D0E5(Global_1590682[iParam0 /*883*/].f_274.f_26, 1))
		{
			return 1;
		}
	}
	if (bParam2)
	{
		if (Global_2426097[iParam0 /*443*/].f_314.f_6 != -1)
		{
			return 1;
		}
	}
	return 0;
}

void func_218()
{
	unk_0xD2459066EA58CE43(&Global_2452015, 15);
}

void func_219(var uParam0)
{
	if (unk_0xCE990E643CD9D0E5(Global_1701669[unk_0x1EF8FCAB83E4BE53() /*68*/].f_42, 1))
	{
		func_220();
	}
	if (func_15() && !unk_0xFC559366953F62B3())
	{
		func_220();
	}
	if ((func_17(uParam0, 3) || func_17(uParam0, 4)) && !func_17(uParam0, 6))
	{
		func_220();
	}
}

void func_220()
{
	int iVar0;
	int iVar1;
	
	iVar0 = 0;
	while (iVar0 < 32)
	{
		if (unk_0x166E920FB00B2DBB(unk_0xF1110FE23C67293A(iVar0)))
		{
			iVar1 = unk_0xF1110FE23C67293A(iVar0);
			if (iVar1 != func_42())
			{
				unk_0x8ECAF24A8B08CF58(iVar1, 0);
			}
		}
		iVar0++;
	}
}

void func_221(var uParam0)
{
	func_508(uParam0);
	if (func_507(uParam0))
	{
		if (!unk_0xCE990E643CD9D0E5(Global_1701669[unk_0x1EF8FCAB83E4BE53() /*68*/].f_42, 1))
		{
			unk_0xBE20AB8238688965(&(Global_1701669[unk_0x1EF8FCAB83E4BE53() /*68*/].f_42), 1);
		}
	}
	if (!func_481(uParam0))
	{
		func_480();
		return;
	}
	if (func_479())
	{
		if (!func_478())
		{
			func_477();
			return;
		}
		if (!func_476())
		{
			func_462(uParam0);
			return;
		}
		if (!func_461(1))
		{
			func_274(uParam0);
			return;
		}
	}
	else
	{
		if (!func_269(func_270()))
		{
			func_268();
			return;
		}
		if (!func_267(func_270()))
		{
			func_266();
			return;
		}
		if (!func_265(func_270()))
		{
			func_228();
			return;
		}
	}
	if (func_225())
	{
		uParam0->f_2604 = 1;
		func_222(uParam0, func_223());
		func_201(uParam0, 1);
		func_480();
	}
}

void func_222(var uParam0, int iParam1)
{
	uParam0->f_4355 = iParam1;
}

int func_223()
{
	if (func_224(0))
	{
		return 0;
	}
	else if (func_224(1))
	{
		return 1;
	}
	else if (func_224(2))
	{
		return 2;
	}
	return -1;
}

bool func_224(int iParam0)
{
	struct<3> Var0;
	struct<3> Var3;
	float fVar6;
	
	switch (iParam0)
	{
		case 0:
			Var0 = { 2711.031f, -366.9407f, -55.7809f };
			Var3 = { 2714.848f, -366.9614f, -53.7809f };
			fVar6 = 1.75f;
			break;
		
		case 1:
			Var0 = { 2715.558f, -371.5293f, -55.7809f };
			Var3 = { 2715.516f, -367.4599f, -53.7809f };
			fVar6 = 1.75f;
			break;
		
		case 2:
			Var0 = { 2715.26f, -371.9485f, -55.7809f };
			Var3 = { 2710.769f, -371.9542f, -53.7809f };
			fVar6 = 1.75f;
			break;
	}
	return unk_0x1037B9D45CE6B788(unk_0x9B0761B4C3EB8BC7(), Var0, Var3, fVar6, 0, 1, 0);
}

bool func_225()
{
	struct<2> Var0;
	
	switch (func_223())
	{
		case 0:
			StringCopy(&Var0, "CH_SETUP_USE", 16);
			break;
		
		case 1:
			StringCopy(&Var0, "CH_PREP_USE", 16);
			break;
		
		case 2:
			StringCopy(&Var0, "CH_FINALE_USE", 16);
			break;
	}
	if (!func_227(&Var0))
	{
		func_226(&Var0);
	}
	return unk_0x1BD7199394D7F19A(0, 51);
}

void func_226(char* sParam0)
{
	unk_0x17751E107423AFE9(sParam0);
	unk_0x1A169149B3D18FCB(0, 1, 1, -1);
}

bool func_227(char* sParam0)
{
	unk_0x69476B1CA6A9FF80(sParam0);
	return unk_0x1A3A9FFE5CA6F64B(0);
}

void func_228()
{
	if (func_270() != -1)
	{
		if (!func_264("CH_OWNER_SCOPE", unk_0x72C1D3A1AB8CAA5B(func_270())))
		{
			func_229("CH_OWNER_SCOPE", unk_0x72C1D3A1AB8CAA5B(func_270()), func_230(func_270(), -2, 0, 0, 0), -1);
		}
	}
}

void func_229(char* sParam0, var uParam1, int iParam2, int iParam3)
{
	unk_0x17751E107423AFE9(sParam0);
	if (!iParam2 == 0)
	{
		unk_0x7006FC1051C85FCA(iParam2);
	}
	unk_0x9EFD301E3BE8324E(uParam1);
	unk_0x1A169149B3D18FCB(0, 0, 1, iParam3);
}

int func_230(int iParam0, int iParam1, bool bParam2, bool bParam3, bool bParam4)
{
	int iVar0;
	var uVar1;
	
	if (func_262(iParam0) && !bParam4)
	{
		if (bParam2)
		{
			return 0;
		}
		else
		{
			return 1;
		}
	}
	if (iParam1 == -2)
	{
		iVar0 = unk_0x6CC163E30ECE0790(iParam0);
		if (iVar0 > -1 && iVar0 < 4)
		{
			if (Global_4456448.f_77785[iVar0] != -1)
			{
				iParam1 = iVar0;
			}
		}
	}
	if (((func_262(unk_0x460153A63B9477BC()) || (func_261() && func_260())) && !unk_0xCE990E643CD9D0E5(Global_2540612.f_4655, 31)) && !bParam4)
	{
		uVar1 = func_259();
		if (unk_0xAE06B9E39EBDE049(uVar1))
		{
			if (unk_0xD0390A93AF3907B8(uVar1))
			{
				if (unk_0xE4400E48E081F17A(uVar1) != -1)
				{
					if (func_203(unk_0xE4400E48E081F17A(uVar1), 0, 1))
					{
						if ((iParam1 > -1 && unk_0xFC559366953F62B3()) && iParam1 < 4)
						{
							if (Global_4456448.f_77785[iParam1] != -1)
							{
								return func_257(iParam1, iParam0, 0);
							}
							else
							{
								return func_243(iParam0, unk_0xE4400E48E081F17A(uVar1), iParam1, bParam2, bParam3);
							}
						}
						else
						{
							return func_243(iParam0, unk_0xE4400E48E081F17A(uVar1), iParam1, bParam2, bParam3);
						}
					}
				}
			}
			else if ((iParam1 > -1 && unk_0xFC559366953F62B3()) && iParam1 < 4)
			{
				if (Global_4456448.f_77785[iParam1] != -1)
				{
					return func_257(iParam1, iParam0, 0);
				}
				else
				{
					return func_231(0, -1, 0);
				}
			}
			else
			{
				return func_231(0, -1, 0);
			}
		}
	}
	if ((iParam1 > -1 && unk_0xFC559366953F62B3()) && iParam1 < 4)
	{
		if (Global_4456448.f_77785[iParam1] != -1)
		{
			return func_257(iParam1, iParam0, 0);
		}
		else
		{
			return func_243(iParam0, unk_0x460153A63B9477BC(), iParam1, bParam2, bParam3);
		}
	}
	return func_243(iParam0, unk_0x460153A63B9477BC(), iParam1, bParam2, bParam3);
}

int func_231(bool bParam0, int iParam1, bool bParam2)
{
	return func_232(unk_0x460153A63B9477BC(), bParam0, iParam1, bParam2);
}

int func_232(int iParam0, bool bParam1, int iParam2, bool bParam3)
{
	int iVar0;
	
	if (!unk_0x166E920FB00B2DBB(iParam0))
	{
		return 3;
	}
	iVar0 = unk_0x6CC163E30ECE0790(iParam0);
	if ((func_242() || (func_241() && func_239())) && Global_1388252.f_1)
	{
		if (bParam1)
		{
			return func_238(iParam2, iVar0);
		}
		else
		{
			return func_238(iVar0, iVar0);
		}
	}
	if (bParam1)
	{
		if (iParam2 > -1)
		{
			if (func_237(iVar0, iParam2, 0) && !unk_0xCE990E643CD9D0E5(Global_4456448.f_15, 18))
			{
				if (iVar0 == iParam2)
				{
					return func_236(1);
				}
				else
				{
					return func_236(0);
				}
			}
			else if (bParam3)
			{
				return 28;
			}
			else if (unk_0xCE990E643CD9D0E5(Global_4456448.f_4, 20))
			{
				return func_233(iVar0, iParam2, 1, 4);
			}
			else
			{
				return func_233(iVar0, iParam2, 0, 4);
			}
		}
		return 28;
	}
	if (iVar0 == iParam2 || iParam2 == -1)
	{
		return func_236(1);
	}
	return func_236(0);
}

int func_233(int iParam0, int iParam1, bool bParam2, int iParam3)
{
	int iVar0;
	
	iVar0 = func_235(iParam0, iParam1, iParam3);
	if (func_234(Global_4456448.f_82708, 1))
	{
		if (iVar0 == 1)
		{
			iVar0 = 0;
		}
	}
	if (bParam2)
	{
		switch (iVar0)
		{
			case 0:
				return 28;
			
			case 1:
				return 29;
			
			case 2:
				return 30;
			
			case 3:
				return 31;
			
			case 4:
				return 32;
			
			case 5:
				return 33;
			
			case 6:
				return 34;
			
			case 7:
				return 35;
			
			case 8:
				return 36;
			
			case 9:
				return 37;
			
			case 10:
				return 38;
			
			case 11:
				return 39;
			
			case 12:
				return 40;
			
			case 13:
				return 41;
			
			case 14:
				return 42;
			
			case 15:
				return 43;
			
			default:
		}
	}
	else
	{
		switch (iVar0)
		{
			case 0:
				return 28;
			
			case 1:
				return 29;
			
			case 2:
				return 30;
			}
		
		default:
	}
	return 28;
}

int func_234(int iParam0, bool bParam1)
{
	int iVar0;
	
	if (bParam1)
	{
		if (Global_4456448.f_129348 == 65)
		{
			return 1;
		}
	}
	if (iParam0 == 0)
	{
		return 0;
	}
	iVar0 = 0;
	while (iVar0 <= 6)
	{
		if (iParam0 == Global_262145.f_9021[iVar0])
		{
			return 1;
		}
		iVar0++;
	}
	return 0;
}

int func_235(int iParam0, int iParam1, int iParam2)
{
	int iVar0;
	int iVar1;
	
	iVar0 = 0;
	while (iVar0 < iParam2)
	{
		if (iVar0 == iParam1)
		{
			return iVar1;
		}
		else if (!iParam0 == iVar0)
		{
			if (!func_237(iParam0, iVar0, 0))
			{
				iVar1++;
			}
		}
		iVar0++;
	}
	return -1;
}

int func_236(bool bParam0)
{
	if (bParam0)
	{
		return 118;
	}
	return 116;
}

int func_237(int iParam0, int iParam1, int iParam2)
{
	if (iParam2 == 1)
	{
		if (iParam0 == iParam1)
		{
			return 1;
		}
		return 0;
	}
	if ((iParam0 > -1 && iParam1 > -1) && iParam0 == iParam1)
	{
		return 1;
	}
	switch (iParam0)
	{
		case 0:
			switch (iParam1)
			{
				case 0:
					return unk_0xCE990E643CD9D0E5(Global_4456448.f_541, 0);
				
				case 1:
					return unk_0xCE990E643CD9D0E5(Global_4456448.f_541, 1);
				
				case 2:
					return unk_0xCE990E643CD9D0E5(Global_4456448.f_541, 2);
				
				case 3:
					return unk_0xCE990E643CD9D0E5(Global_4456448.f_541, 3);
				
				default:
			}
			break;
		
		case 1:
			switch (iParam1)
			{
				case 0:
					return unk_0xCE990E643CD9D0E5(Global_4456448.f_541, 4);
				
				case 1:
					return unk_0xCE990E643CD9D0E5(Global_4456448.f_541, 5);
				
				case 2:
					return unk_0xCE990E643CD9D0E5(Global_4456448.f_541, 6);
				
				case 3:
					return unk_0xCE990E643CD9D0E5(Global_4456448.f_541, 7);
				
				default:
			}
			break;
		
		case 2:
			switch (iParam1)
			{
				case 0:
					return unk_0xCE990E643CD9D0E5(Global_4456448.f_541, 8);
				
				case 1:
					return unk_0xCE990E643CD9D0E5(Global_4456448.f_541, 9);
				
				case 2:
					return unk_0xCE990E643CD9D0E5(Global_4456448.f_541, 10);
				
				case 3:
					return unk_0xCE990E643CD9D0E5(Global_4456448.f_541, 11);
				
				default:
			}
			break;
		
		case 3:
			switch (iParam1)
			{
				case 0:
					return unk_0xCE990E643CD9D0E5(Global_4456448.f_541, 12);
				
				case 1:
					return unk_0xCE990E643CD9D0E5(Global_4456448.f_541, 13);
				
				case 2:
					return unk_0xCE990E643CD9D0E5(Global_4456448.f_541, 14);
				
				case 3:
					return unk_0xCE990E643CD9D0E5(Global_4456448.f_541, 15);
				
				default:
			}
			break;
	}
	return 0;
}

int func_238(int iParam0, int iParam1)
{
	if (iParam0 == -1)
	{
		iParam0 = func_235(iParam1, iParam0, 4);
	}
	switch (iParam0)
	{
		case 0:
			return 15;
		
		case 1:
			return 21;
		
		case 2:
			return 24;
		
		case 3:
			return 18;
		
		default:
	}
	return 28;
}

bool func_239()
{
	if (func_240())
	{
		return 1;
	}
	return unk_0xCE990E643CD9D0E5(Global_4456448.f_133494, 4);
}

bool func_240()
{
	return unk_0xCE990E643CD9D0E5(Global_4456448.f_123318, 12);
}

bool func_241()
{
	if (unk_0xFC559366953F62B3())
	{
		return unk_0xCE990E643CD9D0E5(Global_4456448.f_133494, 0);
	}
	return ((unk_0xCE990E643CD9D0E5(Global_4456448.f_133494, 0) || Global_1655244) && unk_0x7B70881748D166CD(joaat("fm_deathmatch_creator")) > 0);
}

int func_242()
{
	if (func_240() && unk_0xFC559366953F62B3())
	{
		return 1;
	}
	return 0;
}

int func_243(int iParam0, int iParam1, int iParam2, bool bParam3, bool bParam4)
{
	int iVar0;
	bool bVar1;
	int iVar2;
	int iVar3;
	int iVar4;
	
	if (iParam2 == -2)
	{
		iVar0 = unk_0x6CC163E30ECE0790(iParam0);
	}
	else
	{
		iVar0 = iParam2;
	}
	if (Global_1590682[unk_0x460153A63B9477BC() /*883*/] == 148)
	{
		bVar1 = true;
	}
	iVar2 = iParam0;
	if (iVar2 > -1)
	{
		if (Global_1590682[iVar2 /*883*/] == 148)
		{
			bVar1 = true;
		}
	}
	if (!bVar1)
	{
		if (iVar0 != -1)
		{
			if (func_251())
			{
				iVar3 = func_248(iParam0);
				if (!iVar3 == -1)
				{
					return func_246(iVar3);
				}
			}
			if ((func_245(iParam1, iParam0, iVar0, 0) && !unk_0xCE990E643CD9D0E5(Global_4456448.f_15, 18)) || ((func_237(unk_0x6CC163E30ECE0790(iParam1), unk_0x6CC163E30ECE0790(iParam0), 0) && unk_0xCE990E643CD9D0E5(Global_4456448.f_15, 23)) && !unk_0xCE990E643CD9D0E5(Global_4456448.f_15, 18)))
			{
				return func_236(1);
			}
			else if (unk_0xCE990E643CD9D0E5(Global_4456448.f_15, 26))
			{
				return func_244(1);
			}
			else
			{
				return func_232(iParam1, 1, iVar0, bParam4);
			}
		}
		else if ((Global_1574416 || Global_1574407) || Global_1590682[iParam0 /*883*/] == 0)
		{
			if (iParam0 == iParam1 || (Global_1574416 == 1 && Global_1574426 == 0))
			{
				return func_236(1);
			}
			else
			{
				return func_232(iParam1, 1, iVar0, bParam4);
			}
		}
		if (Global_1574411 && Global_1573907.f_14 == iParam0)
		{
			return 28;
		}
	}
	iVar4 = func_248(iParam0);
	if (!iVar4 == -1)
	{
		return func_246(iVar4);
	}
	if (bParam3)
	{
		return 0;
	}
	return 1;
}

int func_244(bool bParam0)
{
	if (bParam0)
	{
		return 119;
	}
	return 116;
}

bool func_245(int iParam0, int iParam1, int iParam2, int iParam3)
{
	if (iParam2 == -2)
	{
		if (iParam3 == 0)
		{
			if (unk_0x6CC163E30ECE0790(iParam0) == -1 && unk_0x6CC163E30ECE0790(iParam1) == -1)
			{
				return 0;
			}
		}
		return unk_0x6CC163E30ECE0790(iParam0) == unk_0x6CC163E30ECE0790(iParam1);
	}
	else
	{
		if (iParam3 == 0)
		{
			if (unk_0x6CC163E30ECE0790(iParam0) == -1 && iParam2 == -1)
			{
				return 0;
			}
		}
		return unk_0x6CC163E30ECE0790(iParam0) == iParam2;
	}
	return unk_0x6CC163E30ECE0790(iParam0) == iParam2;
}

int func_246(int iParam0)
{
	int iVar0;
	
	if (iParam0 > -1)
	{
		iVar0 = func_247(iParam0);
		switch (iVar0)
		{
			case 0:
				return 192;
			
			case 1:
				return 193;
			
			case 2:
				return 194;
			
			case 3:
				return 195;
			
			case 4:
				return 196;
			
			case 5:
				return 197;
			
			case 6:
				return 198;
			
			case 7:
				return 199;
			
			case 8:
				return 200;
			
			case 9:
				return 201;
			
			case 10:
				return 202;
			
			case 11:
				return 203;
			
			case 12:
				return 204;
			
			case 13:
				return 205;
			
			case 14:
				return 206;
			}
		
		default:
	}
	return 1;
}

var func_247(int iParam0)
{
	return Global_2417956.f_818.f_44[iParam0 /*2*/].f_1;
}

int func_248(int iParam0)
{
	if (!iParam0 == func_42())
	{
		if (func_250(iParam0, 1))
		{
			return Global_2417956.f_818.f_11[func_249(iParam0)];
		}
	}
	return -1;
}

int func_249(int iParam0)
{
	if (iParam0 != func_42())
	{
		return Global_1630317[iParam0 /*595*/].f_11;
	}
	return func_42();
}

bool func_250(int iParam0, bool bParam1)
{
	if (!bParam1)
	{
		if (func_269(iParam0))
		{
			return 0;
		}
	}
	return Global_1630317[iParam0 /*595*/].f_11 != func_42();
}

int func_251()
{
	if ((((func_256() || func_255()) || func_254()) || func_253()) || func_252())
	{
		return 1;
	}
	if (unk_0xFC559366953F62B3() && unk_0xCE990E643CD9D0E5(Global_4456448.f_36, 1))
	{
		return 1;
	}
	return 0;
}

var func_252()
{
	return Global_2452015.f_23;
}

var func_253()
{
	return Global_2452015.f_20;
}

var func_254()
{
	return Global_2452015.f_18;
}

var func_255()
{
	return Global_2452015.f_17;
}

var func_256()
{
	return Global_2452015.f_16;
}

int func_257(int iParam0, int iParam1, bool bParam2)
{
	int iVar0;
	int iVar1;
	int iVar2;
	
	iVar1 = Global_969055.f_14[iParam0];
	if (func_251())
	{
		iVar2 = func_248(iParam1);
		if (!iVar2 == -1)
		{
			return func_246(iVar2);
		}
	}
	if (iVar1 > -1 && iVar1 < 17)
	{
		if (unk_0xCE990E643CD9D0E5(Global_4456448.f_592[iParam0 /*16355*/].f_7079[iVar1], 24))
		{
			return 18;
		}
	}
	if (iParam0 > -1 && iParam1 != func_42())
	{
		if (Global_4456448.f_77785[iParam0] != -1 && Global_4456448.f_77785[iParam0] <= 4)
		{
			if (Global_4456448.f_77785[iParam0] == 0)
			{
				iVar0 = 15;
			}
			else if (Global_4456448.f_77785[iParam0] == 1)
			{
				iVar0 = 18;
			}
			else if (Global_4456448.f_77785[iParam0] == 2)
			{
				iVar0 = 24;
			}
			else if (Global_4456448.f_77785[iParam0] == 4)
			{
				if (unk_0xCE990E643CD9D0E5(Global_4456448.f_15, 29))
				{
					iVar0 = 21;
				}
				else
				{
					iVar0 = 6;
				}
			}
			else
			{
				iVar0 = Global_4456448.f_77785[iParam0];
			}
		}
		else
		{
			iVar0 = func_232(iParam1, !bParam2, iParam0, 0);
		}
		if (unk_0xCE990E643CD9D0E5(Global_4456448.f_21, 13))
		{
			iVar0 = func_258(iParam0);
		}
		if (unk_0xCE990E643CD9D0E5(Global_4456448.f_24, 29))
		{
			iVar0 = 0;
		}
		if (unk_0xCE990E643CD9D0E5(Global_4456448.f_15, 26) && !func_237(iParam0, unk_0x6CC163E30ECE0790(iParam1), 0))
		{
			iVar0 = func_244(1);
		}
	}
	else
	{
		iVar0 = 1;
	}
	return iVar0;
}

int func_258(int iParam0)
{
	int iVar0;
	
	switch (iParam0)
	{
		case 0:
			iVar0 = Global_4456448.f_129476;
			break;
		
		case 1:
			iVar0 = Global_4456448.f_129477;
			break;
		
		case 2:
			iVar0 = Global_4456448.f_129478;
			break;
		
		case 3:
			iVar0 = Global_4456448.f_129479;
			break;
	}
	switch (iVar0)
	{
		case 0:
			return 15;
		
		case 1:
			return 21;
		
		case 2:
			return 24;
		
		case 3:
			return 18;
		
		case 4:
			return 6;
		
		case 5:
			return 9;
		
		case 6:
			return 3;
		
		case 7:
			return 1;
		
		case 8:
			return 12;
		
		case 9:
			return 2;
		
		default:
	}
	return 2;
}

var func_259()
{
	return Global_2359302.f_2;
}

bool func_260()
{
	return unk_0xCE990E643CD9D0E5(Global_2359302, 4);
}

bool func_261()
{
	return unk_0xCE990E643CD9D0E5(Global_1590682[unk_0x460153A63B9477BC() /*883*/].f_39.f_18, 14);
}

int func_262(int iParam0)
{
	if (func_85(iParam0, 0))
	{
		return 1;
	}
	if (func_263())
	{
		if (iParam0 == unk_0x460153A63B9477BC())
		{
			return 1;
		}
	}
	if (unk_0xCE990E643CD9D0E5(Global_2426097[iParam0 /*443*/].f_199, 2))
	{
		return 1;
	}
	return 0;
}

bool func_263()
{
	return unk_0xCE990E643CD9D0E5(Global_2359302, 3);
}

bool func_264(char* sParam0, var uParam1)
{
	unk_0x69476B1CA6A9FF80(sParam0);
	unk_0x9EFD301E3BE8324E(uParam1);
	return unk_0x1A3A9FFE5CA6F64B(0);
}

int func_265(int iParam0)
{
	if (iParam0 != -1)
	{
		return unk_0xCE990E643CD9D0E5(Global_1701669[iParam0 /*68*/].f_40, 4);
	}
	return 0;
}

void func_266()
{
	if (func_270() != -1)
	{
		if (!func_264("CH_OWNER_SETUP", unk_0x72C1D3A1AB8CAA5B(func_270())))
		{
			func_229("CH_OWNER_SETUP", unk_0x72C1D3A1AB8CAA5B(func_270()), func_230(func_270(), -2, 0, 0, 0), -1);
		}
	}
}

int func_267(int iParam0)
{
	if (iParam0 != -1)
	{
		return unk_0xCE990E643CD9D0E5(Global_1701669[iParam0 /*68*/].f_40, 0);
	}
	return 0;
}

void func_268()
{
	if (func_270() != -1)
	{
		if (!func_264("CH_SETUP_REG2", unk_0x72C1D3A1AB8CAA5B(func_270())))
		{
			func_229("CH_SETUP_REG2", unk_0x72C1D3A1AB8CAA5B(func_270()), func_230(func_270(), -2, 0, 0, 0), -1);
		}
	}
}

int func_269(int iParam0)
{
	if (iParam0 != func_42())
	{
		if (Global_1630317[iParam0 /*595*/].f_11 != func_42())
		{
			return Global_1630317[iParam0 /*595*/].f_11 == iParam0;
		}
	}
	return 0;
}

int func_270()
{
	return func_271(unk_0x460153A63B9477BC());
}

int func_271(int iParam0)
{
	int iVar0;
	
	if (iParam0 == func_42())
	{
		return iParam0;
	}
	if (func_273(iParam0) != -1)
	{
		iVar0 = func_272(func_273(iParam0));
		if ((iVar0 == 2 || iVar0 == 1) || iVar0 == 0)
		{
			if (func_250(iParam0, 0))
			{
				return func_249(iParam0);
			}
			return iParam0;
		}
		else if (iVar0 == 3)
		{
			return func_42();
		}
		return Global_2426097[iParam0 /*443*/].f_314.f_9;
	}
	return func_42();
}

int func_272(int iParam0)
{
	switch (iParam0)
	{
		case 0:
		case 1:
		case 2:
		case 3:
		case 4:
		case 5:
		case 6:
		case 7:
		case 8:
		case 9:
		case 10:
		case 11:
		case 12:
		case 13:
		case 14:
		case 15:
		case 16:
		case 17:
		case 18:
		case 19:
		case 20:
		case 21:
			return 0;
			break;
		
		case 60:
		case 61:
		case 62:
		case 63:
		case 64:
		case 65:
		case 66:
		case 67:
		case 68:
		case 69:
			return 1;
			break;
		
		case 22:
		case 23:
		case 24:
		case 25:
		case 26:
		case 27:
		case 28:
		case 29:
		case 30:
		case 31:
		case 32:
		case 33:
		case 34:
		case 35:
		case 36:
		case 37:
		case 38:
		case 39:
		case 40:
		case 41:
			return 2;
			break;
		
		case 43:
		case 42:
		case 44:
		case 45:
		case 46:
		case 47:
		case 48:
		case 49:
		case 50:
		case 51:
		case 52:
		case 53:
		case 54:
		case 55:
		case 56:
		case 57:
		case 58:
		case 59:
		case 98:
		case 99:
		case 100:
		case 112:
		case 113:
		case 114:
		case 115:
		case 119:
		case 116:
		case 118:
		case 120:
		case 121:
		case 126:
		case 127:
		case 134:
		case 135:
		case 136:
		case 137:
		case 138:
		case 139:
		case 140:
		case 141:
		case 142:
		case 143:
		case 144:
			return 3;
			break;
		
		case 70:
		case 71:
		case 72:
		case 73:
		case 74:
		case 75:
		case 76:
		case 77:
		case 78:
		case joaat("mpsv_lp0_31"):
		case 80:
			return 4;
			break;
		
		case 81:
			return 5;
			break;
		
		case 82:
			return 6;
			break;
		
		case 83:
		case 84:
		case 85:
		case 86:
		case 87:
			return 7;
			break;
		
		case 88:
			return 8;
			break;
		
		case 89:
		case 90:
		case 91:
		case 92:
		case 93:
		case 94:
		case 95:
		case 96:
		case 97:
			return 9;
			break;
		
		case 101:
			return 10;
			break;
		
		case 102:
		case 103:
		case 104:
		case 105:
		case 106:
		case 107:
		case 108:
		case 109:
		case 110:
		case 111:
			return 11;
			break;
		
		case 117:
			return 12;
			break;
		
		case 122:
			return 13;
			break;
		
		case 123:
			return 14;
			break;
		
		case 124:
			return 15;
			break;
		
		case 125:
			return 16;
			break;
		
		case 128:
		case 129:
		case 130:
		case 131:
		case 132:
		case 133:
			return 17;
			break;
		
		case 145:
			return 18;
			break;
		
		case 146:
			return 19;
			break;
		
		case 147:
			return 20;
			break;
	}
	return -1;
}

int func_273(int iParam0)
{
	if (iParam0 != func_42())
	{
		if (func_203(iParam0, 1, 1))
		{
			return Global_2426097[iParam0 /*443*/].f_314.f_6;
		}
		else if (((Global_1312896 || Global_2405074.f_2673) && iParam0 == unk_0x460153A63B9477BC()) && func_203(iParam0, 1, 0))
		{
			return Global_2426097[iParam0 /*443*/].f_314.f_6;
		}
	}
	return -1;
}

void func_274(var uParam0)
{
	if (func_460(0))
	{
		if (func_17(uParam0, 2))
		{
			func_480();
			return;
		}
		if (!func_227("CH_SETUP_SCOPE"))
		{
			func_226("CH_SETUP_SCOPE");
		}
		if (unk_0x1BD7199394D7F19A(0, 51))
		{
			func_445(1);
			func_443();
			func_442(uParam0, 2);
			func_480();
			unk_0xC4CC25B68A91D144(-1, "Highlight_Accept", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
		}
	}
	else
	{
		switch (func_278(unk_0x460153A63B9477BC(), 158, 0))
		{
			case 15:
				if (func_276(unk_0x460153A63B9477BC()))
				{
					func_10("GENERAL_MLF_G1", -1);
				}
				else if (func_275(unk_0x460153A63B9477BC()))
				{
					func_10("GENERAL_MLF_G2", -1);
				}
				else
				{
					func_10("GENERAL_MLF_G3", -1);
				}
				break;
			
			case 16:
				if (func_276(unk_0x460153A63B9477BC()))
				{
					func_10("GENERAL_MLF_G4", -1);
				}
				else if (func_275(unk_0x460153A63B9477BC()))
				{
					func_10("GENERAL_MLF_G5", -1);
				}
				else
				{
					func_10("GENERAL_MLF_G6", -1);
				}
				break;
			}
	}
}

bool func_275(int iParam0)
{
	if (iParam0 == -1)
	{
		return 0;
	}
	return unk_0xCE990E643CD9D0E5(Global_1590682[iParam0 /*883*/].f_274.f_26, 26);
}

bool func_276(int iParam0)
{
	return func_277(iParam0, 1);
}

int func_277(int iParam0, int iParam1)
{
	if (iParam0 != func_42())
	{
		if (Global_1630317[iParam0 /*595*/].f_11 != func_42())
		{
			if (Global_1630317[iParam0 /*595*/].f_11 == iParam0 && Global_1630317[iParam0 /*595*/].f_11.f_423 == iParam1)
			{
				return 1;
			}
		}
	}
	return 0;
}

int func_278(int iParam0, int iParam1, bool bParam2)
{
	int iVar0;
	int iVar1;
	
	if (!func_309(func_414(iParam1, -1, -1, -1), func_371(iParam1, -1, 1, -1, -1, iParam0), func_318(iParam1, -1, -1, -1, -1, iParam0), 0, 1))
	{
		return 0;
	}
	iVar0 = iParam1;
	if (func_276(iParam0))
	{
		if (iParam1 == 153)
		{
			iVar0 = 211;
		}
	}
	if (func_307(iParam1) >= func_306(iVar0))
	{
		return 5;
	}
	if (func_305(iParam0))
	{
		return 6;
	}
	if (func_304(iParam0))
	{
		return 14;
	}
	iVar1 = func_299(iParam0);
	if (iVar1 != -1)
	{
		return iVar1;
	}
	if (func_298(iParam1) == 1)
	{
		if ((func_297() || func_296()) || ((!bParam2 && func_295() != 0) && func_298(iParam1) == 1))
		{
			return 2;
		}
	}
	else if (func_298(iParam1) == 2)
	{
		if (!func_292(iParam1))
		{
			return 3;
		}
		if (!bParam2)
		{
			if (Global_1630317[iParam0 /*595*/].f_11.f_34 != func_42())
			{
				return 9;
			}
		}
		if (func_291(iParam0))
		{
			return 10;
		}
	}
	else if (func_289(iParam1))
	{
		if (func_288(iParam0) <= 0)
		{
			return 4;
		}
	}
	if (func_287(iParam1))
	{
		if (func_286())
		{
			return 13;
		}
		if (func_284())
		{
			return 12;
		}
	}
	if (func_288(iParam0) + 1 < func_283(iVar0))
	{
		return 4;
	}
	switch (iParam1)
	{
		case 157:
			break;
		
		case 152:
		case 164:
		case 173:
		case 170:
		case 201:
		case 200:
			if (!func_280(iParam0, 0))
			{
				return 1;
			}
			break;
		
		case 189:
			if (func_279(iParam0, 21) || func_279(iParam0, 25))
			{
				return 0;
			}
			break;
	}
	return -1;
}

bool func_279(int iParam0, int iParam1)
{
	return unk_0xCE990E643CD9D0E5(Global_2426097[iParam0 /*443*/].f_212, iParam1);
}

int func_280(int iParam0, int iParam1)
{
	int iVar0;
	int iVar1;
	
	iVar0 = 0;
	while (iVar0 < 32)
	{
		iVar1 = unk_0xF1110FE23C67293A(iVar0);
		if (unk_0x166E920FB00B2DBB(iVar1))
		{
			if (!func_281(iVar1, iParam0, 1))
			{
				if (iParam1 || !func_41(iVar1, 31))
				{
					return 1;
				}
			}
		}
		iVar0++;
	}
	return 0;
}

int func_281(int iParam0, int iParam1, bool bParam2)
{
	if (iParam1 != func_42())
	{
		if (!bParam2)
		{
			if (func_282(iParam0, iParam1))
			{
				return 0;
			}
		}
		if (Global_1630317[iParam0 /*595*/].f_11 != func_42())
		{
			return iParam1 == Global_1630317[iParam0 /*595*/].f_11;
		}
	}
	return 0;
}

int func_282(int iParam0, int iParam1)
{
	if (iParam1 != func_42())
	{
		if (iParam0 != func_42())
		{
			if (Global_1630317[iParam0 /*595*/].f_11 != func_42())
			{
				if (Global_1630317[iParam0 /*595*/].f_11 == iParam0)
				{
					return iParam1 == iParam0;
				}
			}
		}
	}
	return 0;
}

int func_283(int iParam0)
{
	switch (iParam0)
	{
		case 180:
			return Global_262145.f_18171;
		
		case 183:
			return Global_262145.f_18179;
		
		case 185:
			return Global_262145.f_18188;
		
		case 182:
			return Global_262145.f_18199;
		
		case 186:
			return Global_262145.f_18208;
		
		case 195:
			return Global_262145.f_18226;
		
		case 198:
			return Global_262145.f_18236;
		
		case 197:
			return Global_262145.f_18248;
		
		case 207:
			return Global_262145.f_18256;
		
		case 209:
			return Global_262145.f_18269;
		
		case 208:
			return Global_262145.f_18277;
		
		case 201:
			return Global_262145.f_18310;
		
		case 211:
			return Global_262145.f_18378;
		
		case 193:
			return Global_262145.f_18401;
		
		case 205:
			return Global_262145.f_18419;
		
		case 189:
			return Global_262145.f_18384;
		
		case 216:
			return 2;
		
		case 220:
			return 2;
		
		default:
	}
	return 1;
}

int func_284()
{
	if (func_285())
	{
		return 1;
	}
	return Global_2452015.f_742;
}

bool func_285()
{
	return Global_1312855 == 10;
}

bool func_286()
{
	return Global_262145.f_15228;
}

int func_287(int iParam0)
{
	switch (iParam0)
	{
		case 167:
		case 169:
		case 168:
			return 1;
		
		default:
	}
	return 0;
}

int func_288(int iParam0)
{
	if (func_249(iParam0) == func_42())
	{
		return 0;
	}
	return Global_1630317[iParam0 /*595*/].f_11.f_19;
}

int func_289(int iParam0)
{
	switch (iParam0)
	{
		case 155:
		case 160:
		case 153:
		case 162:
		case 154:
		case 163:
		case 171:
		case 172:
		case 240:
		case 239:
			return 1;
		
		default:
	}
	return func_290(iParam0, 0);
	return 0;
}

int func_290(int iParam0, int iParam1)
{
	switch (iParam0)
	{
		case 199:
		case 205:
		case 210:
		case 211:
			return 1;
		
		default:
	}
	if (iParam1 == 0)
	{
		switch (iParam0)
		{
			case 194:
			case 193:
			case 189:
			case 153:
				return 1;
			}
		
		default:
	}
	return 0;
}

int func_291(int iParam0)
{
	int iVar0;
	int iVar1;
	
	iVar0 = 0;
	while (iVar0 < 32)
	{
		iVar1 = unk_0xF1110FE23C67293A(iVar0);
		if (unk_0x166E920FB00B2DBB(iVar1))
		{
			if ((Global_1630317[iParam0 /*595*/].f_11.f_34 != func_42() && Global_1630317[iParam0 /*595*/].f_11.f_34 == iVar1) && Global_1630317[iVar1 /*595*/].f_11.f_34 == iParam0)
			{
				return 1;
			}
		}
		iVar0++;
	}
	return 0;
}

int func_292(int iParam0)
{
	switch (iParam0)
	{
		case 179:
			if (func_294() < 2)
			{
				return 0;
			}
			break;
		
		case 148:
			if (func_293() < 2)
			{
				return 0;
			}
			break;
		
		default:
			if (func_293() < 2)
			{
				return 0;
			}
			break;
	}
	return 1;
}

int func_293()
{
	int iVar0;
	int iVar1;
	int iVar2;
	
	iVar0 = 0;
	while (iVar0 < 32)
	{
		iVar2 = unk_0xF1110FE23C67293A(iVar0);
		if (unk_0x166E920FB00B2DBB(iVar2))
		{
			if (func_269(iVar2))
			{
				iVar1++;
			}
		}
		iVar0++;
	}
	return iVar1;
}

int func_294()
{
	int iVar0;
	int iVar1;
	int iVar2;
	
	iVar0 = 0;
	while (iVar0 < 32)
	{
		iVar2 = unk_0xF1110FE23C67293A(iVar0);
		if (unk_0x166E920FB00B2DBB(iVar2))
		{
			if (func_276(iVar2))
			{
				iVar1++;
			}
		}
		iVar0++;
	}
	return iVar1;
}

int func_295()
{
	return Global_1630317[unk_0x460153A63B9477BC() /*595*/].f_11.f_102;
}

bool func_296()
{
	return Global_1652336.f_11.f_144 != -1;
}

bool func_297()
{
	return Global_1652336.f_11.f_143 != -1;
}

int func_298(int iParam0)
{
	switch (iParam0)
	{
		case 153:
		case 154:
		case 155:
		case 162:
		case 160:
		case 163:
		case 167:
		case 168:
		case 169:
		case 171:
		case 172:
		case 178:
		case 190:
		case 191:
		case 192:
		case 189:
		case 193:
		case 194:
		case 199:
		case 205:
		case 210:
		case 225:
		case 226:
		case 227:
		case 229:
		case 230:
		case 233:
		case 237:
		case 238:
		case 239:
		case 240:
		case 249:
		case 250:
		case 243:
		case 158:
		case 181:
		case 256:
		case 258:
			return 0;
		
		case 152:
		case 159:
		case 142:
		case 164:
		case 157:
		case 166:
		case 170:
		case 173:
		case 180:
		case 183:
		case 185:
		case 182:
		case 186:
		case 197:
		case 200:
		case 201:
		case 198:
		case 195:
		case 207:
		case 208:
		case 209:
		case 214:
		case 215:
		case 216:
		case 217:
		case 218:
		case 219:
		case 220:
		case 221:
		case 241:
		case 242:
		case 244:
		case 248:
		case 24:
		case 26:
		case 259:
			return 1;
		
		case 148:
		case 179:
			return 2;
		
		default:
	}
	return -1;
}

int func_299(int iParam0)
{
	int iVar0;
	int iVar1;
	
	iVar0 = 0;
	while (iVar0 < func_301())
	{
		iVar1 = Global_1630317[iParam0 /*595*/].f_11.f_11[iVar0];
		if (iVar1 != func_42())
		{
			if (unk_0x166E920FB00B2DBB(iVar1))
			{
				if (func_305(iVar1))
				{
					return 7;
				}
				if (func_304(iVar1))
				{
					return 15;
				}
				if (func_300(iVar1))
				{
					return 16;
				}
			}
		}
		iVar0++;
	}
	return -1;
}

int func_300(int iParam0)
{
	if (iParam0 == func_42())
	{
		return 0;
	}
	if (((((unk_0xCE990E643CD9D0E5(Global_2426097[iParam0 /*443*/].f_314.f_4, 2) || unk_0xCE990E643CD9D0E5(Global_2426097[iParam0 /*443*/].f_314.f_4, 3)) || unk_0xCE990E643CD9D0E5(Global_2426097[iParam0 /*443*/].f_314.f_4, 5)) || unk_0xCE990E643CD9D0E5(Global_2426097[iParam0 /*443*/].f_314.f_4, 6)) || unk_0xCE990E643CD9D0E5(Global_2426097[iParam0 /*443*/].f_314.f_4, 7)) || unk_0xCE990E643CD9D0E5(Global_1590682[iParam0 /*883*/].f_274.f_369.f_1, 1))
	{
		return 1;
	}
	return 0;
}

int func_301()
{
	return (func_302() - 1);
}

int func_302()
{
	return func_303(unk_0x460153A63B9477BC());
}

int func_303(int iParam0)
{
	int iVar0;
	
	iVar0 = Global_1630317[iParam0 /*595*/].f_11;
	if (iVar0 != func_42() && Global_1630317[iVar0 /*595*/].f_11.f_423 == 1)
	{
		return 8;
	}
	return 4;
}

int func_304(int iParam0)
{
	var uVar0;
	int iVar1;
	
	if (!unk_0x25DDB354A40FFCDB())
	{
		return 0;
	}
	if (iParam0 == unk_0x460153A63B9477BC())
	{
		return Global_2516074;
	}
	else
	{
		uVar0 = unk_0x1FA7B77001D60F9D(iParam0);
		if (unk_0xAE06B9E39EBDE049(uVar0))
		{
			iVar1 = unk_0x36FE6D3220816ADA(uVar0);
			if (iVar1 == joaat("mp_m_freemode_01") || iVar1 == joaat("mp_f_freemode_01"))
			{
				return 0;
			}
			else
			{
				return 1;
			}
		}
		else
		{
			return 0;
		}
	}
	return 0;
}

int func_305(int iParam0)
{
	int iVar0;
	
	iVar0 = iParam0;
	if (iVar0 != -1)
	{
		return unk_0xCE990E643CD9D0E5(Global_1630317[iVar0 /*595*/].f_1, 0);
	}
	return 0;
}

int func_306(int iParam0)
{
	switch (iParam0)
	{
		case 162:
			return Global_262145.f_12574;
		
		case 155:
			return Global_262145.f_12599;
		
		case 153:
			return Global_262145.f_12563;
		
		case 163:
			return Global_262145.f_12539;
		
		case 160:
			return Global_262145.f_12552;
		
		case 154:
			return Global_262145.f_12609;
		
		case 169:
			return 1;
		
		case 171:
			return Global_262145.f_15055;
		
		case 172:
			return Global_262145.f_15074;
		
		case 148:
			return Global_262145.f_18285;
		
		case 179:
			return Global_262145.f_18293;
		
		case 201:
			return Global_262145.f_18307;
		
		case 200:
			return Global_262145.f_18347;
		
		case 211:
			return Global_262145.f_18376;
		
		case 194:
			return Global_262145.f_18388;
		
		case 193:
			return Global_262145.f_18399;
		
		case 210:
			return Global_262145.f_18411;
		
		case 205:
			return Global_262145.f_18417;
		
		case 199:
			return Global_262145.f_18430;
		
		case 249:
			return 1;
		
		case 238:
			return 1;
		
		default:
	}
	return 2147483647;
}

int func_307(int iParam0)
{
	return Global_1652336.f_11.f_147[func_308(iParam0)];
}

int func_308(int iParam0)
{
	switch (iParam0)
	{
		case 148:
			return 0;
		
		case 152:
			return 1;
		
		case 151:
			return 2;
		
		case 142:
			return 3;
		
		case 157:
			return 4;
		
		case 159:
			return 5;
		
		case 164:
			return 6;
		
		case 160:
			return 7;
		
		case 162:
			return 8;
		
		case 163:
			return 9;
		
		case 154:
			return 10;
		
		case 155:
			return 11;
		
		case 153:
			return 12;
		
		case 166:
			return 13;
		
		case 167:
			return 14;
		
		case 168:
			return 15;
		
		case 169:
			return 16;
		
		case 170:
			return 17;
		
		case 171:
			return 18;
		
		case 172:
			return 19;
		
		case 173:
			return 20;
		
		case 178:
			return 21;
		
		case 179:
			return 22;
		
		case 180:
			return 23;
		
		case 181:
			return 24;
		
		case 182:
			return 25;
		
		case 183:
			return 26;
		
		case 185:
			return 27;
		
		case 186:
			return 28;
		
		case 189:
			return 31;
		
		case 190:
			return 32;
		
		case 191:
			return 33;
		
		case 192:
			return 34;
		
		case 193:
			return 35;
		
		case 194:
			return 36;
		
		case 195:
			return 37;
		
		case 197:
			return 39;
		
		case 198:
			return 40;
		
		case 199:
			return 41;
		
		case 200:
			return 42;
		
		case 201:
			return 43;
		
		case 205:
			return 44;
		
		case 207:
			return 45;
		
		case 208:
			return 46;
		
		case 209:
			return 47;
		
		case 210:
			return 48;
		
		case 214:
			return 49;
		
		case 215:
			return 50;
		
		case 216:
			return 51;
		
		case 217:
			return 52;
		
		case 218:
			return 53;
		
		case 219:
			return 54;
		
		case 220:
			return 55;
		
		case 221:
			return 56;
		
		case 188:
			return 30;
		
		case 225:
			return 57;
		
		case 226:
			return 58;
		
		case 227:
			return 59;
		
		case 229:
			return 60;
		
		case 230:
			return 61;
		
		case 233:
			return 62;
		
		case 237:
			return 63;
		
		case 238:
			return 64;
		
		case 239:
			return 65;
		
		case 240:
			return 66;
		
		case 241:
			return 67;
		
		case 242:
			return 68;
		
		case 244:
			return 69;
		
		case 248:
			return 70;
		
		case 249:
			return 71;
		
		case 250:
			return 72;
		
		case 243:
			return 73;
		
		case 158:
			return 74;
		
		case 256:
			return 75;
		
		case 258:
			return 76;
		
		default:
	}
	return -1;
}

int func_309(int iParam0, int iParam1, int iParam2, bool bParam3, bool bParam4)
{
	if ((func_317(iParam0, bParam3, bParam4) || func_316(iParam1, bParam3, bParam4)) || func_310(iParam2, bParam3, bParam4))
	{
		return 1;
	}
	return 0;
}

int func_310(int iParam0, bool bParam1, bool bParam2)
{
	return func_311(2, iParam0, 0, bParam1, bParam2);
}

int func_311(int iParam0, int iParam1, bool bParam2, bool bParam3, bool bParam4)
{
	int iVar0;
	int iVar1;
	
	if (!unk_0xCE990E643CD9D0E5(Global_1389439, 0))
	{
		return 0;
	}
	if ((bParam2 && !bParam3) && iParam1 <= (func_315(iParam0) - func_314(iParam0, 0)))
	{
		return 1;
	}
	else
	{
		if (bParam3)
		{
			if (bParam2)
			{
				iVar0 = (iParam1 - func_314(iParam0, 0));
			}
			else
			{
				iVar0 = iParam1;
			}
			iVar1 = (func_315(iParam0) - func_313(iParam0));
		}
		else
		{
			if (bParam2)
			{
				iVar0 = (iParam1 - func_314(iParam0, 0));
			}
			else
			{
				iVar0 = iParam1;
			}
			iVar1 = (func_315(iParam0) - func_314(iParam0, 1));
		}
		if (!bParam4 && Global_1590682[unk_0x460153A63B9477BC() /*883*/] != 3)
		{
			iVar1 = (iVar1 - func_312(iParam0));
		}
		if (iVar0 < iVar1)
		{
			return 1;
		}
	}
	return 0;
}

int func_312(int iParam0)
{
	switch (iParam0)
	{
		case 0:
			return 10;
			break;
		
		case 1:
			return 10;
			break;
		
		case 2:
			return 10;
			break;
	}
	return 0;
}

int func_313(int iParam0)
{
	switch (iParam0)
	{
		case 0:
			return Global_1389439.f_1;
			break;
		
		case 1:
			return Global_1389439.f_2;
			break;
		
		case 2:
			return Global_1389439.f_3;
			break;
	}
	return 0;
}

int func_314(int iParam0, bool bParam1)
{
	int iVar0;
	
	iVar0 = unk_0x460153A63B9477BC();
	switch (iParam0)
	{
		case 0:
			if (!unk_0x82CC259B7B4F4AD9())
			{
				return Global_2426097[iVar0 /*443*/].f_213;
			}
			else
			{
				return unk_0x199928B287E7CD6A(!bParam1, 0);
			}
			break;
		
		case 1:
			if (!unk_0x82CC259B7B4F4AD9())
			{
				return Global_2426097[iVar0 /*443*/].f_214;
			}
			else
			{
				return unk_0xCC859AC9B82FF38A(!bParam1, 0);
			}
			break;
		
		case 2:
			if (!unk_0x82CC259B7B4F4AD9())
			{
				return Global_2426097[iVar0 /*443*/].f_215;
			}
			else
			{
				return unk_0x95F0A437EC066E83(!bParam1, 0);
			}
			break;
	}
	return 0;
}

int func_315(int iParam0)
{
	switch (iParam0)
	{
		case 0:
			return Global_1389447;
			break;
		
		case 1:
			return Global_1389448;
			break;
		
		case 2:
			return Global_1389449;
			break;
	}
	return 0;
}

int func_316(int iParam0, bool bParam1, bool bParam2)
{
	return func_311(1, iParam0, 0, bParam1, bParam2);
}

int func_317(int iParam0, bool bParam1, bool bParam2)
{
	return func_311(0, iParam0, 0, bParam1, bParam2);
}

int func_318(int iParam0, int iParam1, int iParam2, int iParam3, int iParam4, int iParam5)
{
	switch (iParam0)
	{
		case 148:
		case 152:
		case 162:
		case 160:
		case 163:
			return 0;
		
		case 159:
		case 142:
			return 1;
		
		case 167:
			return func_370(iParam1);
		
		case 170:
			return 2;
		
		case 171:
			return 0;
		
		case 172:
			return 9;
		
		case 173:
			return 0;
		
		case 168:
			return 5;
		
		case 178:
		case 188:
			return func_368(iParam1, iParam2, iParam3);
		
		case 225:
		case 226:
			return func_362(iParam1, iParam2, iParam3, iParam4);
		
		case 227:
			return 0;
		
		case 229:
		case 230:
			return func_350(iParam1, iParam2, iParam3, unk_0xF1110FE23C67293A(uParam5), iParam4);
		
		case 233:
			return func_343(iParam1, iParam2, iParam3, unk_0xF1110FE23C67293A(iParam5), iParam4);
		
		case 179:
			return 0;
		
		case 182:
			return 7;
		
		case 185:
			return 6;
		
		case 186:
			return 2;
		
		case 180:
			return 9;
		
		case 190:
			return 8;
		
		case 191:
			return 0;
		
		case 192:
			return func_334(iParam1, iParam2);
		
		case 193:
			return 0;
		
		case 194:
			return 0;
		
		case 199:
			return 0;
		
		case 195:
			return 9;
		
		case 201:
			return 16;
		
		case 198:
			return 12;
		
		case 205:
			return 8;
		
		case 207:
			return 0;
		
		case 208:
			return 15;
		
		case 209:
			return 0;
		
		case 210:
			return 0;
		
		case 214:
			return 9;
		
		case 215:
			return 0;
		
		case 216:
			return 16;
		
		case 217:
			return 0;
		
		case 218:
			return 0;
		
		case 219:
			return 0;
		
		case 220:
			return 30;
		
		case 221:
			return 30;
		
		case 237:
		case 250:
			return func_331(iParam1, iParam2, iParam3);
		
		case 238:
		case 249:
			return func_330(iParam1);
		
		case 239:
			return 4;
		
		case 240:
			return 1;
		
		case 241:
			return 1;
		
		case 242:
			return 1;
		
		case 244:
			return 4;
		
		case 248:
			return 5;
		
		case 243:
			return func_326(iParam1, iParam2, unk_0xF1110FE23C67293A(iParam5), iParam4);
		
		case 158:
			return func_322(iParam1, iParam2, unk_0xF1110FE23C67293A(iParam5), iParam4);
		
		case 181:
			return func_319(iParam1, iParam2, unk_0xF1110FE23C67293A(iParam5), iParam4);
		
		default:
	}
	return 0;
}

int func_319(int iParam0, int iParam1, int iParam2, int iParam3)
{
	int iVar0;
	
	iVar0 = func_321(iParam0, iParam1, iParam3);
	iVar0 = (iVar0 + func_320(iParam0, iParam2));
	return iVar0;
}

int func_320(int iParam0, var uParam1)
{
	switch (iParam0)
	{
		case 2:
			return 5;
		
		default:
	}
	return 1;
}

int func_321(int iParam0, var uParam1, var uParam2)
{
	switch (iParam0)
	{
		case 0:
			return 1;
		
		default:
	}
	return 0;
}

int func_322(int iParam0, int iParam1, int iParam2, int iParam3)
{
	int iVar0;
	
	iVar0 = func_325(iParam0, iParam1, iParam3);
	iVar0 = (iVar0 + func_323(iParam0, iParam2));
	return iVar0;
}

int func_323(int iParam0, int iParam1)
{
	struct<3> Var0;
	
	Var0 = -1;
	Var0.f_1 = -1;
	Var0.f_2 = -1;
	switch (iParam0)
	{
		case 4:
		case 2:
		case 1:
		case 3:
		case 30:
		case 31:
		case 32:
		case 28:
		case 29:
		case 52:
		case 37:
			return 0;
		
		case 19:
		case 20:
			return 1;
		
		case 38:
		case 8:
		case 14:
		case 17:
		case 11:
		case 18:
		case 21:
		case 22:
		case 7:
		case 34:
		case 23:
		case 41:
		case 49:
		case 48:
		case 27:
		case 15:
		case 50:
		case 24:
		case 6:
		case 9:
		case 10:
		case 47:
		case 42:
		case 26:
		case 25:
			return 2;
		
		case 43:
			return 3;
		
		case 16:
			return 4;
		
		case 12:
		case 13:
			if (func_250(iParam1, 1))
			{
				Var0 = { func_324(iParam1) };
			}
			if (Var0.f_1 == 4)
			{
				return 4;
			}
			else
			{
				return 2;
			}
			break;
		
		case 33:
			return 5;
	}
	return 1;
}

Vector3 func_324(int iParam0)
{
	return Global_1630317[iParam0 /*595*/].f_11.f_302;
}

int func_325(int iParam0, int iParam1, var uParam2)
{
	switch (iParam0)
	{
		case 0:
			return 5;
		
		case 24:
			return 1;
		
		case 10:
			switch (iParam1)
			{
				case 30:
					return 11;
				
				case 31:
				case 32:
					return 9;
				
				default:
			}
			break;
		
		case 36:
			return 0;
		
		case 38:
			return 3;
		
		case 40:
			return 4;
		
		case 41:
			return 4;
		
		case 8:
			return 13;
		
		case 13:
			switch (iParam1)
			{
				case 38:
				case 39:
					return 10;
				
				case 40:
					return 11;
				
				default:
			}
			break;
		
		case 35:
			return 5;
		
		case 46:
			return 2;
		
		case 9:
			return 8;
		
		case 4:
			return 7;
		
		case 14:
			return 11;
		
		case 17:
			return 8;
		
		case 47:
			return 11;
		
		case 11:
			return 9;
		
		case 18:
			return 13;
		
		case 6:
			return 17;
		
		case 48:
			return 21;
		
		case 33:
			return 5;
		
		case 44:
			return 2;
		
		case 21:
			return 22;
		
		case 22:
			return 12;
		
		case 7:
			return 10;
		
		case 2:
			return 0;
		
		case 34:
			switch (iParam1)
			{
				case 91:
					return 16;
				
				case 92:
					return 14;
				
				default:
			}
			break;
		
		case 27:
			return 12;
		
		case 23:
			return 17;
		
		case 31:
			return 3;
		
		case 12:
			return 4;
		
		case 3:
			return 2;
		
		case 49:
			return 15;
		
		case 19:
			return 14;
		
		case 50:
			return 12;
		
		case 20:
			return 12;
		
		case 32:
			return 15;
		
		case 30:
			return 6;
		
		case 16:
			return 12;
		
		case 39:
			return 5;
		
		case 42:
			return 3;
		
		case 37:
			return 19;
		
		case 52:
			return 6;
		
		case 15:
			return 2;
		
		case 25:
			switch (iParam1)
			{
				case 69:
					return 11;
				
				case 70:
					return 12;
				
				default:
			}
			break;
	}
	return 0;
}

int func_326(int iParam0, int iParam1, int iParam2, int iParam3)
{
	int iVar0;
	
	iVar0 = func_329(iParam0, iParam1, iParam3);
	if (!func_328(iParam0))
	{
		iVar0 = (iVar0 + func_327(iParam0, iParam2));
	}
	return iVar0;
}

int func_327(int iParam0, int iParam1)
{
	switch (iParam0)
	{
		case 14:
		case 5:
		case 0:
		case 3:
		case 2:
			return 0;
		
		case 6:
			if (func_288(iParam1) + 1 == 1)
			{
				return 1;
			}
			else
			{
				return 2;
			}
			break;
	}
	return 1;
}

int func_328(int iParam0)
{
	return 0;
}

int func_329(int iParam0, int iParam1, int iParam2)
{
	switch (iParam0)
	{
		case 13:
			return 3;
		
		case 5:
			switch (iParam1)
			{
				case 15:
					return 17;
				
				case 16:
					return 12;
				
				case 17:
					return 12;
				
				default:
			}
			break;
		
		case 11:
			switch (iParam1)
			{
				case 35:
				case 36:
					return 1;
				
				default:
			}
			break;
		
		case 6:
			switch (iParam1)
			{
				case 18:
					return 3;
				
				default:
			}
			break;
		
		case 12:
			return 15;
		
		case 0:
			return 1;
		
		case 9:
			switch (iParam1)
			{
				case 28:
					switch (iParam2)
					{
						case 0:
							return 1;
						
						case 1:
							return 0;
						
						case 2:
							return 3;
						
						default:
					}
					break;
				
				case 29:
					switch (iParam2)
					{
						case 0:
							return 1;
						
						case 1:
							return 2;
						
						case 2:
							return 0;
						
						default:
					}
					break;
				
				case 30:
					switch (iParam2)
					{
						case 0:
							return 0;
						
						case 1:
							return 0;
						
						case 2:
							return 0;
						
						default:
					}
					break;
			}
			break;
		
		case 3:
			switch (iParam1)
			{
				case 10:
					return 6;
				
				default:
			}
			return 5;
		
		case 2:
			switch (iParam1)
			{
				case 6:
					return 22;
				
				case 7:
					return 23;
				
				case 8:
					return 25;
				
				default:
			}
			break;
		
		case 8:
			switch (iParam1)
			{
				case 25:
					return 3;
				
				case 26:
					return 8;
				
				case 27:
					return 4;
				
				default:
			}
			break;
	}
	return 0;
}

int func_330(int iParam0)
{
	switch (iParam0)
	{
		case 0:
			return 3;
		
		case 1:
			return 3;
		
		case 2:
			return 4;
		
		case 3:
			return 0;
		
		case 4:
			return 3;
		
		case 5:
			return 0;
		
		default:
	}
	return 0;
}

int func_331(int iParam0, int iParam1, int iParam2)
{
	int iVar0;
	
	iVar0 = func_333(iParam0, iParam1);
	if (func_332(iParam0))
	{
		iVar0 = (iVar0 + iParam2);
	}
	return iVar0;
}

int func_332(int iParam0)
{
	return 1;
}

int func_333(int iParam0, int iParam1)
{
	switch (iParam0)
	{
		case 12:
			return 3;
		
		case 9:
			switch (iParam1)
			{
				case 71:
				case 72:
				case 73:
					return 3;
				
				default:
			}
			break;
		
		case 11:
			switch (iParam1)
			{
				case 77:
					return 2;
				
				default:
			}
			break;
		
		case 17:
			switch (iParam1)
			{
				case 102:
				case 103:
				case 104:
					return 4;
				
				default:
			}
			break;
		
		case 23:
			switch (iParam1)
			{
				case 133:
				case 135:
					return 1;
				
				case 136:
					return 2;
				
				default:
			}
			break;
		
		case 1:
			return 10;
		
		case 18:
			return 3;
		
		case 3:
			return 4;
		
		case 21:
			return 2;
		
		case 0:
			switch (iParam1)
			{
				case 0:
					return 9;
				
				case 1:
					return 9;
				
				case 2:
					return 8;
				
				case 3:
					return 6;
				
				case 4:
					return 8;
				
				case 5:
					return 8;
				
				case 6:
					return 8;
				
				case 7:
					return 8;
				
				case 8:
					return 8;
				
				case 9:
					return 6;
				
				default:
			}
			break;
	}
	return 0;
}

int func_334(int iParam0, int iParam1)
{
	int iVar0;
	
	iVar0 = func_342(iParam0, iParam1);
	if (!func_341(iParam0))
	{
		iVar0 = (iVar0 + func_335(iParam0, func_339() + 1));
	}
	if (iParam0 == 14 || ((((func_341(iParam0) && iParam0 != 9) && iParam0 != 16) && iParam0 != 7) && iParam0 != 17))
	{
		iVar0 = (iVar0 + func_335(iParam0, func_339() + 1));
	}
	if (iParam0 == 7)
	{
		iVar0 += 6;
	}
	if (iParam0 == 17)
	{
		iVar0 += 2;
	}
	return iVar0;
}

int func_335(int iParam0, int iParam1)
{
	int iVar0;
	int iVar1;
	int iVar2;
	
	if (func_338(iParam0))
	{
		iVar0 = 1;
	}
	else
	{
		if (iVar0 == 0)
		{
			iVar0 = iParam1;
			iVar1 = func_337(iParam0, iParam1);
			iVar2 = func_336(iParam0);
			if (iVar0 > iVar1)
			{
				iVar0 = iVar1;
			}
			else if (iVar0 < iVar2)
			{
				iVar0 = iVar2;
			}
		}
		if (func_341(iParam0))
		{
			if (iVar0 > Global_262145.f_17922)
			{
				iVar0 = Global_262145.f_17922;
			}
		}
		else
		{
			if (iVar0 < Global_262145.f_17921)
			{
				iVar0 = Global_262145.f_17921;
			}
			if (iVar0 > Global_262145.f_17920)
			{
				iVar0 = Global_262145.f_17920;
			}
		}
	}
	return iVar0;
}

int func_336(int iParam0)
{
	switch (iParam0)
	{
		case 5:
			return Global_262145.f_17959;
		
		case 10:
			return Global_262145.f_17984;
		
		default:
	}
	return 1;
}

int func_337(int iParam0, int iParam1)
{
	if (func_341(iParam0))
	{
		if (iParam1 >= Global_262145.f_17923)
		{
			return 2;
		}
		return 1;
	}
	switch (iParam0)
	{
		case 5:
			return Global_262145.f_17960;
		
		case 10:
			return Global_262145.f_17985;
		
		default:
	}
	return 8;
}

int func_338(int iParam0)
{
	switch (iParam0)
	{
		case 15:
		case 16:
		case 17:
		case 18:
		case 19:
			return 1;
		
		default:
	}
	return 0;
}

int func_339()
{
	if (func_340() == func_42())
	{
		return 0;
	}
	return func_288(func_340());
}

int func_340()
{
	return Global_1630317[unk_0x460153A63B9477BC() /*595*/].f_11;
}

int func_341(int iParam0)
{
	switch (iParam0)
	{
		case 14:
		case 32:
		case 0:
		case 2:
		case 4:
		case 18:
		case 6:
		case 5:
		case 10:
		case 12:
		case 13:
			return 0;
		
		default:
	}
	return 1;
}

int func_342(int iParam0, int iParam1)
{
	switch (iParam0)
	{
		case 32:
			switch (iParam1)
			{
				case 195:
					return 3;
				
				case 196:
					return 3;
				
				case 197:
					return 3;
				
				case 198:
					return 3;
				
				case 199:
					return 3;
				
				default:
			}
			break;
		
		case 37:
			switch (iParam1)
			{
				case 231:
					return 2;
				
				default:
			}
			break;
		
		case 1:
			switch (iParam1)
			{
				case 7:
					return 9;
				
				case 8:
					return 8;
				
				case 9:
					return 7;
				
				case 10:
					return 4;
				
				case 11:
					return 5;
				
				case 12:
					return 6;
				
				default:
			}
			break;
		
		case 0:
			switch (iParam1)
			{
				case 1:
					return 10;
				
				case 2:
					return 10;
				
				case 3:
					return 8;
				
				case 4:
					return 10;
				
				case 5:
					return 10;
				
				case 6:
					return 10;
				
				default:
			}
			break;
		
		case 2:
			switch (iParam1)
			{
				case 13:
					return 10;
				
				case 14:
					return 8;
				
				case 15:
					return 8;
				
				case 16:
					return 8;
				
				case 17:
					return 10;
				
				case 18:
					return 9;
				
				default:
			}
			break;
		
		case 4:
			switch (iParam1)
			{
				case 19:
					return 5;
				
				case 20:
					return 8;
				
				case 21:
					return 4;
				
				case 22:
					return 0;
				
				case 23:
					return 10;
				
				case 24:
					return 1;
				
				default:
			}
			break;
		
		case 6:
			switch (iParam1)
			{
				case 25:
					return 20;
				
				case 26:
					return 15;
				
				case 27:
					return 13;
				
				case 28:
					return 16;
				
				case 29:
					return 14;
				
				case 30:
					return 14;
				
				default:
			}
			break;
		
		case 3:
			switch (iParam1)
			{
				case 74:
					return 7;
				
				case 75:
					return 5;
				
				case 76:
					return 5;
				
				case 77:
					return 4;
				
				default:
			}
			break;
		
		case 8:
			switch (iParam1)
			{
				case 64:
					return 1;
				
				case 67:
					return 1;
				
				default:
			}
			break;
		
		case 14:
			switch (iParam1)
			{
				case 38:
					return 6;
				
				case 39:
					return 6;
				
				case 40:
					return 7;
				
				case 41:
					return 6;
				
				default:
			}
			break;
	}
	return 0;
}

int func_343(int iParam0, int iParam1, int iParam2, int iParam3, int iParam4)
{
	int iVar0;
	
	iVar0 = func_349(iParam0, iParam1, iParam2, iParam4);
	if (!func_348(iParam0))
	{
		if (iParam3 != func_42())
		{
			iVar0 = (iVar0 + func_345(iParam0, func_288(iParam3) + 1, iParam3, -1, iParam2));
		}
	}
	if (func_344(iParam0))
	{
		iVar0 = (iVar0 + iParam2);
	}
	return iVar0;
}

int func_344(int iParam0)
{
	switch (iParam0)
	{
		case 2:
		case 5:
		case 7:
			return 1;
		
		default:
	}
	return 0;
}

int func_345(int iParam0, var uParam1, int iParam2, int iParam3, int iParam4)
{
	int iVar0;
	
	if (iParam4 > 0)
	{
		return iParam4;
	}
	switch (iParam0)
	{
		case 5:
		case 7:
		case 6:
		case 4:
		case 10:
			iVar0 = 1;
			break;
		
		case 8:
			iVar0 = 2;
			break;
		
		case 3:
		case 2:
			iVar0 = 4;
			break;
		
		default:
			if (func_348(iParam0))
			{
				iVar0 = 2;
			}
			else
			{
				iVar0 = 1;
			}
			break;
	}
	if (iVar0 > func_347(iParam0))
	{
		iVar0 = func_347(iParam0);
	}
	else if (iVar0 < func_346(iParam0))
	{
		iVar0 = func_346(iParam0);
	}
	return iVar0;
}

int func_346(int iParam0)
{
	return 1;
}

int func_347(int iParam0)
{
	return 4;
}

int func_348(int iParam0)
{
	switch (iParam0)
	{
		case 3:
		case 2:
		case 4:
		case 5:
		case 6:
		case 7:
		case 10:
			return 1;
		
		default:
	}
	return 0;
}

int func_349(int iParam0, int iParam1, var uParam2, var uParam3)
{
	int iVar0;
	
	switch (iParam0)
	{
		case 0:
			return 0;
		
		case 1:
			return 0;
		
		case 2:
			return 5;
		
		case 3:
			iVar0 = 1;
			switch (iParam1)
			{
				case 26:
					return iVar0 + 1;
				
				case 34:
					return iVar0 + 1;
				
				default:
			}
			return iVar0;
			break;
		
		case 4:
			return 1;
		
		case 5:
			return 1;
		
		case 8:
			return 7;
		
		case 12:
			switch (iParam1)
			{
				case 125:
					return 6;
				
				default:
			}
			return 2;
			break;
		
		case 9:
			return 0;
		
		case 13:
			return 1;
		
		case 10:
			return 12;
	}
	return 0;
}

int func_350(int iParam0, int iParam1, int iParam2, int iParam3, int iParam4)
{
	int iVar0;
	
	iVar0 = func_361(iParam0, iParam1, iParam2, iParam4);
	if (!func_360(iParam0))
	{
		if (iParam3 != func_42())
		{
			iVar0 = (iVar0 + func_352(iParam0, func_288(iParam3) + 1, iParam3, -1, iParam2));
		}
	}
	if (func_351(iParam0))
	{
		iVar0 = (iVar0 + iParam2);
	}
	return iVar0;
}

int func_351(int iParam0)
{
	switch (iParam0)
	{
		case 23:
			return 1;
		
		default:
	}
	return 0;
}

int func_352(int iParam0, int iParam1, int iParam2, int iParam3, int iParam4)
{
	int iVar0;
	int iVar1;
	
	if (iParam4 > 0)
	{
		return iParam4;
	}
	if (func_359(iParam0))
	{
		iVar1 = func_358(iParam3);
		switch (iParam0)
		{
			case 21:
				iVar0 = 1;
				break;
			
			case 23:
				switch (iVar1)
				{
					case 1:
						iVar0 = 1;
						break;
					
					case 2:
						iVar0 = 2;
						break;
					
					case 3:
						iVar0 = 2;
						break;
				}
				break;
			
			case 15:
				switch (iVar1)
				{
					case 1:
						iVar0 = 3;
						break;
					
					case 2:
						iVar0 = 6;
						break;
					
					case 3:
						iVar0 = 8;
						break;
				}
				break;
			
			case 20:
			case 24:
			case 22:
			case 16:
			case 17:
				switch (iVar1)
				{
					case 1:
						iVar0 = 1;
						break;
					
					case 2:
						iVar0 = 2;
						break;
					
					case 3:
						iVar0 = 3;
						break;
				}
				break;
			
			case 25:
				switch (iVar1)
				{
					case 1:
						iVar0 = 2;
						break;
					
					case 2:
						iVar0 = 3;
						break;
					
					case 3:
						iVar0 = 3;
						break;
				}
				break;
		}
		return iVar0;
	}
	else if (func_357(iParam0))
	{
		iVar0 = 1;
		return iVar0;
	}
	else
	{
		iVar0 = iParam1;
		if (iVar0 > func_356(iParam2))
		{
			iVar0 = func_356(iParam2);
		}
	}
	if (iVar0 > func_354(iParam0))
	{
		iVar0 = func_354(iParam0);
	}
	else if (iVar0 < func_353(iParam0))
	{
		iVar0 = func_353(iParam0);
	}
	return iVar0;
}

int func_353(int iParam0)
{
	switch (iParam0)
	{
		case 15:
			return 3;
		
		default:
	}
	return 1;
}

int func_354(int iParam0)
{
	if (!func_359(iParam0))
	{
		if (func_360(iParam0))
		{
			if (func_355(iParam0))
			{
				return 2;
			}
			return 3;
		}
	}
	else
	{
		switch (iParam0)
		{
			case 21:
				return 1;
			
			case 23:
				return 2;
			
			case 15:
				return 9;
			
			default:
		}
		return 3;
	}
	return 4;
}

int func_355(int iParam0)
{
	switch (iParam0)
	{
		case 12:
			return 1;
		
		default:
	}
	return 0;
}

int func_356(int iParam0)
{
	if (iParam0 == func_42() || Global_1590682[iParam0 /*883*/].f_274.f_264 == 0)
	{
		return 50;
	}
	return (50 - Global_1590682[iParam0 /*883*/].f_274.f_264.f_3);
}

int func_357(int iParam0)
{
	switch (iParam0)
	{
		case 26:
			return 1;
		
		default:
	}
	return 0;
}

int func_358(int iParam0)
{
	if (iParam0 > 25)
	{
		return 3;
	}
	else if (iParam0 > 10)
	{
		return 2;
	}
	return 1;
}

int func_359(int iParam0)
{
	switch (iParam0)
	{
		case 15:
		case 16:
		case 17:
		case 18:
		case 19:
		case 20:
		case 21:
		case 22:
		case 23:
		case 24:
		case 25:
			return 1;
		
		default:
	}
	return 0;
}

int func_360(int iParam0)
{
	if (func_359(iParam0) || func_357(iParam0))
	{
		return 1;
	}
	return 0;
}

int func_361(int iParam0, int iParam1, int iParam2, var uParam3)
{
	switch (iParam0)
	{
		case 1:
			return 1;
		
		case 2:
			return 12;
		
		case 11:
			return 1;
		
		case 5:
			return 10;
		
		case 12:
			switch (iParam1)
			{
				case 0:
					return 3;
				
				case 1:
					return 3;
				
				case 2:
					return 10;
				
				default:
			}
			break;
		
		case 10:
			switch (iParam1)
			{
				case 34:
					return 17;
				
				case 35:
					return 20;
				
				case 36:
					return 22;
				
				default:
			}
			break;
		
		case 9:
			switch (iParam1)
			{
				case 6:
					return 9;
				
				case 7:
					return 0;
				
				case 8:
					return 5;
				
				default:
			}
			break;
		
		case 0:
			switch (iParam2)
			{
				case 1:
					return 7;
				
				case 2:
					return 14;
				
				case 3:
					return 14;
				
				case 4:
					return 14;
				
				default:
			}
			break;
		
		case 8:
			return 2;
		
		case 23:
			return (2 * iParam2);
		
		case 24:
			return (2 * iParam2);
		
		case 26:
			return 2;
	}
	return 0;
}

int func_362(int iParam0, int iParam1, int iParam2, int iParam3)
{
	int iVar0;
	
	iVar0 = func_367(iParam0, iParam1, iParam2, iParam3);
	if (!func_366(iParam0))
	{
		iVar0 = (iVar0 + func_364(iParam0, func_339() + 1, -1, iParam2));
	}
	if (func_363(iParam0))
	{
		iVar0 = (iVar0 + iParam2);
	}
	return iVar0;
}

int func_363(int iParam0)
{
	switch (iParam0)
	{
		case 0:
		case 12:
		case 6:
		case 14:
		case 15:
		case 16:
		case 17:
		case 7:
			return 1;
		
		default:
	}
	return 0;
}

int func_364(int iParam0, int iParam1, int iParam2, int iParam3)
{
	int iVar0;
	
	if (iParam3 > 0)
	{
		return iParam3;
	}
	if (func_365(iParam0))
	{
		if (iParam2 <= Global_262145.f_21216)
		{
			iVar0 = 1;
			if (iParam0 == 15)
			{
				iVar0 = 2;
			}
		}
		else if (iParam2 <= Global_262145.f_21217)
		{
			iVar0 = 2;
			if (iParam0 == 18)
			{
				iVar0 = 1;
			}
		}
		else
		{
			iVar0 = 3;
			if (iParam0 == 18)
			{
				iVar0 = 2;
			}
			else if (iParam0 == 19 || iParam0 == 2)
			{
				iVar0 = 4;
			}
		}
	}
	else if (iParam0 == 8)
	{
		if (iParam1 < 4)
		{
			iVar0 = 1;
		}
		else
		{
			iVar0 = 2;
		}
	}
	else if (iParam0 == 0 || iParam0 == 20)
	{
		iVar0 = 1;
	}
	else if (func_366(iParam0))
	{
		if (iParam1 < Global_262145.f_21176)
		{
			iVar0 = 1;
		}
		else
		{
			iVar0 = 2;
		}
	}
	else
	{
		iVar0 = iParam1;
	}
	if (iVar0 == 0)
	{
		iVar0 = 1;
	}
	return iVar0;
}

int func_365(int iParam0)
{
	switch (iParam0)
	{
		case 14:
		case 15:
		case 16:
		case 17:
		case 18:
		case 19:
			return 1;
		
		default:
	}
	return 0;
}

int func_366(int iParam0)
{
	switch (iParam0)
	{
		case 0:
		case 3:
		case 6:
		case 7:
		case 10:
		case 11:
		case 12:
		case 14:
		case 15:
		case 16:
		case 17:
		case 19:
		case 18:
			return 1;
		
		default:
	}
	return 0;
}

int func_367(int iParam0, int iParam1, int iParam2, int iParam3)
{
	switch (iParam0)
	{
		case 0:
			return 0;
		
		case 1:
			switch (iParam1)
			{
				case 0:
					return 3;
				
				case 1:
					return 6;
				
				case 2:
					return 8;
				
				default:
			}
			break;
		
		case 2:
			switch (iParam1)
			{
				case 3:
					return 0;
				
				case 4:
					return 0;
				
				case 5:
					return 0;
				
				case 6:
					return 0;
				
				case 7:
					return 0;
				
				case 8:
					return 0;
				
				default:
			}
			break;
		
		case 3:
			switch (iParam1)
			{
				case 9:
					return 0;
				
				case 10:
					return 0;
				
				case 11:
					return 0;
				
				case 12:
					return 0;
				
				default:
			}
			break;
		
		case 4:
			switch (iParam1)
			{
				case 14:
					return 10;
				
				case 15:
					return 12;
				
				case 13:
					return 2;
				
				default:
			}
			break;
		
		case 5:
			switch (iParam1)
			{
				case 16:
					return 0;
				
				case 17:
					return 0;
				
				case 18:
					return 0;
				
				default:
			}
			break;
		
		case 6:
			switch (iParam1)
			{
				case 19:
					return 15;
				
				case 20:
					return 15;
				
				case 21:
					return 11;
				
				default:
			}
			break;
		
		case 7:
			switch (iParam1)
			{
				case 22:
					return 10;
				
				case 23:
					return 7;
				
				case 24:
					return 6;
				
				default:
			}
			break;
		
		case 8:
			switch (iParam1)
			{
				case 25:
					return (6 + iParam2 * 2);
				
				case 26:
					return (7 + iParam2 * 2);
				
				default:
			}
			break;
		
		case 9:
			switch (iParam1)
			{
				case 27:
					return 9;
				
				case 28:
					return 10;
				
				default:
			}
			break;
		
		case 10:
			switch (iParam1)
			{
				case 29:
					return 6;
				
				case 30:
					return 6;
				
				case 31:
					return 6;
				
				default:
			}
			break;
		
		case 11:
			switch (iParam1)
			{
				case 32:
					return 8;
				
				case 33:
					return 8;
				
				case 34:
					return 8;
				
				default:
			}
			break;
		
		case 12:
			return 0;
		
		case 13:
			switch (iParam1)
			{
				case 35:
					return 18;
				
				case 36:
					return 9;
				
				case 37:
					return 0;
				
				default:
			}
			break;
		
		case 14:
			switch (iParam3)
			{
				case 0:
					return 9;
				
				case 1:
					return 16;
				
				case 2:
					return 6;
				
				case 3:
					return 3;
				
				case 4:
					return 9;
				
				case 5:
					return 6;
				
				case 6:
					return 6;
				
				case 7:
					return 3;
				
				case 8:
					return 3;
				
				case 9:
					return 3;
				
				case 10:
					return 0;
				
				case 11:
					return 9;
				
				case 12:
					return 3;
				
				default:
			}
			break;
		
		case 15:
			return iParam2;
		
		case 16:
			return 0;
		
		case 17:
			return 0;
		
		case 18:
			return 0;
		
		case 19:
			return 0;
		
		case 20:
			return 13;
	}
	return 0;
}

int func_368(int iParam0, int iParam1, var uParam2)
{
	int iVar0;
	
	iVar0 = func_369(iParam0, iParam1);
	if (iParam0 == 15)
	{
		iVar0++;
	}
	return iVar0;
}

int func_369(int iParam0, int iParam1)
{
	switch (iParam0)
	{
		case 6:
			switch (iParam1)
			{
				case 18:
					return 9;
				
				case 19:
					return 7;
				
				case 20:
					return 8;
				
				default:
			}
			break;
		
		case 9:
			switch (iParam1)
			{
				case 27:
					return 1;
				
				case 28:
					return 1;
				
				case 29:
					return 1;
				
				default:
			}
			break;
		
		case 10:
			switch (iParam1)
			{
				case 21:
					return 2;
				
				case 22:
					return 2;
				
				case 23:
					return 2;
				
				default:
			}
			break;
		
		case 14:
			switch (iParam1)
			{
				case 36:
					return 8;
				
				case 37:
					return 7;
				
				case 38:
					return 8;
				
				default:
			}
			break;
		
		case 2:
			switch (iParam1)
			{
				case 39:
					return 8;
				
				case 40:
					return 6;
				
				case 41:
					return 2;
				
				default:
			}
			break;
		
		case 4:
			switch (iParam1)
			{
				case 1:
					return 5;
				
				case 2:
					return 5;
				
				case 3:
					return 5;
				
				default:
			}
			break;
		
		case 17:
			switch (iParam1)
			{
				case 42:
					return 1;
				
				case 43:
					return 1;
				
				case 44:
					return 1;
				
				default:
			}
			break;
		
		case 15:
			switch (iParam1)
			{
				case 45:
					return 12;
				
				case 46:
					return 12;
				
				case 47:
					return 12;
				
				default:
			}
			break;
		
		case 16:
			switch (iParam1)
			{
				case 48:
					return 7;
				
				case 49:
					return 7;
				
				case 50:
					return 7;
				
				default:
			}
			break;
		
		case 12:
			return 3;
	}
	return 0;
}

int func_370(int iParam0)
{
	switch (iParam0)
	{
		case 0:
			return 0;
		
		case 1:
			return 0;
		
		case 2:
			return 0;
		
		case 3:
			return 0;
		
		case 4:
			return 0;
		
		case 5:
			return 0;
		
		case 6:
			return 0;
		
		case 7:
			return 0;
		
		case 8:
			return 0;
		
		case 9:
			return 0;
		
		case 10:
			return 0;
		
		case 11:
			return 0;
		
		case 12:
			return 3;
		
		case 13:
			return 0;
		
		case 14:
			return 0;
		
		case 15:
			return 0;
		
		case 16:
			return 1;
		
		case 17:
			return 3;
		
		case 19:
			return 0;
		
		case 18:
			return 0;
		
		default:
	}
	return 1;
}

int func_371(int iParam0, int iParam1, int iParam2, int iParam3, int iParam4, int iParam5)
{
	switch (iParam0)
	{
		case 148:
		case 152:
		case 162:
		case 160:
		case 163:
			return 0;
		
		case 157:
			return 4;
		
		case 167:
			return func_413(iParam1);
		
		case 170:
			return 2;
		
		case 171:
			return 0;
		
		case 172:
			return 5;
		
		case 173:
			return 14;
		
		case 168:
			return 8;
		
		case 178:
		case 188:
			return func_409(iParam1, iParam3, iParam4);
		
		case 225:
		case 226:
			return func_405(iParam1, iParam3, iParam4, 0, 0);
		
		case 227:
			return 0;
		
		case 229:
		case 230:
			return func_399(iParam1, iParam3, iParam4, unk_0xF1110FE23C67293A(uParam5), 0, 0, 0);
		
		case 233:
			return func_393(iParam1, iParam3, iParam4, unk_0xF1110FE23C67293A(iParam5), 0, 0, 0);
			return 6;
		
		case 190:
			return 18;
		
		case 191:
			return 0;
		
		case 192:
			return func_392(iParam1, iParam3);
		
		case 179:
			return 0;
		
		case 183:
			return 5;
		
		case 182:
			return 14;
		
		case 185:
			return 24;
		
		case 186:
			return 13;
		
		case 180:
			return 6;
		
		case 193:
			return 6;
		
		case 194:
			return 0;
		
		case 197:
			return 4;
		
		case 199:
			return 0;
		
		case 195:
			return 20;
		
		case 201:
			return 0;
		
		case 198:
			return 8;
		
		case 205:
			return 1;
		
		case 207:
			return 8;
		
		case 208:
			return 2;
		
		case 209:
			return 6;
		
		case 210:
			return 0;
		
		case 214:
			return 5;
		
		case 215:
			return 11;
		
		case 216:
			return 8;
		
		case 217:
			return 24;
		
		case 218:
			return 23;
		
		case 219:
			return 1;
		
		case 220:
			return 4;
		
		case 221:
			return 4;
		
		case 237:
		case 250:
			return func_391(iParam1, iParam3, iParam4);
		
		case 238:
		case 249:
			return func_390(iParam1);
		
		case 239:
			return 4;
		
		case 240:
			return 11;
		
		case 241:
			return 3;
		
		case 242:
			return 5;
		
		case 244:
			return 4;
		
		case 248:
			return 1;
		
		case 243:
			return func_384(iParam1, iParam3, unk_0xF1110FE23C67293A(iParam5), 0, -1, 0, 0);
		
		case 158:
			return func_378(iParam1, iParam3, unk_0xF1110FE23C67293A(iParam5), 0, -1, 0, 0);
		
		case 181:
			return func_372(iParam1, iParam3, unk_0xF1110FE23C67293A(iParam5), 0, -1, 0, 0);
		
		default:
	}
	return 0;
}

int func_372(int iParam0, int iParam1, int iParam2, bool bParam3, int iParam4, bool bParam5, bool bParam6)
{
	int iVar0;
	
	switch (iParam0)
	{
		case 0:
			iVar0 = iVar0;
			break;
	}
	if (!bParam5)
	{
		iVar0 = (iVar0 + func_377(iParam0, -1));
	}
	if (!bParam6)
	{
		if (func_376(0, iParam1) != -1)
		{
			iVar0 = (iVar0 + func_375(func_288(iParam2) + 1, iParam0, iParam2));
		}
	}
	if (!bParam3)
	{
		if (func_374(iParam0, iParam1))
		{
			iVar0 = (iVar0 + func_373(iParam0, iParam1));
		}
	}
	return iVar0;
}

int func_373(int iParam0, int iParam1)
{
	switch (iParam0)
	{
		case 2:
			return 2;
		
		default:
	}
	return 0;
}

bool func_374(int iParam0, int iParam1)
{
	return func_373(iParam0, iParam1) > 0;
}

int func_375(var uParam0, int iParam1, int iParam2)
{
	switch (iParam1)
	{
		case 2:
			return 3;
		
		default:
	}
	return 1;
}

int func_376(int iParam0, int iParam1)
{
	switch (iParam1)
	{
		case 0:
			switch (iParam0)
			{
				case 0:
					return 0;
				
				case 1:
					return 0;
				
				default:
			}
			break;
	}
	return -1;
}

int func_377(int iParam0, int iParam1)
{
	switch (iParam0)
	{
		case 2:
			return 0;
		
		default:
	}
	return 0;
}

int func_378(int iParam0, int iParam1, int iParam2, bool bParam3, int iParam4, bool bParam5, bool bParam6)
{
	int iVar0;
	
	switch (iParam0)
	{
		case 0:
			iVar0++;
			break;
		
		case 24:
			iVar0++;
			break;
		
		case 10:
			switch (iParam1)
			{
				case 30:
					iVar0 += 4;
					break;
				
				case 31:
				case 32:
					iVar0 += 3;
					break;
			}
			break;
		
		case 33:
			switch (iParam1)
			{
				case 89:
					iVar0++;
					break;
				
				case 90:
					iVar0 += 2;
					break;
			}
			break;
		
		case 36:
			iVar0 += 2;
			break;
		
		case 37:
			iVar0 += 2;
			break;
		
		case 38:
			iVar0++;
			break;
		
		case 8:
			iVar0 += 7;
			break;
		
		case 13:
			switch (iParam1)
			{
				case 38:
					iVar0 += 3;
					break;
				
				case 39:
					iVar0 += 3;
					break;
				
				case 40:
					iVar0 += 4;
					break;
			}
			break;
		
		case 16:
			iVar0 += 12;
			break;
		
		case 35:
			iVar0 += 4;
			break;
		
		case 4:
			iVar0 += 16;
			break;
		
		case 46:
			iVar0++;
			break;
		
		case 9:
			iVar0 += 7;
			break;
		
		case 14:
			iVar0 += 7;
			break;
		
		case 17:
			iVar0 += 7;
			break;
		
		case 47:
			iVar0 += 2;
			break;
		
		case 11:
			iVar0 += 3;
			break;
		
		case 18:
			iVar0 += 4;
			break;
		
		case 6:
			iVar0 += 11;
			break;
		
		case 48:
			iVar0 += 2;
			break;
		
		case 44:
			iVar0 += 2;
			break;
		
		case 21:
			iVar0 += 8;
			break;
		
		case 22:
			iVar0 += 3;
			break;
		
		case 7:
			iVar0 += 7;
			break;
		
		case 15:
			iVar0++;
			break;
		
		case 2:
			iVar0++;
			break;
		
		case 34:
			iVar0 += 5;
			break;
		
		case 27:
			iVar0 += 9;
			break;
		
		case 26:
			iVar0 += 3;
			break;
		
		case 31:
			iVar0++;
			break;
		
		case 3:
			iVar0 += 21;
			break;
		
		case 12:
			iVar0 += 20;
			break;
		
		case 41:
			iVar0 += 11;
			break;
		
		case 49:
			iVar0 += 3;
			break;
		
		case 29:
			iVar0++;
			break;
		
		case 5:
			iVar0 += 3;
			break;
		
		case 50:
			iVar0 += 4;
			break;
		
		case 23:
			iVar0 += 4;
			break;
		
		case 20:
			iVar0 += 3;
			break;
		
		case 32:
			iVar0 = iVar0;
			break;
		
		case 51:
			iVar0 += 4;
			break;
		
		case 52:
			iVar0 += 6;
			break;
		
		case 39:
			iVar0 += 2;
			break;
		
		case 40:
			iVar0++;
			break;
		
		case 25:
			iVar0 += 3;
			break;
	}
	if (!bParam5)
	{
		iVar0 = (iVar0 + func_383(iParam0, -1));
	}
	if (!bParam6)
	{
		if (func_382(0, iParam1) != -1)
		{
			iVar0 = (iVar0 + func_381(func_288(iParam2) + 1, iParam0, iParam2));
		}
	}
	if (!bParam3)
	{
		if (func_380(iParam0, iParam1))
		{
			iVar0 = (iVar0 + func_379(iParam0, iParam1));
		}
	}
	return iVar0;
}

int func_379(int iParam0, int iParam1)
{
	switch (iParam0)
	{
		case 0:
		case 13:
		case 18:
		case 34:
		case 27:
		case 49:
		case 48:
		case 6:
		case 50:
		case 7:
		case 16:
		case 47:
		case 5:
		case 25:
		case 26:
		case 22:
		case 11:
		case 17:
		case 14:
			return 2;
		
		default:
	}
	return 0;
}

bool func_380(int iParam0, int iParam1)
{
	return func_379(iParam0, iParam1) > 0;
}

int func_381(int iParam0, int iParam1, int iParam2)
{
	struct<3> Var0;
	
	Var0 = -1;
	Var0.f_1 = -1;
	Var0.f_2 = -1;
	switch (iParam1)
	{
		case 43:
			return 3;
		
		case 17:
		case 18:
		case 15:
			return 2;
		
		case 12:
		case 13:
			if (iParam2 != func_42())
			{
				if (func_250(iParam2, 1))
				{
					Var0 = { func_324(iParam2) };
				}
			}
			else
			{
				return 4;
			}
			if (Var0.f_1 == 4)
			{
				return 4;
			}
			else
			{
				return 2;
			}
			break;
		
		case 53:
			if (iParam0 == 1)
			{
			}
			break;
	}
	return 1;
}

int func_382(int iParam0, int iParam1)
{
	switch (iParam1)
	{
		case 96:
		case 97:
		case 98:
		case 108:
		case 109:
		case 110:
		case 118:
		case 119:
		case 120:
		case 105:
		case 106:
		case 107:
			return 0;
		
		case 66:
		case 67:
		case 68:
		case 30:
		case 31:
		case 32:
		case 121:
		case 122:
		case 123:
			return 1;
		
		case 93:
		case 94:
		case 95:
			return 3;
		
		case 0:
		case 1:
		case 2:
		case 3:
		case 4:
			return 1;
		
		case 116:
		case 117:
			return 2;
		
		case 20:
			return 9;
		
		case 21:
			return 5;
		
		case 27:
		case 28:
		case 29:
			return 7;
		
		case 114:
		case 115:
			return iParam0;
		
		case 46:
		case 47:
		case 48:
			switch (iParam0)
			{
				case 0:
					return 2;
				
				case 1:
					return 3;
				
				case 2:
					return 4;
				
				case 3:
					return 5;
				
				default:
			}
			break;
		
		case 38:
		case 39:
		case 40:
			switch (iParam0)
			{
				case 0:
					return 3;
				
				case 1:
					return 4;
				
				case 2:
					return 5;
				
				case 3:
					return 6;
				
				default:
			}
			break;
		
		case 41:
		case 42:
			switch (iParam0)
			{
				case 0:
					return 5;
				
				case 1:
					return 6;
				
				default:
			}
			break;
		
		case 49:
		case 50:
			switch (iParam0)
			{
				case 0:
					return 7;
				
				case 1:
					return 8;
				
				default:
			}
			break;
		
		case 33:
		case 34:
		case 35:
			switch (iParam0)
			{
				case 0:
				case 1:
					return 3;
				
				default:
			}
			break;
		
		case 51:
		case 52:
			switch (iParam0)
			{
				case 0:
					return 1;
				
				case 1:
					return 2;
				
				default:
			}
			break;
		
		case 43:
		case 44:
		case 45:
			switch (iParam0)
			{
				case 0:
					return 1;
				
				case 1:
					return 2;
				
				default:
			}
			break;
		
		case 36:
		case 37:
			switch (iParam0)
			{
				case 0:
					return 20;
				
				case 1:
					return 21;
				
				case 2:
					return 22;
				
				case 3:
					return 23;
				
				default:
			}
			break;
		
		case 17:
		case 18:
		case 19:
			return 3;
		
		case 131:
			return 4;
	}
	return -1;
}

int func_383(int iParam0, int iParam1)
{
	switch (iParam0)
	{
		case 53:
			return 0;
		
		default:
	}
	return 0;
}

int func_384(int iParam0, int iParam1, int iParam2, bool bParam3, int iParam4, bool bParam5, bool bParam6)
{
	int iVar0;
	
	switch (iParam0)
	{
		case 13:
			iVar0 += 4;
			break;
		
		case 1:
			iVar0 += 10;
			break;
		
		case 15:
			iVar0 += 21;
			break;
		
		case 14:
			iVar0 += 2;
			break;
		
		case 11:
			switch (iParam1)
			{
				case 35:
					iVar0 += 4;
					break;
				
				case 36:
					iVar0 += 5;
					break;
				
				case 37:
					iVar0 += 5;
					break;
			}
			break;
		
		case 5:
			iVar0 += 3;
			break;
		
		case 6:
			iVar0 += 2;
			break;
		
		case 0:
			switch (iParam1)
			{
				case 0:
					iVar0 += 5;
					break;
				
				case 1:
					iVar0 += 5;
					break;
			}
			break;
		
		case 10:
			switch (iParam1)
			{
				case 31:
					iVar0 += 2;
					break;
				
				case 32:
					iVar0++;
					break;
				
				case 33:
					iVar0 += 2;
					break;
				
				case 34:
					iVar0 += 2;
					break;
			}
			break;
		
		case 12:
			iVar0 += 3;
			break;
		
		case 9:
			switch (iParam1)
			{
				case 28:
					switch (iParam4)
					{
						case 0:
							iVar0 += 3;
							break;
						
						case 1:
							iVar0 += 4;
							break;
						
						case 2:
							iVar0 += 3;
							break;
					}
					break;
				
				case 29:
					switch (iParam4)
					{
						case 0:
							iVar0 += 4;
							break;
						
						case 1:
							iVar0 += 3;
							break;
						
						case 2:
							iVar0 += 3;
							break;
					}
					break;
				
				case 30:
					switch (iParam4)
					{
						case 0:
							iVar0 += 3;
							break;
						
						case 1:
							iVar0 += 3;
							break;
						
						case 2:
							iVar0 += 3;
							break;
					}
					break;
			}
			break;
		
		case 3:
			iVar0 += 6;
			break;
		
		case 4:
			switch (iParam1)
			{
				case 12:
					iVar0 += 7;
					break;
				
				case 13:
					iVar0 += 10;
					break;
				
				case 14:
					iVar0 += 8;
					break;
			}
			break;
		
		case 2:
			iVar0 += 3;
			break;
		
		case 8:
			iVar0 = iVar0;
			break;
		
		case 7:
			iVar0++;
			break;
	}
	if (!bParam5)
	{
		if (func_328(iParam0))
		{
			iVar0 = (iVar0 + func_327(iParam0, iParam2));
		}
		iVar0 = (iVar0 + func_389(iParam0, -1));
	}
	if (!bParam6)
	{
		if (func_388(0, iParam1) != -1)
		{
			iVar0 = (iVar0 + func_387(func_288(iParam2) + 1, iParam0));
		}
	}
	if (!bParam3)
	{
		if (func_386(iParam0, iParam1))
		{
			iVar0 = (iVar0 + func_385(iParam0, iParam1));
		}
	}
	return iVar0;
}

int func_385(int iParam0, int iParam1)
{
	switch (iParam0)
	{
		case 4:
		case 9:
		case 8:
			return 2;
		
		default:
	}
	switch (iParam1)
	{
		case 37:
			return 2;
		
		default:
	}
	return 0;
}

bool func_386(int iParam0, int iParam1)
{
	return func_385(iParam0, iParam1) > 0;
}

int func_387(int iParam0, int iParam1)
{
	switch (iParam1)
	{
		case 6:
			if (iParam0 == 1)
			{
				return 1;
			}
			else
			{
				return 2;
			}
			break;
	}
	return 1;
}

int func_388(int iParam0, int iParam1)
{
	switch (iParam1)
	{
		case 46:
		case 47:
		case 48:
			switch (iParam0)
			{
				case 0:
					return 20;
				
				default:
			}
			break;
		
		case 35:
		case 36:
		case 37:
			return 0;
		
		case 2:
		case 3:
		case 4:
		case 5:
			return 0;
		
		case 28:
		case 29:
		case 30:
			return 0;
		
		case 18:
		case 19:
		case 20:
		case 21:
			switch (iParam0)
			{
				case 0:
					return 2;
				
				case 1:
					return 3;
				
				default:
			}
			break;
		
		case 31:
		case 32:
		case 33:
		case 34:
			return 0;
		
		case 38:
		case 39:
		case 40:
			return 0;
		
		case 12:
			return 6;
		
		case 13:
			return 9;
		
		case 14:
			return 7;
		
		case 22:
		case 23:
		case 24:
			return 0;
		
		case 25:
		case 26:
		case 27:
			return 0;
	}
	return -1;
}

int func_389(int iParam0, int iParam1)
{
	switch (iParam0)
	{
		case 16:
			return 0;
		
		default:
	}
	return 0;
}

int func_390(int iParam0)
{
	switch (iParam0)
	{
		case 0:
			return 5;
		
		case 1:
			return 4;
		
		case 2:
			return 6;
		
		case 3:
			return 1;
		
		case 4:
			return 3;
		
		case 5:
			return 4;
		
		default:
	}
	return 0;
}

int func_391(int iParam0, int iParam1, int iParam2)
{
	int iVar0;
	
	switch (iParam0)
	{
		case 12:
			switch (iParam1)
			{
				case 80:
					iVar0++;
					break;
				
				case 81:
					iVar0++;
					break;
				
				case 82:
					iVar0++;
					break;
			}
			break;
		
		case 22:
			iVar0 += 4;
			break;
		
		case 18:
			iVar0 += 4;
			break;
		
		case 9:
			switch (iParam1)
			{
				case 71:
				case 72:
				case 73:
					iVar0 += 3;
					break;
			}
			break;
		
		case 11:
			switch (iParam1)
			{
				case 77:
				case 78:
				case joaat("mpsv_lp0_31"):
					iVar0 += 2;
					break;
			}
			break;
		
		case 15:
			iVar0++;
			break;
		
		case 23:
			iVar0++;
			break;
		
		case 20:
			iVar0 += 4;
			break;
		
		case 17:
			iVar0++;
			break;
		
		case 19:
			iVar0++;
			break;
		
		case 16:
			iVar0++;
			break;
		
		case 3:
			iVar0 += 8;
			break;
		
		case 14:
			iVar0++;
			break;
		
		case 10:
			iVar0 += 8;
			break;
		
		case 4:
		case 24:
		case 2:
			iVar0++;
			break;
		
		case 1:
			iVar0 += 5;
			break;
		
		case 5:
			iVar0 += 3;
			break;
		
		case 0:
			iVar0 += 3;
			break;
	}
	if (func_332(iParam0))
	{
		iVar0 = (iVar0 + iParam2);
	}
	return iVar0;
}

int func_392(int iParam0, int iParam1)
{
	int iVar0;
	
	switch (iParam0)
	{
		case 20:
			iVar0 = iVar0;
			break;
		
		case 0:
			switch (iParam1)
			{
				case 1:
					iVar0 += 7;
					break;
				
				case 2:
					iVar0 += 6;
					break;
				
				case 3:
					iVar0 += 4;
					break;
				
				case 4:
					iVar0 += 2;
					break;
				
				case 5:
					iVar0 += 3;
					break;
				
				case 6:
					iVar0 += 2;
					break;
			}
			break;
		
		case 1:
			switch (iParam1)
			{
				case 7:
					iVar0 += 4;
					break;
				
				case 8:
					iVar0 += 4;
					break;
				
				case 9:
					iVar0 += 2;
					break;
				
				case 10:
					iVar0 += 2;
					break;
				
				case 11:
					iVar0 += 2;
					break;
				
				case 12:
					iVar0 += 4;
					break;
			}
			break;
		
		case 2:
			switch (iParam1)
			{
				case 13:
					iVar0 += 2;
					break;
				
				case 14:
					iVar0 += 4;
					break;
				
				case 15:
					iVar0 += 6;
					break;
				
				case 16:
					iVar0 += 3;
					break;
				
				case 17:
					iVar0 += 3;
					break;
				
				case 18:
					iVar0 += 2;
					break;
			}
			break;
		
		case 4:
			switch (iParam1)
			{
				case 19:
					iVar0 += 5;
					break;
				
				case 20:
					iVar0 += 4;
					break;
				
				case 21:
					iVar0 += 6;
					break;
				
				case 22:
					iVar0 += 2;
					break;
				
				case 23:
					iVar0 += 4;
					break;
				
				case 24:
					iVar0 += 5;
					break;
			}
			break;
		
		case 6:
			switch (iParam1)
			{
				case 19:
					iVar0 += 5;
					break;
				
				case 20:
					iVar0 += 4;
					break;
				
				case 21:
					iVar0 += 6;
					break;
				
				case 22:
					iVar0 += 2;
					break;
				
				case 23:
					iVar0 += 4;
					break;
				
				case 24:
					iVar0 += 5;
					break;
			}
			break;
		
		case 5:
			switch (iParam1)
			{
				case 31:
					iVar0 += 5;
					break;
				
				case 32:
					iVar0 += 2;
					break;
				
				case 33:
					iVar0 += 3;
					break;
				
				case 34:
					iVar0 += 5;
					break;
				
				case 35:
					iVar0 += 5;
					break;
				
				case 36:
					iVar0 += 5;
					break;
			}
			break;
		
		case 9:
			switch (iParam1)
			{
				case 68:
					iVar0 = iVar0;
					break;
				
				case 69:
					iVar0 = iVar0;
					break;
				
				case 70:
					iVar0 = iVar0;
					break;
				
				case 71:
					iVar0 = iVar0;
					break;
				
				case 72:
					iVar0 = iVar0;
					break;
				
				case 73:
					iVar0 = iVar0;
					break;
			}
			break;
		
		case 3:
			switch (iParam1)
			{
				case 74:
					iVar0++;
					break;
				
				case 75:
					iVar0++;
					break;
				
				case 76:
					iVar0++;
					break;
				
				case 77:
					iVar0++;
					break;
			}
			break;
		
		case 10:
			switch (iParam1)
			{
				case 78:
					iVar0 = iVar0;
					break;
				
				case joaat("mpsv_lp0_31"):
					iVar0 = iVar0;
					break;
				
				case 80:
					iVar0 = iVar0;
					break;
				
				case 81:
					iVar0 = iVar0;
					break;
				
				case 82:
					iVar0 = iVar0;
					break;
				
				case 83:
					iVar0 = iVar0;
					break;
				
				case 84:
					iVar0 = iVar0;
					break;
			}
			break;
		
		case 7:
			iVar0 += 4;
			break;
		
		case 8:
			switch (iParam1)
			{
				case 62:
					iVar0 += 6;
					break;
				
				case 63:
					iVar0 += 6;
					break;
				
				case 64:
					iVar0 += 6;
					break;
				
				case 65:
					iVar0 += 6;
					break;
				
				case 66:
					iVar0 += 6;
					break;
				
				case 67:
					iVar0 += 6;
					break;
			}
			break;
		
		case 11:
			switch (iParam1)
			{
				case 85:
					iVar0 = iVar0;
					break;
				
				case 86:
					iVar0 = iVar0;
					break;
				
				case 87:
					iVar0 = iVar0;
					break;
				
				case 88:
					iVar0 = iVar0;
					break;
				
				case 89:
					iVar0 = iVar0;
					break;
			}
			break;
		
		case 12:
			switch (iParam1)
			{
				case 90:
					iVar0 += 2;
					break;
				
				case 91:
					iVar0 += 2;
					break;
				
				case 92:
					iVar0 += 2;
					break;
				
				case 93:
					iVar0 += 2;
					break;
				
				case 94:
					iVar0 += 2;
					break;
			}
			break;
		
		case 13:
			switch (iParam1)
			{
				case 37:
					iVar0 += 3;
					break;
			}
			break;
		
		case 14:
			switch (iParam1)
			{
				case 38:
					iVar0 += 4;
					break;
				
				case 39:
					iVar0 += 4;
					break;
				
				case 40:
					iVar0 += 4;
					break;
				
				case 41:
					iVar0 += 4;
					break;
			}
			break;
		
		case 15:
			iVar0 = iVar0;
			break;
		
		case 16:
			iVar0 = iVar0;
			break;
		
		case 17:
			iVar0 = iVar0;
			break;
		
		case 18:
			iVar0 = iVar0;
			break;
		
		case 19:
			iVar0 = iVar0;
			break;
		
		default:
			iVar0 = iVar0;
			break;
	}
	if ((func_341(iParam0) && iParam0 != 7) || iParam0 == 12)
	{
		iVar0 = (iVar0 + func_335(iParam0, func_339() + 1));
	}
	return iVar0;
}

int func_393(int iParam0, int iParam1, int iParam2, int iParam3, bool bParam4, bool bParam5, bool bParam6)
{
	int iVar0;
	
	switch (iParam0)
	{
		case 0:
			iVar0 = iVar0;
			break;
		
		case 1:
			iVar0 += 5;
			break;
		
		case 2:
			iVar0 += 2;
			break;
		
		case 3:
			iVar0++;
			break;
		
		case 4:
			iVar0 += 3;
			break;
		
		case 5:
			iVar0 += 2;
			break;
		
		case 8:
			iVar0 += 3;
			break;
		
		case 6:
			iVar0++;
			break;
		
		case 12:
			iVar0 += 2;
			break;
		
		case 7:
			iVar0 += 6;
			break;
		
		case 9:
			iVar0 = iVar0;
			break;
		
		case 11:
			iVar0 = iVar0;
			break;
		
		case 13:
			iVar0 += 2;
			break;
		
		case 10:
			iVar0 += 4;
			break;
	}
	if (!bParam5)
	{
		if (func_348(iParam0))
		{
			if (iParam3 != func_42())
			{
				iVar0 = (iVar0 + func_345(iParam0, func_288(iParam3) + 1, iParam3, -1, iParam2));
			}
		}
		if (iParam3 != func_42())
		{
			iVar0 = (iVar0 + func_398(iParam0, func_288(iParam3) + 1));
		}
	}
	if (!bParam6)
	{
		if (func_397(0, iParam1) != -1)
		{
			if (iParam3 != func_42())
			{
				iVar0 = (iVar0 + func_396(func_288(iParam3) + 1, iParam0));
			}
		}
	}
	if (!bParam4)
	{
		if (func_395(iParam0))
		{
			iVar0 = (iVar0 + func_394(iParam0));
		}
	}
	return iVar0;
}

int func_394(int iParam0)
{
	switch (iParam0)
	{
		case 0:
		case 2:
		case 8:
		case 1:
		case 12:
		case 10:
		case 11:
		case 13:
			return 2;
		
		default:
	}
	return 0;
}

bool func_395(int iParam0)
{
	return func_394(iParam0) > 0;
}

int func_396(var uParam0, int iParam1)
{
	return 2;
}

int func_397(int iParam0, int iParam1)
{
	switch (iParam1)
	{
		case 136:
			switch (iParam0)
			{
				case 0:
					return 1;
				
				case 1:
					return 1;
				
				case 2:
					return 2;
				
				case 3:
					return 2;
				
				default:
			}
			break;
	}
	return -1;
}

int func_398(int iParam0, int iParam1)
{
	int iVar0;
	
	switch (iParam0)
	{
		case 0:
			return 0;
		
		case 8:
			if (iParam1 > 4)
			{
				return 4;
			}
			return iParam1;
		
		case 1:
			iVar0 = func_9(5327, -1, 0);
			if (unk_0xCE990E643CD9D0E5(iVar0, 7))
			{
				if (iParam1 > 4)
				{
					return 4;
				}
				return iParam1;
			}
			else
			{
				return 0;
			}
			break;
	}
	return 0;
}

int func_399(int iParam0, int iParam1, int iParam2, int iParam3, bool bParam4, bool bParam5, bool bParam6)
{
	int iVar0;
	
	switch (iParam0)
	{
		case 11:
			iVar0 += 9;
			break;
		
		case 1:
			iVar0 = iVar0;
			break;
		
		case 7:
			iVar0 += 10;
			break;
		
		case 2:
			iVar0 += 12;
			break;
		
		case 4:
			iVar0 += 10;
			break;
		
		case 8:
			iVar0++;
			break;
		
		case 5:
			iVar0 += 2;
			break;
		
		case 12:
			switch (iParam1)
			{
				case 0:
					iVar0++;
					break;
				
				case 1:
					iVar0++;
					break;
				
				case 2:
					iVar0 = iVar0;
					break;
			}
			break;
		
		case 14:
			switch (iParam1)
			{
				case 3:
					iVar0 += 4;
					break;
				
				case 4:
					iVar0 += 2;
					break;
				
				case 5:
					iVar0 += 5;
					break;
			}
			break;
		
		case 3:
			if (iParam2 >= 4)
			{
				iVar0 += 10;
			}
			else if (iParam2 == 3)
			{
				iVar0 += 8;
			}
			else if (iParam2 == 2)
			{
				iVar0 += 7;
			}
			else if (iParam2 == 1)
			{
				iVar0 += 6;
			}
			break;
		
		case 10:
			switch (iParam1)
			{
				case 34:
					iVar0++;
					break;
			}
			break;
		
		case 9:
			switch (iParam1)
			{
				case 6:
					iVar0++;
					break;
				
				case 7:
					iVar0 += 4;
					break;
				
				case 8:
					iVar0 += 2;
					break;
			}
			break;
		
		case 0:
			if (iParam2 >= 3)
			{
				iVar0 += 2;
			}
			else
			{
				iVar0++;
			}
			break;
		
		case 23:
			iVar0 = (iVar0 + iParam2);
			break;
		
		case 20:
			iVar0 += 5;
			break;
		
		case 22:
			iVar0 = (iVar0 + (5 * iParam2));
			break;
		
		case 25:
			iVar0 = (iVar0 + iParam2);
			break;
		
		case 24:
			iVar0 = (iVar0 + (3 * iParam2));
			break;
		
		case 16:
			iVar0 = (iVar0 + (5 * iParam2));
			break;
		
		case 17:
			iVar0 += 5;
			break;
		
		case 26:
			iVar0++;
			break;
	}
	if (!bParam5)
	{
		if (func_360(iParam0))
		{
			if (iParam3 != func_42())
			{
				iVar0 = (iVar0 + func_352(iParam0, func_288(iParam3) + 1, iParam3, -1, iParam2));
			}
		}
		if (iParam3 != func_42())
		{
			iVar0 = (iVar0 + func_404(iParam0, func_288(iParam3) + 1));
		}
	}
	if (!bParam6)
	{
		if (func_403(0, iParam1) != -1)
		{
			if (iParam3 != func_42())
			{
				iVar0 = (iVar0 + func_402(func_288(iParam3) + 1, iParam0));
			}
		}
	}
	if (!bParam4)
	{
		if (func_401(iParam0))
		{
			iVar0 = (iVar0 + func_400(iParam0));
		}
	}
	return iVar0;
}

int func_400(int iParam0)
{
	switch (iParam0)
	{
		case 12:
		case 11:
		case 1:
		case 2:
		case 14:
		case 4:
		case 10:
		case 7:
		case 3:
		case 8:
		case 0:
		case 5:
			return 2;
		
		case 21:
		case 16:
		case 17:
			return 2;
		
		default:
	}
	return 0;
}

bool func_401(int iParam0)
{
	return func_400(iParam0) > 0;
}

int func_402(int iParam0, int iParam1)
{
	switch (iParam0)
	{
		case 1:
			if (func_355(iParam1))
			{
				return 1;
			}
			else
			{
				return 1;
			}
			break;
		
		case 2:
			if (func_355(iParam1))
			{
				return 1;
			}
			else
			{
				return 1;
			}
			break;
		
		case 3:
			if (func_355(iParam1))
			{
				return 2;
			}
			else
			{
				return 2;
			}
			break;
		
		case 4:
			if (func_355(iParam1))
			{
				return 2;
			}
			else
			{
				return 2;
			}
			break;
		
		case 5:
			if (func_355(iParam1))
			{
				return 2;
			}
			else
			{
				return 2;
			}
			break;
		
		case 6:
			if (func_355(iParam1))
			{
				return 2;
			}
			else
			{
				return 2;
			}
			break;
		
		case 7:
			if (func_355(iParam1))
			{
				return 2;
			}
			else
			{
				return 2;
			}
			break;
		
		case 8:
			if (func_355(iParam1))
			{
				return 2;
			}
			else
			{
				return 2;
			}
			break;
	}
	return 1;
}

int func_403(int iParam0, int iParam1)
{
	switch (iParam1)
	{
		case 0:
			switch (iParam0)
			{
				case 0:
					return 1;
				
				case 1:
					return 1;
				
				case 2:
					return 2;
				
				case 3:
					return 2;
				
				default:
			}
			break;
		
		case 1:
			switch (iParam0)
			{
				case 0:
					return 1;
				
				case 1:
					return 1;
				
				case 2:
					return 2;
				
				case 3:
					return 2;
				
				default:
			}
			break;
		
		case 2:
			switch (iParam0)
			{
				case 0:
					return 0;
				
				case 1:
					return 0;
				
				case 2:
					return 1;
				
				case 3:
					return 1;
				
				default:
			}
			break;
		
		case 19:
			switch (iParam0)
			{
				case 0:
					return 12;
				
				case 1:
					return 12;
				
				case 2:
					return 13;
				
				case 3:
					return 13;
				
				default:
			}
			break;
		
		case 20:
			switch (iParam0)
			{
				case 0:
					return 12;
				
				case 1:
					return 12;
				
				case 2:
					return 13;
				
				case 3:
					return 13;
				
				default:
			}
			break;
		
		case 21:
			switch (iParam0)
			{
				case 0:
					return 12;
				
				case 1:
					return 12;
				
				case 2:
					return 13;
				
				case 3:
					return 13;
				
				default:
			}
			break;
		
		case 6:
			switch (iParam0)
			{
				case 0:
					return 1;
				
				case 1:
					return 1;
				
				case 2:
					return 2;
				
				case 3:
					return 2;
				
				default:
			}
			break;
		
		case 7:
			switch (iParam0)
			{
				case 0:
					return 4;
				
				case 1:
					return 4;
				
				case 2:
					return 5;
				
				case 3:
					return 5;
				
				default:
			}
			break;
		
		case 8:
			switch (iParam0)
			{
				case 0:
					return 2;
				
				case 1:
					return 2;
				
				case 2:
					return 3;
				
				case 3:
					return 3;
				
				default:
			}
			break;
		
		case 48:
		case 49:
		case 50:
			switch (iParam0)
			{
				case 0:
					return 2;
				
				case 1:
					return 2;
				
				case 2:
					return 3;
				
				case 3:
					return 3;
				
				default:
			}
			break;
		
		case 9:
		case 10:
		case 11:
		case 12:
		case 13:
		case 14:
		case 15:
		case 16:
		case 17:
		case 18:
			switch (iParam0)
			{
				case 0:
					return 0;
				
				case 1:
					return 0;
				
				case 2:
					return 1;
				
				case 3:
					return 1;
				
				default:
			}
			break;
		
		case 28:
		case 29:
		case 30:
			switch (iParam0)
			{
				case 0:
					return 6;
				
				case 1:
					return 7;
				
				case 2:
					return 8;
				
				case 3:
					return 9;
				
				default:
			}
			break;
		
		case 43:
		case 44:
		case 45:
		case 46:
		case 47:
			switch (iParam0)
			{
				case 0:
					return 0;
				
				case 1:
					return 0;
				
				case 2:
					return 1;
				
				case 3:
					return 1;
				
				default:
			}
			break;
		
		case 40:
		case 41:
		case 42:
			switch (iParam0)
			{
				case 0:
					return 0;
				
				case 1:
					return 0;
				
				case 2:
					return 0;
				
				case 3:
					return 0;
				
				default:
			}
			break;
	}
	return -1;
}

int func_404(int iParam0, int iParam1)
{
	switch (iParam0)
	{
		case 1:
			if (iParam1 > 4)
			{
				return 2;
			}
			return 1;
		
		case 14:
			if (iParam1 > 3)
			{
				return 2;
			}
			return 1;
		
		case 7:
			if (iParam1 > 4)
			{
				return 4;
			}
			return iParam1;
		
		case 2:
		case 4:
		case 3:
			if (iParam1 > 4)
			{
				return 4;
			}
			return iParam1;
		
		case 10:
			if (iParam1 > 4)
			{
				return 2;
			}
			return 1;
		
		case 9:
			return 1;
		
		case 0:
			if (iParam1 > 1)
			{
				return 2;
			}
			return 1;
		
		case 8:
			if (iParam1 > 3)
			{
				return 2;
			}
			return 1;
		
		case 5:
			if (iParam1 > 2)
			{
				return 2;
			}
			return 1;
		
		case 13:
			if (iParam1 > 4)
			{
				return 4;
			}
			return iParam1;
		
		case 11:
			if (iParam1 > 2)
			{
				return 2;
			}
			return 1;
		
		default:
	}
	return unk_0x11E019C8F43ACC8A((IntToFloat(iParam1) / 2f));
}

int func_405(int iParam0, int iParam1, int iParam2, bool bParam3, bool bParam4)
{
	int iVar0;
	
	switch (iParam0)
	{
		case 0:
			iVar0 = iVar0;
			break;
		
		case 1:
			switch (iParam1)
			{
				case 0:
					iVar0 += 3;
					break;
				
				case 1:
					iVar0 += 3;
					break;
				
				case 2:
					iVar0 += 3;
					break;
			}
			break;
		
		case 2:
			if (iParam2 >= 4)
			{
				iVar0 = (iVar0 + iParam2 + 2);
			}
			else
			{
				iVar0 += 5;
			}
			break;
		
		case 3:
			switch (iParam1)
			{
				case 9:
					iVar0 += 3;
					break;
				
				case 10:
					iVar0 += 3;
					break;
				
				case 11:
					iVar0 += 2;
					break;
				
				case 12:
					iVar0 += 2;
					break;
			}
			break;
		
		case 4:
			switch (iParam1)
			{
				case 14:
					iVar0 += 2;
					break;
				
				case 15:
					iVar0 += 4;
					break;
				
				case 13:
					iVar0 += 2;
					break;
			}
			break;
		
		case 5:
			iVar0 += 4;
			break;
		
		case 6:
			switch (iParam1)
			{
				case 19:
					iVar0 += 5;
					break;
				
				case 20:
					iVar0 += 5;
					break;
				
				case 21:
					iVar0 += 5;
					break;
			}
			break;
		
		case 7:
			switch (iParam1)
			{
				case 22:
					iVar0 += 4;
					break;
				
				case 23:
					iVar0 += 3;
					break;
				
				case 24:
					iVar0 += 3;
					break;
			}
			if (func_364(iParam0, func_339() + 1, -1, iParam2) == 1)
			{
				iVar0++;
			}
			break;
		
		case 8:
			switch (iParam1)
			{
				case 25:
					iVar0 += 5;
					break;
				
				case 26:
					iVar0 += 5;
					break;
			}
			break;
		
		case 9:
			switch (iParam1)
			{
				case 27:
					iVar0 += 2;
					break;
				
				case 28:
					iVar0 += 2;
					break;
			}
			break;
		
		case 10:
			switch (iParam1)
			{
				case 29:
					iVar0 += 4;
					break;
				
				case 30:
					iVar0 += 4;
					break;
				
				case 31:
					iVar0 += 4;
					break;
			}
			break;
		
		case 11:
			switch (iParam1)
			{
				case 32:
					iVar0 += 2;
					break;
				
				case 33:
					iVar0 += 2;
					break;
				
				case 34:
					iVar0 += 2;
					break;
			}
			break;
		
		case 12:
			iVar0 = iVar0;
			break;
		
		case 13:
			switch (iParam1)
			{
				case 35:
					iVar0 += 3;
					break;
				
				case 36:
					iVar0 += 3;
					break;
				
				case 37:
					iVar0 += 2;
					break;
			}
			break;
		
		case 14:
			iVar0 += 8;
			break;
		
		case 15:
			iVar0 = iVar0;
			break;
		
		case 16:
			iVar0 = iVar0;
			break;
		
		case 17:
			iVar0 = iVar0;
			break;
		
		case 19:
			iVar0 = iVar0;
			break;
		
		case 18:
			iVar0 = (iVar0 + iParam2);
			break;
		
		case 20:
			iVar0 += 5;
			break;
	}
	if (!bParam4)
	{
		if (func_366(iParam0))
		{
			iVar0 = (iVar0 + func_364(iParam0, func_339() + 1, -1, iParam2));
		}
		iVar0 = (iVar0 + func_408(iParam0, func_339() + 1));
	}
	if (!bParam3)
	{
		if (func_407(iParam0))
		{
			iVar0 = (iVar0 + func_406(iParam0));
		}
	}
	return iVar0;
}

int func_406(int iParam0)
{
	switch (iParam0)
	{
		case 1:
		case 2:
		case 4:
		case 5:
		case 6:
		case 7:
		case 9:
		case 10:
		case 11:
		case 12:
		case 13:
		case 15:
		case 16:
		case 17:
		case 19:
		case 8:
			return 2;
		
		default:
	}
	return 0;
}

bool func_407(int iParam0)
{
	return func_406(iParam0) > 0;
}

int func_408(int iParam0, int iParam1)
{
	switch (iParam0)
	{
		case 2:
			if (iParam1 == 1)
			{
				return 1;
			}
			return unk_0xF34EE736CF047844((IntToFloat(iParam1) / 1.5f));
		
		case 5:
			if (iParam1 == 1)
			{
				return 1;
			}
			return unk_0xF34EE736CF047844((IntToFloat(iParam1) / 2f));
		
		default:
	}
	return 0;
}

int func_409(int iParam0, int iParam1, int iParam2)
{
	int iVar0;
	
	switch (iParam0)
	{
		case 0:
			iVar0 = iVar0;
			break;
		
		case 1:
			iVar0 = iVar0;
			break;
		
		case 7:
			iVar0 = iVar0;
			break;
		
		case 8:
			iVar0 = iVar0;
			break;
		
		case 5:
			iVar0 = iVar0;
			break;
		
		case 11:
			iVar0 += 3;
			break;
		
		case 13:
			iVar0 = iVar0;
			break;
		
		case 4:
			switch (iParam1)
			{
				case 1:
					iVar0 += 2;
					break;
				
				case 2:
					iVar0 += 2;
					break;
				
				case 3:
					iVar0 += 2;
					break;
			}
			break;
		
		case 6:
			switch (iParam1)
			{
				case 18:
					iVar0 += 2;
					break;
				
				case 19:
					iVar0 += 2;
					break;
				
				case 20:
					iVar0 += 2;
					break;
			}
			break;
		
		case 10:
			switch (iParam1)
			{
				case 21:
					iVar0 += 7;
					break;
				
				case 22:
					iVar0 += 7;
					break;
				
				case 23:
					iVar0 += 7;
					break;
			}
			break;
		
		case 9:
			switch (iParam1)
			{
				case 27:
					iVar0 = iVar0;
					break;
				
				case 28:
					iVar0 = iVar0;
					break;
				
				case 29:
					iVar0 = iVar0;
					break;
			}
			break;
		
		case 3:
			switch (iParam1)
			{
				case 30:
					iVar0 += 2;
					break;
				
				case 31:
					iVar0 += 2;
					break;
				
				case 32:
					iVar0 += 2;
					break;
			}
			break;
		
		case 14:
			switch (iParam1)
			{
				case 36:
					iVar0 += 7;
					break;
				
				case 37:
					iVar0 += 7;
					break;
				
				case 38:
					iVar0 += 7;
					break;
			}
			break;
		
		case 2:
			switch (iParam1)
			{
				case 39:
					iVar0 += 4;
					break;
				
				case 40:
					iVar0 += 4;
					break;
				
				case 41:
					iVar0 += 6;
					break;
			}
			break;
		
		case 17:
			switch (iParam1)
			{
				case 42:
					iVar0 += 2;
					break;
				
				case 43:
					iVar0 += 2;
					break;
				
				case 44:
					iVar0 += 2;
					break;
			}
			break;
		
		case 15:
			switch (iParam1)
			{
				case 45:
					iVar0 += 6;
					break;
				
				case 46:
					iVar0 += 6;
					break;
				
				case 47:
					iVar0 += 6;
					break;
			}
			break;
		
		case 16:
			switch (iParam1)
			{
				case 48:
					iVar0 += 2;
					break;
				
				case 49:
					iVar0 += 2;
					break;
				
				case 50:
					iVar0 += 2;
					break;
			}
			break;
		
		case 12:
			iVar0 += 11;
			break;
	}
	iVar0 = (iVar0 + func_412(iParam0, func_339() + 1, iParam2));
	iVar0 = (iVar0 + func_410(iParam0));
	return iVar0;
}

int func_410(int iParam0)
{
	if (func_411(iParam0))
	{
		return 2;
	}
	return 0;
}

int func_411(int iParam0)
{
	switch (iParam0)
	{
		case 7:
		case 2:
		case 9:
		case 4:
		case 17:
		case 15:
		case 16:
		case 8:
		case 10:
			return 1;
		
		default:
	}
	return 0;
}

int func_412(int iParam0, var uParam1, int iParam2)
{
	int iVar0;
	
	if (iVar0 == 0)
	{
		if (iParam0 == 8)
		{
			iVar0 = iParam2;
		}
		else
		{
			iVar0 = 1;
		}
	}
	if (iVar0 == 0)
	{
		iVar0 = 1;
	}
	return iVar0;
}

int func_413(int iParam0)
{
	switch (iParam0)
	{
		case 0:
			return 1;
		
		case 1:
			return 1;
		
		case 2:
			return 0;
		
		case 3:
			return 3;
		
		case 4:
			return 8;
		
		case 5:
			return 3;
		
		case 6:
			return 1;
		
		case 7:
			return 1;
		
		case 8:
			return 6;
		
		case 9:
			return 6;
		
		case 10:
			return 3;
		
		case 11:
			return 3;
		
		case 12:
			return 2;
		
		case 13:
			return 1;
		
		case 14:
			return 3;
		
		case 15:
			return 5;
		
		case 16:
			return 6;
		
		case 17:
			return 2;
		
		case 19:
			return 4;
		
		case 18:
			return 5;
		
		default:
	}
	return 2;
}

int func_414(int iParam0, int iParam1, int iParam2, int iParam3)
{
	switch (iParam0)
	{
		case 148:
		case 152:
		case 162:
		case 160:
		case 163:
			return 0;
		
		case 167:
			return func_441(iParam1);
		
		case 168:
			return 12;
		
		case 170:
			return 4;
		
		case 171:
			return 0;
		
		case 172:
			return 0;
		
		case 173:
			return 4;
		
		case 178:
		case 188:
			return func_438(iParam1, iParam2, iParam3);
		
		case 225:
		case 226:
			return func_435(iParam1, iParam2, iParam3, 0);
		
		case 227:
			return 0;
		
		case 229:
		case 230:
			return func_432(iParam1, iParam2, iParam3, 0);
		
		case 233:
			return func_429(iParam1, iParam2, iParam3, 0);
		
		case 190:
			return 20;
		
		case 191:
			return 4;
		
		case 192:
			return func_428(iParam1, iParam2);
		
		case 179:
			return 0;
		
		case 183:
			return 15;
		
		case 182:
			return 20;
		
		case 185:
			return 21;
		
		case 186:
			return 36;
		
		case 180:
			return 10;
		
		case 193:
			return 10;
		
		case 194:
			return 0;
		
		case 197:
			return 12;
		
		case 199:
			return 6;
		
		case 195:
			return 15;
		
		case 201:
			return 0;
		
		case 198:
			return 18;
		
		case 205:
			return 1;
		
		case 207:
			return 24;
		
		case 208:
			return 14;
		
		case 209:
			return 11;
		
		case 210:
			return 0;
		
		case 214:
			return 12;
		
		case 215:
			return 20;
		
		case 216:
			return 20;
		
		case 217:
			return func_427();
		
		case 218:
			return func_426();
		
		case 219:
			return 0;
		
		case 220:
			return 0;
		
		case 221:
			return 0;
		
		case 237:
		case 250:
			return func_425(iParam1, iParam2, iParam3);
		
		case 238:
		case 249:
			return func_424(iParam1, iParam2);
		
		case 239:
			return 6;
		
		case 240:
			return 11;
		
		case 241:
			return 14;
		
		case 242:
			return 14;
		
		case 244:
			return 9;
		
		case 248:
			return 10;
		
		case 243:
			return func_421(iParam1, iParam2, 0, -1);
		
		case 158:
			return func_418(iParam1, iParam2, 0, -1);
		
		case 181:
			return func_415(iParam1, iParam2, 0, -1);
		
		default:
	}
	return 0;
}

int func_415(int iParam0, int iParam1, bool bParam2, int iParam3)
{
	int iVar0;
	
	switch (iParam0)
	{
		case 2:
			iVar0 += 6;
			break;
	}
	if (!bParam2)
	{
		if (func_374(iParam0, iParam1))
		{
			iVar0 = (iVar0 + func_416(iParam0, iParam1));
		}
	}
	return iVar0;
}

int func_416(int iParam0, var uParam1)
{
	if (func_374(iParam0, uParam1))
	{
		return (func_373(iParam0, uParam1) + func_417(iParam0, uParam1));
	}
	return 0;
}

int func_417(int iParam0, var uParam1)
{
	switch (iParam0)
	{
		case 2:
			return 2;
		
		default:
	}
	return 0;
}

int func_418(int iParam0, int iParam1, bool bParam2, int iParam3)
{
	int iVar0;
	
	switch (iParam0)
	{
		case 0:
			iVar0 += 6;
			break;
		
		case 24:
			iVar0 += 2;
			break;
		
		case 10:
			switch (iParam1)
			{
				case 30:
					iVar0 += 12;
					break;
				
				case 31:
				case 32:
					iVar0 += 10;
					break;
			}
			break;
		
		case 33:
			switch (iParam1)
			{
				case 88:
					iVar0 += 10;
					break;
				
				case 89:
					iVar0 += 9;
					break;
				
				case 90:
					iVar0 += 10;
					break;
			}
			break;
		
		case 36:
			iVar0 += 9;
			break;
		
		case 43:
			iVar0 += 6;
			break;
		
		case 38:
			iVar0 += 6;
			break;
		
		case 40:
			iVar0 += 6;
			break;
		
		case 8:
			iVar0 += 16;
			break;
		
		case 16:
			iVar0 += 14;
			break;
		
		case 35:
			iVar0 += 13;
			break;
		
		case 4:
			iVar0 += 32;
			break;
		
		case 45:
			iVar0 += 2;
			break;
		
		case 46:
			iVar0 += 9;
			break;
		
		case 9:
			iVar0 += 15;
			break;
		
		case 14:
			iVar0 += 16;
			break;
		
		case 17:
			iVar0 += 14;
			break;
		
		case 47:
			iVar0 += 13;
			break;
		
		case 11:
			iVar0 += 11;
			break;
		
		case 18:
			iVar0 += 24;
			break;
		
		case 6:
			iVar0 += 14;
			break;
		
		case 48:
			iVar0 += 15;
			break;
		
		case 44:
			iVar0 += 9;
			break;
		
		case 21:
			iVar0 += 20;
			break;
		
		case 22:
			iVar0 += 14;
			break;
		
		case 7:
			iVar0 += 22;
			break;
		
		case 15:
			iVar0 += 10;
			break;
		
		case 2:
			iVar0 += 2;
			break;
		
		case 34:
			iVar0 += 18;
			break;
		
		case 27:
			iVar0 += 16;
			break;
		
		case 26:
			iVar0 += 10;
			break;
		
		case 23:
			iVar0 += 10;
			break;
		
		case 31:
			iVar0 += 20;
			break;
		
		case 3:
			iVar0 += 14;
			break;
		
		case 12:
			iVar0 += 7;
			break;
		
		case 41:
			iVar0 += 10;
			break;
		
		case 49:
			iVar0 += 30;
			break;
		
		case 19:
			iVar0 += 15;
			break;
		
		case 5:
			iVar0 += 7;
			break;
		
		case 30:
			iVar0++;
			break;
		
		case 13:
			iVar0 += 20;
			break;
		
		case 29:
			iVar0 += 9;
			break;
		
		case 50:
			iVar0 += 24;
			break;
		
		case 20:
			iVar0 += 26;
			break;
		
		case 32:
			iVar0 += 30;
			break;
		
		case 28:
			iVar0 += 3;
			break;
		
		case 42:
			iVar0 += 13;
			break;
		
		case 52:
			iVar0 += 12;
			break;
		
		case 39:
			iVar0 += 5;
			break;
		
		case 37:
			iVar0 += 6;
			break;
		
		case 25:
			iVar0 += 6;
			break;
		
		case 51:
			iVar0 += 20;
			break;
	}
	if (!bParam2)
	{
		if (func_380(iParam0, iParam1))
		{
			iVar0 = (iVar0 + func_419(iParam0, iParam1));
		}
	}
	return iVar0;
}

int func_419(int iParam0, int iParam1)
{
	if (func_380(iParam0, iParam1))
	{
		return (func_379(iParam0, iParam1) + func_420(iParam0, iParam1));
	}
	return 0;
}

int func_420(int iParam0, int iParam1)
{
	switch (iParam0)
	{
		case 0:
		case 13:
		case 18:
		case 34:
		case 49:
		case 48:
		case 7:
		case 16:
		case 5:
		case 26:
		case 22:
		case 25:
		case 11:
		case 17:
		case 14:
			return 2;
		
		case 6:
		case 50:
		case 47:
			return 4;
		
		case 27:
			return 2;
		
		default:
	}
	return 0;
}

int func_421(int iParam0, int iParam1, bool bParam2, int iParam3)
{
	int iVar0;
	
	switch (iParam0)
	{
		case 13:
			iVar0 += 8;
			break;
		
		case 1:
			switch (iParam1)
			{
				case 2:
					iVar0 += 10;
					break;
				
				case 3:
					iVar0 += 10;
					break;
				
				case 4:
					iVar0 += 14;
					break;
				
				case 5:
					iVar0 += 10;
					break;
			}
			break;
		
		case 15:
			iVar0 += 20;
			break;
		
		case 14:
			iVar0 += 19;
			break;
		
		case 11:
			switch (iParam1)
			{
				case 35:
					iVar0 += 6;
					break;
				
				case 36:
					iVar0 += 7;
					break;
				
				case 37:
					iVar0 += 17;
					break;
			}
			break;
		
		case 6:
			iVar0 += 8;
			break;
		
		case 5:
			iVar0 += 11;
			break;
		
		case 0:
			iVar0 += 12;
			break;
		
		case 10:
			switch (iParam1)
			{
				case 31:
					iVar0++;
					break;
				
				case 32:
					iVar0 += 7;
					break;
				
				case 33:
					iVar0 += 2;
					break;
				
				case 34:
					iVar0++;
					break;
			}
			break;
		
		case 12:
			iVar0 += 30;
			break;
		
		case 9:
			switch (iParam1)
			{
				case 28:
					iVar0 += 10;
					break;
				
				case 29:
					iVar0 += 8;
					break;
				
				case 30:
					iVar0 += 9;
					break;
			}
			break;
		
		case 3:
			iVar0 += 19;
			break;
		
		case 4:
			switch (iParam1)
			{
				case 12:
					iVar0 += 12;
					break;
				
				case 13:
					iVar0 += 14;
					break;
				
				case 14:
					iVar0 += 12;
					break;
			}
			break;
		
		case 2:
			iVar0 += 12;
			break;
		
		case 8:
			iVar0 += 7;
			break;
		
		case 7:
			iVar0++;
			break;
	}
	if (!bParam2)
	{
		if (func_386(iParam0, iParam1))
		{
			iVar0 = (iVar0 + func_422(iParam0, iParam1));
		}
	}
	return iVar0;
}

int func_422(int iParam0, int iParam1)
{
	if (func_386(iParam0, iParam1))
	{
		return (func_385(iParam0, iParam1) + func_423(iParam0, iParam1));
	}
	return 0;
}

int func_423(int iParam0, int iParam1)
{
	switch (iParam0)
	{
		case 4:
		case 9:
		case 8:
			return 2;
		
		default:
	}
	switch (iParam1)
	{
		case 37:
			return 2;
		
		default:
	}
	return 0;
}

int func_424(int iParam0, int iParam1)
{
	switch (iParam0)
	{
		case 0:
			return 14;
		
		case 1:
			return 7;
		
		case 2:
			return 27;
		
		case 3:
			return 3;
		
		case 4:
			return 8;
		
		case 5:
			switch (iParam1)
			{
				case 0:
				case 2:
					return 11;
				
				case 1:
					return 9;
				
				default:
			}
			break;
	}
	return 0;
}

int func_425(int iParam0, int iParam1, int iParam2)
{
	int iVar0;
	
	switch (iParam0)
	{
		case 0:
			switch (iParam1)
			{
				case 0:
				case 1:
				case 2:
				case 3:
				case 4:
				case 5:
				case 6:
				case 7:
				case 8:
				case 9:
					iVar0 += 19;
					break;
			}
			break;
		
		case 11:
			switch (iParam1)
			{
				case 77:
				case 78:
				case joaat("mpsv_lp0_31"):
					iVar0 += 2;
					break;
			}
			break;
		
		case 9:
			switch (iParam1)
			{
				case 71:
				case 72:
				case 73:
					iVar0 += 9;
					break;
			}
			break;
		
		case 12:
			switch (iParam1)
			{
				case 80:
				case 81:
				case 82:
					iVar0 += 3;
					break;
			}
			break;
		
		case 22:
			iVar0 += 8;
			break;
		
		case 15:
			iVar0++;
			break;
		
		case 23:
			iVar0 += 3;
			break;
		
		case 20:
			iVar0 += 12;
			break;
		
		case 18:
			iVar0 += 20;
			break;
		
		case 17:
			iVar0 += 9;
			break;
		
		case 19:
			iVar0++;
			break;
		
		case 3:
			iVar0 += 5;
			break;
		
		case 14:
			iVar0++;
			break;
		
		case 10:
			iVar0 += 17;
			break;
		
		case 4:
			iVar0 += 3;
			break;
		
		case 1:
			iVar0 += 30;
			break;
		
		case 2:
			iVar0 += 4;
			break;
		
		case 5:
			iVar0 += 16;
			break;
	}
	return iVar0;
}

int func_426()
{
	int iVar0;
	
	iVar0 = 32;
	return iVar0;
}

int func_427()
{
	int iVar0;
	
	iVar0 = 8;
	return iVar0;
}

int func_428(int iParam0, int iParam1)
{
	int iVar0;
	
	switch (iParam0)
	{
		case 20:
			iVar0 += 2;
			break;
		
		case 0:
			switch (iParam1)
			{
				case 1:
					iVar0 += 16;
					break;
				
				case 2:
					iVar0 += 14;
					break;
				
				case 3:
					iVar0 += 13;
					break;
				
				case 4:
					iVar0 += 12;
					break;
				
				case 5:
					iVar0 += 12;
					break;
				
				case 6:
					iVar0 += 12;
					break;
			}
			break;
		
		case 1:
			switch (iParam1)
			{
				case 7:
					iVar0 += 12;
					break;
				
				case 8:
					iVar0 += 12;
					break;
				
				case 9:
					iVar0 += 12;
					break;
				
				case 10:
					iVar0 += 12;
					break;
				
				case 11:
					iVar0 += 12;
					break;
				
				case 12:
					iVar0 += 12;
					break;
			}
			break;
		
		case 2:
			switch (iParam1)
			{
				case 13:
					iVar0 += 17;
					break;
				
				case 14:
					iVar0 += 12;
					break;
				
				case 15:
					iVar0 += 13;
					break;
				
				case 16:
					iVar0 += 14;
					break;
				
				case 17:
					iVar0 += 12;
					break;
				
				case 18:
					iVar0 += 14;
					break;
			}
			break;
		
		case 4:
			switch (iParam1)
			{
				case 19:
					iVar0 += 14;
					break;
				
				case 20:
					iVar0 += 14;
					break;
				
				case 21:
					iVar0 += 14;
					break;
				
				case 22:
					iVar0 += 14;
					break;
				
				case 23:
					iVar0 += 14;
					break;
				
				case 24:
					iVar0 += 14;
					break;
			}
			break;
		
		case 6:
			switch (iParam1)
			{
				case 25:
					iVar0 += 4;
					break;
				
				case 26:
					iVar0 += 4;
					break;
				
				case 27:
					iVar0 += 4;
					break;
				
				case 28:
					iVar0 += 4;
					break;
				
				case 29:
					iVar0 += 4;
					break;
				
				case 30:
					iVar0 += 4;
					break;
			}
			break;
		
		case 5:
			switch (iParam1)
			{
				case 31:
					iVar0 += 13;
					break;
				
				case 32:
					iVar0 += 13;
					break;
				
				case 33:
					iVar0 += 13;
					break;
				
				case 34:
					iVar0 += 13;
					break;
				
				case 35:
					iVar0 += 13;
					break;
				
				case 36:
					iVar0 += 13;
					break;
			}
			break;
		
		case 9:
			switch (iParam1)
			{
				case 68:
					iVar0 = iVar0;
					break;
				
				case 69:
					iVar0 = iVar0;
					break;
				
				case 70:
					iVar0 = iVar0;
					break;
				
				case 71:
					iVar0 = iVar0;
					break;
				
				case 72:
					iVar0 = iVar0;
					break;
				
				case 73:
					iVar0 = iVar0;
					break;
			}
			break;
		
		case 3:
			switch (iParam1)
			{
				case 74:
					iVar0 += 10;
					break;
				
				case 75:
					iVar0 += 10;
					break;
				
				case 76:
					iVar0 += 10;
					break;
				
				case 77:
					iVar0 += 10;
					break;
			}
			break;
		
		case 10:
			switch (iParam1)
			{
				case 78:
					iVar0 += 3;
					break;
				
				case joaat("mpsv_lp0_31"):
					iVar0 += 3;
					break;
				
				case 80:
					iVar0 += 3;
					break;
				
				case 81:
					iVar0 += 3;
					break;
				
				case 82:
					iVar0 += 5;
					break;
				
				case 83:
					iVar0 += 3;
					break;
				
				case 84:
					iVar0 += 3;
					break;
			}
			break;
		
		case 7:
			iVar0 += 8;
			break;
		
		case 8:
			switch (iParam1)
			{
				case 62:
					iVar0 += 8;
					break;
				
				case 63:
					iVar0 += 8;
					break;
				
				case 64:
					iVar0 += 8;
					break;
				
				case 65:
					iVar0 += 8;
					break;
				
				case 66:
					iVar0 += 8;
					break;
				
				case 67:
					iVar0 += 8;
					break;
			}
			break;
		
		case 11:
			switch (iParam1)
			{
				case 85:
					iVar0 += 3;
					break;
				
				case 86:
					iVar0 += 3;
					break;
				
				case 87:
					iVar0 += 3;
					break;
				
				case 88:
					iVar0 += 3;
					break;
				
				case 89:
					iVar0 += 3;
					break;
			}
			break;
		
		case 12:
			switch (iParam1)
			{
				case 90:
					iVar0 += 2;
					break;
				
				case 91:
					iVar0 += 2;
					break;
				
				case 92:
					iVar0 += 2;
					break;
				
				case 93:
					iVar0 += 2;
					break;
				
				case 94:
					iVar0 += 2;
					break;
			}
			break;
		
		case 13:
			switch (iParam1)
			{
				case 37:
					iVar0 += 18;
					break;
			}
			break;
		
		case 14:
			switch (iParam1)
			{
				case 38:
					iVar0 += 8;
					break;
				
				case 39:
					iVar0 += 8;
					break;
				
				case 40:
					iVar0 += 8;
					break;
				
				case 41:
					iVar0 += 8;
					break;
			}
			break;
		
		case 15:
			iVar0 = iVar0;
			break;
		
		case 16:
			iVar0 = iVar0;
			break;
		
		case 17:
			iVar0 = iVar0;
			break;
		
		case 18:
			iVar0 = iVar0;
			break;
		
		case 19:
			iVar0 = iVar0;
			break;
		
		default:
			iVar0 = iVar0;
			break;
	}
	if (iParam0 == 12 || iParam0 == 9)
	{
		iVar0 = (iVar0 + func_335(iParam0, func_339() + 1));
	}
	return iVar0;
}

int func_429(int iParam0, int iParam1, int iParam2, bool bParam3)
{
	int iVar0;
	
	switch (iParam0)
	{
		case 0:
			iVar0 = iVar0;
			break;
		
		case 1:
			iVar0 += 14;
			break;
		
		case 2:
			iVar0 += 12;
			break;
		
		case 3:
			iVar0 += 23;
			break;
		
		case 4:
			iVar0 += 20;
			break;
		
		case 5:
			iVar0 += 5;
			break;
		
		case 8:
			iVar0 += 11;
			break;
		
		case 6:
			iVar0 += 7;
			break;
		
		case 12:
			iVar0 += 8;
			break;
		
		case 7:
			iVar0 += 18;
			break;
		
		case 9:
			iVar0 = iVar0;
			break;
		
		case 11:
			iVar0 += 12;
			break;
		
		case 13:
			iVar0++;
			break;
		
		case 10:
			iVar0 += 16;
			break;
	}
	if (!bParam3)
	{
		if (func_395(iParam0))
		{
			iVar0 = (iVar0 + func_430(iParam0, iParam1));
		}
	}
	return iVar0;
}

int func_430(int iParam0, var uParam1)
{
	if (func_395(iParam0))
	{
		return (func_394(iParam0) + func_431(iParam0, uParam1));
	}
	return 0;
}

int func_431(int iParam0, var uParam1)
{
	switch (iParam0)
	{
		case 0:
			return 6;
		
		case 2:
		case 12:
		case 1:
		case 11:
		case 10:
			return 2;
		
		case 8:
			return 4;
		
		default:
	}
	return 0;
}

int func_432(int iParam0, int iParam1, int iParam2, bool bParam3)
{
	int iVar0;
	
	switch (iParam0)
	{
		case 12:
			iVar0 += 16;
			break;
		
		case 11:
			iVar0 += 21;
			break;
		
		case 1:
			iVar0 += 4;
			break;
		
		case 14:
			iVar0 += 15;
			break;
		
		case 7:
			iVar0 += 10;
			break;
		
		case 2:
			iVar0 += 15;
			break;
		
		case 4:
			iVar0 += 9;
			break;
		
		case 10:
			iVar0 += 20;
			break;
		
		case 3:
			iVar0 += 7;
			break;
		
		case 8:
			iVar0 += 2;
			break;
		
		case 5:
			iVar0 += 10;
			break;
		
		case 9:
			switch (iParam1)
			{
				case 7:
					iVar0 += 17;
					break;
				
				default:
					iVar0 += 16;
					break;
			}
			break;
		
		case 0:
			if (iParam2 >= 3)
			{
				iVar0 += 2;
			}
			else
			{
				iVar0++;
			}
			break;
		
		case 23:
			iVar0 = (iVar0 + iParam2);
			break;
		
		case 20:
			iVar0 += 10;
			break;
		
		case 22:
			iVar0 = (iVar0 + (5 * iParam2));
			break;
		
		case 25:
			iVar0 = (iVar0 + iParam2);
			break;
		
		case 24:
			iVar0 = (iVar0 + (3 * iParam2));
			break;
		
		case 26:
			iVar0 += 2;
			break;
	}
	if (!bParam3)
	{
		if (func_401(iParam0))
		{
			iVar0 = (iVar0 + func_433(iParam0, iParam1));
		}
	}
	return iVar0;
}

int func_433(int iParam0, int iParam1)
{
	if (func_401(iParam0))
	{
		return (func_400(iParam0) + func_434(iParam0, iParam1));
	}
	return 0;
}

int func_434(int iParam0, int iParam1)
{
	switch (iParam0)
	{
		case 11:
		case 14:
		case 2:
		case 10:
		case 5:
			return 2;
		
		case 1:
		case 0:
			return 4;
		
		case 4:
			return 1;
		
		case 12:
			switch (iParam1)
			{
				case 2:
					return 2;
				
				default:
			}
			break;
		
		case 21:
			return 2;
		
		case 17:
			return 4;
	}
	return 0;
}

int func_435(int iParam0, int iParam1, int iParam2, bool bParam3)
{
	int iVar0;
	
	switch (iParam0)
	{
		case 0:
			iVar0 = iVar0;
			break;
		
		case 1:
			switch (iParam1)
			{
				case 0:
					iVar0 += 18;
					break;
				
				case 1:
					iVar0 += 18;
					break;
				
				case 2:
					iVar0 += 18;
					break;
			}
			break;
		
		case 2:
			if (iParam2 >= 4)
			{
				iVar0 = (iVar0 + iParam2 + 2);
			}
			else
			{
				iVar0 += 5;
			}
			break;
		
		case 3:
			switch (iParam1)
			{
				case 9:
					iVar0 += 4;
					break;
				
				case 10:
					iVar0 += 4;
					break;
				
				case 11:
					iVar0 += 4;
					break;
				
				case 12:
					iVar0 += 4;
					break;
			}
			break;
		
		case 4:
			switch (iParam1)
			{
				case 13:
					iVar0 += 20;
					break;
				
				case 14:
					iVar0 += 20;
					break;
				
				case 15:
					iVar0 += 20;
					break;
			}
			break;
		
		case 5:
			iVar0 += 12;
			break;
		
		case 6:
			switch (iParam1)
			{
				case 19:
					iVar0 += 24;
					break;
				
				case 20:
					iVar0 += 24;
					break;
				
				case 21:
					iVar0 += 24;
					break;
			}
			break;
		
		case 7:
			switch (iParam1)
			{
				case 22:
					iVar0 += 12;
					break;
				
				case 23:
					iVar0 += 12;
					break;
				
				case 24:
					iVar0 += 12;
					break;
			}
			break;
		
		case 8:
			switch (iParam1)
			{
				case 25:
					iVar0 += 16;
					break;
				
				case 26:
					iVar0 += 15;
					break;
			}
			break;
		
		case 9:
			switch (iParam1)
			{
				case 27:
					iVar0 += 25;
					break;
				
				case 28:
					iVar0 += 25;
					break;
			}
			break;
		
		case 10:
			switch (iParam1)
			{
				case 29:
					iVar0 += 15;
					break;
				
				case 30:
					iVar0 += 15;
					break;
				
				case 31:
					iVar0 += 15;
					break;
			}
			break;
		
		case 11:
			switch (iParam1)
			{
				case 32:
					iVar0 += 12;
					break;
				
				case 33:
					iVar0 += 12;
					break;
				
				case 34:
					iVar0 += 12;
					break;
			}
			break;
		
		case 12:
			iVar0 = iVar0;
			break;
		
		case 13:
			switch (iParam1)
			{
				case 35:
					iVar0 += 22;
					break;
				
				case 36:
					iVar0 += 22;
					break;
				
				case 37:
					iVar0 += 22;
					break;
			}
			break;
		
		case 14:
			iVar0 += 16;
			break;
		
		case 15:
			iVar0 = iVar0;
			break;
		
		case 16:
			iVar0 = iVar0;
			break;
		
		case 17:
			iVar0 = iVar0;
			break;
		
		case 18:
			iVar0 = iVar0;
			break;
		
		case 19:
			iVar0 = iVar0;
			break;
		
		case 20:
			iVar0 += 22;
			break;
	}
	if (!bParam3)
	{
		if (func_407(iParam0))
		{
			iVar0 = (iVar0 + func_436(iParam0));
		}
	}
	return iVar0;
}

int func_436(int iParam0)
{
	if (func_407(iParam0))
	{
		return (func_406(iParam0) + func_437(iParam0));
	}
	return 0;
}

int func_437(int iParam0)
{
	switch (iParam0)
	{
		case 1:
		case 2:
		case 4:
		case 6:
		case 7:
		case 9:
		case 12:
		case 13:
		case 17:
		case 11:
			return 2;
		
		case 15:
		case 16:
		case 19:
			return 3;
		
		case 10:
		case 8:
		case 5:
			return 4;
		
		default:
	}
	return 0;
}

int func_438(int iParam0, int iParam1, var uParam2)
{
	int iVar0;
	
	switch (iParam0)
	{
		case 0:
			iVar0 = iVar0;
			break;
		
		case 1:
			iVar0 += 5;
			break;
		
		case 7:
			iVar0 += 6;
			break;
		
		case 8:
			iVar0 = iVar0;
			break;
		
		case 5:
			iVar0 = iVar0;
			break;
		
		case 11:
			iVar0 += 6;
			break;
		
		case 2:
			iVar0 += 10;
			break;
		
		case 12:
			iVar0 += 11;
			break;
		
		case 13:
			iVar0 += 10;
			break;
		
		case 4:
			switch (iParam1)
			{
				case 1:
					iVar0 += 12;
					break;
				
				case 2:
					iVar0 += 12;
					break;
				
				case 3:
					iVar0 += 12;
					break;
			}
			break;
		
		case 6:
			switch (iParam1)
			{
				case 18:
					iVar0 += 8;
					break;
				
				case 19:
					iVar0 += 8;
					break;
				
				case 20:
					iVar0 += 8;
					break;
			}
			break;
		
		case 10:
			switch (iParam1)
			{
				case 21:
					iVar0 += 14;
					break;
				
				case 22:
					iVar0 += 14;
					break;
				
				case 23:
					iVar0 += 14;
					break;
			}
			break;
		
		case 9:
			switch (iParam1)
			{
				case 27:
					iVar0 += 14;
					break;
				
				case 28:
					iVar0 += 14;
					break;
				
				case 29:
					iVar0 += 14;
					break;
			}
			break;
		
		case 3:
			switch (iParam1)
			{
				case 30:
					iVar0 += 3;
					break;
				
				case 31:
					iVar0 += 3;
					break;
				
				case 32:
					iVar0 += 3;
					break;
			}
			break;
		
		case 14:
			switch (iParam1)
			{
				case 36:
					iVar0 += 18;
					break;
				
				case 37:
					iVar0 += 18;
					break;
				
				case 38:
					iVar0 += 18;
					break;
			}
			break;
		
		case 17:
			switch (iParam1)
			{
				case 42:
					iVar0 += 10;
					break;
				
				case 43:
					iVar0 += 10;
					break;
				
				case 44:
					iVar0 += 10;
					break;
			}
			break;
		
		case 15:
			switch (iParam1)
			{
				case 45:
					iVar0 += 17;
					break;
				
				case 46:
					iVar0 += 17;
					break;
				
				case 47:
					iVar0 += 17;
					break;
			}
			break;
		
		case 16:
			switch (iParam1)
			{
				case 48:
					iVar0 += 11;
					break;
				
				case 49:
					iVar0 += 11;
					break;
				
				case 50:
					iVar0 += 11;
					break;
			}
			break;
	}
	iVar0 = (iVar0 + func_439(iParam0));
	return iVar0;
}

int func_439(int iParam0)
{
	if (func_411(iParam0))
	{
		if (func_440(iParam0))
		{
			return 4;
		}
		return 2;
	}
	return 0;
}

int func_440(int iParam0)
{
	switch (iParam0)
	{
		case 7:
		case 2:
		case 9:
		case 4:
		case 17:
		case 15:
		case 16:
		case 8:
		case 10:
			return 1;
		
		default:
	}
	return 0;
}

int func_441(int iParam0)
{
	switch (iParam0)
	{
		case 0:
			return 0;
		
		case 1:
			return 0;
		
		case 2:
			return 0;
		
		case 3:
			return 5;
		
		case 4:
			return 12;
		
		case 5:
			return 4;
		
		case 6:
			return 4;
		
		case 7:
			return 2;
		
		case 8:
			return 12;
		
		case 9:
			return 12;
		
		case 10:
			return 9;
		
		case 11:
			return 8;
		
		case 12:
			return 7;
		
		case 13:
			return 5;
		
		case 14:
			return 12;
		
		case 15:
			return 13;
		
		case 16:
			return 5;
		
		case 17:
			return 15;
		
		case 19:
			return 4;
		
		case 18:
			return 11;
		
		default:
	}
	return 8;
}

void func_442(var uParam0, int iParam1)
{
	unk_0xBE20AB8238688965(&(uParam0->f_4357), iParam1);
}

void func_443()
{
	if (!func_444())
	{
		Global_1678289.f_3140 = 1;
	}
}

bool func_444()
{
	return Global_1678289.f_482;
}

void func_445(int iParam0)
{
	func_446(iParam0, -1, -1, -1);
}

void func_446(int iParam0, int iParam1, int iParam2, int iParam3)
{
	if (!func_459(unk_0x460153A63B9477BC()))
	{
		return;
	}
	if (iParam0 != -1)
	{
		func_458(iParam0);
	}
	func_457(iParam1, iParam2, iParam3);
	func_447(158, -1);
}

void func_447(int iParam0, int iParam1)
{
	if (func_478() || !func_456(1))
	{
		if (!func_454(unk_0x460153A63B9477BC(), 0))
		{
			func_453(iParam0);
			func_448(iParam0, func_452(1, 1), func_451(), func_450(), func_449(), iParam1);
		}
	}
}

void func_448(var uParam0, int iParam1, var uParam2, var uParam3, var uParam4, var uParam5)
{
	struct<7> Var0;
	
	Var0.f_0 = -1759105725;
	Var0.f_1 = unk_0x460153A63B9477BC();
	Var0.f_2 = uParam0;
	Var0.f_3 = uParam2;
	Var0.f_4 = uParam3;
	Var0.f_5 = uParam4;
	Var0.f_6 = uParam5;
	if (!iParam1 == 0)
	{
		unk_0xB802750B43D4047C(1, &Var0, 7, iParam1);
	}
}

var func_449()
{
	return Global_2540612.f_5188.f_342;
}

var func_450()
{
	return Global_2540612.f_5188.f_341;
}

var func_451()
{
	return Global_2540612.f_5188.f_340;
}

int func_452(int iParam0, bool bParam1)
{
	var uVar0;
	int iVar1;
	int iVar2;
	
	iVar1 = 0;
	while (iVar1 < 32)
	{
		iVar2 = unk_0xF1110FE23C67293A(iVar1);
		if (func_203(iVar2, 0, 0))
		{
			if (iVar2 != unk_0x460153A63B9477BC() || iParam0)
			{
				if (bParam1)
				{
					unk_0xBE20AB8238688965(&uVar0, iVar1);
				}
				else if (!func_85(iVar2, 0))
				{
					unk_0xBE20AB8238688965(&uVar0, iVar1);
				}
			}
		}
		iVar1++;
	}
	return uVar0;
}

void func_453(var uParam0)
{
	Global_1630317[unk_0x460153A63B9477BC() /*595*/].f_11.f_32 = uParam0;
}

int func_454(int iParam0, int iParam1)
{
	if (func_455(iParam0, 0))
	{
		return func_298(Global_1630317[iParam0 /*595*/].f_11.f_33) == iParam1;
	}
	return 0;
}

int func_455(int iParam0, int iParam1)
{
	if (Global_1630317[iParam0 /*595*/].f_11.f_33 != -1 || (iParam1 && Global_1630317[iParam0 /*595*/].f_11.f_32 != -1))
	{
		return 1;
	}
	return 0;
}

bool func_456(bool bParam0)
{
	return func_250(unk_0x460153A63B9477BC(), bParam0);
}

void func_457(var uParam0, var uParam1, var uParam2)
{
	Global_1630317[unk_0x460153A63B9477BC() /*595*/].f_11.f_302 = uParam0;
	Global_1630317[unk_0x460153A63B9477BC() /*595*/].f_11.f_302.f_1 = uParam1;
	Global_1630317[unk_0x460153A63B9477BC() /*595*/].f_11.f_302.f_2 = uParam2;
}

void func_458(int iParam0)
{
	if (Global_2540612.f_5188.f_340 != iParam0)
	{
		Global_2540612.f_5188.f_340 = iParam0;
	}
}

bool func_459(int iParam0)
{
	return Global_1590682[iParam0 /*883*/].f_274.f_393 != 0;
}

int func_460(bool bParam0)
{
	if (func_278(unk_0x460153A63B9477BC(), 158, 0) != -1)
	{
		return 0;
	}
	if (!bParam0)
	{
		if (unk_0x06EBE4F22EC30D45())
		{
			return 0;
		}
	}
	return 1;
}

bool func_461(bool bParam0)
{
	if (bParam0)
	{
		return unk_0xCE990E643CD9D0E5(func_9(8724, -1, 0), 4);
	}
	return func_265(unk_0x460153A63B9477BC());
}

void func_462(var uParam0)
{
	switch (uParam0->f_4358)
	{
		case 0:
			if (unk_0xCE990E643CD9D0E5(Global_1697243.f_23, 4))
			{
				if (!func_227("CH_ON_COOLDOWN"))
				{
					func_226("CH_ON_COOLDOWN");
				}
				return;
			}
			if (!func_475(14))
			{
				func_10("CH_SETUP_COST", -1);
				func_474(14);
			}
			if (!func_227("CH_SETUP_COST"))
			{
				if (!func_227("CH_SETUP_HEIST"))
				{
					func_226("CH_SETUP_HEIST");
				}
				if (unk_0x1BD7199394D7F19A(0, 51))
				{
					func_480();
					func_463(unk_0x460153A63B9477BC(), 0, 0, 0);
					unk_0x6A8F948698B360B4(unk_0x9B0761B4C3EB8BC7(), true);
					uParam0->f_4360 = 1;
					uParam0->f_4358 = 1;
				}
			}
			break;
	}
}

void func_463(int iParam0, bool bParam1, int iParam2, int iParam3)
{
	var uVar0;
	bool bVar1;
	bool bVar2;
	bool bVar3;
	bool bVar4;
	bool bVar5;
	bool bVar6;
	bool bVar7;
	bool bVar8;
	bool bVar9;
	bool bVar10;
	bool bVar11;
	bool bVar12;
	bool bVar13;
	bool bVar14;
	bool bVar15;
	bool bVar16;
	bool bVar17;
	bool bVar18;
	bool bVar19;
	bool bVar20;
	bool bVar21;
	bool bVar22;
	bool bVar23;
	bool bVar24;
	bool bVar25;
	bool bVar26;
	var uVar27;
	int iVar28;
	
	if (bParam1)
	{
		if (unk_0x4F315B95B1461ED4())
		{
			unk_0xFF8E7A064055ECA6(0);
		}
	}
	if (func_473())
	{
		if (bParam1)
		{
			return;
		}
		else
		{
			iParam3 = 1;
		}
	}
	if (!unk_0x25DDB354A40FFCDB())
	{
		uVar0 = iParam2;
		unk_0xDDEDC06C88DA4E04(iParam0, bParam1, uVar0);
	}
	else
	{
		bVar1 = true;
		bVar2 = iParam2 & 2 != false;
		bVar3 = iParam2 & 4 != false;
		bVar4 = iParam2 & 8 != false;
		bVar5 = iParam2 & 16 != false;
		bVar6 = iParam2 & 32 != false;
		bVar7 = iParam2 & 64 != false;
		bVar8 = iParam2 & 128 != false;
		bVar9 = iParam2 & 256 != false;
		bVar10 = iParam2 & 512 != false;
		bVar11 = iParam2 & 1024 != false;
		bVar12 = iParam2 & 2048 != false;
		bVar13 = iParam2 & 4096 != false;
		bVar14 = iParam2 & 8192 != false;
		bVar15 = iParam2 & 16384 != false;
		bVar16 = iParam2 & 32768 != false;
		bVar17 = iParam2 & 65536 != false;
		bVar18 = iParam2 & 131072 != false;
		bVar19 = iParam2 & 262144 != false;
		bVar20 = iParam2 & 524288 != false;
		bVar21 = iParam2 & 1048576 != false;
		bVar22 = iParam2 & 2097152 != false;
		bVar23 = iParam2 & 4194304 != false;
		bVar24 = iParam2 & 8388608 != false;
		bVar25 = iParam2 & 16777216 != false;
		if (iParam2 & 33554432 != 0 || unk_0xFC559366953F62B3())
		{
			bVar1 = false;
		}
		if (!func_471())
		{
			bVar26 = false;
			if (bParam1 == 1)
			{
				bVar26 = true;
			}
			if (bVar16 == 0 && !bVar21)
			{
				bVar26 = true;
			}
			if (bVar10 == 1)
			{
				bVar26 = true;
			}
			if (bVar26)
			{
				return;
			}
		}
		if (bVar18)
		{
		}
		if (unk_0x166E920FB00B2DBB(iParam0) && (unk_0x5A228A6A51E757C8(iParam0) || iParam3 == 1))
		{
			if (iParam3 && unk_0xA59F96B50B97E63C(unk_0x1FA7B77001D60F9D(iParam0), 0))
			{
				return;
			}
			uVar27 = unk_0x1FA7B77001D60F9D(iParam0);
			if (!bVar20)
			{
				if ((bVar19 && bParam1 == 0) && unk_0x25DDB354A40FFCDB())
				{
					unk_0x9675999D57D9F953(1);
				}
				else if (bVar14 || (!func_85(unk_0x460153A63B9477BC(), 0) && !func_263()))
				{
					unk_0x80250B8368A4E611(uVar27, !bVar14, 0);
				}
				if (!bVar14)
				{
					if (unk_0x25DDB354A40FFCDB() && !bVar19)
					{
						unk_0x9675999D57D9F953(0);
					}
					Global_2426097[iParam0 /*443*/].f_248 = 0;
				}
			}
			if (bParam1)
			{
				if (bVar1)
				{
					func_468(0, 0, 0);
					if (bVar25)
					{
						unk_0x701364DD9F501826();
					}
				}
				if (!func_467(uVar27) && !unk_0xEB1D872D96DDA8E2(uVar27))
				{
					if (!bVar22)
					{
						unk_0x58ADF88C51AAFDC5(uVar27, true, 0);
					}
				}
				if (!unk_0xB77B1396EAA80D8F(uVar27))
				{
					if (!bVar21)
					{
						unk_0x6A8F948698B360B4(uVar27, false);
					}
					unk_0x7C47E49129337557(uVar27, 1);
				}
				else if (!bVar21)
				{
					unk_0x6A8F948698B360B4(uVar27, false);
				}
				unk_0x0DC39BE87E4C5599(uVar27, true);
				unk_0xBF70D1B666A353F5(iParam0, 0);
				unk_0x330ACDACFF26728A(iParam0, 0);
				if (unk_0x36E2FC4B39D3018D(uVar27) && unk_0x51E339346E2220A4(uVar27))
				{
					unk_0x46468389F464E317(uVar27);
				}
				unk_0x4478D8ED4673EF14(uVar27, 1);
				if (unk_0x4DC4117F94CD5B84(unk_0x460153A63B9477BC()) == 0)
				{
					func_466();
					func_465();
				}
				if (unk_0x5C6122477A723551())
				{
					if (!bVar23)
					{
					}
				}
				if (unk_0xE9159DB182CF0841())
				{
				}
				Global_2426097[iParam0 /*443*/].f_249 = 0;
				if (!bVar24)
				{
					bVar3 = true;
				}
				if (Global_2405074.f_2674)
				{
					Global_2405074.f_2674 = 0;
				}
			}
			else
			{
				if (!func_467(uVar27) && !unk_0xEB1D872D96DDA8E2(uVar27))
				{
					if (!bVar22)
					{
						unk_0x58ADF88C51AAFDC5(uVar27, !bVar15, 0);
					}
					if (!unk_0xB77B1396EAA80D8F(uVar27))
					{
						if (!bVar21)
						{
							unk_0x6A8F948698B360B4(uVar27, bVar16);
						}
						if (!bVar16)
						{
							unk_0x7C47E49129337557(uVar27, 1);
						}
					}
					if (func_464(Global_4456448.f_129348))
					{
						unk_0x6A8F948698B360B4(uVar27, true);
					}
				}
				if (Global_1312872)
				{
					bVar10 = false;
				}
				if (bVar10)
				{
					unk_0xBF70D1B666A353F5(iParam0, 0);
				}
				else
				{
					unk_0xBF70D1B666A353F5(iParam0, 1);
				}
				unk_0x0DC39BE87E4C5599(uVar27, bVar17);
				if (bVar3)
				{
					if (!unk_0x5A898C4DF17FB664(uVar27) && !unk_0x47DBF174A0CB9D55(uVar27, 0))
					{
						unk_0x2BB0EF9DE445EA13(uVar27);
					}
				}
			}
			iVar28 = 0;
			if (bVar2)
			{
				iVar28 |= 2;
			}
			if (bVar3)
			{
				iVar28 |= 4;
			}
			if (bVar4)
			{
				iVar28 |= 8;
			}
			if (bVar5)
			{
				iVar28 |= 16;
			}
			if (bVar6)
			{
				iVar28 |= 32;
			}
			if (bVar7)
			{
				iVar28 |= 64;
			}
			if (bVar8)
			{
				iVar28 |= 128;
			}
			if (bVar9)
			{
				iVar28 |= 256;
			}
			if (bVar10)
			{
				iVar28 |= 512;
			}
			if (bVar11)
			{
				iVar28 |= 1024;
			}
			if (bVar12)
			{
				iVar28 |= 2048;
			}
			if (bVar13)
			{
				iVar28 |= 4096;
			}
			unk_0xDDEDC06C88DA4E04(iParam0, bParam1, iVar28);
		}
	}
}

bool func_464(int iParam0)
{
	return iParam0 == 17;
}

void func_465()
{
	struct<3> Var0;
	
	Global_2440277.f_1344 = 0;
	Global_2440277.f_1345 = 0;
	Global_2440277.f_1346 = { 9999.9f, 9999.9f, 9999.9f };
	Global_2440277.f_1351 = -1;
	Global_2440277.f_1352 = 0;
	Global_2405074.f_2685 = { Var0 };
}

void func_466()
{
	Global_2405074.f_696 = 0;
	Global_2405074.f_2728 = 0;
	Global_2405074.f_512 = 0;
	Global_2405074.f_601 = 0;
	Global_2426097[unk_0x460153A63B9477BC() /*443*/].f_211 = 0;
	Global_2405074.f_2683 = 0;
}

int func_467(var uParam0)
{
	int iVar0;
	
	if (unk_0x47DBF174A0CB9D55(uParam0, 1))
	{
		return 1;
	}
	else
	{
		iVar0 = unk_0x16BC17A8EDC701A2(uParam0, -1794415470);
		if (iVar0 == 0)
		{
			return 1;
		}
	}
	return 0;
}

void func_468(int iParam0, int iParam1, int iParam2)
{
	int iVar0;
	int iVar1;
	
	if (unk_0xA59F96B50B97E63C(iParam1, 0))
	{
	}
	if (iParam0 == 1)
	{
		if (unk_0xAE06B9E39EBDE049(iParam1))
		{
			if (unk_0xD0390A93AF3907B8(iParam1))
			{
				if (!unk_0x56B60AE958412839(unk_0x460153A63B9477BC(), unk_0xE4400E48E081F17A(iParam1)))
				{
					iVar0 = 1;
				}
			}
		}
	}
	if (iVar0 == 0)
	{
		if (iParam2 == 1)
		{
			if (iParam0 == 1)
			{
				func_470();
			}
			else if (!unk_0xCE990E643CD9D0E5(Global_2359302.f_67, 1))
			{
				if (unk_0xCE990E643CD9D0E5(Global_2359302.f_67, 2))
				{
					iVar1 = 0;
					while (iVar1 < 8)
					{
						unk_0x80610B7EA03DE232(iVar1, Global_2359302.f_58[iVar1]);
						iVar1++;
					}
				}
				unk_0xBE20AB8238688965(&(Global_2359302.f_67), 1);
			}
		}
		if (func_85(unk_0x460153A63B9477BC(), 0))
		{
			unk_0x80B3C69DE260B92D(iParam0, iParam1, 1);
		}
		else
		{
			unk_0xE86692F29AFB9043(iParam0, iParam1);
		}
		unk_0x0F9CBC63296852FD(iParam0, iParam1);
		func_469(joaat("mpply_is_char_spectating"), iParam0);
	}
}

void func_469(int iParam0, int iParam1)
{
	int iVar0;
	
	iVar0 = iParam0;
	if (iVar0 != 0)
	{
		unk_0xBD9D9E72ABB288E4(iVar0, iParam1, 1);
	}
}

void func_470()
{
	int iVar0;
	
	if (!unk_0x509C03DADAEE0A94())
	{
		if (!unk_0xCE990E643CD9D0E5(Global_2359302.f_67, 2))
		{
			iVar0 = 0;
			while (iVar0 < 8)
			{
				Global_2359302.f_58[iVar0] = unk_0xF7B3A1430308F92B(iVar0);
				iVar0++;
			}
			unk_0xBE20AB8238688965(&(Global_2359302.f_67), 2);
			unk_0xBE20AB8238688965(&(Global_2359302.f_67), 0);
		}
	}
}

int func_471()
{
	if (func_472() == 0)
	{
		return 1;
	}
	return 0;
}

int func_472()
{
	return Global_1312485.f_18;
}

int func_473()
{
	if (Global_1590682[unk_0x460153A63B9477BC() /*883*/].f_35 && !Global_2452907.f_2846.f_215 == -1)
	{
		return 1;
	}
	return 0;
}

void func_474(int iParam0)
{
	int iVar0;
	
	iVar0 = func_9(8724, -1, 0);
	if (!unk_0xCE990E643CD9D0E5(iVar0, iParam0))
	{
		unk_0xBE20AB8238688965(&iVar0, iParam0);
		Global_1697243.f_24 = iVar0;
		func_6(8724, iVar0, -1, 1, 0);
	}
}

bool func_475(int iParam0)
{
	return unk_0xCE990E643CD9D0E5(func_9(8724, -1, 0), iParam0);
}

bool func_476()
{
	return unk_0xCE990E643CD9D0E5(func_9(8724, -1, 0), 0);
}

void func_477()
{
	struct<2> Var0;
	
	switch (func_223())
	{
		case 0:
			StringCopy(&Var0, "CH_SETUP_REG", 16);
			break;
		
		case 1:
			StringCopy(&Var0, "CH_PREP_REG", 16);
			break;
		
		case 2:
			StringCopy(&Var0, "CH_FINALE_REG", 16);
			break;
	}
	if (!func_227(&Var0))
	{
		func_226(&Var0);
	}
}

bool func_478()
{
	return func_269(unk_0x460153A63B9477BC());
}

bool func_479()
{
	return func_270() == unk_0x460153A63B9477BC();
}

void func_480()
{
	if ((((((((func_227("CH_SETUP_USE") || func_227("CH_PREP_USE")) || func_227("CH_FINALE_USE")) || func_227("CH_SETUP_HEIST")) || func_227("CH_SETUP_REG")) || func_227("CH_PREP_REG")) || func_227("CH_FINALE_REG")) || func_227("CH_SETUP_SCOPE")) || func_227("CH_ON_COOLDOWN"))
	{
		unk_0xD289B55B6AADBA10(1);
	}
	if (func_270() != -1)
	{
		if ((func_264("CH_OWNER_SCOPE", unk_0x72C1D3A1AB8CAA5B(func_270())) || func_264("CH_OWNER_SETUP", unk_0x72C1D3A1AB8CAA5B(func_270()))) || func_264("CH_SETUP_REG2", unk_0x72C1D3A1AB8CAA5B(func_270())))
		{
			unk_0xD289B55B6AADBA10(1);
		}
	}
}

int func_481(var uParam0)
{
	if (func_17(uParam0, 2))
	{
		return 0;
	}
	if (func_17(uParam0, 3) || func_17(uParam0, 4))
	{
		return 0;
	}
	if (func_506() || func_505())
	{
		return 0;
	}
	if (unk_0xFC559366953F62B3())
	{
		return 0;
	}
	if (unk_0x3EBB66F936A3436E())
	{
		return 0;
	}
	if (!func_41(unk_0x460153A63B9477BC(), 2))
	{
		return 0;
	}
	if (func_501())
	{
		return 0;
	}
	if (func_500())
	{
		return 0;
	}
	if (func_455(unk_0x460153A63B9477BC(), 0) && func_40(unk_0x460153A63B9477BC()))
	{
		return 0;
	}
	if (!func_499(uParam0))
	{
		return 0;
	}
	if (func_15())
	{
		return 0;
	}
	if (func_498())
	{
		return 0;
	}
	if (func_497())
	{
		return 0;
	}
	if (!func_496())
	{
		return 0;
	}
	if (!func_495(func_223()))
	{
		return 0;
	}
	if (!func_483(uParam0, func_223()))
	{
		return 0;
	}
	if (func_482() == 2 || func_482() == 16)
	{
		return 0;
	}
	if (func_73(0))
	{
		return 0;
	}
	return 1;
}

int func_482()
{
	return Global_968397;
}

int func_483(var uParam0, int iParam1)
{
	switch (iParam1)
	{
		case 0:
			break;
		
		case 1:
			if (!func_267(uParam0->f_4356))
			{
				return 0;
			}
			if (func_113(uParam0->f_4356) == 0)
			{
				return 0;
			}
			break;
		
		case 2:
			if (!func_267(uParam0->f_4356))
			{
				return 0;
			}
			if (func_113(uParam0->f_4356) == 0)
			{
				return 0;
			}
			if (!func_484(uParam0->f_4356))
			{
				return 0;
			}
			break;
	}
	return 1;
}

int func_484(int iParam0)
{
	if (iParam0 == func_42())
	{
		return 0;
	}
	if (!func_494(iParam0))
	{
		return 0;
	}
	if (func_164(iParam0, 0) == 0)
	{
		return 0;
	}
	if (!func_493(iParam0))
	{
		return 0;
	}
	if (func_153(iParam0, 0) == 0)
	{
		return 0;
	}
	if (!func_492(iParam0))
	{
		return 0;
	}
	if (func_491(iParam0, 0) == 0)
	{
		return 0;
	}
	if (!func_490(iParam0))
	{
		return 0;
	}
	switch (func_113(iParam0))
	{
		case 1:
			if (!func_489(iParam0))
			{
				return 0;
			}
			if (!func_488(iParam0))
			{
				return 0;
			}
			break;
		
		case 2:
			if (!func_172(iParam0))
			{
				return 0;
			}
			if (!func_487(iParam0))
			{
				return 0;
			}
			break;
		
		case 3:
			if (!func_486(iParam0))
			{
				return 0;
			}
			if (!func_485(iParam0))
			{
				return 0;
			}
			break;
		
		default:
			return 0;
			break;
	}
	return 1;
}

int func_485(int iParam0)
{
	if (iParam0 != -1)
	{
		return unk_0xCE990E643CD9D0E5(Global_1701669[iParam0 /*68*/].f_18.f_1, 9);
	}
	return 0;
}

int func_486(int iParam0)
{
	if (iParam0 != -1)
	{
		return unk_0xCE990E643CD9D0E5(Global_1701669[iParam0 /*68*/].f_18.f_1, 8);
	}
	return 0;
}

int func_487(int iParam0)
{
	if (iParam0 != -1)
	{
		return unk_0xCE990E643CD9D0E5(Global_1701669[iParam0 /*68*/].f_18.f_1, 7);
	}
	return 0;
}

int func_488(int iParam0)
{
	if (iParam0 != -1)
	{
		return unk_0xCE990E643CD9D0E5(Global_1701669[iParam0 /*68*/].f_18.f_1, 6);
	}
	return 0;
}

int func_489(int iParam0)
{
	if (iParam0 != -1)
	{
		return unk_0xCE990E643CD9D0E5(Global_1701669[iParam0 /*68*/].f_18.f_1, 5);
	}
	return 0;
}

int func_490(int iParam0)
{
	if (iParam0 != -1)
	{
		return unk_0xCE990E643CD9D0E5(Global_1701669[iParam0 /*68*/].f_18.f_1, 4);
	}
	return 0;
}

int func_491(int iParam0, bool bParam1)
{
	if (iParam0 != -1)
	{
		if (Global_1701669[iParam0 /*68*/].f_18.f_14 != 0 || bParam1)
		{
			return Global_1701669[iParam0 /*68*/].f_18.f_14;
		}
	}
	if (bParam1)
	{
		return Global_1697272.f_17;
	}
	return 0;
}

int func_492(int iParam0)
{
	if (iParam0 != -1)
	{
		return unk_0xCE990E643CD9D0E5(Global_1701669[iParam0 /*68*/].f_18.f_1, 3);
	}
	return 0;
}

int func_493(int iParam0)
{
	if (iParam0 != -1)
	{
		return unk_0xCE990E643CD9D0E5(Global_1701669[iParam0 /*68*/].f_18.f_1, 2);
	}
	return 0;
}

int func_494(int iParam0)
{
	if (iParam0 != -1)
	{
		return unk_0xCE990E643CD9D0E5(Global_1701669[iParam0 /*68*/].f_18.f_1, 1);
	}
	return 0;
}

int func_495(int iParam0)
{
	float fVar0;
	
	if (!func_203(unk_0x460153A63B9477BC(), 1, 1))
	{
		return 0;
	}
	fVar0 = unk_0xEF7858F25585F853(unk_0x9B0761B4C3EB8BC7());
	switch (iParam0)
	{
		case 0:
			if (fVar0 < 45f || fVar0 > 315f)
			{
				return 1;
			}
			break;
		
		case 1:
			if (fVar0 < 315f && fVar0 > 225f)
			{
				return 1;
			}
			break;
		
		case 2:
			if (fVar0 < 225f && fVar0 > 135f)
			{
				return 1;
			}
			break;
	}
	return 0;
}

bool func_496()
{
	return ((func_224(0) || func_224(1)) || func_224(2));
}

bool func_497()
{
	return Global_2452015.f_690;
}

bool func_498()
{
	return Global_2452015.f_689;
}

int func_499(var uParam0)
{
	if ((uParam0->f_724 == 3 && uParam0->f_744.f_724 == 3) && uParam0->f_1497.f_724 == 3)
	{
		return 1;
	}
	return 0;
}

bool func_500()
{
	return Global_74030;
}

int func_501()
{
	bool bVar0;
	
	bVar0 = ((unk_0x22D6FB6153F774D3(2, 199) || unk_0x1BD7199394D7F19A(2, 199)) || unk_0xC27AEE8B64EE0F8D(2, 199));
	if (bVar0)
	{
		iLocal_256 = 1;
		return 1;
	}
	else if (iLocal_256)
	{
		if (func_503(&uLocal_257, 1000, 0))
		{
			func_502(&uLocal_257);
			iLocal_256 = 0;
			return 0;
		}
		return 1;
	}
	return 0;
}

void func_502(var uParam0)
{
	uParam0->f_1 = 0;
}

int func_503(var uParam0, int iParam1, bool bParam2)
{
	if (iParam1 == -1)
	{
		return 1;
	}
	func_504(uParam0, bParam2, 0);
	if (unk_0x25DDB354A40FFCDB() && !bParam2)
	{
		if (unk_0x44F29CF6A0675F4F(unk_0x624736CA66E14161(unk_0x6CAAB7E78B5D978A(), *uParam0)) >= iParam1)
		{
			return 1;
		}
	}
	else if (unk_0x44F29CF6A0675F4F(unk_0x624736CA66E14161(unk_0x578C4EF320340AF7(), *uParam0)) >= iParam1)
	{
		return 1;
	}
	return 0;
}

void func_504(var uParam0, bool bParam1, bool bParam2)
{
	if (uParam0->f_1 == 0)
	{
		if (unk_0x25DDB354A40FFCDB() && !bParam1)
		{
			if (!bParam2)
			{
				*uParam0 = unk_0x6CAAB7E78B5D978A();
			}
			else
			{
				*uParam0 = unk_0x29B9AF1CB5B8175D();
			}
		}
		else
		{
			*uParam0 = unk_0x578C4EF320340AF7();
		}
		uParam0->f_1 = 1;
	}
}

var func_505()
{
	return Global_1689767;
}

var func_506()
{
	return Global_1689768;
}

int func_507(var uParam0)
{
	if ((uParam0->f_724.f_5 || uParam0->f_744.f_724.f_5) || uParam0->f_1497.f_724.f_5)
	{
		return 1;
	}
	return 0;
}

void func_508(var uParam0)
{
	bool bVar0;
	int iVar1;
	int iVar2;
	int iVar3;
	var uVar4;
	int iVar5;
	
	switch (uParam0->f_4358)
	{
		case 1:
			if (func_529(uParam0, 6, &bVar0))
			{
				if (bVar0)
				{
					uParam0->f_4358 = 2;
					if (uParam0->f_4360)
					{
						func_463(unk_0x460153A63B9477BC(), 1, 0, 0);
						unk_0x6A8F948698B360B4(unk_0x9B0761B4C3EB8BC7(), false);
						uParam0->f_4360 = 0;
					}
				}
				else
				{
					uParam0->f_4358 = 0;
					if (uParam0->f_4360)
					{
						func_463(unk_0x460153A63B9477BC(), 1, 0, 0);
						unk_0x6A8F948698B360B4(unk_0x9B0761B4C3EB8BC7(), false);
						uParam0->f_4360 = 0;
					}
				}
			}
			break;
		
		case 2:
			if (unk_0xF73A2DAF57FFFE87(&iVar1, &iVar2))
			{
				func_6(8911, iVar1, -1, 1, 0);
				func_6(8912, iVar2, -1, 1, 0);
				iVar3 = func_528(uParam0);
				if (func_527())
				{
					func_515(914079366, iVar3, &uVar4, 0, 1, 0);
				}
				else
				{
					unk_0x171DB82E3FFD657F(iVar3, 0, 1, 1, iVar3, 0, 0, 0, 0, 0, 0);
				}
				uParam0->f_4358 = 3;
			}
			break;
		
		case 3:
			if (func_514(&iVar5))
			{
				func_513(iVar5);
				uParam0->f_4358 = 4;
			}
			break;
		
		case 4:
			func_509();
			func_222(uParam0, func_223());
			func_201(uParam0, 1);
			uParam0->f_4358 = 5;
			break;
		
		case 5:
			break;
	}
}

void func_509()
{
	int iVar0;
	
	iVar0 = func_9(8724, -1, 0);
	if (!unk_0xCE990E643CD9D0E5(iVar0, 0))
	{
		unk_0xBE20AB8238688965(&iVar0, 0);
		Global_1697243.f_24 = iVar0;
		func_6(8724, iVar0, -1, 1, 0);
		func_510(-1713398555, 18, 0);
	}
}

void func_510(int iParam0, int iParam1, int iParam2)
{
	int iVar0;
	
	if (func_512(iParam1, iParam2))
	{
		iVar0 = func_511();
		Global_2462355[iVar0] = iParam1;
		Global_2462366[iVar0] = iParam0;
	}
}

int func_511()
{
	int iVar0;
	int iVar1;
	
	iVar0 = 9;
	iVar1 = 0;
	while (iVar1 <= 9)
	{
		if (Global_2462355[iVar1] == 0)
		{
			iVar0 = iVar1;
			iVar1 = 10;
		}
		iVar1++;
	}
	return iVar0;
}

int func_512(int iParam0, var uParam1)
{
	if (Global_1312884)
	{
		return 0;
	}
	if (iParam0 == 22)
	{
		return 1;
	}
	if ((((((((uParam1 || !Global_1312896) || iParam0 == 3) || iParam0 == 10) || iParam0 == 11) || iParam0 == 27) || iParam0 == 28) || iParam0 == 29) || iParam0 == 30)
	{
		return 1;
	}
	else
	{
		return 0;
	}
	return 1;
}

void func_513(int iParam0)
{
	func_6(8531, iParam0, -1, 1, 0);
	Global_1701669[unk_0x460153A63B9477BC() /*68*/].f_18.f_7 = iParam0;
}

int func_514(var uParam0)
{
	var uVar0[4];
	float fVar5;
	float fVar6;
	float fVar7;
	int iVar8;
	
	if ((Global_262145.f_27694 != -1 && !unk_0xCE990E643CD9D0E5(func_9(8724, -1, 0), 17)) && func_193(unk_0x460153A63B9477BC(), 0))
	{
		*uParam0 = Global_262145.f_27694;
		return 1;
	}
	uVar0[0] = Global_262145.f_28092;
	uVar0[1] = Global_262145.f_28094;
	uVar0[2] = Global_262145.f_28093;
	uVar0[3] = Global_262145.f_28095;
	iVar8 = 0;
	while (iVar8 <= 3)
	{
		fVar5 = (fVar5 + uVar0[iVar8]);
		iVar8++;
	}
	fVar7 = unk_0x658086F2B3C5E4E4(0f, fVar5);
	iVar8 = 0;
	while (iVar8 <= 3)
	{
		fVar6 = (fVar6 + uVar0[iVar8]);
		if (fVar7 < fVar6)
		{
			*uParam0 = iVar8;
			return 1;
		}
		iVar8++;
	}
	return 0;
}

void func_515(int iParam0, int iParam1, var uParam2, bool bParam3, bool bParam4, bool bParam5)
{
	int iVar0;
	
	if (!func_527())
	{
		return;
	}
	iVar0 = 1;
	if (bParam4)
	{
		iVar0 = 4;
	}
	else if (bParam3)
	{
		iVar0 = 2;
	}
	else if (bParam5)
	{
		iVar0 = 8;
	}
	switch (iParam0)
	{
		case -1645229221:
		case -585718395:
		case -1359375127:
		case 454359403:
		case -982394051:
		case 1643659499:
		case -2072289654:
		case 650665123:
		case 1654961868:
		case -876847842:
		case 68030260:
		case -2122299283:
		case 366672791:
		case 283351220:
		case 291576838:
		case 1182673174:
		case -516219046:
		case 1036455748:
		case 277665518:
		case 2043854386:
		case 1839532116:
		case 1022400740:
		case 1940862352:
		case -1389227906:
		case 711665950:
		case 1704445500:
		case 1515774909:
		case 1173654533:
		case -899802304:
		case -663944335:
		case 1208553146:
		case -613221010:
		case -671062876:
		case -407201236:
		case -754024203:
		case -1885444887:
		case 1931729587:
		case 1064954035:
		case -180141073:
		case 2131324797:
		case 1612072658:
		case -517447402:
		case 1948102096:
		case 1108628223:
		case -1834046564:
		case -1239008812:
		case -222363842:
		case -1276678868:
		case 1564537328:
		case -96593501:
		case 742499889:
		case 2050093329:
		case -1752488069:
		case 634375935:
		case 1850983186:
		case 763367758:
		case 1941570214:
		case 665109499:
		case -1330755006:
		case 1976384368:
		case 268924934:
		case 1869490922:
		case -336803850:
		case -1412692765:
		case 618167454:
		case 980623936:
		case 437291904:
		case -135813048:
		case -930104477:
		case -1420909320:
		case -1733398043:
		case -1892760262:
		case -1545737048:
		case 2039302543:
		case 402505853:
		case -1143510182:
		case 1678112166:
		case 837955913:
		case -1532467144:
		case 1815541181:
			if (iParam1 > 0 || Global_262145.f_27658)
			{
				func_516(uParam2, -1135378931, 537254313, 1474183246, iParam0, iParam1, iVar0, 7);
			}
			break;
		
		case -31156877:
		case -1027218631:
		case -1398318418:
		case 1652884147:
		case -57868256:
		case -1216489292:
		case -46622315:
		case -352356931:
		case -990286235:
		case 563463121:
		case 1734805203:
		case 941287179:
		case -1186079845:
		case -1985150258:
		case -1127021384:
		case -109201286:
		case 312105838:
		case 1982688246:
		case -661030418:
		case 1301046174:
		case -1586170317:
		case 393059668:
		case 23796958:
		case -1077156170:
		case 1780666425:
		case -2043695058:
		case -1922554349:
		case 1287308202:
		case 691372038:
		case 1480707108:
		case 1512499951:
		case 562283735:
		case -154732333:
		case -1362660491:
		case 645708827:
		case 767907967:
		case -1970151306:
		case 718859568:
		case -1955564771:
		case 892388724:
		case -1426920838:
		case 1349151477:
		case 1620609399:
		case 1961641934:
		case 210955503:
		case -59668082:
		case 1736933716:
		case -1468524125:
		case 111573502:
		case 1525644423:
		case 968073639:
		case 1577781788:
		case -934465332:
		case -1194253122:
		case -212607085:
		case -815546555:
		case 1048226110:
		case 569170531:
		case -856006867:
		case 848090538:
		case -47546905:
		case -293060240:
		case 463142405:
		case 1550217370:
		case -101307780:
		case -664597565:
		case 599804707:
		case -327918414:
		case 550898518:
		case 835976347:
		case 1347433368:
		case 1052472386:
		case -2130199671:
		case -1227654538:
		case 1864522104:
		case 215608230:
		case -876012764:
		case -722894325:
		case 1407278493:
		case -1579394494:
		case -27443911:
		case 1179783540:
		case 923419301:
		case -308826175:
		case 603298940:
		case -12619854:
		case -311112675:
		case 870439158:
		case -974288740:
		case -4138654:
		case -1180954122:
		case -1918051016:
		case 844330594:
		case 1934825517:
		case 1852024236:
		case 2099238988:
		case 1952643559:
		case -1172900789:
		case -2015399333:
		case -1574795641:
		case -961034881:
		case 1135468152:
		case 1265272476:
		case -634726636:
		case 696556762:
		case 403506509:
		case -883876414:
		case -1064150715:
		case -1387253055:
		case -716963152:
		case 1138089938:
		case -561012053:
			func_516(uParam2, -1135378931, 1445302971, 1474183246, iParam0, iParam1, iVar0, 7);
			break;
	}
	switch (iParam0)
	{
		case -1342064661:
		case -1693570755:
		case 1868043300:
		case -697248883:
		case -1295545795:
		case 914079366:
		case 395122350:
		case -331981076:
		case 1671535231:
		case -1896606724:
		case -1291433573:
		case 538631715:
		case 133782822:
		case 2081885153:
		case -1314365345:
		case 1322977732:
		case 396623013:
		case 1981664462:
		case -1921250821:
		case -1370731547:
		case 1683798242:
		case -910968288:
			if (iParam1 > 0 || Global_262145.f_27658)
			{
				func_516(uParam2, -1135378931, 537254313, 1474183246, iParam0, iParam1, iVar0, 7);
			}
			break;
		
		case 1240683675:
		case 1241904665:
		case -1100963799:
		case -494565059:
		case 827308208:
		case -1857685192:
		case 1698417709:
		case 1057653594:
		case 1810506918:
		case 451427308:
		case 824622151:
		case 1253978276:
		case -1576080766:
		case 1508411869:
		case 1428501742:
		case -1918967151:
		case 1261538664:
		case 1180397655:
		case 1414674366:
		case 261460130:
		case -2027658376:
		case -2017925037:
		case 1668610896:
		case -2032529561:
		case -1224479447:
		case -319306689:
		case -466527264:
		case 1925965142:
		case 592152676:
		case 2035612943:
		case 1568659720:
		case 1220095570:
		case 2050320631:
		case 592672421:
		case 1775876058:
		case -842062976:
		case -518651910:
		case 14658715:
		case -604793592:
		case -823426392:
		case -1401862980:
		case -173354274:
		case 409533976:
		case -1472522337:
		case 542574408:
		case -1261799063:
		case 784631574:
		case -2027479156:
		case -837690641:
			func_516(uParam2, -1135378931, 1445302971, 1474183246, iParam0, iParam1, iVar0, 7);
			break;
	}
}

int func_516(var uParam0, int iParam1, int iParam2, int iParam3, int iParam4, int iParam5, int iParam6, int iParam7)
{
	bool bVar0;
	int iVar1;
	int iVar2;
	bool bVar3;
	int iVar4;
	bool bVar5;
	
	bVar0 = false;
	if (!func_527())
	{
		bVar0 = true;
	}
	iVar1 = 1;
	if (!bVar0)
	{
		if (!unk_0x5053392075AA583C(func_8()) || unk_0x701B05C16EF4F48E())
		{
			Global_4265553 = 1;
			return 0;
		}
		if (Global_2463638)
		{
			if (iParam3 == 1067618600 || iParam3 == -1303831698)
			{
				Global_4265554 = 1;
				return 0;
			}
		}
	}
	bVar3 = false;
	iVar2 = 0;
	while (iVar2 < 15)
	{
		if (Global_4264051[iVar2 /*85*/].f_66.f_2 == 0)
		{
			bVar3 = true;
		}
		iVar2++;
	}
	if (!bVar3)
	{
		return 0;
	}
	*uParam0 = 15;
	iVar4 = 2147483647;
	if ((bVar0 || iVar1) || unk_0x734A68E809CC23F0(&iVar4, iParam3, iParam4, iParam2, iParam5, iParam6))
	{
		if ((bVar0 || iVar1) || unk_0x9F5556ED9A739FBC(iVar4))
		{
			*uParam0 = func_523(iVar4, iParam1, iParam4, iParam2, iParam3, iParam5, 0, iParam6, iParam7, 1, 1);
			if (iVar1 && !bVar0)
			{
			}
			if (bVar0)
			{
				if (*uParam0 != -1)
				{
					Global_4264051[*uParam0 /*85*/].f_66.f_8 = 1;
					Global_4264051[*uParam0 /*85*/].f_66.f_12 = 1;
				}
			}
			Global_4265538 = 1;
			return 1;
		}
	}
	else
	{
		if (iParam7 & 2 != 0)
		{
			Global_4265552 = 1;
			Global_4265555 = iParam4;
			Global_4265557 = iParam3;
			Global_4265558 = 1;
			Global_4265556 = iParam5;
		}
		if (iParam7 & 8 != 0)
		{
			Global_4265555 = iParam4;
			Global_4265557 = iParam3;
			Global_4265558 = 1;
			Global_4265556 = iParam5;
		}
		bVar5 = false;
		if (bVar5)
		{
			func_522(1, iParam4);
			Global_4265552 = 0;
		}
		if (iParam7 & 4 != 0)
		{
			func_517(-1, iParam4, iParam6, iParam5, -1);
		}
	}
	return 0;
}

void func_517(int iParam0, int iParam1, var uParam2, var uParam3, int iParam4)
{
	switch (iParam1)
	{
		case 1704445500:
			unk_0xBE20AB8238688965(&(Global_2426097[unk_0x460153A63B9477BC() /*443*/].f_126.f_71), 0);
			break;
	}
	if (iParam0 != -1)
	{
		func_518(iParam0);
	}
}

void func_518(int iParam0)
{
	bool bVar0;
	
	bVar0 = false;
	if (!func_527())
	{
		bVar0 = true;
	}
	if (iParam0 != -1)
	{
		if (func_521(iParam0))
		{
			if (!bVar0)
			{
				unk_0x2ABDF07188759AF4();
			}
		}
		else if (!bVar0)
		{
			unk_0x7365588A145923CE(Global_4264051[iParam0 /*85*/].f_66);
		}
		func_519(&(Global_4264051[iParam0 /*85*/]));
	}
}

void func_519(var uParam0)
{
	uParam0->f_66 = 0;
	uParam0->f_66 = 2147483647;
	uParam0->f_66.f_1 = 0;
	uParam0->f_66.f_2 = 0;
	uParam0->f_66.f_3 = -1593119440;
	uParam0->f_66.f_4 = -2085313189;
	uParam0->f_66.f_5 = 0;
	uParam0->f_66.f_6 = 1227573907;
	uParam0->f_66.f_7 = -1161833819;
	*uParam0 = 0;
	uParam0->f_1 = 0;
	uParam0->f_13 = 0;
	uParam0->f_2 = 0;
	func_520(&(uParam0->f_14));
	func_520(&(uParam0->f_14.f_13));
	StringCopy(&(uParam0->f_14.f_26), "", 32);
	StringCopy(&(uParam0->f_14.f_34), "", 24);
	StringCopy(&(uParam0->f_14.f_40), "", 16);
	StringCopy(&(uParam0->f_14.f_44), "", 32);
	uParam0->f_3 = 0;
	uParam0->f_4 = 0;
	uParam0->f_5 = 0;
	uParam0->f_6 = 0;
	uParam0->f_7 = 0;
	uParam0->f_8 = 0;
	uParam0->f_66.f_8 = 0;
	uParam0->f_66.f_9 = 0;
	uParam0->f_66.f_10 = 0;
	uParam0->f_66.f_11 = 0;
	uParam0->f_66.f_13 = 0;
	uParam0->f_66.f_12 = 0;
	uParam0->f_66.f_14 = 0;
	uParam0->f_66.f_15 = 0;
	uParam0->f_66.f_16 = 0;
	uParam0->f_66.f_18 = 0;
}

void func_520(var uParam0)
{
	*uParam0 = 0;
	uParam0->f_1 = 0;
	uParam0->f_2 = 0;
	uParam0->f_3 = 0;
	uParam0->f_4 = 0;
	uParam0->f_5 = 0;
	uParam0->f_6 = 0;
	uParam0->f_7 = 0;
	uParam0->f_8 = 0;
	uParam0->f_9 = 0;
	uParam0->f_10 = 0;
	uParam0->f_11 = 0;
	uParam0->f_12 = 0;
}

int func_521(int iParam0)
{
	if (iParam0 >= 0 && iParam0 < 15)
	{
		return Global_4264051[iParam0 /*85*/].f_66.f_5 == 1;
	}
	return 0;
}

void func_522(int iParam0, var uParam1)
{
	Global_2464818 = uParam1;
	Global_2464817 = iParam0;
}

int func_523(int iParam0, int iParam1, var uParam2, var uParam3, int iParam4, var uParam5, bool bParam6, var uParam7, var uParam8, int iParam9, int iParam10)
{
	int iVar0;
	
	iVar0 = 0;
	iVar0 = 0;
	while (iVar0 < 15)
	{
		if (Global_4264051[iVar0 /*85*/].f_66.f_2 == 0)
		{
			if (!func_527())
			{
				iParam0 = iVar0 + 900;
			}
			Global_4264051[iVar0 /*85*/].f_66.f_2 = 1;
			Global_4264051[iVar0 /*85*/].f_66.f_1 = uParam5;
			Global_4264051[iVar0 /*85*/].f_66.f_3 = iParam1;
			Global_4264051[iVar0 /*85*/].f_66.f_4 = uParam2;
			Global_4264051[iVar0 /*85*/].f_66.f_7 = uParam3;
			Global_4264051[iVar0 /*85*/].f_66.f_5 = 0;
			Global_4264051[iVar0 /*85*/].f_66 = iParam0;
			Global_4264051[iVar0 /*85*/].f_66.f_6 = iParam4;
			Global_4264051[iVar0 /*85*/].f_66.f_11 = uParam8;
			Global_4264051[iVar0 /*85*/].f_66.f_10 = uParam7;
			Global_4264051[iVar0 /*85*/].f_66.f_13 = iParam9;
			Global_4264051[iVar0 /*85*/].f_66.f_12 = 0;
			Global_4264051[iVar0 /*85*/].f_66.f_14 = unk_0xD3137A576BE9EC5C();
			Global_4264051[iVar0 /*85*/].f_66.f_18 = 0;
			Global_4265538 = 0;
			if (bParam6)
			{
				Global_4264051[iVar0 /*85*/].f_66.f_5 = 1;
			}
			if (iParam1 == -1135378931 && iParam10)
			{
				func_524(Global_4264051[iVar0 /*85*/], iVar0);
			}
			return iVar0;
		}
		iVar0++;
	}
	return -1;
}

void func_524(struct<67> Param0, var uParam67, var uParam68, var uParam69, var uParam70, var uParam71, var uParam72, var uParam73, var uParam74, var uParam75, var uParam76, var uParam77, var uParam78, var uParam79, var uParam80, var uParam81, var uParam82, var uParam83, var uParam84, int iParam85)
{
	struct<3> Var0;
	int iVar36;
	
	if (iParam85 < 0)
	{
		return;
	}
	Var0.f_2 = 2147483647;
	Var0.f_0 = 1191422458;
	Var0.f_1 = unk_0x460153A63B9477BC();
	Var0.f_2 = { Param0.f_66 };
	Var0.f_2.f_33 = iParam85;
	iVar36 = func_526(Var0.f_1);
	if ((Global_262145.f_23581 && !Global_262145.f_23582) && !Global_262145.f_23583)
	{
		return;
	}
	if (!iVar36 == 0)
	{
		func_525();
		unk_0xB802750B43D4047C(1, &Var0, 36, iVar36);
	}
}

void func_525()
{
	unk_0xBE9B3870B1B370E3("AM_ARENA_SHP");
}

var func_526(int iParam0)
{
	var uVar0;
	
	if (iParam0 != -1)
	{
		unk_0xBE20AB8238688965(&uVar0, iParam0);
	}
	return uVar0;
}

int func_527()
{
	if (unk_0xF8EDFF98A9C94C74())
	{
		return unk_0xC5AA3415F982DC6C();
	}
	return 0;
}

int func_528(var uParam0)
{
	if (!func_193(uParam0->f_4356, 0))
	{
		return 0;
	}
	return Global_262145.f_28096;
}

int func_529(var uParam0, int iParam1, var uParam2)
{
	char cVar0[64];
	char cVar16[64];
	char cVar32[64];
	var uVar48;
	char cVar64[64];
	bool bVar80;
	int iVar81;
	int iVar82;
	
	bVar80 = false;
	iVar82 = 36;
	StringCopy(&cVar0, "HPWARN_CONF", 64);
	switch (iParam1)
	{
		case 0:
			StringCopy(&cVar16, "CH_WARN_CWE", 64);
			StringCopy(&cVar32, func_535(uParam0->f_744.f_738, 0), 64);
			break;
		
		case 1:
			StringCopy(&cVar16, "CH_WARN_CDR", 64);
			StringCopy(&cVar32, func_534(uParam0->f_744.f_740, 0), 64);
			break;
		
		case 2:
			StringCopy(&cVar16, "CH_WARN_CHR", 64);
			StringCopy(&cVar32, func_533(uParam0->f_744.f_743, 0), 64);
			break;
		
		case 3:
			StringCopy(&cVar16, "CH_WARN_APR", 64);
			StringCopy(&cVar32, func_133(1, 0), 64);
			break;
		
		case 4:
			StringCopy(&cVar16, "CH_WARN_APRb", 64);
			StringCopy(&cVar32, func_133(2, 0), 64);
			break;
		
		case 5:
			StringCopy(&cVar16, "CH_WARN_APR", 64);
			StringCopy(&cVar32, func_133(3, 0), 64);
			break;
		
		case 6:
			StringCopy(&cVar16, "CH_WARN_SETUP", 64);
			StringCopy(&cVar64, "HEIST_WARN_2", 64);
			bVar80 = true;
			iVar81 = func_528(uParam0);
			break;
		
		case 7:
			StringCopy(&cVar16, "CH_WARN_CWS", 64);
			StringCopy(&cVar32, func_532(uParam0, 0), 64);
			break;
		
		case 8:
			StringCopy(&cVar16, "CH_WARN_CVS", 64);
			StringCopy(&cVar32, func_531(uParam0, 0), 64);
			break;
		
		case 9:
			StringCopy(&cVar16, "CH_WARN_PCM", 64);
			bVar80 = true;
			iVar81 = func_126(2);
			break;
		
		case 10:
			StringCopy(&cVar16, "CH_WARN_PVD", 64);
			bVar80 = true;
			iVar81 = func_126(4);
			break;
		
		case 11:
			StringCopy(&cVar16, "CH_WARN_PDS", 64);
			bVar80 = true;
			iVar81 = func_126(3);
			break;
		
		case 12:
			StringCopy(&cVar16, "CH_WARN_SKIP", 64);
			bVar80 = true;
			iVar81 = func_530();
			break;
		
		case 13:
			switch (uParam0->f_744.f_742)
			{
				case 1:
					StringCopy(&cVar16, "CH_WARN_CIM1", 64);
					break;
				
				case 2:
					StringCopy(&cVar16, "CH_WARN_CIM2", 64);
					break;
			}
			break;
		
		case 14:
			StringCopy(&cVar0, "FM_CSC_QUIT", 64);
			StringCopy(&cVar16, "FM_CSC_QUIT1", 64);
			break;
		
		case 15:
			StringCopy(&cVar16, "CH_WARN_PDV", 64);
			bVar80 = true;
			iVar81 = func_110(6);
			break;
		
		case 16:
			StringCopy(&cVar0, "HPWARN_ALRT", 64);
			StringCopy(&cVar16, "CH_WARN_CADV", 64);
			bVar80 = true;
			iVar81 = func_110(6);
			iVar82 = 10;
			break;
		
		case 17:
			StringCopy(&cVar16, "CH_WARN_PSV", 64);
			bVar80 = true;
			iVar81 = func_110(7);
			break;
		
		case 18:
			StringCopy(&cVar0, "HPWARN_ALRT", 64);
			StringCopy(&cVar16, "CH_WARN_CASV", 64);
			bVar80 = true;
			iVar81 = func_110(7);
			iVar82 = 10;
			break;
	}
	unk_0xDC38CC1E35B6A5D7(&cVar0, &cVar16, iVar82, &cVar64, bVar80, iVar81, &cVar32, &uVar48, true, 0);
	if (unk_0xC1F43F8D0D5EB664(2))
	{
		unk_0x80DA25AB4C5012E9(1);
	}
	if (unk_0x275A6259432E6B3C(2, 202) || unk_0xC27AEE8B64EE0F8D(2, 202))
	{
		unk_0xC4CC25B68A91D144(-1, "Back", "HUD_FRONTEND_DEFAULT_SOUNDSET", 1);
		*uParam2 = 0;
		return 1;
	}
	if (unk_0x275A6259432E6B3C(2, 201) || unk_0xC27AEE8B64EE0F8D(2, 201))
	{
		if (((((iParam1 == 12 || iParam1 == 9) || iParam1 == 15) || iParam1 == 11) || iParam1 == 17) || iParam1 == 10)
		{
			unk_0xC4CC25B68A91D144(-1, "Paper_Stick", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
		}
		else
		{
			unk_0xC4CC25B68A91D144(-1, "Highlight_Accept", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
		}
		*uParam2 = 1;
		return 1;
	}
	return 0;
}

int func_530()
{
	switch (func_9(8802, -1, 0))
	{
		case 0:
			return 70000;
		
		case 1:
			return 100000;
		
		case 2:
			return 150000;
		
		case 3:
			return 200000;
		
		case 4:
			return 250000;
		
		default:
	}
	return 300000;
}

char* func_531(var uParam0, bool bParam1)
{
	switch (uParam0->f_744.f_741)
	{
		case 0:
			switch (uParam0->f_744.f_740)
			{
				case 1:
					if (!bParam1)
					{
						return "CH_VEH_0_01b";
					}
					return "CH_VEH_0_01";
				
				case 2:
					if (!bParam1)
					{
						return "CH_VEH_1_01b";
					}
					return "CH_VEH_1_01";
				
				case 3:
					if (!bParam1)
					{
						return "CH_VEH_2_01b";
					}
					return "CH_VEH_2_01";
				
				case 4:
					if (!bParam1)
					{
						return "CH_VEH_3_01b";
					}
					return "CH_VEH_3_01";
				
				case 5:
					if (!bParam1)
					{
						return "CH_VEH_4_01b";
					}
					return "CH_VEH_4_01";
				
				default:
			}
			break;
		
		case 1:
			switch (uParam0->f_744.f_740)
			{
				case 1:
					if (!bParam1)
					{
						return "CH_VEH_0_02b";
					}
					return "CH_VEH_0_02";
				
				case 2:
					if (!bParam1)
					{
						return "CH_VEH_1_02b";
					}
					return "CH_VEH_1_02";
				
				case 3:
					if (!bParam1)
					{
						return "CH_VEH_2_02b";
					}
					return "CH_VEH_2_02";
				
				case 4:
					if (!bParam1)
					{
						return "CH_VEH_3_02b";
					}
					return "CH_VEH_3_02";
				
				case 5:
					if (!bParam1)
					{
						return "CH_VEH_4_02b";
					}
					return "CH_VEH_4_02";
				
				default:
			}
			break;
		
		case 2:
			switch (uParam0->f_744.f_740)
			{
				case 1:
					if (!bParam1)
					{
						return "CH_VEH_0_03b";
					}
					return "CH_VEH_0_03";
				
				case 2:
					if (!bParam1)
					{
						return "CH_VEH_1_03b";
					}
					return "CH_VEH_1_03";
				
				case 3:
					if (!bParam1)
					{
						return "CH_VEH_2_03b";
					}
					return "CH_VEH_2_03";
				
				case 4:
					if (!bParam1)
					{
						return "CH_VEH_3_03b";
					}
					return "CH_VEH_3_03";
				
				case 5:
					if (!bParam1)
					{
						return "CH_VEH_4_03b";
					}
					return "CH_VEH_4_03";
				
				default:
			}
			break;
		
		case 3:
			switch (uParam0->f_744.f_740)
			{
				case 1:
					if (!bParam1)
					{
						return "CH_VEH_0_04b";
					}
					return "CH_VEH_0_04";
				
				case 2:
					if (!bParam1)
					{
						return "CH_VEH_1_04b";
					}
					return "CH_VEH_1_04";
				
				case 3:
					if (!bParam1)
					{
						return "CH_VEH_2_04b";
					}
					return "CH_VEH_2_04";
				
				case 4:
					if (!bParam1)
					{
						return "CH_VEH_3_04b";
					}
					return "CH_VEH_3_04";
				
				case 5:
					if (!bParam1)
					{
						return "CH_VEH_4_04b";
					}
					return "CH_VEH_4_04";
				
				default:
			}
			break;
	}
	return "";
}

char* func_532(var uParam0, bool bParam1)
{
	switch (uParam0->f_744.f_738)
	{
		case 1:
			switch (func_113(uParam0->f_4356))
			{
				case 1:
					switch (uParam0->f_744.f_739)
					{
						case 0:
							if (!bParam1)
							{
								return "CH_WEAP_0_01b";
							}
							return "CH_WEAP_0_01";
						
						case 1:
							if (!bParam1)
							{
								return "CH_WEAP_0_02b";
							}
							return "CH_WEAP_0_02";
						
						default:
					}
					break;
				
				case 2:
					switch (uParam0->f_744.f_739)
					{
						case 0:
							if (!bParam1)
							{
								return "CH_WEAP_0_03b";
							}
							return "CH_WEAP_0_03";
						
						case 1:
							if (!bParam1)
							{
								return "CH_WEAP_0_04b";
							}
							return "CH_WEAP_0_04";
						
						default:
					}
					break;
				
				case 3:
					switch (uParam0->f_744.f_739)
					{
						case 0:
							if (!bParam1)
							{
								return "CH_WEAP_0_05b";
							}
							return "CH_WEAP_0_05";
						
						case 1:
							if (!bParam1)
							{
								return "CH_WEAP_0_06b";
							}
							return "CH_WEAP_0_06";
						
						default:
					}
					break;
			}
			break;
		
		case 2:
			switch (func_113(uParam0->f_4356))
			{
				case 1:
					switch (uParam0->f_744.f_739)
					{
						case 0:
							if (!bParam1)
							{
								return "CH_WEAP_1_01b";
							}
							return "CH_WEAP_1_01";
						
						case 1:
							if (!bParam1)
							{
								return "CH_WEAP_1_02b";
							}
							return "CH_WEAP_1_02";
						
						default:
					}
					break;
				
				case 2:
					switch (uParam0->f_744.f_739)
					{
						case 0:
							if (!bParam1)
							{
								return "CH_WEAP_1_03b";
							}
							return "CH_WEAP_1_03";
						
						case 1:
							if (!bParam1)
							{
								return "CH_WEAP_1_04b";
							}
							return "CH_WEAP_1_04";
						
						default:
					}
					break;
				
				case 3:
					switch (uParam0->f_744.f_739)
					{
						case 0:
							if (!bParam1)
							{
								return "CH_WEAP_1_05b";
							}
							return "CH_WEAP_1_05";
						
						case 1:
							if (!bParam1)
							{
								return "CH_WEAP_1_06b";
							}
							return "CH_WEAP_1_06";
						
						default:
					}
					break;
			}
			break;
		
		case 3:
			switch (func_113(uParam0->f_4356))
			{
				case 1:
					switch (uParam0->f_744.f_739)
					{
						case 0:
							if (!bParam1)
							{
								return "CH_WEAP_2_01b";
							}
							return "CH_WEAP_2_01";
						
						case 1:
							if (!bParam1)
							{
								return "CH_WEAP_2_02b";
							}
							return "CH_WEAP_2_02";
						
						default:
					}
					break;
				
				case 2:
					switch (uParam0->f_744.f_739)
					{
						case 0:
							if (!bParam1)
							{
								return "CH_WEAP_2_03b";
							}
							return "CH_WEAP_2_03";
						
						case 1:
							if (!bParam1)
							{
								return "CH_WEAP_2_04b";
							}
							return "CH_WEAP_2_04";
						
						default:
					}
					break;
				
				case 3:
					switch (uParam0->f_744.f_739)
					{
						case 0:
							if (!bParam1)
							{
								return "CH_WEAP_2_05b";
							}
							return "CH_WEAP_2_05";
						
						case 1:
							if (!bParam1)
							{
								return "CH_WEAP_2_06b";
							}
							return "CH_WEAP_2_06";
						
						default:
					}
					break;
			}
			break;
		
		case 4:
			switch (func_113(uParam0->f_4356))
			{
				case 1:
					switch (uParam0->f_744.f_739)
					{
						case 0:
							if (!bParam1)
							{
								return "CH_WEAP_3_01b";
							}
							return "CH_WEAP_3_01";
						
						case 1:
							if (!bParam1)
							{
								return "CH_WEAP_3_02b";
							}
							return "CH_WEAP_3_02";
						
						default:
					}
					break;
				
				case 2:
					switch (uParam0->f_744.f_739)
					{
						case 0:
							if (!bParam1)
							{
								return "CH_WEAP_3_03b";
							}
							return "CH_WEAP_3_03";
						
						case 1:
							if (!bParam1)
							{
								return "CH_WEAP_3_04b";
							}
							return "CH_WEAP_3_04";
						
						default:
					}
					break;
				
				case 3:
					switch (uParam0->f_744.f_739)
					{
						case 0:
							if (!bParam1)
							{
								return "CH_WEAP_3_05b";
							}
							return "CH_WEAP_3_05";
						
						case 1:
							if (!bParam1)
							{
								return "CH_WEAP_3_06b";
							}
							return "CH_WEAP_3_06";
						
						default:
					}
					break;
			}
			break;
		
		case 5:
			switch (func_113(uParam0->f_4356))
			{
				case 1:
					switch (uParam0->f_744.f_739)
					{
						case 0:
							if (!bParam1)
							{
								return "CH_WEAP_4_01b";
							}
							return "CH_WEAP_4_01";
						
						case 1:
							if (!bParam1)
							{
								return "CH_WEAP_4_02b";
							}
							return "CH_WEAP_4_02";
						
						default:
					}
					break;
				
				case 2:
					switch (uParam0->f_744.f_739)
					{
						case 0:
							if (!bParam1)
							{
								return "CH_WEAP_4_03b";
							}
							return "CH_WEAP_4_03";
						
						case 1:
							if (!bParam1)
							{
								return "CH_WEAP_4_04b";
							}
							return "CH_WEAP_4_04";
						
						default:
					}
					break;
				
				case 3:
					switch (uParam0->f_744.f_739)
					{
						case 0:
							if (!bParam1)
							{
								return "CH_WEAP_4_05b";
							}
							return "CH_WEAP_4_05";
						
						case 1:
							if (!bParam1)
							{
								return "CH_WEAP_4_06b";
							}
							return "CH_WEAP_4_06";
						
						default:
					}
					break;
			}
			break;
	}
	return "";
}

char* func_533(int iParam0, bool bParam1)
{
	switch (iParam0)
	{
		case 1:
			if (!bParam1)
			{
				return "CH_HR_NAME_0b";
			}
			return "CH_HR_NAME_0";
		
		case 2:
			if (!bParam1)
			{
				return "CH_HR_NAME_1b";
			}
			return "CH_HR_NAME_1";
		
		case 3:
			if (!bParam1)
			{
				return "CH_HR_NAME_2b";
			}
			return "CH_HR_NAME_2";
		
		case 4:
			if (!bParam1)
			{
				return "CH_HR_NAME_3b";
			}
			return "CH_HR_NAME_3";
		
		case 5:
			if (!bParam1)
			{
				return "CH_HR_NAME_4b";
			}
			return "CH_HR_NAME_4";
		
		default:
	}
	return "";
}

char* func_534(int iParam0, bool bParam1)
{
	switch (iParam0)
	{
		case 1:
			if (!bParam1)
			{
				return "CH_DR_NAME_0b";
			}
			return "CH_DR_NAME_0";
		
		case 2:
			if (!bParam1)
			{
				return "CH_DR_NAME_1b";
			}
			return "CH_DR_NAME_1";
		
		case 3:
			if (!bParam1)
			{
				return "CH_DR_NAME_2b";
			}
			return "CH_DR_NAME_2";
		
		case 4:
			if (!bParam1)
			{
				return "CH_DR_NAME_3b";
			}
			return "CH_DR_NAME_3";
		
		case 5:
			if (!bParam1)
			{
				return "CH_DR_NAME_4b";
			}
			return "CH_DR_NAME_4";
		
		default:
	}
	return "";
}

char* func_535(int iParam0, bool bParam1)
{
	switch (iParam0)
	{
		case 1:
			if (!bParam1)
			{
				return "CH_WE_NAME_0b";
			}
			return "CH_WE_NAME_0";
		
		case 2:
			if (!bParam1)
			{
				return "CH_WE_NAME_1b";
			}
			return "CH_WE_NAME_1";
		
		case 3:
			if (!bParam1)
			{
				return "CH_WE_NAME_2b";
			}
			return "CH_WE_NAME_2";
		
		case 4:
			if (!bParam1)
			{
				return "CH_WE_NAME_3b";
			}
			return "CH_WE_NAME_3";
		
		case 5:
			if (!bParam1)
			{
				return "CH_WE_NAME_4b";
			}
			return "CH_WE_NAME_4";
		
		default:
	}
	return "";
}

void func_536(var uParam0)
{
	func_680(uParam0);
	func_537(uParam0);
}

void func_537(var uParam0)
{
	if (func_17(uParam0, 3) && !func_17(uParam0, 4))
	{
		if (uParam0->f_4356 == unk_0x460153A63B9477BC())
		{
			if (func_679() || func_678())
			{
				func_677();
			}
			else if (!func_17(uParam0, 4))
			{
				if (func_538(uParam0))
				{
					func_442(uParam0, 4);
					func_442(uParam0, 5);
				}
			}
		}
	}
	else if (func_17(uParam0, 5))
	{
		if (!func_17(uParam0, 6))
		{
			func_442(uParam0, 6);
			func_208(0, 0);
			func_208(0, 0);
		}
		func_205(uParam0, 5);
	}
}

int func_538(var uParam0)
{
	int iVar0;
	int iVar1;
	int iVar2;
	struct<6> Var3;
	int iVar9;
	var uVar10;
	var uVar11;
	int iVar14;
	struct<3> Var15;
	
	iVar0 = func_113(uParam0->f_4356);
	iVar1 = func_676(iVar0);
	iVar2 = func_674(iVar1);
	if (iVar2 == -1)
	{
		return 0;
	}
	Var3 = { Global_794709.f_4[iVar2 /*88*/] };
	iVar9 = Global_794709.f_4[iVar2 /*88*/].f_65;
	uVar10 = Global_794709.f_4[iVar2 /*88*/].f_71;
	func_616(func_617(unk_0x460153A63B9477BC()));
	func_463(unk_0x460153A63B9477BC(), 0, 376832, 0);
	Global_2452907.f_3826 = 1;
	func_615(unk_0x460153A63B9477BC(), 0);
	iVar9 = 158;
	func_614();
	func_613();
	func_612(65);
	func_611();
	if (func_679() || func_610())
	{
		func_677();
	}
	else
	{
		Global_1678289.f_461 = 1;
		func_609(iVar9);
		if (func_339() <= 3)
		{
			func_608();
		}
		func_606(func_97(unk_0x460153A63B9477BC()));
		func_604(&Var3);
		func_603(uVar10);
		func_602();
		func_600();
		func_599();
		func_218();
		func_598();
		Var15 = { 0f, 0f, 0f };
		func_539(&(Global_2452015.f_778), 0, 0, Var15, &uVar11, Var15, 1, 0, &uVar11, &iVar14, iVar14, 0, 0, 0, 0, 0, 1, 0, 0);
		func_208(0, 0);
		Global_1590682[unk_0x460153A63B9477BC() /*883*/].f_98 = 8;
		return 1;
	}
	return 0;
}

void func_539(var uParam0, int iParam1, int iParam2, struct<3> Param3, var uParam6, struct<3> Param7, bool bParam10, bool bParam11, var uParam12, var uParam13, int iParam14, bool bParam15, int iParam16, int iParam17, int iParam18, bool bParam19, int iParam20, bool bParam21, bool bParam22)
{
	struct<3> Var0;
	struct<3> Var3;
	float fVar6;
	int iVar7;
	struct<3> Var8;
	
	func_206(uParam0, iParam1, 1, iParam14, 1);
	if (func_85(unk_0x460153A63B9477BC(), 0))
	{
		if (bParam11)
		{
			if (!unk_0x9CC4D06F5338CA6B())
			{
				if (!func_597() || bParam15)
				{
					func_586(0);
				}
			}
			return;
		}
	}
	if (!unk_0x40397A9A17EEC1C5(*uParam0))
	{
		*uParam0 = unk_0x68C2E3309DCAD71E("DEFAULT_SCRIPTED_CAMERA", 0);
	}
	Var3 = { Param7 };
	fVar6 = -1f;
	if (func_585(iParam1))
	{
		*uParam6 = { func_584(iParam1, iParam2) };
		Var3 = { func_583(iParam1, iParam2) };
		fVar6 = func_582(iParam1, iParam2);
	}
	if (!func_581(*uParam6))
	{
		Var0 = { *uParam6 };
	}
	if (func_581(Var0))
	{
		if (!bParam10)
		{
			Var0 = { Param3 };
			if (unk_0xECF201F09142F9D9(Var0.f_0, Var0.f_1, (Var0.f_2 + 1f)))
			{
			}
			else
			{
				Var0.f_2 = (Var0.f_2 + 4f);
			}
		}
	}
	if (!unk_0x5AEB5DDFFAD43CA5(unk_0x9B0761B4C3EB8BC7()))
	{
		if (func_581(Var0))
		{
			if (func_85(unk_0x460153A63B9477BC(), 0))
			{
				if (unk_0xAE06B9E39EBDE049(func_579()) && !unk_0xA59F96B50B97E63C(func_579(), 0))
				{
					Var0 = { unk_0x3E4D3CCC220BDFB1(func_579(), 1) };
					Var3 = { 0f, 0f, unk_0xEF7858F25585F853(func_579()) };
					if (func_217(Global_1590682[unk_0x460153A63B9477BC() /*883*/].f_747, 1, 0))
					{
						Var0 = { unk_0xA28D6B5DB2149368() };
						Var3 = { unk_0x528A2AFB21867CAE(2) };
					}
				}
				else
				{
					Var0 = { unk_0x3E4D3CCC220BDFB1(unk_0x9B0761B4C3EB8BC7(), 1) };
					Var3 = { 0f, 0f, unk_0xEF7858F25585F853(unk_0x9B0761B4C3EB8BC7()) };
				}
			}
			else
			{
				Var0 = { unk_0x3E4D3CCC220BDFB1(unk_0x9B0761B4C3EB8BC7(), 1) };
				Var3 = { 0f, 0f, unk_0xEF7858F25585F853(unk_0x9B0761B4C3EB8BC7()) };
			}
			if ((func_578(unk_0x460153A63B9477BC()) || func_577(unk_0x460153A63B9477BC())) && func_576())
			{
				Var0.f_2 = (Var0.f_2 + 1.5f);
			}
			else if (unk_0x0B1B45E748AD48BA(unk_0x518D695E2DBA9DB8(unk_0x9B0761B4C3EB8BC7())))
			{
				if (func_578(unk_0x460153A63B9477BC()))
				{
					Var0.f_2 = (Var0.f_2 + 0.4f);
				}
				else
				{
					Var0.f_2 = (Var0.f_2 + 1.5f);
				}
			}
			else
			{
				Var0.f_2 = (Var0.f_2 + 4f);
			}
		}
	}
	if (bParam19)
	{
		Var0 = { 1124.745f, 263.6554f, -48.8552f };
		Var3 = { -22.4617f, 0f, -75.8431f };
		fVar6 = 50f;
	}
	else if (func_575(Global_4456448.f_82708) || iParam18)
	{
		if (func_574(unk_0x460153A63B9477BC()))
		{
			Var0 = { unk_0x3E4D3CCC220BDFB1(unk_0x9B0761B4C3EB8BC7(), 1) };
			Var3 = { 0f, 0f, unk_0xEF7858F25585F853(unk_0x9B0761B4C3EB8BC7()) };
			Var0.f_2 = (Var0.f_2 + 0.4f);
			fVar6 = unk_0xE5D3BD8BA3386277();
		}
		else
		{
			Var0 = { -194.2056f, -1853.75f, 70.3345f };
			Var3 = { -10.0011f, 0f, 131.0271f };
			fVar6 = 42.6052f;
		}
	}
	else if (func_573(Global_4456448.f_82708) || iParam17)
	{
		Var0 = { 347.2638f, 4867.324f, -59.2679f };
		Var3 = { 1.2933f, 0.0747f, -34.6934f };
		fVar6 = 50f;
	}
	else if (iParam20 || func_572(Global_4456448.f_82708))
	{
		Var0 = { 2709.332f, -366.5988f, -52.2786f };
		Var3 = { -19.962f, 0f, -121.3927f };
		fVar6 = 56.6574f;
	}
	else if (bParam21)
	{
		iVar7 = func_571(unk_0x460153A63B9477BC());
		Var0 = { unk_0x1BAE5C94E38E44FE(func_570(iVar7), func_569(iVar7), 3.5688f, 7.9792f, 4.1641f) };
		Var3 = { -21.6791f, 0f, (func_569(iVar7) + 40.237f) };
		fVar6 = 70f;
	}
	else if (bParam22 || func_568())
	{
		if (bParam22)
		{
			Var0 = { Global_1704130.f_1073 };
			Var3 = { Global_1704130.f_1073.f_3 };
			fVar6 = Global_1704130.f_1073.f_6;
		}
		else
		{
			func_566(&Var0, &Var3, &fVar6);
		}
	}
	if (func_597() && !bParam15)
	{
		Var0 = { -213.5595f, 316.9778f, 96.9244f };
		Var3 = { -1.0483f, 0f, -127.3989f };
		fVar6 = 25.5102f;
	}
	if (iParam1 == 0 && bParam11)
	{
		func_559();
		func_558(&Var8, 144, Global_1590682[unk_0x460153A63B9477BC() /*883*/].f_274.f_28);
		if (!func_581(Var8))
		{
			Var0 = { Var8 };
		}
		func_552(&Var8, 144, Global_1590682[unk_0x460153A63B9477BC() /*883*/].f_274.f_28);
		Var3 = { Var8 };
	}
	unk_0x856549C07003344B(*uParam0, 1);
	unk_0x8A3898631CCEC253(*uParam0, Var0);
	unk_0xBF92D2A60C7C686E(*uParam0, Var3, 2);
	if (fVar6 > 0f)
	{
		unk_0xD64C2E5B5BA2D347(*uParam0, fVar6);
	}
	else
	{
		unk_0xD64C2E5B5BA2D347(*uParam0, 40f);
	}
	if (func_597() && !bParam15)
	{
		if (!unk_0x5AEB5DDFFAD43CA5(unk_0x9B0761B4C3EB8BC7()))
		{
			if (!func_551(unk_0x460153A63B9477BC()))
			{
				func_463(unk_0x460153A63B9477BC(), 0, 4, 0);
				unk_0x69C71C91F56D82B9(unk_0x9B0761B4C3EB8BC7(), joaat("weapon_unarmed"), 1);
				unk_0x03D382CB0B44E2FC(unk_0x9B0761B4C3EB8BC7(), -209.434f, 305.016f, 95.9464f, 1, 0, 0, 1);
				unk_0xE922BAA80E8F9324(unk_0x9B0761B4C3EB8BC7(), 12.5f);
				unk_0x83CAF206A9B8748C(unk_0x9B0761B4C3EB8BC7(), -209.867f, 306.969f, 95.9464f, 1f, 20000, 1193033728, 0.1f);
				unk_0x854FB153F2EA838B(unk_0x9B0761B4C3EB8BC7(), joaat("MotionState_Walk"), 0, 0, 0);
				unk_0x1322CA891C5DF18A(unk_0x9B0761B4C3EB8BC7(), 0, 0);
			}
		}
		unk_0xF27E737981AFB47D(-209.434f, 305.016f, 95.9464f, 30f, 0, 0, 0, 0, 0, 0);
		func_550(1);
	}
	if (!unk_0x9CC4D06F5338CA6B())
	{
		if (!func_597() || bParam15)
		{
			func_586(0);
		}
	}
	unk_0x3ED9330214992278(1, 0, 3000, 1, 0, 0);
	if ((!func_549() && !func_548()) && !func_547())
	{
		func_546(0);
	}
	if (func_216())
	{
		unk_0x701364DD9F501826();
	}
	if (unk_0x9CC4D06F5338CA6B())
	{
		*uParam12 = { Var0 };
		*uParam13 = 1;
	}
	else
	{
		func_545(Var0);
	}
	unk_0xD28B53FDA67841A1(unk_0x460153A63B9477BC(), 1);
	func_540(func_597(), ((iParam16 || iParam17) || iParam18));
}

void func_540(bool bParam0, bool bParam1)
{
	if (func_85(unk_0x460153A63B9477BC(), 0))
	{
		return;
	}
	if (!bParam1 && !unk_0x5AEB5DDFFAD43CA5(unk_0x9B0761B4C3EB8BC7()))
	{
		if (!unk_0x47DBF174A0CB9D55(unk_0x9B0761B4C3EB8BC7(), 1))
		{
			if (unk_0xB77B1396EAA80D8F(unk_0x9B0761B4C3EB8BC7()))
			{
				unk_0x1F8106DB8796859F(unk_0x9B0761B4C3EB8BC7(), 1, 1);
			}
		}
	}
	if (!bParam0)
	{
		if (unk_0x25DDB354A40FFCDB())
		{
			if (func_542())
			{
				unk_0x2BB0EF9DE445EA13(unk_0x9B0761B4C3EB8BC7());
			}
			if (func_541())
			{
				func_463(unk_0x460153A63B9477BC(), 0, 32772, 0);
			}
			else if (!bParam1)
			{
				func_463(unk_0x460153A63B9477BC(), 0, 319492, 0);
			}
		}
		else
		{
			func_463(unk_0x460153A63B9477BC(), 0, 57348, 0);
		}
	}
}

bool func_541()
{
	return Global_2452907.f_6234;
}

int func_542()
{
	var uVar0;
	
	if (unk_0x47DBF174A0CB9D55(unk_0x9B0761B4C3EB8BC7(), 0))
	{
		uVar0 = unk_0xA5D5B1042E8F47BD(unk_0x9B0761B4C3EB8BC7(), 0);
		if (func_544(uVar0, 1) && !func_543(uVar0, 1))
		{
			return 1;
		}
	}
	return 0;
}

int func_543(int iParam0, bool bParam1)
{
	if (Global_76833)
	{
		if (unk_0xAE06B9E39EBDE049(uParam0) && (!bParam1 || unk_0x80FF6C016CDB0FAF(iParam0, 0)))
		{
			if (unk_0x871819940BD2E434(iParam0, "Player_Vehicle"))
			{
				if (unk_0x8C0800114C2864BE(iParam0, "Player_Vehicle") == unk_0x69FCAB6112C0ABA7(unk_0x460153A63B9477BC()))
				{
					return 1;
				}
			}
		}
	}
	return 0;
}

int func_544(int iParam0, bool bParam1)
{
	if (Global_76833)
	{
		if (unk_0xAE06B9E39EBDE049(uParam0) && (!bParam1 || unk_0x80FF6C016CDB0FAF(iParam0, 0)))
		{
			if (unk_0x871819940BD2E434(iParam0, "Player_Vehicle"))
			{
				return 1;
			}
		}
	}
	return 0;
}

void func_545(struct<3> Param0)
{
	if (!func_216())
	{
		return;
	}
	unk_0xE8B1D4190AF4EE5A(Param0, 0f, 0f, 0f);
	unk_0xDF93DB0848FA3B5E(1);
}

void func_546(int iParam0)
{
	if (func_214())
	{
		return;
	}
	if (Global_1590682[unk_0x460153A63B9477BC() /*883*/].f_35 && !Global_2452907.f_2846.f_215 == -1)
	{
		return;
	}
	if (unk_0x03DB5C6AABF8DA46() || unk_0x5DA78AD6801A0523())
	{
		unk_0xFD45D90342D2A0CD(iParam0);
	}
}

bool func_547()
{
	return unk_0xCE990E643CD9D0E5(Global_2452015.f_3, 12);
}

bool func_548()
{
	return unk_0xCE990E643CD9D0E5(Global_2452015.f_3, 9);
}

bool func_549()
{
	return unk_0xCE990E643CD9D0E5(Global_2452015.f_3, 6);
}

void func_550(bool bParam0)
{
	if (bParam0)
	{
		unk_0xBE20AB8238688965(&(Global_1590682[unk_0x460153A63B9477BC() /*883*/].f_146), 27);
	}
	else
	{
		unk_0xD2459066EA58CE43(&(Global_1590682[unk_0x460153A63B9477BC() /*883*/].f_146), 27);
	}
}

bool func_551(int iParam0)
{
	return unk_0xCE990E643CD9D0E5(Global_1590682[iParam0 /*883*/].f_146, 27);
}

int func_552(var uParam0, int iParam1, int iParam2)
{
	struct<4> Var0;
	
	switch (iParam2)
	{
		case default:
			return 0;
		
		case 1:
		case 2:
		case 3:
		case 4:
		case 5:
		case 6:
		case 7:
		case 34:
		case 35:
		case 36:
		case 37:
		case 38:
		case 39:
		case 40:
		case 41:
		case 42:
		case 43:
			func_553(iParam2, iParam1, &Var0, -1, 0);
			*uParam0 = { Var0.f_3 };
			return 1;
		
		case 61:
		case 62:
		case 63:
		case 64:
		case 65:
			func_553(iParam2, iParam1, &Var0, 61, 0);
			*uParam0 = { Var0.f_3 };
			return 1;
		
		case 83:
		case 84:
		case 85:
			func_553(iParam2, iParam1, &Var0, 83, 0);
			*uParam0 = { Var0.f_3 };
			return 1;
		
		case 77:
		case 78:
		case joaat("mpsv_lp0_31"):
		case 80:
		case 81:
		case 82:
			func_553(iParam2, iParam1, &Var0, 77, 0);
			*uParam0 = { Var0.f_3 };
			return 1;
		
		case 73:
		case 74:
		case 75:
		case 76:
			func_553(iParam2, iParam1, &Var0, 73, 0);
			*uParam0 = { Var0.f_3 };
			return 1;
		
		case 87:
		case 88:
		case 89:
		case 90:
			func_553(iParam2, iParam1, &Var0, 88, 0);
			*uParam0 = { Var0.f_3 };
			return 1;
			return 0;
	}
}

void func_553(int iParam0, int iParam1, var uParam2, int iParam3, bool bParam4)
{
	struct<6> Var0[2];
	struct<3> Var13;
	
	Var0[0 /*6*/] = { func_557(iParam3, bParam4) };
	Var0[1 /*6*/] = { func_557(iParam0, bParam4) };
	*uParam2 = { func_555(iParam1, iParam3) };
	Var13 = { *uParam2 - Var0[0 /*6*/] };
	Var13 = { func_554(Var13, -Var0[0 /*6*/].f_3.f_2) };
	Var13 = { func_554(Var13, Var0[1 /*6*/].f_3.f_2) };
	*uParam2 = { unk_0x1BAE5C94E38E44FE(Var0[1 /*6*/], 0f, Var13) };
	switch (iParam1)
	{
		case 6:
		case 2:
		case 9:
		case 10:
		case 11:
		case 12:
		case 13:
		case 14:
		case 34:
		case 35:
		case 36:
		case 38:
		case 39:
		case 120:
		case 121:
		case 125:
		case 40:
		case 41:
		case 43:
		case 44:
		case 45:
		case 42:
		case 46:
		case 47:
		case 48:
		case 49:
		case 50:
		case 51:
		case 52:
		case 53:
		case 54:
		case 55:
		case 56:
		case 64:
		case 65:
		case 57:
		case 58:
		case 59:
		case 60:
		case 61:
		case 62:
		case 63:
		case 66:
		case 67:
		case 68:
		case 109:
		case 69:
		case 70:
		case 71:
		case 72:
		case 73:
		case 74:
		case 75:
		case 76:
		case 77:
		case 78:
		case joaat("mpsv_lp0_31"):
		case 80:
		case 81:
		case 82:
		case 83:
		case 85:
		case 84:
		case 89:
		case 90:
		case 91:
		case 92:
		case 94:
		case 95:
		case 96:
		case 97:
		case 98:
		case 93:
		case 99:
		case 100:
		case 106:
		case 107:
		case 108:
		case 119:
		case 122:
		case 123:
		case 124:
		case 138:
		case 139:
		case 140:
		case 130:
		case 128:
		case 141:
		case 142:
		case 143:
		case 144:
		case 145:
		case 146:
		case 147:
		case 148:
		case 153:
		case 154:
		case 155:
		case 157:
		case 234:
		case 174:
		case 175:
		case 176:
		case 177:
		case 178:
		case 179:
		case 180:
		case 235:
		case 149:
		case 150:
		case 151:
		case 152:
		case 172:
		case 187:
		case 173:
		case 201:
		case 188:
		case 189:
		case 190:
		case 191:
		case 192:
		case 193:
		case 194:
		case 195:
		case 196:
		case 197:
		case 198:
		case 199:
		case 200:
		case 284:
		case 285:
		case 286:
		case 287:
		case 202:
		case 203:
		case 204:
		case 205:
		case 206:
		case 207:
		case 208:
		case 209:
		case 210:
		case 211:
		case 212:
		case 213:
		case 214:
		case 216:
		case 217:
		case 182:
		case 183:
		case 181:
		case 156:
		case 236:
		case 237:
		case 238:
		case 239:
		case 240:
		case 241:
		case 242:
		case 243:
		case 244:
		case 245:
		case 246:
		case 247:
		case 248:
		case 249:
		case 250:
		case 251:
		case 252:
		case 253:
		case 254:
		case 255:
		case 256:
		case 257:
		case 258:
		case 259:
		case 260:
		case 261:
		case 262:
		case 263:
		case 264:
		case 265:
		case 266:
		case 267:
		case 268:
		case 269:
		case 270:
		case 271:
		case 272:
		case 273:
		case 274:
		case 275:
			while (Var0[0 /*6*/].f_3.f_2 > 180f)
			{
				Var0[0 /*6*/].f_3.f_2 = (Var0[0 /*6*/].f_3.f_2 - 360f);
			}
			while (Var0[0 /*6*/].f_3.f_2 < -180f)
			{
				Var0[0 /*6*/].f_3.f_2 = (Var0[0 /*6*/].f_3.f_2 + 360f);
			}
			while (Var0[1 /*6*/].f_3.f_2 > 180f)
			{
				Var0[1 /*6*/].f_3.f_2 = (Var0[1 /*6*/].f_3.f_2 - 360f);
			}
			while (Var0[1 /*6*/].f_3.f_2 < -180f)
			{
				Var0[1 /*6*/].f_3.f_2 = (Var0[1 /*6*/].f_3.f_2 + 360f);
			}
			uParam2->f_3.f_2 = (uParam2->f_3.f_2 + (Var0[1 /*6*/].f_3.f_2 - Var0[0 /*6*/].f_3.f_2));
			while (uParam2->f_3.f_2 > 180f)
			{
				uParam2->f_3.f_2 = (uParam2->f_3.f_2 - 360f);
			}
			while (uParam2->f_3.f_2 < -180f)
			{
				uParam2->f_3.f_2 = (uParam2->f_3.f_2 + 360f);
			}
			break;
	}
	switch (iParam1)
	{
		case 278:
		case 279:
		case 280:
		case 281:
		case 282:
		case 283:
		case 300:
		case 301:
		case 302:
		case 303:
		case 304:
		case 306:
		case 305:
		case 308:
		case 309:
		case 312:
		case 313:
		case 314:
		case 315:
		case 316:
		case 317:
		case 318:
		case 319:
		case 320:
		case 321:
		case 322:
		case 323:
		case 324:
		case 325:
		case 326:
		case 327:
		case 328:
		case 329:
		case 307:
		case 331:
		case 332:
		case 333:
		case 334:
		case 310:
		case 335:
		case 336:
		case 337:
		case 338:
		case 339:
		case 311:
		case 340:
		case 435:
		case 436:
		case 437:
		case 470:
		case 477:
		case 504:
		case 507:
		case 510:
		case 513:
		case 528:
		case 531:
		case 534:
		case 537:
		case 540:
		case 547:
		case 445:
		case 446:
		case 447:
		case 448:
		case 449:
		case 450:
		case 471:
		case 472:
		case 478:
		case 479:
		case 505:
		case 506:
		case 508:
		case 509:
		case 511:
		case 512:
		case 514:
		case 515:
		case 529:
		case 530:
		case 532:
		case 533:
		case 535:
		case 536:
		case 538:
		case 539:
		case 541:
		case 542:
		case 548:
		case 549:
		case 432:
		case 433:
		case 434:
		case 451:
		case 452:
		case 453:
		case 454:
		case 455:
		case 456:
		case 459:
		case 460:
		case 461:
		case 462:
		case 463:
		case 464:
		case 560:
		case 546:
		case 587:
		case 588:
		case 589:
		case 590:
		case 591:
		case 592:
		case 501:
		case 502:
		case 500:
		case 616:
		case 615:
		case 612:
		case 629:
		case 630:
		case 631:
		case 714:
		case 633:
		case 634:
		case 635:
		case 636:
		case 637:
		case 638:
		case 639:
		case 640:
		case 643:
		case 644:
		case 641:
		case 642:
		case 646:
		case 645:
		case 647:
		case 648:
		case 649:
		case 650:
		case 666:
		case 667:
		case 668:
		case 669:
		case 670:
		case 671:
		case 673:
		case 674:
		case 675:
		case 676:
		case 677:
		case 689:
		case 690:
		case 691:
		case 692:
		case 693:
		case 694:
		case 695:
		case 696:
		case 697:
		case 698:
		case 699:
		case 700:
		case 701:
		case 702:
		case 703:
		case 704:
		case 705:
		case 706:
		case 707:
		case 708:
		case 709:
		case 710:
		case 711:
		case 712:
		case 713:
		case 632:
		case 738:
		case 739:
		case 740:
		case 741:
		case 742:
		case 743:
		case 744:
		case 745:
		case 746:
			while (Var0[0 /*6*/].f_3.f_2 > 180f)
			{
				Var0[0 /*6*/].f_3.f_2 = (Var0[0 /*6*/].f_3.f_2 - 360f);
			}
			while (Var0[0 /*6*/].f_3.f_2 < -180f)
			{
				Var0[0 /*6*/].f_3.f_2 = (Var0[0 /*6*/].f_3.f_2 + 360f);
			}
			while (Var0[1 /*6*/].f_3.f_2 > 180f)
			{
				Var0[1 /*6*/].f_3.f_2 = (Var0[1 /*6*/].f_3.f_2 - 360f);
			}
			while (Var0[1 /*6*/].f_3.f_2 < -180f)
			{
				Var0[1 /*6*/].f_3.f_2 = (Var0[1 /*6*/].f_3.f_2 + 360f);
			}
			uParam2->f_3.f_2 = (uParam2->f_3.f_2 + (Var0[1 /*6*/].f_3.f_2 - Var0[0 /*6*/].f_3.f_2));
			while (uParam2->f_3.f_2 > 180f)
			{
				uParam2->f_3.f_2 = (uParam2->f_3.f_2 - 360f);
			}
			while (uParam2->f_3.f_2 < -180f)
			{
				uParam2->f_3.f_2 = (uParam2->f_3.f_2 + 360f);
			}
			break;
	}
	switch (iParam1)
	{
		case 715:
		case 716:
		case 717:
		case 718:
		case 719:
		case 720:
		case 721:
		case 722:
		case 723:
		case 724:
		case 725:
		case 726:
		case 727:
		case 728:
		case 729:
		case 730:
		case 731:
		case 732:
		case 733:
		case 734:
		case 735:
		case 736:
		case 737:
		case 747:
		case 748:
		case 749:
		case 750:
		case 751:
		case 752:
		case 753:
		case 754:
		case 755:
		case 756:
		case 757:
		case 758:
		case 759:
		case 760:
		case 761:
		case 762:
		case 763:
		case 764:
		case 765:
		case 766:
		case 767:
		case 768:
		case 769:
		case 770:
		case 771:
		case 772:
		case 773:
		case 774:
		case 775:
		case 776:
		case 777:
		case 778:
		case 779:
		case 780:
		case 781:
		case 782:
		case 783:
		case 784:
		case 785:
		case 786:
		case 787:
		case 788:
		case 789:
		case 790:
		case 791:
		case 792:
		case 793:
		case 794:
		case 795:
		case 796:
		case 797:
		case 798:
		case 799:
		case 800:
		case 802:
		case 801:
		case 803:
		case 804:
		case 805:
		case 806:
		case 807:
		case 808:
		case 809:
		case 678:
		case 679:
		case 680:
		case 681:
		case 682:
		case 683:
		case 684:
		case 685:
		case 810:
		case 811:
		case 812:
		case 813:
		case 814:
		case 815:
		case 817:
		case 816:
		case 819:
		case 818:
		case 820:
		case 821:
		case 822:
		case 823:
		case 824:
		case 825:
		case 826:
		case 827:
		case 828:
		case 829:
		case 830:
		case 831:
		case 832:
		case 833:
		case 834:
		case 835:
		case 836:
		case 837:
		case 838:
		case 839:
		case 840:
		case 841:
		case 842:
		case 843:
		case 844:
		case 845:
		case 846:
		case 847:
		case 848:
		case 849:
		case 850:
		case 851:
		case 852:
		case 853:
		case 854:
		case 855:
		case 856:
		case 686:
		case 687:
		case 688:
			while (Var0[0 /*6*/].f_3.f_2 > 180f)
			{
				Var0[0 /*6*/].f_3.f_2 = (Var0[0 /*6*/].f_3.f_2 - 360f);
			}
			while (Var0[0 /*6*/].f_3.f_2 < -180f)
			{
				Var0[0 /*6*/].f_3.f_2 = (Var0[0 /*6*/].f_3.f_2 + 360f);
			}
			while (Var0[1 /*6*/].f_3.f_2 > 180f)
			{
				Var0[1 /*6*/].f_3.f_2 = (Var0[1 /*6*/].f_3.f_2 - 360f);
			}
			while (Var0[1 /*6*/].f_3.f_2 < -180f)
			{
				Var0[1 /*6*/].f_3.f_2 = (Var0[1 /*6*/].f_3.f_2 + 360f);
			}
			uParam2->f_3.f_2 = (uParam2->f_3.f_2 + (Var0[1 /*6*/].f_3.f_2 - Var0[0 /*6*/].f_3.f_2));
			while (uParam2->f_3.f_2 > 180f)
			{
				uParam2->f_3.f_2 = (uParam2->f_3.f_2 - 360f);
			}
			while (uParam2->f_3.f_2 < -180f)
			{
				uParam2->f_3.f_2 = (uParam2->f_3.f_2 + 360f);
			}
			break;
	}
	switch (iParam1)
	{
		case 857:
		case 858:
		case 859:
		case 860:
		case 861:
		case 862:
		case 863:
		case 864:
		case 865:
		case 866:
		case 867:
		case 868:
		case 869:
		case 870:
		case 871:
		case 872:
		case 873:
		case 874:
		case 875:
		case 876:
		case 877:
		case 878:
		case 879:
		case 880:
		case 881:
		case 882:
		case 883:
		case 884:
		case 888:
		case 889:
		case 890:
		case 891:
		case 651:
		case 652:
		case 653:
		case 654:
		case 655:
		case 656:
		case 657:
		case 658:
		case 659:
		case 660:
		case 661:
		case 662:
		case 663:
		case 664:
		case 665:
		case 887:
		case 894:
		case 895:
		case 896:
		case 897:
		case 898:
		case 899:
		case 900:
		case 901:
		case 902:
		case 903:
		case 904:
		case 905:
		case 906:
		case 907:
		case 908:
		case 911:
		case 913:
		case 914:
		case 915:
		case 916:
		case 917:
		case 918:
		case 919:
		case 920:
		case 921:
		case 922:
		case 923:
		case 924:
		case 925:
		case 926:
		case 927:
		case 928:
		case 929:
		case 930:
		case 931:
		case 932:
		case 954:
		case 955:
		case 956:
		case 957:
		case 958:
		case 959:
		case 968:
		case 969:
		case 970:
		case 973:
		case 974:
		case 975:
		case 976:
		case 977:
		case 978:
		case 979:
		case 980:
		case 981:
		case 982:
		case 983:
		case 984:
		case 985:
		case 986:
		case 987:
		case 988:
		case 989:
		case 990:
		case 1008:
		case 1009:
		case 1010:
		case 991:
		case 992:
		case 993:
		case 994:
		case 995:
		case 996:
		case 997:
		case 998:
		case 999:
		case 960:
		case 961:
		case 962:
		case 963:
		case 964:
		case 965:
		case 966:
		case 967:
		case 1000:
		case 1001:
		case 1002:
		case 1003:
		case 1004:
		case 1005:
		case 1006:
		case 1007:
		case 1011:
		case 1012:
		case 1013:
			while (Var0[0 /*6*/].f_3.f_2 > 180f)
			{
				Var0[0 /*6*/].f_3.f_2 = (Var0[0 /*6*/].f_3.f_2 - 360f);
			}
			while (Var0[0 /*6*/].f_3.f_2 < -180f)
			{
				Var0[0 /*6*/].f_3.f_2 = (Var0[0 /*6*/].f_3.f_2 + 360f);
			}
			while (Var0[1 /*6*/].f_3.f_2 > 180f)
			{
				Var0[1 /*6*/].f_3.f_2 = (Var0[1 /*6*/].f_3.f_2 - 360f);
			}
			while (Var0[1 /*6*/].f_3.f_2 < -180f)
			{
				Var0[1 /*6*/].f_3.f_2 = (Var0[1 /*6*/].f_3.f_2 + 360f);
			}
			uParam2->f_3.f_2 = (uParam2->f_3.f_2 + (Var0[1 /*6*/].f_3.f_2 - Var0[0 /*6*/].f_3.f_2));
			while (uParam2->f_3.f_2 > 180f)
			{
				uParam2->f_3.f_2 = (uParam2->f_3.f_2 - 360f);
			}
			while (uParam2->f_3.f_2 < -180f)
			{
				uParam2->f_3.f_2 = (uParam2->f_3.f_2 + 360f);
			}
			break;
	}
}

Vector3 func_554(struct<3> Param0, float fParam3)
{
	struct<3> Var0;
	float fVar3;
	float fVar4;
	
	fVar3 = unk_0x0BADBFA3B172435F(fParam3);
	fVar4 = unk_0xD0FFB162F40A139C(fParam3);
	Var0.f_0 = ((Param0.f_0 * fVar4) - (Param0.f_1 * fVar3));
	Var0.f_1 = ((Param0.f_0 * fVar3) + (Param0.f_1 * fVar4));
	Var0.f_2 = Param0.f_2;
	return Var0;
}

struct<6> func_555(var uParam0, int iParam1)
{
	struct<6> Var0;
	var uVar6;
	int iVar23;
	
	iVar23 = 0;
	if (func_556(iParam1, &uVar6))
	{
		iVar23 = 1;
	}
	if (iVar23 && unk_0xE9886E8386F849A7(&Var0, &(Var0.f_3), uParam0, uVar6))
	{
		return Var0;
	}
	return Var0;
}

int func_556(int iParam0, var uParam1)
{
	switch (iParam0)
	{
		case -1:
		case 1:
			*uParam1 = 0;
			StringCopy(&(uParam1->f_1), "BaseElementLocationsMap", 64);
			return 1;
			break;
		
		case 61:
			*uParam1 = 1;
			StringCopy(&(uParam1->f_1), "BaseElementLocationsMap_HighApt", 64);
			return 1;
			break;
		
		case 73:
			*uParam1 = 2;
			StringCopy(&(uParam1->f_1), "ExtraBaseElementLocMap1", 64);
			return 1;
			break;
		
		case 77:
			*uParam1 = 3;
			StringCopy(&(uParam1->f_1), "ExtraBaseElementLocMap2", 64);
			return 1;
			break;
		
		case 83:
			*uParam1 = 4;
			StringCopy(&(uParam1->f_1), "ExtraBaseElementLocMap3", 64);
			return 1;
			break;
		
		case 86:
			*uParam1 = 5;
			StringCopy(&(uParam1->f_1), "ExtraBaseElementLocMap4", 64);
			return 1;
			break;
		
		case 88:
			*uParam1 = 6;
			StringCopy(&(uParam1->f_1), "ExtraBaseElementLocMap5", 64);
			return 1;
			break;
		
		case 91:
			*uParam1 = 7;
			StringCopy(&(uParam1->f_1), "ExtraBaseElementLocMap6", 64);
			return 1;
			break;
		
		case 97:
			*uParam1 = 8;
			StringCopy(&(uParam1->f_1), "ExtraBaseElementLocMap7", 64);
			return 1;
			break;
		
		case 109:
			*uParam1 = 9;
			StringCopy(&(uParam1->f_1), "ExtraBaseElementLocMap8", 64);
			return 1;
			break;
	}
	return 0;
}

struct<6> func_557(int iParam0, bool bParam1)
{
	struct<6> Var0;
	
	switch (iParam0)
	{
		case -1:
			Var0 = { -794.9184f, 339.6266f, 200.4135f };
			Var0.f_3 = { 0f, 0f, 180f };
			break;
		
		case 1:
			Var0 = { -794.9184f, 339.6266f, 200.4135f };
			Var0.f_3 = { 0f, 0f, 180f };
			break;
		
		case 2:
			Var0 = { -761.0982f, 317.6259f, 169.5963f };
			Var0.f_3 = { 0f, 0f, 0f };
			break;
		
		case 3:
			Var0 = { -761.1888f, 317.6295f, 216.0503f };
			Var0.f_3 = { 0f, 0f, 0f };
			break;
		
		case 4:
			Var0 = { -795.3856f, 340.0188f, 152.7941f };
			Var0.f_3 = { 0f, 0f, 180f };
			break;
		
		case 61:
			Var0 = { -778.5056f, 332.3779f, 212.1968f };
			Var0.f_3 = { 0f, 0f, 90f };
			break;
		
		case 5:
			Var0 = { -258.1807f, -950.6853f, 70.0239f };
			Var0.f_3 = { 0f, 0f, 70f };
			break;
		
		case 6:
			Var0 = { -285.0051f, -957.6552f, 85.3035f };
			Var0.f_3 = { 0f, 0f, -110f };
			break;
		
		case 7:
			Var0 = { -1471.882f, -530.7484f, 62.34918f };
			Var0.f_3 = { 0f, 0f, -145f };
			break;
		
		case 34:
			Var0 = { -1471.882f, -530.7484f, 49.72156f };
			Var0.f_3 = { 0f, 0f, -145f };
			break;
		
		case 62:
			Var0 = { -1463.15f, -540.2369f, 74.2439f };
			Var0.f_3 = { 0f, 0f, -145f };
			break;
		
		case 35:
			Var0 = { -885.3702f, -451.4775f, 119.327f };
			Var0.f_3 = { 0f, 0f, 27.55617f };
			break;
		
		case 36:
			Var0 = { -913.0385f, -438.4284f, 114.3997f };
			Var0.f_3 = { 0f, 0f, -153.3093f };
			break;
		
		case 37:
			Var0 = { -892.5499f, -430.4789f, 88.25368f };
			Var0.f_3 = { 0f, 0f, 116.9193f };
			break;
		
		case 38:
			Var0 = { -35.0462f, -576.317f, 82.90739f };
			Var0.f_3 = { 0f, 0f, 160f };
			break;
		
		case 39:
			Var0 = { -10.3788f, -590.7431f, 93.02542f };
			Var0.f_3 = { 0f, 0f, 70f };
			break;
		
		case 65:
			Var0 = { -22.2487f, -589.1461f, 80.2305f };
			Var0.f_3 = { 0f, 0f, 69.88f };
			break;
		
		case 40:
			Var0 = { -900.6311f, -376.7462f, 78.27306f };
			Var0.f_3 = { 0f, 0f, 26.92611f };
			break;
		
		case 41:
			Var0 = { -929.483f, -374.5104f, 102.2329f };
			Var0.f_3 = { 0f, 0f, -152.5531f };
			break;
		
		case 63:
			Var0 = { -914.4202f, -375.8189f, 114.4743f };
			Var0.f_3 = { 0f, 0f, -63f };
			break;
		
		case 42:
			Var0 = { -617.1647f, 64.6042f, 100.8196f };
			Var0.f_3 = { 0f, 0f, 180f };
			break;
		
		case 43:
			Var0 = { -584.2015f, 42.7133f, 86.4187f };
			Var0.f_3 = { 0f, 0f, 0f };
			break;
		
		case 64:
			Var0 = { -609.5665f, 50.2203f, 98.3998f };
			Var0.f_3 = { 0f, 0f, -90f };
			break;
		
		case 73:
			Var0 = { -171.3969f, 494.2671f, 134.4935f };
			Var0.f_3 = { 0f, 0f, 11f };
			break;
		
		case 74:
			Var0 = { 339.4982f, 434.0887f, 146.2206f };
			Var0.f_3 = { 0f, 0f, -63.5f };
			break;
		
		case 75:
			Var0 = { -761.3884f, 615.7333f, 140.9805f };
			Var0.f_3 = { 0f, 0f, -71.5f };
			break;
		
		case 76:
			Var0 = { -678.1752f, 591.0076f, 142.2196f };
			Var0.f_3 = { 0f, 0f, 40.5f };
			break;
		
		case 77:
			Var0 = { 120.0541f, 553.793f, 181.0943f };
			Var0.f_3 = { 0f, 0f, 6f };
			break;
		
		case 78:
			Var0 = { -571.4039f, 655.2008f, 142.6293f };
			Var0.f_3 = { 0f, 0f, -14.5f };
			break;
		
		case joaat("mpsv_lp0_31"):
			Var0 = { -742.2565f, 587.6547f, 143.0577f };
			Var0.f_3 = { 0f, 0f, -29f };
			break;
		
		case 80:
			Var0 = { -857.2222f, 685.051f, 149.6502f };
			Var0.f_3 = { 0f, 0f, 4.5f };
			break;
		
		case 81:
			Var0 = { -1287.65f, 443.2707f, 94.6919f };
			Var0.f_3 = { 0f, 0f, 0f };
			break;
		
		case 82:
			Var0 = { 374.2012f, 416.9688f, 142.6977f };
			Var0.f_3 = { 0f, 0f, -14f };
			break;
		
		case 83:
			Var0 = { -787.7805f, 334.9232f, 186.1134f };
			Var0.f_3 = { 0f, 0f, 90f };
			break;
		
		case 84:
			Var0 = { -787.7805f, 334.9232f, 215.8384f };
			Var0.f_3 = { 0f, 0f, 90f };
			break;
		
		case 85:
			Var0 = { -773.2258f, 322.8252f, 194.8862f };
			Var0.f_3 = { 0f, 0f, -90f };
			break;
		
		case 86:
			Var0 = { -1573.098f, -4085.806f, 9.7851f };
			Var0.f_3 = { 0f, 0f, 162f };
			break;
		
		case 8:
		case 9:
		case 10:
		case 11:
		case 12:
		case 13:
		case 14:
		case 15:
		case 16:
		case 66:
		case 67:
		case 68:
		case 69:
			Var0 = { 342.8157f, -997.4288f, -100f };
			Var0.f_3 = { 0f, 0f, 0f };
			break;
		
		case 17:
		case 18:
		case 19:
		case 20:
		case 21:
		case 22:
		case 23:
		case 70:
		case 71:
		case 72:
			Var0 = { 260.3297f, -997.4288f, -100f };
			Var0.f_3 = { 0f, 0f, 0f };
			break;
		
		case 87:
			Var0 = { -1572.187f, -570.8315f, 109.9879f };
			Var0.f_3 = { 0f, 0f, -54f };
			break;
		
		case 88:
			Var0 = { -1383.954f, -476.7112f, 73.507f };
			Var0.f_3 = { 0f, 0f, 8f };
			break;
		
		case 89:
			Var0 = { -138.0029f, -629.739f, 170.2854f };
			Var0.f_3 = { 0f, 0f, -84f };
			break;
		
		case 90:
			Var0 = { -74.8895f, -817.6883f, 244.8508f };
			Var0.f_3 = { 0f, 0f, 70f };
			break;
		
		case 91:
		case 92:
		case 93:
		case 94:
		case 95:
		case 96:
			Var0 = { 1100.764f, -3159.384f, -34.9342f };
			Var0.f_3 = { 0f, 0f, 0f };
			break;
		
		case 97:
		case 98:
		case 99:
		case 100:
		case 101:
		case 102:
			Var0 = { 1005.806f, -3157.67f, -36.0897f };
			Var0.f_3 = { 0f, 0f, 0f };
			break;
		
		case 103:
			if (!bParam1)
			{
				Var0 = { -1576.571f, -569.7595f, 85.5f };
				Var0.f_3 = { 0f, 0f, 36.1f };
			}
			else
			{
				Var0 = { -1578.022f, -576.4251f, 104.2f };
				Var0.f_3 = { 0f, 0f, -144.04f };
			}
			break;
		
		case 104:
			if (!bParam1)
			{
				Var0 = { -1571.254f, -566.5865f, 85.5f };
				Var0.f_3 = { 0f, 0f, -53.9f };
			}
			else
			{
				Var0 = { -1578.022f, -576.4251f, 104.2f };
				Var0.f_3 = { 0f, 0f, -144.04f };
			}
			break;
		
		case 105:
			if (!bParam1)
			{
				Var0 = { -1568.098f, -571.9171f, 85.5f };
				Var0.f_3 = { 0f, 0f, -143.9f };
			}
			else
			{
				Var0 = { -1578.022f, -576.4251f, 104.2f };
				Var0.f_3 = { 0f, 0f, -144.04f };
			}
			break;
		
		case 106:
			if (!bParam1)
			{
				Var0 = { -1384.518f, -475.8657f, 56.1f };
				Var0.f_3 = { 0f, 0f, 98.7f };
			}
			else
			{
				Var0 = { -1391.245f, -473.9638f, 77.2f };
				Var0.f_3 = { 0f, 0f, 98.86f };
			}
			break;
		
		case 107:
			if (!bParam1)
			{
				Var0 = { -1384.538f, -475.8829f, 48.1f };
				Var0.f_3 = { 0f, 0f, 98.7f };
			}
			else
			{
				Var0 = { -1391.245f, -473.9638f, 77.2f };
				Var0.f_3 = { 0f, 0f, 98.86f };
			}
			break;
		
		case 108:
			if (!bParam1)
			{
				Var0 = { -1378.994f, -477.2481f, 56.1f };
				Var0.f_3 = { 0f, 0f, -81.1f };
			}
			else
			{
				Var0 = { -1391.245f, -473.9638f, 77.2f };
				Var0.f_3 = { 0f, 0f, 98.86f };
			}
			break;
		
		case 109:
			if (!bParam1)
			{
				Var0 = { -186.5683f, -576.4624f, 135f };
				Var0.f_3 = { 0f, 0f, 96.16f };
			}
			else
			{
				Var0 = { -146.6167f, -596.6301f, 166f };
				Var0.f_3 = { 0f, 0f, -140f };
			}
			break;
		
		case 110:
			if (!bParam1)
			{
				Var0 = { -113.886f, -564.3862f, 135f };
				Var0.f_3 = { 0f, 0f, 110.96f };
			}
			else
			{
				Var0 = { -146.6167f, -596.6301f, 166f };
				Var0.f_3 = { 0f, 0f, -140f };
			}
			break;
		
		case 111:
			if (!bParam1)
			{
				Var0 = { -134.6568f, -635.1774f, 135f };
				Var0.f_3 = { 0f, 0f, -9.04f };
			}
			else
			{
				Var0 = { -146.6167f, -596.6301f, 166f };
				Var0.f_3 = { 0f, 0f, -140f };
			}
			break;
		
		case 112:
			if (!bParam1)
			{
				Var0 = { -79.0479f, -822.6393f, 221f };
				Var0.f_3 = { 0f, 0f, 70f };
			}
			else
			{
				Var0 = { -73.904f, -821.6204f, 284f };
				Var0.f_3 = { 0f, 0f, -110f };
			}
			break;
		
		case 113:
			if (!bParam1)
			{
				Var0 = { -70.3086f, -819.5784f, 221f };
				Var0.f_3 = { 0f, 0f, 160f };
			}
			else
			{
				Var0 = { -73.904f, -821.6204f, 284f };
				Var0.f_3 = { 0f, 0f, -110f };
			}
			break;
		
		case 114:
			if (!bParam1)
			{
				Var0 = { -79.9861f, -818.425f, 221f };
				Var0.f_3 = { 0f, 0f, -20f };
			}
			else
			{
				Var0 = { -73.904f, -821.6204f, 284f };
				Var0.f_3 = { 0f, 0f, -110f };
			}
			break;
	}
	return Var0;
}

int func_558(var uParam0, int iParam1, int iParam2)
{
	struct<3> Var0;
	
	switch (iParam2)
	{
		case default:
			return 0;
		
		case 1:
		case 2:
		case 3:
		case 4:
		case 5:
		case 6:
		case 7:
		case 34:
		case 35:
		case 36:
		case 37:
		case 38:
		case 39:
		case 40:
		case 41:
		case 42:
		case 43:
			func_553(iParam2, iParam1, &Var0, -1, 0);
			*uParam0 = { Var0 };
			return 1;
		
		case 61:
		case 62:
		case 63:
		case 64:
		case 65:
			func_553(iParam2, iParam1, &Var0, 61, 0);
			*uParam0 = { Var0 };
			return 1;
		
		case 83:
		case 84:
		case 85:
			func_553(iParam2, iParam1, &Var0, 83, 0);
			*uParam0 = { Var0 };
			return 1;
		
		case 77:
		case 78:
		case joaat("mpsv_lp0_31"):
		case 80:
		case 81:
		case 82:
			func_553(iParam2, iParam1, &Var0, 77, 0);
			*uParam0 = { Var0 };
			return 1;
		
		case 73:
		case 74:
		case 75:
		case 76:
			func_553(iParam2, iParam1, &Var0, 73, 0);
			*uParam0 = { Var0 };
			return 1;
		
		case 87:
		case 88:
		case 89:
		case 90:
			func_553(iParam2, iParam1, &Var0, 88, 0);
			*uParam0 = { Var0 };
			return 1;
			return 0;
	}
}

void func_559()
{
	int iVar0;
	
	if (func_563())
	{
		return;
	}
	func_562(4, -1);
	if (func_561())
	{
		return;
	}
	iVar0 = 0;
	while (iVar0 < 32)
	{
		if (unk_0x166E920FB00B2DBB(unk_0xF1110FE23C67293A(iVar0)))
		{
			if (unk_0xF1110FE23C67293A(iVar0) == unk_0x460153A63B9477BC())
			{
				if (!func_560(48))
				{
					unk_0x8ECAF24A8B08CF58(unk_0xF1110FE23C67293A(iVar0), 0);
				}
			}
			else
			{
				unk_0x8ECAF24A8B08CF58(unk_0xF1110FE23C67293A(iVar0), 0);
			}
		}
		iVar0++;
	}
}

bool func_560(int iParam0)
{
	int iVar0;
	int iVar1;
	
	iVar0 = (iParam0 / 32);
	iVar1 = (iParam0 % 32);
	if (iVar0 >= 8)
	{
		return 0;
	}
	return unk_0xCE990E643CD9D0E5(Global_2452907.f_1.f_2813[iVar0], iVar1);
}

bool func_561()
{
	return unk_0xCE990E643CD9D0E5(Global_2452015.f_42.f_4, 0);
}

void func_562(int iParam0, int iParam1)
{
	unk_0xBE20AB8238688965(&(Global_1377236.f_1047), iParam0);
	switch (iParam0)
	{
		case 5:
			if (iParam1 > -1)
			{
				Global_1377236.f_170[iParam1] = 1;
			}
			break;
	}
}

int func_563()
{
	if (func_565() != 2)
	{
		return 0;
	}
	if (!func_561() && !((Global_1672145 || Global_1312809) && func_564()))
	{
		return 0;
	}
	return 1;
}

var func_564()
{
	return Global_2452015.f_42.f_57;
}

int func_565()
{
	return Global_2452015.f_42.f_40;
}

void func_566(var uParam0, var uParam1, float fParam2)
{
	if (func_567(Global_4456448.f_82708))
	{
		if (unk_0x0D77CDCF403AEBD2((unk_0x8940B46482DD0A7D(0) - 1.333333f)) < 0.1f)
		{
			*uParam0 = { 1561.161f, 384.824f, -48.6215f };
			*uParam1 = { -89.3034f, -0.1435f, 180f };
			*fParam2 = 69f;
		}
		else
		{
			*uParam0 = { 1561.16f, 384.8204f, -48.9135f };
			*uParam1 = { -89.5003f, -0.204f, 180f };
			*fParam2 = 69f;
		}
	}
}

int func_567(int iParam0)
{
	int iVar0;
	
	iVar0 = 0;
	while (iVar0 < 11)
	{
		if (Global_262145.f_29188[iVar0] == iParam0)
		{
			return 1;
		}
		iVar0++;
	}
	return 0;
}

int func_568()
{
	if (func_567(Global_4456448.f_82708))
	{
		return 1;
	}
	return 0;
}

float func_569(int iParam0)
{
	return Global_4014600[iParam0 /*45*/].f_7;
}

Vector3 func_570(int iParam0)
{
	return Global_4014600[iParam0 /*45*/].f_4;
}

var func_571(int iParam0)
{
	return Global_2421089[iParam0 /*3*/];
}

int func_572(int iParam0)
{
	int iVar0;
	
	iVar0 = 0;
	while (iVar0 < 21)
	{
		if (Global_262145.f_5799[iVar0] == iParam0)
		{
			return 1;
		}
		iVar0++;
	}
	return 0;
}

int func_573(int iParam0)
{
	int iVar0;
	
	if (iParam0 == 0)
	{
		return 0;
	}
	iVar0 = 0;
	while (iVar0 < 16)
	{
		if (Global_262145.f_5009[iVar0] == iParam0)
		{
			return 1;
		}
		iVar0++;
	}
	return 0;
}

bool func_574(int iParam0)
{
	return unk_0xCE990E643CD9D0E5(Global_2426097[iParam0 /*443*/].f_200, 24);
}

int func_575(int iParam0)
{
	int iVar0;
	int iVar1;
	
	if (iParam0 != 0)
	{
		iVar1 = 0;
		while (iVar1 < 14)
		{
			if (Global_262145.f_6447[iVar1] == 251)
			{
				iVar0 = 0;
				while (iVar0 < Global_262145.f_5923[iVar1])
				{
					if (iParam0 == Global_262145.f_5033[iVar1 /*51*/][iVar0])
					{
						return 1;
					}
					iVar0++;
				}
			}
			iVar1++;
		}
	}
	return 0;
}

bool func_576()
{
	return unk_0xCE990E643CD9D0E5(Global_2452015.f_3, 4);
}

int func_577(int iParam0)
{
	if (iParam0 != func_42())
	{
		if (func_203(iParam0, 1, 1))
		{
			if (Global_2426097[iParam0 /*443*/].f_314.f_6 != -1)
			{
				return func_272(Global_2426097[iParam0 /*443*/].f_314.f_6) == 6;
			}
		}
	}
	return 0;
}

int func_578(int iParam0)
{
	if (iParam0 != func_42())
	{
		if (func_203(iParam0, 1, 1))
		{
			if (Global_2426097[iParam0 /*443*/].f_314.f_6 != -1 && Global_2426097[iParam0 /*443*/].f_314.f_9 != func_42())
			{
				return func_272(Global_2426097[iParam0 /*443*/].f_314.f_6) == 5;
			}
		}
	}
	return 0;
}

var func_579()
{
	if (unk_0xAE06B9E39EBDE049(func_580()))
	{
		return func_580();
	}
	return func_259();
}

var func_580()
{
	return Global_2359302.f_3;
}

int func_581(struct<3> Param0)
{
	if ((Param0.f_0 == 0f && Param0.f_1 == 0f) && Param0.f_2 == 0f)
	{
		return 1;
	}
	return 0;
}

float func_582(int iParam0, int iParam1)
{
	switch (iParam0)
	{
		case 11:
			return 40.5914f;
		
		case 122:
			return 50f;
		
		case 13:
			switch (iParam1)
			{
				case 0:
					return 50f;
				
				case 1:
					return 50f;
				
				default:
			}
			break;
		
		case 12:
			switch (iParam1)
			{
				case 0:
					return 50f;
				
				case 1:
					return 40.4324f;
				
				case 2:
					return 42.2709f;
				
				case 3:
					return 43.6444f;
				
				case 4:
					return 37.3862f;
				
				case 5:
					return 38.6952f;
				
				case 6:
					return 40.9091f;
				
				default:
			}
			break;
		
		case 15:
			switch (iParam1)
			{
				case 0:
					return 40.5238f;
				
				case 1:
					return 47.4321f;
				
				case 2:
					return 38.6571f;
				
				case 3:
					return 44.0641f;
				
				case 4:
					return 39.2991f;
				
				case 5:
					return 31.7484f;
				
				case 6:
					return 35.6342f;
				
				case 7:
					return 36.0072f;
				
				case 8:
					return 42.5782f;
				
				case 9:
					return 37.4336f;
				
				default:
			}
			break;
		
		case 14:
			return 50f;
	}
	return 50f;
}

Vector3 func_583(int iParam0, int iParam1)
{
	switch (iParam0)
	{
		case 11:
			return 1.3773f, 0f, -88.827f;
		
		case 122:
			return -18.1605f, 2.6167f, 113.0513f;
		
		case 13:
			switch (iParam1)
			{
				case 0:
					return 2.3039f, 0f, -19.6903f;
				
				case 1:
					return -5.9341f, 0f, -151.6385f;
				
				default:
			}
			break;
		
		case 12:
			switch (iParam1)
			{
				case 0:
					return -3.627f, 0f, 34.6935f;
				
				case 1:
					return -57.9221f, 0f, 58.3042f;
				
				case 2:
					return -15.4498f, 0f, 161.1361f;
				
				case 3:
					return -1.8386f, 0f, -135.0223f;
				
				case 4:
					return 0.5233f, 0f, -85.7377f;
				
				case 5:
					return -4.2364f, 0f, 173.6634f;
				
				case 6:
					return -3.74f, 0f, -30.7752f;
				
				default:
			}
			break;
		
		case 15:
			switch (iParam1)
			{
				case 0:
					return 4.1441f, 0f, 15.6909f;
				
				case 1:
					return 5.4815f, 0f, -127.6695f;
				
				case 2:
					return -4.1889f, 0f, -15.0654f;
				
				case 3:
					return 16.4433f, 0f, -60.3988f;
				
				case 4:
					return 7.8225f, 0f, 39.2409f;
				
				case 5:
					return 2.0405f, 0f, -122.3459f;
				
				case 6:
					return 7.3382f, 0f, 157.6388f;
				
				case 7:
					return 6.481f, 0f, 172.9874f;
				
				case 8:
					return 14.0573f, 0f, 5.0622f;
				
				case 9:
					return -10.1444f, 0f, -131.4038f;
				
				default:
			}
			break;
		
		case 14:
			switch (iParam1)
			{
				case 0:
					return 8.8196f, 0f, 57.5677f;
				
				default:
			}
			break;
	}
	return 0f, 0f, 0f;
}

Vector3 func_584(int iParam0, int iParam1)
{
	switch (iParam0)
	{
		case 11:
			return -1297.855f, 84.6478f, 54.6519f;
		
		case 122:
			return -855.7661f, -2645.376f, 100.8438f;
		
		case 13:
			switch (iParam1)
			{
				case 0:
					return 13.4678f, -1097.781f, 30.192f;
				
				case 1:
					return 816.6221f, -2160.659f, 30.3139f;
				
				default:
			}
			break;
		
		case 12:
			switch (iParam1)
			{
				case 0:
					return -1172.111f, -1641.549f, 5.7494f;
				
				case 1:
					return 498.4758f, -226.4275f, 71.0569f;
				
				case 2:
					return -1217.632f, 364.4556f, 86.527f;
				
				case 3:
					return -2884.754f, 32.9823f, 15.1123f;
				
				case 4:
					return -948.1036f, -1260.249f, 8.228f;
				
				case 5:
					return -1614.216f, 302.0945f, 66.0158f;
				
				case 6:
					return -1380.721f, -107.9713f, 50.6998f;
				
				default:
			}
			break;
		
		case 15:
			switch (iParam1)
			{
				case 0:
					return 903.8763f, -192.214f, 74.6401f;
				
				case 1:
					return 1419.517f, -2216.034f, 61.8467f;
				
				case 2:
					return -83.9713f, -1331.635f, 29.8383f;
				
				case 3:
					return 364.6455f, -2441.897f, 6.2607f;
				
				case 4:
					return 1237.207f, 2715.999f, 37.6424f;
				
				case 5:
					return 1019.153f, 2680.427f, 41.2254f;
				
				case 6:
					return 269.5203f, 2617.41f, 44.7054f;
				
				case 7:
					return -194.2518f, 6295.688f, 31.0991f;
				
				case 8:
					return 1682.759f, 4954.088f, 42.6891f;
				
				case 9:
					return 2325.447f, 3145.91f, 58.4284f;
				
				default:
			}
			break;
		
		case 14:
			switch (iParam1)
			{
				case 0:
					return 1993.697f, 3049.667f, 47.7241f;
				
				default:
			}
			break;
	}
	return 0f, 0f, 0f;
}

int func_585(int iParam0)
{
	switch (iParam0)
	{
		case 15:
		case 14:
		case 11:
		case 13:
		case 12:
		case 122:
			return 1;
		
		default:
	}
	return 0;
}

void func_586(int iParam0)
{
	if (!unk_0x6F45B807E1EF196F(func_587(0)))
	{
		unk_0x0A794A8277A63161(func_587(iParam0), 0, 1);
	}
}

char* func_587(int iParam0)
{
	if (func_596(Global_4456448.f_82708))
	{
		return "MenuMGIslandHeistIn";
	}
	if (iParam0 || func_560(134))
	{
		return "MenuMGHeistIn";
	}
	if (func_595())
	{
		return "MenuMGTournamentIn";
	}
	if (func_589(Global_4456448.f_129348))
	{
		return "MenuMGRemixIn";
	}
	if (func_588(Global_4456448.f_129348) && unk_0xCE990E643CD9D0E5(Global_4456448.f_130685.f_2808, 2))
	{
		return "MenuSurvivalAlienIn";
	}
	return "MenuMGSelectionIn";
}

bool func_588(int iParam0)
{
	return iParam0 == 998;
}

int func_589(int iParam0)
{
	if ((((func_594(iParam0) || func_593(iParam0)) || func_592(iParam0)) || func_591(iParam0)) || func_590(iParam0))
	{
		return 1;
	}
	return 0;
}

bool func_590(int iParam0)
{
	return iParam0 == 64;
}

bool func_591(int iParam0)
{
	return iParam0 == 65;
}

bool func_592(int iParam0)
{
	return iParam0 == 69;
}

bool func_593(int iParam0)
{
	return iParam0 == 67;
}

bool func_594(int iParam0)
{
	return iParam0 == 68;
}

bool func_595()
{
	return unk_0xCE990E643CD9D0E5(Global_2452907.f_1.f_2809, 12);
}

int func_596(int iParam0)
{
	int iVar0;
	
	if (iParam0 == 0)
	{
		return 0;
	}
	iVar0 = 0;
	while (iVar0 < 11)
	{
		if (Global_262145.f_29188[iVar0] == iParam0)
		{
			return 1;
		}
		iVar0++;
	}
	return 0;
}

bool func_597()
{
	return unk_0xCE990E643CD9D0E5(Global_1590682[unk_0x460153A63B9477BC() /*883*/].f_142, 2);
}

void func_598()
{
	Global_2452015.f_715 = 1;
}

void func_599()
{
	unk_0xD2459066EA58CE43(&Global_2452015, 7);
}

void func_600()
{
	unk_0xBE20AB8238688965(&Global_2452015, 5);
	func_601();
}

void func_601()
{
	unk_0xBE20AB8238688965(&Global_2452015, 8);
}

void func_602()
{
	unk_0xD2459066EA58CE43(&(Global_2452015.f_2), 14);
}

void func_603(var uParam0)
{
	Global_2452015.f_41 = uParam0;
}

void func_604(char* sParam0)
{
	StringCopy(&(Global_2452015.f_857), sParam0, 24);
	if (func_605())
	{
		StringCopy(&(Global_959568.f_42), sParam0, 24);
	}
}

bool func_605()
{
	return unk_0xCE990E643CD9D0E5(Global_2452907.f_1.f_2809, 5);
}

void func_606(struct<13> Param0)
{
	if (func_607(Param0))
	{
		Global_2452907.f_6482 = { Param0 };
	}
}

bool func_607(var uParam0, var uParam1, var uParam2, var uParam3, var uParam4, var uParam5, var uParam6, var uParam7, var uParam8, var uParam9, var uParam10, var uParam11, var uParam12)
{
	return unk_0xF6B8932480699645(&uParam0, 13);
}

void func_608()
{
	unk_0xBE20AB8238688965(&(Global_1627436[unk_0x460153A63B9477BC() /*90*/].f_29), 12);
}

void func_609(int iParam0)
{
	Global_2452015.f_8 = iParam0;
}

var func_610()
{
	return Global_2452015.f_754;
}

void func_611()
{
	unk_0xBE20AB8238688965(&(Global_2452015.f_3), 10);
}

void func_612(int iParam0)
{
	Global_968397 = iParam0;
}

void func_613()
{
	unk_0xBE20AB8238688965(&(Global_2452015.f_3), 2);
}

void func_614()
{
	Global_2460706.f_1 = 0;
}

void func_615(int iParam0, int iParam1)
{
	int iVar0;
	
	iVar0 = 0;
	if (unk_0x871819940BD2E434(unk_0x04E90C1EB827C162(iParam0), "MPBitset"))
	{
		iVar0 = unk_0x8C0800114C2864BE(unk_0x04E90C1EB827C162(iParam0), "MPBitset");
	}
	unk_0xBE20AB8238688965(&iVar0, iParam1);
	unk_0x00AF6A2AAFB41525(unk_0x04E90C1EB827C162(iParam0), "MPBitset", iVar0);
}

void func_616(struct<3> Param0)
{
	Global_2462465 = { Param0 };
}

Vector3 func_617(int iParam0)
{
	struct<3> Var0;
	
	if (unk_0x8F678487EEBD8CE4(Global_2416085[iParam0]))
	{
		Var0 = { func_673(Global_2416085[iParam0]) };
	}
	else if (unk_0x8F678487EEBD8CE4(Global_2416085.f_33[iParam0]) && !iParam0 == Global_2440277)
	{
		Var0 = { func_673(Global_2416085.f_33[iParam0]) };
	}
	if (unk_0x652D2EEEF1D3E62C(Var0) > 0f)
	{
		return Var0;
	}
	return func_618(iParam0);
}

Vector3 func_618(int iParam0)
{
	int iVar0;
	struct<3> Var1;
	int iVar4;
	
	if (Global_1590682[iParam0 /*883*/].f_274.f_28 > 0)
	{
		Var1 = { func_671(func_672(Global_1590682[unk_0x460153A63B9477BC() /*883*/].f_274.f_28)) };
		if (unk_0x652D2EEEF1D3E62C(Var1) > 0f)
		{
			return Var1;
		}
	}
	else
	{
		func_668(func_670(iParam0), &iVar0);
		if (iVar0 > 0)
		{
			Var1 = { func_671(func_672(iVar0)) };
			if (unk_0x652D2EEEF1D3E62C(Var1) > 0f)
			{
				return Var1;
			}
		}
	}
	if (func_667(iParam0))
	{
		Var1 = { func_623(iParam0) };
		if (unk_0x652D2EEEF1D3E62C(Var1) > 0f)
		{
			return Var1;
		}
	}
	if (func_622(iParam0))
	{
		iVar4 = func_621(iParam0);
		if (iVar4 > -1)
		{
			Var1 = { Global_1678289.f_496[iVar4 /*3*/] };
			if (unk_0x652D2EEEF1D3E62C(Var1) > 0f)
			{
				return Var1;
			}
		}
	}
	if (func_620(iParam0))
	{
		Var1 = { func_619(iParam0) };
		if (unk_0x652D2EEEF1D3E62C(Var1) > 0f)
		{
			return Var1;
		}
	}
	if (Global_2426097[iParam0 /*443*/].f_71.f_11)
	{
		if (unk_0x652D2EEEF1D3E62C(Global_2426097[iParam0 /*443*/].f_71.f_12) > 0f)
		{
			return Global_2426097[iParam0 /*443*/].f_71.f_12;
		}
	}
	return func_670(iParam0);
}

Vector3 func_619(int iParam0)
{
	return Global_2426097[iParam0 /*443*/].f_71.f_7;
}

int func_620(int iParam0)
{
	if (Global_2426097[iParam0 /*443*/].f_71.f_6 != -1)
	{
		return 1;
	}
	return 0;
}

int func_621(int iParam0)
{
	if (iParam0 != func_42())
	{
		return Global_2426097[iParam0 /*443*/].f_314.f_14;
	}
	return -1;
}

int func_622(int iParam0)
{
	bool bVar0;
	
	if (iParam0 != func_42() && (func_203(iParam0, 1, 1) || Global_2405074.f_2673))
	{
		bVar0 = unk_0xCE990E643CD9D0E5(Global_2426097[iParam0 /*443*/].f_314, 1);
		return bVar0;
	}
	return 0;
}

Vector3 func_623(int iParam0)
{
	int iVar0;
	int iVar1;
	int iVar2;
	int iVar3;
	int iVar4;
	int iVar5;
	int iVar6;
	int iVar7;
	int iVar8;
	int iVar9;
	int iVar10;
	int iVar11;
	int iVar12;
	int iVar13;
	int iVar14;
	
	if (iParam0 == func_42())
	{
	}
	if (func_578(iParam0))
	{
		iVar0 = func_666(iParam0);
		if (iVar0 != func_42())
		{
			if (!func_665(iVar0))
			{
				if (unk_0xAE06B9E39EBDE049(Global_2440277.f_673[iVar0 /*3*/][1]))
				{
					return unk_0x3E4D3CCC220BDFB1(Global_2440277.f_673[iVar0 /*3*/][1], 0);
				}
				else
				{
					return Global_2426097[func_666(iParam0) /*443*/].f_314.f_10;
				}
			}
			else
			{
				iVar1 = func_662(iVar0);
				if (!iVar1 == -1)
				{
					return Global_1678289.f_496[iVar1 /*3*/];
				}
			}
		}
	}
	else if (func_577(iParam0))
	{
		if (unk_0xAE06B9E39EBDE049(Global_2540612.f_307))
		{
			return unk_0x3E4D3CCC220BDFB1(Global_2540612.f_307, 0);
		}
	}
	else if (func_661(iParam0) && !func_660(iParam0))
	{
		iVar2 = Global_2426097[iParam0 /*443*/].f_314.f_9;
		if (iVar2 != func_42())
		{
			iVar3 = func_662(iVar2);
			if (!iVar3 == -1)
			{
				return Global_1678289.f_496[iVar3 /*3*/];
			}
		}
	}
	else if (func_659(iParam0) && !func_658(iParam0))
	{
		if (func_667(iParam0) && func_657())
		{
			return Global_1678289.f_496[Global_2426097[iParam0 /*443*/].f_314.f_6 /*3*/];
		}
		iVar4 = Global_2426097[iParam0 /*443*/].f_314.f_9;
		if (iVar4 != func_42())
		{
			if (func_656(iVar4))
			{
				iVar5 = func_652(iVar4);
				if (iVar5 != -1)
				{
					return Global_1678289.f_496[iVar5 /*3*/];
				}
			}
		}
	}
	else if (func_651(iParam0))
	{
		iVar6 = func_650(iParam0);
		if (iVar6 != func_42())
		{
			if (!func_649(iVar6))
			{
				if (unk_0xAE06B9E39EBDE049(Global_2440277.f_770[iVar6]))
				{
					return unk_0x3E4D3CCC220BDFB1(Global_2440277.f_770[iVar6], 0);
				}
				else
				{
					return Global_2426097[func_650(iParam0) /*443*/].f_314.f_17;
				}
			}
			else
			{
				iVar7 = func_652(iVar6);
				if (!iVar7 == -1)
				{
					return Global_1678289.f_496[iVar7 /*3*/];
				}
			}
		}
	}
	else if (func_648(iParam0))
	{
		if (unk_0xAE06B9E39EBDE049(Global_2540612.f_309))
		{
			return unk_0x3E4D3CCC220BDFB1(Global_2540612.f_309, 0);
		}
	}
	else if (func_647(iParam0) && !func_646(iParam0))
	{
		iVar8 = Global_2426097[iParam0 /*443*/].f_314.f_9;
		if (iVar8 != func_42())
		{
			if (func_142(iVar8))
			{
				iVar9 = func_642(iVar8);
				if (iVar9 != -1)
				{
					return func_641(iVar9);
				}
			}
		}
	}
	else if (func_640(iParam0) && !func_639(iParam0))
	{
		iVar10 = Global_2426097[iParam0 /*443*/].f_314.f_9;
		if (iVar10 != func_42())
		{
			if (func_638(iVar10))
			{
				iVar11 = func_634(iVar10);
				if (iVar11 != -1)
				{
					return Global_1678289.f_496[iVar11 /*3*/];
				}
			}
		}
	}
	else if (func_633(iParam0, 0))
	{
		iVar12 = func_632(iParam0);
		if (iVar12 != func_42())
		{
			if (!func_631(iVar12))
			{
				if (unk_0xAE06B9E39EBDE049(Global_2440277.f_803[iVar12]))
				{
					return unk_0x3E4D3CCC220BDFB1(Global_2440277.f_803[iVar12], 0);
				}
				else
				{
					return Global_1630317[func_632(iParam0) /*595*/].f_580;
				}
			}
			else
			{
				iVar13 = func_634(iVar12);
				if (!iVar13 == -1)
				{
					return Global_1678289.f_496[iVar13 /*3*/];
				}
			}
		}
	}
	else if (func_630(iParam0))
	{
		return -366.7f, -1909.6f, 20f;
	}
	else if (func_629(iParam0))
	{
		iVar14 = func_271(iParam0);
		if (iVar14 != func_42())
		{
			if (unk_0xAE06B9E39EBDE049(Global_2440277.f_836[iVar14]))
			{
				return unk_0x3E4D3CCC220BDFB1(Global_2440277.f_836[iVar14], 0);
			}
			else
			{
				return Global_1649358[iVar14 /*93*/].f_50;
			}
		}
		else
		{
			return 0f, 0f, 0f;
		}
	}
	if (func_647(iParam0))
	{
		return func_641(Global_2426097[iParam0 /*443*/].f_314.f_6);
	}
	if (func_624(iParam0))
	{
		return 965.8165f, 42.25042f, 122.1267f;
	}
	return Global_1678289.f_496[Global_2426097[iParam0 /*443*/].f_314.f_6 /*3*/];
}

int func_624(int iParam0)
{
	if (((func_628(iParam0) || func_627(iParam0)) || func_626(iParam0)) || func_625(iParam0))
	{
		return 1;
	}
	return 0;
}

int func_625(int iParam0)
{
	if (iParam0 != func_42())
	{
		if (func_203(iParam0, 1, 1))
		{
			if (Global_2426097[iParam0 /*443*/].f_314.f_6 != -1)
			{
				return func_272(Global_2426097[iParam0 /*443*/].f_314.f_6) == 19;
			}
		}
	}
	return 0;
}

int func_626(int iParam0)
{
	if (iParam0 != func_42())
	{
		if (func_203(iParam0, 1, 1))
		{
			if (Global_2426097[iParam0 /*443*/].f_314.f_6 != -1)
			{
				return func_272(Global_2426097[iParam0 /*443*/].f_314.f_6) == 15;
			}
		}
	}
	return 0;
}

int func_627(int iParam0)
{
	if (iParam0 != func_42())
	{
		if (func_203(iParam0, 1, 1))
		{
			if (Global_2426097[iParam0 /*443*/].f_314.f_6 != -1)
			{
				return func_272(Global_2426097[iParam0 /*443*/].f_314.f_6) == 16;
			}
		}
	}
	return 0;
}

int func_628(int iParam0)
{
	if (iParam0 != func_42())
	{
		if (func_203(iParam0, 1, 1))
		{
			if (Global_2426097[iParam0 /*443*/].f_314.f_6 != -1)
			{
				return func_272(Global_2426097[iParam0 /*443*/].f_314.f_6) == 14;
			}
		}
	}
	return 0;
}

int func_629(int iParam0)
{
	if (iParam0 != func_42())
	{
		if (func_203(iParam0, 1, 1))
		{
			if (Global_2426097[iParam0 /*443*/].f_314.f_6 != -1)
			{
				return func_272(Global_2426097[iParam0 /*443*/].f_314.f_6) == 20;
			}
		}
		else if (((Global_2426097[iParam0 /*443*/].f_314.f_6 != -1 && Global_1312896) && iParam0 == unk_0x460153A63B9477BC()) && func_203(iParam0, 1, 0))
		{
			return func_272(Global_2426097[iParam0 /*443*/].f_314.f_6) == 20;
		}
	}
	return 0;
}

int func_630(int iParam0)
{
	if (iParam0 != func_42())
	{
		if (func_203(iParam0, 1, 1))
		{
			if (Global_2426097[iParam0 /*443*/].f_314.f_6 != -1)
			{
				return func_272(Global_2426097[iParam0 /*443*/].f_314.f_6) == 13;
			}
		}
	}
	return 0;
}

int func_631(int iParam0)
{
	if (iParam0 != func_42())
	{
		return unk_0xCE990E643CD9D0E5(Global_2426097[iParam0 /*443*/].f_314.f_3, 4);
	}
	return 0;
}

int func_632(int iParam0)
{
	if (iParam0 == func_42())
	{
		return iParam0;
	}
	return Global_2426097[iParam0 /*443*/].f_314.f_9;
}

int func_633(int iParam0, bool bParam1)
{
	var uVar0;
	
	if (bParam1)
	{
		if (unk_0x47DBF174A0CB9D55(unk_0x9B0761B4C3EB8BC7(), 0))
		{
			uVar0 = unk_0xA5D5B1042E8F47BD(unk_0x9B0761B4C3EB8BC7(), 0);
			if (unk_0x36FE6D3220816ADA(uVar0) == joaat("terbyte"))
			{
				return 1;
			}
		}
	}
	if (iParam0 != func_42())
	{
		if (func_203(iParam0, 1, 1))
		{
			if (Global_2426097[iParam0 /*443*/].f_314.f_6 != -1 && Global_2426097[iParam0 /*443*/].f_314.f_9 != func_42())
			{
				return func_272(Global_2426097[iParam0 /*443*/].f_314.f_6) == 12;
			}
		}
	}
	return 0;
}

int func_634(int iParam0)
{
	int iVar0;
	
	if (iParam0 != func_42())
	{
		iVar0 = func_637(iParam0);
		if (iVar0 != 0)
		{
			return func_635(iVar0);
		}
	}
	return -1;
}

int func_635(int iParam0)
{
	int iVar0;
	
	iVar0 = 0;
	while (iVar0 < 148)
	{
		if (func_272(iVar0) == 11)
		{
			if (func_636(iVar0) == iParam0)
			{
				return iVar0;
			}
		}
		iVar0++;
	}
	return -1;
}

int func_636(int iParam0)
{
	switch (iParam0)
	{
		case 102:
			return 1;
		
		case 103:
			return 2;
		
		case 104:
			return 3;
		
		case 105:
			return 4;
		
		case 106:
			return 5;
		
		case 107:
			return 6;
		
		case 108:
			return 7;
		
		case 109:
			return 8;
		
		case 110:
			return 9;
		
		case 111:
			return 10;
		
		default:
	}
	return 0;
}

int func_637(int iParam0)
{
	if (iParam0 != func_42())
	{
		return Global_1590682[iParam0 /*883*/].f_274.f_281;
	}
	return 0;
}

int func_638(int iParam0)
{
	if (iParam0 != func_42())
	{
		if (Global_1590682[iParam0 /*883*/].f_274.f_281 != 0)
		{
			return 1;
		}
	}
	return 0;
}

int func_639(int iParam0)
{
	if (iParam0 != func_42())
	{
		if (func_640(iParam0) && Global_2426097[iParam0 /*443*/].f_314.f_9 == iParam0)
		{
			return 1;
		}
	}
	return 0;
}

int func_640(int iParam0)
{
	if (iParam0 != func_42())
	{
		if (func_203(iParam0, 1, 1))
		{
			if (Global_2426097[iParam0 /*443*/].f_314.f_6 != -1)
			{
				return func_272(Global_2426097[iParam0 /*443*/].f_314.f_6) == 11;
			}
		}
	}
	return 0;
}

Vector3 func_641(int iParam0)
{
	switch (iParam0)
	{
		case 102:
			return 750.5f, -1322.3f, 26.2802f;
		
		case 103:
			return 331.9f, -974.9f, 30f;
		
		case 104:
			return -146f, -1270f, 28.3088f;
		
		case 105:
			return -17.7f, 225.7f, 106.7566f;
		
		case 106:
			return 894.4f, -2106.4f, 29.4768f;
		
		case 107:
			return -668f, -2431.5f, 12.9444f;
		
		case 108:
			return 213.4f, -3166.6f, 4.7903f;
		
		case 109:
			return 366.6f, 237.6f, 104.9f;
		
		case 110:
			return -1275.3f, -666.8f, 25.6332f;
		
		case 111:
			return -1188.8f, -1156.9f, 4.6582f;
		
		default:
	}
	return Global_1678289.f_496[iParam0 /*3*/];
}

int func_642(int iParam0)
{
	int iVar0;
	
	if (iParam0 != func_42())
	{
		iVar0 = func_645(iParam0);
		if (iVar0 != 0)
		{
			return func_643(iVar0);
		}
	}
	return -1;
}

int func_643(int iParam0)
{
	int iVar0;
	
	iVar0 = 0;
	while (iVar0 < 148)
	{
		if (func_272(iVar0) == 11)
		{
			if (func_644(iVar0) == iParam0)
			{
				return iVar0;
			}
		}
		iVar0++;
	}
	return -1;
}

int func_644(int iParam0)
{
	switch (iParam0)
	{
		case 102:
			return 1;
		
		case 103:
			return 2;
		
		case 104:
			return 3;
		
		case 105:
			return 4;
		
		case 106:
			return 5;
		
		case 107:
			return 6;
		
		case 108:
			return 7;
		
		case 109:
			return 8;
		
		case 110:
			return 9;
		
		case 111:
			return 10;
		
		default:
	}
	return 0;
}

int func_645(int iParam0)
{
	if (iParam0 != func_42())
	{
		return Global_1590682[iParam0 /*883*/].f_274.f_322;
	}
	return 0;
}

int func_646(int iParam0)
{
	if (iParam0 != func_42())
	{
		if (func_647(iParam0) && Global_2426097[iParam0 /*443*/].f_314.f_9 == iParam0)
		{
			return 1;
		}
	}
	return 0;
}

int func_647(int iParam0)
{
	if (iParam0 != func_42())
	{
		if (func_203(iParam0, 1, 1))
		{
			if (Global_2426097[iParam0 /*443*/].f_314.f_6 != -1)
			{
				return func_272(Global_2426097[iParam0 /*443*/].f_314.f_6) == 11;
			}
		}
	}
	return 0;
}

int func_648(int iParam0)
{
	if (iParam0 != func_42())
	{
		if (func_203(iParam0, 1, 1))
		{
			if (Global_2426097[iParam0 /*443*/].f_314.f_6 != -1)
			{
				return func_272(Global_2426097[iParam0 /*443*/].f_314.f_6) == 10;
			}
		}
	}
	return 0;
}

int func_649(int iParam0)
{
	if (iParam0 != func_42())
	{
		return unk_0xCE990E643CD9D0E5(Global_2426097[iParam0 /*443*/].f_314.f_2, 6);
	}
	return 0;
}

int func_650(int iParam0)
{
	if (iParam0 == func_42())
	{
		return iParam0;
	}
	return Global_2426097[iParam0 /*443*/].f_314.f_9;
}

int func_651(int iParam0)
{
	if (iParam0 != func_42())
	{
		if (func_203(iParam0, 1, 1))
		{
			if (Global_2426097[iParam0 /*443*/].f_314.f_6 != -1 && Global_2426097[iParam0 /*443*/].f_314.f_9 != func_42())
			{
				return func_272(Global_2426097[iParam0 /*443*/].f_314.f_6) == 8;
			}
		}
	}
	return 0;
}

int func_652(int iParam0)
{
	int iVar0;
	
	if (iParam0 == func_42())
	{
		return -1;
	}
	iVar0 = func_655(iParam0);
	if (!iVar0 == 0)
	{
		return func_653(iVar0);
	}
	return -1;
}

int func_653(int iParam0)
{
	int iVar0;
	
	iVar0 = 0;
	while (iVar0 < 148)
	{
		if (func_272(iVar0) == 9)
		{
			if (func_654(iVar0) == iParam0)
			{
				return iVar0;
			}
		}
		iVar0++;
	}
	return -1;
}

int func_654(int iParam0)
{
	switch (iParam0)
	{
		case 89:
			return 1;
			break;
		
		case 90:
			return 2;
			break;
		
		case 91:
			return 3;
			break;
		
		case 92:
			return 4;
			break;
		
		case 93:
			return 5;
			break;
		
		case 94:
			return 6;
			break;
		
		case 95:
			return 7;
			break;
		
		case 96:
			return 8;
			break;
		
		case 97:
			return 9;
			break;
	}
	return 0;
}

int func_655(int iParam0)
{
	if (iParam0 == func_42())
	{
		return 0;
	}
	return Global_1590682[iParam0 /*883*/].f_274.f_271;
}

int func_656(int iParam0)
{
	if (iParam0 != func_42())
	{
		return Global_1590682[iParam0 /*883*/].f_274.f_271 != 0;
	}
	return 0;
}

int func_657()
{
	if (unk_0xCE990E643CD9D0E5(Global_1678289.f_2, 13) || Global_1678289.f_3057)
	{
		return 1;
	}
	return 0;
}

int func_658(int iParam0)
{
	if (iParam0 == func_42())
	{
		return 0;
	}
	if (func_659(iParam0) && Global_2426097[iParam0 /*443*/].f_314.f_9 == iParam0)
	{
		return 1;
	}
	return 0;
}

int func_659(int iParam0)
{
	if (iParam0 != func_42())
	{
		if (func_203(iParam0, 1, 1))
		{
			if (Global_2426097[iParam0 /*443*/].f_314.f_6 != -1)
			{
				return func_272(Global_2426097[iParam0 /*443*/].f_314.f_6) == 9;
			}
		}
	}
	return 0;
}

int func_660(int iParam0)
{
	if (iParam0 == func_42())
	{
		return 0;
	}
	if (func_661(iParam0) && Global_2426097[iParam0 /*443*/].f_314.f_9 == iParam0)
	{
		return 1;
	}
	return 0;
}

int func_661(int iParam0)
{
	if (iParam0 != func_42())
	{
		if (func_203(iParam0, 1, 1))
		{
			if (Global_2426097[iParam0 /*443*/].f_314.f_6 != -1)
			{
				return func_272(Global_2426097[iParam0 /*443*/].f_314.f_6) == 4;
			}
		}
	}
	return 0;
}

int func_662(int iParam0)
{
	int iVar0;
	
	if (iParam0 == func_42())
	{
		return -1;
	}
	iVar0 = func_151(iParam0);
	if (!iVar0 == 0)
	{
		return func_663(iVar0);
	}
	return -1;
}

int func_663(int iParam0)
{
	int iVar0;
	
	iVar0 = 0;
	while (iVar0 < 148)
	{
		if (func_664(iVar0) == iParam0)
		{
			return iVar0;
		}
		iVar0++;
	}
	return -1;
}

int func_664(int iParam0)
{
	switch (iParam0)
	{
		case 22:
			return 1;
			break;
		
		case 23:
			return 2;
			break;
		
		case 24:
			return 3;
			break;
		
		case 25:
			return 4;
			break;
		
		case 26:
			return 5;
			break;
		
		case 27:
			return 6;
			break;
		
		case 28:
			return 7;
			break;
		
		case 29:
			return 8;
			break;
		
		case 30:
			return 9;
			break;
		
		case 31:
			return 10;
			break;
		
		case 32:
			return 11;
			break;
		
		case 33:
			return 12;
			break;
		
		case 34:
			return 13;
			break;
		
		case 35:
			return 14;
			break;
		
		case 36:
			return 15;
			break;
		
		case 37:
			return 16;
			break;
		
		case 38:
			return 17;
			break;
		
		case 39:
			return 18;
			break;
		
		case 40:
			return 19;
			break;
		
		case 41:
			return 20;
			break;
		
		case 70:
			return 21;
			break;
		
		case 71:
			return 22;
			break;
		
		case 72:
			return 23;
			break;
		
		case 73:
			return 24;
			break;
		
		case 74:
			return 25;
			break;
		
		case 75:
			return 26;
			break;
		
		case 76:
			return 27;
			break;
		
		case 77:
			return 28;
			break;
		
		case 78:
			return 29;
			break;
		
		case joaat("mpsv_lp0_31"):
			return 30;
			break;
		
		case 80:
			return 31;
			break;
	}
	return 0;
}

int func_665(int iParam0)
{
	if (iParam0 != func_42())
	{
		return unk_0xCE990E643CD9D0E5(Global_2426097[iParam0 /*443*/].f_314, 6);
	}
	return 0;
}

int func_666(int iParam0)
{
	if (iParam0 == func_42())
	{
		return iParam0;
	}
	return Global_2426097[iParam0 /*443*/].f_314.f_9;
}

int func_667(int iParam0)
{
	if (iParam0 != func_42())
	{
		if (func_203(iParam0, 1, 1))
		{
			return Global_2426097[iParam0 /*443*/].f_314.f_6 != -1;
		}
		else if ((Global_1312896 && iParam0 == unk_0x460153A63B9477BC()) && func_203(iParam0, 1, 0))
		{
			return Global_2426097[iParam0 /*443*/].f_314.f_6 != -1;
		}
	}
	return 0;
}

void func_668(struct<3> Param0, var uParam3)
{
	int iVar0;
	int iVar1;
	
	iVar0 = 1;
	iVar1 = 7;
	while (iVar0 <= iVar1)
	{
		if (func_669(Param0, iVar0, 0))
		{
			*uParam3 = iVar0;
			return;
		}
		iVar0++;
	}
	iVar0 = 34;
	iVar1 = 43;
	while (iVar0 <= iVar1)
	{
		if (func_669(Param0, iVar0, 0))
		{
			*uParam3 = iVar0;
			return;
		}
		iVar0++;
	}
	iVar0 = 61;
	iVar1 = 65;
	while (iVar0 <= iVar1)
	{
		if (func_669(Param0, iVar0, 0))
		{
			*uParam3 = iVar0;
			return;
		}
		iVar0++;
	}
	iVar0 = 8;
	if (func_669(Param0, iVar0, 0))
	{
		if (Global_1590682[unk_0x460153A63B9477BC() /*883*/].f_274.f_28 > 0)
		{
			*uParam3 = Global_1590682[unk_0x460153A63B9477BC() /*883*/].f_274.f_28;
		}
		else if (Global_2404660.f_1 > 0)
		{
			*uParam3 = Global_2404660.f_1;
		}
		else
		{
			*uParam3 = iVar0;
		}
		return;
	}
	iVar0 = 17;
	if (func_669(Param0, iVar0, 0))
	{
		if (Global_1590682[unk_0x460153A63B9477BC() /*883*/].f_274.f_28 > 0)
		{
			*uParam3 = Global_1590682[unk_0x460153A63B9477BC() /*883*/].f_274.f_28;
		}
		else if (Global_2404660.f_1 > 0)
		{
			*uParam3 = Global_2404660.f_1;
		}
		else
		{
			*uParam3 = iVar0;
		}
		return;
	}
}

int func_669(struct<3> Param0, int iParam3, float fParam4)
{
	int iVar0;
	struct<3> Var1[3];
	struct<3> Var11[3];
	int iVar21;
	
	if (iParam3 != 999)
	{
		iVar21 = 0;
		while (iVar21 < 3)
		{
			if (Global_1049924[iParam3 /*1951*/].f_146.f_57[iVar21 /*8*/].f_2 > Global_1049924[iParam3 /*1951*/].f_146.f_57[iVar21 /*8*/].f_3.f_2)
			{
				Var1[iVar21 /*3*/] = { Global_1049924[iParam3 /*1951*/].f_146.f_57[iVar21 /*8*/] };
				Var11[iVar21 /*3*/] = { Global_1049924[iParam3 /*1951*/].f_146.f_57[iVar21 /*8*/].f_3 };
			}
			else
			{
				Var1[iVar21 /*3*/] = { Global_1049924[iParam3 /*1951*/].f_146.f_57[iVar21 /*8*/].f_3 };
				Var11[iVar21 /*3*/] = { Global_1049924[iParam3 /*1951*/].f_146.f_57[iVar21 /*8*/] };
			}
			Var1[iVar21 /*3*/].f_2 = (Var1[iVar21 /*3*/].f_2 + fParam4);
			Var11[iVar21 /*3*/].f_2 = (Var11[iVar21 /*3*/].f_2 - fParam4);
			iVar21++;
		}
		if (((Global_1049924[iParam3 /*1951*/].f_146.f_57[0 /*8*/].f_6 != 0f && unk_0x3BC3A4E4A1827EEE(Param0, Var11[0 /*3*/], Var1[0 /*3*/], Global_1049924[iParam3 /*1951*/].f_146.f_57[0 /*8*/].f_6, 0, 1)) || (Global_1049924[iParam3 /*1951*/].f_146.f_57[1 /*8*/].f_6 != 0f && unk_0x3BC3A4E4A1827EEE(Param0, Var11[1 /*3*/], Var1[1 /*3*/], Global_1049924[iParam3 /*1951*/].f_146.f_57[1 /*8*/].f_6, 0, 1))) || (Global_1049924[iParam3 /*1951*/].f_146.f_57[2 /*8*/].f_6 != 0f && unk_0x3BC3A4E4A1827EEE(Param0, Var11[2 /*3*/], Var1[2 /*3*/], Global_1049924[iParam3 /*1951*/].f_146.f_57[2 /*8*/].f_6, 0, 1)))
		{
			return 1;
		}
	}
	else
	{
		iVar0 = 1;
		while (iVar0 <= 7)
		{
			if (func_669(Param0, iVar0, fParam4))
			{
				return 1;
			}
			iVar0++;
		}
		iVar0 = 34;
		while (iVar0 <= 43)
		{
			if (func_669(Param0, iVar0, fParam4))
			{
				return 1;
			}
			iVar0++;
		}
		iVar0 = 61;
		while (iVar0 <= 65)
		{
			if (func_669(Param0, iVar0, fParam4))
			{
				return 1;
			}
			iVar0++;
		}
		iVar0 = 73;
		while (iVar0 <= 76)
		{
			if (func_669(Param0, iVar0, fParam4))
			{
				return 1;
			}
			iVar0++;
		}
		iVar0 = 77;
		while (iVar0 <= 82)
		{
			if (func_669(Param0, iVar0, fParam4))
			{
				return 1;
			}
			iVar0++;
		}
		iVar0 = 83;
		while (iVar0 <= 85)
		{
			if (func_669(Param0, iVar0, fParam4))
			{
				return 1;
			}
			iVar0++;
		}
		iVar0 = 87;
		while (iVar0 <= 90)
		{
			if (func_669(Param0, iVar0, fParam4))
			{
				return 1;
			}
			iVar0++;
		}
		iVar0 = 91;
		while (iVar0 <= 102)
		{
			if (func_669(Param0, iVar0, fParam4))
			{
				return 1;
			}
			iVar0++;
		}
		iVar0 = 103;
		while (iVar0 <= 113)
		{
			if (func_669(Param0, iVar0, fParam4))
			{
				return 1;
			}
			iVar0++;
		}
		if (func_669(Param0, 8, fParam4))
		{
			return 1;
		}
		if (func_669(Param0, 17, fParam4))
		{
			return 1;
		}
	}
	return 0;
}

Vector3 func_670(int iParam0)
{
	return unk_0x3E4D3CCC220BDFB1(unk_0x1FA7B77001D60F9D(iParam0), 0);
}

Vector3 func_671(int iParam0)
{
	struct<3> Var0;
	
	switch (iParam0)
	{
		case 1:
			Var0 = { -773.4775f, 310.6321f, 84.6981f };
			break;
		
		case 2:
			Var0 = { -252.5713f, -949.9199f, 30.221f };
			break;
		
		case 3:
			Var0 = { -1443.094f, -544.7684f, 33.7424f };
			break;
		
		case 4:
			Var0 = { -913.85f, -455.1392f, 38.5999f };
			break;
		
		case 5:
			Var0 = { -47.3145f, -585.9766f, 36.9593f };
			break;
		
		case 6:
			Var0 = { -932.7474f, -383.9246f, 37.9613f };
			break;
		
		case 7:
			Var0 = { -619.1315f, 37.8841f, 42.5883f };
			break;
		
		case 8:
			Var0 = { 284.9614f, -159.9891f, 63.6221f };
			break;
		
		case 9:
			Var0 = { 2.8889f, 35.7762f, 70.5349f };
			break;
		
		case 10:
			Var0 = { 9.74f, 84.6906f, 77.3975f };
			break;
		
		case 11:
			Var0 = { -512.1465f, 111.2223f, 62.351f };
			break;
		
		case 12:
			Var0 = { -197.3405f, 88.1053f, 68.7422f };
			break;
		
		case 13:
			Var0 = { -628.3212f, 165.8364f, 60.1651f };
			break;
		
		case 14:
			Var0 = { -973.3757f, -1429.425f, 6.6791f };
			break;
		
		case 15:
			Var0 = { -829.1362f, -855.0104f, 18.6297f };
			break;
		
		case 16:
			Var0 = { -757.5739f, -755.5591f, 25.5697f };
			break;
		
		case 17:
			Var0 = { -45.1289f, -57.0925f, 62.2531f };
			break;
		
		case 18:
			Var0 = { -206.7293f, 184.142f, 79.3279f };
			break;
		
		case 19:
			Var0 = { -811.7045f, -984.1961f, 13.1538f };
			break;
		
		case 20:
			Var0 = { -664.0032f, -853.6744f, 23.4325f };
			break;
		
		case 21:
			Var0 = { -1534.025f, -324.5247f, 46.5237f };
			break;
		
		case 22:
			Var0 = { -1561.381f, -412.1974f, 41.389f };
			break;
		
		case 23:
			Var0 = { -1608.851f, -429.184f, 39.439f };
			break;
		
		case 24:
			Var0 = { 964.3583f, -1034.199f, 39.8303f };
			break;
		
		case 25:
			Var0 = { 894.2868f, -885.5679f, 26.1212f };
			break;
		
		case 26:
			Var0 = { 821.1741f, -924.1658f, 25.1998f };
			break;
		
		case 27:
			Var0 = { 759.7933f, -759.8209f, 25.759f };
			break;
		
		case 28:
			Var0 = { 844.7289f, -1164.1f, 24.2681f };
			break;
		
		case 29:
			Var0 = { 526.2521f, -1604.613f, 28.2625f };
			break;
		
		case 30:
			Var0 = { 572.0462f, -1570.736f, 27.4904f };
			break;
		
		case 31:
			Var0 = { 722.2852f, -1190.617f, 23.282f };
			break;
		
		case 32:
			Var0 = { 497.6212f, -1494.084f, 28.2893f };
			break;
		
		case 33:
			Var0 = { 480.3127f, -1549.937f, 28.2828f };
			break;
		
		case 34:
			Var0 = { -68.702f, 6426.148f, 30.439f };
			break;
		
		case 35:
			Var0 = { -247.4374f, 6240.294f, 30.4892f };
			break;
		
		case 36:
			Var0 = { 2554.165f, 4668.059f, 33.0233f };
			break;
		
		case 37:
			Var0 = { 2461.22f, 1589.255f, 32.0443f };
			break;
		
		case 38:
			Var0 = { -2203.335f, 4244.427f, 47.3305f };
			break;
		
		case 39:
			Var0 = { 218.0665f, 2601.817f, 44.7668f };
			break;
		
		case 40:
			Var0 = { 186.1719f, 2786.343f, 45.0144f };
			break;
		
		case 41:
			Var0 = { 642.0746f, 2791.729f, 40.9795f };
			break;
		
		case 42:
			Var0 = { -1130.938f, 2701.133f, 17.8004f };
			break;
		
		case 43:
			Var0 = { -10.944f, -1646.76f, 28.3125f };
			break;
		
		case 44:
			Var0 = { 1024.263f, -2398.404f, 29.1261f };
			break;
		
		case 45:
			Var0 = { 870.8577f, -2232.323f, 29.5508f };
			break;
		
		case 46:
			Var0 = { -663.8541f, -2380.389f, 12.9446f };
			break;
		
		case 47:
			Var0 = { -1088.616f, -2235.098f, 12.2182f };
			break;
		
		case 48:
			Var0 = { -342.5126f, -1468.675f, 29.6107f };
			break;
		
		case 49:
			Var0 = { -1241.54f, -259.8841f, 37.9445f };
			break;
		
		case 50:
			Var0 = { 899.8448f, -147.528f, 75.5674f };
			break;
		
		case 51:
			Var0 = { -1405.411f, 526.8572f, 122.8361f };
			break;
		
		case 52:
			Var0 = { 1341.552f, -1578.037f, 53.4443f };
			break;
		
		case 53:
			Var0 = { -105.7029f, 6528.539f, 29.1719f };
			break;
		
		case 54:
			Var0 = { -302.3985f, 6327.434f, 31.8918f };
			break;
		
		case 55:
			Var0 = { -15.258f, 6557.378f, 32.2454f };
			break;
		
		case 56:
			Var0 = { 1899.673f, 3773.178f, 31.8829f };
			break;
		
		case 57:
			Var0 = { 1662.121f, 4776.317f, 41.0075f };
			break;
		
		case 58:
			Var0 = { -178.2278f, 490.886f, 134.0466f };
			break;
		
		case 59:
			Var0 = { 339.5743f, 439.7083f, 145.5896f };
			break;
		
		case 60:
			Var0 = { -764.6163f, 618.3144f, 137.5536f };
			break;
		
		case 61:
			Var0 = { -679.5461f, 592.5162f, 139.693f };
			break;
		
		case 62:
			Var0 = { 124.4571f, 551.8877f, 181.658f };
			break;
		
		case 63:
			Var0 = { -563.7349f, 689.099f, 151.6596f };
			break;
		
		case 64:
			Var0 = { -743.0927f, 590.0371f, 140.9221f };
			break;
		
		case 65:
			Var0 = { -861.252f, 684.8923f, 146.2643f };
			break;
		
		case 66:
			Var0 = { -1292.456f, 440.9454f, 93.7572f };
			break;
		
		case 67:
			Var0 = { 369.5891f, 417.4813f, 136.8344f };
			break;
		
		case 68:
			Var0 = { -1581.501f, -558.2578f, 33.9528f };
			break;
		
		case 69:
			Var0 = { -1379.546f, -499.1783f, 32.1574f };
			break;
		
		case 70:
			Var0 = { -117.5296f, -605.7157f, 35.2857f };
			break;
		
		case 71:
			Var0 = { -67.0943f, -802.4415f, 43.2273f };
			break;
		
		case 72:
			Var0 = { 254.1892f, -1809.183f, 26.1805f };
			break;
		
		case 73:
			Var0 = { -1472.278f, -920.0677f, 8.9683f };
			break;
		
		case 74:
			Var0 = { 38.9478f, -1026.629f, 28.6354f };
			break;
		
		case 75:
			Var0 = { 46.903f, 2789.825f, 57.1124f };
			break;
		
		case 76:
			Var0 = { -342.3246f, 6065.316f, 30.4895f };
			break;
		
		case 77:
			Var0 = { 1737.878f, 3709.088f, 33.1348f };
			break;
		
		case 78:
			Var0 = { 939.7161f, -1459.204f, 30.467f };
			break;
		
		case joaat("mpsv_lp0_31"):
			Var0 = { 189.7624f, 309.7488f, 104.4714f };
			break;
		
		case 80:
			Var0 = { -21.9593f, -191.3618f, 51.5599f };
			break;
		
		case 81:
			Var0 = { 2472.22f, 4110.493f, 36.9629f };
			break;
		
		case 82:
			Var0 = { -39.3286f, 6420.603f, 30.7017f };
			break;
		
		case 83:
			Var0 = { -1134.762f, -1568.848f, 3.4077f };
			break;
	}
	return Var0;
}

int func_672(int iParam0)
{
	switch (iParam0)
	{
		case 1:
		case 2:
		case 3:
		case 4:
		case 61:
		case 83:
		case 84:
		case 85:
			return 1;
			break;
		
		case 5:
		case 6:
			return 2;
			break;
		
		case 7:
		case 34:
		case 62:
			return 3;
			break;
		
		case 35:
		case 36:
		case 37:
			return 4;
			break;
		
		case 38:
		case 39:
		case 65:
			return 5;
			break;
		
		case 40:
		case 41:
		case 63:
			return 6;
			break;
		
		case 42:
		case 43:
		case 64:
			return 7;
			break;
		
		case 8:
			return 8;
			break;
		
		case 9:
			return 9;
			break;
		
		case 10:
			return 10;
			break;
		
		case 11:
			return 11;
			break;
		
		case 12:
			return 12;
			break;
		
		case 13:
			return 13;
			break;
		
		case 14:
			return 14;
			break;
		
		case 15:
			return 15;
			break;
		
		case 16:
			return 16;
			break;
		
		case 17:
			return 17;
			break;
		
		case 18:
			return 18;
			break;
		
		case 19:
			return 19;
			break;
		
		case 20:
			return 20;
			break;
		
		case 21:
			return 21;
			break;
		
		case 22:
			return 22;
			break;
		
		case 23:
			return 23;
			break;
		
		case 24:
			return 24;
			break;
		
		case 25:
			return 25;
			break;
		
		case 26:
			return 26;
			break;
		
		case 27:
			return 27;
			break;
		
		case 28:
			return 28;
			break;
		
		case 29:
			return 29;
			break;
		
		case 30:
			return 30;
			break;
		
		case 31:
			return 31;
			break;
		
		case 32:
			return 32;
			break;
		
		case 33:
			return 33;
			break;
		
		case 44:
			return 34;
			break;
		
		case 45:
			return 35;
			break;
		
		case 46:
			return 36;
			break;
		
		case 47:
			return 37;
			break;
		
		case 48:
			return 38;
			break;
		
		case 49:
			return 39;
			break;
		
		case 50:
			return 40;
			break;
		
		case 51:
			return 41;
			break;
		
		case 52:
			return 42;
			break;
		
		case 53:
			return 43;
			break;
		
		case 54:
			return 44;
			break;
		
		case 55:
			return 45;
			break;
		
		case 56:
			return 46;
			break;
		
		case 57:
			return 47;
			break;
		
		case 58:
			return 48;
			break;
		
		case 59:
			return 49;
			break;
		
		case 60:
			return 50;
			break;
		
		case 66:
			return 51;
			break;
		
		case 67:
			return 52;
			break;
		
		case 68:
			return 53;
			break;
		
		case 69:
			return 54;
			break;
		
		case 70:
			return 55;
			break;
		
		case 71:
			return 56;
			break;
		
		case 72:
			return 57;
			break;
		
		case 73:
			return 58;
			break;
		
		case 74:
			return 59;
			break;
		
		case 75:
			return 60;
			break;
		
		case 76:
			return 61;
			break;
		
		case 77:
			return 62;
			break;
		
		case 78:
			return 63;
			break;
		
		case joaat("mpsv_lp0_31"):
			return 64;
			break;
		
		case 80:
			return 65;
			break;
		
		case 81:
			return 66;
			break;
		
		case 82:
			return 67;
			break;
		
		case 87:
		case 103:
		case 104:
		case 105:
			return 68;
			break;
		
		case 88:
		case 106:
		case 107:
		case 108:
			return 69;
			break;
		
		case 89:
		case 109:
		case 110:
		case 111:
			return 70;
			break;
		
		case 90:
		case 112:
		case 113:
		case 114:
			return 71;
			break;
		
		case 91:
			return 72;
			break;
		
		case 92:
			return 73;
			break;
		
		case 93:
			return 74;
			break;
		
		case 94:
			return 75;
			break;
		
		case 95:
			return 76;
			break;
		
		case 96:
			return 77;
			break;
		
		case 97:
			return 78;
			break;
		
		case 98:
			return 79;
			break;
		
		case 99:
			return 80;
			break;
		
		case 100:
			return 81;
			break;
		
		case 101:
			return 82;
			break;
		
		case 102:
			return 83;
			break;
	}
	return 0;
}

Vector3 func_673(var uParam0)
{
	var uVar0;
	
	switch (unk_0x04FA724B68B59AC3(uParam0))
	{
		case 1:
		case 2:
		case 3:
			uVar0 = unk_0x255EBCF840E6A9BB(uParam0);
			if (unk_0xAE06B9E39EBDE049(uVar0))
			{
				return unk_0x3E4D3CCC220BDFB1(iVar0, 0);
			}
			break;
	}
	return unk_0x41A1ED9BB4ED4772(uParam0);
}

int func_674(int iParam0)
{
	int iVar0;
	int iVar1;
	
	iVar0 = func_675(iParam0);
	iVar1 = 0;
	while (iVar1 < 1218)
	{
		if (Global_794709.f_107189[iVar1 /*13*/].f_1 == iVar0)
		{
			return iVar1;
		}
		iVar1++;
	}
	return -1;
}

int func_675(int iParam0)
{
	if (iParam0 == -1 || iParam0 == 21)
	{
		return 0;
	}
	return Global_262145.f_5799[iParam0];
}

int func_676(int iParam0)
{
	switch (iParam0)
	{
		case 1:
			return 0;
		
		case 2:
			return 6;
		
		case 3:
			return 13;
		
		default:
	}
	return -1;
}

void func_677()
{
	Global_2452015.f_754 = 1;
}

var func_678()
{
	return Global_2452015.f_733;
}

bool func_679()
{
	return unk_0xCE990E643CD9D0E5(Global_2452015.f_2, 11);
}

void func_680(var uParam0)
{
	switch (uParam0->f_1497.f_724)
	{
		case 0:
			func_916(uParam0);
			break;
		
		case 1:
			if (!func_915(&(uParam0->f_1497.f_724)))
			{
				func_914(uParam0);
				func_913(&(uParam0->f_1497.f_724));
			}
			func_916(uParam0);
			if (!func_912(&(uParam0->f_1497.f_724)))
			{
				func_911(uParam0);
				func_910(&(uParam0->f_1497.f_724));
			}
			break;
		
		case 2:
			if (!func_915(&(uParam0->f_1497.f_724)))
			{
				func_907(uParam0);
				func_913(&(uParam0->f_1497.f_724));
			}
			func_901(uParam0);
			if (!func_912(&(uParam0->f_1497.f_724)))
			{
				func_900(uParam0);
				func_910(&(uParam0->f_1497.f_724));
			}
			break;
		
		case 3:
			if (!func_915(&(uParam0->f_1497.f_724)))
			{
				func_899(uParam0);
				func_913(&(uParam0->f_1497.f_724));
			}
			func_897(uParam0);
			if (!func_912(&(uParam0->f_1497.f_724)))
			{
				func_896(uParam0);
				func_910(&(uParam0->f_1497.f_724));
			}
			break;
		
		case 4:
			if (!func_915(&(uParam0->f_1497.f_724)))
			{
				func_892(uParam0);
				func_913(&(uParam0->f_1497.f_724));
			}
			func_702(uParam0);
			if (!func_912(&(uParam0->f_1497.f_724)))
			{
				func_700(uParam0);
				func_910(&(uParam0->f_1497.f_724));
			}
			break;
		
		case 5:
			if (!func_915(&(uParam0->f_1497.f_724)))
			{
				func_682(uParam0);
				func_913(&(uParam0->f_1497.f_724));
			}
			func_681(uParam0);
			break;
	}
}

void func_681(var uParam0)
{
}

void func_682(var uParam0)
{
	func_698(uParam0);
	func_685(0);
	func_684();
	func_683(&(uParam0->f_1497));
}

void func_683(var uParam0)
{
	unk_0xD8D551845E3EA730(1);
	unk_0xCA1F114B668562E5(uParam0->f_700);
	unk_0x8DDE075026DB7EFD(0.501f, 0.5f, 1f, 1f, 255, 255, 255, 255, 0);
	unk_0xCA1F114B668562E5(unk_0xC48A590C39E17456());
	unk_0xD8D551845E3EA730(0);
}

void func_684()
{
	struct<141> Var0;
	
	Var0 = 4;
	Var0.f_63.f_2 = 5;
	Var0.f_77 = 2;
	Var0.f_82 = 4;
	Var0.f_82.f_5 = 4;
	Var0.f_82.f_10 = 5;
	Var0.f_82.f_16 = 4;
	Var0.f_82.f_21 = -1;
	Var0.f_82.f_22 = -1;
	Var0.f_109 = 4;
	Global_1666211.f_3033 = { Var0 };
}

void func_685(bool bParam0)
{
	func_697();
	func_696();
	func_690(bParam0);
	func_689(&(Global_1697306.f_2529));
	func_688(&(Global_1697306.f_2593));
	func_687(&(Global_1697306.f_2604));
	func_686(&(Global_1697306.f_2607));
	func_684();
	Global_1697306.f_4355 = -1;
	Global_1697306.f_4356 = func_42();
	Global_1697306.f_4357 = 0;
	Global_1697306.f_4358 = 0;
}

void func_686(var uParam0)
{
	struct<1566> Var0;
	
	Var0 = 7;
	Var0.f_1.f_52 = 8;
	Var0.f_1.f_193 = 4;
	Var0.f_1.f_218.f_52 = 8;
	Var0.f_1.f_218.f_193 = 4;
	Var0.f_1.f_218.f_218.f_52 = 8;
	Var0.f_1.f_218.f_218.f_193 = 4;
	Var0.f_1.f_218.f_218.f_218.f_52 = 8;
	Var0.f_1.f_218.f_218.f_218.f_193 = 4;
	Var0.f_1.f_218.f_218.f_218.f_218.f_52 = 8;
	Var0.f_1.f_218.f_218.f_218.f_218.f_193 = 4;
	Var0.f_1.f_218.f_218.f_218.f_218.f_218.f_52 = 8;
	Var0.f_1.f_218.f_218.f_218.f_218.f_218.f_193 = 4;
	Var0.f_1.f_218.f_218.f_218.f_218.f_218.f_218.f_52 = 8;
	Var0.f_1.f_218.f_218.f_218.f_218.f_218.f_218.f_193 = 4;
	*uParam0 = { Var0 };
}

void func_687(var uParam0)
{
	struct<3> Var0;
	
	*uParam0 = { Var0 };
}

void func_688(var uParam0)
{
	struct<11> Var0;
	
	*uParam0 = { Var0 };
}

void func_689(var uParam0)
{
	struct<64> Var0;
	
	Var0.f_63 = 500;
	*uParam0 = { Var0 };
}

void func_690(bool bParam0)
{
	func_695(&(Global_1697306.f_1497));
	func_694(&(Global_1697306.f_1497.f_724));
	func_693(&(Global_1697306.f_1497.f_730));
	func_692(&(Global_1697306.f_1497.f_736));
	Global_1697306.f_1497.f_1015 = 0;
	Global_1697306.f_1497.f_1027 = -1;
	Global_1697306.f_1497.f_1028 = 0;
	Global_1697306.f_1497.f_1029 = 0;
	Global_1697306.f_1497.f_1017 = -1;
	Global_1697306.f_1497.f_1018 = -1;
	Global_1697306.f_1497.f_1019 = -1;
	Global_1697306.f_1497.f_1020 = 0;
	Global_1697306.f_1497.f_1021 = 5;
	if (!bParam0)
	{
		func_691();
	}
	func_684();
}

void func_691()
{
	struct<47> Var0;
	
	Var0.f_2 = 4;
	Var0.f_7 = 4;
	Var0.f_12 = 5;
	Var0.f_46 = 4;
	Var0.f_46.f_5 = 4;
	Var0.f_46.f_10 = 4;
	Global_1701669[unk_0x1EF8FCAB83E4BE53() /*68*/].f_2 = { Var0.f_2 };
	Global_1701669[unk_0x1EF8FCAB83E4BE53() /*68*/].f_7 = { Var0.f_7 };
	Global_1701669[unk_0x1EF8FCAB83E4BE53() /*68*/].f_12 = { Var0.f_12 };
	Global_1701669[unk_0x1EF8FCAB83E4BE53() /*68*/] = Var0.f_0;
}

void func_692(var uParam0)
{
	struct<279> Var0;
	
	Var0 = 7;
	Var0.f_1 = 4;
	Var0.f_1.f_5 = 4;
	Var0.f_1.f_5.f_5 = 4;
	Var0.f_1.f_5.f_5.f_5 = 4;
	Var0.f_1.f_5.f_5.f_5.f_5 = 4;
	Var0.f_1.f_5.f_5.f_5.f_5.f_5 = 4;
	Var0.f_1.f_5.f_5.f_5.f_5.f_5.f_5 = 4;
	Var0.f_36 = 4;
	Var0.f_87 = 4;
	Var0.f_92 = 5;
	Var0.f_98 = 4;
	Var0.f_103 = 4;
	Var0.f_108 = 4;
	Var0.f_108.f_1 = 17;
	Var0.f_108.f_1.f_18 = 17;
	Var0.f_108.f_1.f_18.f_18 = 17;
	Var0.f_108.f_1.f_18.f_18.f_18 = 17;
	Var0.f_181 = 4;
	Var0.f_181.f_1 = 17;
	Var0.f_181.f_1.f_18 = 17;
	Var0.f_181.f_1.f_18.f_18 = 17;
	Var0.f_181.f_1.f_18.f_18.f_18 = 17;
	Var0.f_254 = 4;
	Var0.f_259 = 4;
	Var0.f_264 = 5;
	Var0.f_270 = 4;
	Var0.f_278 = -1;
	*uParam0 = { Var0 };
}

void func_693(var uParam0)
{
	struct<6> Var0;
	
	*uParam0 = { Var0 };
}

void func_694(var uParam0)
{
	struct<6> Var0;
	
	*uParam0 = { Var0 };
}

void func_695(var uParam0)
{
	struct<724> Var0;
	
	Var0.f_1 = 12;
	Var0.f_699 = 1065353216;
	Var0.f_701 = 1;
	Var0.f_702 = 1;
	*uParam0 = { Var0 };
}

void func_696()
{
	func_695(&(Global_1697306.f_744));
	func_694(&(Global_1697306.f_744.f_724));
	func_693(&(Global_1697306.f_744.f_730));
	Global_1697306.f_744.f_736 = 0;
	Global_1697306.f_744.f_748 = -1;
	Global_1697306.f_744.f_749 = 0;
	Global_1697306.f_744.f_750 = 0;
	Global_1697306.f_744.f_738 = 0;
	Global_1697306.f_744.f_739 = 0;
	Global_1697306.f_744.f_740 = 0;
	Global_1697306.f_744.f_741 = 0;
	Global_1697306.f_744.f_742 = 0;
	Global_1697306.f_744.f_743 = 0;
	Global_1697306.f_744.f_744 = 0;
	Global_1697306.f_744.f_745 = 0;
	Global_1697306.f_744.f_746 = 0;
	func_684();
}

void func_697()
{
	func_695(&Global_1697306);
	func_694(&(Global_1697306.f_724));
	func_693(&(Global_1697306.f_730));
	Global_1697306.f_736 = 0;
	func_684();
}

void func_698(var uParam0)
{
	unk_0x9128BA5F998AA176("DLC_MPHEIST/HEIST_PLANNING_BOARD");
	unk_0x2E352DDBBF674246(&(uParam0->f_1497.f_709));
	func_699(&(uParam0->f_2529.f_1), 1, 1);
	if ((!unk_0xFC559366953F62B3() && uParam0->f_4360) && !func_202(unk_0x460153A63B9477BC()))
	{
		func_463(unk_0x460153A63B9477BC(), 1, 0, 0);
		unk_0x6A8F948698B360B4(unk_0x9B0761B4C3EB8BC7(), false);
		uParam0->f_4360 = 0;
	}
	func_690(uParam0->f_4356 == unk_0x460153A63B9477BC());
}

void func_699(var uParam0, int iParam1, bool bParam2)
{
	if (unk_0x40397A9A17EEC1C5(*uParam0))
	{
		if (bParam2)
		{
			unk_0x3ED9330214992278(0, 0, 3000, 1, iParam1, 0);
		}
		if (unk_0xC516FEEB950FFD58(*uParam0))
		{
			unk_0x856549C07003344B(*uParam0, 0);
		}
		unk_0xB8B0F7C8C1548C5B(*uParam0, iParam1);
	}
	if (uParam0->f_23)
	{
		unk_0x49EAC3B3479D60FE();
		uParam0->f_23 = 0;
	}
	uParam0->f_1 = { 0f, 0f, 0f };
	uParam0->f_4 = { 0f, 0f, 0f };
	uParam0->f_7 = 0f;
	uParam0->f_20 = 0;
	uParam0->f_21 = 0;
	uParam0->f_22 = 0;
	uParam0->f_8 = { 0f, 0f, 0f };
	uParam0->f_11 = { 0f, 0f, 0f };
	uParam0->f_14 = { 0f, 0f, 0f };
	uParam0->f_17 = 0f;
	uParam0->f_18 = 0f;
}

void func_700(var uParam0)
{
	if (!func_17(uParam0, 1))
	{
		func_701(&(uParam0->f_2593), 0, 0);
		uParam0->f_1497.f_703 = 0;
	}
}

void func_701(var uParam0, int iParam1, bool bParam2)
{
	if (bParam2)
	{
		*uParam0 = iParam1;
		uParam0->f_1 = iParam1;
	}
	else
	{
		*uParam0 = iParam1;
		uParam0->f_1 = iParam1;
		uParam0->f_2 = iParam1;
		uParam0->f_3 = iParam1;
		uParam0->f_4 = 0;
		uParam0->f_6 = *uParam0;
	}
}

void func_702(var uParam0)
{
	func_891();
	func_887(uParam0);
	func_879(uParam0);
	func_878(uParam0);
	func_862(uParam0);
	func_859(uParam0);
	func_817(uParam0);
	func_726(uParam0);
	func_714(uParam0);
	func_713();
	func_712(uParam0, 2);
	if (uParam0->f_4356 == unk_0x460153A63B9477BC())
	{
		func_707(uParam0);
	}
	else
	{
		func_705(uParam0);
	}
	if (!uParam0->f_1497.f_724.f_5)
	{
		if (!func_17(uParam0, 1))
		{
			if (uParam0->f_4360)
			{
				func_463(unk_0x460153A63B9477BC(), 1, 0, 0);
				if (!unk_0xFC559366953F62B3())
				{
					unk_0x6A8F948698B360B4(unk_0x9B0761B4C3EB8BC7(), false);
				}
				uParam0->f_4360 = 0;
			}
			func_704(uParam0);
		}
		func_510(-1713398555, 12, 0);
		func_703(&(uParam0->f_1497.f_724), 2);
	}
}

void func_703(var uParam0, int iParam1)
{
	int iVar0;
	
	uParam0->f_1 = *uParam0;
	unk_0xBE20AB8238688965(&(uParam0->f_4), uParam0->f_1);
	unk_0xD2459066EA58CE43(&(uParam0->f_3), uParam0->f_1);
	iVar0 = *uParam0;
	iVar0 = (iVar0 - 1);
	*uParam0 = iVar0;
	if (*uParam0 == 0)
	{
		*uParam0 = 1;
	}
}

void func_704(var uParam0)
{
	func_699(&(uParam0->f_2529.f_1), 0, 1);
	unk_0xD50CC9B03AAD56FF(0, 0);
}

void func_705(var uParam0)
{
	if (uParam0->f_4356 == unk_0x460153A63B9477BC() || uParam0->f_4356 == -1)
	{
		return;
	}
	if (Global_1701669[uParam0->f_4356 /*68*/].f_7[0] == 1)
	{
		func_706(&(uParam0->f_1497.f_724), 2);
	}
}

void func_706(var uParam0, int iParam1)
{
	int iVar0;
	
	uParam0->f_1 = *uParam0;
	unk_0xBE20AB8238688965(&(uParam0->f_4), uParam0->f_1);
	unk_0xD2459066EA58CE43(&(uParam0->f_3), uParam0->f_1);
	iVar0 = *uParam0;
	iVar0++;
	*uParam0 = iVar0;
	if (*uParam0 == 7)
	{
		*uParam0 = 5;
	}
}

void func_707(var uParam0)
{
	bool bVar0;
	bool bVar1;
	int iVar2;
	
	if (uParam0->f_4356 != unk_0x460153A63B9477BC())
	{
		return;
	}
	if (!unk_0xFC559366953F62B3())
	{
		return;
	}
	bVar0 = true;
	bVar1 = true;
	if (uParam0->f_1497.f_736.f_103[0] == 1)
	{
		return;
	}
	uParam0->f_1497.f_736.f_278 = -1;
	if (uParam0->f_1497.f_724.f_2 == 1)
	{
		bVar0 = false;
		uParam0->f_1497.f_736.f_278 = 0;
	}
	if (bVar0)
	{
		if (uParam0->f_1497.f_736.f_92[4] > 0)
		{
			bVar0 = false;
			uParam0->f_1497.f_736.f_278 = 1;
		}
	}
	if (bVar0)
	{
		if (uParam0->f_1497.f_1024 == -1)
		{
			bVar0 = false;
			uParam0->f_1497.f_736.f_278 = 3;
		}
	}
	if (bVar0)
	{
		if (uParam0->f_1497.f_1022 == -1)
		{
			bVar0 = false;
			uParam0->f_1497.f_736.f_278 = 4;
		}
	}
	if (bVar0)
	{
		if (uParam0->f_1497.f_1023 == -1)
		{
			bVar0 = false;
			uParam0->f_1497.f_736.f_278 = 5;
		}
	}
	if (bVar0)
	{
		if (func_113(uParam0->f_4356) == 2)
		{
			if (uParam0->f_1497.f_1025 == 0)
			{
				bVar0 = false;
				uParam0->f_1497.f_736.f_278 = 6;
			}
		}
	}
	if (bVar0)
	{
		if (uParam0->f_1497.f_736.f_277 == 0)
		{
			func_711(uParam0->f_1497.f_709, 12, 1);
			func_710(uParam0->f_1497.f_709, 12, 1);
			func_190(&(uParam0->f_1497), 1);
			unk_0xBE20AB8238688965(&(Global_1701669[unk_0x1EF8FCAB83E4BE53() /*68*/].f_1), 5);
			unk_0xBE20AB8238688965(&(uParam0->f_1497.f_736.f_78), 5);
			Global_1701669[unk_0x1EF8FCAB83E4BE53() /*68*/]++;
			uParam0->f_1497.f_736.f_277 = 1;
		}
	}
	if (bVar0)
	{
		iVar2 = 1;
		while (iVar2 <= 3)
		{
			if (uParam0->f_1497.f_736.f_87[iVar2] == -1)
			{
			}
			else if (!func_709(uParam0, unk_0xF1110FE23C67293A(uParam0->f_1497.f_736.f_87[iVar2])))
			{
				bVar0 = false;
				bVar1 = false;
				uParam0->f_1497.f_736.f_278 = 2;
			}
			else
			{
				iVar2++;
			}
		}
	}
	if (bVar0)
	{
		if (!bVar1)
		{
			bVar0 = false;
		}
	}
	if (bVar0)
	{
		if (uParam0->f_1497.f_736.f_276 == 0)
		{
			func_708(uParam0->f_1497.f_709, 12, 0);
			uParam0->f_1497.f_736.f_276 = 1;
		}
	}
	else if (uParam0->f_1497.f_736.f_276 == 1)
	{
		func_708(uParam0->f_1497.f_709, 12, 1);
		uParam0->f_1497.f_736.f_276 = 0;
	}
}

void func_708(var uParam0, int iParam1, int iParam2)
{
	unk_0x830F007E19C63E14(uParam0, "SET_BUTTON_GREYED_OUT");
	unk_0xAD291B8F75D737AD(iParam1);
	unk_0xF1B28F753235CE2A(iParam2);
	unk_0xE1FDD153F5858534();
}

int func_709(var uParam0, int iParam1)
{
	int iVar0;
	
	iVar0 = 0;
	while (iVar0 <= 3)
	{
		if (unk_0xF1110FE23C67293A(uParam0->f_1497.f_736.f_87[iVar0]) != iParam1)
		{
		}
		else
		{
			return uParam0->f_1497.f_736.f_103[iVar0] == 1;
		}
		iVar0++;
	}
	return 0;
}

void func_710(var uParam0, int iParam1, int iParam2)
{
	unk_0x830F007E19C63E14(uParam0, "SET_BUTTON_VISIBLE");
	unk_0xAD291B8F75D737AD(iParam1);
	unk_0xF1B28F753235CE2A(iParam2);
	unk_0xE1FDD153F5858534();
}

void func_711(var uParam0, int iParam1, int iParam2)
{
	unk_0x830F007E19C63E14(uParam0, "SET_BUTTON_ENABLED");
	unk_0xAD291B8F75D737AD(iParam1);
	unk_0xF1B28F753235CE2A(iParam2);
	unk_0xE1FDD153F5858534();
}

void func_712(var uParam0, int iParam1)
{
	if (uParam0->f_4355 == iParam1 && uParam0->f_2604)
	{
		uParam0->f_2604 = 0;
	}
	if (uParam0->f_4355 == iParam1 && uParam0->f_743)
	{
		uParam0->f_743 = 0;
	}
	if (uParam0->f_4355 == iParam1 && uParam0->f_744.f_750)
	{
		uParam0->f_744.f_750 = 0;
	}
	if (uParam0->f_4355 == iParam1 && uParam0->f_1497.f_1029)
	{
		uParam0->f_1497.f_1029 = 0;
	}
}

void func_713()
{
	unk_0xD10328357EA7B9E5();
	unk_0x4CB4237D8858ADA6(18);
	unk_0xA86915034F55A3BF();
	func_69(0);
	unk_0x779660A9E5364C4D(0, 199, 1);
	unk_0xE94F4428D732FD15();
	unk_0x8ECAF24A8B08CF58(unk_0x460153A63B9477BC(), 0);
	unk_0x689789B905574795(19);
	func_562(11, -1);
}

void func_714(var uParam0)
{
	struct<3> Var0;
	struct<3> Var3;
	
	if (uParam0->f_2529.f_32 != uParam0->f_2529.f_33)
	{
		func_725(uParam0, uParam0->f_2529.f_33);
		uParam0->f_2529.f_62 = 1;
		Var0 = { 0f, 0f, 0f };
		Var3 = { 0f, 0f, 0f };
		uParam0->f_2529.f_44 = { func_724(&(uParam0->f_2529)) };
		uParam0->f_2529.f_47 = { func_723(&(uParam0->f_2529)) };
		uParam0->f_2529.f_1.f_17 = func_722(&(uParam0->f_2529));
		uParam0->f_2529.f_38 = { unk_0xC29BFC65584F2213(uParam0->f_2529.f_1) };
		uParam0->f_2529.f_41 = { unk_0x64F296BB45631A29(uParam0->f_2529.f_1, 2) };
		uParam0->f_2529.f_1.f_18 = unk_0x9CF655D210CC30DC(uParam0->f_2529.f_1);
		uParam0->f_2529.f_1.f_14 = { 0f, 0f, 0f };
		uParam0->f_2529.f_63 = 750;
		unk_0x2311A669433389E8(uParam0->f_2529.f_1, "HAND_SHAKE", 0f);
		unk_0x3FD8029C7A771A0E(uParam0->f_2529.f_1, uParam0->f_2529.f_38, uParam0->f_2529.f_41, uParam0->f_2529.f_1.f_18, 0, 1, 1, 2);
		unk_0x3FD8029C7A771A0E(uParam0->f_2529.f_1, uParam0->f_2529.f_44, uParam0->f_2529.f_47, uParam0->f_2529.f_1.f_17, uParam0->f_2529.f_63, 1, 1, 2);
	}
	if (uParam0->f_2529.f_62)
	{
		uParam0->f_2529.f_1.f_18 = func_722(&(uParam0->f_2529));
		if (!unk_0xC70648DE87CC1919(uParam0->f_2529.f_1))
		{
			uParam0->f_2529.f_38 = { uParam0->f_2529.f_44 };
			uParam0->f_2529.f_41 = { uParam0->f_2529.f_47 };
			uParam0->f_2529.f_1.f_1 = { uParam0->f_2529.f_44 };
			uParam0->f_2529.f_1.f_4 = { uParam0->f_2529.f_47 };
			unk_0x2311A669433389E8(uParam0->f_2529.f_1, "HAND_SHAKE", 0.1f);
			uParam0->f_2529.f_62 = 0;
			if (func_17(uParam0, 1))
			{
				func_205(uParam0, 1);
			}
			func_721(&(uParam0->f_2529), uParam0->f_2529.f_1.f_18);
			func_717(&(uParam0->f_2529.f_1), 0, 0, 1, 0, 1045220557, 0, 1065353216, 0);
		}
	}
	else
	{
		func_717(&(uParam0->f_2529.f_1), func_716(&(uParam0->f_2529)), 1, 0, 0, 1045220557, 0, 1065353216, 1);
	}
	if (unk_0xFC559366953F62B3() && func_560(232))
	{
		if (!unk_0x87F0CB19476706C3() && !unk_0x5B772EFCB84373BE())
		{
			unk_0xFD45D90342D2A0CD(500);
		}
		func_715(232);
	}
}

void func_715(int iParam0)
{
	int iVar0;
	int iVar1;
	
	iVar0 = (iParam0 / 32);
	iVar1 = (iParam0 % 32);
	if (iVar0 >= 8)
	{
		return;
	}
	if (unk_0xCE990E643CD9D0E5(Global_2452907.f_1.f_2813[iVar0], iVar1))
	{
	}
	unk_0xD2459066EA58CE43(&(Global_2452907.f_1.f_2813[iVar0]), iVar1);
}

int func_716(var uParam0)
{
	switch (uParam0->f_33)
	{
		case 0:
		case 1:
		case 2:
			return 1;
		
		default:
	}
	return 0;
}

void func_717(var uParam0, bool bParam1, bool bParam2, bool bParam3, bool bParam4, float fParam5, bool bParam6, float fParam7, bool bParam8)
{
	int iVar0[4];
	float fVar5;
	float fVar6;
	float fVar7;
	float fVar8;
	float fVar9;
	struct<3> Var10;
	int iVar13;
	int iVar14;
	
	unk_0x2B6C81DF4CD1093C(2);
	func_720(&(iVar0[0]), &(iVar0[1]), &(iVar0[2]), &(iVar0[3]), 0, 0);
	if (unk_0xB12E59C58CB342A4())
	{
		iVar0[3] = (iVar0[3] * -1);
	}
	if (unk_0xC1F43F8D0D5EB664(2))
	{
		fVar5 = unk_0xB0F4BDC84084072E(2, 239);
		fVar6 = unk_0xB0F4BDC84084072E(2, 240);
		fVar7 = (fVar5 - uParam0->f_29);
		fVar8 = (fVar6 - uParam0->f_30);
		uParam0->f_29 = fVar5;
		uParam0->f_30 = fVar6;
		if (bParam4)
		{
			iVar0[2] = -unk_0xF2DB717A73826179(((fVar7 * fParam5) * 127f));
			iVar0[3] = -unk_0xF2DB717A73826179(((fVar8 * fParam5) * 127f));
		}
		else
		{
			iVar0[2] = unk_0xF2DB717A73826179(((unk_0xB0F4BDC84084072E(2, 290) * fParam5) * 127f));
			iVar0[3] = unk_0xF2DB717A73826179(((unk_0xB0F4BDC84084072E(2, 291) * fParam5) * 127f));
		}
		iVar0[2] = func_719((iVar0[2] + uParam0->f_24), -127, 127);
		iVar0[3] = func_719((iVar0[3] + uParam0->f_25), -127, 127);
	}
	if (uParam0->f_24 == iVar0[2] && uParam0->f_25 == iVar0[3])
	{
		if (uParam0->f_27 < unk_0x578C4EF320340AF7())
		{
			uParam0->f_24 = 0;
			uParam0->f_25 = 0;
			if (unk_0xC1F43F8D0D5EB664(2))
			{
				iVar0[2] = 0;
				iVar0[3] = 0;
				uParam0->f_28 = 1;
			}
		}
	}
	else
	{
		uParam0->f_24 = iVar0[2];
		uParam0->f_25 = iVar0[3];
		uParam0->f_27 = unk_0x578C4EF320340AF7() + 4000;
		uParam0->f_28 = 0;
	}
	if (bParam2)
	{
		uParam0->f_8.f_2 = (-(unk_0xBBDA792448DB5A89(iVar0[2]) / 127f) * IntToFloat(uParam0->f_20));
		uParam0->f_8.f_1 = ((-uParam0->f_8.f_2 * IntToFloat(uParam0->f_22)) / IntToFloat(uParam0->f_20));
		uParam0->f_8 = (-(unk_0xBBDA792448DB5A89(iVar0[3]) / 127f) * IntToFloat(uParam0->f_21));
	}
	else
	{
		uParam0->f_8 = { 0f, 0f, 0f };
		uParam0->f_24 = 0;
		uParam0->f_25 = 0;
	}
	fVar9 = (30f * unk_0x0000000050597EE2());
	Var10 = { uParam0->f_8 + uParam0->f_11 };
	if ((unk_0xC1F43F8D0D5EB664(2) && bParam2) && !uParam0->f_28)
	{
		uParam0->f_14 = Var10.f_0;
		uParam0->f_14.f_1 = Var10.f_1;
		uParam0->f_14.f_2 = Var10.f_2;
	}
	else
	{
		uParam0->f_14 = (uParam0->f_14 + func_718(((((Var10.f_0 - uParam0->f_14) * 0.05f) * fVar9) * fParam7), -3f, 3f));
		uParam0->f_14.f_1 = (uParam0->f_14.f_1 + func_718(((((Var10.f_1 - uParam0->f_14.f_1) * 0.05f) * fVar9) * fParam7), -3f, 3f));
		uParam0->f_14.f_2 = (uParam0->f_14.f_2 + func_718(((((Var10.f_2 - uParam0->f_14.f_2) * 0.05f) * fVar9) * fParam7), -3f, 3f));
	}
	if (uParam0->f_26)
	{
		uParam0->f_14 = func_718(uParam0->f_14, unk_0xBBDA792448DB5A89(-uParam0->f_21), unk_0xBBDA792448DB5A89(uParam0->f_21));
		uParam0->f_14.f_1 = func_718(uParam0->f_14.f_1, unk_0xBBDA792448DB5A89(-uParam0->f_22), unk_0xBBDA792448DB5A89(uParam0->f_22));
		uParam0->f_14.f_2 = func_718(uParam0->f_14.f_2, unk_0xBBDA792448DB5A89(-uParam0->f_20), unk_0xBBDA792448DB5A89(uParam0->f_20));
	}
	if (unk_0xC1F43F8D0D5EB664(0) && bParam1)
	{
		if (uParam0->f_28)
		{
			uParam0->f_17 = uParam0->f_7;
		}
	}
	else
	{
		uParam0->f_17 = uParam0->f_7;
	}
	if (bParam1)
	{
		if (unk_0xC1F43F8D0D5EB664(0))
		{
			iVar13 = 40;
			iVar14 = 41;
			if (bParam6)
			{
				iVar13 = 241;
				iVar14 = 242;
			}
			if (unk_0x79800E51B3B6D197(0, iVar13))
			{
				uParam0->f_17 = (uParam0->f_17 - 5f);
				uParam0->f_27 = unk_0x578C4EF320340AF7() + 4000;
				uParam0->f_28 = 0;
			}
			else if (unk_0x79800E51B3B6D197(0, iVar14))
			{
				uParam0->f_17 = (uParam0->f_17 + 5f);
				uParam0->f_27 = unk_0x578C4EF320340AF7() + 4000;
				uParam0->f_28 = 0;
			}
			if (bParam3)
			{
				uParam0->f_17 = func_718(uParam0->f_17, (uParam0->f_7 - uParam0->f_19), uParam0->f_7);
			}
			else
			{
				uParam0->f_17 = func_718(uParam0->f_17, (uParam0->f_7 - uParam0->f_19), (uParam0->f_7 + uParam0->f_19));
			}
		}
		else if (bParam8)
		{
			iVar0[1] = unk_0x0EEB12E4AD6BD29B(2, 207);
			iVar0[3] = unk_0x0EEB12E4AD6BD29B(2, 208);
			if (bParam3)
			{
				if (unk_0xBBDA792448DB5A89(iVar0[3]) > 127f)
				{
					uParam0->f_17 = (uParam0->f_17 - IntToFloat(unk_0xF2DB717A73826179(((unk_0xBBDA792448DB5A89(iVar0[3]) / 128f) * (uParam0->f_19 / 2f)))));
				}
			}
			else
			{
				uParam0->f_17 = (uParam0->f_17 + IntToFloat(unk_0xF2DB717A73826179(((unk_0xBBDA792448DB5A89(iVar0[1]) / 128f) * uParam0->f_19))));
				uParam0->f_17 = (uParam0->f_17 - IntToFloat(unk_0xF2DB717A73826179(((unk_0xBBDA792448DB5A89(iVar0[3]) / 128f) * uParam0->f_19))));
			}
		}
		else if (bParam3)
		{
			if (unk_0xBBDA792448DB5A89(iVar0[1]) < 0f)
			{
				uParam0->f_17 = (uParam0->f_17 + IntToFloat(unk_0xF2DB717A73826179(((unk_0xBBDA792448DB5A89(iVar0[1]) / 128f) * uParam0->f_19))));
			}
		}
		else
		{
			uParam0->f_17 = (uParam0->f_17 + IntToFloat(unk_0xF2DB717A73826179(((unk_0xBBDA792448DB5A89(iVar0[1]) / 128f) * uParam0->f_19))));
		}
	}
	uParam0->f_18 = (uParam0->f_18 + (((uParam0->f_17 - uParam0->f_18) * 0.06f) * fVar9));
	unk_0x3FD8029C7A771A0E(*uParam0, uParam0->f_1, uParam0->f_4 + uParam0->f_14, uParam0->f_18, 0, 1, 1, 2);
	if (unk_0x40397A9A17EEC1C5(*uParam0))
	{
		if (unk_0xC516FEEB950FFD58(*uParam0))
		{
			if (unk_0x09F689AE170A62EC(*uParam0))
			{
				unk_0x17513DA51251A563();
			}
		}
	}
}

float func_718(float fParam0, float fParam1, float fParam2)
{
	if (fParam0 > fParam2)
	{
		return fParam2;
	}
	else if (fParam0 < fParam1)
	{
		return fParam1;
	}
	return fParam0;
}

int func_719(int iParam0, int iParam1, int iParam2)
{
	if (iParam0 > iParam2)
	{
		return iParam2;
	}
	else if (iParam0 < iParam1)
	{
		return iParam1;
	}
	return iParam0;
}

void func_720(var uParam0, var uParam1, var uParam2, var uParam3, bool bParam4, bool bParam5)
{
	*uParam0 = unk_0xF34EE736CF047844((unk_0xB0F4BDC84084072E(2, 218) * 127f));
	*uParam1 = unk_0xF34EE736CF047844((unk_0xB0F4BDC84084072E(2, 219) * 127f));
	*uParam2 = unk_0xF34EE736CF047844((unk_0xB0F4BDC84084072E(2, 220) * 127f));
	*uParam3 = unk_0xF34EE736CF047844((unk_0xB0F4BDC84084072E(2, 221) * 127f));
	if (bParam4)
	{
		if (!unk_0xC6F4182B2D1E3EAD(2, 218))
		{
			*uParam0 = unk_0xF34EE736CF047844((unk_0xAA5DA46D3DBFD541(2, 218) * 127f));
		}
		if (!unk_0xC6F4182B2D1E3EAD(2, 219))
		{
			*uParam1 = unk_0xF34EE736CF047844((unk_0xAA5DA46D3DBFD541(2, 219) * 127f));
		}
		if (!unk_0xC6F4182B2D1E3EAD(2, 220))
		{
			*uParam2 = unk_0xF34EE736CF047844((unk_0xAA5DA46D3DBFD541(2, 220) * 127f));
		}
		if (!unk_0xC6F4182B2D1E3EAD(2, 221))
		{
			*uParam3 = unk_0xF34EE736CF047844((unk_0xAA5DA46D3DBFD541(2, 221) * 127f));
		}
	}
	if (unk_0xC1F43F8D0D5EB664(2))
	{
		if (bParam5)
		{
			if (unk_0xB12E59C58CB342A4())
			{
				*uParam3 = (*uParam3 * -1);
			}
			if (unk_0x4940106A0CAC814B())
			{
				*uParam3 = (*uParam3 * -1);
			}
		}
	}
}

void func_721(var uParam0, var uParam1)
{
	uParam0->f_1.f_7 = uParam1;
	uParam0->f_1.f_20 = 15;
	uParam0->f_1.f_21 = 10;
	uParam0->f_1.f_22 = 1;
	uParam0->f_1.f_19 = 10f;
}

float func_722(var uParam0)
{
	switch (uParam0->f_33)
	{
		case 0:
			return 24.9482f;
		
		case 1:
			return 24.9482f;
		
		case 2:
			return 24.9482f;
		
		case 3:
			return 8.6949f;
		
		case 4:
			return 11.6733f;
		
		case 5:
			return 11.6334f;
		
		case 6:
			return 24.9482f;
		
		case 7:
			return 24.9482f;
		
		case 8:
			return 24.9482f;
		
		case 9:
			return 24.9482f;
		
		case 10:
			return 12.1654f;
		
		case 11:
			return 24.9482f;
		
		case 12:
			return 24.9482f;
		
		case 13:
			return 24.9482f;
		
		case 14:
			return 24.9482f;
		
		case 15:
			return 24.9482f;
		
		case 16:
			return 24.9482f;
		
		case 17:
			return 24.9482f;
		
		case 18:
			return 24.9482f;
		
		case 19:
			return 22.8918f;
		
		case 20:
			return 22.8918f;
		
		case 21:
			return 22.8918f;
		
		case 22:
			return 22.8918f;
		
		case 23:
			return 22.8918f;
		
		default:
	}
	return uParam0->f_1.f_7;
}

Vector3 func_723(var uParam0)
{
	switch (uParam0->f_33)
	{
		case 0:
			return 1.0456f, -0.0173f, -0.3992f;
		
		case 1:
			return -0.7083f, -0.0232f, -89.0889f;
		
		case 2:
			return 0.4921f, 0.0269f, -179.4778f;
		
		case 3:
			return 0.8281f, 0.0207f, -0.372f;
		
		case 4:
			return 0.6552f, -0.0496f, -8.747f;
		
		case 5:
			return -0.4704f, -0.0496f, -0.2582f;
		
		case 6:
			return -2.3366f, -0.0013f, -0.2903f;
		
		case 7:
			return -2.3366f, -0.0013f, -0.2903f;
		
		case 8:
			return -2.3366f, -0.0013f, -0.2903f;
		
		case 9:
			return 0.4201f, -0.0249f, -0.3753f;
		
		case 10:
			return 1.9801f, 0.0066f, -89.1273f;
		
		case 11:
			return -8.3531f, 0.021f, -89.0267f;
		
		case 12:
			return -8.3531f, 0.021f, -89.0267f;
		
		case 13:
			return -8.3531f, 0.021f, -89.0267f;
		
		case 14:
			return -0.6781f, 0.021f, -89.166f;
		
		case 15:
			return -0.6781f, 0.021f, -89.166f;
		
		case 16:
			return -0.6781f, 0.021f, -89.166f;
		
		case 17:
			return -1.6772f, 0.021f, -89.166f;
		
		case 18:
			return 0.0156f, 0.021f, -89.166f;
		
		case 19:
			return 0.4623f, -0.0078f, -179.4444f;
		
		case 20:
			return 0.4623f, -0.0078f, -179.4444f;
		
		case 21:
			return 0.4623f, -0.0078f, -179.4444f;
		
		case 22:
			return 0.4623f, -0.0078f, -179.4444f;
		
		case 23:
			return 0.4623f, -0.0078f, -179.4444f;
		
		default:
	}
	return 6.6407f, 0.0116f, -35.027f;
}

Vector3 func_724(var uParam0)
{
	switch (uParam0->f_33)
	{
		case 0:
			return 2712.844f, -371.4153f, -54.3227f;
		
		case 1:
			return 2711.157f, -369.6294f, -54.1598f;
		
		case 2:
			return 2712.865f, -367.5525f, -54.281f;
		
		case 3:
			return 2713.022f, -371.3072f, -53.7836f;
		
		case 4:
			return 2713.036f, -371.4736f, -54.5479f;
		
		case 5:
			return 2712.681f, -372.199f, -55.0138f;
		
		case 6:
			return 2712.043f, -368.0018f, -54.9149f;
		
		case 7:
			return 2712.659f, -368.0018f, -54.9149f;
		
		case 8:
			return 2713.321f, -368.0018f, -54.9149f;
		
		case 9:
			return 2712.411f, -368.7706f, -54.4171f;
		
		case 10:
			return 2710.063f, -369.0359f, -54.366f;
		
		case 11:
			return 2714.658f, -368.6344f, -53.5536f;
		
		case 12:
			return 2714.658f, -369.0623f, -53.5536f;
		
		case 13:
			return 2714.658f, -369.4877f, -53.5536f;
		
		case 14:
			return 2714.667f, -368.6385f, -54.4841f;
		
		case 15:
			return 2714.667f, -369.1197f, -54.4841f;
		
		case 16:
			return 2714.667f, -370.511f, -54.8597f;
		
		case 17:
			return 2714.663f, -370.2132f, -54.1153f;
		
		case 18:
			return 2714.663f, -370.5733f, -54.4841f;
		
		case 19:
			return 2713.151f, -368.5675f, -54.3901f;
		
		case 20:
			return 2713.151f, -368.5675f, -54.3901f;
		
		case 21:
			return 2713.151f, -368.5675f, -54.3901f;
		
		case 22:
			return 2713.151f, -368.5675f, -54.3901f;
		
		case 23:
			return 2713.151f, -368.5675f, -54.3901f;
		
		default:
	}
	return 350.4201f, 4871.844f, -59.2717f;
}

void func_725(var uParam0, var uParam1)
{
	uParam0->f_2529.f_32 = uParam1;
}

void func_726(var uParam0)
{
	int iVar0;
	int iVar1;
	int iVar2;
	struct<6> Var3;
	int iVar9;
	bool bVar10;
	int iVar11;
	
	if (!func_483(uParam0, 2))
	{
		return;
	}
	bVar10 = func_816(uParam0);
	iVar0 = 0;
	while (iVar0 <= 3)
	{
		if (uParam0->f_1497.f_736.f_87[iVar0] == -1)
		{
		}
		else
		{
			if (unk_0xFC559366953F62B3())
			{
				iVar2 = unk_0xF1110FE23C67293A(uParam0->f_1497.f_736.f_87[iVar0]);
			}
			else
			{
				iVar2 = uParam0->f_4356;
			}
			Var3 = { func_805(uParam0, iVar2) };
			iVar9 = func_804(iVar0);
			iVar1 = uParam0->f_1497.f_736.f_92[iVar0];
			if (func_803(uParam0, iVar2))
			{
				if (bVar10)
				{
					func_801(uParam0->f_1497.f_709, iVar9, unk_0x72C1D3A1AB8CAA5B(iVar2), &Var3);
					func_710(uParam0->f_1497.f_709, func_800(iVar0), 1);
					func_711(uParam0->f_1497.f_709, func_800(iVar0), 1);
				}
				func_798(uParam0->f_1497.f_709, iVar9, func_709(uParam0, iVar2), func_799(iVar2));
				func_797(uParam0->f_1497.f_709, iVar9, iVar1);
			}
			else if (bVar10)
			{
				func_710(uParam0->f_1497.f_709, func_800(iVar0), 0);
				func_711(uParam0->f_1497.f_709, func_800(iVar0), 0);
			}
		}
		iVar0++;
	}
	if (bVar10)
	{
		func_796(uParam0, 0);
	}
	if (func_795(uParam0) || func_794())
	{
		if (unk_0xFC559366953F62B3())
		{
			func_793(uParam0->f_1497.f_709, "CH_FINALE_LBLH");
			func_708(uParam0->f_1497.f_709, 12, 1);
		}
		else
		{
			func_793(uParam0->f_1497.f_709, "CH_FINALE_LBS");
			func_708(uParam0->f_1497.f_709, 12, 0);
		}
		func_792(uParam0, 0);
	}
	if (func_791(uParam0) || func_794())
	{
		if (unk_0xFC559366953F62B3())
		{
			func_790(uParam0->f_1497.f_709);
			iVar0 = 0;
			while (iVar0 < 5)
			{
				if (!unk_0x7BCC91F3C1CF24E8(func_789(uParam0, iVar0)))
				{
					func_787(uParam0->f_1497.f_709, func_789(uParam0, iVar0), func_788(uParam0, iVar0));
				}
				iVar0++;
			}
		}
		func_786(uParam0, 0);
	}
	if (func_785(uParam0) || func_794())
	{
		if (unk_0xFC559366953F62B3())
		{
			func_784(uParam0->f_1497.f_709);
			iVar0 = 0;
			while (iVar0 < 3)
			{
				if (!unk_0x7BCC91F3C1CF24E8(func_783(uParam0, iVar0)))
				{
					func_781(uParam0->f_1497.f_709, func_783(uParam0, iVar0), func_782(uParam0, iVar0));
				}
				iVar0++;
			}
		}
		func_780(uParam0, 0);
	}
	if (func_779(uParam0) || func_794())
	{
		func_770(uParam0->f_1497.f_709, func_133(func_778(uParam0), 1), func_120(func_121(uParam0->f_4356), 1), func_528(uParam0), func_777(uParam0), func_774(uParam0), func_773(uParam0, uParam0->f_1497.f_1017), func_773(uParam0, uParam0->f_1497.f_1018), func_772(uParam0->f_1497.f_1019), func_771(uParam0->f_1497.f_1020), func_771(uParam0->f_1497.f_1021));
		func_769(uParam0, 0);
	}
	if (func_768(uParam0) || func_794())
	{
		iVar11 = 0;
		while (iVar11 < 18)
		{
			func_710(uParam0->f_1497.f_709, iVar11, func_765(uParam0, iVar11));
			iVar11++;
		}
		func_763(uParam0->f_1497.f_709, 6, func_764(uParam0->f_4356));
		func_763(uParam0->f_1497.f_709, 7, func_762(uParam0->f_4356));
		func_761(uParam0->f_1497.f_709, 6, 1);
		func_761(uParam0->f_1497.f_709, 7, 1);
		func_760(uParam0, 0);
	}
	if (func_759(uParam0) || func_794())
	{
		func_761(uParam0->f_1497.f_709, 2, func_758(uParam0));
		if (uParam0->f_1497.f_1017 == -1)
		{
			func_757(uParam0->f_1497.f_709, 2, 1);
		}
		else
		{
			func_757(uParam0->f_1497.f_709, 2, 0);
		}
		if (func_113(uParam0->f_4356) == 2)
		{
			func_756(uParam0->f_1497.f_709, 2, 2);
		}
		func_755(uParam0, 0);
	}
	if (func_754(uParam0) || func_794())
	{
		func_761(uParam0->f_1497.f_709, 3, func_753(uParam0));
		if (uParam0->f_1497.f_1018 == -1)
		{
			func_757(uParam0->f_1497.f_709, 3, 1);
		}
		else
		{
			func_757(uParam0->f_1497.f_709, 3, 0);
		}
		func_752(uParam0, 0);
	}
	if (func_751(uParam0) || func_794())
	{
		func_761(uParam0->f_1497.f_709, 4, func_750(uParam0));
		if (uParam0->f_1497.f_1019 == -1)
		{
			func_757(uParam0->f_1497.f_709, 4, 1);
		}
		else
		{
			func_757(uParam0->f_1497.f_709, 4, 0);
		}
		func_748(uParam0->f_1497.f_709, func_749(uParam0));
		func_747(uParam0, 0);
	}
	if (func_746(uParam0) || func_794())
	{
		if (func_113(uParam0->f_4356) == 2)
		{
			func_761(uParam0->f_1497.f_709, 13, func_745(uParam0));
			if (uParam0->f_1497.f_1020 == 0)
			{
				func_757(uParam0->f_1497.f_709, 13, 1);
			}
			else
			{
				func_757(uParam0->f_1497.f_709, 13, 0);
			}
			if (!unk_0xFC559366953F62B3())
			{
				if (func_744(uParam0))
				{
					func_756(uParam0->f_1497.f_709, 13, 0);
				}
				else
				{
					func_756(uParam0->f_1497.f_709, 13, 2);
				}
			}
		}
		else
		{
			func_710(uParam0->f_1497.f_709, 13, 0);
		}
		func_743(uParam0, 0);
	}
	if (func_742(uParam0) || func_794())
	{
		if (func_113(uParam0->f_4356) == 2)
		{
			func_761(uParam0->f_1497.f_709, 14, func_741(uParam0));
			if (uParam0->f_1497.f_1021 == 5)
			{
				func_757(uParam0->f_1497.f_709, 14, 1);
			}
			else
			{
				func_757(uParam0->f_1497.f_709, 14, 0);
			}
			if (!unk_0xFC559366953F62B3())
			{
				if (func_740(uParam0))
				{
					func_756(uParam0->f_1497.f_709, 14, 0);
				}
				else
				{
					func_756(uParam0->f_1497.f_709, 14, 2);
				}
			}
		}
		else
		{
			func_710(uParam0->f_1497.f_709, 14, 0);
		}
		func_739(uParam0, 0);
	}
	if (func_738(uParam0) || func_794())
	{
		func_761(uParam0->f_1497.f_709, 15, func_736(uParam0));
		func_735(uParam0, 0);
	}
	if (func_734(uParam0) || func_794())
	{
		func_761(uParam0->f_1497.f_709, 5, func_732(uParam0));
		func_731(uParam0, 0);
	}
	if (func_730(uParam0) || func_794())
	{
		func_761(uParam0->f_1497.f_709, 16, func_729(uParam0));
		func_728(uParam0, 0);
	}
	func_727(0);
}

void func_727(int iParam0)
{
	if (Global_1697302 && Global_1697305 != iParam0)
	{
		Global_1697305 = iParam0;
	}
}

void func_728(var uParam0, bool bParam1)
{
	if (bParam1)
	{
		unk_0xBE20AB8238688965(&(uParam0->f_1497.f_1015), 18);
	}
	else
	{
		unk_0xD2459066EA58CE43(&(uParam0->f_1497.f_1015), 18);
	}
}

int func_729(var uParam0)
{
	return 1;
}

bool func_730(var uParam0)
{
	return unk_0xCE990E643CD9D0E5(uParam0->f_1497.f_1015, 18);
}

void func_731(var uParam0, bool bParam1)
{
	if (bParam1)
	{
		unk_0xBE20AB8238688965(&(uParam0->f_1497.f_1015), 17);
	}
	else
	{
		unk_0xD2459066EA58CE43(&(uParam0->f_1497.f_1015), 17);
	}
}

int func_732(var uParam0)
{
	switch (func_153(uParam0->f_4356, 0))
	{
		case 1:
			switch (func_733(uParam0->f_4356))
			{
				case 0:
					return 1;
				
				case 1:
					return 2;
				
				case 2:
					return 3;
				
				case 3:
					return 4;
				
				default:
			}
			break;
		
		case 2:
			switch (func_733(uParam0->f_4356))
			{
				case 0:
					return 5;
				
				case 1:
					return 6;
				
				case 2:
					return 7;
				
				case 3:
					return 8;
				
				default:
			}
			break;
		
		case 3:
			switch (func_733(uParam0->f_4356))
			{
				case 0:
					return 9;
				
				case 1:
					return 10;
				
				case 2:
					return 11;
				
				case 3:
					return 12;
				
				default:
			}
			break;
		
		case 4:
			switch (func_733(uParam0->f_4356))
			{
				case 0:
					return 13;
				
				case 1:
					return 14;
				
				case 2:
					return 15;
				
				case 3:
					return 16;
				
				default:
			}
			break;
		
		case 5:
			switch (func_733(uParam0->f_4356))
			{
				case 0:
					return 17;
				
				case 1:
					return 18;
				
				case 2:
					return 19;
				
				case 3:
					return 20;
				
				default:
			}
			break;
	}
	return -1;
}

int func_733(int iParam0)
{
	if (iParam0 != -1)
	{
		return Global_1701669[iParam0 /*68*/].f_18.f_13;
	}
	return 0;
}

bool func_734(var uParam0)
{
	return unk_0xCE990E643CD9D0E5(uParam0->f_1497.f_1015, 17);
}

void func_735(var uParam0, bool bParam1)
{
	if (bParam1)
	{
		unk_0xBE20AB8238688965(&(uParam0->f_1497.f_1015), 16);
	}
	else
	{
		unk_0xD2459066EA58CE43(&(uParam0->f_1497.f_1015), 16);
	}
}

int func_736(var uParam0)
{
	switch (func_164(uParam0->f_4356, 0))
	{
		case 1:
			switch (func_113(uParam0->f_4356))
			{
				case 1:
					switch (func_737(uParam0->f_4356))
					{
						case 0:
							return -1;
						
						case 1:
							return -1;
						
						default:
					}
					break;
				
				case 2:
					switch (func_737(uParam0->f_4356))
					{
						case 0:
							return -1;
						
						case 1:
							return -1;
						
						default:
					}
					break;
				
				case 3:
					switch (func_737(uParam0->f_4356))
					{
						case 0:
							return 1;
						
						case 1:
							return 2;
						
						default:
					}
					break;
			}
			break;
		
		case 2:
			switch (func_113(uParam0->f_4356))
			{
				case 1:
					switch (func_737(uParam0->f_4356))
					{
						case 0:
							return -1;
						
						case 1:
							return -1;
						
						default:
					}
					break;
				
				case 2:
					switch (func_737(uParam0->f_4356))
					{
						case 0:
							return -1;
						
						case 1:
							return -1;
						
						default:
					}
					break;
				
				case 3:
					switch (func_737(uParam0->f_4356))
					{
						case 0:
							return 3;
						
						case 1:
							return 4;
						
						default:
					}
					break;
			}
			break;
		
		case 3:
			switch (func_113(uParam0->f_4356))
			{
				case 1:
					switch (func_737(uParam0->f_4356))
					{
						case 0:
							return -1;
						
						case 1:
							return -1;
						
						default:
					}
					break;
				
				case 2:
					switch (func_737(uParam0->f_4356))
					{
						case 0:
							return -1;
						
						case 1:
							return -1;
						
						default:
					}
					break;
				
				case 3:
					switch (func_737(uParam0->f_4356))
					{
						case 0:
							return 5;
						
						case 1:
							return 6;
						
						default:
					}
					break;
			}
			break;
		
		case 4:
			switch (func_113(uParam0->f_4356))
			{
				case 1:
					switch (func_737(uParam0->f_4356))
					{
						case 0:
							return -1;
						
						case 1:
							return -1;
						
						default:
					}
					break;
				
				case 2:
					switch (func_737(uParam0->f_4356))
					{
						case 0:
							return -1;
						
						case 1:
							return -1;
						
						default:
					}
					break;
				
				case 3:
					switch (func_737(uParam0->f_4356))
					{
						case 0:
							return 7;
						
						case 1:
							return 8;
						
						default:
					}
					break;
			}
			break;
		
		case 5:
			switch (func_113(uParam0->f_4356))
			{
				case 1:
					switch (func_737(uParam0->f_4356))
					{
						case 0:
							return -1;
						
						case 1:
							return -1;
						
						default:
					}
					break;
				
				case 2:
					switch (func_737(uParam0->f_4356))
					{
						case 0:
							return -1;
						
						case 1:
							return -1;
						
						default:
					}
					break;
				
				case 3:
					switch (func_737(uParam0->f_4356))
					{
						case 0:
							return 9;
						
						case 1:
							return 10;
						
						default:
					}
					break;
			}
			break;
	}
	return -1;
}

int func_737(int iParam0)
{
	if (iParam0 != -1)
	{
		return Global_1701669[iParam0 /*68*/].f_18.f_11;
	}
	return 0;
}

bool func_738(var uParam0)
{
	return unk_0xCE990E643CD9D0E5(uParam0->f_1497.f_1015, 16);
}

void func_739(var uParam0, bool bParam1)
{
	if (bParam1)
	{
		unk_0xBE20AB8238688965(&(uParam0->f_1497.f_1015), 14);
	}
	else
	{
		unk_0xD2459066EA58CE43(&(uParam0->f_1497.f_1015), 14);
	}
}

bool func_740(var uParam0)
{
	int iVar0;
	
	if (func_173(uParam0->f_4356, 6))
	{
		iVar0++;
	}
	if (func_173(uParam0->f_4356, 7))
	{
		iVar0++;
	}
	if (func_173(uParam0->f_4356, 8))
	{
		iVar0++;
	}
	return iVar0 > 1;
}

int func_741(var uParam0)
{
	switch (uParam0->f_1497.f_1021)
	{
		case 6:
			return 2;
		
		case 7:
			return 1;
		
		case 8:
			return 3;
		
		default:
	}
	return -1;
}

bool func_742(var uParam0)
{
	return unk_0xCE990E643CD9D0E5(uParam0->f_1497.f_1015, 14);
}

void func_743(var uParam0, bool bParam1)
{
	if (bParam1)
	{
		unk_0xBE20AB8238688965(&(uParam0->f_1497.f_1015), 13);
	}
	else
	{
		unk_0xD2459066EA58CE43(&(uParam0->f_1497.f_1015), 13);
	}
}

bool func_744(var uParam0)
{
	int iVar0;
	
	if (func_173(uParam0->f_4356, 1))
	{
		iVar0++;
	}
	if (func_173(uParam0->f_4356, 2))
	{
		iVar0++;
	}
	if (func_173(uParam0->f_4356, 3))
	{
		iVar0++;
	}
	if (func_173(uParam0->f_4356, 4))
	{
		iVar0++;
	}
	return iVar0 > 1;
}

int func_745(var uParam0)
{
	switch (uParam0->f_1497.f_1020)
	{
		case 1:
			return 1;
		
		case 2:
			return 2;
		
		case 3:
			return 3;
		
		case 4:
			return 4;
		
		default:
	}
	return -1;
}

bool func_746(var uParam0)
{
	return unk_0xCE990E643CD9D0E5(uParam0->f_1497.f_1015, 13);
}

void func_747(var uParam0, bool bParam1)
{
	if (bParam1)
	{
		unk_0xBE20AB8238688965(&(uParam0->f_1497.f_1015), 12);
	}
	else
	{
		unk_0xD2459066EA58CE43(&(uParam0->f_1497.f_1015), 12);
	}
}

void func_748(var uParam0, int iParam1)
{
	unk_0x830F007E19C63E14(uParam0, "SET_MAP_MARKERS");
	unk_0xAD291B8F75D737AD(iParam1);
	unk_0xE1FDD153F5858534();
}

int func_749(var uParam0)
{
	switch (uParam0->f_1497.f_1019)
	{
		case -1:
			return 0;
		
		case 0:
		case 1:
		case 2:
			return 1;
		
		case 3:
		case 4:
		case 5:
			return 2;
		
		case 6:
		case 7:
		case 8:
			return 3;
		
		default:
	}
	return 0;
}

int func_750(var uParam0)
{
	switch (uParam0->f_1497.f_1019)
	{
		case 0:
		case 1:
		case 2:
			return 1;
		
		case 3:
		case 4:
		case 5:
			return 2;
		
		case 6:
		case 7:
		case 8:
			return 3;
		
		default:
	}
	return 0;
}

bool func_751(var uParam0)
{
	return unk_0xCE990E643CD9D0E5(uParam0->f_1497.f_1015, 12);
}

void func_752(var uParam0, bool bParam1)
{
	if (bParam1)
	{
		unk_0xBE20AB8238688965(&(uParam0->f_1497.f_1015), 11);
	}
	else
	{
		unk_0xD2459066EA58CE43(&(uParam0->f_1497.f_1015), 11);
	}
}

int func_753(var uParam0)
{
	switch (uParam0->f_1497.f_1018)
	{
		case 0:
			return 2;
		
		case 1:
			return 11;
		
		case 2:
			return 1;
		
		case 3:
			return 10;
		
		case 4:
			return 5;
		
		case 5:
			return 8;
		
		case 6:
			return 3;
		
		case 7:
			return 9;
		
		case 8:
			return 4;
		
		case 9:
			return 6;
		
		case 10:
			return 7;
		
		default:
	}
	return -1;
}

bool func_754(var uParam0)
{
	return unk_0xCE990E643CD9D0E5(uParam0->f_1497.f_1015, 11);
}

void func_755(var uParam0, bool bParam1)
{
	if (bParam1)
	{
		unk_0xBE20AB8238688965(&(uParam0->f_1497.f_1015), 10);
	}
	else
	{
		unk_0xD2459066EA58CE43(&(uParam0->f_1497.f_1015), 10);
	}
}

void func_756(var uParam0, int iParam1, int iParam2)
{
	unk_0x830F007E19C63E14(uParam0, "SET_SELECTION_ARROWS_VISIBLE");
	unk_0xAD291B8F75D737AD(iParam1);
	unk_0xAD291B8F75D737AD(iParam2);
	unk_0xE1FDD153F5858534();
}

void func_757(var uParam0, int iParam1, int iParam2)
{
	unk_0x830F007E19C63E14(uParam0, "SET_NOT_SELECTED_VISIBLE");
	unk_0xAD291B8F75D737AD(iParam1);
	unk_0xF1B28F753235CE2A(iParam2);
	unk_0xE1FDD153F5858534();
}

int func_758(var uParam0)
{
	switch (uParam0->f_1497.f_1017)
	{
		case 0:
			return 2;
		
		case 1:
			return 11;
		
		case 2:
			return 1;
		
		case 3:
			return 10;
		
		case 4:
			return 5;
		
		case 5:
			return 8;
		
		case 6:
			return 3;
		
		case 7:
			return 9;
		
		case 8:
			return 4;
		
		case 9:
			return 6;
		
		case 10:
			return 7;
		
		default:
	}
	return -1;
}

bool func_759(var uParam0)
{
	return unk_0xCE990E643CD9D0E5(uParam0->f_1497.f_1015, 10);
}

void func_760(var uParam0, bool bParam1)
{
	if (bParam1)
	{
		unk_0xBE20AB8238688965(&(uParam0->f_1497.f_1015), 15);
	}
	else
	{
		unk_0xD2459066EA58CE43(&(uParam0->f_1497.f_1015), 15);
	}
}

void func_761(var uParam0, int iParam1, int iParam2)
{
	unk_0x830F007E19C63E14(uParam0, "SET_BUTTON_IMAGE");
	unk_0xAD291B8F75D737AD(iParam1);
	unk_0xAD291B8F75D737AD(iParam2);
	unk_0xE1FDD153F5858534();
}

int func_762(int iParam0)
{
	if (iParam0 != -1)
	{
		return unk_0xCE990E643CD9D0E5(Global_1701669[iParam0 /*68*/].f_18, 7);
	}
	return 0;
}

void func_763(var uParam0, int iParam1, bool bParam2)
{
	unk_0x830F007E19C63E14(uParam0, "SET_TICK");
	unk_0xAD291B8F75D737AD(iParam1);
	unk_0xF1B28F753235CE2A(bParam2);
	unk_0xE1FDD153F5858534();
}

int func_764(int iParam0)
{
	if (iParam0 != -1)
	{
		return unk_0xCE990E643CD9D0E5(Global_1701669[iParam0 /*68*/].f_18, 6);
	}
	return 0;
}

int func_765(var uParam0, int iParam1)
{
	switch (iParam1)
	{
		case 12:
			if (uParam0->f_4356 == unk_0x460153A63B9477BC())
			{
				if (uParam0->f_1497.f_736.f_276 == 0)
				{
					return 0;
				}
			}
			else
			{
				return 0;
			}
			break;
		
		case 13:
			switch (func_113(uParam0->f_4356))
			{
				case 1:
				case 3:
					return 0;
				
				default:
			}
			break;
		
		case 14:
			switch (func_113(uParam0->f_4356))
			{
				case 1:
				case 3:
					return 0;
				
				case 2:
					return func_767(uParam0->f_4356);
				
				default:
			}
			break;
		
		case 8:
			return func_766(uParam0, 0);
		
		case 9:
			if (!unk_0xFC559366953F62B3())
			{
				return 0;
			}
			return func_766(uParam0, 1);
		
		case 10:
			if (!unk_0xFC559366953F62B3())
			{
				return 0;
			}
			return func_766(uParam0, 2);
		
		case 11:
			if (!unk_0xFC559366953F62B3())
			{
				return 0;
			}
			return func_766(uParam0, 3);
	}
	return 1;
}

int func_766(var uParam0, int iParam1)
{
	if (iParam1 == 0 && !unk_0xFC559366953F62B3())
	{
		return 1;
	}
	if (uParam0->f_1497.f_736.f_87[iParam1] == -1)
	{
		return 0;
	}
	return 1;
}

int func_767(int iParam0)
{
	if ((func_173(iParam0, 6) || func_173(iParam0, 7)) || func_173(iParam0, 8))
	{
		return 1;
	}
	return 0;
}

bool func_768(var uParam0)
{
	return unk_0xCE990E643CD9D0E5(uParam0->f_1497.f_1015, 15);
}

void func_769(var uParam0, bool bParam1)
{
	if (bParam1)
	{
		unk_0xBE20AB8238688965(&(uParam0->f_1497.f_1015), 7);
	}
	else
	{
		unk_0xD2459066EA58CE43(&(uParam0->f_1497.f_1015), 7);
	}
}

void func_770(var uParam0, var uParam1, var uParam2, int iParam3, int iParam4, int iParam5, var uParam6, var uParam7, var uParam8, var uParam9, var uParam10)
{
	unk_0x830F007E19C63E14(uParam0, "SET_HEADINGS");
	func_83(uParam1);
	func_83(uParam2);
	unk_0xAD291B8F75D737AD(iParam3);
	unk_0xAD291B8F75D737AD(iParam4);
	unk_0xAD291B8F75D737AD(iParam5);
	func_83(uParam6);
	func_83(uParam7);
	func_83(uParam8);
	func_83(uParam9);
	func_83(uParam10);
	unk_0xE1FDD153F5858534();
}

char* func_771(int iParam0)
{
	switch (iParam0)
	{
		case 0:
		case 5:
			return "";
		
		case 1:
			return "CH_OUTFIT_0";
		
		case 2:
			return "CH_OUTFIT_1";
		
		case 3:
			return "CH_OUTFIT_2";
		
		case 4:
			return "CH_OUTFIT_3";
		
		case 6:
			return "CH_OUTFIT_4";
		
		case 7:
			return "CH_OUTFIT_5";
		
		case 8:
			return "CH_OUTFIT_6";
		
		default:
	}
	return "";
}

char* func_772(int iParam0)
{
	switch (iParam0)
	{
		case 0:
		case 1:
		case 2:
			return "CH_DROPOFF_0";
		
		case 3:
		case 4:
		case 5:
			return "CH_DROPOFF_1";
		
		case 6:
		case 7:
		case 8:
			return "CH_DROPOFF_2";
		
		default:
	}
	return "";
}

char* func_773(var uParam0, int iParam1)
{
	switch (iParam1)
	{
		case 0:
			return "CH_ACCPNT_0";
		
		case 1:
			return "CH_ACCPNT_1";
		
		case 2:
			return "CH_ACCPNT_2";
		
		case 3:
			return "CH_ACCPNT_3";
		
		case 4:
			return "CH_ACCPNT_4";
		
		case 5:
			return "CH_ACCPNT_5";
		
		case 6:
			return "CH_ACCPNT_6";
		
		case 7:
			return "CH_ACCPNT_7";
		
		case 8:
			return "CH_ACCPNT_8";
		
		case 9:
			return "CH_ACCPNT_9";
		
		case 10:
			return "CH_ACCPNT_10";
		
		default:
	}
	return "";
}

int func_774(var uParam0)
{
	int iVar0;
	int iVar1;
	
	iVar0 = func_776(func_121(uParam0->f_4356));
	if (func_113(uParam0->f_4356) == func_194(uParam0->f_4356))
	{
		iVar0 = unk_0xF2DB717A73826179((IntToFloat(iVar0) * func_775(2)));
	}
	iVar1 = (iVar1 + func_161(func_164(uParam0->f_4356, 0)));
	iVar1 = (iVar1 + func_160(func_153(uParam0->f_4356, 0)));
	iVar1 = (iVar1 + func_159(func_491(uParam0->f_4356, 0)));
	iVar1 = (iVar1 + Global_262145.f_28074);
	iVar1 = ((iVar0 / 100) * iVar1);
	return iVar1;
}

float func_775(int iParam0)
{
	switch (iParam0)
	{
		case 1:
			return Global_262145.f_28069;
		
		case 2:
			return Global_262145.f_28070;
		
		default:
	}
	return 1f;
}

int func_776(int iParam0)
{
	switch (iParam0)
	{
		case 0:
			return Global_262145.f_28088;
		
		case 1:
			return Global_262145.f_28090;
		
		case 2:
			return Global_262145.f_28089;
		
		case 3:
			return Global_262145.f_28091;
		
		default:
	}
	return 0;
}

int func_777(var uParam0)
{
	int iVar0;
	float fVar1;
	
	iVar0 = func_776(func_121(uParam0->f_4356));
	if (func_113(uParam0->f_4356) == func_194(uParam0->f_4356))
	{
		iVar0 = unk_0xF2DB717A73826179((IntToFloat(iVar0) * func_775(2)));
	}
	if (unk_0xFC559366953F62B3())
	{
		fVar1 = 1f;
		switch (uParam0->f_1497.f_1019)
		{
			case 0:
			case 1:
			case 2:
				fVar1 = Global_262145.f_28071;
				break;
			
			case 3:
			case 4:
			case 5:
				fVar1 = Global_262145.f_28072;
				break;
			
			case 6:
			case 7:
			case 8:
				fVar1 = Global_262145.f_28073;
				break;
		}
		iVar0 = unk_0xF2DB717A73826179((IntToFloat(iVar0) * fVar1));
	}
	return iVar0;
}

int func_778(var uParam0)
{
	return func_113(uParam0->f_4356);
}

bool func_779(var uParam0)
{
	return unk_0xCE990E643CD9D0E5(uParam0->f_1497.f_1015, 7);
}

void func_780(var uParam0, bool bParam1)
{
	if (bParam1)
	{
		unk_0xBE20AB8238688965(&(uParam0->f_1497.f_1015), 4);
	}
	else
	{
		unk_0xD2459066EA58CE43(&(uParam0->f_1497.f_1015), 4);
	}
}

void func_781(var uParam0, var uParam1, bool bParam2)
{
	unk_0x830F007E19C63E14(uParam0, "ADD_OPTIONAL_LIST_ITEM");
	func_83(uParam1);
	unk_0xF1B28F753235CE2A(uParam2);
	unk_0xE1FDD153F5858534();
}

int func_782(var uParam0, int iParam1)
{
	switch (iParam1)
	{
		case 0:
			return func_764(uParam0->f_4356);
		
		case 1:
			return func_762(uParam0->f_4356);
		
		case 2:
			if (func_778(uParam0) == 2)
			{
				return uParam0->f_1497.f_1021 != 5;
			}
			break;
	}
	return 0;
}

char* func_783(var uParam0, int iParam1)
{
	switch (iParam1)
	{
		case 0:
			return "CH_OPT_FIN_0";
		
		case 1:
			return "CH_OPT_FIN_1";
		
		case 2:
			if (func_778(uParam0) == 2)
			{
				return "CH_OPT_FIN_2";
			}
			break;
	}
	return "";
}

void func_784(var uParam0)
{
	unk_0x903663B04BCCB2FB(uParam0, "CLEAR_OPTIONAL_LIST");
}

bool func_785(var uParam0)
{
	return unk_0xCE990E643CD9D0E5(uParam0->f_1497.f_1015, 4);
}

void func_786(var uParam0, bool bParam1)
{
	if (bParam1)
	{
		unk_0xBE20AB8238688965(&(uParam0->f_1497.f_1015), 3);
	}
	else
	{
		unk_0xD2459066EA58CE43(&(uParam0->f_1497.f_1015), 3);
	}
}

void func_787(var uParam0, var uParam1, bool bParam2)
{
	unk_0x830F007E19C63E14(uParam0, "ADD_TODO_LIST_ITEM");
	func_83(uParam1);
	unk_0xF1B28F753235CE2A(uParam2);
	unk_0xE1FDD153F5858534();
}

int func_788(var uParam0, int iParam1)
{
	switch (iParam1)
	{
		case 0:
			if (func_778(uParam0) == 2)
			{
				return uParam0->f_1497.f_1025 != 0;
			}
			return uParam0->f_1497.f_1022 != -1;
		
		case 1:
			return uParam0->f_1497.f_1023 != -1;
		
		case 2:
			return uParam0->f_1497.f_1024 != -1;
		
		case 3:
			if (unk_0xFC559366953F62B3())
			{
				return uParam0->f_1497.f_736.f_92[4] <= 0;
			}
			break;
	}
	return 0;
}

char* func_789(var uParam0, int iParam1)
{
	switch (iParam1)
	{
		case 0:
			if (func_778(uParam0) == 2)
			{
				return "CH_TODO_FIN_1";
			}
			return "CH_TODO_FIN_0";
		
		case 1:
			return "CH_TODO_FIN_2";
		
		case 2:
			return "CH_TODO_FIN_3";
		
		case 3:
			if (unk_0xFC559366953F62B3())
			{
				return "CH_TODO_FIN_4";
			}
			break;
	}
	return "";
}

void func_790(var uParam0)
{
	unk_0x903663B04BCCB2FB(uParam0, "CLEAR_TODO_LIST");
}

bool func_791(var uParam0)
{
	return unk_0xCE990E643CD9D0E5(uParam0->f_1497.f_1015, 3);
}

void func_792(var uParam0, bool bParam1)
{
	if (bParam1)
	{
		unk_0xBE20AB8238688965(&(uParam0->f_1497.f_1015), 9);
	}
	else
	{
		unk_0xD2459066EA58CE43(&(uParam0->f_1497.f_1015), 9);
	}
}

void func_793(var uParam0, char* sParam1)
{
	unk_0x830F007E19C63E14(uParam0, "SET_LAUNCH_BUTTON_LABEL");
	func_83(sParam1);
	unk_0xE1FDD153F5858534();
}

var func_794()
{
	return Global_1697305;
}

bool func_795(var uParam0)
{
	return unk_0xCE990E643CD9D0E5(uParam0->f_1497.f_1015, 9);
}

void func_796(var uParam0, bool bParam1)
{
	if (bParam1)
	{
		unk_0xBE20AB8238688965(&(uParam0->f_1497.f_1015), 5);
	}
	else
	{
		unk_0xD2459066EA58CE43(&(uParam0->f_1497.f_1015), 5);
	}
}

void func_797(var uParam0, int iParam1, int iParam2)
{
	unk_0x830F007E19C63E14(uParam0, "SET_CREW_CUT");
	unk_0xAD291B8F75D737AD(iParam1);
	unk_0xAD291B8F75D737AD(iParam2);
	unk_0xE1FDD153F5858534();
}

void func_798(var uParam0, int iParam1, bool bParam2, int iParam3)
{
	unk_0x830F007E19C63E14(uParam0, "SET_CREW_MEMBER_STATE");
	unk_0xAD291B8F75D737AD(iParam1);
	unk_0xF1B28F753235CE2A(bParam2);
	unk_0xAD291B8F75D737AD(iParam3);
	unk_0xE1FDD153F5858534();
}

int func_799(int iParam0)
{
	struct<13> Var0;
	
	if (iParam0 != func_42() && unk_0x166E920FB00B2DBB(iParam0))
	{
		Var0 = { func_97(iParam0) };
		if (func_607(Var0))
		{
			if (unk_0x4B9AEC757F715980(&Var0))
			{
				if (unk_0x30EE34228B1AF045(&Var0))
				{
					return 47;
				}
				else if (unk_0x7677D662EB0C9C8A(&Var0))
				{
					return 49;
				}
				else
				{
					return 48;
				}
			}
			else if (unk_0x7677D662EB0C9C8A(&Var0))
			{
				return 49;
			}
			else
			{
				return 0;
			}
		}
	}
	return 0;
}

int func_800(int iParam0)
{
	switch (iParam0)
	{
		case 0:
			return 8;
		
		case 1:
			return 9;
		
		case 2:
			return 10;
		
		case 3:
			return 11;
		
		default:
	}
	return -1;
}

void func_801(var uParam0, int iParam1, var uParam2, var uParam3)
{
	unk_0x830F007E19C63E14(uParam0, "SET_CREW_MEMBER");
	unk_0xAD291B8F75D737AD(iParam1);
	func_802(uParam2);
	unk_0x4A096F645CED26D9(uParam3);
	unk_0xE1FDD153F5858534();
}

void func_802(var uParam0)
{
	unk_0x54AED56A452DCB4F(uParam0);
}

int func_803(var uParam0, int iParam1)
{
	if (unk_0xFC559366953F62B3())
	{
		return 1;
	}
	else if (iParam1 == uParam0->f_4356)
	{
		return 1;
	}
	return 0;
}

int func_804(int iParam0)
{
	int iVar0;
	
	iVar0 = 8;
	iVar0 = (iVar0 + iParam0);
	return iVar0;
}

struct<6> func_805(var uParam0, int iParam1)
{
	struct<6> Var0;
	int iVar6;
	
	iVar6 = func_815(uParam0, iParam1);
	if (iVar6 == -1)
	{
		return Var0;
	}
	if (uParam0->f_1497.f_736.f_36[iVar6 /*10*/].f_9)
	{
		return uParam0->f_1497.f_736.f_36[iVar6 /*10*/].f_3;
	}
	if (func_807(uParam0, &(uParam0->f_1497.f_736.f_36[iVar6 /*10*/])))
	{
		if (unk_0xCE990E643CD9D0E5(uParam0->f_4357, 7))
		{
			if (func_806(uParam0))
			{
				Var0 = { uParam0->f_1497.f_736.f_36[iVar6 /*10*/].f_3 };
				if (!uParam0->f_1497.f_736.f_36[iVar6 /*10*/].f_9)
				{
					uParam0->f_1497.f_736.f_36[iVar6 /*10*/].f_9 = 1;
					func_796(uParam0, 1);
				}
			}
		}
		else
		{
			Var0 = { uParam0->f_1497.f_736.f_36[iVar6 /*10*/].f_3 };
		}
	}
	return Var0;
}

int func_806(var uParam0)
{
	int iVar0;
	int iVar1;
	
	iVar1 = 1;
	iVar0 = 0;
	while (iVar0 <= 3)
	{
		if (uParam0->f_1497.f_736.f_87[iVar0] == -1)
		{
		}
		else if (!func_203(uParam0->f_1497.f_736.f_36[iVar0 /*10*/], 1, 1))
		{
		}
		else if (uParam0->f_1497.f_736.f_36[iVar0 /*10*/].f_2 != 2)
		{
			iVar1 = 0;
		}
		else
		{
			iVar0++;
		}
	}
	return iVar1;
}

int func_807(var uParam0, var uParam1)
{
	char* sVar0;
	int iVar1;
	bool bVar2;
	
	bVar2 = false;
	switch (uParam1->f_2)
	{
		case 0:
			if (!func_814(uParam0))
			{
				uParam1->f_2 = 1;
			}
			break;
		
		case 1:
			if (unk_0x5AEB5DDFFAD43CA5(unk_0x1FA7B77001D60F9D(*uParam1)))
			{
				return 0;
			}
			if (uParam1->f_1 == 0)
			{
				uParam1->f_1 = func_808(*uParam1);
			}
			else
			{
				StringCopy(&(uParam1->f_3), unk_0x3EF2973864CBE5FC(uParam1->f_1), 24);
				iVar1 = 0;
				while (iVar1 < Global_1389382)
				{
					if (Global_1389221[iVar1 /*5*/].f_1 == *uParam1)
					{
						bVar2 = uParam1->f_1 == Global_1389221[iVar1 /*5*/].f_2;
					}
					else
					{
						iVar1++;
					}
				}
				if (!bVar2)
				{
					uParam1->f_1 = 0;
					return 0;
				}
				if (unk_0x7BCC91F3C1CF24E8(&(uParam1->f_3)) || unk_0xF005CCA4263DF67F(" ", &(uParam1->f_3)))
				{
					uParam1->f_1 = 0;
					return 0;
				}
				uParam1->f_2 = 2;
				func_796(uParam0, 1);
				return 1;
			}
			break;
		
		case 2:
			sVar0 = unk_0x3EF2973864CBE5FC(uParam1->f_1);
			StringCopy(&(uParam1->f_3), sVar0, 24);
			return 1;
	}
	return 0;
}

int func_808(int iParam0)
{
	int iVar0;
	
	iVar0 = func_811(iParam0);
	if (iVar0 == -1)
	{
		func_809(iParam0, 1);
		return 0;
	}
	Global_1389221[iVar0 /*5*/].f_4 = 1;
	return Global_1389221[iVar0 /*5*/].f_2;
}

void func_809(int iParam0, bool bParam1)
{
	if (!func_203(iParam0, 0, 1))
	{
		return;
	}
	if (func_811(iParam0) != -1)
	{
		return;
	}
	if (Global_1389384)
	{
		if (iParam0 == Global_1389384.f_1)
		{
			return;
		}
	}
	if (func_810(iParam0))
	{
		return;
	}
	if (Global_1389422 >= 32)
	{
		return;
	}
	Global_1389389[Global_1389422] = iParam0;
	Global_1389422++;
	if (bParam1)
	{
	}
}

int func_810(int iParam0)
{
	int iVar0;
	
	iVar0 = 0;
	iVar0 = 0;
	while (iVar0 < Global_1389422)
	{
		if (Global_1389389[iVar0] == iParam0)
		{
			return 1;
		}
		iVar0++;
	}
	return 0;
}

int func_811(int iParam0)
{
	int iVar0;
	
	if (!func_203(iParam0, 0, 1))
	{
		return -1;
	}
	if (Global_1389382 == 0)
	{
		return -1;
	}
	iVar0 = 0;
	iVar0 = 0;
	while (iVar0 < Global_1389382)
	{
		if (Global_1389221[iVar0 /*5*/].f_1 == iParam0)
		{
			if (unk_0x3F2B70A051A32606(Global_1389221[iVar0 /*5*/].f_2) && unk_0xCA635244223E5D25(Global_1389221[iVar0 /*5*/].f_2))
			{
				return iVar0;
			}
			func_812(iVar0);
			return -1;
		}
		iVar0++;
	}
	return -1;
}

void func_812(int iParam0)
{
	char cVar0[64];
	char cVar16[64];
	int iVar32;
	int iVar33;
	
	if (iParam0 >= Global_1389382)
	{
		return;
	}
	if (unk_0x3F2B70A051A32606(Global_1389221[iParam0 /*5*/].f_2))
	{
		StringCopy(&cVar0, "CHAR_DEFAULT", 64);
		if (Global_1389221[iParam0 /*5*/].f_2 != 0)
		{
			StringCopy(&cVar16, unk_0x3EF2973864CBE5FC(Global_1389221[iParam0 /*5*/].f_2), 64);
			unk_0xC16699C9E79C9231(&cVar16, &cVar16, &cVar0, &cVar0);
		}
		unk_0x709B7208105825BB(Global_1389221[iParam0 /*5*/].f_2);
	}
	iVar32 = iParam0;
	iVar33 = iVar32 + 1;
	while (iVar33 < Global_1389382)
	{
		Global_1389221[iVar32 /*5*/] = { Global_1389221[iVar33 /*5*/] };
		iVar32++;
		iVar33++;
	}
	func_813(&(Global_1389221[iVar32 /*5*/]));
	Global_1389382 = (Global_1389382 - 1);
}

void func_813(var uParam0)
{
	*uParam0 = 0;
	uParam0->f_1 = func_42();
	uParam0->f_2 = 0;
	uParam0->f_4 = 0;
	if (unk_0x25DDB354A40FFCDB())
	{
		uParam0->f_3 = unk_0x6CAAB7E78B5D978A();
	}
}

int func_814(var uParam0)
{
	int iVar0;
	
	iVar0 = 0;
	while (iVar0 <= 3)
	{
		if (uParam0->f_1497.f_736.f_36[iVar0 /*10*/].f_2 == 1)
		{
			return 1;
		}
		iVar0++;
	}
	return 0;
}

int func_815(var uParam0, int iParam1)
{
	int iVar0;
	
	if (iParam1 == func_42())
	{
		return -1;
	}
	iVar0 = 0;
	while (iVar0 < 4)
	{
		if (uParam0->f_1497.f_736.f_36[iVar0 /*10*/] == iParam1)
		{
			return iVar0;
		}
		iVar0++;
	}
	iVar0 = 0;
	while (iVar0 < 4)
	{
		if (uParam0->f_1497.f_736.f_36[iVar0 /*10*/] == func_42())
		{
			uParam0->f_1497.f_736.f_36[iVar0 /*10*/] = iParam1;
			uParam0->f_1497.f_736.f_36[iVar0 /*10*/].f_2 = 0;
			return iVar0;
		}
		iVar0++;
	}
	return -1;
}

bool func_816(var uParam0)
{
	return unk_0xCE990E643CD9D0E5(uParam0->f_1497.f_1015, 5);
}

void func_817(var uParam0)
{
	int iVar0;
	int iVar1;
	int iVar2;
	struct<13> Var3;
	
	if ((((((func_17(uParam0, 1) || uParam0->f_2604) || uParam0->f_1497.f_1027 != -1) || uParam0->f_1497.f_1029) || uParam0->f_1497.f_708) || func_16(uParam0)) || (func_15() && func_14() != 33))
	{
		return;
	}
	iVar0 = func_857(&(uParam0->f_2604));
	iVar1 = uParam0->f_2593;
	switch (uParam0->f_1497.f_724.f_2)
	{
		case 0:
			switch (iVar0)
			{
				case 0:
				case 1:
				case 2:
				case 3:
					func_855(uParam0, iVar0);
					func_196(uParam0, 1);
					func_854();
					func_853(uParam0);
					break;
				
				case 9:
					if (!unk_0xFC559366953F62B3())
					{
						func_850(uParam0, 1);
					}
					break;
				
				case 10:
					break;
				
				case 7:
					if (uParam0->f_4356 == unk_0x460153A63B9477BC())
					{
						if (func_849(iVar1))
						{
							if (unk_0xFC559366953F62B3())
							{
								func_846(uParam0);
								unk_0xC4CC25B68A91D144(-1, "Highlight_Accept", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
							}
							else
							{
								func_845(uParam0);
								unk_0xC4CC25B68A91D144(-1, "Highlight_Accept", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
							}
						}
						if (func_844(uParam0, iVar1))
						{
							func_841(uParam0, iVar1);
						}
						if (iVar1 == 2 && func_113(uParam0->f_4356) == 2)
						{
							func_189(uParam0, 15);
						}
					}
					else if (unk_0xFC559366953F62B3())
					{
						func_840(uParam0);
						unk_0xC4CC25B68A91D144(-1, "Highlight_Accept", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
					}
					break;
				
				case 4:
					if (uParam0->f_4356 == unk_0x460153A63B9477BC())
					{
						if (func_839(uParam0, iVar1))
						{
							uParam0->f_1497.f_1027 = func_838(iVar1);
							unk_0xC4CC25B68A91D144(-1, "Highlight_Accept", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
							func_854();
						}
					}
					break;
				
				case 8:
					if (unk_0xFC559366953F62B3())
					{
						uParam0->f_1497.f_1027 = 14;
						unk_0xC4CC25B68A91D144(-1, "Highlight_Accept", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
						func_854();
					}
					else
					{
						func_201(uParam0, 0);
					}
					break;
				
				case 5:
					if (func_117(uParam0))
					{
						func_836(uParam0, 0);
					}
					break;
				
				case 11:
					if (func_835(iVar1))
					{
						func_854();
						iVar2 = unk_0xF1110FE23C67293A(uParam0->f_1497.f_736.f_87[func_834(iVar1)]);
						Var3 = { func_97(iVar2) };
						unk_0xC4CC25B68A91D144(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 1);
						unk_0x95B0B37E5A65C8E6(&Var3);
					}
					break;
			}
			break;
		
		case 1:
			switch (iVar0)
			{
				case 2:
				case 3:
					func_820(uParam0, iVar1, iVar0);
					break;
				
				case 7:
					if (unk_0xFC559366953F62B3())
					{
						func_818(uParam0, iVar1, 1);
					}
					break;
				
				case 8:
					func_818(uParam0, iVar1, 0);
					break;
				
				case 5:
					if (func_117(uParam0))
					{
						func_836(uParam0, 0);
					}
					break;
			}
			break;
	}
}

void func_818(var uParam0, int iParam1, bool bParam2)
{
	int iVar0;
	
	if (uParam0->f_1497.f_724.f_2 == 0)
	{
		return;
	}
	uParam0->f_1497.f_724.f_2 = 0;
	uParam0->f_1497.f_706 = 1;
	func_756(uParam0->f_1497.f_709, iParam1, 0);
	func_190(&(uParam0->f_1497), 1);
	func_786(uParam0, 1);
	func_780(uParam0, 1);
	if (bParam2)
	{
		unk_0xC4CC25B68A91D144(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 1);
		if (unk_0xFC559366953F62B3())
		{
			Global_1701669[unk_0x1EF8FCAB83E4BE53() /*68*/]++;
			switch (iParam1)
			{
				case 2:
					uParam0->f_1497.f_1022 = uParam0->f_1497.f_1017;
					switch (uParam0->f_1497.f_1017)
					{
						case 3:
						case 4:
						case 5:
						case 6:
						case 7:
						case 8:
							func_191(uParam0, 49);
							break;
					}
					unk_0xBE20AB8238688965(&(Global_1701669[unk_0x1EF8FCAB83E4BE53() /*68*/].f_1), 0);
					break;
				
				case 3:
					uParam0->f_1497.f_1023 = uParam0->f_1497.f_1018;
					unk_0xBE20AB8238688965(&(Global_1701669[unk_0x1EF8FCAB83E4BE53() /*68*/].f_1), 1);
					break;
				
				case 4:
					uParam0->f_1497.f_1024 = uParam0->f_1497.f_1019;
					unk_0xBE20AB8238688965(&(Global_1701669[unk_0x1EF8FCAB83E4BE53() /*68*/].f_1), 2);
					break;
				
				case 13:
					uParam0->f_1497.f_1025 = uParam0->f_1497.f_1020;
					if (func_113(uParam0->f_4356) == 2)
					{
						if (uParam0->f_1497.f_1017 != func_819(uParam0))
						{
							uParam0->f_1497.f_1017 = func_819(uParam0);
							uParam0->f_1497.f_1022 = uParam0->f_1497.f_1017;
							unk_0xBE20AB8238688965(&(Global_1701669[unk_0x1EF8FCAB83E4BE53() /*68*/].f_1), 0);
							func_755(uParam0, 1);
							func_769(uParam0, 1);
						}
					}
					unk_0xBE20AB8238688965(&(Global_1701669[unk_0x1EF8FCAB83E4BE53() /*68*/].f_1), 3);
					break;
				
				case 14:
					uParam0->f_1497.f_1026 = uParam0->f_1497.f_1021;
					unk_0xBE20AB8238688965(&(Global_1701669[unk_0x1EF8FCAB83E4BE53() /*68*/].f_1), 4);
					break;
				}
		}
	}
	else
	{
		unk_0xC4CC25B68A91D144(-1, "BACK", "HUD_FRONTEND_DEFAULT_SOUNDSET", 1);
		if (func_835(iParam1))
		{
			iVar0 = 0;
			while (iVar0 <= 4)
			{
				if (iVar0 >= 4)
				{
					uParam0->f_1497.f_736.f_92[4] = uParam0->f_1497.f_736.f_264[4];
				}
				else
				{
					uParam0->f_1497.f_736.f_92[iVar0] = uParam0->f_1497.f_736.f_264[iVar0];
				}
				iVar0++;
			}
		}
		else
		{
			switch (iParam1)
			{
				case 2:
					if (uParam0->f_1497.f_1022 != uParam0->f_1497.f_1017)
					{
						uParam0->f_1497.f_1017 = uParam0->f_1497.f_1022;
						func_755(uParam0, 1);
					}
					break;
				
				case 3:
					if (uParam0->f_1497.f_1023 != uParam0->f_1497.f_1018)
					{
						uParam0->f_1497.f_1018 = uParam0->f_1497.f_1023;
						func_752(uParam0, 1);
					}
					break;
				
				case 4:
					if (uParam0->f_1497.f_1024 != uParam0->f_1497.f_1019)
					{
						uParam0->f_1497.f_1019 = uParam0->f_1497.f_1024;
						func_747(uParam0, 1);
					}
					break;
				
				case 13:
					if (uParam0->f_1497.f_1025 != uParam0->f_1497.f_1020)
					{
						uParam0->f_1497.f_1020 = uParam0->f_1497.f_1025;
						func_743(uParam0, 1);
					}
					break;
				
				case 14:
					if (uParam0->f_1497.f_1026 != uParam0->f_1497.f_1021)
					{
						uParam0->f_1497.f_1021 = uParam0->f_1497.f_1026;
						func_739(uParam0, 1);
					}
					break;
			}
			func_769(uParam0, 1);
		}
	}
	if (uParam0->f_2529.f_33 != 2)
	{
		uParam0->f_2529.f_33 = 2;
	}
}

int func_819(var uParam0)
{
	switch (uParam0->f_1497.f_1020)
	{
		case 1:
			return 1;
		
		case 2:
			return 2;
		
		case 3:
			return 9;
		
		case 4:
			return 0;
		
		default:
	}
	return -1;
}

void func_820(var uParam0, int iParam1, int iParam2)
{
	int iVar0;
	int iVar1;
	int iVar2;
	
	switch (iParam1)
	{
		case 2:
			switch (iParam2)
			{
				case 2:
					if (func_832(uParam0, 0))
					{
						unk_0xC4CC25B68A91D144(-1, "Highlight_Move", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
						func_755(uParam0, 1);
						func_769(uParam0, 1);
					}
					break;
				
				case 3:
					if (func_832(uParam0, 1))
					{
						unk_0xC4CC25B68A91D144(-1, "Highlight_Move", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
						func_755(uParam0, 1);
						func_769(uParam0, 1);
					}
					break;
			}
			break;
		
		case 3:
			switch (iParam2)
			{
				case 2:
					if (func_830(uParam0, 0))
					{
						unk_0xC4CC25B68A91D144(-1, "Highlight_Move", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
						func_752(uParam0, 1);
						func_769(uParam0, 1);
					}
					break;
				
				case 3:
					if (func_830(uParam0, 1))
					{
						unk_0xC4CC25B68A91D144(-1, "Highlight_Move", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
						func_752(uParam0, 1);
						func_769(uParam0, 1);
					}
					break;
			}
			break;
		
		case 4:
			switch (iParam2)
			{
				case 2:
					if (func_828(uParam0, 0))
					{
						unk_0xC4CC25B68A91D144(-1, "Highlight_Move", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
						func_747(uParam0, 1);
						func_769(uParam0, 1);
					}
					break;
				
				case 3:
					if (func_828(uParam0, 1))
					{
						unk_0xC4CC25B68A91D144(-1, "Highlight_Move", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
						func_747(uParam0, 1);
						func_769(uParam0, 1);
					}
					break;
			}
			break;
		
		case 13:
			switch (iParam2)
			{
				case 2:
					if (func_826(uParam0, 0))
					{
						unk_0xC4CC25B68A91D144(-1, "Highlight_Move", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
						func_743(uParam0, 1);
						func_769(uParam0, 1);
					}
					break;
				
				case 3:
					if (func_826(uParam0, 1))
					{
						unk_0xC4CC25B68A91D144(-1, "Highlight_Move", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
						func_743(uParam0, 1);
						func_769(uParam0, 1);
					}
					break;
			}
			break;
		
		case 14:
			switch (iParam2)
			{
				case 2:
					if (func_824(uParam0, 0))
					{
						unk_0xC4CC25B68A91D144(-1, "Highlight_Move", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
						func_739(uParam0, 1);
						func_769(uParam0, 1);
					}
					break;
				
				case 3:
					if (func_824(uParam0, 1))
					{
						unk_0xC4CC25B68A91D144(-1, "Highlight_Move", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
						func_739(uParam0, 1);
						func_769(uParam0, 1);
					}
					break;
			}
			break;
		
		case 8:
		case 9:
		case 10:
		case 11:
			switch (iParam2)
			{
				case 2:
					iVar0 = -5;
					break;
				
				case 3:
					if (uParam0->f_1497.f_736.f_92[4] >= 5)
					{
						iVar0 = 5;
					}
					break;
			}
			iVar1 = func_834(iParam1);
			func_821(uParam0, iVar1, iVar0);
			iVar2 = 0;
			while (iVar2 <= 4)
			{
				iVar2++;
			}
			break;
	}
}

void func_821(var uParam0, int iParam1, int iParam2)
{
	int iVar0;
	int iVar1;
	int iVar2;
	int iVar3;
	int iVar4;
	int iVar5;
	
	iVar0 = 15;
	iVar1 = 85;
	if (uParam0->f_1497.f_736.f_85 == 1)
	{
		iVar1 = 100;
	}
	if (iParam2 < 0)
	{
		iVar2 = uParam0->f_1497.f_736.f_92[iParam1];
		if (iVar2 < unk_0x44F29CF6A0675F4F(iParam2))
		{
			iParam2 = iVar2;
		}
		if (uParam0->f_1497.f_736.f_92[iParam1] < iVar0)
		{
			iVar3 = (iVar0 - uParam0->f_1497.f_736.f_92[iParam1]);
			uParam0->f_1497.f_736.f_92[iParam1] = (uParam0->f_1497.f_736.f_92[iParam1] + iVar3);
			unk_0xC4CC25B68A91D144(-1, "Highlight_Error", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
		}
		else if (uParam0->f_1497.f_736.f_92[iParam1] == iVar0)
		{
			unk_0xC4CC25B68A91D144(-1, "Highlight_Error", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
		}
		else
		{
			if (uParam0->f_1497.f_736.f_92[4] <= (100 - unk_0x44F29CF6A0675F4F(iParam2)))
			{
				uParam0->f_1497.f_736.f_92[4] = (uParam0->f_1497.f_736.f_92[4] + unk_0x44F29CF6A0675F4F(iParam2));
				uParam0->f_1497.f_736.f_92[iParam1] = (uParam0->f_1497.f_736.f_92[iParam1] - unk_0x44F29CF6A0675F4F(iParam2));
			}
			if (iParam2 == 0)
			{
				unk_0x057194D526464DC0(-1, "Error", func_823(), func_822(), 0, 0, 0);
			}
			else
			{
				unk_0xC4CC25B68A91D144(-1, "Highlight_Move", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
			}
		}
		if (uParam0->f_1497.f_736.f_92[iParam1] <= iVar0)
		{
			if (!func_227("HEIST_NOTE_6"))
			{
				unk_0xD289B55B6AADBA10(1);
				func_10("HEIST_NOTE_6", 5000);
			}
		}
	}
	else
	{
		iVar4 = uParam0->f_1497.f_736.f_92[4];
		if (iVar4 < unk_0x44F29CF6A0675F4F(iParam2))
		{
			iParam2 = iVar4;
		}
		if (uParam0->f_1497.f_736.f_92[iParam1] > iVar1)
		{
			iVar5 = (uParam0->f_1497.f_736.f_92[iParam1] - iVar1);
			uParam0->f_1497.f_736.f_92[4] = (uParam0->f_1497.f_736.f_92[4] + iVar5);
			unk_0xC4CC25B68A91D144(-1, "Highlight_Error", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
		}
		else if (uParam0->f_1497.f_736.f_92[iParam1] == iVar1)
		{
			unk_0xC4CC25B68A91D144(-1, "Highlight_Error", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
		}
		else
		{
			if (uParam0->f_1497.f_736.f_92[4] >= unk_0x44F29CF6A0675F4F(iParam2))
			{
				uParam0->f_1497.f_736.f_92[4] = (uParam0->f_1497.f_736.f_92[4] - unk_0x44F29CF6A0675F4F(iParam2));
				uParam0->f_1497.f_736.f_92[iParam1] = (uParam0->f_1497.f_736.f_92[iParam1] + unk_0x44F29CF6A0675F4F(iParam2));
			}
			if (iParam2 == 0)
			{
				unk_0xC4CC25B68A91D144(-1, "Highlight_Error", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
			}
			else
			{
				unk_0xC4CC25B68A91D144(-1, "Highlight_Move", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
			}
		}
		if (uParam0->f_1497.f_736.f_92[iParam1] >= iVar1)
		{
			if (!func_227("HEIST_NOTE_5"))
			{
				unk_0xD289B55B6AADBA10(1);
				func_10("HEIST_NOTE_5", 5000);
			}
		}
	}
}

char* func_822()
{
	return "DLC_XM17_Planning_Board_Soundset";
}

Vector3 func_823()
{
	return 353.793f, 4877.004f, -58.4718f;
}

int func_824(var uParam0, bool bParam1)
{
	bool bVar0;
	
	while (!bVar0)
	{
		if (bParam1)
		{
			uParam0->f_1497.f_1021++;
			if (uParam0->f_1497.f_1021 > 8)
			{
				uParam0->f_1497.f_1021 = 5;
			}
		}
		else
		{
			uParam0->f_1497.f_1021 = (uParam0->f_1497.f_1021 - 1);
			if (uParam0->f_1497.f_1021 < 5)
			{
				uParam0->f_1497.f_1021 = 8;
			}
		}
		if (func_825(uParam0))
		{
			bVar0 = true;
			return 1;
		}
	}
	return 0;
}

int func_825(var uParam0)
{
	int iVar0;
	
	iVar0 = uParam0->f_1497.f_1021;
	switch (iVar0)
	{
		case 6:
		case 7:
		case 8:
			return func_173(uParam0->f_4356, iVar0);
		
		default:
	}
	return 0;
}

int func_826(var uParam0, bool bParam1)
{
	bool bVar0;
	
	while (!bVar0)
	{
		if (bParam1)
		{
			uParam0->f_1497.f_1020++;
			if (uParam0->f_1497.f_1020 > 4)
			{
				uParam0->f_1497.f_1020 = 0;
			}
		}
		else
		{
			uParam0->f_1497.f_1020 = (uParam0->f_1497.f_1020 - 1);
			if (uParam0->f_1497.f_1020 < 0)
			{
				uParam0->f_1497.f_1020 = 4;
			}
		}
		if (func_827(uParam0))
		{
			bVar0 = true;
			return 1;
		}
	}
	return 0;
}

int func_827(var uParam0)
{
	int iVar0;
	
	iVar0 = uParam0->f_1497.f_1020;
	switch (iVar0)
	{
		case 1:
		case 2:
		case 3:
		case 4:
			return func_173(uParam0->f_4356, iVar0);
		
		default:
	}
	return 0;
}

int func_828(var uParam0, bool bParam1)
{
	bool bVar0;
	
	while (!bVar0)
	{
		if (bParam1)
		{
			uParam0->f_1497.f_1019++;
			if (uParam0->f_1497.f_1019 > 8)
			{
				uParam0->f_1497.f_1019 = -1;
			}
		}
		else
		{
			uParam0->f_1497.f_1019 = (uParam0->f_1497.f_1019 - 1);
			if (uParam0->f_1497.f_1019 < -1)
			{
				uParam0->f_1497.f_1019 = 8;
			}
		}
		if (func_829(uParam0))
		{
			bVar0 = true;
			return 1;
		}
	}
	return 0;
}

int func_829(var uParam0)
{
	int iVar0;
	
	iVar0 = uParam0->f_1497.f_1019;
	switch (iVar0)
	{
		case 0:
		case 3:
		case 6:
			return 1;
		
		default:
	}
	return 0;
}

int func_830(var uParam0, bool bParam1)
{
	bool bVar0;
	
	while (!bVar0)
	{
		if (bParam1)
		{
			uParam0->f_1497.f_1018++;
			if (uParam0->f_1497.f_1018 > 10)
			{
				uParam0->f_1497.f_1018 = -1;
			}
		}
		else
		{
			uParam0->f_1497.f_1018 = (uParam0->f_1497.f_1018 - 1);
			if (uParam0->f_1497.f_1018 < -1)
			{
				uParam0->f_1497.f_1018 = 10;
			}
		}
		if (func_831(uParam0))
		{
			bVar0 = true;
			return 1;
		}
	}
	return 0;
}

int func_831(var uParam0)
{
	int iVar0;
	
	iVar0 = uParam0->f_1497.f_1018;
	if (iVar0 != -1)
	{
		if (!func_179(uParam0->f_4356, iVar0))
		{
			return 0;
		}
	}
	else
	{
		return 0;
	}
	switch (func_113(uParam0->f_4356))
	{
		case 1:
			switch (iVar0)
			{
				case 0:
				case 10:
				case 9:
					return 0;
				
				default:
			}
			break;
		
		case 2:
			switch (iVar0)
			{
				case 0:
				case 10:
				case 9:
					return 0;
				
				default:
			}
			break;
		
		case 3:
			switch (iVar0)
			{
				case 10:
				case 9:
					return 0;
				
				default:
			}
			break;
	}
	return 1;
}

int func_832(var uParam0, bool bParam1)
{
	bool bVar0;
	
	while (!bVar0)
	{
		if (bParam1)
		{
			uParam0->f_1497.f_1017++;
			if (uParam0->f_1497.f_1017 > 10)
			{
				uParam0->f_1497.f_1017 = -1;
			}
		}
		else
		{
			uParam0->f_1497.f_1017 = (uParam0->f_1497.f_1017 - 1);
			if (uParam0->f_1497.f_1017 < -1)
			{
				uParam0->f_1497.f_1017 = 10;
			}
		}
		if (func_833(uParam0))
		{
			bVar0 = true;
			return 1;
		}
	}
	return 0;
}

int func_833(var uParam0)
{
	int iVar0;
	
	iVar0 = uParam0->f_1497.f_1017;
	if (iVar0 != -1)
	{
		if (!func_179(uParam0->f_4356, iVar0))
		{
			return 0;
		}
	}
	else
	{
		return 0;
	}
	switch (func_113(uParam0->f_4356))
	{
		case 1:
			switch (iVar0)
			{
				case 0:
				case 10:
				case 9:
					return 0;
				
				default:
			}
			break;
		
		case 2:
			switch (uParam0->f_1497.f_1020)
			{
				case 1:
					if (iVar0 != 1)
					{
						return 0;
					}
					break;
				
				case 2:
					if (iVar0 != 2)
					{
						return 0;
					}
					break;
				
				case 3:
					if (iVar0 != 9)
					{
						return 0;
					}
					break;
				
				case 4:
					if (iVar0 != 0)
					{
						return 0;
					}
					break;
			}
			break;
		
		case 3:
			switch (iVar0)
			{
				case 9:
					return 0;
				
				default:
			}
			break;
	}
	return 1;
}

int func_834(int iParam0)
{
	switch (iParam0)
	{
		case 8:
			return 0;
		
		case 9:
			return 1;
		
		case 10:
			return 2;
		
		case 11:
			return 3;
		
		default:
	}
	return -1;
}

int func_835(int iParam0)
{
	switch (iParam0)
	{
		case 8:
		case 9:
		case 10:
		case 11:
			return 1;
		
		default:
	}
	return 0;
}

void func_836(var uParam0, bool bParam1)
{
	uParam0->f_4361 = !uParam0->f_4361;
	switch (uParam0->f_4355)
	{
		case 0:
			func_190(uParam0, 1);
			break;
		
		case 1:
			func_190(&(uParam0->f_744), 1);
			break;
		
		case 2:
			func_190(&(uParam0->f_1497), 1);
			break;
	}
	if ((uParam0->f_4361 && uParam0->f_4356 == unk_0x460153A63B9477BC()) && !bParam1)
	{
		func_191(uParam0, func_837(uParam0));
	}
}

int func_837(var uParam0)
{
	switch (uParam0->f_4355)
	{
		case 1:
			switch (uParam0->f_2593)
			{
				case 3:
					return 27;
				
				case 4:
					return 28;
				
				case 9:
					return 31;
				
				case 14:
					switch (func_113(uParam0->f_4356))
					{
						case 1:
							return 22;
						
						default:
					}
					break;
				
				case 15:
					switch (func_113(uParam0->f_4356))
					{
						case 1:
							return 20;
						
						case 2:
							return 23;
						
						case 3:
							return 25;
						
						default:
					}
					break;
				
				case 16:
					switch (func_113(uParam0->f_4356))
					{
						case 1:
							return 21;
						
						case 2:
							if (unk_0xCE990E643CD9D0E5(Global_1701669[uParam0->f_4356 /*68*/].f_42, 6))
							{
								return 18;
							}
							break;
					}
					break;
				
				case 17:
					switch (func_113(uParam0->f_4356))
					{
						case 1:
							return 21;
						
						case 2:
							return 24;
						
						default:
					}
					break;
				
				case 18:
					return 19;
			}
			break;
		
		case 2:
			if (unk_0xFC559366953F62B3())
			{
				if (func_193(uParam0->f_4356, 0))
				{
					switch (uParam0->f_2593)
					{
						case 6:
							return 71;
						
						case 7:
							return 72;
						
						default:
					}
				}
				else
				{
					switch (uParam0->f_2593)
					{
						case 6:
							return 56;
						
						case 7:
							return 57;
						}
					}
				
				default:
			}
			break;
	}
	return -1;
}

int func_838(int iParam0)
{
	switch (iParam0)
	{
		case 6:
			if (unk_0x6914A85D2E17013B(func_110(iParam0), 0, 1, 0, -1, 0))
			{
				return 15;
			}
			else
			{
				return 16;
			}
			break;
		
		case 7:
			if (unk_0x6914A85D2E17013B(func_110(iParam0), 0, 1, 0, -1, 0))
			{
				return 17;
			}
			else
			{
				return 18;
			}
			break;
	}
	return -1;
}

int func_839(var uParam0, int iParam1)
{
	switch (iParam1)
	{
		case 6:
			return !func_764(uParam0->f_4356);
		
		case 7:
			return !func_762(uParam0->f_4356);
		
		default:
	}
	return 0;
}

void func_840(var uParam0)
{
	int iVar0;
	
	iVar0 = 0;
	while (iVar0 <= 3)
	{
		if (unk_0xF1110FE23C67293A(uParam0->f_1497.f_736.f_87[iVar0]) != unk_0x460153A63B9477BC())
		{
		}
		else if (uParam0->f_1497.f_736.f_103[iVar0] == 1)
		{
			uParam0->f_1497.f_736.f_103[iVar0] = 0;
		}
		else
		{
			uParam0->f_1497.f_736.f_103[iVar0] = 1;
		}
		iVar0++;
	}
	func_190(&(uParam0->f_1497), 1);
}

void func_841(var uParam0, int iParam1)
{
	int iVar0;
	
	if (uParam0->f_1497.f_724.f_2 == 1)
	{
		return;
	}
	if (func_835(iParam1))
	{
		iVar0 = 0;
		while (iVar0 <= 4)
		{
			if (iVar0 >= 4)
			{
				uParam0->f_1497.f_736.f_264[4] = uParam0->f_1497.f_736.f_92[4];
			}
			else
			{
				uParam0->f_1497.f_736.f_264[iVar0] = uParam0->f_1497.f_736.f_92[iVar0];
			}
			iVar0++;
		}
	}
	else
	{
		switch (iParam1)
		{
			case 2:
				uParam0->f_1497.f_1022 = uParam0->f_1497.f_1017;
				break;
			
			case 3:
				uParam0->f_1497.f_1023 = uParam0->f_1497.f_1018;
				break;
			
			case 4:
				uParam0->f_1497.f_1024 = uParam0->f_1497.f_1019;
				break;
			
			case 13:
				uParam0->f_1497.f_1025 = uParam0->f_1497.f_1020;
				break;
			
			case 14:
				uParam0->f_1497.f_1026 = uParam0->f_1497.f_1021;
				break;
			}
	}
	uParam0->f_1497.f_724.f_2 = 1;
	uParam0->f_1497.f_706 = 1;
	func_756(uParam0->f_1497.f_709, iParam1, 1);
	func_190(&(uParam0->f_1497), 1);
	if (uParam0->f_4356 == unk_0x460153A63B9477BC())
	{
		func_191(uParam0, func_843(uParam0));
	}
	if (!unk_0xFC559366953F62B3())
	{
		func_189(uParam0, 12);
	}
	else if (iParam1 == 4)
	{
		func_189(uParam0, 13);
	}
	else if (iParam1 == 2 || iParam1 == 3)
	{
		func_189(uParam0, 14);
	}
	else if (iParam1 == 13)
	{
		func_189(uParam0, 15);
	}
	if (uParam0->f_2529.f_33 != func_842(uParam0))
	{
		uParam0->f_2529.f_33 = func_842(uParam0);
	}
	unk_0xC4CC25B68A91D144(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 1);
}

int func_842(var uParam0)
{
	switch (uParam0->f_4355)
	{
		case 0:
			switch (uParam0->f_2593)
			{
				case 11:
				case 12:
				case 13:
				case 14:
				case 15:
				case 16:
					return 9;
				
				default:
			}
			return 0;
		
		case 1:
			switch (uParam0->f_2593)
			{
				case 10:
					return 10;
				
				case 11:
					return 10;
				
				case 12:
					return 10;
				
				case 5:
					return 10;
				
				case 6:
					return 10;
				
				case 13:
					return 16;
				
				case 14:
					switch (func_113(uParam0->f_4356))
					{
						case 2:
							return 17;
						
						default:
					}
					break;
				
				case 17:
					switch (func_113(uParam0->f_4356))
					{
						case 2:
							return 18;
						
						default:
					}
					break;
			}
			return 1;
		
		case 2:
			switch (uParam0->f_2593)
			{
				case 2:
					return 19;
				
				case 3:
					return 20;
				
				case 4:
					return 21;
				
				case 13:
					return 22;
				
				case 14:
					return 23;
				
				default:
			}
			return 2;
		
		default:
	}
	return -1;
}

int func_843(var uParam0)
{
	switch (uParam0->f_4355)
	{
		case 1:
			switch (uParam0->f_2593)
			{
				case 5:
					return 29;
				
				case 6:
					return 30;
				
				case 13:
					return 17;
				
				default:
			}
			break;
		
		case 2:
			if (unk_0xFC559366953F62B3())
			{
				switch (uParam0->f_2593)
				{
					case 2:
						return 48;
					
					case 3:
						return 50;
					
					case 4:
						return 55;
					
					case 13:
						return 52;
					
					case 14:
						return 53;
					}
				
				default:
			}
			break;
	}
	return -1;
}

int func_844(var uParam0, int iParam1)
{
	if (func_835(iParam1))
	{
		return 1;
	}
	switch (iParam1)
	{
		case 2:
			if (func_113(uParam0->f_4356) == 2)
			{
				return 0;
			}
			return 1;
		
		case 3:
		case 4:
			return 1;
		
		case 13:
			if (!unk_0xFC559366953F62B3())
			{
				return func_744(uParam0);
			}
			return 1;
		
		case 14:
			if (!unk_0xFC559366953F62B3())
			{
				return func_740(uParam0);
			}
			return 1;
		
		default:
	}
	return 0;
}

void func_845(var uParam0)
{
	if (uParam0->f_4356 != unk_0x460153A63B9477BC())
	{
		return;
	}
	if (func_674(func_676(func_113(uParam0->f_4356))) == -1)
	{
		return;
	}
	func_442(uParam0, 3);
	func_201(uParam0, 0);
	uParam0->f_1497.f_703 = 0;
}

void func_846(var uParam0)
{
	if (uParam0->f_4356 != unk_0x460153A63B9477BC())
	{
		return;
	}
	if (uParam0->f_1497.f_736.f_276 == 0)
	{
		func_848(uParam0);
		return;
	}
	func_854();
	uParam0->f_1497.f_736.f_103[0] = 1;
	Global_1701669[unk_0x1EF8FCAB83E4BE53() /*68*/].f_7[0] = 1;
	func_847(uParam0);
}

void func_847(var uParam0)
{
	uParam0->f_1497.f_1019 = (uParam0->f_1497.f_1019 + unk_0x344C570D6F8700DF(0, 3));
	Global_1701669[unk_0x460153A63B9477BC() /*68*/].f_18.f_17 = uParam0->f_1497.f_1019;
}

void func_848(var uParam0)
{
	switch (uParam0->f_1497.f_736.f_278)
	{
		case 0:
			if (!func_227("CH_HELP_LBDR0"))
			{
				func_10("CH_HELP_LBDR0", -1);
			}
			break;
		
		case 1:
			if (!func_227("CH_HELP_LBDR1"))
			{
				func_10("CH_HELP_LBDR1", -1);
			}
			break;
		
		case 2:
			if (!func_227("CH_HELP_LBDR2"))
			{
				func_10("CH_HELP_LBDR2", -1);
			}
			break;
		
		case 3:
			if (!func_227("CH_HELP_LBDR3"))
			{
				func_10("CH_HELP_LBDR3", -1);
			}
			break;
		
		case 4:
			if (!func_227("CH_HELP_LBDR4"))
			{
				func_10("CH_HELP_LBDR4", -1);
			}
			break;
		
		case 5:
			if (!func_227("CH_HELP_LBDR5"))
			{
				func_10("CH_HELP_LBDR5", -1);
			}
			break;
		
		case 6:
			if (!func_227("CH_HELP_LBDR6"))
			{
				func_10("CH_HELP_LBDR6", -1);
			}
			break;
	}
}

int func_849(int iParam0)
{
	switch (iParam0)
	{
		case 12:
			return 1;
		
		default:
	}
	return 0;
}

void func_850(var uParam0, int iParam1)
{
	func_442(uParam0, 1);
	func_201(uParam0, 0);
	func_222(uParam0, iParam1);
	func_201(uParam0, 1);
	func_851(uParam0, func_852(iParam1));
	unk_0xC4CC25B68A91D144(-1, "Highlight_Move", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
}

void func_851(var uParam0, int iParam1)
{
	uParam0->f_2529.f_33 = iParam1;
}

int func_852(int iParam0)
{
	switch (iParam0)
	{
		case 0:
			return 0;
		
		case 1:
			return 1;
		
		case 2:
			return 2;
		
		default:
	}
	return -1;
}

void func_853(var uParam0)
{
	if (uParam0->f_4361)
	{
		uParam0->f_4361 = 0;
	}
}

void func_854()
{
	if ((((((func_227("CH_HELP_LBDR0") || func_227("CH_HELP_LBDR1")) || func_227("CH_HELP_LBDR2")) || func_227("CH_HELP_LBDR3")) || func_227("CH_HELP_LBDR4")) || func_227("CH_HELP_LBDR5")) || func_227("CH_HELP_LBDR6"))
	{
		unk_0xD289B55B6AADBA10(1);
	}
}

void func_855(var uParam0, int iParam1)
{
	uParam0->f_2593.f_1 = 0;
	switch (iParam1)
	{
		case 1:
			func_856(uParam0->f_1497.f_709, 187);
			unk_0xC4CC25B68A91D144(-1, "Highlight_Move", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
			break;
		
		case 0:
			func_856(uParam0->f_1497.f_709, 188);
			unk_0xC4CC25B68A91D144(-1, "Highlight_Move", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
			break;
		
		case 2:
			func_856(uParam0->f_1497.f_709, 189);
			unk_0xC4CC25B68A91D144(-1, "Highlight_Move", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
			break;
		
		case 3:
			func_856(uParam0->f_1497.f_709, 190);
			unk_0xC4CC25B68A91D144(-1, "Highlight_Move", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
			break;
	}
}

void func_856(var uParam0, int iParam1)
{
	unk_0xC90636D3CB7D4CEF(uParam0, "SET_INPUT_EVENT", unk_0xBBDA792448DB5A89(iParam1), -1082130432, -1082130432, -1082130432, -1082130432);
}

int func_857(var uParam0)
{
	if (unk_0xDFAB5E5CBE16E8B1())
	{
		return -1;
	}
	if (unk_0xF0C12886E5C1B20E())
	{
		return -1;
	}
	if (unk_0x1BD7199394D7F19A(2, 188) || func_858(uParam0, 0, 0))
	{
		return 0;
	}
	else if (unk_0x1BD7199394D7F19A(2, 187) || func_858(uParam0, 0, 1))
	{
		return 1;
	}
	else if (unk_0x1BD7199394D7F19A(2, 189) || func_858(uParam0, 0, 2))
	{
		return 2;
	}
	else if (unk_0x1BD7199394D7F19A(2, 190) || func_858(uParam0, 0, 3))
	{
		return 3;
	}
	else if (unk_0x1BD7199394D7F19A(2, 205))
	{
		return 9;
	}
	else if (unk_0x1BD7199394D7F19A(2, 206))
	{
		return 10;
	}
	else if (unk_0xC27AEE8B64EE0F8D(2, 203))
	{
		return 4;
	}
	else if (unk_0xC27AEE8B64EE0F8D(2, 204))
	{
		return 5;
	}
	else if (unk_0xC27AEE8B64EE0F8D(2, 204))
	{
		return 6;
	}
	else if (unk_0xC27AEE8B64EE0F8D(2, 202))
	{
		return 8;
	}
	else if (unk_0xC27AEE8B64EE0F8D(2, 201))
	{
		return 7;
	}
	else if (unk_0xC27AEE8B64EE0F8D(2, 217))
	{
		return 11;
	}
	return -1;
}

int func_858(var uParam0, int iParam1, int iParam2)
{
	int iVar0;
	int iVar1;
	int iVar2;
	
	iVar0 = 0;
	switch (iParam1)
	{
		case 0:
			iVar1 = unk_0xF34EE736CF047844((unk_0xAAC78749C4BB5219(2, 218) * 127f));
			iVar2 = unk_0xF34EE736CF047844((unk_0xAAC78749C4BB5219(2, 219) * 127f));
			switch (iParam2)
			{
				case 0:
					if (iVar2 <= -110)
					{
						if (!unk_0xCE990E643CD9D0E5(uParam0->f_1, iParam2))
						{
							iVar0 = 1;
							unk_0xBE20AB8238688965(&(uParam0->f_1), iParam2);
						}
					}
					else
					{
						unk_0xD2459066EA58CE43(&(uParam0->f_1), iParam2);
					}
					break;
				
				case 1:
					if (iVar2 >= 110)
					{
						if (!unk_0xCE990E643CD9D0E5(uParam0->f_1, iParam2))
						{
							iVar0 = 1;
							unk_0xBE20AB8238688965(&(uParam0->f_1), iParam2);
						}
					}
					else
					{
						unk_0xD2459066EA58CE43(&(uParam0->f_1), iParam2);
					}
					break;
				
				case 2:
					if (iVar1 <= -110)
					{
						if (!unk_0xCE990E643CD9D0E5(uParam0->f_1, iParam2))
						{
							iVar0 = 1;
							unk_0xBE20AB8238688965(&(uParam0->f_1), iParam2);
						}
					}
					else
					{
						unk_0xD2459066EA58CE43(&(uParam0->f_1), iParam2);
					}
					break;
				
				case 3:
					if (iVar1 >= 110)
					{
						if (!unk_0xCE990E643CD9D0E5(uParam0->f_1, iParam2))
						{
							iVar0 = 1;
							unk_0xBE20AB8238688965(&(uParam0->f_1), iParam2);
						}
					}
					else
					{
						unk_0xD2459066EA58CE43(&(uParam0->f_1), iParam2);
					}
					break;
			}
			break;
		
		case 1:
			iVar1 = unk_0xF34EE736CF047844((unk_0xAAC78749C4BB5219(2, 220) * 127f));
			iVar2 = unk_0xF34EE736CF047844((unk_0xAAC78749C4BB5219(2, 221) * 127f));
			switch (iParam2)
			{
				case 0:
					if (iVar2 <= -110)
					{
						if (!unk_0xCE990E643CD9D0E5(uParam0->f_2, iParam2))
						{
							iVar0 = 1;
							unk_0xBE20AB8238688965(&(uParam0->f_2), iParam2);
						}
					}
					else
					{
						unk_0xD2459066EA58CE43(&(uParam0->f_2), iParam2);
					}
					break;
				
				case 1:
					if (iVar2 >= 110)
					{
						if (!unk_0xCE990E643CD9D0E5(uParam0->f_2, iParam2))
						{
							iVar0 = 1;
							unk_0xBE20AB8238688965(&(uParam0->f_2), iParam2);
						}
					}
					else
					{
						unk_0xD2459066EA58CE43(&(uParam0->f_2), iParam2);
					}
					break;
				
				case 2:
					if (iVar1 <= -110)
					{
						if (!unk_0xCE990E643CD9D0E5(uParam0->f_2, iParam2))
						{
							iVar0 = 1;
							unk_0xBE20AB8238688965(&(uParam0->f_2), iParam2);
						}
					}
					else
					{
						unk_0xD2459066EA58CE43(&(uParam0->f_2), iParam2);
					}
					break;
				
				case 3:
					if (iVar1 >= 110)
					{
						if (!unk_0xCE990E643CD9D0E5(uParam0->f_2, iParam2))
						{
							iVar0 = 1;
							unk_0xBE20AB8238688965(&(uParam0->f_2), iParam2);
						}
					}
					else
					{
						unk_0xD2459066EA58CE43(&(uParam0->f_2), iParam2);
					}
					break;
			}
			break;
	}
	return iVar0;
}

void func_859(var uParam0)
{
	bool bVar0;
	int iVar1;
	int iVar2;
	
	if (uParam0->f_1497.f_1027 == -1)
	{
		return;
	}
	if (func_529(uParam0, uParam0->f_1497.f_1027, &bVar0))
	{
		if (bVar0)
		{
			switch (uParam0->f_1497.f_1027)
			{
				case 14:
					unk_0xBE20AB8238688965(&(Global_1701669[unk_0x1EF8FCAB83E4BE53() /*68*/].f_42), 3);
					break;
				
				case 15:
					func_861(1);
					func_763(uParam0->f_1497.f_709, 6, 1);
					func_780(uParam0, 1);
					func_190(&(uParam0->f_1497), 1);
					if (func_527())
					{
						func_515(-1295545795, func_110(6), &iVar1, 0, 1, 0);
						Global_4264051[iVar1 /*85*/].f_9 = uParam0->f_2593;
					}
					else
					{
						unk_0x028944378AC816A9(uParam0->f_2593, func_110(6), 0, 1);
					}
					Global_1701669[unk_0x1EF8FCAB83E4BE53() /*68*/]++;
					unk_0xBE20AB8238688965(&(Global_1701669[unk_0x1EF8FCAB83E4BE53() /*68*/].f_1), 6);
					break;
				
				case 16:
					break;
				
				case 17:
					func_860(1);
					func_763(uParam0->f_1497.f_709, 7, 1);
					func_780(uParam0, 1);
					func_190(&(uParam0->f_1497), 1);
					if (func_527())
					{
						func_515(-1295545795, func_110(7), &iVar2, 0, 1, 0);
						Global_4264051[iVar2 /*85*/].f_9 = uParam0->f_2593;
					}
					else
					{
						unk_0x028944378AC816A9(uParam0->f_2593, func_110(7), 0, 1);
					}
					Global_1701669[unk_0x1EF8FCAB83E4BE53() /*68*/]++;
					unk_0xBE20AB8238688965(&(Global_1701669[unk_0x1EF8FCAB83E4BE53() /*68*/].f_1), 7);
					break;
				
				case 18:
					break;
			}
			uParam0->f_1497.f_1027 = -1;
			uParam0->f_1497.f_1029 = 1;
		}
		else
		{
			uParam0->f_1497.f_1027 = -1;
			uParam0->f_1497.f_1029 = 1;
			uParam0->f_1497.f_1028 = 0;
		}
	}
}

void func_860(bool bParam0)
{
	int iVar0;
	
	iVar0 = func_9(8529, -1, 0);
	if (bParam0)
	{
		unk_0xBE20AB8238688965(&iVar0, 7);
		unk_0xBE20AB8238688965(&(Global_1701669[unk_0x460153A63B9477BC() /*68*/].f_18), 7);
	}
	else
	{
		unk_0xD2459066EA58CE43(&iVar0, 7);
		unk_0xD2459066EA58CE43(&(Global_1701669[unk_0x460153A63B9477BC() /*68*/].f_18), 7);
	}
	func_6(8529, iVar0, -1, 1, 0);
}

void func_861(bool bParam0)
{
	int iVar0;
	
	iVar0 = func_9(8529, -1, 0);
	if (bParam0)
	{
		unk_0xBE20AB8238688965(&iVar0, 6);
		unk_0xBE20AB8238688965(&(Global_1701669[unk_0x460153A63B9477BC() /*68*/].f_18), 6);
	}
	else
	{
		unk_0xD2459066EA58CE43(&iVar0, 6);
		unk_0xD2459066EA58CE43(&(Global_1701669[unk_0x460153A63B9477BC() /*68*/].f_18), 6);
	}
	func_6(8529, iVar0, -1, 1, 0);
}

void func_862(var uParam0)
{
	if (uParam0->f_1497.f_702)
	{
		if (unk_0x95EF219D38B20CFF(uParam0->f_1497.f_709))
		{
			unk_0xD8D551845E3EA730(1);
			unk_0xCA1F114B668562E5(uParam0->f_1497.f_700);
			switch (2)
			{
				case 0:
					break;
				
				case 1:
					unk_0x6D9B01B4CCEFF373(uParam0->f_1497.f_709, 1);
					break;
				
				case 2:
					unk_0x178D84F9C5057462(uParam0->f_1497.f_709, 1);
					break;
			}
			unk_0xE6A03BA84586E67B(uParam0->f_1497.f_709, 0.25f, 0.5f, 0.5f, 1f, 255, 255, 255, 255, 0);
			unk_0xCA1F114B668562E5(unk_0xC48A590C39E17456());
			unk_0xD8D551845E3EA730(0);
			if (uParam0->f_1497.f_705)
			{
			}
		}
	}
	else if (func_484(uParam0->f_4356))
	{
		uParam0->f_1497.f_702 = 1;
	}
	if (uParam0->f_1497.f_703 && func_14() != 31)
	{
		func_863(uParam0);
	}
}

void func_863(var uParam0)
{
	struct<9> Var0;
	bool bVar9;
	int iVar10;
	
	Var0 = { func_877() };
	if (unk_0xDFAB5E5CBE16E8B1())
	{
		return;
	}
	bVar9 = uParam0->f_1497.f_706;
	if (func_16(uParam0) || uParam0->f_1497.f_1016 > 0)
	{
		if (!uParam0->f_1497.f_704)
		{
			bVar9 = true;
			uParam0->f_1497.f_704 = 1;
		}
	}
	else if (uParam0->f_1497.f_704)
	{
		bVar9 = true;
		uParam0->f_1497.f_704 = 0;
	}
	if (!bVar9)
	{
		func_872(&(uParam0->f_1497.f_710), &Var0, &(uParam0->f_1497), 0);
		return;
	}
	if (!unk_0xE5EF21F14D0F9D25() || unk_0xA2C6E70C9BCA9C34() == 10)
	{
		func_871(&(uParam0->f_1497), 0.5f);
	}
	else
	{
		func_871(&(uParam0->f_1497), 0.7f);
	}
	func_869(&(uParam0->f_1497));
	if (unk_0xCE990E643CD9D0E5(uParam0->f_1497.f_736.f_78, 5))
	{
		func_868("", "HEIST_IB_CONT", uParam0->f_1497.f_1030, &(uParam0->f_1497), 0);
	}
	iVar10 = uParam0->f_2593;
	if (uParam0->f_1497.f_704)
	{
		if (func_716(&(uParam0->f_2529)))
		{
			if (unk_0xC1F43F8D0D5EB664(2))
			{
				func_867(2, 39, "CH_INSTR_ZOOM", &(uParam0->f_1497), 1, 361);
			}
			else
			{
				func_866(2, 15, "CH_INSTR_ZOOM", &(uParam0->f_1497));
			}
		}
		func_866(2, 1, "HEIST_IB_LOOK", &(uParam0->f_1497));
	}
	else
	{
		switch (uParam0->f_1497.f_724.f_2)
		{
			case 0:
				if (uParam0->f_4356 == unk_0x460153A63B9477BC())
				{
					if (func_849(iVar10))
					{
						func_867(2, 201, "CH_INSTR_SEL", &(uParam0->f_1497), 1, 361);
					}
					else if (func_844(uParam0, iVar10))
					{
						func_867(2, 201, func_865(unk_0xFC559366953F62B3(), "CH_INSTR_MODI", "CH_INSTR_PREV"), &(uParam0->f_1497), 1, 361);
					}
					func_867(2, 202, "CH_INSTR_QUIT", &(uParam0->f_1497), 1, 361);
					if (func_839(uParam0, iVar10))
					{
						func_867(2, 203, "CH_INSTR_PURCH", &(uParam0->f_1497), 1, 361);
					}
				}
				else if (unk_0xFC559366953F62B3())
				{
					if (func_709(uParam0, unk_0x460153A63B9477BC()))
					{
						func_867(2, 201, "HEIST_IB_UNRDY", &(uParam0->f_1497), 1, 361);
					}
					else
					{
						func_867(2, 201, "HEIST_IB_RDY", &(uParam0->f_1497), 1, 361);
					}
					func_867(2, 202, "CH_INSTR_QUIT", &(uParam0->f_1497), 1, 361);
				}
				if (func_117(uParam0))
				{
					func_867(2, 204, "CH_INSTR_INFO", &(uParam0->f_1497), 1, 361);
				}
				if (!unk_0xFC559366953F62B3())
				{
					if (func_483(uParam0, 1))
					{
						func_867(2, 205, "CH_INSTR_PREP", &(uParam0->f_1497), 1, 361);
					}
				}
				if (unk_0xC1F43F8D0D5EB664(2))
				{
					func_867(2, 39, "CH_INSTR_ZOOM", &(uParam0->f_1497), 1, 361);
				}
				else
				{
					func_866(2, 15, "CH_INSTR_ZOOM", &(uParam0->f_1497));
				}
				func_866(2, 1, "HEIST_IB_LOOK", &(uParam0->f_1497));
				func_866(2, 0, "HEIST_IB_NAV", &(uParam0->f_1497));
				if (func_835(iVar10))
				{
					if ((unk_0x59ED414969A48197() || unk_0xDCA0815D900F27F8()) || unk_0xF8EDFF98A9C94C74())
					{
						func_867(2, 217, "HEIST_IB_PSN", &(uParam0->f_1497), 1, 361);
					}
					else if (unk_0x11E5DE894D6A8A5F() || unk_0xC6C08C02733D02C8())
					{
						func_864(unk_0xED026A379B794F61(2, 217, 1), "HEIST_IB_XBL", &(uParam0->f_1497), 0);
					}
					else
					{
						func_864(unk_0xED026A379B794F61(2, 217, 1), "HEIST_IB_PSNXBL", &(uParam0->f_1497), 0);
					}
				}
				break;
			
			case 1:
				if (uParam0->f_4356 != unk_0x460153A63B9477BC())
				{
					return;
				}
				if (unk_0xFC559366953F62B3())
				{
					func_867(2, 201, "CH_INSTR_SET", &(uParam0->f_1497), 1, 361);
				}
				func_867(2, 202, func_865(unk_0xFC559366953F62B3(), "CH_INSTR_CANC", "CH_INSTR_BACK"), &(uParam0->f_1497), 1, 361);
				if (func_117(uParam0))
				{
					func_867(2, 204, "CH_INSTR_INFO", &(uParam0->f_1497), 1, 361);
				}
				if (func_835(iVar10))
				{
					func_866(2, 8, "CH_INSTR_CUT", &(uParam0->f_1497));
				}
				else if (iVar10 == 2)
				{
					func_866(2, 8, "CH_INSTR_ENT", &(uParam0->f_1497));
				}
				else if (iVar10 == 3)
				{
					func_866(2, 8, "CH_INSTR_EXT", &(uParam0->f_1497));
				}
				else if (iVar10 == 4)
				{
					func_866(2, 8, "CH_INSTR_BUY", &(uParam0->f_1497));
				}
				else if (iVar10 == 13)
				{
					func_866(2, 8, "CH_INSTR_OUTI", &(uParam0->f_1497));
				}
				else if (iVar10 == 14)
				{
					func_866(2, 8, "CH_INSTR_OUTO", &(uParam0->f_1497));
				}
				func_866(2, 1, "HEIST_IB_LOOK", &(uParam0->f_1497));
				break;
			}
	}
	func_872(&(uParam0->f_1497.f_710), &Var0, &(uParam0->f_1497), uParam0->f_1497.f_706);
	func_190(&(uParam0->f_1497), 0);
}

void func_864(char* sParam0, char* sParam1, var uParam2, char* sParam3)
{
	int iVar0;
	
	if (uParam2->f_693 >= 12)
	{
		return;
	}
	iVar0 = uParam2->f_693;
	StringCopy(&(uParam2->f_1[iVar0 /*57*/]), sParam0, 64);
	StringCopy(&(uParam2->f_1[iVar0 /*57*/].f_16), sParam3, 64);
	StringCopy(&(uParam2->f_1[iVar0 /*57*/].f_32), sParam1, 16);
	uParam2->f_693++;
}

char* func_865(bool bParam0, char* sParam1, char* sParam2)
{
	if (bParam0)
	{
		return sParam1;
	}
	return sParam2;
}

void func_866(int iParam0, int iParam1, char* sParam2, var uParam3)
{
	int iVar0;
	
	if (uParam3->f_693 >= 12)
	{
		return;
	}
	iVar0 = uParam3->f_693;
	StringCopy(&(uParam3->f_1[iVar0 /*57*/].f_32), sParam2, 16);
	unk_0xBE20AB8238688965(&(uParam3->f_689), iVar0);
	uParam3->f_1[iVar0 /*57*/].f_54 = iParam0;
	uParam3->f_1[iVar0 /*57*/].f_55 = iParam1;
	unk_0xBE20AB8238688965(&(uParam3->f_690), iVar0);
	uParam3->f_693++;
}

void func_867(int iParam0, int iParam1, char* sParam2, var uParam3, bool bParam4, int iParam5)
{
	int iVar0;
	
	if (uParam3->f_693 >= 12)
	{
		return;
	}
	iVar0 = uParam3->f_693;
	StringCopy(&(uParam3->f_1[iVar0 /*57*/].f_32), sParam2, 16);
	unk_0xBE20AB8238688965(&(uParam3->f_689), iVar0);
	uParam3->f_1[iVar0 /*57*/].f_54 = iParam0;
	uParam3->f_1[iVar0 /*57*/].f_55 = iParam1;
	uParam3->f_1[iVar0 /*57*/].f_56 = iParam5;
	if (bParam4)
	{
		unk_0xBE20AB8238688965(&(uParam3->f_691), iVar0);
		if (iParam5 < 361)
		{
		}
	}
	uParam3->f_693++;
}

void func_868(char* sParam0, char* sParam1, var uParam2, var uParam3, char* sParam4)
{
	int iVar0;
	
	if (uParam3->f_693 >= 12)
	{
		return;
	}
	iVar0 = uParam3->f_693;
	StringCopy(&(uParam3->f_1[iVar0 /*57*/]), sParam0, 64);
	StringCopy(&(uParam3->f_1[iVar0 /*57*/].f_16), sParam4, 64);
	StringCopy(&(uParam3->f_1[iVar0 /*57*/].f_32), sParam1, 16);
	uParam3->f_1[iVar0 /*57*/].f_36 = uParam2;
	unk_0xBE20AB8238688965(&(uParam3->f_686), iVar0);
	uParam3->f_694 = iVar0;
	uParam3->f_693++;
}

void func_869(var uParam0)
{
	func_870(uParam0);
	uParam0->f_692 = 1;
}

void func_870(var uParam0)
{
	int iVar0;
	
	*uParam0 = 0;
	iVar0 = 0;
	iVar0 = 0;
	while (iVar0 < 12)
	{
		StringCopy(&(uParam0->f_1[iVar0 /*57*/]), "", 64);
		StringCopy(&(uParam0->f_1[iVar0 /*57*/].f_16), "", 64);
		StringCopy(&(uParam0->f_1[iVar0 /*57*/].f_32), "", 16);
		uParam0->f_1[iVar0 /*57*/].f_36 = 0;
		uParam0->f_1[iVar0 /*57*/].f_37 = 0;
		StringCopy(&(uParam0->f_1[iVar0 /*57*/].f_38), "", 64);
		uParam0->f_1[iVar0 /*57*/].f_54 = 2;
		uParam0->f_1[iVar0 /*57*/].f_55 = 361;
		uParam0->f_1[iVar0 /*57*/].f_56 = 361;
		iVar0++;
	}
	uParam0->f_686 = 0;
	uParam0->f_687 = 0;
	uParam0->f_688 = 0;
	uParam0->f_689 = 0;
	uParam0->f_691 = 0;
	uParam0->f_690 = 0;
	uParam0->f_692 = 0;
	uParam0->f_693 = 0;
	uParam0->f_695 = 0f;
	uParam0->f_696 = 0f;
	uParam0->f_697 = 0f;
	uParam0->f_698 = 0f;
	uParam0->f_699 = 1f;
}

void func_871(var uParam0, float fParam1)
{
	uParam0->f_699 = fParam1;
}

void func_872(var uParam0, var uParam1, var uParam2, int iParam3)
{
	int iVar0;
	var uVar1;
	var uVar2;
	int iVar3;
	var uVar4;
	var uVar5;
	
	if (iParam3 == 1 || unk_0x628AF533B14EF310(2))
	{
		*uParam2 = 0;
		if (unk_0x95EF219D38B20CFF(*uParam0))
		{
			if (unk_0xF8EDFF98A9C94C74())
			{
				unk_0x830F007E19C63E14(*uParam0, "TOGGLE_MOUSE_BUTTONS");
				unk_0xF1B28F753235CE2A(false);
				unk_0xE1FDD153F5858534();
			}
			unk_0x830F007E19C63E14(*uParam0, "CLEAR_ALL");
			unk_0xE1FDD153F5858534();
		}
		func_876(uParam2);
	}
	if (Global_1315735 < 2)
	{
		func_875(1);
	}
	if (*uParam2 == 0)
	{
		if (!unk_0x95EF219D38B20CFF(*uParam0))
		{
			*uParam0 = unk_0x5DEA4192B46CB99B("instructional_buttons");
		}
		if (unk_0x95EF219D38B20CFF(*uParam0))
		{
			unk_0x903663B04BCCB2FB(*uParam0, "CLEAR_ALL");
			if (unk_0xF8EDFF98A9C94C74())
			{
				unk_0x830F007E19C63E14(*uParam0, "TOGGLE_MOUSE_BUTTONS");
				unk_0xF1B28F753235CE2A(true);
				unk_0xE1FDD153F5858534();
			}
			iVar0 = 0;
			iVar0 = 0;
			while (iVar0 < uParam2->f_693)
			{
				if (unk_0xCE990E643CD9D0E5(uParam2->f_689, iVar0))
				{
					unk_0x830F007E19C63E14(*uParam0, "SET_DATA_SLOT");
					unk_0xAD291B8F75D737AD(iVar0);
					if (!unk_0xCE990E643CD9D0E5(uParam2->f_690, iVar0))
					{
						uVar1 = uParam2->f_1[iVar0 /*57*/].f_54;
						uVar2 = uParam2->f_1[iVar0 /*57*/].f_55;
						iVar3 = uParam2->f_1[iVar0 /*57*/].f_56;
						func_84(unk_0xED026A379B794F61(uVar1, uVar2, 1));
						if (iVar3 < 361)
						{
							func_84(unk_0xED026A379B794F61(iVar1, iVar3, 1));
						}
					}
					else
					{
						uVar4 = uParam2->f_1[iVar0 /*57*/].f_54;
						uVar5 = uParam2->f_1[iVar0 /*57*/].f_55;
						func_84(unk_0x32F8D6499042A233(uVar4, uVar5, 1));
					}
					if (unk_0xCE990E643CD9D0E5(uParam2->f_686, iVar0))
					{
						unk_0xB23270F3D5E62FDE(&(uParam2->f_1[iVar0 /*57*/].f_32));
						if (uParam2->f_694 == iVar0)
						{
							unk_0x7899A951E85F4B28(uParam2->f_1[iVar0 /*57*/].f_36, 70);
						}
						else
						{
							unk_0x42710E9E08FA375A(uParam2->f_1[iVar0 /*57*/].f_36);
							if (unk_0xCE990E643CD9D0E5(uParam2->f_687, iVar0))
							{
								unk_0x42710E9E08FA375A(uParam2->f_1[iVar0 /*57*/].f_37);
							}
						}
						unk_0x2042E9CA4306F725();
					}
					else if (unk_0xCE990E643CD9D0E5(uParam2->f_688, iVar0))
					{
						unk_0xB23270F3D5E62FDE(&(uParam2->f_1[iVar0 /*57*/].f_32));
						unk_0x9EFD301E3BE8324E(&(uParam2->f_1[iVar0 /*57*/].f_38));
						unk_0x2042E9CA4306F725();
					}
					else
					{
						func_83(&(uParam2->f_1[iVar0 /*57*/].f_32));
					}
					if (unk_0xF8EDFF98A9C94C74())
					{
						if (unk_0xCE990E643CD9D0E5(uParam2->f_691, iVar0))
						{
							unk_0xF1B28F753235CE2A(true);
							unk_0xAD291B8F75D737AD(uParam2->f_1[iVar0 /*57*/].f_55);
						}
						else
						{
							unk_0xF1B28F753235CE2A(false);
							unk_0xAD291B8F75D737AD(361);
						}
					}
					unk_0xE1FDD153F5858534();
				}
				else
				{
					unk_0x830F007E19C63E14(*uParam0, "SET_DATA_SLOT");
					unk_0xAD291B8F75D737AD(iVar0);
					func_84(&(uParam2->f_1[iVar0 /*57*/]));
					if (!unk_0x7BCC91F3C1CF24E8(&(uParam2->f_1[iVar0 /*57*/].f_16)))
					{
						func_84(&(uParam2->f_1[iVar0 /*57*/].f_16));
					}
					if (unk_0xCE990E643CD9D0E5(uParam2->f_686, iVar0))
					{
						unk_0xB23270F3D5E62FDE(&(uParam2->f_1[iVar0 /*57*/].f_32));
						if (uParam2->f_694 == iVar0)
						{
							unk_0x7899A951E85F4B28(uParam2->f_1[iVar0 /*57*/].f_36, 70);
						}
						else
						{
							unk_0x42710E9E08FA375A(uParam2->f_1[iVar0 /*57*/].f_36);
							if (unk_0xCE990E643CD9D0E5(uParam2->f_687, iVar0))
							{
								unk_0x42710E9E08FA375A(uParam2->f_1[iVar0 /*57*/].f_37);
							}
						}
						unk_0x2042E9CA4306F725();
					}
					else if (unk_0xCE990E643CD9D0E5(uParam2->f_688, iVar0))
					{
						unk_0xB23270F3D5E62FDE(&(uParam2->f_1[iVar0 /*57*/].f_32));
						unk_0x9EFD301E3BE8324E(&(uParam2->f_1[iVar0 /*57*/].f_38));
						unk_0x2042E9CA4306F725();
					}
					else
					{
						func_83(&(uParam2->f_1[iVar0 /*57*/].f_32));
					}
					if (unk_0xF8EDFF98A9C94C74())
					{
						unk_0xF1B28F753235CE2A(false);
						unk_0xAD291B8F75D737AD(361);
					}
					unk_0xE1FDD153F5858534();
				}
				iVar0++;
			}
			unk_0x830F007E19C63E14(*uParam0, "SET_MAX_WIDTH");
			unk_0x49B9B8101B158AEE(uParam2->f_699);
			unk_0xE1FDD153F5858534();
			unk_0x830F007E19C63E14(*uParam0, "DRAW_INSTRUCTIONAL_BUTTONS");
			unk_0xAD291B8F75D737AD(0);
			unk_0xE1FDD153F5858534();
			*uParam2 = 1;
		}
	}
	uParam2->f_695 = 0.05f;
	uParam2->f_696 = 0.045f;
	uParam2->f_697 = 0f;
	uParam2->f_698 = 0f;
	if (*uParam2 == 1)
	{
		func_874(*uParam0, uParam1);
	}
	func_873();
}

void func_873()
{
	unk_0x4CB4237D8858ADA6(7);
	unk_0x4CB4237D8858ADA6(6);
	unk_0x4CB4237D8858ADA6(8);
	unk_0x4CB4237D8858ADA6(9);
}

void func_874(var uParam0, var uParam1)
{
	unk_0x37FC7E88C7659D33(uParam0, uParam1->f_4, uParam1->f_5, uParam1->f_6, uParam1->f_7, 0);
}

void func_875(int iParam0)
{
	Global_1315735 = iParam0;
}

void func_876(var uParam0)
{
	uParam0->f_692 = 0;
}

struct<9> func_877()
{
	struct<9> Var0;
	
	Var0.f_0 = 0.5f;
	Var0.f_1 = 0.5f;
	Var0.f_2 = 1f;
	Var0.f_3 = 1f;
	Var0.f_4 = 255;
	Var0.f_5 = 255;
	Var0.f_6 = 255;
	Var0.f_7 = 200;
	Var0.f_8 = 0f;
	return Var0;
}

void func_878(var uParam0)
{
	int iVar0;
	int iVar1;
	int iVar2;
	int iVar3;
	bool bVar4;
	bool bVar5;
	bool bVar6;
	bool bVar7;
	int iVar8;
	
	if (!func_15())
	{
		return;
	}
	if (func_14() == 33)
	{
		if (Global_1697306.f_4356 == func_42() || !unk_0x166E920FB00B2DBB(Global_1697306.f_4356))
		{
			return;
		}
	}
	iVar0 = unk_0x1EF8FCAB83E4BE53();
	bVar4 = false;
	bVar5 = false;
	bVar6 = false;
	bVar7 = false;
	if (uParam0->f_4356 == unk_0x460153A63B9477BC())
	{
		iVar1 = 0;
		while (iVar1 <= 4)
		{
			if (iVar1 >= 4)
			{
				if (Global_1701669[iVar0 /*68*/].f_12[4] != uParam0->f_1497.f_736.f_92[4])
				{
					Global_1701669[iVar0 /*68*/].f_12[4] = uParam0->f_1497.f_736.f_92[4];
					bVar6 = true;
				}
			}
			else
			{
				if (Global_1701669[iVar0 /*68*/].f_12[iVar1] != uParam0->f_1497.f_736.f_92[iVar1])
				{
					Global_1701669[iVar0 /*68*/].f_12[iVar1] = uParam0->f_1497.f_736.f_92[iVar1];
					if (uParam0->f_1497.f_736.f_87[iVar1] == unk_0x1EF8FCAB83E4BE53())
					{
						Global_2452907.f_6480 = Global_1701669[iVar0 /*68*/].f_12[iVar1];
						Global_1676693.f_46 = Global_2452907.f_6480;
					}
					bVar6 = true;
				}
				if (Global_1701669[iVar0 /*68*/].f_2[iVar1] != uParam0->f_1497.f_736.f_87[iVar1])
				{
					Global_1701669[iVar0 /*68*/].f_2[iVar1] = uParam0->f_1497.f_736.f_87[iVar1];
					bVar4 = true;
				}
				iVar3 = 0;
				while (iVar3 <= 31)
				{
					if (uParam0->f_1497.f_736.f_87[iVar1] != iVar3 || uParam0->f_1497.f_736.f_87[iVar1] == uParam0->f_4356)
					{
					}
					else if (uParam0->f_1497.f_736.f_103[iVar1] != Global_1701669[iVar3 /*68*/].f_7[iVar1])
					{
						uParam0->f_1497.f_736.f_103[iVar1] = Global_1701669[iVar3 /*68*/].f_7[iVar1];
						Global_1701669[iVar0 /*68*/].f_7[iVar1] = uParam0->f_1497.f_736.f_103[iVar1];
						bVar5 = true;
					}
					iVar3++;
				}
			}
			iVar1++;
		}
		if (bVar4)
		{
			Global_1701669[iVar0 /*68*/]++;
			func_796(uParam0, 1);
		}
		if (bVar6)
		{
			Global_1701669[iVar0 /*68*/]++;
		}
		if (bVar5)
		{
			Global_1701669[iVar0 /*68*/]++;
		}
		if (Global_1701669[iVar0 /*68*/].f_18.f_17 != uParam0->f_1497.f_1019)
		{
			Global_1701669[iVar0 /*68*/]++;
			Global_1701669[iVar0 /*68*/].f_18.f_17 = uParam0->f_1497.f_1019;
		}
		if (Global_1701669[iVar0 /*68*/].f_18.f_18 != uParam0->f_1497.f_1017)
		{
			Global_1701669[iVar0 /*68*/]++;
			Global_1701669[iVar0 /*68*/].f_18.f_18 = uParam0->f_1497.f_1017;
		}
		if (Global_1701669[iVar0 /*68*/].f_18.f_19 != uParam0->f_1497.f_1018)
		{
			Global_1701669[iVar0 /*68*/]++;
			Global_1701669[iVar0 /*68*/].f_18.f_19 = uParam0->f_1497.f_1018;
		}
		if (func_113(unk_0x460153A63B9477BC()) == 2)
		{
			if (Global_1701669[iVar0 /*68*/].f_18.f_20 != uParam0->f_1497.f_1020)
			{
				Global_1701669[iVar0 /*68*/]++;
				Global_1701669[iVar0 /*68*/].f_18.f_20 = uParam0->f_1497.f_1020;
			}
			if (Global_1701669[iVar0 /*68*/].f_18.f_21 != uParam0->f_1497.f_1021)
			{
				Global_1701669[iVar0 /*68*/]++;
				Global_1701669[iVar0 /*68*/].f_18.f_21 = uParam0->f_1497.f_1021;
			}
		}
	}
	else
	{
		iVar2 = uParam0->f_4356;
		if (iVar2 <= -1)
		{
			return;
		}
		iVar1 = 0;
		while (iVar1 <= 3)
		{
			if (uParam0->f_1497.f_736.f_87[iVar1] != unk_0x1EF8FCAB83E4BE53())
			{
			}
			else
			{
				Global_1701669[unk_0x1EF8FCAB83E4BE53() /*68*/].f_7[iVar1] = uParam0->f_1497.f_736.f_103[iVar1];
				iVar8 = iVar1;
			}
			iVar1++;
		}
		if (Global_1701669[iVar2 /*68*/] > uParam0->f_1497.f_736.f_77)
		{
			iVar1 = 0;
			while (iVar1 <= 4)
			{
				if (iVar1 >= 4)
				{
					if (uParam0->f_1497.f_736.f_92[4] != Global_1701669[iVar2 /*68*/].f_12[4])
					{
						uParam0->f_1497.f_736.f_92[4] = Global_1701669[iVar2 /*68*/].f_12[4];
						bVar6 = true;
					}
				}
				else
				{
					if (uParam0->f_1497.f_736.f_92[iVar1] != Global_1701669[iVar2 /*68*/].f_12[iVar1])
					{
						uParam0->f_1497.f_736.f_92[iVar1] = Global_1701669[iVar2 /*68*/].f_12[iVar1];
						bVar6 = true;
						if (iVar8 == iVar1)
						{
							Global_2452907.f_6480 = Global_1701669[iVar2 /*68*/].f_12[iVar1];
							Global_1676693.f_46 = Global_2452907.f_6480;
						}
					}
					if (uParam0->f_1497.f_736.f_87[iVar1] != Global_1701669[iVar2 /*68*/].f_2[iVar1])
					{
						uParam0->f_1497.f_736.f_87[iVar1] = Global_1701669[iVar2 /*68*/].f_2[iVar1];
						bVar4 = true;
					}
					if (uParam0->f_1497.f_736.f_103[iVar1] != Global_1701669[iVar2 /*68*/].f_7[iVar1])
					{
						if (iVar8 != iVar1)
						{
							uParam0->f_1497.f_736.f_103[iVar1] = Global_1701669[iVar2 /*68*/].f_7[iVar1];
						}
						bVar5 = true;
					}
				}
				iVar1++;
			}
			if (bVar4)
			{
				func_796(uParam0, 1);
			}
			if (bVar6)
			{
			}
			if (bVar5)
			{
			}
			if (uParam0->f_1497.f_1017 != Global_1701669[iVar2 /*68*/].f_18.f_18)
			{
				uParam0->f_1497.f_1017 = Global_1701669[iVar2 /*68*/].f_18.f_18;
				func_755(uParam0, 1);
				func_769(uParam0, 1);
			}
			if (uParam0->f_1497.f_1018 != Global_1701669[iVar2 /*68*/].f_18.f_19)
			{
				uParam0->f_1497.f_1018 = Global_1701669[iVar2 /*68*/].f_18.f_19;
				func_752(uParam0, 1);
				func_769(uParam0, 1);
			}
			if (uParam0->f_1497.f_1019 != Global_1701669[iVar2 /*68*/].f_18.f_17)
			{
				uParam0->f_1497.f_1019 = Global_1701669[iVar2 /*68*/].f_18.f_17;
				func_747(uParam0, 1);
				func_769(uParam0, 1);
			}
			if (func_113(uParam0->f_4356) == 2)
			{
				if (uParam0->f_1497.f_1020 != Global_1701669[iVar2 /*68*/].f_18.f_20)
				{
					uParam0->f_1497.f_1020 = Global_1701669[iVar2 /*68*/].f_18.f_20;
					func_743(uParam0, 1);
					func_769(uParam0, 1);
				}
				if (uParam0->f_1497.f_1021 != Global_1701669[iVar2 /*68*/].f_18.f_21)
				{
					uParam0->f_1497.f_1021 = Global_1701669[iVar2 /*68*/].f_18.f_21;
					func_739(uParam0, 1);
					func_769(uParam0, 1);
				}
			}
			uParam0->f_1497.f_736.f_77 = Global_1701669[iVar2 /*68*/];
			if (unk_0xCE990E643CD9D0E5(Global_1701669[iVar2 /*68*/].f_1, 0) && !unk_0xCE990E643CD9D0E5(uParam0->f_1497.f_736.f_78, 0))
			{
				bVar7 = true;
				uParam0->f_1497.f_1022 = Global_1701669[iVar2 /*68*/].f_18.f_18;
				unk_0xBE20AB8238688965(&(uParam0->f_1497.f_736.f_78), 0);
			}
			if (unk_0xCE990E643CD9D0E5(Global_1701669[iVar2 /*68*/].f_1, 1) && !unk_0xCE990E643CD9D0E5(uParam0->f_1497.f_736.f_78, 1))
			{
				bVar7 = true;
				uParam0->f_1497.f_1023 = Global_1701669[iVar2 /*68*/].f_18.f_19;
				unk_0xBE20AB8238688965(&(uParam0->f_1497.f_736.f_78), 1);
			}
			if (unk_0xCE990E643CD9D0E5(Global_1701669[iVar2 /*68*/].f_1, 2) && !unk_0xCE990E643CD9D0E5(uParam0->f_1497.f_736.f_78, 2))
			{
				bVar7 = true;
				uParam0->f_1497.f_1024 = Global_1701669[iVar2 /*68*/].f_18.f_17;
				unk_0xBE20AB8238688965(&(uParam0->f_1497.f_736.f_78), 2);
			}
			if (unk_0xCE990E643CD9D0E5(Global_1701669[iVar2 /*68*/].f_1, 3) && !unk_0xCE990E643CD9D0E5(uParam0->f_1497.f_736.f_78, 3))
			{
				bVar7 = true;
				uParam0->f_1497.f_1025 = Global_1701669[iVar2 /*68*/].f_18.f_20;
				unk_0xBE20AB8238688965(&(uParam0->f_1497.f_736.f_78), 3);
			}
			if (unk_0xCE990E643CD9D0E5(Global_1701669[iVar2 /*68*/].f_1, 4) && !unk_0xCE990E643CD9D0E5(uParam0->f_1497.f_736.f_78, 4))
			{
				bVar7 = true;
				uParam0->f_1497.f_1026 = Global_1701669[iVar2 /*68*/].f_18.f_21;
				unk_0xBE20AB8238688965(&(uParam0->f_1497.f_736.f_78), 4);
			}
			if (unk_0xCE990E643CD9D0E5(Global_1701669[iVar2 /*68*/].f_1, 5) && !unk_0xCE990E643CD9D0E5(uParam0->f_1497.f_736.f_78, 5))
			{
				func_190(&(uParam0->f_1497), 1);
				unk_0xBE20AB8238688965(&(uParam0->f_1497.f_736.f_78), 5);
			}
			if (unk_0xCE990E643CD9D0E5(Global_1701669[iVar2 /*68*/].f_1, 6) && !unk_0xCE990E643CD9D0E5(uParam0->f_1497.f_736.f_78, 6))
			{
				bVar7 = true;
				func_763(uParam0->f_1497.f_709, 6, 1);
				unk_0xBE20AB8238688965(&(uParam0->f_1497.f_736.f_78), 6);
			}
			if (unk_0xCE990E643CD9D0E5(Global_1701669[iVar2 /*68*/].f_1, 7) && !unk_0xCE990E643CD9D0E5(uParam0->f_1497.f_736.f_78, 7))
			{
				bVar7 = true;
				func_763(uParam0->f_1497.f_709, 7, 1);
				unk_0xBE20AB8238688965(&(uParam0->f_1497.f_736.f_78), 7);
			}
			if (bVar7)
			{
				func_786(uParam0, 1);
				func_780(uParam0, 1);
			}
		}
	}
}

void func_879(var uParam0)
{
	if (unk_0x460153A63B9477BC() != uParam0->f_4356)
	{
		return;
	}
	if (!func_193(uParam0->f_4356, 0))
	{
		if (!func_883(5))
		{
			switch (uParam0->f_1497.f_1016)
			{
				case 0:
					func_191(uParam0, 47);
					uParam0->f_1497.f_1016++;
					break;
				
				case 1:
					if (func_882(uParam0, 47))
					{
						func_880(5, 1);
						uParam0->f_1497.f_1016 = 0;
					}
					break;
				}
		}
	}
	else if (unk_0xFC559366953F62B3())
	{
		if (!func_883(11))
		{
			switch (uParam0->f_1497.f_1016)
			{
				case 0:
					func_191(uParam0, 70);
					uParam0->f_1497.f_1016++;
					break;
				
				case 1:
					if (func_882(uParam0, 70))
					{
						func_880(11, 1);
						uParam0->f_1497.f_1016 = 0;
					}
					break;
			}
		}
		else if (!uParam0->f_4173.f_179)
		{
			switch (func_113(uParam0->f_4356))
			{
				case 1:
				case 3:
					if (!func_193(uParam0->f_4356, 1) && !func_193(uParam0->f_4356, 3))
					{
						func_191(uParam0, 48);
					}
					break;
				
				case 2:
					if (!func_193(uParam0->f_4356, 2))
					{
						func_191(uParam0, 52);
					}
					break;
			}
			uParam0->f_4173.f_179 = 1;
		}
	}
}

void func_880(int iParam0, int iParam1)
{
	switch (iParam0)
	{
		case 0:
			func_881(27233, iParam1, -1, 1);
			break;
		
		case 1:
			func_881(27234, iParam1, -1, 1);
			break;
		
		case 2:
			func_881(27235, iParam1, -1, 1);
			break;
		
		case 3:
			func_881(27236, iParam1, -1, 1);
			break;
		
		case 4:
			func_881(27237, iParam1, -1, 1);
			break;
		
		case 5:
			func_881(27238, iParam1, -1, 1);
			break;
		
		case 6:
			func_881(27239, iParam1, -1, 1);
			break;
		
		case 7:
			func_881(27240, iParam1, -1, 1);
			break;
		
		case 8:
			func_881(27241, iParam1, -1, 1);
			break;
		
		case 9:
			func_881(27242, iParam1, -1, 1);
			break;
		
		case 10:
			func_881(27243, iParam1, -1, 1);
			break;
		
		case 11:
			func_881(27244, iParam1, -1, 1);
			break;
	}
}

int func_881(int iParam0, var uParam1, int iParam2, int iParam3)
{
	int iVar0;
	int iVar1;
	int iVar2;
	int iVar3;
	int iVar4;
	int iVar5;
	int iVar6;
	int iVar7;
	int iVar8;
	int iVar9;
	int iVar10;
	int iVar11;
	int iVar12;
	int iVar13;
	int iVar14;
	int iVar15;
	int iVar16;
	int iVar17;
	int iVar18;
	int iVar19;
	int iVar20;
	int iVar21;
	int iVar22;
	int iVar23;
	int iVar24;
	int iVar25;
	
	if (iParam2 == -1)
	{
		iParam2 = func_8();
	}
	iVar0 = 0;
	if (iParam0 >= 0 && iParam0 < 192)
	{
		iVar2 = unk_0x43BE40E27DB6DC2E((iParam0 - 0), 0, 1, iParam2);
		iVar1 = ((iParam0 - 0) - unk_0x12730A13C85FC896((iParam0 - 0)) * 64);
		iVar0 = unk_0xC06DB57B81E2C02B(iVar2, uParam1, iVar1, iParam3);
	}
	else if (iParam0 >= 192 && iParam0 < 384)
	{
		iVar3 = unk_0x43BE40E27DB6DC2E((iParam0 - 192), 1, 1, iParam2);
		iVar1 = ((iParam0 - 192) - unk_0x12730A13C85FC896((iParam0 - 192)) * 64);
		iVar0 = unk_0xC06DB57B81E2C02B(iVar3, uParam1, iVar1, iParam3);
	}
	else if (iParam0 >= 513 && iParam0 < 705)
	{
		iVar4 = unk_0x43BE40E27DB6DC2E((iParam0 - 513), 0, 0, 0);
		iVar1 = ((iParam0 - 513) - unk_0x12730A13C85FC896((iParam0 - 513)) * 64);
		iVar0 = unk_0xC06DB57B81E2C02B(iVar4, uParam1, iVar1, iParam3);
	}
	else if (iParam0 >= 705 && iParam0 < 1281)
	{
		iVar5 = unk_0x43BE40E27DB6DC2E((iParam0 - 705), 1, 0, 0);
		iVar1 = ((iParam0 - 705) - unk_0x12730A13C85FC896((iParam0 - 705)) * 64);
		iVar0 = unk_0xC06DB57B81E2C02B(iVar5, uParam1, iVar1, iParam3);
	}
	else if (iParam0 >= 3111 && iParam0 < 3879)
	{
		iVar6 = unk_0x840F9A9E6D216561((iParam0 - 3111), 0, 1, iParam2);
		iVar1 = ((iParam0 - 3111) - unk_0x12730A13C85FC896((iParam0 - 3111)) * 64);
		iVar0 = unk_0xC06DB57B81E2C02B(iVar6, uParam1, iVar1, iParam3);
	}
	else if (iParam0 >= 2919 && iParam0 < 3111)
	{
		iVar7 = unk_0x840F9A9E6D216561((iParam0 - 2919), 0, 0, 0);
		iVar1 = ((iParam0 - 2919) - unk_0x12730A13C85FC896((iParam0 - 2919)) * 64);
		iVar0 = unk_0xC06DB57B81E2C02B(iVar7, uParam1, iVar1, iParam3);
	}
	else if (iParam0 >= 4207 && iParam0 < 4335)
	{
		iVar8 = unk_0x4B2E4855CB2B4FAA((iParam0 - 4207), 0, 1, iParam2, "_NGPSTAT_BOOL");
		iVar1 = ((iParam0 - 4207) - unk_0x12730A13C85FC896((iParam0 - 4207)) * 64);
		iVar0 = unk_0xC06DB57B81E2C02B(iVar8, uParam1, iVar1, iParam3);
	}
	else if (iParam0 >= 4335 && iParam0 < 4399)
	{
		iVar9 = unk_0x4B2E4855CB2B4FAA((iParam0 - 4335), 0, 0, 0, "_NGPSTAT_BOOL");
		iVar1 = ((iParam0 - 4335) - unk_0x12730A13C85FC896((iParam0 - 4335)) * 64);
		iVar0 = unk_0xC06DB57B81E2C02B(iVar9, uParam1, iVar1, iParam3);
	}
	else if (iParam0 >= 6029 && iParam0 < 6413)
	{
		iVar10 = unk_0x4B2E4855CB2B4FAA((iParam0 - 6029), 0, 1, iParam2, "_NGTATPSTAT_BOOL");
		iVar1 = ((iParam0 - 6029) - unk_0x12730A13C85FC896((iParam0 - 6029)) * 64);
		iVar0 = unk_0xC06DB57B81E2C02B(iVar10, uParam1, iVar1, iParam3);
	}
	else if (iParam0 >= 7385 && iParam0 < 7641)
	{
		iVar11 = unk_0x4B2E4855CB2B4FAA((iParam0 - 7385), 0, 1, iParam2, "_NGDLCPSTAT_BOOL");
		iVar1 = ((iParam0 - 7385) - unk_0x12730A13C85FC896((iParam0 - 7385)) * 64);
		iVar0 = unk_0xC06DB57B81E2C02B(iVar11, uParam1, iVar1, iParam3);
	}
	else if (iParam0 >= 7321 && iParam0 < 7385)
	{
		iVar12 = unk_0x4B2E4855CB2B4FAA((iParam0 - 7321), 0, 0, 0, "_NGDLCPSTAT_BOOL");
		iVar1 = ((iParam0 - 7321) - unk_0x12730A13C85FC896((iParam0 - 7321)) * 64);
		iVar0 = unk_0xC06DB57B81E2C02B(iVar12, uParam1, iVar1, iParam3);
	}
	else if (iParam0 >= 9361 && iParam0 < 9553)
	{
		iVar13 = unk_0x4B2E4855CB2B4FAA((iParam0 - 9361), 0, 1, iParam2, "_DLCBIKEPSTAT_BOOL");
		iVar1 = ((iParam0 - 9361) - unk_0x12730A13C85FC896((iParam0 - 9361)) * 64);
		iVar0 = unk_0xC06DB57B81E2C02B(iVar13, uParam1, iVar1, iParam3);
	}
	else if (iParam0 >= 15369 && iParam0 < 15561)
	{
		iVar14 = unk_0x4B2E4855CB2B4FAA((iParam0 - 15369), 0, 1, iParam2, "_DLCGUNPSTAT_BOOL");
		iVar1 = ((iParam0 - 15369) - unk_0x12730A13C85FC896((iParam0 - 15369)) * 64);
		iVar0 = unk_0xC06DB57B81E2C02B(iVar14, uParam1, iVar1, iParam3);
	}
	else if (iParam0 >= 15562 && iParam0 < 15946)
	{
		iVar15 = unk_0x4B2E4855CB2B4FAA((iParam0 - 15562), 0, 1, iParam2, "_GUNTATPSTAT_BOOL");
		iVar1 = ((iParam0 - 15562) - unk_0x12730A13C85FC896((iParam0 - 15562)) * 64);
		iVar0 = unk_0xC06DB57B81E2C02B(iVar15, uParam1, iVar1, iParam3);
	}
	else if (iParam0 >= 15946 && iParam0 < 16010)
	{
		iVar16 = unk_0x4B2E4855CB2B4FAA((iParam0 - 15946), 0, 1, iParam2, "_DLCSMUGCHARPSTAT_BOOL");
		iVar1 = ((iParam0 - 15946) - unk_0x12730A13C85FC896((iParam0 - 15946)) * 64);
		iVar0 = unk_0xC06DB57B81E2C02B(iVar16, uParam1, iVar1, iParam3);
	}
	else if (iParam0 >= 18098 && iParam0 < 18162)
	{
		iVar17 = unk_0x4B2E4855CB2B4FAA((iParam0 - 18098), 0, 1, iParam2, "_GANGOPSPSTAT_BOOL");
		iVar1 = ((iParam0 - 18098) - unk_0x12730A13C85FC896((iParam0 - 18098)) * 64);
		iVar0 = unk_0xC06DB57B81E2C02B(iVar17, uParam1, iVar1, iParam3);
	}
	else if (iParam0 >= 22066 && iParam0 < 22194)
	{
		iVar18 = unk_0x4B2E4855CB2B4FAA((iParam0 - 22066), 0, 1, iParam2, "_BUSINESSBATPSTAT_BOOL");
		iVar1 = ((iParam0 - 22066) - unk_0x12730A13C85FC896((iParam0 - 22066)) * 64);
		iVar0 = unk_0xC06DB57B81E2C02B(iVar18, uParam1, iVar1, iParam3);
	}
	else if (iParam0 >= 24962 && iParam0 < 25538)
	{
		iVar19 = unk_0x4B2E4855CB2B4FAA((iParam0 - 24962), 0, 1, iParam2, "_ARENAWARSPSTAT_BOOL");
		iVar1 = ((iParam0 - 24962) - unk_0x12730A13C85FC896((iParam0 - 24962)) * 64);
		iVar0 = unk_0xC06DB57B81E2C02B(iVar19, uParam1, iVar1, iParam3);
	}
	else if (iParam0 >= 26810 && iParam0 < 27258)
	{
		iVar20 = unk_0x4B2E4855CB2B4FAA((iParam0 - 26810), 0, 1, iParam2, "_CASINOPSTAT_BOOL");
		iVar1 = ((iParam0 - 26810) - unk_0x12730A13C85FC896((iParam0 - 26810)) * 64);
		iVar0 = unk_0xC06DB57B81E2C02B(iVar20, uParam1, iVar1, iParam3);
	}
	else if (iParam0 >= 28098 && iParam0 < 28354)
	{
		iVar21 = unk_0x4B2E4855CB2B4FAA((iParam0 - 28098), 0, 1, iParam2, "_CASINOHSTPSTAT_BOOL");
		iVar1 = ((iParam0 - 28098) - unk_0x12730A13C85FC896((iParam0 - 28098)) * 64);
		iVar0 = unk_0xC06DB57B81E2C02B(iVar21, uParam1, iVar1, iParam3);
	}
	else if (iParam0 >= 28355 && iParam0 < 28483)
	{
		iVar22 = unk_0x4B2E4855CB2B4FAA((iParam0 - 28355), 0, 1, iParam2, "_HEIST3TATTOOSTAT_BOOL");
		iVar1 = ((iParam0 - 28355) - unk_0x12730A13C85FC896((iParam0 - 28355)) * 64);
		iVar0 = unk_0xC06DB57B81E2C02B(iVar22, uParam1, iVar1, iParam3);
	}
	else if (iParam0 >= 30227 && iParam0 < 30355)
	{
		iVar23 = unk_0x4B2E4855CB2B4FAA((iParam0 - 30227), 0, 1, iParam2, "_SU20PSTAT_BOOL");
		iVar1 = ((iParam0 - 30227) - unk_0x12730A13C85FC896((iParam0 - 30227)) * 64);
		iVar0 = unk_0xC06DB57B81E2C02B(iVar23, uParam1, iVar1, iParam3);
	}
	else if (iParam0 >= 30355 && iParam0 < 30483)
	{
		iVar24 = unk_0x4B2E4855CB2B4FAA((iParam0 - 30355), 0, 1, iParam2, "_SU20TATTOOSTAT_BOOL");
		iVar1 = ((iParam0 - 30355) - unk_0x12730A13C85FC896((iParam0 - 30355)) * 64);
		iVar0 = unk_0xC06DB57B81E2C02B(iVar24, uParam1, iVar1, iParam3);
	}
	else if (iParam0 >= 30515 && iParam0 < 30707)
	{
		iVar25 = unk_0x4B2E4855CB2B4FAA((iParam0 - 30515), 0, 1, iParam2, "_HISLANDPSTAT_BOOL");
		iVar1 = ((iParam0 - 30515) - unk_0x12730A13C85FC896((iParam0 - 30515)) * 64);
		iVar0 = unk_0xC06DB57B81E2C02B(iVar25, uParam1, iVar1, iParam3);
	}
	return iVar0;
}

bool func_882(var uParam0, int iParam1)
{
	return (func_31(uParam0, iParam1) && uParam0->f_4173.f_8 != iParam1);
}

int func_883(int iParam0)
{
	switch (iParam0)
	{
		case 0:
			return func_884(27233, -1, -1);
			break;
		
		case 1:
			return func_884(27234, -1, -1);
			break;
		
		case 2:
			return func_884(27235, -1, -1);
			break;
		
		case 3:
			return func_884(27236, -1, -1);
			break;
		
		case 4:
			return func_884(27237, -1, -1);
			break;
		
		case 5:
			return func_884(27238, -1, -1);
			break;
		
		case 6:
			return func_884(27239, -1, -1);
			break;
		
		case 7:
			return func_884(27240, -1, -1);
			break;
		
		case 8:
			return func_884(27241, -1, -1);
			break;
		
		case 9:
			return func_884(27242, -1, -1);
			break;
		
		case 10:
			return func_884(27243, -1, -1);
			break;
		
		case 11:
			return func_884(27244, -1, -1);
			break;
	}
	return 0;
}

int func_884(int iParam0, int iParam1, int iParam2)
{
	int iVar0;
	int iVar1;
	var uVar2;
	
	iVar0 = 0;
	if (iParam1 == -1)
	{
		iParam1 = func_8();
	}
	iVar1 = func_886(iParam0, iParam1);
	uVar2 = func_885(iParam0);
	if (0 != iVar1)
	{
		iVar0 = unk_0x9FD0AD883E4A7C1E(iVar1, uVar2, iParam2);
	}
	return iVar0;
}

int func_885(int iParam0)
{
	int iVar0;
	
	iVar0 = 0;
	if (iParam0 >= 0 && iParam0 < 192)
	{
		iVar0 = ((iParam0 - 0) - unk_0x12730A13C85FC896((iParam0 - 0)) * 64);
	}
	else if (iParam0 >= 192 && iParam0 < 384)
	{
		iVar0 = ((iParam0 - 192) - unk_0x12730A13C85FC896((iParam0 - 192)) * 64);
	}
	else if (iParam0 >= 513 && iParam0 < 705)
	{
		iVar0 = ((iParam0 - 513) - unk_0x12730A13C85FC896((iParam0 - 513)) * 64);
	}
	else if (iParam0 >= 705 && iParam0 < 1281)
	{
		iVar0 = ((iParam0 - 705) - unk_0x12730A13C85FC896((iParam0 - 705)) * 64);
	}
	else if (iParam0 >= 2919 && iParam0 < 3111)
	{
		iVar0 = ((iParam0 - 2919) - unk_0x12730A13C85FC896((iParam0 - 2919)) * 64);
	}
	else if (iParam0 >= 3111 && iParam0 < 3879)
	{
		iVar0 = ((iParam0 - 3111) - unk_0x12730A13C85FC896((iParam0 - 3111)) * 64);
	}
	else if (iParam0 >= 4335 && iParam0 < 4399)
	{
		iVar0 = ((iParam0 - 4335) - unk_0x12730A13C85FC896((iParam0 - 4335)) * 64);
	}
	else if (iParam0 >= 4207 && iParam0 < 4335)
	{
		iVar0 = ((iParam0 - 4207) - unk_0x12730A13C85FC896((iParam0 - 4207)) * 64);
	}
	else if (iParam0 >= 6029 && iParam0 < 6413)
	{
		iVar0 = ((iParam0 - 6029) - unk_0x12730A13C85FC896((iParam0 - 6029)) * 64);
	}
	else if (iParam0 >= 7385 && iParam0 < 7641)
	{
		iVar0 = ((iParam0 - 7385) - unk_0x12730A13C85FC896((iParam0 - 7385)) * 64);
	}
	else if (iParam0 >= 7321 && iParam0 < 7385)
	{
		iVar0 = ((iParam0 - 7321) - unk_0x12730A13C85FC896((iParam0 - 7321)) * 64);
	}
	else if (iParam0 >= 9361 && iParam0 < 9553)
	{
		iVar0 = ((iParam0 - 9361) - unk_0x12730A13C85FC896((iParam0 - 9361)) * 64);
	}
	else if (iParam0 >= 15369 && iParam0 < 15561)
	{
		iVar0 = ((iParam0 - 15369) - unk_0x12730A13C85FC896((iParam0 - 15369)) * 64);
	}
	else if (iParam0 >= 15562 && iParam0 < 15946)
	{
		iVar0 = ((iParam0 - 15562) - unk_0x12730A13C85FC896((iParam0 - 15562)) * 64);
	}
	else if (iParam0 >= 15946 && iParam0 < 16010)
	{
		iVar0 = ((iParam0 - 15946) - unk_0x12730A13C85FC896((iParam0 - 15946)) * 64);
	}
	else if (iParam0 >= 18098 && iParam0 < 18162)
	{
		iVar0 = ((iParam0 - 18098) - unk_0x12730A13C85FC896((iParam0 - 18098)) * 64);
	}
	else if (iParam0 >= 22066 && iParam0 < 22194)
	{
		iVar0 = ((iParam0 - 22066) - unk_0x12730A13C85FC896((iParam0 - 22066)) * 64);
	}
	else if (iParam0 >= 24962 && iParam0 < 25538)
	{
		iVar0 = ((iParam0 - 24962) - unk_0x12730A13C85FC896((iParam0 - 24962)) * 64);
	}
	else if (iParam0 >= 26810 && iParam0 < 27258)
	{
		iVar0 = ((iParam0 - 26810) - unk_0x12730A13C85FC896((iParam0 - 26810)) * 64);
	}
	else if (iParam0 >= 28098 && iParam0 < 28354)
	{
		iVar0 = ((iParam0 - 28098) - unk_0x12730A13C85FC896((iParam0 - 28098)) * 64);
	}
	else if (iParam0 >= 28355 && iParam0 < 28483)
	{
		iVar0 = ((iParam0 - 28355) - unk_0x12730A13C85FC896((iParam0 - 28355)) * 64);
	}
	else if (iParam0 >= 30227 && iParam0 < 30355)
	{
		iVar0 = ((iParam0 - 30227) - unk_0x12730A13C85FC896((iParam0 - 30227)) * 64);
	}
	else if (iParam0 >= 30355 && iParam0 < 30483)
	{
		iVar0 = ((iParam0 - 30355) - unk_0x12730A13C85FC896((iParam0 - 30355)) * 64);
	}
	else if (iParam0 >= 30515 && iParam0 < 30707)
	{
		iVar0 = ((iParam0 - 30515) - unk_0x12730A13C85FC896((iParam0 - 30515)) * 64);
	}
	return iVar0;
}

int func_886(int iParam0, int iParam1)
{
	int iVar0;
	
	if (iParam1 == -1)
	{
		iParam1 = func_8();
	}
	iVar0 = 0;
	if (iParam0 >= 0 && iParam0 < 192)
	{
		iVar0 = unk_0x43BE40E27DB6DC2E((iParam0 - 0), 0, 1, iParam1);
	}
	else if (iParam0 >= 192 && iParam0 < 384)
	{
		iVar0 = unk_0x43BE40E27DB6DC2E((iParam0 - 192), 1, 1, iParam1);
	}
	else if (iParam0 >= 513 && iParam0 < 705)
	{
		iVar0 = unk_0x43BE40E27DB6DC2E((iParam0 - 513), 0, 0, 0);
	}
	else if (iParam0 >= 705 && iParam0 < 1281)
	{
		iVar0 = unk_0x43BE40E27DB6DC2E((iParam0 - 705), 1, 0, 0);
	}
	else if (iParam0 >= 2919 && iParam0 < 3111)
	{
		iVar0 = unk_0x840F9A9E6D216561((iParam0 - 2919), 0, 0, 0);
	}
	else if (iParam0 >= 3111 && iParam0 < 3879)
	{
		iVar0 = unk_0x840F9A9E6D216561((iParam0 - 3111), 0, 1, iParam1);
	}
	else if (iParam0 >= 4335 && iParam0 < 4399)
	{
		iVar0 = unk_0x4B2E4855CB2B4FAA((iParam0 - 4335), 0, 0, 0, "_NGPSTAT_BOOL");
	}
	else if (iParam0 >= 4207 && iParam0 < 4335)
	{
		iVar0 = unk_0x4B2E4855CB2B4FAA((iParam0 - 4207), 0, 1, iParam1, "_NGPSTAT_BOOL");
	}
	else if (iParam0 >= 6029 && iParam0 < 6413)
	{
		iVar0 = unk_0x4B2E4855CB2B4FAA((iParam0 - 6029), 0, 1, iParam1, "_NGTATPSTAT_BOOL");
	}
	else if (iParam0 >= 7321 && iParam0 < 7385)
	{
		iVar0 = unk_0x4B2E4855CB2B4FAA((iParam0 - 7321), 0, 0, 0, "_NGDLCPSTAT_BOOL");
	}
	else if (iParam0 >= 7385 && iParam0 < 7641)
	{
		iVar0 = unk_0x4B2E4855CB2B4FAA((iParam0 - 7385), 0, 1, iParam1, "_NGDLCPSTAT_BOOL");
	}
	else if (iParam0 >= 9361 && iParam0 < 9553)
	{
		iVar0 = unk_0x4B2E4855CB2B4FAA((iParam0 - 9361), 0, 1, iParam1, "_DLCBIKEPSTAT_BOOL");
	}
	else if (iParam0 >= 15369 && iParam0 < 15561)
	{
		iVar0 = unk_0x4B2E4855CB2B4FAA((iParam0 - 15369), 0, 1, iParam1, "_DLCGUNPSTAT_BOOL");
	}
	else if (iParam0 >= 15562 && iParam0 < 15946)
	{
		iVar0 = unk_0x4B2E4855CB2B4FAA((iParam0 - 15562), 0, 1, iParam1, "_GUNTATPSTAT_BOOL");
	}
	else if (iParam0 >= 15946 && iParam0 < 16010)
	{
		iVar0 = unk_0x4B2E4855CB2B4FAA((iParam0 - 15946), 0, 1, iParam1, "_DLCSMUGCHARPSTAT_BOOL");
	}
	else if (iParam0 >= 18098 && iParam0 < 18162)
	{
		iVar0 = unk_0x4B2E4855CB2B4FAA((iParam0 - 18098), 0, 1, iParam1, "_GANGOPSPSTAT_BOOL");
	}
	else if (iParam0 >= 22066 && iParam0 < 22194)
	{
		iVar0 = unk_0x4B2E4855CB2B4FAA((iParam0 - 22066), 0, 1, iParam1, "_BUSINESSBATPSTAT_BOOL");
	}
	else if (iParam0 >= 24962 && iParam0 < 25538)
	{
		iVar0 = unk_0x4B2E4855CB2B4FAA((iParam0 - 24962), 0, 1, iParam1, "_ARENAWARSPSTAT_BOOL");
	}
	else if (iParam0 >= 26810 && iParam0 < 27258)
	{
		iVar0 = unk_0x4B2E4855CB2B4FAA((iParam0 - 26810), 0, 1, iParam1, "_CASINOPSTAT_BOOL");
	}
	else if (iParam0 >= 28098 && iParam0 < 28354)
	{
		iVar0 = unk_0x4B2E4855CB2B4FAA((iParam0 - 28098), 0, 1, iParam1, "_CASINOHSTPSTAT_BOOL");
	}
	else if (iParam0 >= 28355 && iParam0 < 28483)
	{
		iVar0 = unk_0x4B2E4855CB2B4FAA((iParam0 - 28355), 0, 1, iParam1, "_HEIST3TATTOOSTAT_BOOL");
	}
	else if (iParam0 >= 30227 && iParam0 < 30355)
	{
		iVar0 = unk_0x4B2E4855CB2B4FAA((iParam0 - 30227), 0, 1, iParam1, "_SU20PSTAT_BOOL");
	}
	else if (iParam0 >= 30355 && iParam0 < 30483)
	{
		iVar0 = unk_0x4B2E4855CB2B4FAA((iParam0 - 30355), 0, 1, iParam1, "_SU20TATTOOSTAT_BOOL");
	}
	else if (iParam0 >= 30515 && iParam0 < 30707)
	{
		iVar0 = unk_0x4B2E4855CB2B4FAA((iParam0 - 30515), 0, 1, iParam1, "_HISLANDPSTAT_BOOL");
	}
	return iVar0;
}

void func_887(var uParam0)
{
	int iVar0;
	
	if (unk_0xCE990E643CD9D0E5(uParam0->f_4357, 7))
	{
		iVar0 = uParam0->f_4356;
		if (iVar0 != -1)
		{
			if (!func_890(&(Global_1701669[iVar0 /*68*/].f_65)))
			{
				if (uParam0->f_4356 == unk_0x460153A63B9477BC())
				{
					if (unk_0xCE990E643CD9D0E5(uParam0->f_1497.f_736.f_78, 5))
					{
						func_504(&(Global_1701669[iVar0 /*68*/].f_65), 0, 0);
					}
				}
			}
			else
			{
				uParam0->f_1497.f_1030 = (120000 - func_889(&(Global_1701669[iVar0 /*68*/].f_65), 0, 0));
				if (uParam0->f_1497.f_1030 > 0)
				{
					if (uParam0->f_1497.f_1030 < (uParam0->f_1497.f_1031 - 1000))
					{
						func_190(&(uParam0->f_1497), 1);
						uParam0->f_1497.f_1031 = uParam0->f_1497.f_1030;
					}
				}
				else
				{
					uParam0->f_1497.f_1030 = 0;
					func_888(uParam0);
					if (uParam0->f_4356 == unk_0x460153A63B9477BC())
					{
						uParam0->f_1497.f_736.f_103[0] = 1;
						Global_1701669[unk_0x1EF8FCAB83E4BE53() /*68*/].f_7[0] = 1;
						unk_0xBE20AB8238688965(&(Global_1701669[unk_0x1EF8FCAB83E4BE53() /*68*/].f_42), 2);
					}
				}
			}
		}
	}
}

void func_888(var uParam0)
{
	int iVar0;
	int iVar1;
	
	switch (uParam0->f_1497.f_736.f_85)
	{
		case 1:
			if (uParam0->f_4356 == unk_0x460153A63B9477BC())
			{
				Global_1701669[unk_0x1EF8FCAB83E4BE53() /*68*/].f_12[0] = 100;
				Global_1701669[unk_0x1EF8FCAB83E4BE53() /*68*/].f_12[1] = 0;
				Global_1701669[unk_0x1EF8FCAB83E4BE53() /*68*/].f_12[2] = 0;
				Global_1701669[unk_0x1EF8FCAB83E4BE53() /*68*/].f_12[3] = 0;
				Global_1701669[unk_0x1EF8FCAB83E4BE53() /*68*/].f_12[4] = 0;
			}
			uParam0->f_1497.f_736.f_92[0] = 100;
			uParam0->f_1497.f_736.f_92[1] = 0;
			uParam0->f_1497.f_736.f_92[2] = 0;
			uParam0->f_1497.f_736.f_92[3] = 0;
			uParam0->f_1497.f_736.f_92[4] = 0;
			break;
		
		case 2:
			if (uParam0->f_4356 == unk_0x460153A63B9477BC())
			{
				Global_1701669[unk_0x1EF8FCAB83E4BE53() /*68*/].f_12[0] = 85;
				Global_1701669[unk_0x1EF8FCAB83E4BE53() /*68*/].f_12[1] = 15;
				Global_1701669[unk_0x1EF8FCAB83E4BE53() /*68*/].f_12[2] = 0;
				Global_1701669[unk_0x1EF8FCAB83E4BE53() /*68*/].f_12[3] = 0;
				Global_1701669[unk_0x1EF8FCAB83E4BE53() /*68*/].f_12[4] = 0;
			}
			uParam0->f_1497.f_736.f_92[0] = 85;
			uParam0->f_1497.f_736.f_92[1] = 15;
			uParam0->f_1497.f_736.f_92[2] = 0;
			uParam0->f_1497.f_736.f_92[3] = 0;
			uParam0->f_1497.f_736.f_92[4] = 0;
			break;
		
		case 3:
			if (uParam0->f_4356 == unk_0x460153A63B9477BC())
			{
				Global_1701669[unk_0x1EF8FCAB83E4BE53() /*68*/].f_12[0] = 70;
				Global_1701669[unk_0x1EF8FCAB83E4BE53() /*68*/].f_12[1] = 15;
				Global_1701669[unk_0x1EF8FCAB83E4BE53() /*68*/].f_12[2] = 15;
				Global_1701669[unk_0x1EF8FCAB83E4BE53() /*68*/].f_12[3] = 0;
				Global_1701669[unk_0x1EF8FCAB83E4BE53() /*68*/].f_12[4] = 0;
			}
			uParam0->f_1497.f_736.f_92[0] = 70;
			uParam0->f_1497.f_736.f_92[1] = 15;
			uParam0->f_1497.f_736.f_92[2] = 15;
			uParam0->f_1497.f_736.f_92[3] = 0;
			uParam0->f_1497.f_736.f_92[4] = 0;
			break;
		
		case 4:
			if (uParam0->f_4356 == unk_0x460153A63B9477BC())
			{
				Global_1701669[unk_0x1EF8FCAB83E4BE53() /*68*/].f_12[0] = 55;
				Global_1701669[unk_0x1EF8FCAB83E4BE53() /*68*/].f_12[1] = 15;
				Global_1701669[unk_0x1EF8FCAB83E4BE53() /*68*/].f_12[2] = 15;
				Global_1701669[unk_0x1EF8FCAB83E4BE53() /*68*/].f_12[3] = 15;
				Global_1701669[unk_0x1EF8FCAB83E4BE53() /*68*/].f_12[4] = 0;
			}
			uParam0->f_1497.f_736.f_92[0] = 55;
			uParam0->f_1497.f_736.f_92[1] = 15;
			uParam0->f_1497.f_736.f_92[2] = 15;
			uParam0->f_1497.f_736.f_92[3] = 15;
			uParam0->f_1497.f_736.f_92[4] = 0;
			break;
	}
	iVar0 = 0;
	while (iVar0 <= 3)
	{
		if (uParam0->f_1497.f_736.f_87[iVar0] != unk_0x1EF8FCAB83E4BE53())
		{
		}
		else
		{
			iVar1 = iVar0;
		}
		iVar0++;
	}
	Global_2452907.f_6480 = uParam0->f_1497.f_736.f_92[iVar1];
	Global_1676693.f_46 = Global_2452907.f_6480;
}

int func_889(var uParam0, bool bParam1, bool bParam2)
{
	if (unk_0x25DDB354A40FFCDB() && !bParam1)
	{
		if (!bParam2)
		{
			return unk_0x624736CA66E14161(unk_0x6CAAB7E78B5D978A(), *uParam0);
		}
		else
		{
			return unk_0x624736CA66E14161(unk_0x29B9AF1CB5B8175D(), *uParam0);
		}
	}
	return unk_0x624736CA66E14161(unk_0x578C4EF320340AF7(), *uParam0);
}

bool func_890(var uParam0)
{
	return uParam0->f_1;
}

void func_891()
{
	if (unk_0xC1F43F8D0D5EB664(2))
	{
		unk_0x26A60F42A1132477(2, 202);
	}
}

void func_892(var uParam0)
{
	if (!func_17(uParam0, 1))
	{
		func_894(uParam0, 2);
	}
	if (unk_0xFC559366953F62B3())
	{
		if (func_113(uParam0->f_4356) == 2)
		{
			func_701(&(uParam0->f_2593), 13, 0);
		}
		else
		{
			func_701(&(uParam0->f_2593), 2, 0);
		}
	}
	else if (uParam0->f_4356 == unk_0x460153A63B9477BC())
	{
		func_701(&(uParam0->f_2593), 12, 0);
		func_189(uParam0, 11);
	}
	else if (func_113(uParam0->f_4356) == 2)
	{
		func_701(&(uParam0->f_2593), 13, 0);
	}
	else
	{
		func_701(&(uParam0->f_2593), 2, 0);
	}
	func_893(uParam0->f_1497.f_709, uParam0->f_2593);
	uParam0->f_1497.f_703 = 1;
}

void func_893(var uParam0, int iParam1)
{
	unk_0xC90636D3CB7D4CEF(uParam0, "SET_CURRENT_SELECTION", unk_0xBBDA792448DB5A89(iParam1), -1082130432, -1082130432, -1082130432, -1082130432);
}

void func_894(var uParam0, int iParam1)
{
	uParam0->f_2529.f_35 = 0f;
	uParam0->f_2529.f_35.f_1 = 0f;
	uParam0->f_2529.f_35.f_2 = -180f;
	func_851(uParam0, iParam1);
	func_895(&(uParam0->f_2529.f_1), func_724(&(uParam0->f_2529)), func_723(&(uParam0->f_2529)), func_722(&(uParam0->f_2529)), 20, 10, 3, 10f, 1, 0, -1f, 0);
	func_721(&(uParam0->f_2529), func_722(&(uParam0->f_2529)));
	unk_0xD50CC9B03AAD56FF(1, 0);
	uParam0->f_2529.f_38 = { func_724(&(uParam0->f_2529)) };
	uParam0->f_2529.f_41 = { func_723(&(uParam0->f_2529)) };
}

void func_895(var uParam0, struct<3> Param1, struct<3> Param4, var uParam7, int iParam8, int iParam9, int iParam10, float fParam11, int iParam12, int iParam13, float fParam14, bool bParam15)
{
	uParam0->f_1 = { Param1 };
	uParam0->f_4 = { Param4 };
	uParam0->f_7 = uParam7;
	uParam0->f_20 = iParam8;
	uParam0->f_21 = iParam9;
	uParam0->f_22 = iParam10;
	uParam0->f_8 = { 0f, 0f, 0f };
	uParam0->f_11 = { 0f, 0f, 0f };
	uParam0->f_14 = { 0f, 0f, 0f };
	uParam0->f_17 = uParam7;
	uParam0->f_18 = uParam7;
	uParam0->f_23 = iParam12;
	uParam0->f_19 = fParam11;
	*uParam0 = unk_0x68C2E3309DCAD71E("DEFAULT_SCRIPTED_CAMERA", 0);
	unk_0x856549C07003344B(*uParam0, 1);
	unk_0x3FD8029C7A771A0E(*uParam0, uParam0->f_1, uParam0->f_4, uParam0->f_7, 0, 1, 1, 2);
	if (!bParam15)
	{
		unk_0x2311A669433389E8(*uParam0, "HAND_SHAKE", 0.19f);
	}
	unk_0x3ED9330214992278(1, 0, 3000, 1, 0, 0);
	if (fParam14 > 0f)
	{
		unk_0x8FFDD70F5142C6E0(*uParam0, fParam14);
	}
	if (uParam0->f_23)
	{
		unk_0x5F199185DD70A092(iParam13);
	}
	uParam0->f_24 = 0;
	uParam0->f_25 = 0;
	uParam0->f_29 = 0f;
	uParam0->f_30 = 0f;
	uParam0->f_26 = 0;
	uParam0->f_28 = 0;
	uParam0->f_27 = 0;
}

void func_896(var uParam0)
{
}

void func_897(var uParam0)
{
	func_878(uParam0);
	func_862(uParam0);
	func_726(uParam0);
	func_898(uParam0);
	if (uParam0->f_1497.f_724.f_5)
	{
		func_463(unk_0x460153A63B9477BC(), 0, 0, 0);
		if (!unk_0xFC559366953F62B3())
		{
			unk_0x6A8F948698B360B4(unk_0x9B0761B4C3EB8BC7(), true);
		}
		uParam0->f_4360 = 1;
		func_706(&(uParam0->f_1497.f_724), 2);
	}
}

void func_898(var uParam0)
{
	if ((uParam0->f_724 == 3 && uParam0->f_744.f_724 == 3) && uParam0->f_1497.f_724 == 3)
	{
		uParam0->f_703 = 0;
		uParam0->f_744.f_703 = 0;
		uParam0->f_1497.f_703 = 0;
	}
}

void func_899(var uParam0)
{
	if (unk_0xCE990E643CD9D0E5(Global_1701669[unk_0x1EF8FCAB83E4BE53() /*68*/].f_42, 1))
	{
		unk_0xD2459066EA58CE43(&(Global_1701669[unk_0x1EF8FCAB83E4BE53() /*68*/].f_42), 1);
	}
}

void func_900(var uParam0)
{
	func_878(uParam0);
	func_786(uParam0, 1);
	func_780(uParam0, 1);
	func_796(uParam0, 1);
	func_792(uParam0, 1);
	func_769(uParam0, 1);
	func_760(uParam0, 1);
	func_755(uParam0, 1);
	func_752(uParam0, 1);
	func_747(uParam0, 1);
	if (func_113(uParam0->f_4356) == 2)
	{
		func_743(uParam0, 1);
		func_739(uParam0, 1);
	}
	func_735(uParam0, 1);
	func_731(uParam0, 1);
	func_728(uParam0, 1);
}

void func_901(var uParam0)
{
	bool bVar0;
	
	bVar0 = true;
	if (!func_906(&(uParam0->f_1497)))
	{
		bVar0 = false;
	}
	if (!func_902(uParam0))
	{
		bVar0 = false;
	}
	if (bVar0)
	{
		func_706(&(uParam0->f_1497.f_724), 2);
	}
}

int func_902(var uParam0)
{
	uParam0->f_1497.f_736.f_85 = func_903(uParam0);
	if (uParam0->f_4356 != unk_0x460153A63B9477BC())
	{
		return 1;
	}
	switch (uParam0->f_1497.f_736.f_85)
	{
		case 1:
			uParam0->f_1497.f_736.f_92[0] = 100;
			uParam0->f_1497.f_736.f_92[1] = 0;
			uParam0->f_1497.f_736.f_92[2] = 0;
			uParam0->f_1497.f_736.f_92[3] = 0;
			uParam0->f_1497.f_736.f_92[4] = 0;
			break;
		
		case 2:
			uParam0->f_1497.f_736.f_92[0] = 85;
			uParam0->f_1497.f_736.f_92[1] = 15;
			uParam0->f_1497.f_736.f_92[2] = 0;
			uParam0->f_1497.f_736.f_92[3] = 0;
			uParam0->f_1497.f_736.f_92[4] = 0;
			break;
		
		case 3:
			uParam0->f_1497.f_736.f_92[0] = 70;
			uParam0->f_1497.f_736.f_92[1] = 15;
			uParam0->f_1497.f_736.f_92[2] = 15;
			uParam0->f_1497.f_736.f_92[3] = 0;
			uParam0->f_1497.f_736.f_92[4] = 0;
			break;
		
		case 4:
			uParam0->f_1497.f_736.f_92[0] = 55;
			uParam0->f_1497.f_736.f_92[1] = 15;
			uParam0->f_1497.f_736.f_92[2] = 15;
			uParam0->f_1497.f_736.f_92[3] = 15;
			uParam0->f_1497.f_736.f_92[4] = 0;
			break;
	}
	return 1;
}

int func_903(var uParam0)
{
	int iVar0;
	int iVar1;
	int iVar2;
	
	if (!unk_0xFC559366953F62B3())
	{
		uParam0->f_1497.f_736.f_87[0] = uParam0->f_4356;
		uParam0->f_1497.f_736.f_87[1] = -1;
		uParam0->f_1497.f_736.f_87[2] = -1;
		uParam0->f_1497.f_736.f_87[3] = -1;
		return 1;
	}
	iVar0 = func_42();
	iVar1 = 0;
	while (iVar1 <= 3)
	{
		uParam0->f_1497.f_736.f_87[iVar1] = -1;
		iVar1++;
	}
	iVar2 = 0;
	iVar1 = 0;
	while (iVar1 <= 31)
	{
		iVar0 = unk_0xF1110FE23C67293A(iVar1);
		if (func_203(iVar0, 1, 1))
		{
			if (iVar0 != func_42())
			{
				if (!func_904(iVar0))
				{
					if (iVar2 > 3)
					{
					}
					else
					{
						uParam0->f_1497.f_736.f_87[iVar2] = iVar0;
						iVar2++;
					}
				}
			}
		}
		iVar1++;
	}
	return iVar2;
}

int func_904(int iParam0)
{
	if (func_85(iParam0, 0) || func_905(iParam0))
	{
		return 1;
	}
	return 0;
}

bool func_905(int iParam0)
{
	return unk_0xCE990E643CD9D0E5(Global_1590682[iParam0 /*883*/].f_99.f_32, 5);
}

int func_906(var uParam0)
{
	if (!unk_0x95EF219D38B20CFF(uParam0->f_709))
	{
		return 0;
	}
	if (!unk_0x95EF219D38B20CFF(uParam0->f_710))
	{
		return 0;
	}
	return 1;
}

void func_907(var uParam0)
{
	func_909(&(uParam0->f_1497));
	func_908(uParam0);
}

void func_908(var uParam0)
{
	int iVar0;
	
	iVar0 = 0;
	iVar0 = 0;
	while (iVar0 < 4)
	{
		uParam0->f_1497.f_736.f_36[iVar0 /*10*/] = func_42();
		iVar0++;
	}
}

void func_909(var uParam0)
{
	uParam0->f_709 = unk_0x5DEA4192B46CB99B("CASINO_HEIST_BOARD_FINALE");
	uParam0->f_710 = unk_0x5DEA4192B46CB99B("INSTRUCTIONAL_BUTTONS");
}

void func_910(var uParam0)
{
	unk_0xD2459066EA58CE43(&(uParam0->f_4), uParam0->f_1);
}

void func_911(var uParam0)
{
	func_701(&(uParam0->f_2593), 99, 1);
	func_190(&(uParam0->f_1497), 1);
}

bool func_912(var uParam0)
{
	return !unk_0xCE990E643CD9D0E5(uParam0->f_4, uParam0->f_1);
}

void func_913(var uParam0)
{
	unk_0xBE20AB8238688965(&(uParam0->f_3), *uParam0);
}

void func_914(var uParam0)
{
}

bool func_915(var uParam0)
{
	return unk_0xCE990E643CD9D0E5(uParam0->f_3, *uParam0);
}

void func_916(var uParam0)
{
	if (!func_918(uParam0) && !func_917())
	{
		func_706(&(uParam0->f_1497.f_724), 2);
	}
}

int func_917()
{
	if (!unk_0x25DDB354A40FFCDB())
	{
		return 1;
	}
	if (func_498() && func_211() == 1)
	{
		return 1;
	}
	if (unk_0xCE990E643CD9D0E5(Global_1678289.f_6, 10))
	{
		return 1;
	}
	return 0;
}

int func_918(var uParam0)
{
	if (func_17(uParam0, 0))
	{
		return 1;
	}
	if (unk_0xAE06B9E39EBDE049(unk_0x9B0761B4C3EB8BC7()) && func_203(unk_0x460153A63B9477BC(), 1, 1))
	{
		if ((!func_15() && !func_498()) && !func_497())
		{
			if (!func_921(joaat("Main_RM")))
			{
				return 1;
			}
		}
	}
	if (func_920() == 1 && unk_0x7B70881748D166CD(unk_0x856D5567211886A2("appArcadeBusinessHub")) > 0)
	{
		return 1;
	}
	if (unk_0xCE990E643CD9D0E5(uParam0->f_1497.f_1015, 2))
	{
		return 1;
	}
	if (func_919())
	{
		return 1;
	}
	return 0;
}

bool func_919()
{
	return Global_2452015.f_694;
}

int func_920()
{
	return Global_2462507;
}

bool func_921(int iParam0)
{
	return Global_99007.f_350 == iParam0;
}

void func_922(var uParam0)
{
	func_923(uParam0);
}

void func_923(var uParam0)
{
	switch (uParam0->f_744.f_724)
	{
		case 0:
			func_1098(uParam0);
			break;
		
		case 1:
			if (!func_915(&(uParam0->f_744.f_724)))
			{
				func_1097(uParam0);
				func_913(&(uParam0->f_744.f_724));
			}
			func_1096(uParam0);
			if (!func_912(&(uParam0->f_744.f_724)))
			{
				func_1095(uParam0);
				func_910(&(uParam0->f_744.f_724));
			}
			break;
		
		case 2:
			if (!func_915(&(uParam0->f_744.f_724)))
			{
				func_1093(uParam0);
				func_913(&(uParam0->f_744.f_724));
			}
			func_1091(uParam0);
			if (!func_912(&(uParam0->f_744.f_724)))
			{
				func_1090(uParam0);
				func_910(&(uParam0->f_744.f_724));
			}
			break;
		
		case 3:
			if (!func_915(&(uParam0->f_744.f_724)))
			{
				func_1089(uParam0);
				func_913(&(uParam0->f_744.f_724));
			}
			func_1088(uParam0);
			if (!func_912(&(uParam0->f_744.f_724)))
			{
				func_1087(uParam0);
				func_910(&(uParam0->f_744.f_724));
			}
			break;
		
		case 4:
			if (!func_915(&(uParam0->f_744.f_724)))
			{
				func_1086(uParam0);
				func_913(&(uParam0->f_744.f_724));
			}
			func_927(uParam0);
			if (!func_912(&(uParam0->f_744.f_724)))
			{
				func_926(uParam0);
				func_910(&(uParam0->f_744.f_724));
			}
			break;
		
		case 5:
			if (!func_915(&(uParam0->f_744.f_724)))
			{
				func_925(uParam0);
				func_913(&(uParam0->f_744.f_724));
			}
			func_924(uParam0);
			break;
	}
}

void func_924(var uParam0)
{
}

void func_925(var uParam0)
{
	func_684();
	func_683(&(uParam0->f_744));
}

void func_926(var uParam0)
{
	if (!func_17(uParam0, 1))
	{
		func_701(&(uParam0->f_2593), 0, 0);
		uParam0->f_744.f_703 = 0;
	}
}

void func_927(var uParam0)
{
	func_891();
	func_1082(uParam0);
	func_1023(uParam0);
	func_1020(uParam0);
	func_984(uParam0);
	func_944(uParam0);
	func_929(uParam0);
	func_714(uParam0);
	func_713();
	func_712(uParam0, 1);
	if (!uParam0->f_744.f_724.f_5)
	{
		if (!func_17(uParam0, 1))
		{
			if (uParam0->f_4360 && !func_202(unk_0x460153A63B9477BC()))
			{
				func_463(unk_0x460153A63B9477BC(), 1, 0, 0);
				unk_0x6A8F948698B360B4(unk_0x9B0761B4C3EB8BC7(), false);
				uParam0->f_4360 = 0;
			}
			func_928(uParam0);
		}
		func_510(-1713398555, 12, 0);
		func_703(&(uParam0->f_744.f_724), 1);
	}
}

void func_928(var uParam0)
{
	func_699(&(uParam0->f_2529.f_1), 0, 1);
	unk_0xD50CC9B03AAD56FF(0, 0);
}

void func_929(var uParam0)
{
	int iVar0;
	
	iVar0 = uParam0->f_2593;
	if (uParam0->f_4355 != 1 || uParam0->f_4356 != unk_0x460153A63B9477BC())
	{
		func_943();
		return;
	}
	if ((((func_17(uParam0, 1) || uParam0->f_2529.f_62) || uParam0->f_2604) || uParam0->f_744.f_748 != -1) || uParam0->f_744.f_750)
	{
		return;
	}
	if (func_942(uParam0, iVar0) && !func_941(uParam0))
	{
		func_226("CH_HELP_CREW");
		return;
	}
	if (unk_0xCE990E643CD9D0E5(uParam0->f_4357, 9) && !unk_0xCE990E643CD9D0E5(uParam0->f_4357, 10))
	{
		if (!unk_0xDABD547F0DF2906C())
		{
			func_10("CSH_LAUNCHF_2", -1);
			unk_0xD2459066EA58CE43(&(uParam0->f_4357), 9);
			unk_0xBE20AB8238688965(&(uParam0->f_4357), 10);
		}
		return;
	}
	if (!func_19(uParam0, 7) && !func_18(uParam0, 7))
	{
		if (func_931(uParam0->f_4356))
		{
			func_189(uParam0, 7);
		}
	}
	if (!func_19(uParam0, 8) && !func_18(uParam0, 8))
	{
		if (func_484(uParam0->f_4356))
		{
			func_189(uParam0, 8);
		}
	}
	switch (iVar0)
	{
		case 18:
			if (func_930(uParam0->f_4356) == 0)
			{
				func_226("CH_HELP_MASKS");
			}
			break;
		
		case 2:
			if (func_942(uParam0, iVar0) && func_941(uParam0))
			{
				func_226("CH_HELP_SECI");
			}
			break;
		
		default:
			func_943();
			break;
	}
}

int func_930(int iParam0)
{
	if (iParam0 != -1)
	{
		return Global_1701669[iParam0 /*68*/].f_18.f_16;
	}
	return 0;
}

int func_931(int iParam0)
{
	if ((((((((func_494(iParam0) || func_493(iParam0)) || func_492(iParam0)) || func_490(iParam0)) || func_940(iParam0)) || func_939(iParam0)) || func_938(iParam0)) || func_937(iParam0)) || func_936(iParam0) != 0)
	{
		return 1;
	}
	switch (func_113(iParam0))
	{
		case 1:
			if (((func_489(iParam0) || func_488(iParam0)) || func_935(iParam0)) || func_934(iParam0))
			{
				return 1;
			}
			break;
		
		case 2:
			if ((func_172(iParam0) || func_487(iParam0)) || func_767(iParam0))
			{
				return 1;
			}
			break;
		
		case 3:
			if (((func_486(iParam0) || func_485(iParam0)) || func_933(iParam0)) || func_932(iParam0))
			{
				return 1;
			}
			break;
	}
	return 0;
}

int func_932(int iParam0)
{
	if (iParam0 != -1)
	{
		return unk_0xCE990E643CD9D0E5(Global_1701669[iParam0 /*68*/].f_18, 19);
	}
	return 0;
}

int func_933(int iParam0)
{
	if (iParam0 != -1)
	{
		return unk_0xCE990E643CD9D0E5(Global_1701669[iParam0 /*68*/].f_18, 20);
	}
	return 0;
}

int func_934(int iParam0)
{
	if (iParam0 != -1)
	{
		return unk_0xCE990E643CD9D0E5(Global_1701669[iParam0 /*68*/].f_18, 5);
	}
	return 0;
}

int func_935(int iParam0)
{
	if (iParam0 != -1)
	{
		return unk_0xCE990E643CD9D0E5(Global_1701669[iParam0 /*68*/].f_18, 3);
	}
	return 0;
}

int func_936(int iParam0)
{
	if (iParam0 != -1)
	{
		return Global_1701669[iParam0 /*68*/].f_18.f_15;
	}
	return 0;
}

int func_937(int iParam0)
{
	if (iParam0 != -1)
	{
		return unk_0xCE990E643CD9D0E5(Global_1701669[iParam0 /*68*/].f_18, 1);
	}
	return 0;
}

int func_938(int iParam0)
{
	if (iParam0 != -1)
	{
		return unk_0xCE990E643CD9D0E5(Global_1701669[iParam0 /*68*/].f_18, 4);
	}
	return 0;
}

int func_939(int iParam0)
{
	if (iParam0 != -1)
	{
		return unk_0xCE990E643CD9D0E5(Global_1701669[iParam0 /*68*/].f_18, 2);
	}
	return 0;
}

int func_940(int iParam0)
{
	if (iParam0 != -1)
	{
		return unk_0xCE990E643CD9D0E5(Global_1701669[iParam0 /*68*/].f_18, 0);
	}
	return 0;
}

int func_941(var uParam0)
{
	if ((func_164(uParam0->f_4356, 0) == 0 || func_153(uParam0->f_4356, 0) == 0) || func_491(uParam0->f_4356, 0) == 0)
	{
		return 0;
	}
	return 1;
}

int func_942(var uParam0, int iParam1)
{
	switch (iParam1)
	{
		case 13:
		case 15:
		case 16:
		case 17:
		case 9:
		case 8:
		case 4:
		case 3:
			if (!func_941(uParam0))
			{
				return 1;
			}
			break;
		
		case 14:
			if (!func_941(uParam0))
			{
				return 1;
			}
			switch (func_113(uParam0->f_4356))
			{
				case 2:
					switch (uParam0->f_744.f_744)
					{
						case 1:
							if (!func_179(uParam0->f_4356, 1))
							{
								return 1;
							}
							break;
						
						case 2:
							if (!func_179(uParam0->f_4356, 2))
							{
								return 1;
							}
							break;
					}
					break;
			}
			break;
		
		case 2:
			if (!func_180(uParam0->f_4356))
			{
				return 1;
			}
			if (!func_941(uParam0))
			{
				return 1;
			}
			break;
	}
	return 0;
}

void func_943()
{
	if ((func_227("CH_HELP_MASKS") || func_227("CH_HELP_CREW")) || func_227("CH_HELP_SECI"))
	{
		unk_0xD289B55B6AADBA10(1);
	}
}

void func_944(var uParam0)
{
	int iVar0;
	int iVar1;
	bool bVar2;
	
	iVar0 = func_857(&(uParam0->f_2604));
	iVar1 = uParam0->f_2593;
	if (((((func_17(uParam0, 1) || uParam0->f_2604) || uParam0->f_744.f_748 != -1) || uParam0->f_744.f_750) || uParam0->f_744.f_708) || (func_16(uParam0) && iVar0 != 5))
	{
		return;
	}
	if (func_890(&(uParam0->f_744.f_751)))
	{
		if (func_503(&(uParam0->f_744.f_751), 1000, 0))
		{
			func_502(&(uParam0->f_744.f_751));
		}
		if (((iVar0 == 0 || iVar0 == 1) || iVar0 == 2) || iVar0 == 3)
		{
			return;
		}
	}
	switch (uParam0->f_744.f_724.f_2)
	{
		case 0:
			switch (iVar0)
			{
				case 0:
				case 1:
				case 2:
				case 3:
					func_983(uParam0, iVar0);
					func_196(uParam0, 1);
					func_853(uParam0);
					break;
				
				case 9:
					if (func_483(uParam0, 0))
					{
						func_850(uParam0, 0);
					}
					break;
				
				case 10:
					if (func_483(uParam0, 2))
					{
						func_850(uParam0, 2);
					}
					break;
				
				case 7:
					if (func_982(iVar1) && !func_981(uParam0, iVar1))
					{
						func_980(uParam0, iVar1);
					}
					if (func_979(uParam0, iVar1) && (func_978(uParam0, iVar1) || func_977(uParam0, iVar1)))
					{
						func_980(uParam0, iVar1);
					}
					if ((func_976(uParam0, iVar1) && func_978(uParam0, iVar1)) && unk_0x460153A63B9477BC() == uParam0->f_4356)
					{
						func_970(uParam0, iVar1, 0);
					}
					break;
				
				case 8:
					func_201(uParam0, 0);
					break;
				
				case 5:
					if (func_979(uParam0, iVar1) || func_976(uParam0, iVar1))
					{
						if (!unk_0x7BCC91F3C1CF24E8(func_181(uParam0, 1, iVar1)))
						{
							func_836(uParam0, 0);
						}
					}
					else if (func_982(iVar1))
					{
						if (!unk_0x7BCC91F3C1CF24E8(func_166(uParam0, iVar1)))
						{
							func_836(uParam0, 0);
						}
					}
					else if (iVar1 == 18)
					{
						func_836(uParam0, 0);
					}
					break;
				
				case 4:
					if ((func_976(uParam0, iVar1) || func_979(uParam0, iVar1)) && unk_0x460153A63B9477BC() == uParam0->f_4356)
					{
						if (func_965(uParam0, iVar1) && func_964(uParam0, iVar1))
						{
							uParam0->f_744.f_748 = 12;
							unk_0xC4CC25B68A91D144(-1, "Highlight_Accept", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
						}
					}
					bVar2 = true;
					break;
			}
			break;
		
		case 1:
			switch (iVar0)
			{
				case 2:
				case 3:
					if (func_982(iVar1) || func_979(uParam0, iVar1))
					{
						func_949(uParam0, iVar1, iVar0);
					}
					func_853(uParam0);
					break;
				
				case 7:
					if (((func_982(iVar1) && !func_981(uParam0, iVar1)) && func_162(uParam0, iVar1)) && unk_0x460153A63B9477BC() == uParam0->f_4356)
					{
						uParam0->f_744.f_748 = func_948(iVar1);
						unk_0xC4CC25B68A91D144(-1, "Highlight_Accept", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
					}
					if ((func_979(uParam0, iVar1) && func_978(uParam0, iVar1)) && unk_0x460153A63B9477BC() == uParam0->f_4356)
					{
						if (func_947(iVar1))
						{
							func_970(uParam0, iVar1, 0);
						}
						else
						{
							uParam0->f_744.f_748 = func_948(iVar1);
							unk_0xC4CC25B68A91D144(-1, "Highlight_Accept", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
						}
					}
					break;
				
				case 8:
					func_946(uParam0, iVar1, 0);
					break;
				
				case 5:
					if (func_979(uParam0, iVar1) || func_982(iVar1))
					{
						func_836(uParam0, 0);
					}
					break;
				
				case 4:
					if ((func_976(uParam0, iVar1) || func_979(uParam0, iVar1)) && unk_0x460153A63B9477BC() == uParam0->f_4356)
					{
						if (func_965(uParam0, iVar1) && func_964(uParam0, iVar1))
						{
							uParam0->f_744.f_748 = 12;
							unk_0xC4CC25B68A91D144(-1, "Highlight_Accept", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
						}
					}
					bVar2 = true;
					break;
			}
			break;
	}
	if (bVar2)
	{
		func_945(&(uParam0->f_744.f_751), 0, 0);
	}
}

void func_945(var uParam0, bool bParam1, bool bParam2)
{
	if (unk_0x25DDB354A40FFCDB() && !bParam1)
	{
		if (!bParam2)
		{
			*uParam0 = unk_0x6CAAB7E78B5D978A();
		}
		else
		{
			*uParam0 = unk_0x29B9AF1CB5B8175D();
		}
	}
	else
	{
		*uParam0 = unk_0x578C4EF320340AF7();
	}
	uParam0->f_1 = 1;
}

void func_946(var uParam0, int iParam1, bool bParam2)
{
	if (uParam0->f_744.f_724.f_2 == 0)
	{
		return;
	}
	uParam0->f_744.f_724.f_2 = 0;
	uParam0->f_744.f_706 = 1;
	if (bParam2)
	{
		func_756(uParam0->f_744.f_709, iParam1, 2);
		unk_0xC4CC25B68A91D144(-1, "Highlight_Accept", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
	}
	else
	{
		func_756(uParam0->f_744.f_709, iParam1, 0);
		unk_0xC4CC25B68A91D144(-1, "Back", "HUD_FRONTEND_DEFAULT_SOUNDSET", 1);
	}
	func_190(&(uParam0->f_744), 1);
	if (uParam0->f_2529.f_33 != 1)
	{
		uParam0->f_2529.f_33 = 1;
	}
}

int func_947(int iParam0)
{
	switch (iParam0)
	{
		case 14:
		case 15:
		case 16:
		case 17:
			return 1;
		
		default:
	}
	return 0;
}

int func_948(int iParam0)
{
	switch (iParam0)
	{
		case 10:
			return 0;
		
		case 11:
			return 1;
		
		case 12:
			return 2;
		
		case 5:
			return 7;
		
		case 6:
			return 8;
		
		case 13:
			return 13;
		
		default:
	}
	return -1;
}

void func_949(var uParam0, int iParam1, int iParam2)
{
	switch (iParam1)
	{
		case 10:
			switch (iParam2)
			{
				case 2:
					if (func_963(uParam0, 0))
					{
						unk_0xC4CC25B68A91D144(-1, "Paper_Shuffle", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
						func_962(uParam0, 1);
						func_190(&(uParam0->f_744), 1);
					}
					break;
				
				case 3:
					if (func_963(uParam0, 1))
					{
						unk_0xC4CC25B68A91D144(-1, "Paper_Shuffle", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
						func_962(uParam0, 1);
						func_190(&(uParam0->f_744), 1);
					}
					break;
			}
			break;
		
		case 11:
			switch (iParam2)
			{
				case 2:
					if (func_961(uParam0, 0))
					{
						unk_0xC4CC25B68A91D144(-1, "Paper_Shuffle", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
						func_960(uParam0, 1);
						func_190(&(uParam0->f_744), 1);
					}
					break;
				
				case 3:
					if (func_961(uParam0, 1))
					{
						unk_0xC4CC25B68A91D144(-1, "Paper_Shuffle", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
						func_960(uParam0, 1);
						func_190(&(uParam0->f_744), 1);
					}
					break;
			}
			break;
		
		case 12:
			switch (iParam2)
			{
				case 2:
					if (func_959(uParam0, 0))
					{
						unk_0xC4CC25B68A91D144(-1, "Paper_Shuffle", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
						func_958(uParam0, 1);
						func_190(&(uParam0->f_744), 1);
					}
					break;
				
				case 3:
					if (func_959(uParam0, 1))
					{
						unk_0xC4CC25B68A91D144(-1, "Paper_Shuffle", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
						func_958(uParam0, 1);
						func_190(&(uParam0->f_744), 1);
					}
					break;
			}
			break;
		
		case 5:
			switch (iParam2)
			{
				case 2:
					uParam0->f_744.f_739 = (uParam0->f_744.f_739 - 1);
					if (uParam0->f_744.f_739 < 0)
					{
						uParam0->f_744.f_739 = 1;
					}
					unk_0xC4CC25B68A91D144(-1, "Paper_Shuffle", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
					func_957(uParam0, 1);
					break;
				
				case 3:
					uParam0->f_744.f_739++;
					if (uParam0->f_744.f_739 > 1)
					{
						uParam0->f_744.f_739 = 0;
					}
					unk_0xC4CC25B68A91D144(-1, "Paper_Shuffle", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
					func_957(uParam0, 1);
					break;
			}
			break;
		
		case 6:
			switch (iParam2)
			{
				case 2:
					uParam0->f_744.f_741 = (uParam0->f_744.f_741 - 1);
					if (uParam0->f_744.f_741 < 0)
					{
						uParam0->f_744.f_741 = 3;
					}
					unk_0xC4CC25B68A91D144(-1, "Paper_Shuffle", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
					func_956(uParam0, 1);
					break;
				
				case 3:
					uParam0->f_744.f_741++;
					if (uParam0->f_744.f_741 > 3)
					{
						uParam0->f_744.f_741 = 0;
					}
					unk_0xC4CC25B68A91D144(-1, "Paper_Shuffle", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
					func_956(uParam0, 1);
					break;
			}
			break;
		
		case 13:
			switch (iParam2)
			{
				case 2:
					uParam0->f_744.f_742 = (uParam0->f_744.f_742 - 1);
					if (uParam0->f_744.f_742 < 1)
					{
						uParam0->f_744.f_742 = 2;
					}
					unk_0xC4CC25B68A91D144(-1, "Paper_Shuffle", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
					func_955(uParam0, 1);
					func_954(uParam0, 1);
					break;
				
				case 3:
					uParam0->f_744.f_742++;
					if (uParam0->f_744.f_742 > 2)
					{
						uParam0->f_744.f_742 = 1;
					}
					unk_0xC4CC25B68A91D144(-1, "Paper_Shuffle", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
					func_955(uParam0, 1);
					func_954(uParam0, 1);
					break;
			}
			break;
		
		case 14:
			switch (iParam2)
			{
				case 2:
					if (func_953(uParam0, 0))
					{
						unk_0xC4CC25B68A91D144(-1, "Paper_Shuffle", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
						func_952(uParam0, 1, 0);
						func_190(&(uParam0->f_744), 1);
						if (func_170(uParam0->f_4356, uParam0->f_744.f_744))
						{
							func_189(uParam0, 10);
						}
					}
					break;
				
				case 3:
					if (func_953(uParam0, 1))
					{
						unk_0xC4CC25B68A91D144(-1, "Paper_Shuffle", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
						func_952(uParam0, 1, 0);
						func_190(&(uParam0->f_744), 1);
						if (func_170(uParam0->f_4356, uParam0->f_744.f_744))
						{
							func_189(uParam0, 10);
						}
					}
					break;
			}
			break;
		
		case 17:
			switch (iParam2)
			{
				case 2:
					if (func_950(uParam0, 0))
					{
						unk_0xC4CC25B68A91D144(-1, "Paper_Shuffle", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
						func_952(uParam0, 1, 3);
						func_190(&(uParam0->f_744), 1);
						func_954(uParam0, 1);
					}
					break;
				
				case 3:
					if (func_950(uParam0, 1))
					{
						unk_0xC4CC25B68A91D144(-1, "Paper_Shuffle", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
						func_952(uParam0, 1, 3);
						func_190(&(uParam0->f_744), 1);
						func_954(uParam0, 1);
					}
					break;
			}
			break;
	}
}

int func_950(var uParam0, bool bParam1)
{
	bool bVar0;
	
	while (!bVar0)
	{
		if (bParam1)
		{
			uParam0->f_744.f_745++;
			if (uParam0->f_744.f_745 > 7)
			{
				uParam0->f_744.f_745 = 6;
			}
		}
		else
		{
			uParam0->f_744.f_745 = (uParam0->f_744.f_745 - 1);
			if (uParam0->f_744.f_745 == 5)
			{
				uParam0->f_744.f_745 = 7;
			}
		}
		if (func_951(uParam0))
		{
			bVar0 = true;
			return 1;
		}
	}
	return 0;
}

int func_951(var uParam0)
{
	switch (uParam0->f_744.f_744)
	{
		case 3:
			if (!func_179(uParam0->f_4356, 9))
			{
				return 0;
			}
			break;
	}
	return 1;
}

void func_952(var uParam0, bool bParam1, int iParam2)
{
	if (bParam1)
	{
		unk_0xBE20AB8238688965(&(uParam0->f_744.f_736), 7);
		uParam0->f_744.f_747 = iParam2;
	}
	else
	{
		unk_0xD2459066EA58CE43(&(uParam0->f_744.f_736), 7);
		uParam0->f_744.f_747 = -1;
	}
}

int func_953(var uParam0, bool bParam1)
{
	bool bVar0;
	
	while (!bVar0)
	{
		if (bParam1)
		{
			uParam0->f_744.f_744++;
			if (uParam0->f_744.f_744 > 3)
			{
				uParam0->f_744.f_744 = 1;
			}
		}
		else
		{
			uParam0->f_744.f_744 = (uParam0->f_744.f_744 - 1);
			if (uParam0->f_744.f_744 == 0)
			{
				uParam0->f_744.f_744 = 3;
			}
		}
		if (func_951(uParam0))
		{
			bVar0 = true;
			return 1;
		}
	}
	return 0;
}

void func_954(var uParam0, bool bParam1)
{
	if (bParam1)
	{
		unk_0xBE20AB8238688965(&(uParam0->f_744.f_736), 15);
	}
	else
	{
		unk_0xD2459066EA58CE43(&(uParam0->f_744.f_736), 15);
	}
}

void func_955(var uParam0, bool bParam1)
{
	if (bParam1)
	{
		unk_0xBE20AB8238688965(&(uParam0->f_744.f_736), 11);
	}
	else
	{
		unk_0xD2459066EA58CE43(&(uParam0->f_744.f_736), 11);
	}
}

void func_956(var uParam0, bool bParam1)
{
	if (bParam1)
	{
		unk_0xBE20AB8238688965(&(uParam0->f_744.f_736), 10);
	}
	else
	{
		unk_0xD2459066EA58CE43(&(uParam0->f_744.f_736), 10);
	}
}

void func_957(var uParam0, bool bParam1)
{
	if (bParam1)
	{
		unk_0xBE20AB8238688965(&(uParam0->f_744.f_736), 9);
	}
	else
	{
		unk_0xD2459066EA58CE43(&(uParam0->f_744.f_736), 9);
	}
}

void func_958(var uParam0, bool bParam1)
{
	if (bParam1)
	{
		unk_0xBE20AB8238688965(&(uParam0->f_744.f_736), 5);
	}
	else
	{
		unk_0xD2459066EA58CE43(&(uParam0->f_744.f_736), 5);
	}
}

int func_959(var uParam0, bool bParam1)
{
	bool bVar0;
	
	while (!bVar0)
	{
		if (bParam1)
		{
			uParam0->f_744.f_743++;
			if (uParam0->f_744.f_743 > 5)
			{
				uParam0->f_744.f_743 = 1;
			}
		}
		else
		{
			uParam0->f_744.f_743 = (uParam0->f_744.f_743 - 1);
			if (uParam0->f_744.f_743 == 0)
			{
				uParam0->f_744.f_743 = 5;
			}
		}
		bVar0 = true;
		return 1;
	}
	return 0;
}

void func_960(var uParam0, bool bParam1)
{
	if (bParam1)
	{
		unk_0xBE20AB8238688965(&(uParam0->f_744.f_736), 4);
	}
	else
	{
		unk_0xD2459066EA58CE43(&(uParam0->f_744.f_736), 4);
	}
}

int func_961(var uParam0, bool bParam1)
{
	bool bVar0;
	
	while (!bVar0)
	{
		if (bParam1)
		{
			uParam0->f_744.f_740++;
			if (uParam0->f_744.f_740 > 5)
			{
				uParam0->f_744.f_740 = 1;
			}
		}
		else
		{
			uParam0->f_744.f_740 = (uParam0->f_744.f_740 - 1);
			if (uParam0->f_744.f_740 == 0)
			{
				uParam0->f_744.f_740 = 5;
			}
		}
		bVar0 = true;
		return 1;
	}
	return 0;
}

void func_962(var uParam0, bool bParam1)
{
	if (bParam1)
	{
		unk_0xBE20AB8238688965(&(uParam0->f_744.f_736), 3);
	}
	else
	{
		unk_0xD2459066EA58CE43(&(uParam0->f_744.f_736), 3);
	}
}

int func_963(var uParam0, bool bParam1)
{
	bool bVar0;
	
	while (!bVar0)
	{
		if (bParam1)
		{
			uParam0->f_744.f_738++;
			if (uParam0->f_744.f_738 > 5)
			{
				uParam0->f_744.f_738 = 1;
			}
		}
		else
		{
			uParam0->f_744.f_738 = (uParam0->f_744.f_738 - 1);
			if (uParam0->f_744.f_738 == 0)
			{
				uParam0->f_744.f_738 = 5;
			}
		}
		bVar0 = true;
		return 1;
	}
	return 0;
}

bool func_964(var uParam0, int iParam1)
{
	return unk_0x6914A85D2E17013B(func_530(), 0, 1, 0, -1, 0);
}

int func_965(var uParam0, int iParam1)
{
	if ((!func_969(uParam0, iParam1) || func_942(uParam0, iParam1)) || func_966(uParam0, iParam1, 0))
	{
		return 0;
	}
	switch (iParam1)
	{
		case 3:
		case 8:
		case 13:
			return 1;
		
		case 16:
			switch (func_113(uParam0->f_4356))
			{
				case 1:
					return 1;
				
				case 3:
					return 1;
				
				default:
			}
			break;
		
		case 17:
			switch (func_113(uParam0->f_4356))
			{
				case 1:
					return 1;
				
				case 2:
					return 1;
				
				case 3:
					return 1;
				
				default:
			}
			break;
	}
	return 0;
}

int func_966(var uParam0, int iParam1, bool bParam2)
{
	switch (iParam1)
	{
		case 2:
			if (bParam2)
			{
				return func_940(uParam0->f_4356);
			}
			return func_968(uParam0->f_4356);
		
		case 3:
			return func_937(uParam0->f_4356);
		
		case 4:
			return func_939(uParam0->f_4356);
		
		case 5:
			return func_493(uParam0->f_4356);
		
		case 6:
			return func_492(uParam0->f_4356);
		
		case 7:
			return func_490(uParam0->f_4356);
		
		case 9:
			return func_494(uParam0->f_4356);
		
		case 8:
			return func_938(uParam0->f_4356);
		
		case 13:
			return func_936(uParam0->f_4356) != 0;
		
		case 14:
			return func_967(uParam0, func_113(uParam0->f_4356), 0);
		
		case 15:
			return func_967(uParam0, func_113(uParam0->f_4356), 1);
		
		case 16:
			return func_967(uParam0, func_113(uParam0->f_4356), 2);
		
		case 17:
			return func_967(uParam0, func_113(uParam0->f_4356), 3);
		
		default:
	}
	return 0;
}

int func_967(var uParam0, int iParam1, int iParam2)
{
	switch (iParam1)
	{
		case 1:
			switch (iParam2)
			{
				case 0:
					return func_489(uParam0->f_4356);
				
				case 1:
					return func_488(uParam0->f_4356);
				
				case 2:
					return func_934(uParam0->f_4356);
				
				case 3:
					return func_935(uParam0->f_4356);
				
				default:
			}
			break;
		
		case 2:
			switch (iParam2)
			{
				case 0:
					return func_173(uParam0->f_4356, uParam0->f_744.f_744);
				
				case 1:
					return func_487(uParam0->f_4356);
				
				case 2:
					return func_173(uParam0->f_4356, 4);
				
				case 3:
					return func_173(uParam0->f_4356, uParam0->f_744.f_745);
				
				default:
			}
			break;
		
		case 3:
			switch (iParam2)
			{
				case 0:
					return func_485(uParam0->f_4356);
				
				case 1:
					return func_486(uParam0->f_4356);
				
				case 2:
					return func_933(uParam0->f_4356);
				
				case 3:
					return func_932(uParam0->f_4356);
				
				default:
			}
			break;
	}
	return 0;
}

int func_968(int iParam0)
{
	if (iParam0 != -1)
	{
		return unk_0xCE990E643CD9D0E5(Global_1701669[iParam0 /*68*/].f_40, 22);
	}
	return 0;
}

int func_969(var uParam0, int iParam1)
{
	switch (iParam1)
	{
		case 5:
			if (func_164(uParam0->f_4356, 0) == 0)
			{
				return 0;
			}
			break;
		
		case 6:
			if (func_153(uParam0->f_4356, 0) == 0)
			{
				return 0;
			}
			break;
		
		case 7:
			if (func_491(uParam0->f_4356, 0) == 0)
			{
				return 0;
			}
			break;
		
		case 16:
			switch (func_778(uParam0))
			{
				case 2:
					if (!unk_0xCE990E643CD9D0E5(Global_1701669[uParam0->f_4356 /*68*/].f_42, 6))
					{
						return 0;
					}
					break;
			}
			break;
		
		case 17:
			switch (func_778(uParam0))
			{
				case 3:
					if (!func_179(uParam0->f_4356, 10))
					{
						return 0;
					}
					break;
			}
			break;
	}
	return 1;
}

void func_970(var uParam0, int iParam1, bool bParam2)
{
	int iVar0;
	int iVar1;
	int iVar2;
	
	if (func_460(bParam2))
	{
		iVar0 = func_975(uParam0, iParam1);
		iVar1 = func_974(uParam0, iVar0);
		iVar2 = func_973(uParam0, iVar0);
		func_972(iVar0, iVar1, iVar2);
		func_443();
		func_442(uParam0, 2);
		func_853(uParam0);
		Global_1701669[unk_0x1EF8FCAB83E4BE53() /*68*/].f_67 = 1;
		if (iParam1 == 14 && func_113(uParam0->f_4356) == 2)
		{
			Global_1701669[uParam0->f_4356 /*68*/].f_64 = uParam0->f_744.f_744;
		}
		unk_0xC4CC25B68A91D144(-1, "Highlight_Accept", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
	}
	else
	{
		switch (func_278(unk_0x460153A63B9477BC(), 158, 0))
		{
			case 15:
				if (func_276(unk_0x460153A63B9477BC()))
				{
					func_10("GENERAL_MLF_G1", -1);
				}
				else if (func_275(unk_0x460153A63B9477BC()))
				{
					func_10("GENERAL_MLF_G2", -1);
				}
				else
				{
					func_10("GENERAL_MLF_G3", -1);
				}
				break;
			
			case 16:
				if (func_276(unk_0x460153A63B9477BC()))
				{
					func_10("GENERAL_MLF_G4", -1);
				}
				else if (func_275(unk_0x460153A63B9477BC()))
				{
					func_10("GENERAL_MLF_G5", -1);
				}
				else
				{
					func_10("GENERAL_MLF_G6", -1);
				}
				break;
			
			default:
				if (func_971(func_975(uParam0, iParam1)))
				{
					func_10("CSH_LAUNCHF_0", -1);
				}
				else
				{
					func_10("CSH_LAUNCHF_1", -1);
				}
				if (func_965(uParam0, iParam1))
				{
					unk_0xBE20AB8238688965(&(uParam0->f_4357), 9);
				}
				break;
			}
	}
}

int func_971(int iParam0)
{
	switch (iParam0)
	{
		case 1:
		case 13:
		case 7:
		case 3:
		case 30:
		case 5:
		case 24:
		case 4:
		case 22:
		case 6:
		case 27:
		case 28:
			return 1;
		
		default:
	}
	return 0;
}

void func_972(int iParam0, int iParam1, int iParam2)
{
	func_446(-1, iParam0, iParam1, iParam2);
}

int func_973(var uParam0, int iParam1)
{
	switch (iParam1)
	{
		case 1:
			return uParam0->f_744.f_739;
		
		case 2:
			return uParam0->f_744.f_741;
		
		default:
	}
	return -1;
}

int func_974(var uParam0, int iParam1)
{
	switch (iParam1)
	{
		case 1:
			return uParam0->f_744.f_738;
		
		case 2:
			return uParam0->f_744.f_740;
		
		case 3:
			return uParam0->f_744.f_743;
		
		default:
	}
	return -1;
}

int func_975(var uParam0, int iParam1)
{
	switch (iParam1)
	{
		case 2:
			return 17;
		
		case 3:
			return 18;
		
		case 4:
			return 16;
		
		case 5:
			return 1;
		
		case 6:
			return 2;
		
		case 7:
			return 3;
		
		case 8:
			return 5;
		
		case 9:
			return 8;
		
		case 13:
			switch (uParam0->f_744.f_742)
			{
				case 1:
					return 9;
				
				case 2:
					return 10;
				
				default:
			}
			break;
		
		case 14:
			switch (func_113(uParam0->f_4356))
			{
				case 1:
					return 11;
					break;
				
				case 2:
					switch (uParam0->f_744.f_744)
					{
						case 1:
							if (!func_170(uParam0->f_4356, 1))
							{
								return 23;
							}
							else
							{
								return 21;
							}
							break;
						
						case 2:
							if (!func_170(uParam0->f_4356, 2))
							{
								return 24;
							}
							else
							{
								return 25;
							}
							break;
						
						case 3:
							if (!func_170(uParam0->f_4356, 3))
							{
								return 26;
							}
							else
							{
								return 27;
							}
							break;
					}
					break;
				
				case 3:
					return 13;
					break;
			}
			break;
		
		case 15:
			switch (func_113(uParam0->f_4356))
			{
				case 1:
					return 4;
					break;
				
				case 2:
					return 6;
					break;
				
				case 3:
					return 7;
					break;
			}
			break;
		
		case 16:
			switch (func_113(uParam0->f_4356))
			{
				case 1:
					return 14;
					break;
				
				case 2:
					if (!func_170(uParam0->f_4356, 4))
					{
						return 32;
					}
					else
					{
						return 33;
					}
					break;
				
				case 3:
					return 30;
					break;
			}
			break;
		
		case 17:
			switch (func_113(uParam0->f_4356))
			{
				case 1:
					return 22;
					break;
				
				case 2:
					switch (uParam0->f_744.f_745)
					{
						case 6:
							return 28;
						
						case 7:
							return 29;
						
						default:
					}
					break;
				
				case 3:
					return 15;
					break;
			}
			break;
	}
	return -1;
}

int func_976(var uParam0, int iParam1)
{
	switch (iParam1)
	{
		case 2:
		case 3:
		case 4:
		case 9:
		case 7:
		case 8:
		case 15:
		case 16:
			return 1;
		
		case 14:
		case 17:
			switch (func_113(uParam0->f_4356))
			{
				case 1:
				case 3:
					return 1;
				
				default:
			}
			break;
	}
	return 0;
}

int func_977(var uParam0, int iParam1)
{
	switch (iParam1)
	{
		case 14:
		case 17:
			switch (func_113(uParam0->f_4356))
			{
				case 2:
					return 1;
				
				default:
			}
			break;
	}
	return 0;
}

int func_978(var uParam0, int iParam1)
{
	switch (iParam1)
	{
		case 5:
			if (func_164(uParam0->f_4356, 0) == 0)
			{
				return 0;
			}
			if (func_493(uParam0->f_4356))
			{
				return 0;
			}
			break;
		
		case 6:
			if (func_153(uParam0->f_4356, 0) == 0)
			{
				return 0;
			}
			if (func_492(uParam0->f_4356))
			{
				return 0;
			}
			break;
		
		case 7:
			if (func_491(uParam0->f_4356, 0) == 0)
			{
				return 0;
			}
			if (func_490(uParam0->f_4356))
			{
				return 0;
			}
			break;
		
		case 13:
			if (func_936(uParam0->f_4356) == 2)
			{
				return 0;
			}
			else if (func_969(uParam0, iParam1) && !func_942(uParam0, iParam1))
			{
				return 1;
			}
			break;
	}
	if ((!func_969(uParam0, iParam1) || func_942(uParam0, iParam1)) || func_966(uParam0, iParam1, 1))
	{
		return 0;
	}
	return 1;
}

int func_979(var uParam0, int iParam1)
{
	switch (iParam1)
	{
		case 5:
		case 6:
		case 13:
			return 1;
		
		case 14:
		case 17:
			switch (func_113(uParam0->f_4356))
			{
				case 2:
					return 1;
				
				default:
			}
			break;
	}
	return 0;
}

void func_980(var uParam0, int iParam1)
{
	if (uParam0->f_744.f_724.f_2 == 1)
	{
		return;
	}
	uParam0->f_744.f_724.f_2 = 1;
	uParam0->f_744.f_706 = 1;
	func_756(uParam0->f_744.f_709, iParam1, 1);
	func_190(&(uParam0->f_744), 1);
	if (uParam0->f_4356 == unk_0x460153A63B9477BC())
	{
		func_191(uParam0, func_843(uParam0));
		if (func_982(iParam1))
		{
			func_189(uParam0, 4);
		}
		else if (func_113(uParam0->f_4356) == 2 && iParam1 == 14)
		{
			if (func_170(uParam0->f_4356, uParam0->f_744.f_744))
			{
				func_189(uParam0, 10);
			}
			else
			{
				func_189(uParam0, 9);
			}
		}
	}
	if (uParam0->f_2529.f_33 != func_842(uParam0))
	{
		uParam0->f_2529.f_33 = func_842(uParam0);
	}
	unk_0xC4CC25B68A91D144(-1, "Highlight_Accept", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
}

int func_981(var uParam0, int iParam1)
{
	switch (iParam1)
	{
		case 10:
			return func_164(uParam0->f_4356, 0) != 0;
		
		case 11:
			return func_153(uParam0->f_4356, 0) != 0;
		
		case 12:
			return func_491(uParam0->f_4356, 0) != 0;
		
		default:
	}
	return 0;
}

int func_982(int iParam0)
{
	switch (iParam0)
	{
		case 10:
		case 11:
		case 12:
			return 1;
		
		default:
	}
	return 0;
}

void func_983(var uParam0, int iParam1)
{
	uParam0->f_2593.f_1 = 0;
	switch (iParam1)
	{
		case 1:
			func_856(uParam0->f_744.f_709, 187);
			unk_0xC4CC25B68A91D144(-1, "Highlight_Move", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
			break;
		
		case 0:
			func_856(uParam0->f_744.f_709, 188);
			unk_0xC4CC25B68A91D144(-1, "Highlight_Move", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
			break;
		
		case 2:
			func_856(uParam0->f_744.f_709, 189);
			unk_0xC4CC25B68A91D144(-1, "Highlight_Move", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
			break;
		
		case 3:
			func_856(uParam0->f_744.f_709, 190);
			unk_0xC4CC25B68A91D144(-1, "Highlight_Move", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
			break;
	}
}

void func_984(var uParam0)
{
	bool bVar0;
	
	if (uParam0->f_744.f_748 == -1)
	{
		return;
	}
	if (uParam0->f_744.f_748 == 12)
	{
		func_191(uParam0, 75);
	}
	if (func_529(uParam0, uParam0->f_744.f_748, &bVar0))
	{
		if (bVar0)
		{
			switch (uParam0->f_744.f_748)
			{
				case 0:
					func_1019(uParam0->f_744.f_738);
					func_957(uParam0, 1);
					func_1018(uParam0, 1);
					func_1017(uParam0, 1);
					func_1016(uParam0->f_744.f_709, 10, 1);
					func_946(uParam0, 10, 1);
					func_191(uParam0, func_1015(uParam0->f_744.f_738));
					func_1014(uParam0, 1);
					break;
				
				case 1:
					func_1013(uParam0->f_744.f_740);
					func_956(uParam0, 1);
					func_1018(uParam0, 1);
					func_1017(uParam0, 1);
					func_1016(uParam0->f_744.f_709, 11, 1);
					func_946(uParam0, 11, 1);
					func_191(uParam0, func_1012(uParam0->f_744.f_740));
					func_1014(uParam0, 1);
					break;
				
				case 2:
					func_1011(uParam0->f_744.f_743);
					func_1018(uParam0, 1);
					func_1017(uParam0, 1);
					func_1016(uParam0->f_744.f_709, 12, 1);
					func_946(uParam0, 12, 1);
					func_191(uParam0, func_1010(uParam0->f_744.f_743));
					func_1014(uParam0, 1);
					break;
				
				case 7:
					func_946(uParam0, 5, 1);
					func_970(uParam0, 5, 1);
					func_1014(uParam0, 1);
					break;
				
				case 8:
					func_946(uParam0, 6, 1);
					func_970(uParam0, 6, 1);
					func_1014(uParam0, 1);
					break;
				
				case 12:
					func_1009(uParam0, 1);
					func_1008(uParam0, 1);
					func_1007(uParam0, 1);
					func_1005();
					func_191(uParam0, 76);
					func_985(uParam0);
					func_1014(uParam0, 1);
					break;
				
				case 13:
					func_946(uParam0, 13, 1);
					func_970(uParam0, 13, 1);
					func_1014(uParam0, 1);
					break;
			}
			uParam0->f_744.f_748 = -1;
			uParam0->f_744.f_750 = 1;
		}
		else
		{
			uParam0->f_744.f_748 = -1;
			uParam0->f_744.f_750 = 1;
			uParam0->f_744.f_749 = 0;
		}
	}
}

void func_985(var uParam0)
{
	int iVar0;
	
	if (func_527())
	{
		func_515(-1295545795, func_530(), &iVar0, 0, 1, 0);
		Global_4264051[iVar0 /*85*/].f_9 = uParam0->f_2593;
	}
	else
	{
		unk_0x028944378AC816A9(uParam0->f_2593, func_530(), 0, 1);
	}
	unk_0xC4CC25B68A91D144(-1, "Highlight_Accept", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
	func_1003(1);
	func_986(uParam0);
	func_190(&(uParam0->f_744), 1);
}

void func_986(var uParam0)
{
	int iVar0;
	bool bVar1;
	
	iVar0 = uParam0->f_2593;
	func_1000(iVar0, func_113(uParam0->f_4356), func_1002(uParam0, iVar0), 1);
	func_999(uParam0->f_744.f_709, iVar0, 1);
	func_998(uParam0->f_744.f_709, iVar0, 0, -1, -1);
	switch (iVar0)
	{
		case 3:
			func_997(1);
			break;
		
		case 8:
			func_996(1);
			break;
		
		case 13:
			func_995(2);
			func_994(uParam0->f_744.f_709, 2);
			break;
		
		case 16:
			switch (func_113(uParam0->f_4356))
			{
				case 1:
					func_993(1);
					break;
				
				case 3:
					func_992(1);
					break;
			}
			break;
		
		case 17:
			switch (func_113(uParam0->f_4356))
			{
				case 1:
					func_991(1);
					break;
				
				case 2:
					switch (uParam0->f_744.f_745)
					{
						case 6:
							func_988(6, 1);
							break;
						
						case 7:
							func_988(7, 1);
							break;
					}
					bVar1 = false;
					break;
				
				case 3:
					func_987(1);
					break;
			}
			break;
	}
	if (bVar1)
	{
		func_756(uParam0->f_744.f_709, uParam0->f_2593, 2);
	}
}

void func_987(bool bParam0)
{
	int iVar0;
	
	iVar0 = func_9(8529, -1, 0);
	if (bParam0)
	{
		unk_0xBE20AB8238688965(&iVar0, 19);
		unk_0xBE20AB8238688965(&(Global_1701669[unk_0x460153A63B9477BC() /*68*/].f_18), 19);
		if (!unk_0xCE990E643CD9D0E5(func_9(8533, -1, 0), 9))
		{
			unk_0xBE20AB8238688965(&iVar0, 21);
			unk_0xBE20AB8238688965(&(Global_1701669[unk_0x460153A63B9477BC() /*68*/].f_18), 21);
		}
	}
	else
	{
		unk_0xD2459066EA58CE43(&iVar0, 19);
		unk_0xD2459066EA58CE43(&(Global_1701669[unk_0x460153A63B9477BC() /*68*/].f_18), 19);
		unk_0xD2459066EA58CE43(&iVar0, 21);
		unk_0xD2459066EA58CE43(&(Global_1701669[unk_0x460153A63B9477BC() /*68*/].f_18), 21);
	}
	func_6(8529, iVar0, -1, 1, 0);
}

void func_988(int iParam0, bool bParam1)
{
	int iVar0;
	
	iVar0 = func_9(8529, -1, 0);
	switch (iParam0)
	{
		case 1:
			if (bParam1)
			{
				unk_0xBE20AB8238688965(&iVar0, 8);
				unk_0xBE20AB8238688965(&(Global_1701669[unk_0x460153A63B9477BC() /*68*/].f_18), 8);
			}
			else
			{
				unk_0xD2459066EA58CE43(&iVar0, 8);
				unk_0xD2459066EA58CE43(&(Global_1701669[unk_0x460153A63B9477BC() /*68*/].f_18), 8);
			}
			break;
		
		case 2:
			if (bParam1)
			{
				unk_0xBE20AB8238688965(&iVar0, 10);
				unk_0xBE20AB8238688965(&(Global_1701669[unk_0x460153A63B9477BC() /*68*/].f_18), 10);
			}
			else
			{
				unk_0xD2459066EA58CE43(&iVar0, 10);
				unk_0xD2459066EA58CE43(&(Global_1701669[unk_0x460153A63B9477BC() /*68*/].f_18), 10);
			}
			break;
		
		case 3:
			if (bParam1)
			{
				unk_0xBE20AB8238688965(&iVar0, 12);
				unk_0xBE20AB8238688965(&(Global_1701669[unk_0x460153A63B9477BC() /*68*/].f_18), 12);
			}
			else
			{
				unk_0xD2459066EA58CE43(&iVar0, 12);
				unk_0xD2459066EA58CE43(&(Global_1701669[unk_0x460153A63B9477BC() /*68*/].f_18), 12);
			}
			break;
		
		case 4:
			if (bParam1)
			{
				unk_0xBE20AB8238688965(&iVar0, 14);
				unk_0xBE20AB8238688965(&(Global_1701669[unk_0x460153A63B9477BC() /*68*/].f_18), 14);
			}
			else
			{
				unk_0xD2459066EA58CE43(&iVar0, 14);
				unk_0xD2459066EA58CE43(&(Global_1701669[unk_0x460153A63B9477BC() /*68*/].f_18), 14);
			}
			break;
		
		case 6:
			if (bParam1)
			{
				unk_0xBE20AB8238688965(&iVar0, 16);
				unk_0xBE20AB8238688965(&(Global_1701669[unk_0x460153A63B9477BC() /*68*/].f_18), 16);
			}
			else
			{
				unk_0xD2459066EA58CE43(&iVar0, 16);
				unk_0xD2459066EA58CE43(&(Global_1701669[unk_0x460153A63B9477BC() /*68*/].f_18), 16);
			}
			func_989(28, 0, -1);
			break;
		
		case 7:
			if (bParam1)
			{
				unk_0xBE20AB8238688965(&iVar0, 17);
				unk_0xBE20AB8238688965(&(Global_1701669[unk_0x460153A63B9477BC() /*68*/].f_18), 17);
			}
			else
			{
				unk_0xD2459066EA58CE43(&iVar0, 17);
				unk_0xD2459066EA58CE43(&(Global_1701669[unk_0x460153A63B9477BC() /*68*/].f_18), 17);
			}
			break;
		
		case 8:
			if (bParam1)
			{
				unk_0xBE20AB8238688965(&iVar0, 18);
				unk_0xBE20AB8238688965(&(Global_1701669[unk_0x460153A63B9477BC() /*68*/].f_18), 18);
			}
			else
			{
				unk_0xD2459066EA58CE43(&iVar0, 18);
				unk_0xD2459066EA58CE43(&(Global_1701669[unk_0x460153A63B9477BC() /*68*/].f_18), 18);
			}
			break;
	}
	func_6(8529, iVar0, -1, 1, 0);
}

void func_989(int iParam0, bool bParam1, int iParam2)
{
	int iVar0;
	
	if (!func_990(iParam0))
	{
		return;
	}
	iVar0 = func_9(8534, -1, 0);
	if (bParam1)
	{
		switch (iParam0)
		{
			case 5:
				unk_0xBE20AB8238688965(&iVar0, 0);
				break;
			
			case 8:
				unk_0xBE20AB8238688965(&iVar0, 1);
				break;
			
			case 1:
				unk_0xBE20AB8238688965(&iVar0, 5);
				break;
			
			case 11:
				unk_0xBE20AB8238688965(&iVar0, 6);
				break;
			
			case 22:
				unk_0xBE20AB8238688965(&iVar0, 7);
				break;
			
			case 4:
				unk_0xBE20AB8238688965(&iVar0, 8);
				break;
			
			case 6:
				unk_0xBE20AB8238688965(&iVar0, 9);
				break;
			
			case 24:
				unk_0xBE20AB8238688965(&iVar0, 10);
				break;
			
			case 27:
				unk_0xBE20AB8238688965(&iVar0, 11);
				break;
			
			case 28:
				unk_0xBE20AB8238688965(&iVar0, 12);
				break;
			
			case 13:
				unk_0xBE20AB8238688965(&iVar0, 13);
				break;
			
			case 30:
				unk_0xBE20AB8238688965(&iVar0, 14);
				break;
			
			case 7:
				unk_0xBE20AB8238688965(&iVar0, 15);
				break;
		}
	}
	else
	{
		switch (iParam0)
		{
			case 5:
				unk_0xD2459066EA58CE43(&iVar0, 0);
				break;
			
			case 8:
				unk_0xD2459066EA58CE43(&iVar0, 1);
				break;
			
			case 1:
				unk_0xD2459066EA58CE43(&iVar0, 5);
				break;
			
			case 11:
				unk_0xD2459066EA58CE43(&iVar0, 6);
				break;
			
			case 22:
				unk_0xD2459066EA58CE43(&iVar0, 7);
				break;
			
			case 4:
				unk_0xD2459066EA58CE43(&iVar0, 8);
				break;
			
			case 6:
				unk_0xD2459066EA58CE43(&iVar0, 9);
				break;
			
			case 24:
				unk_0xD2459066EA58CE43(&iVar0, 10);
				break;
			
			case 27:
				unk_0xD2459066EA58CE43(&iVar0, 11);
				break;
			
			case 28:
				unk_0xD2459066EA58CE43(&iVar0, 12);
				break;
			
			case 13:
				unk_0xD2459066EA58CE43(&iVar0, 13);
				break;
			
			case 30:
				unk_0xD2459066EA58CE43(&iVar0, 14);
				break;
			
			case 7:
				unk_0xD2459066EA58CE43(&iVar0, 15);
				break;
			}
	}
	func_6(8534, iVar0, -1, 1, 0);
}

int func_990(int iParam0)
{
	switch (iParam0)
	{
		case 5:
		case 8:
		case 1:
		case 11:
		case 22:
		case 4:
		case 6:
		case 24:
		case 27:
		case 28:
		case 13:
		case 30:
		case 7:
			return 1;
		
		default:
	}
	return 0;
}

void func_991(bool bParam0)
{
	int iVar0;
	
	iVar0 = func_9(8529, -1, 0);
	if (bParam0)
	{
		unk_0xBE20AB8238688965(&iVar0, 3);
		unk_0xBE20AB8238688965(&(Global_1701669[unk_0x460153A63B9477BC() /*68*/].f_18), 3);
	}
	else
	{
		unk_0xD2459066EA58CE43(&iVar0, 3);
		unk_0xD2459066EA58CE43(&(Global_1701669[unk_0x460153A63B9477BC() /*68*/].f_18), 3);
	}
	func_6(8529, iVar0, -1, 1, 0);
	func_989(22, 0, -1);
}

void func_992(bool bParam0)
{
	int iVar0;
	
	iVar0 = func_9(8529, -1, 0);
	if (bParam0)
	{
		unk_0xBE20AB8238688965(&iVar0, 20);
		unk_0xBE20AB8238688965(&(Global_1701669[unk_0x460153A63B9477BC() /*68*/].f_18), 20);
	}
	else
	{
		unk_0xD2459066EA58CE43(&iVar0, 20);
		unk_0xD2459066EA58CE43(&(Global_1701669[unk_0x460153A63B9477BC() /*68*/].f_18), 20);
	}
	func_6(8529, iVar0, -1, 1, 0);
	func_989(30, 0, -1);
}

void func_993(bool bParam0)
{
	int iVar0;
	
	iVar0 = func_9(8529, -1, 0);
	if (bParam0)
	{
		unk_0xBE20AB8238688965(&iVar0, 5);
		unk_0xBE20AB8238688965(&(Global_1701669[unk_0x460153A63B9477BC() /*68*/].f_18), 5);
	}
	else
	{
		unk_0xD2459066EA58CE43(&iVar0, 5);
		unk_0xD2459066EA58CE43(&(Global_1701669[unk_0x460153A63B9477BC() /*68*/].f_18), 5);
	}
	func_6(8529, iVar0, -1, 1, 0);
}

void func_994(var uParam0, int iParam1)
{
	unk_0x830F007E19C63E14(uParam0, "SET_SECURITY_PASS_VISIBLE");
	unk_0xAD291B8F75D737AD(iParam1);
	unk_0xE1FDD153F5858534();
}

void func_995(int iParam0)
{
	func_6(8523, iParam0, -1, 1, 0);
	Global_1701669[unk_0x460153A63B9477BC() /*68*/].f_18.f_15 = iParam0;
}

void func_996(bool bParam0)
{
	int iVar0;
	
	iVar0 = func_9(8529, -1, 0);
	if (bParam0)
	{
		unk_0xBE20AB8238688965(&iVar0, 4);
		unk_0xBE20AB8238688965(&(Global_1701669[unk_0x460153A63B9477BC() /*68*/].f_18), 4);
	}
	else
	{
		unk_0xD2459066EA58CE43(&iVar0, 4);
		unk_0xD2459066EA58CE43(&(Global_1701669[unk_0x460153A63B9477BC() /*68*/].f_18), 4);
	}
	func_6(8529, iVar0, -1, 1, 0);
	func_989(5, 0, -1);
}

void func_997(bool bParam0)
{
	int iVar0;
	
	iVar0 = func_9(8529, -1, 0);
	if (bParam0)
	{
		unk_0xBE20AB8238688965(&iVar0, 1);
		unk_0xBE20AB8238688965(&(Global_1701669[unk_0x460153A63B9477BC() /*68*/].f_18), 1);
	}
	else
	{
		unk_0xD2459066EA58CE43(&iVar0, 1);
		unk_0xD2459066EA58CE43(&(Global_1701669[unk_0x460153A63B9477BC() /*68*/].f_18), 1);
	}
	func_6(8529, iVar0, -1, 1, 0);
}

void func_998(var uParam0, int iParam1, bool bParam2, int iParam3, int iParam4)
{
	unk_0x830F007E19C63E14(uParam0, "SET_MISSION_COMPLETION");
	unk_0xAD291B8F75D737AD(iParam1);
	unk_0xF1B28F753235CE2A(iParam2);
	unk_0xAD291B8F75D737AD(iParam3);
	unk_0xAD291B8F75D737AD(iParam4);
	unk_0xE1FDD153F5858534();
}

void func_999(var uParam0, int iParam1, bool bParam2)
{
	unk_0x830F007E19C63E14(uParam0, "SET_PURCHASED");
	unk_0xAD291B8F75D737AD(iParam1);
	unk_0xF1B28F753235CE2A(iParam2);
	unk_0xE1FDD153F5858534();
}

void func_1000(int iParam0, int iParam1, int iParam2, bool bParam3)
{
	int iVar0;
	int iVar1;
	
	iVar0 = func_9(8803, -1, 0);
	iVar1 = func_1001(iParam0, iParam1, iParam2);
	if (iVar1 != -1)
	{
		if (bParam3)
		{
			unk_0xBE20AB8238688965(&iVar0, iVar1);
			unk_0xBE20AB8238688965(&(Global_1701669[unk_0x460153A63B9477BC() /*68*/].f_44), iVar1);
		}
		else
		{
			unk_0xD2459066EA58CE43(&iVar0, iVar1);
			unk_0xD2459066EA58CE43(&(Global_1701669[unk_0x460153A63B9477BC() /*68*/].f_44), iVar1);
		}
	}
	func_6(8803, iVar0, -1, 1, 0);
}

int func_1001(int iParam0, int iParam1, int iParam2)
{
	switch (iParam0)
	{
		case 3:
			return 0;
		
		case 8:
			return 1;
		
		case 13:
			return 2;
		
		case 16:
			switch (iParam1)
			{
				case 1:
					return 3;
				
				case 3:
					return 4;
				
				default:
			}
			break;
		
		case 17:
			switch (iParam1)
			{
				case 1:
					return 5;
				
				case 2:
					switch (iParam2)
					{
						case 6:
							return 6;
						
						case 7:
							return 7;
						
						default:
					}
					break;
				
				case 3:
					return 8;
			}
			break;
	}
	return -1;
}

int func_1002(var uParam0, int iParam1)
{
	switch (iParam1)
	{
		case 17:
			if (func_113(uParam0->f_4356) == 2)
			{
				return uParam0->f_744.f_745;
			}
			break;
	}
	return -1;
}

void func_1003(bool bParam0)
{
	func_1004(1, 8802);
	if (bParam0)
	{
		func_510(-1713398555, 8, 0);
	}
}

void func_1004(int iParam0, int iParam1)
{
	int iVar0;
	
	if (iParam0 <= 0)
	{
		return;
	}
	iVar0 = func_9(iParam1, -1, 0);
	func_6(iParam1, (iVar0 + iParam0), -1, 1, 0);
}

void func_1005()
{
	Global_19871 = 0;
	func_1006();
}

void func_1006()
{
	unk_0x62B2C9892E3E142D();
	Global_22016 = 0;
	if (unk_0x6A5CC4A2C9D9ED8D())
	{
		unk_0xBE4122AC23440E7D(0);
		Global_21005 = 6;
	}
}

void func_1007(var uParam0, bool bParam1)
{
	if (bParam1)
	{
		unk_0xBE20AB8238688965(&(uParam0->f_744.f_736), 2);
	}
	else
	{
		unk_0xD2459066EA58CE43(&(uParam0->f_744.f_736), 2);
	}
}

void func_1008(var uParam0, bool bParam1)
{
	if (bParam1)
	{
		unk_0xBE20AB8238688965(&(uParam0->f_744.f_736), 1);
	}
	else
	{
		unk_0xD2459066EA58CE43(&(uParam0->f_744.f_736), 1);
	}
}

void func_1009(var uParam0, bool bParam1)
{
	if (bParam1)
	{
		unk_0xBE20AB8238688965(&(uParam0->f_744.f_736), 12);
	}
	else
	{
		unk_0xD2459066EA58CE43(&(uParam0->f_744.f_736), 12);
	}
}

int func_1010(int iParam0)
{
	switch (iParam0)
	{
		case 1:
			return 42;
		
		case 2:
			return 43;
		
		case 3:
			return 44;
		
		case 4:
			return 45;
		
		case 5:
			return 46;
		
		default:
	}
	return -1;
}

void func_1011(int iParam0)
{
	func_6(8522, iParam0, -1, 1, 0);
	Global_1701669[unk_0x460153A63B9477BC() /*68*/].f_18.f_14 = iParam0;
}

int func_1012(int iParam0)
{
	switch (iParam0)
	{
		case 1:
			return 37;
		
		case 2:
			return 38;
		
		case 3:
			return 39;
		
		case 4:
			return 40;
		
		case 5:
			return 41;
		
		default:
	}
	return -1;
}

void func_1013(int iParam0)
{
	func_6(8521, iParam0, -1, 1, 0);
	Global_1701669[unk_0x460153A63B9477BC() /*68*/].f_18.f_12 = iParam0;
}

void func_1014(var uParam0, bool bParam1)
{
	if (bParam1)
	{
		unk_0xBE20AB8238688965(&(uParam0->f_744.f_736), 16);
	}
	else
	{
		unk_0xD2459066EA58CE43(&(uParam0->f_744.f_736), 16);
	}
}

int func_1015(int iParam0)
{
	switch (iParam0)
	{
		case 1:
			return 32;
		
		case 2:
			return 33;
		
		case 3:
			return 34;
		
		case 4:
			return 35;
		
		case 5:
			return 36;
		
		default:
	}
	return -1;
}

void func_1016(var uParam0, int iParam1, bool bParam2)
{
	unk_0x830F007E19C63E14(uParam0, "SET_CREW_MEMBER_HIRED");
	unk_0xAD291B8F75D737AD(iParam1);
	unk_0xF1B28F753235CE2A(bParam2);
	unk_0xE1FDD153F5858534();
}

void func_1017(var uParam0, bool bParam1)
{
	if (bParam1)
	{
		unk_0xBE20AB8238688965(&(uParam0->f_744.f_736), 8);
	}
	else
	{
		unk_0xD2459066EA58CE43(&(uParam0->f_744.f_736), 8);
	}
}

void func_1018(var uParam0, bool bParam1)
{
	if (bParam1)
	{
		unk_0xBE20AB8238688965(&(uParam0->f_744.f_736), 13);
	}
	else
	{
		unk_0xD2459066EA58CE43(&(uParam0->f_744.f_736), 13);
	}
}

void func_1019(int iParam0)
{
	func_6(8520, iParam0, -1, 1, 0);
	Global_1701669[unk_0x460153A63B9477BC() /*68*/].f_18.f_10 = iParam0;
}

void func_1020(var uParam0)
{
	if (uParam0->f_744.f_702)
	{
		if (unk_0x95EF219D38B20CFF(uParam0->f_744.f_709))
		{
			unk_0xD8D551845E3EA730(1);
			unk_0xCA1F114B668562E5(uParam0->f_744.f_700);
			switch (2)
			{
				case 0:
					break;
				
				case 1:
					unk_0x6D9B01B4CCEFF373(uParam0->f_744.f_709, 1);
					break;
				
				case 2:
					unk_0x178D84F9C5057462(uParam0->f_744.f_709, 1);
					break;
			}
			unk_0xE6A03BA84586E67B(uParam0->f_744.f_709, 0.25f, 0.5f, 0.5f, 1f, 255, 255, 255, 255, 0);
			unk_0xCA1F114B668562E5(unk_0xC48A590C39E17456());
			unk_0xD8D551845E3EA730(0);
			if (uParam0->f_744.f_705)
			{
			}
		}
	}
	else if (func_483(uParam0, 1))
	{
		uParam0->f_744.f_702 = 1;
	}
	if (uParam0->f_744.f_703)
	{
		func_1021(uParam0);
	}
}

void func_1021(var uParam0)
{
	struct<9> Var0;
	bool bVar9;
	int iVar10;
	
	Var0 = { func_877() };
	if (unk_0xDFAB5E5CBE16E8B1())
	{
		return;
	}
	bVar9 = uParam0->f_744.f_706;
	if (func_16(uParam0) || uParam0->f_744.f_737 > 0)
	{
		if (!uParam0->f_744.f_704)
		{
			bVar9 = true;
			uParam0->f_744.f_704 = 1;
		}
	}
	else if (uParam0->f_744.f_704)
	{
		bVar9 = true;
		uParam0->f_744.f_704 = 0;
	}
	if (!bVar9)
	{
		func_872(&(uParam0->f_744.f_710), &Var0, &(uParam0->f_744), 0);
		return;
	}
	if (!unk_0xE5EF21F14D0F9D25() || unk_0xA2C6E70C9BCA9C34() == 10)
	{
		func_871(&(uParam0->f_744), 0.5f);
	}
	else
	{
		func_871(&(uParam0->f_744), 0.7f);
	}
	func_869(&(uParam0->f_744));
	iVar10 = uParam0->f_2593;
	if (uParam0->f_744.f_704)
	{
		if (func_976(uParam0, iVar10) || func_979(uParam0, iVar10))
		{
			if (!unk_0x7BCC91F3C1CF24E8(func_181(uParam0, 1, iVar10)))
			{
				func_867(2, 204, "CH_INSTR_INFO", &(uParam0->f_744), 1, 361);
			}
		}
		else if (func_982(iVar10))
		{
			if (!unk_0x7BCC91F3C1CF24E8(func_166(uParam0, iVar10)))
			{
				func_867(2, 204, "CH_INSTR_INFO", &(uParam0->f_744), 1, 361);
			}
		}
		if (func_716(&(uParam0->f_2529)))
		{
			if (unk_0xC1F43F8D0D5EB664(2))
			{
				func_867(2, 39, "CH_INSTR_ZOOM", &(uParam0->f_744), 1, 361);
			}
			else
			{
				func_866(2, 15, "CH_INSTR_ZOOM", &(uParam0->f_744));
			}
		}
		func_866(2, 1, "HEIST_IB_LOOK", &(uParam0->f_744));
	}
	else
	{
		switch (uParam0->f_744.f_724.f_2)
		{
			case 0:
				if (uParam0->f_4356 == unk_0x460153A63B9477BC())
				{
					if (func_976(uParam0, iVar10))
					{
						if (iVar10 == 2)
						{
							if (!func_942(uParam0, iVar10))
							{
								if (!func_968(uParam0->f_4356))
								{
									func_867(2, 201, "CH_INSTR_LM", &(uParam0->f_744), 1, 361);
								}
								else if (!func_940(uParam0->f_4356))
								{
									func_867(2, 201, "CH_INSTR_RM", &(uParam0->f_744), 1, 361);
								}
							}
						}
						else if (func_978(uParam0, iVar10))
						{
							func_867(2, 201, "CH_INSTR_LM", &(uParam0->f_744), 1, 361);
						}
					}
					else if (func_979(uParam0, iVar10))
					{
						if (func_978(uParam0, iVar10) || func_977(uParam0, iVar10))
						{
							func_867(2, 201, "CH_INSTR_SEL", &(uParam0->f_744), 1, 361);
						}
					}
					else if (func_982(iVar10))
					{
						if (!func_981(uParam0, iVar10))
						{
							func_867(2, 201, "CH_INSTR_SEL", &(uParam0->f_744), 1, 361);
						}
					}
					else if (func_1022(iVar10))
					{
						func_867(2, 201, "CH_INSTR_SEL", &(uParam0->f_744), 1, 361);
					}
				}
				func_867(2, 202, "CH_INSTR_QUIT", &(uParam0->f_744), 1, 361);
				if (uParam0->f_4356 == unk_0x460153A63B9477BC())
				{
					if (func_976(uParam0, iVar10) || func_979(uParam0, iVar10))
					{
						if (func_965(uParam0, iVar10) && func_964(uParam0, iVar10))
						{
							func_867(2, 203, "CH_INSTR_SKIP", &(uParam0->f_744), 1, 361);
						}
					}
				}
				if (func_976(uParam0, iVar10) || func_979(uParam0, iVar10))
				{
					if (!unk_0x7BCC91F3C1CF24E8(func_181(uParam0, 1, iVar10)))
					{
						func_867(2, 204, "CH_INSTR_INFO", &(uParam0->f_744), 1, 361);
					}
				}
				else if (func_982(iVar10))
				{
					if (!unk_0x7BCC91F3C1CF24E8(func_166(uParam0, iVar10)))
					{
						func_867(2, 204, "CH_INSTR_INFO", &(uParam0->f_744), 1, 361);
					}
				}
				else if (iVar10 == 18)
				{
					func_867(2, 204, "CH_INSTR_INFO", &(uParam0->f_744), 1, 361);
				}
				if (func_483(uParam0, 2))
				{
					func_867(2, 206, "CH_INSTR_FIN", &(uParam0->f_744), 1, 361);
				}
				if (func_483(uParam0, 0))
				{
					func_867(2, 205, "CH_INSTR_SETUP", &(uParam0->f_744), 1, 361);
				}
				if (unk_0xC1F43F8D0D5EB664(2))
				{
					func_867(2, 39, "CH_INSTR_ZOOM", &(uParam0->f_744), 1, 361);
				}
				else
				{
					func_866(2, 15, "CH_INSTR_ZOOM", &(uParam0->f_744));
				}
				func_866(2, 1, "HEIST_IB_LOOK", &(uParam0->f_744));
				func_866(2, 0, "HEIST_IB_NAV", &(uParam0->f_744));
				break;
			
			case 1:
				if (func_976(uParam0, iVar10) || func_979(uParam0, iVar10))
				{
					if (uParam0->f_4356 == unk_0x460153A63B9477BC())
					{
						if (func_978(uParam0, iVar10))
						{
							func_867(2, 201, "CH_INSTR_LM", &(uParam0->f_744), 1, 361);
						}
					}
				}
				else if ((func_982(iVar10) && !func_981(uParam0, iVar10)) && func_162(uParam0, iVar10))
				{
					func_867(2, 201, "CH_INSTR_SEL", &(uParam0->f_744), 1, 361);
				}
				func_867(2, 202, "CH_INSTR_CANC", &(uParam0->f_744), 1, 361);
				if (func_976(uParam0, iVar10) || func_979(uParam0, iVar10))
				{
					if (func_965(uParam0, iVar10) && func_964(uParam0, iVar10))
					{
						func_867(2, 203, "CH_INSTR_SKIP", &(uParam0->f_744), 1, 361);
					}
				}
				if (func_976(uParam0, iVar10) || func_979(uParam0, iVar10))
				{
					if (!unk_0x7BCC91F3C1CF24E8(func_181(uParam0, 1, iVar10)))
					{
						func_867(2, 204, "CH_INSTR_INFO", &(uParam0->f_744), 1, 361);
					}
				}
				else if (func_982(iVar10))
				{
					if (!unk_0x7BCC91F3C1CF24E8(func_166(uParam0, iVar10)))
					{
						func_867(2, 204, "CH_INSTR_INFO", &(uParam0->f_744), 1, 361);
					}
				}
				func_866(2, 1, "HEIST_IB_LOOK", &(uParam0->f_744));
				switch (iVar10)
				{
					case 10:
						func_866(2, 8, "CH_INSTR_CHGUN", &(uParam0->f_744));
						break;
					
					case 11:
						func_866(2, 8, "CH_INSTR_CHDRV", &(uParam0->f_744));
						break;
					
					case 12:
						func_866(2, 8, "CH_INSTR_CHHCK", &(uParam0->f_744));
						break;
					
					case 5:
						func_866(2, 8, "CH_INSTR_CHWEP", &(uParam0->f_744));
						break;
					
					case 6:
						func_866(2, 8, "CH_INSTR_CHVEH", &(uParam0->f_744));
						break;
					
					case 13:
						func_866(2, 8, "CH_INSTR_CHPAS", &(uParam0->f_744));
						break;
					
					case 14:
						if (func_113(uParam0->f_4356) == 2)
						{
							func_866(2, 8, "CH_INSTR_CHENT", &(uParam0->f_744));
						}
						break;
					
					case 17:
						if (func_113(uParam0->f_4356) == 2)
						{
							func_866(2, 8, "CH_INSTR_CHEXT", &(uParam0->f_744));
						}
						break;
				}
				break;
			}
	}
	func_872(&(uParam0->f_744.f_710), &Var0, &(uParam0->f_744), uParam0->f_744.f_706);
	func_190(&(uParam0->f_744), 0);
}

int func_1022(int iParam0)
{
	switch (iParam0)
	{
		case 18:
			return 0;
		
		default:
	}
	return 1;
}

void func_1023(var uParam0)
{
	int iVar0;
	int iVar1;
	char* sVar2;
	char* sVar3;
	bool bVar4;
	int iVar5;
	int iVar6;
	
	if (!func_483(uParam0, 1))
	{
		return;
	}
	sVar2 = "";
	sVar3 = "";
	bVar4 = true;
	if (func_1081(uParam0) || func_1080())
	{
		func_790(uParam0->f_744.f_709);
		iVar0 = 0;
		while (iVar0 < 11)
		{
			if (!unk_0x7BCC91F3C1CF24E8(func_1079(uParam0, iVar0)))
			{
				func_787(uParam0->f_744.f_709, func_1079(uParam0, iVar0), func_1078(uParam0, iVar0));
			}
			iVar0++;
		}
		func_1008(uParam0, 0);
	}
	if (func_1077(uParam0) || func_1080())
	{
		func_784(uParam0->f_744.f_709);
		iVar0 = 0;
		while (iVar0 < 11)
		{
			if (!unk_0x7BCC91F3C1CF24E8(func_1076(uParam0, iVar0)))
			{
				func_781(uParam0->f_744.f_709, func_1076(uParam0, iVar0), func_1075(uParam0, iVar0));
			}
			iVar0++;
		}
		func_1007(uParam0, 0);
	}
	if (func_1074(uParam0) || func_1080())
	{
		iVar1 = 2;
		while (iVar1 <= 13)
		{
			if (func_1073(uParam0, iVar1) != -1)
			{
				func_1072(uParam0->f_744.f_709, iVar1, func_1073(uParam0, iVar1));
			}
			iVar1++;
		}
		func_1071(uParam0, 0);
	}
	if (func_1070(uParam0) || func_1080())
	{
		sVar2 = func_535(uParam0->f_744.f_738, 1);
		sVar3 = func_156(uParam0, 10);
		bVar4 = func_144(uParam0->f_4356, uParam0->f_744.f_738);
		if (!bVar4 && func_143(uParam0))
		{
			sVar2 = "CH_LIST_BLANK";
			sVar3 = "CH_LIST_BLANK";
		}
		func_1067(uParam0->f_744.f_709, 10, sVar2, sVar3, func_1069(uParam0), func_158(uParam0, 10), func_1068(uParam0));
		func_1016(uParam0->f_744.f_709, 10, func_164(uParam0->f_4356, 0) == uParam0->f_744.f_738);
		func_1066(uParam0->f_744.f_709, 10, !bVar4);
		func_962(uParam0, 0);
	}
	if (func_1065(uParam0) || func_1080())
	{
		sVar2 = func_534(uParam0->f_744.f_740, 1);
		sVar3 = func_156(uParam0, 11);
		bVar4 = func_163(uParam0->f_4356, uParam0->f_744.f_740);
		func_1067(uParam0->f_744.f_709, 11, sVar2, sVar3, func_1064(uParam0), func_158(uParam0, 11), -1);
		func_1016(uParam0->f_744.f_709, 11, func_153(uParam0->f_4356, 0) == uParam0->f_744.f_740);
		func_1066(uParam0->f_744.f_709, 11, !bVar4);
		func_960(uParam0, 0);
	}
	if (func_1063(uParam0) || func_1080())
	{
		sVar2 = func_533(uParam0->f_744.f_743, 1);
		sVar3 = func_156(uParam0, 12);
		bVar4 = func_138(uParam0->f_4356, uParam0->f_744.f_743);
		if (!bVar4 && func_137(uParam0))
		{
			sVar2 = "CH_LIST_BLANK";
			sVar3 = "CH_LIST_BLANK";
		}
		func_1067(uParam0->f_744.f_709, 12, sVar2, sVar3, func_1062(uParam0), func_158(uParam0, 12), -1);
		func_1016(uParam0->f_744.f_709, 12, func_491(uParam0->f_4356, 0) == uParam0->f_744.f_743);
		func_1066(uParam0->f_744.f_709, 12, !bVar4);
		func_958(uParam0, 0);
	}
	if (func_1061(uParam0) || func_1080())
	{
		iVar6 = 0;
		while (iVar6 < func_1060(func_778(uParam0)))
		{
			if (iVar6 == uParam0->f_744.f_747 || uParam0->f_744.f_747 == -1)
			{
				func_1054(uParam0->f_744.f_709, func_1059(iVar6), func_1058(uParam0, func_778(uParam0), iVar6), func_1057(uParam0, func_778(uParam0), iVar6), func_967(uParam0, func_778(uParam0), iVar6), func_1056(func_1059(iVar6)), func_1055(uParam0, func_1059(iVar6)));
				func_1066(uParam0->f_744.f_709, func_1059(iVar6), func_942(uParam0, func_1059(iVar6)));
			}
			iVar6++;
		}
		func_952(uParam0, 0, -1);
	}
	if (func_1053(uParam0) || func_1080())
	{
		iVar1 = 0;
		while (iVar1 < 19)
		{
			func_710(uParam0->f_744.f_709, iVar1, func_969(uParam0, iVar1));
			if (func_1052(uParam0, iVar1) != -1)
			{
				func_756(uParam0->f_744.f_709, iVar1, func_1052(uParam0, iVar1));
			}
			func_998(uParam0->f_744.f_709, iVar1, func_1051(uParam0, iVar1), func_1050(uParam0, iVar1), func_1047(uParam0, iVar1));
			iVar1++;
		}
		if (iVar1 == 7 && func_491(uParam0->f_4356, 0) != 0)
		{
			func_711(uParam0->f_744.f_709, 7, 1);
		}
		if (func_113(uParam0->f_4356) == 2 && func_969(uParam0, 16))
		{
			func_1045(uParam0->f_744.f_709, 16, 1, func_1046(uParam0), 2);
		}
		func_1017(uParam0, 0);
	}
	if (func_1044(uParam0) || func_1043())
	{
		iVar1 = 0;
		while (iVar1 < 19)
		{
			func_1066(uParam0->f_744.f_709, iVar1, func_942(uParam0, iVar1));
			iVar1++;
		}
		func_1018(uParam0, 0);
	}
	if (func_1042(uParam0) || func_1080())
	{
		func_1041(uParam0->f_744.f_709, func_133(func_778(uParam0), 1), func_120(func_121(uParam0->f_4356), 1));
		func_1040(uParam0, 0);
	}
	if (func_1039(uParam0) || func_1080())
	{
		func_1037(uParam0->f_744.f_709, 5, func_1038(uParam0), func_532(uParam0, 1));
		func_711(uParam0->f_744.f_709, 5, 1);
		func_710(uParam0->f_744.f_709, 5, 1);
		func_957(uParam0, 0);
	}
	if (func_1036(uParam0) || func_1080())
	{
		func_1037(uParam0->f_744.f_709, 6, func_1035(uParam0), func_531(uParam0, 1));
		func_711(uParam0->f_744.f_709, 6, 1);
		func_710(uParam0->f_744.f_709, 6, 1);
		func_956(uParam0, 0);
	}
	if (func_1034(uParam0) || func_1080())
	{
		func_1031(uParam0->f_744.f_709, func_1033(uParam0), func_1032(uParam0));
		func_994(uParam0->f_744.f_709, func_936(uParam0->f_4356));
		func_955(uParam0, 0);
	}
	if (func_1030(uParam0) || func_1080())
	{
		iVar1 = 0;
		while (iVar1 < 19)
		{
			if (func_969(uParam0, iVar1) && func_1029(uParam0, iVar1))
			{
				func_763(uParam0->f_744.f_709, iVar1, func_966(uParam0, iVar1, 0));
			}
			iVar1++;
		}
		func_1009(uParam0, 0);
	}
	if (func_1028(uParam0) || func_1080())
	{
		iVar1 = 0;
		while (iVar1 < 19)
		{
			iVar5 = func_1027(uParam0->f_4356, iVar1, func_113(uParam0->f_4356), func_1002(uParam0, iVar1));
			func_999(uParam0->f_744.f_709, iVar1, iVar5);
			iVar1++;
		}
		func_954(uParam0, 0);
	}
	if (func_1026(uParam0))
	{
		iVar1 = 0;
		while (iVar1 < 19)
		{
			if (func_1025(uParam0, iVar1))
			{
				func_708(uParam0->f_744.f_709, iVar1, 1);
			}
			else
			{
				func_708(uParam0->f_744.f_709, iVar1, 0);
			}
			iVar1++;
		}
		func_1014(uParam0, 0);
	}
	func_1024(0);
}

void func_1024(int iParam0)
{
	if (Global_1697302 && Global_1697304 != iParam0)
	{
		Global_1697304 = iParam0;
	}
}

int func_1025(var uParam0, int iParam1)
{
	if (func_976(uParam0, iParam1))
	{
		if (iParam1 == 2)
		{
			if (!func_968(uParam0->f_4356))
			{
				return 0;
			}
			else if (!func_940(uParam0->f_4356))
			{
				return 0;
			}
		}
		else if (func_978(uParam0, iParam1))
		{
			return 0;
		}
	}
	else if (func_979(uParam0, iParam1))
	{
		if (func_978(uParam0, iParam1) || func_977(uParam0, iParam1))
		{
			return 0;
		}
	}
	else if (func_982(iParam1))
	{
		if (!func_981(uParam0, iParam1))
		{
			return 0;
		}
	}
	else if (func_1022(iParam1))
	{
		return 0;
	}
	return 1;
}

bool func_1026(var uParam0)
{
	return unk_0xCE990E643CD9D0E5(uParam0->f_744.f_736, 16);
}

int func_1027(int iParam0, int iParam1, int iParam2, int iParam3)
{
	int iVar0;
	
	if (iParam0 != -1)
	{
		iVar0 = func_1001(iParam1, iParam2, iParam3);
		if (iVar0 != -1)
		{
			return unk_0xCE990E643CD9D0E5(Global_1701669[iParam0 /*68*/].f_44, iVar0);
		}
	}
	return 0;
}

bool func_1028(var uParam0)
{
	return unk_0xCE990E643CD9D0E5(uParam0->f_744.f_736, 15);
}

int func_1029(var uParam0, int iParam1)
{
	switch (iParam1)
	{
		case 2:
		case 3:
		case 4:
		case 5:
		case 6:
		case 7:
		case 9:
		case 8:
		case 14:
		case 15:
		case 17:
			return 1;
		
		case 16:
			if (func_113(uParam0->f_4356) == 2)
			{
				return 0;
			}
			return 1;
		
		default:
	}
	return 0;
}

bool func_1030(var uParam0)
{
	return unk_0xCE990E643CD9D0E5(uParam0->f_744.f_736, 12);
}

void func_1031(var uParam0, var uParam1, int iParam2)
{
	unk_0x830F007E19C63E14(uParam0, "SET_INSIDE_MAN");
	func_83(uParam1);
	unk_0xAD291B8F75D737AD(iParam2);
	unk_0xE1FDD153F5858534();
}

int func_1032(var uParam0)
{
	switch (uParam0->f_744.f_742)
	{
		case 1:
			return 1;
		
		case 2:
			return 2;
		
		default:
	}
	return 0;
}

char* func_1033(var uParam0)
{
	switch (uParam0->f_744.f_742)
	{
		case 1:
			return "CH_KEYACCESS_0";
		
		case 2:
			return "CH_KEYACCESS_1";
		
		default:
	}
	return "";
}

bool func_1034(var uParam0)
{
	return unk_0xCE990E643CD9D0E5(uParam0->f_744.f_736, 11);
}

int func_1035(var uParam0)
{
	switch (uParam0->f_744.f_740)
	{
		case 1:
			switch (uParam0->f_744.f_741)
			{
				case 0:
					return 1;
				
				case 1:
					return 2;
				
				case 2:
					return 3;
				
				case 3:
					return 4;
				
				default:
			}
			break;
		
		case 2:
			switch (uParam0->f_744.f_741)
			{
				case 0:
					return 5;
				
				case 1:
					return 6;
				
				case 2:
					return 7;
				
				case 3:
					return 8;
				
				default:
			}
			break;
		
		case 3:
			switch (uParam0->f_744.f_741)
			{
				case 0:
					return 9;
				
				case 1:
					return 10;
				
				case 2:
					return 11;
				
				case 3:
					return 12;
				
				default:
			}
			break;
		
		case 4:
			switch (uParam0->f_744.f_741)
			{
				case 0:
					return 13;
				
				case 1:
					return 14;
				
				case 2:
					return 15;
				
				case 3:
					return 16;
				
				default:
			}
			break;
		
		case 5:
			switch (uParam0->f_744.f_741)
			{
				case 0:
					return 17;
				
				case 1:
					return 18;
				
				case 2:
					return 19;
				
				case 3:
					return 20;
				
				default:
			}
			break;
	}
	return -1;
}

bool func_1036(var uParam0)
{
	return unk_0xCE990E643CD9D0E5(uParam0->f_744.f_736, 10);
}

void func_1037(var uParam0, int iParam1, int iParam2, var uParam3)
{
	unk_0x830F007E19C63E14(uParam0, "SET_MISSION");
	unk_0xAD291B8F75D737AD(iParam1);
	unk_0xAD291B8F75D737AD(iParam2);
	func_83(uParam3);
	unk_0xE1FDD153F5858534();
}

int func_1038(var uParam0)
{
	switch (uParam0->f_744.f_738)
	{
		case 1:
			switch (func_113(uParam0->f_4356))
			{
				case 1:
					switch (uParam0->f_744.f_739)
					{
						case 0:
							return 11;
						
						case 1:
							return 12;
						
						default:
					}
					break;
				
				case 2:
					switch (uParam0->f_744.f_739)
					{
						case 0:
							return 21;
						
						case 1:
							return 22;
						
						default:
					}
					break;
				
				case 3:
					switch (uParam0->f_744.f_739)
					{
						case 0:
							return 1;
						
						case 1:
							return 2;
						
						default:
					}
					break;
			}
			break;
		
		case 2:
			switch (func_113(uParam0->f_4356))
			{
				case 1:
					switch (uParam0->f_744.f_739)
					{
						case 0:
							return 17;
						
						case 1:
							return 18;
						
						default:
					}
					break;
				
				case 2:
					switch (uParam0->f_744.f_739)
					{
						case 0:
							return 27;
						
						case 1:
							return 28;
						
						default:
					}
					break;
				
				case 3:
					switch (uParam0->f_744.f_739)
					{
						case 0:
							return 7;
						
						case 1:
							return 8;
						
						default:
					}
					break;
			}
			break;
		
		case 3:
			switch (func_113(uParam0->f_4356))
			{
				case 1:
					switch (uParam0->f_744.f_739)
					{
						case 0:
							return 13;
						
						case 1:
							return 14;
						
						default:
					}
					break;
				
				case 2:
					switch (uParam0->f_744.f_739)
					{
						case 0:
							return 23;
						
						case 1:
							return 24;
						
						default:
					}
					break;
				
				case 3:
					switch (uParam0->f_744.f_739)
					{
						case 0:
							return 3;
						
						case 1:
							return 4;
						
						default:
					}
					break;
			}
			break;
		
		case 4:
			switch (func_113(uParam0->f_4356))
			{
				case 1:
					switch (uParam0->f_744.f_739)
					{
						case 0:
							return 19;
						
						case 1:
							return 20;
						
						default:
					}
					break;
				
				case 2:
					switch (uParam0->f_744.f_739)
					{
						case 0:
							return 29;
						
						case 1:
							return 30;
						
						default:
					}
					break;
				
				case 3:
					switch (uParam0->f_744.f_739)
					{
						case 0:
							return 9;
						
						case 1:
							return 10;
						
						default:
					}
					break;
			}
			break;
		
		case 5:
			switch (func_113(uParam0->f_4356))
			{
				case 1:
					switch (uParam0->f_744.f_739)
					{
						case 0:
							return 15;
						
						case 1:
							return 16;
						
						default:
					}
					break;
				
				case 2:
					switch (uParam0->f_744.f_739)
					{
						case 0:
							return 25;
						
						case 1:
							return 26;
						
						default:
					}
					break;
				
				case 3:
					switch (uParam0->f_744.f_739)
					{
						case 0:
							return 5;
						
						case 1:
							return 6;
						
						default:
					}
					break;
			}
			break;
	}
	return 0;
}

bool func_1039(var uParam0)
{
	return unk_0xCE990E643CD9D0E5(uParam0->f_744.f_736, 9);
}

void func_1040(var uParam0, bool bParam1)
{
	if (bParam1)
	{
		unk_0xBE20AB8238688965(&(uParam0->f_744.f_736), 6);
	}
	else
	{
		unk_0xD2459066EA58CE43(&(uParam0->f_744.f_736), 6);
	}
}

void func_1041(var uParam0, var uParam1, var uParam2)
{
	unk_0x830F007E19C63E14(uParam0, "SET_HEADINGS");
	func_83(uParam1);
	func_83(uParam2);
	unk_0xE1FDD153F5858534();
}

bool func_1042(var uParam0)
{
	return unk_0xCE990E643CD9D0E5(uParam0->f_744.f_736, 6);
}

var func_1043()
{
	return Global_1697303;
}

bool func_1044(var uParam0)
{
	return unk_0xCE990E643CD9D0E5(uParam0->f_744.f_736, 13);
}

void func_1045(var uParam0, int iParam1, bool bParam2, int iParam3, int iParam4)
{
	unk_0x830F007E19C63E14(uParam0, "SET_POSTER_VISIBLE");
	unk_0xAD291B8F75D737AD(iParam1);
	unk_0xF1B28F753235CE2A(iParam2);
	unk_0xAD291B8F75D737AD(iParam3);
	unk_0xAD291B8F75D737AD(iParam4);
	unk_0xE1FDD153F5858534();
}

int func_1046(var uParam0)
{
	if (func_173(uParam0->f_4356, 4))
	{
		return 2;
	}
	else if (func_170(uParam0->f_4356, 4))
	{
		return 1;
	}
	return 0;
}

int func_1047(var uParam0, int iParam1)
{
	int iVar0;
	var uVar1;
	
	iVar0 = func_975(uParam0, iParam1);
	uVar1 = func_1048(uParam0->f_4356, iVar0, 1);
	return uVar1;
}

int func_1048(int iParam0, int iParam1, bool bParam2)
{
	int iVar0;
	int iVar1;
	
	iVar0 = iParam0;
	iVar1 = -1;
	if (iVar0 != -1)
	{
		switch (iParam1)
		{
			case 5:
				iVar1 = 2;
				break;
			
			case 8:
				iVar1 = 2;
				break;
			
			case 2:
				switch (Global_1701669[iParam0 /*68*/].f_18.f_12)
				{
					case 4:
						iVar1 = 4;
						break;
					
					default:
						iVar1 = 2;
						break;
				}
				break;
			
			case 1:
				iVar1 = 2;
				break;
			
			case 11:
				iVar1 = 2;
				break;
			
			case 22:
				iVar1 = 2;
				break;
			
			case 4:
				iVar1 = 2;
				break;
			
			case 6:
				iVar1 = 2;
				break;
			
			case 24:
				iVar1 = 2;
				break;
			
			case 27:
				iVar1 = 2;
				break;
			
			case 28:
				iVar1 = 2;
				break;
			
			case 13:
				iVar1 = 2;
				break;
			
			case 30:
				iVar1 = 2;
				break;
			
			case 7:
				iVar1 = 2;
				break;
		}
		if (!bParam2)
		{
			if (func_1049(iParam0, iParam1))
			{
				iVar1 = 1;
			}
		}
	}
	return iVar1;
}

int func_1049(int iParam0, int iParam1)
{
	int iVar0;
	
	iVar0 = iParam0;
	if (iVar0 != -1)
	{
		switch (iParam1)
		{
			case 5:
				return unk_0xCE990E643CD9D0E5(Global_1701669[iVar0 /*68*/].f_45, 0);
			
			case 8:
				return unk_0xCE990E643CD9D0E5(Global_1701669[iVar0 /*68*/].f_45, 1);
			
			case 1:
				return unk_0xCE990E643CD9D0E5(Global_1701669[iVar0 /*68*/].f_45, 5);
			
			case 11:
				return unk_0xCE990E643CD9D0E5(Global_1701669[iVar0 /*68*/].f_45, 6);
			
			case 22:
				return unk_0xCE990E643CD9D0E5(Global_1701669[iVar0 /*68*/].f_45, 7);
			
			case 4:
				return unk_0xCE990E643CD9D0E5(Global_1701669[iVar0 /*68*/].f_45, 8);
			
			case 6:
				return unk_0xCE990E643CD9D0E5(Global_1701669[iVar0 /*68*/].f_45, 9);
			
			case 24:
				return unk_0xCE990E643CD9D0E5(Global_1701669[iVar0 /*68*/].f_45, 10);
			
			case 27:
				return unk_0xCE990E643CD9D0E5(Global_1701669[iVar0 /*68*/].f_45, 11);
			
			case 28:
				return unk_0xCE990E643CD9D0E5(Global_1701669[iVar0 /*68*/].f_45, 12);
			
			case 13:
				return unk_0xCE990E643CD9D0E5(Global_1701669[iVar0 /*68*/].f_45, 13);
			
			case 30:
				return unk_0xCE990E643CD9D0E5(Global_1701669[iVar0 /*68*/].f_45, 14);
			
			case 7:
				return unk_0xCE990E643CD9D0E5(Global_1701669[iVar0 /*68*/].f_45, 15);
			}
		
		default:
	}
	return 0;
}

int func_1050(var uParam0, int iParam1)
{
	int iVar0;
	int iVar1;
	int iVar2;
	int iVar3;
	
	iVar0 = func_975(uParam0, iParam1);
	iVar1 = func_1048(uParam0->f_4356, iVar0, 1);
	iVar2 = func_1048(uParam0->f_4356, iVar0, 0);
	iVar3 = (iVar1 - iVar2);
	return iVar3;
}

int func_1051(var uParam0, int iParam1)
{
	int iVar0;
	
	iVar0 = func_975(uParam0, iParam1);
	if (!func_990(iVar0))
	{
		return 0;
	}
	if (!func_1049(uParam0->f_4356, iVar0))
	{
		return 0;
	}
	return 1;
}

int func_1052(var uParam0, int iParam1)
{
	switch (iParam1)
	{
		case 5:
			if (func_493(uParam0->f_4356))
			{
				return 2;
			}
			else
			{
				return 0;
			}
			break;
		
		case 6:
			if (func_492(uParam0->f_4356))
			{
				return 2;
			}
			else
			{
				return 0;
			}
			break;
		
		case 7:
			return 2;
		
		case 10:
			if (func_164(uParam0->f_4356, 0) != 0)
			{
				return 2;
			}
			else
			{
				return 0;
			}
			break;
		
		case 11:
			if (func_153(uParam0->f_4356, 0) != 0)
			{
				return 2;
			}
			else
			{
				return 0;
			}
			break;
		
		case 12:
			if (func_491(uParam0->f_4356, 0) != 0)
			{
				return 2;
			}
			else
			{
				return 0;
			}
			break;
		
		case 13:
			if (func_936(uParam0->f_4356) == 2)
			{
				return 2;
			}
			else
			{
				return 0;
			}
			break;
		
		case 14:
			switch (func_113(uParam0->f_4356))
			{
				case 2:
					return 0;
				
				default:
			}
			break;
		
		case 15:
			return 2;
		
		case 16:
			return 2;
		
		case 17:
			switch (func_113(uParam0->f_4356))
			{
				case 2:
					return 0;
				
				default:
			}
			break;
	}
	return -1;
}

bool func_1053(var uParam0)
{
	return unk_0xCE990E643CD9D0E5(uParam0->f_744.f_736, 8);
}

void func_1054(var uParam0, int iParam1, int iParam2, var uParam3, bool bParam4, bool bParam5, var uParam6)
{
	unk_0x830F007E19C63E14(uParam0, "ADD_APPROACH");
	unk_0xAD291B8F75D737AD(iParam1);
	unk_0xAD291B8F75D737AD(iParam2);
	func_83(uParam3);
	unk_0xF1B28F753235CE2A(iParam4);
	unk_0xF1B28F753235CE2A(iParam5);
	func_83(uParam6);
	unk_0xE1FDD153F5858534();
}

char* func_1055(var uParam0, int iParam1)
{
	switch (iParam1)
	{
		case 14:
			switch (func_113(uParam0->f_4356))
			{
				case 2:
					return "CH_PREP_OUTFITIN";
				
				default:
			}
			break;
		
		case 17:
			switch (func_113(uParam0->f_4356))
			{
				case 2:
					return "CH_PREP_OUTFITOUT";
				
				default:
			}
			break;
	}
	return "";
}

int func_1056(int iParam0)
{
	switch (iParam0)
	{
		case 14:
		case 15:
			return 1;
		
		default:
	}
	return 0;
}

char* func_1057(var uParam0, int iParam1, int iParam2)
{
	switch (iParam1)
	{
		case 1:
			switch (iParam2)
			{
				case 0:
					return "CH_MISS_ST_0";
				
				case 1:
					return "CH_MISS_ST_1";
				
				case 2:
					return "CH_MISS_ST_2";
				
				case 3:
					return "CH_MISS_ST_3";
				
				default:
			}
			break;
		
		case 2:
			switch (iParam2)
			{
				case 0:
					switch (uParam0->f_744.f_744)
					{
						case 1:
							return "CH_MISS_SU_0a";
						
						case 2:
							return "CH_MISS_SU_0b";
						
						case 3:
							return "CH_MISS_SU_0c";
						
						default:
					}
					break;
				
				case 1:
					return "CH_MISS_SU_1";
				
				case 2:
					return "CH_MISS_SU_2";
				
				case 3:
					switch (uParam0->f_744.f_745)
					{
						case 6:
							return "CH_MISS_SU_3a";
						
						case 7:
							return "CH_MISS_SU_3b";
						
						default:
					}
					break;
			}
			break;
		
		case 3:
			switch (iParam2)
			{
				case 0:
					if (func_178(uParam0->f_4356))
					{
						return "CH_MISS_DI_0b";
					}
					return "CH_MISS_DI_0";
				
				case 1:
					return "CH_MISS_DI_1";
				
				case 2:
					return "CH_MISS_DI_2";
				
				case 3:
					return "CH_MISS_DI_3";
				
				default:
			}
			break;
	}
	return "";
}

int func_1058(var uParam0, int iParam1, int iParam2)
{
	switch (iParam1)
	{
		case 1:
			switch (iParam2)
			{
				case 0:
					return 5;
				
				case 1:
					return 7;
				
				case 2:
					return 6;
				
				case 3:
					return 8;
				
				default:
			}
			break;
		
		case 2:
			switch (iParam2)
			{
				case 0:
					switch (uParam0->f_744.f_744)
					{
						case 1:
							if (func_170(uParam0->f_4356, 1))
							{
								return 20;
							}
							return 19;
						
						case 2:
							if (func_170(uParam0->f_4356, 2))
							{
								return 24;
							}
							return 23;
						
						case 3:
							if (func_170(uParam0->f_4356, 3))
							{
								return 22;
							}
							return 21;
						
						default:
					}
					break;
				
				case 1:
					return 12;
				
				case 2:
					switch (uParam0->f_744.f_746)
					{
						case 0:
							return 9;
						
						case 1:
							return 10;
						
						case 2:
							return 11;
						
						default:
					}
					break;
				
				case 3:
					switch (uParam0->f_744.f_745)
					{
						case 6:
							return 17;
						
						case 7:
							return 16;
						
						case 8:
							return 18;
						
						default:
					}
					break;
			}
			break;
		
		case 3:
			switch (iParam2)
			{
				case 0:
					return 3;
				
				case 1:
					return 4;
				
				case 2:
					return 1;
				
				case 3:
					return 2;
				
				default:
			}
			break;
	}
	return -1;
}

int func_1059(int iParam0)
{
	return (14 + iParam0);
}

int func_1060(int iParam0)
{
	switch (iParam0)
	{
		case 1:
			return 4;
		
		case 2:
			return 4;
		
		case 3:
			return 4;
		
		default:
	}
	return 0;
}

bool func_1061(var uParam0)
{
	return unk_0xCE990E643CD9D0E5(uParam0->f_744.f_736, 7);
}

int func_1062(var uParam0)
{
	if (func_137(uParam0))
	{
		return 6;
	}
	return uParam0->f_744.f_743;
}

bool func_1063(var uParam0)
{
	return unk_0xCE990E643CD9D0E5(uParam0->f_744.f_736, 5);
}

int func_1064(var uParam0)
{
	return uParam0->f_744.f_740;
}

bool func_1065(var uParam0)
{
	return unk_0xCE990E643CD9D0E5(uParam0->f_744.f_736, 4);
}

void func_1066(var uParam0, int iParam1, bool bParam2)
{
	unk_0x830F007E19C63E14(uParam0, "SET_PADLOCK");
	unk_0xAD291B8F75D737AD(iParam1);
	unk_0xF1B28F753235CE2A(bParam2);
	unk_0xE1FDD153F5858534();
}

void func_1067(var uParam0, int iParam1, char* sParam2, char* sParam3, int iParam4, int iParam5, int iParam6)
{
	unk_0x830F007E19C63E14(uParam0, "SET_CREW_MEMBER");
	unk_0xAD291B8F75D737AD(iParam1);
	func_83(sParam2);
	func_83(sParam3);
	unk_0xAD291B8F75D737AD(iParam4);
	unk_0xAD291B8F75D737AD(iParam5);
	unk_0xAD291B8F75D737AD(iParam6);
	unk_0xE1FDD153F5858534();
}

int func_1068(var uParam0)
{
	switch (uParam0->f_744.f_738)
	{
		case 1:
			switch (func_113(uParam0->f_4356))
			{
				case 1:
					return 2;
				
				case 2:
					return 1;
				
				case 3:
					return 3;
				
				default:
			}
			break;
		
		case 2:
			switch (func_113(uParam0->f_4356))
			{
				case 1:
					return 5;
				
				case 2:
					return 4;
				
				case 3:
					return 6;
				
				default:
			}
			break;
		
		case 3:
			switch (func_113(uParam0->f_4356))
			{
				case 1:
					return 8;
				
				case 2:
					return 7;
				
				case 3:
					return 9;
				
				default:
			}
			break;
		
		case 4:
			switch (func_113(uParam0->f_4356))
			{
				case 1:
					return 11;
				
				case 2:
					return 10;
				
				case 3:
					return 12;
				
				default:
			}
			break;
		
		case 5:
			switch (func_113(uParam0->f_4356))
			{
				case 1:
					return 14;
				
				case 2:
					return 13;
				
				case 3:
					return 15;
				
				default:
			}
			break;
	}
	return 16;
}

int func_1069(var uParam0)
{
	if (func_143(uParam0))
	{
		return 6;
	}
	return uParam0->f_744.f_738;
}

bool func_1070(var uParam0)
{
	return unk_0xCE990E643CD9D0E5(uParam0->f_744.f_736, 3);
}

void func_1071(var uParam0, bool bParam1)
{
	if (bParam1)
	{
		unk_0xBE20AB8238688965(&(uParam0->f_744.f_736), 14);
	}
	else
	{
		unk_0xD2459066EA58CE43(&(uParam0->f_744.f_736), 14);
	}
}

void func_1072(var uParam0, int iParam1, int iParam2)
{
	unk_0x830F007E19C63E14(uParam0, "SET_BUTTON_IMAGE");
	unk_0xAD291B8F75D737AD(iParam1);
	unk_0xAD291B8F75D737AD(iParam2);
	unk_0xE1FDD153F5858534();
}

int func_1073(var uParam0, int iParam1)
{
	switch (iParam1)
	{
		case 2:
			return 1;
		
		case 3:
			return 2;
		
		case 4:
			return 3;
		
		case 7:
			return 1;
		
		case 9:
			return 4;
		
		case 8:
			return 5;
		
		default:
	}
	return -1;
}

bool func_1074(var uParam0)
{
	return unk_0xCE990E643CD9D0E5(uParam0->f_744.f_736, 14);
}

int func_1075(var uParam0, int iParam1)
{
	switch (func_778(uParam0))
	{
		case 1:
			switch (iParam1)
			{
				case 0:
					return func_937(uParam0->f_4356);
				
				case 1:
					return func_939(uParam0->f_4356);
				
				case 2:
					return func_968(uParam0->f_4356);
				
				case 3:
					return func_938(uParam0->f_4356);
				
				case 4:
					return func_936(uParam0->f_4356) != 0;
				
				case 5:
					return func_930(uParam0->f_4356) != 0;
				
				case 6:
					return func_934(uParam0->f_4356);
				
				case 7:
					return func_935(uParam0->f_4356);
				
				default:
			}
			break;
		
		case 2:
			switch (iParam1)
			{
				case 0:
					return func_937(uParam0->f_4356);
				
				case 1:
					return func_939(uParam0->f_4356);
				
				case 2:
					return func_968(uParam0->f_4356);
				
				case 3:
					return func_938(uParam0->f_4356);
				
				case 4:
					return func_936(uParam0->f_4356) != 0;
				
				case 5:
					return func_930(uParam0->f_4356) != 0;
				
				case 6:
					return func_767(uParam0->f_4356);
				
				default:
			}
			break;
		
		case 3:
			switch (iParam1)
			{
				case 0:
					return func_937(uParam0->f_4356);
				
				case 1:
					return func_939(uParam0->f_4356);
				
				case 2:
					return func_968(uParam0->f_4356);
				
				case 3:
					return func_938(uParam0->f_4356);
				
				case 4:
					return func_936(uParam0->f_4356) != 0;
				
				case 5:
					return func_930(uParam0->f_4356) != 0;
				
				case 6:
					return func_933(uParam0->f_4356);
				
				case 7:
					return func_932(uParam0->f_4356);
				
				default:
			}
			break;
	}
	return 0;
}

char* func_1076(var uParam0, int iParam1)
{
	switch (func_778(uParam0))
	{
		case 1:
			switch (iParam1)
			{
				case 0:
					return "CH_OPT_PREP_0";
				
				case 1:
					return "CH_OPT_PREP_1";
				
				case 2:
					return "CH_OPT_PREP_2";
				
				case 3:
					return "CH_OPT_PREP_3";
				
				case 4:
					return "CH_OPT_PREP_4";
				
				case 5:
					return "CH_OPT_PREP_5";
				
				case 6:
					return "CH_OPT_PREP_6";
				
				case 7:
					return "CH_OPT_PREP_7";
				
				default:
			}
			break;
		
		case 2:
			switch (iParam1)
			{
				case 0:
					return "CH_OPT_PREP_0";
				
				case 1:
					return "CH_OPT_PREP_1";
				
				case 2:
					return "CH_OPT_PREP_2";
				
				case 3:
					return "CH_OPT_PREP_3";
				
				case 4:
					return "CH_OPT_PREP_4";
				
				case 5:
					return "CH_OPT_PREP_5";
				
				case 6:
					return "CH_OPT_PREP_8";
				
				default:
			}
			break;
		
		case 3:
			switch (iParam1)
			{
				case 0:
					return "CH_OPT_PREP_0";
				
				case 1:
					return "CH_OPT_PREP_1";
				
				case 2:
					return "CH_OPT_PREP_2";
				
				case 3:
					return "CH_OPT_PREP_3";
				
				case 4:
					return "CH_OPT_PREP_4";
				
				case 5:
					return "CH_OPT_PREP_5";
				
				case 6:
					return "CH_OPT_PREP_9";
				
				case 7:
					if (func_179(uParam0->f_4356, 10))
					{
						return "CH_OPT_PREP_10";
					}
					else
					{
						return "CH_LIST_BLANK";
					}
					break;
			}
			break;
	}
	return "";
}

bool func_1077(var uParam0)
{
	return unk_0xCE990E643CD9D0E5(uParam0->f_744.f_736, 2);
}

int func_1078(var uParam0, int iParam1)
{
	switch (func_778(uParam0))
	{
		case 1:
			switch (iParam1)
			{
				case 0:
					return func_493(uParam0->f_4356);
				
				case 1:
					return func_492(uParam0->f_4356);
				
				case 2:
					return func_490(uParam0->f_4356);
				
				case 3:
					return func_494(uParam0->f_4356);
				
				case 4:
					return func_489(uParam0->f_4356);
				
				case 5:
					return func_488(uParam0->f_4356);
				
				default:
			}
			break;
		
		case 2:
			switch (iParam1)
			{
				case 0:
					return func_493(uParam0->f_4356);
				
				case 1:
					return func_492(uParam0->f_4356);
				
				case 2:
					return func_490(uParam0->f_4356);
				
				case 3:
					return func_494(uParam0->f_4356);
				
				case 4:
					return func_172(uParam0->f_4356);
				
				case 5:
					return func_487(uParam0->f_4356);
				
				default:
			}
			break;
		
		case 3:
			switch (iParam1)
			{
				case 0:
					return func_493(uParam0->f_4356);
				
				case 1:
					return func_492(uParam0->f_4356);
				
				case 2:
					return func_490(uParam0->f_4356);
				
				case 3:
					return func_494(uParam0->f_4356);
				
				case 4:
					return func_485(uParam0->f_4356);
				
				case 5:
					return func_486(uParam0->f_4356);
				
				default:
			}
			break;
	}
	return 0;
}

char* func_1079(var uParam0, int iParam1)
{
	switch (func_778(uParam0))
	{
		case 1:
			switch (iParam1)
			{
				case 0:
					return "CH_TODO_PREP_0";
				
				case 1:
					return "CH_TODO_PREP_1";
				
				case 2:
					return "CH_TODO_PREP_2";
				
				case 3:
					return "CH_TODO_PREP_3";
				
				case 4:
					return "CH_TODO_PREP_4";
				
				case 5:
					return "CH_TODO_PREP_5";
				
				default:
			}
			break;
		
		case 2:
			switch (iParam1)
			{
				case 0:
					return "CH_TODO_PREP_0";
				
				case 1:
					return "CH_TODO_PREP_1";
				
				case 2:
					return "CH_TODO_PREP_2";
				
				case 3:
					return "CH_TODO_PREP_3";
				
				case 4:
					return "CH_TODO_PREP_8";
				
				case 5:
					return "CH_TODO_PREP_9";
				
				default:
			}
			break;
		
		case 3:
			switch (iParam1)
			{
				case 0:
					return "CH_TODO_PREP_0";
				
				case 1:
					return "CH_TODO_PREP_1";
				
				case 2:
					return "CH_TODO_PREP_2";
				
				case 3:
					return "CH_TODO_PREP_3";
				
				case 4:
					if (func_178(uParam0->f_4356))
					{
						return "CH_TODO_PREP_6b";
					}
					return "CH_TODO_PREP_6";
				
				case 5:
					return "CH_TODO_PREP_7";
				
				default:
			}
			break;
	}
	return "";
}

var func_1080()
{
	return Global_1697304;
}

bool func_1081(var uParam0)
{
	return unk_0xCE990E643CD9D0E5(uParam0->f_744.f_736, 1);
}

void func_1082(var uParam0)
{
	if (unk_0x460153A63B9477BC() != uParam0->f_4356)
	{
		return;
	}
	if (func_17(uParam0, 1))
	{
		return;
	}
	if (!func_193(uParam0->f_4356, func_113(uParam0->f_4356)))
	{
		if (!func_883(func_1085(uParam0)))
		{
			switch (uParam0->f_744.f_737)
			{
				case 0:
					func_191(uParam0, func_1084(func_1085(uParam0)));
					uParam0->f_744.f_737++;
					break;
				
				case 1:
					if (func_882(uParam0, func_1084(func_1085(uParam0))))
					{
						if (func_193(uParam0->f_4356, 0))
						{
							func_191(uParam0, 69);
							uParam0->f_744.f_737 = 2;
						}
						else
						{
							func_851(uParam0, 10);
							uParam0->f_744.f_737 = 3;
						}
					}
					break;
				
				case 2:
					if (func_882(uParam0, 69))
					{
						func_880(func_1085(uParam0), 1);
						uParam0->f_744.f_737 = 0;
					}
					break;
				
				case 3:
					if (!uParam0->f_2529.f_62)
					{
						func_191(uParam0, 13);
						uParam0->f_744.f_737++;
					}
					break;
				
				case 4:
					if (func_882(uParam0, 13))
					{
						func_502(&(uParam0->f_4173.f_174));
						uParam0->f_744.f_737++;
					}
					break;
				
				case 5:
					if (func_503(&(uParam0->f_4173.f_174), 500, 0))
					{
						func_893(uParam0->f_744.f_709, 10);
						uParam0->f_2593 = 10;
						func_191(uParam0, 14);
						uParam0->f_744.f_737++;
					}
					break;
				
				case 6:
					if (func_882(uParam0, 14))
					{
						func_853(uParam0);
						func_502(&(uParam0->f_4173.f_174));
						uParam0->f_744.f_737++;
					}
					break;
				
				case 7:
					if (func_503(&(uParam0->f_4173.f_174), 500, 0))
					{
						func_893(uParam0->f_744.f_709, 11);
						uParam0->f_2593 = 11;
						func_191(uParam0, 15);
						uParam0->f_744.f_737++;
					}
					break;
				
				case 8:
					if (func_882(uParam0, 15))
					{
						func_853(uParam0);
						func_502(&(uParam0->f_4173.f_174));
						uParam0->f_744.f_737++;
					}
					break;
				
				case 9:
					if (func_503(&(uParam0->f_4173.f_174), 500, 0))
					{
						func_893(uParam0->f_744.f_709, 12);
						uParam0->f_2593 = 12;
						func_191(uParam0, 16);
						uParam0->f_744.f_737++;
					}
					break;
				
				case 10:
					if (func_882(uParam0, 16))
					{
						func_851(uParam0, 1);
						func_853(uParam0);
						func_502(&(uParam0->f_4173.f_174));
						func_880(func_1085(uParam0), 1);
						uParam0->f_744.f_737 = 0;
						func_189(uParam0, 3);
					}
					break;
				}
		}
	}
	else if (!func_883(func_1083(uParam0)))
	{
		switch (uParam0->f_744.f_737)
		{
			case 0:
				func_191(uParam0, 68);
				uParam0->f_744.f_737++;
				break;
			
			case 1:
				if (func_882(uParam0, 68))
				{
					func_191(uParam0, 69);
					uParam0->f_744.f_737++;
				}
				break;
			
			case 2:
				if (func_882(uParam0, 69))
				{
					func_880(func_1083(uParam0), 1);
					uParam0->f_744.f_737 = 0;
				}
				break;
			}
	}
}

int func_1083(var uParam0)
{
	switch (func_113(uParam0->f_4356))
	{
		case 1:
			return 8;
		
		case 2:
			return 9;
		
		case 3:
			return 10;
		
		default:
	}
	return -1;
}

int func_1084(int iParam0)
{
	switch (iParam0)
	{
		case 2:
			return 10;
		
		case 3:
			return 11;
		
		case 4:
			return 12;
		
		default:
	}
	return -1;
}

int func_1085(var uParam0)
{
	switch (func_113(uParam0->f_4356))
	{
		case 1:
			return 2;
		
		case 2:
			return 3;
		
		case 3:
			return 4;
		
		default:
	}
	return -1;
}

void func_1086(var uParam0)
{
	if (!func_17(uParam0, 1))
	{
		func_894(uParam0, 1);
	}
	func_701(&(uParam0->f_2593), 10, 0);
	func_893(uParam0->f_744.f_709, uParam0->f_2593);
	uParam0->f_744.f_703 = 1;
	func_190(&(uParam0->f_744), 1);
}

void func_1087(var uParam0)
{
}

void func_1088(var uParam0)
{
	func_1023(uParam0);
	func_1020(uParam0);
	func_898(uParam0);
	if (uParam0->f_744.f_724.f_5)
	{
		func_463(unk_0x460153A63B9477BC(), 0, 0, 0);
		uParam0->f_4360 = 1;
		unk_0x6A8F948698B360B4(unk_0x9B0761B4C3EB8BC7(), true);
		func_706(&(uParam0->f_744.f_724), 1);
	}
}

void func_1089(var uParam0)
{
	if (unk_0xCE990E643CD9D0E5(Global_1701669[unk_0x1EF8FCAB83E4BE53() /*68*/].f_42, 1))
	{
		unk_0xD2459066EA58CE43(&(Global_1701669[unk_0x1EF8FCAB83E4BE53() /*68*/].f_42), 1);
	}
}

void func_1090(var uParam0)
{
	func_1008(uParam0, 1);
	func_1007(uParam0, 1);
	func_962(uParam0, 1);
	func_960(uParam0, 1);
	func_958(uParam0, 1);
	func_1040(uParam0, 1);
	func_952(uParam0, 1, -1);
	func_1017(uParam0, 1);
	func_1018(uParam0, 1);
	if (func_164(uParam0->f_4356, 0) != 0)
	{
		func_957(uParam0, 1);
	}
	if (func_153(uParam0->f_4356, 0) != 0)
	{
		func_956(uParam0, 1);
	}
	func_955(uParam0, 1);
	func_1009(uParam0, 1);
	func_1071(uParam0, 1);
	func_954(uParam0, 1);
	func_1014(uParam0, 1);
}

void func_1091(var uParam0)
{
	bool bVar0;
	
	bVar0 = true;
	if (!func_906(&(uParam0->f_744)))
	{
		bVar0 = false;
	}
	if (!func_1092())
	{
		bVar0 = false;
	}
	if (bVar0)
	{
		func_706(&(uParam0->f_744.f_724), 1);
	}
}

bool func_1092()
{
	return Global_2460706.f_6;
}

void func_1093(var uParam0)
{
	func_1094(&(uParam0->f_744));
}

void func_1094(var uParam0)
{
	uParam0->f_709 = unk_0x5DEA4192B46CB99B("CASINO_HEIST_BOARD_PREP");
	uParam0->f_710 = unk_0x5DEA4192B46CB99B("INSTRUCTIONAL_BUTTONS");
}

void func_1095(var uParam0)
{
	func_701(&(uParam0->f_2593), 0, 0);
	func_190(&(uParam0->f_744), 1);
}

void func_1096(var uParam0)
{
	func_706(&(uParam0->f_744.f_724), 1);
}

void func_1097(var uParam0)
{
}

void func_1098(var uParam0)
{
	if (!func_918(uParam0) && !func_917())
	{
		func_706(&(uParam0->f_744.f_724), 1);
	}
}

void func_1099(var uParam0)
{
	func_1100(uParam0);
}

void func_1100(var uParam0)
{
	switch (uParam0->f_724)
	{
		case 0:
			func_1215(uParam0);
			break;
		
		case 1:
			if (!func_915(&(uParam0->f_724)))
			{
				func_1214(uParam0);
				func_913(&(uParam0->f_724));
			}
			func_1213(uParam0);
			if (!func_912(&(uParam0->f_724)))
			{
				func_1212(uParam0);
				func_910(&(uParam0->f_724));
			}
			break;
		
		case 2:
			if (!func_915(&(uParam0->f_724)))
			{
				func_1210(uParam0);
				func_913(&(uParam0->f_724));
			}
			func_1209(uParam0);
			if (!func_912(&(uParam0->f_724)))
			{
				func_1208(uParam0);
				func_910(&(uParam0->f_724));
			}
			break;
		
		case 3:
			if (!func_915(&(uParam0->f_724)))
			{
				func_1207(uParam0);
				func_913(&(uParam0->f_724));
			}
			func_1206(uParam0);
			if (!func_912(&(uParam0->f_724)))
			{
				func_1205(uParam0);
				func_910(&(uParam0->f_724));
			}
			break;
		
		case 4:
			if (!func_915(&(uParam0->f_724)))
			{
				func_1204(uParam0);
				func_913(&(uParam0->f_724));
			}
			func_1104(uParam0);
			if (!func_912(&(uParam0->f_724)))
			{
				func_1103(uParam0);
				func_910(&(uParam0->f_724));
			}
			break;
		
		case 5:
			if (!func_915(&(uParam0->f_724)))
			{
				func_1102(uParam0);
				func_913(&(uParam0->f_724));
			}
			func_1101(uParam0);
			break;
	}
}

void func_1101(var uParam0)
{
}

void func_1102(var uParam0)
{
	func_684();
	func_683(uParam0);
}

void func_1103(var uParam0)
{
	if (!func_17(uParam0, 1))
	{
		func_701(&(uParam0->f_2593), 0, 0);
		uParam0->f_703 = 0;
	}
}

void func_1104(var uParam0)
{
	func_891();
	func_1203(uParam0);
	func_1154(uParam0);
	func_1152(uParam0);
	func_1142(uParam0);
	func_1107(uParam0);
	func_1106(uParam0);
	func_714(uParam0);
	func_713();
	func_712(uParam0, 0);
	if (!uParam0->f_724.f_5)
	{
		if (!func_17(uParam0, 1))
		{
			if (uParam0->f_4360 && !func_202(unk_0x460153A63B9477BC()))
			{
				func_463(unk_0x460153A63B9477BC(), 1, 0, 0);
				unk_0x6A8F948698B360B4(unk_0x9B0761B4C3EB8BC7(), false);
				uParam0->f_4360 = 0;
			}
			func_1105(uParam0);
		}
		func_510(-1713398555, 12, 0);
		func_703(&(uParam0->f_724), 0);
	}
}

void func_1105(var uParam0)
{
	func_699(&(uParam0->f_2529.f_1), 0, 1);
	unk_0xD50CC9B03AAD56FF(0, 0);
}

void func_1106(var uParam0)
{
	int iVar0;
	
	if ((((func_17(uParam0, 1) || uParam0->f_2529.f_62) || uParam0->f_2604) || uParam0->f_741 != -1) || uParam0->f_743)
	{
		return;
	}
	if ((func_883(2) || func_883(3)) || func_883(4))
	{
		if (!unk_0xCE990E643CD9D0E5(func_9(8724, -1, 0), 23))
		{
			iVar0 = func_9(8724, -1, 0);
			unk_0xBE20AB8238688965(&iVar0, 23);
			func_6(8724, iVar0, -1, 1, 0);
			func_10("CH_SETUP_CANUSE", -1);
		}
	}
}

void func_1107(var uParam0)
{
	int iVar0;
	int iVar1;
	
	iVar0 = func_857(&(uParam0->f_2604));
	iVar1 = uParam0->f_2593;
	if (((((func_17(uParam0, 1) || uParam0->f_2604) || uParam0->f_741 != -1) || uParam0->f_743) || uParam0->f_708) || (func_16(uParam0) && iVar0 != 5))
	{
		return;
	}
	switch (uParam0->f_724.f_2)
	{
		case 0:
			switch (iVar0)
			{
				case 0:
				case 1:
				case 2:
				case 3:
					func_1141(uParam0, iVar0);
					func_196(uParam0, 1);
					func_853(uParam0);
					break;
				
				case 9:
					break;
				
				case 10:
					if (func_483(uParam0, 1))
					{
						func_850(uParam0, 1);
					}
					break;
				
				case 7:
					if (uParam0->f_4356 == unk_0x460153A63B9477BC())
					{
						if ((func_1140(iVar1) && !func_1139(uParam0)) && !func_1138(uParam0, iVar1))
						{
							uParam0->f_741 = func_1137(iVar1);
							unk_0xC4CC25B68A91D144(-1, "Highlight_Accept", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
						}
						if (func_1136(iVar1) && func_1133(uParam0, iVar1))
						{
							func_1131(uParam0, iVar1);
						}
					}
					if (func_1130(iVar1) && func_1124(uParam0, iVar1))
					{
						func_1123(uParam0, iVar1);
					}
					break;
				
				case 4:
					if (uParam0->f_4356 == unk_0x460153A63B9477BC())
					{
						if ((func_1121(uParam0, iVar1) && !func_1117(uParam0, iVar1)) && func_1116(uParam0, iVar1))
						{
							uParam0->f_741 = func_1137(iVar1);
							unk_0xC4CC25B68A91D144(-1, "Highlight_Accept", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
						}
					}
					break;
				
				case 8:
					func_201(uParam0, 0);
					break;
				
				case 5:
					if (func_1136(iVar1))
					{
						if (!unk_0x7BCC91F3C1CF24E8(func_181(uParam0, 0, iVar1)))
						{
							func_836(uParam0, 0);
						}
					}
					else if (func_1140(iVar1))
					{
						if (!unk_0x7BCC91F3C1CF24E8(func_132(uParam0, iVar1)))
						{
							func_836(uParam0, 0);
						}
					}
					else if (func_1121(uParam0, iVar1))
					{
						if (!unk_0x7BCC91F3C1CF24E8(func_128(uParam0, iVar1)))
						{
							func_836(uParam0, 0);
						}
					}
					else if (iVar1 == 8)
					{
						func_836(uParam0, 0);
					}
					break;
			}
			break;
		
		case 1:
			switch (iVar0)
			{
				case 2:
				case 3:
					if (func_1130(iVar1) && func_1124(uParam0, iVar1))
					{
						func_1109(uParam0, iVar1, iVar0);
					}
					break;
				
				case 7:
					break;
				
				case 8:
					func_1108(uParam0, iVar1);
					break;
			}
			break;
	}
}

void func_1108(var uParam0, int iParam1)
{
	if (uParam0->f_724.f_2 == 0)
	{
		return;
	}
	uParam0->f_724.f_2 = 0;
	uParam0->f_706 = 1;
	func_756(uParam0->f_709, iParam1, 0);
	func_190(uParam0, 1);
	if (uParam0->f_2529.f_33 != 0)
	{
		uParam0->f_2529.f_33 = 0;
	}
	unk_0xC4CC25B68A91D144(-1, "Back", "HUD_FRONTEND_DEFAULT_SOUNDSET", 1);
}

void func_1109(var uParam0, int iParam1, int iParam2)
{
	switch (iParam1)
	{
		case 12:
			switch (iParam2)
			{
				case 2:
					if (func_1115(uParam0, 0))
					{
						unk_0xC4CC25B68A91D144(-1, "Paper_Shuffle", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
						func_1114(uParam0, 1);
						func_190(uParam0, 1);
					}
					break;
				
				case 3:
					if (func_1115(uParam0, 1))
					{
						unk_0xC4CC25B68A91D144(-1, "Paper_Shuffle", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
						func_1114(uParam0, 1);
						func_190(uParam0, 1);
					}
					break;
			}
			break;
		
		case 14:
			switch (iParam2)
			{
				case 2:
					if (func_1113(uParam0, 0))
					{
						unk_0xC4CC25B68A91D144(-1, "Paper_Shuffle", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
						func_1112(uParam0, 1);
						func_190(uParam0, 1);
					}
					break;
				
				case 3:
					if (func_1113(uParam0, 1))
					{
						unk_0xC4CC25B68A91D144(-1, "Paper_Shuffle", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
						func_1112(uParam0, 1);
						func_190(uParam0, 1);
					}
					break;
			}
			break;
		
		case 16:
			switch (iParam2)
			{
				case 2:
					if (func_1111(uParam0, 0))
					{
						unk_0xC4CC25B68A91D144(-1, "Paper_Shuffle", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
						func_1110(uParam0, 1);
						func_190(uParam0, 1);
					}
					break;
				
				case 3:
					if (func_1111(uParam0, 1))
					{
						unk_0xC4CC25B68A91D144(-1, "Paper_Shuffle", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
						func_1110(uParam0, 1);
						func_190(uParam0, 1);
					}
					break;
			}
			break;
	}
}

void func_1110(var uParam0, bool bParam1)
{
	if (bParam1)
	{
		unk_0xBE20AB8238688965(&(uParam0->f_736), 13);
	}
	else
	{
		unk_0xD2459066EA58CE43(&(uParam0->f_736), 13);
	}
}

int func_1111(var uParam0, bool bParam1)
{
	bool bVar0;
	
	while (!bVar0)
	{
		if (bParam1)
		{
			uParam0->f_739++;
			if (uParam0->f_739 > 8)
			{
				uParam0->f_739 = 7;
			}
		}
		else
		{
			uParam0->f_739 = (uParam0->f_739 - 1);
			if (uParam0->f_739 < 7)
			{
				uParam0->f_739 = 8;
			}
		}
		if (func_179(uParam0->f_4356, uParam0->f_739))
		{
			bVar0 = true;
			return 1;
		}
	}
	return 0;
}

void func_1112(var uParam0, bool bParam1)
{
	if (bParam1)
	{
		unk_0xBE20AB8238688965(&(uParam0->f_736), 12);
	}
	else
	{
		unk_0xD2459066EA58CE43(&(uParam0->f_736), 12);
	}
}

int func_1113(var uParam0, bool bParam1)
{
	bool bVar0;
	
	while (!bVar0)
	{
		if (bParam1)
		{
			uParam0->f_738++;
			if (uParam0->f_738 > 6)
			{
				uParam0->f_738 = 3;
			}
		}
		else
		{
			uParam0->f_738 = (uParam0->f_738 - 1);
			if (uParam0->f_738 < 3)
			{
				uParam0->f_738 = 6;
			}
		}
		if (func_179(uParam0->f_4356, uParam0->f_738))
		{
			bVar0 = true;
			return 1;
		}
	}
	return 0;
}

void func_1114(var uParam0, bool bParam1)
{
	if (bParam1)
	{
		unk_0xBE20AB8238688965(&(uParam0->f_736), 11);
	}
	else
	{
		unk_0xD2459066EA58CE43(&(uParam0->f_736), 11);
	}
}

int func_1115(var uParam0, bool bParam1)
{
	bool bVar0;
	
	while (!bVar0)
	{
		if (bParam1)
		{
			uParam0->f_737++;
			if (uParam0->f_737 > 2)
			{
				uParam0->f_737 = 1;
			}
		}
		else
		{
			uParam0->f_737 = (uParam0->f_737 - 1);
			if (uParam0->f_737 < 1)
			{
				uParam0->f_737 = 2;
			}
		}
		if (func_179(uParam0->f_4356, uParam0->f_737))
		{
			bVar0 = true;
			return 1;
		}
	}
	return 0;
}

bool func_1116(var uParam0, int iParam1)
{
	return unk_0x6914A85D2E17013B(func_126(iParam1), 0, 1, 0, -1, 0);
}

int func_1117(var uParam0, int iParam1)
{
	switch (iParam1)
	{
		case 2:
			return func_1120(uParam0->f_4356);
		
		case 4:
			return func_1119(uParam0->f_4356);
		
		case 3:
			return func_1118(uParam0->f_4356);
		
		default:
	}
	return 0;
}

int func_1118(int iParam0)
{
	if (iParam0 != -1)
	{
		return unk_0xCE990E643CD9D0E5(Global_1701669[iParam0 /*68*/].f_40, 7);
	}
	return 0;
}

int func_1119(int iParam0)
{
	if (iParam0 != -1)
	{
		return unk_0xCE990E643CD9D0E5(Global_1701669[iParam0 /*68*/].f_40, 6);
	}
	return 0;
}

int func_1120(int iParam0)
{
	if (iParam0 != -1)
	{
		return unk_0xCE990E643CD9D0E5(Global_1701669[iParam0 /*68*/].f_40, 8);
	}
	return 0;
}

int func_1121(var uParam0, int iParam1)
{
	switch (iParam1)
	{
		case 2:
		case 3:
			return 1;
		
		case 4:
			return func_1122(uParam0->f_4356);
		
		default:
	}
	return 0;
}

int func_1122(int iParam0)
{
	if (iParam0 != -1)
	{
		return unk_0xCE990E643CD9D0E5(Global_1701669[iParam0 /*68*/].f_40, 5);
	}
	return 0;
}

void func_1123(var uParam0, int iParam1)
{
	if (uParam0->f_724.f_2 == 1)
	{
		return;
	}
	uParam0->f_724.f_2 = 1;
	uParam0->f_706 = 1;
	func_756(uParam0->f_709, iParam1, 1);
	func_190(uParam0, 1);
	if (uParam0->f_2529.f_33 != func_842(uParam0))
	{
		uParam0->f_2529.f_33 = func_842(uParam0);
	}
	unk_0xC4CC25B68A91D144(-1, "Highlight_Accept", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
}

int func_1124(var uParam0, int iParam1)
{
	if (func_1128(uParam0, iParam1) || func_1138(uParam0, iParam1))
	{
		return 0;
	}
	switch (iParam1)
	{
		case 5:
		case 6:
		case 7:
			return !func_1139(uParam0);
		
		case 11:
			return 0;
		
		case 12:
			return func_1127(uParam0);
		
		case 13:
			return 0;
		
		case 14:
			return func_1126(uParam0);
		
		case 15:
			return 0;
		
		case 16:
			return func_1125(uParam0);
		
		case 8:
			return 0;
		
		case 2:
			return 0;
		
		case 3:
			return 0;
		
		case 4:
			return 0;
		
		default:
	}
	return 1;
}

bool func_1125(var uParam0)
{
	int iVar0;
	
	iVar0 = 0;
	if (func_179(uParam0->f_4356, 7))
	{
		iVar0++;
	}
	if (func_179(uParam0->f_4356, 8))
	{
		iVar0++;
	}
	return iVar0 > 1;
}

bool func_1126(var uParam0)
{
	int iVar0;
	
	iVar0 = 0;
	if (func_179(uParam0->f_4356, 3))
	{
		iVar0++;
	}
	if (func_179(uParam0->f_4356, 4))
	{
		iVar0++;
	}
	if (func_179(uParam0->f_4356, 5))
	{
		iVar0++;
	}
	if (func_179(uParam0->f_4356, 5))
	{
		iVar0++;
	}
	return iVar0 > 1;
}

bool func_1127(var uParam0)
{
	int iVar0;
	
	iVar0 = 0;
	if (func_179(uParam0->f_4356, 1))
	{
		iVar0++;
	}
	if (func_179(uParam0->f_4356, 2))
	{
		iVar0++;
	}
	return iVar0 > 1;
}

int func_1128(var uParam0, int iParam1)
{
	switch (iParam1)
	{
		case 5:
			return func_778(uParam0) == 1;
		
		case 6:
			return func_778(uParam0) == 2;
		
		case 7:
			return func_778(uParam0) == 3;
		
		case 2:
			return func_1120(uParam0->f_4356);
		
		case 4:
			return func_1119(uParam0->f_4356);
		
		case 3:
			return func_1118(uParam0->f_4356);
		
		case 10:
			return func_1129(uParam0->f_4356);
		
		default:
	}
	return 0;
}

int func_1129(int iParam0)
{
	if (iParam0 != -1)
	{
		return unk_0xCE990E643CD9D0E5(Global_1701669[iParam0 /*68*/].f_18.f_1, 0);
	}
	return 0;
}

int func_1130(int iParam0)
{
	switch (iParam0)
	{
		case 11:
		case 12:
		case 13:
		case 14:
		case 15:
		case 16:
			return 1;
		
		default:
	}
	return 0;
}

void func_1131(var uParam0, int iParam1)
{
	int iVar0;
	
	if (func_460(0))
	{
		iVar0 = func_1132(iParam1);
		if (iVar0 != -1)
		{
			func_445(iVar0);
			func_443();
			func_442(uParam0, 2);
			func_853(uParam0);
			Global_1701669[unk_0x1EF8FCAB83E4BE53() /*68*/].f_67 = 1;
			unk_0xC4CC25B68A91D144(-1, "Highlight_Accept", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
		}
	}
	else
	{
		switch (func_278(unk_0x460153A63B9477BC(), 158, 0))
		{
			case 15:
				if (func_276(unk_0x460153A63B9477BC()))
				{
					func_10("GENERAL_MLF_G1", -1);
				}
				else if (func_275(unk_0x460153A63B9477BC()))
				{
					func_10("GENERAL_MLF_G2", -1);
				}
				else
				{
					func_10("GENERAL_MLF_G3", -1);
				}
				break;
			
			case 16:
				if (func_276(unk_0x460153A63B9477BC()))
				{
					func_10("GENERAL_MLF_G4", -1);
				}
				else if (func_275(unk_0x460153A63B9477BC()))
				{
					func_10("GENERAL_MLF_G5", -1);
				}
				else
				{
					func_10("GENERAL_MLF_G6", -1);
				}
				break;
			}
	}
}

int func_1132(int iParam0)
{
	switch (iParam0)
	{
		case 9:
			return 1;
		
		case 10:
			return 2;
		
		default:
	}
	return -1;
}

int func_1133(var uParam0, int iParam1)
{
	switch (iParam1)
	{
		case 9:
			if ((func_1135(uParam0->f_4356) && func_1134(uParam0->f_4356)) && func_1122(uParam0->f_4356))
			{
				return 0;
			}
			break;
		
		case 10:
			if (!func_265(uParam0->f_4356))
			{
				return 0;
			}
			break;
	}
	if (func_1128(uParam0, iParam1))
	{
		return 0;
	}
	if (func_1138(uParam0, iParam1))
	{
		return 0;
	}
	return 1;
}

int func_1134(int iParam0)
{
	int iVar0;
	
	if (iParam0 != -1)
	{
		iVar0 = 0;
		while (iVar0 < 10)
		{
			if (!unk_0xCE990E643CD9D0E5(Global_1701669[iParam0 /*68*/].f_18.f_3, iVar0))
			{
				return 0;
			}
			iVar0++;
		}
		return 1;
	}
	return 0;
}

int func_1135(int iParam0)
{
	int iVar0;
	
	if (iParam0 != -1)
	{
		iVar0 = 0;
		while (iVar0 < 11)
		{
			if (!unk_0xCE990E643CD9D0E5(Global_1701669[iParam0 /*68*/].f_18.f_2, iVar0))
			{
				return 0;
			}
			iVar0++;
		}
		return 1;
	}
	return 0;
}

int func_1136(int iParam0)
{
	switch (iParam0)
	{
		case 9:
		case 10:
			return 1;
		
		default:
	}
	return 0;
}

int func_1137(int iParam0)
{
	switch (iParam0)
	{
		case 5:
			return 3;
		
		case 6:
			return 4;
		
		case 7:
			return 5;
		
		case 2:
			return 9;
		
		case 4:
			return 10;
		
		case 3:
			return 11;
		
		default:
	}
	return -1;
}

int func_1138(var uParam0, int iParam1)
{
	switch (iParam1)
	{
		case 10:
			if (!func_265(uParam0->f_4356))
			{
				return 1;
			}
			break;
		
		case 5:
			if (func_195(uParam0->f_4356) == 1)
			{
				return 1;
			}
			break;
		
		case 6:
			if (func_195(uParam0->f_4356) == 2)
			{
				return 1;
			}
			break;
		
		case 7:
			if (func_195(uParam0->f_4356) == 3)
			{
				return 1;
			}
			break;
	}
	return 0;
}

bool func_1139(var uParam0)
{
	return func_778(uParam0) != 0;
}

int func_1140(int iParam0)
{
	switch (iParam0)
	{
		case 5:
		case 6:
		case 7:
			return 1;
		
		default:
	}
	return 0;
}

void func_1141(var uParam0, int iParam1)
{
	uParam0->f_2593.f_1 = 0;
	switch (iParam1)
	{
		case 1:
			func_856(uParam0->f_709, 187);
			unk_0xC4CC25B68A91D144(-1, "Highlight_Move", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
			break;
		
		case 0:
			func_856(uParam0->f_709, 188);
			unk_0xC4CC25B68A91D144(-1, "Highlight_Move", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
			break;
		
		case 2:
			func_856(uParam0->f_709, 189);
			unk_0xC4CC25B68A91D144(-1, "Highlight_Move", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
			break;
		
		case 3:
			func_856(uParam0->f_709, 190);
			unk_0xC4CC25B68A91D144(-1, "Highlight_Move", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 1);
			break;
	}
}

void func_1142(var uParam0)
{
	bool bVar0;
	int iVar1;
	var uVar2;
	bool bVar3;
	
	if (uParam0->f_741 == -1)
	{
		return;
	}
	if (func_529(uParam0, uParam0->f_741, &bVar0))
	{
		if (bVar0)
		{
			switch (uParam0->f_741)
			{
				case 3:
					func_1151(1);
					func_763(uParam0->f_709, 5, 1);
					func_1150(uParam0, 1);
					func_1149(uParam0, 1);
					bVar3 = true;
					if (func_193(uParam0->f_4356, 3))
					{
						func_191(uParam0, 65);
						uParam0->f_4173.f_177 = 1;
					}
					else
					{
						func_191(uParam0, 7);
						uParam0->f_4173.f_176 = 1;
					}
					uParam0->f_744.f_702 = 1;
					break;
				
				case 4:
					func_1151(2);
					func_763(uParam0->f_709, 6, 1);
					func_1150(uParam0, 1);
					func_1149(uParam0, 1);
					bVar3 = true;
					if (func_193(uParam0->f_4356, 3))
					{
						func_191(uParam0, 66);
						uParam0->f_4173.f_177 = 1;
					}
					else
					{
						func_191(uParam0, 8);
						uParam0->f_4173.f_176 = 1;
					}
					uParam0->f_744.f_702 = 1;
					break;
				
				case 5:
					func_1151(3);
					func_763(uParam0->f_709, 7, 1);
					func_1150(uParam0, 1);
					func_1149(uParam0, 1);
					bVar3 = true;
					if (func_193(uParam0->f_4356, 3))
					{
						func_191(uParam0, 67);
						uParam0->f_4173.f_177 = 1;
					}
					else
					{
						func_191(uParam0, 9);
						uParam0->f_4173.f_176 = 1;
					}
					uParam0->f_744.f_702 = 1;
					break;
				
				case 9:
					iVar1 = func_126(2);
					if (func_527())
					{
						func_515(395122350, iVar1, &uVar2, 0, 1, 0);
					}
					else
					{
						unk_0x171DB82E3FFD657F(iVar1, 0, 1, 0, 0, 1, iVar1, 0, 0, 0, 0);
					}
					func_1148(1);
					func_763(uParam0->f_709, 2, 1);
					func_1147(uParam0, 1);
					func_1149(uParam0, 1);
					bVar3 = true;
					break;
				
				case 10:
					iVar1 = func_126(4);
					if (func_527())
					{
						func_515(-331981076, iVar1, &uVar2, 0, 1, 0);
					}
					else
					{
						unk_0x171DB82E3FFD657F(iVar1, 0, 1, 0, 0, 0, 0, 1, iVar1, 0, 0);
					}
					func_1146(1);
					func_763(uParam0->f_709, 4, 1);
					func_1147(uParam0, 1);
					func_1149(uParam0, 1);
					bVar3 = true;
					break;
				
				case 11:
					iVar1 = func_126(3);
					if (func_527())
					{
						func_515(1671535231, iVar1, &uVar2, 0, 1, 0);
					}
					else
					{
						unk_0x171DB82E3FFD657F(iVar1, 0, 1, 0, 0, 0, 0, 0, 0, 1, iVar1);
					}
					func_1143(1);
					func_763(uParam0->f_709, 3, 1);
					func_1147(uParam0, 1);
					func_1149(uParam0, 1);
					bVar3 = true;
					break;
			}
			if (bVar3)
			{
				func_190(uParam0, 1);
			}
			uParam0->f_741 = -1;
			uParam0->f_743 = 1;
		}
		else
		{
			uParam0->f_741 = -1;
			uParam0->f_743 = 1;
			uParam0->f_742 = 0;
		}
	}
}

void func_1143(bool bParam0)
{
	int iVar0;
	
	iVar0 = func_9(8724, -1, 0);
	if (bParam0)
	{
		unk_0xBE20AB8238688965(&(Global_1701669[unk_0x460153A63B9477BC() /*68*/].f_40), 7);
		unk_0xBE20AB8238688965(&iVar0, 7);
		func_6(8724, iVar0, -1, 1, 0);
		func_1144(35, 18);
	}
	else
	{
		unk_0xD2459066EA58CE43(&(Global_1701669[unk_0x460153A63B9477BC() /*68*/].f_40), 7);
		unk_0xD2459066EA58CE43(&iVar0, 7);
		func_6(8724, iVar0, -1, 1, 0);
		func_1144(35, 0);
	}
}

void func_1144(int iParam0, int iParam1)
{
	func_6(func_1145(iParam0), iParam1, -1, 1, 0);
	Global_1590682[unk_0x460153A63B9477BC() /*883*/].f_274.f_401[iParam0] = iParam1;
}

int func_1145(int iParam0)
{
	switch (iParam0)
	{
		case 0:
			return 8748;
			break;
		
		case 1:
			return 8749;
			break;
		
		case 2:
			return 8750;
			break;
		
		case 3:
			return 8751;
			break;
		
		case 4:
			return 8752;
			break;
		
		case 5:
			return 8753;
			break;
		
		case 6:
			return 8754;
			break;
		
		case 7:
			return 8755;
			break;
		
		case 8:
			return 8756;
			break;
		
		case 9:
			return 8757;
			break;
		
		case 10:
			return 8758;
			break;
		
		case 11:
			return 8759;
			break;
		
		case 12:
			return 8760;
			break;
		
		case 13:
			return 8761;
			break;
		
		case 14:
			return 8762;
			break;
		
		case 15:
			return 8763;
			break;
		
		case 16:
			return 8764;
			break;
		
		case 17:
			return 8765;
			break;
		
		case 18:
			return 8766;
			break;
		
		case 19:
			return 8767;
			break;
		
		case 20:
			return 8768;
			break;
		
		case 21:
			return 8769;
			break;
		
		case 22:
			return 8770;
			break;
		
		case 23:
			return 8771;
			break;
		
		case 24:
			return 8772;
			break;
		
		case 25:
			return 8773;
			break;
		
		case 26:
			return 8774;
			break;
		
		case 27:
			return 8775;
			break;
		
		case 28:
			return 8776;
			break;
		
		case 29:
			return 8777;
			break;
		
		case 30:
			return 8778;
			break;
		
		case 31:
			return 8779;
			break;
		
		case 32:
			return 8780;
			break;
		
		case 33:
			return 8781;
			break;
		
		case 34:
			return 8782;
			break;
		
		case 35:
			return 8783;
			break;
		
		case 36:
			return 8784;
			break;
		
		case 37:
		case 38:
		case 39:
		case 40:
			return 8785;
			break;
	}
	return 8748;
}

void func_1146(bool bParam0)
{
	int iVar0;
	
	iVar0 = func_9(8724, -1, 0);
	if (bParam0)
	{
		unk_0xBE20AB8238688965(&iVar0, 6);
		func_6(8724, iVar0, -1, 1, 0);
		func_1144(36, 19);
	}
	else
	{
		unk_0xD2459066EA58CE43(&iVar0, 6);
		func_6(8724, iVar0, -1, 1, 0);
		func_1144(36, 0);
	}
}

void func_1147(var uParam0, bool bParam1)
{
	if (bParam1)
	{
		unk_0xBE20AB8238688965(&(uParam0->f_736), 2);
	}
	else
	{
		unk_0xD2459066EA58CE43(&(uParam0->f_736), 2);
	}
}

void func_1148(bool bParam0)
{
	int iVar0;
	
	iVar0 = func_9(8724, -1, 0);
	if (bParam0)
	{
		unk_0xBE20AB8238688965(&(Global_1701669[unk_0x460153A63B9477BC() /*68*/].f_40), 8);
		unk_0xBE20AB8238688965(&iVar0, 8);
		func_6(8724, iVar0, -1, 1, 0);
	}
	else
	{
		unk_0xD2459066EA58CE43(&(Global_1701669[unk_0x460153A63B9477BC() /*68*/].f_40), 8);
		unk_0xD2459066EA58CE43(&iVar0, 8);
		func_6(8724, iVar0, -1, 1, 0);
	}
}

void func_1149(var uParam0, bool bParam1)
{
	if (bParam1)
	{
		unk_0xBE20AB8238688965(&(uParam0->f_736), 14);
	}
	else
	{
		unk_0xD2459066EA58CE43(&(uParam0->f_736), 14);
	}
}

void func_1150(var uParam0, bool bParam1)
{
	if (bParam1)
	{
		unk_0xBE20AB8238688965(&(uParam0->f_736), 1);
	}
	else
	{
		unk_0xD2459066EA58CE43(&(uParam0->f_736), 1);
	}
}

void func_1151(int iParam0)
{
	func_6(8528, iParam0, -1, 1, 0);
	Global_1701669[unk_0x460153A63B9477BC() /*68*/].f_18.f_4 = iParam0;
}

void func_1152(var uParam0)
{
	if (uParam0->f_702)
	{
		if (unk_0x95EF219D38B20CFF(uParam0->f_709))
		{
			unk_0xD8D551845E3EA730(1);
			unk_0xCA1F114B668562E5(uParam0->f_700);
			switch (2)
			{
				case 0:
					break;
				
				case 1:
					unk_0x6D9B01B4CCEFF373(uParam0->f_709, 1);
					break;
				
				case 2:
					unk_0x178D84F9C5057462(uParam0->f_709, 1);
					break;
			}
			unk_0xE6A03BA84586E67B(uParam0->f_709, 0.25f, 0.5f, 0.5f, 1f, 255, 255, 255, 255, 0);
			unk_0xCA1F114B668562E5(unk_0xC48A590C39E17456());
			unk_0xD8D551845E3EA730(0);
			if (uParam0->f_705)
			{
			}
		}
	}
	if (uParam0->f_703)
	{
		func_1153(uParam0);
	}
}

void func_1153(var uParam0)
{
	struct<9> Var0;
	bool bVar9;
	int iVar10;
	
	Var0 = { func_877() };
	if (unk_0xDFAB5E5CBE16E8B1())
	{
		return;
	}
	bVar9 = uParam0->f_706;
	if (func_16(uParam0) || uParam0->f_740 > 0)
	{
		if (!uParam0->f_704)
		{
			bVar9 = true;
			uParam0->f_704 = 1;
		}
	}
	else if (uParam0->f_704)
	{
		bVar9 = true;
		uParam0->f_704 = 0;
	}
	if (!bVar9)
	{
		func_872(&(uParam0->f_710), &Var0, uParam0, 0);
		return;
	}
	if (!unk_0xE5EF21F14D0F9D25() || unk_0xA2C6E70C9BCA9C34() == 10)
	{
		func_871(uParam0, 0.5f);
	}
	else
	{
		func_871(uParam0, 0.7f);
	}
	func_869(uParam0);
	iVar10 = uParam0->f_2593;
	if (uParam0->f_704)
	{
		func_867(2, 204, "CH_INSTR_INFO", uParam0, 1, 361);
		if (func_716(&(uParam0->f_2529)))
		{
			if (unk_0xC1F43F8D0D5EB664(2))
			{
				func_867(2, 39, "CH_INSTR_ZOOM", uParam0, 1, 361);
			}
			else
			{
				func_866(2, 15, "CH_INSTR_ZOOM", uParam0);
			}
		}
		func_866(2, 1, "HEIST_IB_LOOK", uParam0);
	}
	else
	{
		switch (uParam0->f_724.f_2)
		{
			case 0:
				if (uParam0->f_4356 == unk_0x460153A63B9477BC())
				{
					if (func_1136(iVar10))
					{
						if (func_1133(uParam0, iVar10))
						{
							func_867(2, 201, "CH_INSTR_LM", uParam0, 1, 361);
						}
					}
					else if (func_1124(uParam0, iVar10))
					{
						func_867(2, 201, "CH_INSTR_SEL", uParam0, 1, 361);
					}
				}
				func_867(2, 202, "CH_INSTR_QUIT", uParam0, 1, 361);
				if (uParam0->f_4356 == unk_0x460153A63B9477BC())
				{
					if (func_1121(uParam0, iVar10) && func_1116(uParam0, iVar10))
					{
						if (!func_1117(uParam0, iVar10))
						{
							func_867(2, 203, "CH_INSTR_PURCH", uParam0, 1, 361);
						}
					}
				}
				if (func_1136(iVar10))
				{
					if (!unk_0x7BCC91F3C1CF24E8(func_181(uParam0, 0, iVar10)))
					{
						func_867(2, 204, "CH_INSTR_INFO", uParam0, 1, 361);
					}
				}
				else if (func_1140(iVar10))
				{
					if (!unk_0x7BCC91F3C1CF24E8(func_132(uParam0, iVar10)))
					{
						func_867(2, 204, "CH_INSTR_INFO", uParam0, 1, 361);
					}
				}
				else if (func_1121(uParam0, iVar10))
				{
					if (!unk_0x7BCC91F3C1CF24E8(func_128(uParam0, iVar10)))
					{
						func_867(2, 204, "CH_INSTR_INFO", uParam0, 1, 361);
					}
				}
				else if (iVar10 == 8)
				{
					func_867(2, 204, "CH_INSTR_INFO", uParam0, 1, 361);
				}
				if (func_483(uParam0, 1))
				{
					func_867(2, 206, "CH_INSTR_PREP", uParam0, 1, 361);
				}
				if (unk_0xC1F43F8D0D5EB664(2))
				{
					func_867(2, 39, "CH_INSTR_ZOOM", uParam0, 1, 361);
				}
				else
				{
					func_866(2, 15, "CH_INSTR_ZOOM", uParam0);
				}
				func_866(2, 1, "HEIST_IB_LOOK", uParam0);
				func_866(2, 0, "HEIST_IB_NAV", uParam0);
				break;
			
			case 1:
				func_867(2, 202, "CH_INSTR_BACK", uParam0, 1, 361);
				func_866(2, 1, "HEIST_IB_LOOK", uParam0);
				func_866(2, 8, "CH_INSTR_CHACP", uParam0);
				break;
			}
	}
	func_872(&(uParam0->f_710), &Var0, uParam0, uParam0->f_706);
	func_190(uParam0, 0);
}

void func_1154(var uParam0)
{
	int iVar0;
	int iVar1;
	var uVar2;
	
	if (func_1202(uParam0))
	{
		if (func_1201(uParam0))
		{
			func_1199(uParam0);
			func_1198(uParam0, 0);
			func_1197(uParam0, 1);
		}
		return;
	}
	if (func_1196(uParam0))
	{
		func_1195(uParam0->f_709, 1);
		func_1197(uParam0, 0);
	}
	if (func_1194(uParam0) || func_1043())
	{
		func_790(uParam0->f_709);
		iVar0 = 0;
		while (iVar0 < 4)
		{
			if (!unk_0x7BCC91F3C1CF24E8(func_1193(iVar0)))
			{
				func_787(uParam0->f_709, func_1193(iVar0), func_1192(uParam0, iVar0));
			}
			iVar0++;
		}
		func_1150(uParam0, 0);
	}
	if (func_1191(uParam0) || func_1043())
	{
		func_784(uParam0->f_709);
		iVar0 = 0;
		while (iVar0 < 5)
		{
			if (!unk_0x7BCC91F3C1CF24E8(func_1190(uParam0, iVar0)))
			{
				func_781(uParam0->f_709, func_1190(uParam0, iVar0), func_1189(uParam0, iVar0));
			}
			iVar0++;
		}
		func_1147(uParam0, 0);
	}
	if (func_1188(uParam0) || func_1043())
	{
		iVar1 = 2;
		while (iVar1 <= 16)
		{
			if (func_1187(uParam0, iVar1) != -1)
			{
				func_1186(uParam0->f_709, iVar1, func_1187(uParam0, iVar1));
			}
			iVar1++;
		}
		if (func_1129(uParam0->f_4356))
		{
			func_1185(uParam0->f_709, func_120(func_121(uParam0->f_4356), 1));
		}
		func_1184(uParam0, 0);
	}
	if (func_1183(uParam0) || func_1043())
	{
		uVar2 = 10;
		func_1179(uParam0, &uVar2);
		func_1178(uParam0->f_709, &uVar2);
		func_1177(uParam0, 0);
	}
	if (func_1176(uParam0) || func_1043())
	{
		switch (func_113(uParam0->f_4356))
		{
			case 1:
				func_763(uParam0->f_709, 5, 1);
				break;
			
			case 2:
				func_763(uParam0->f_709, 6, 1);
				break;
			
			case 3:
				func_763(uParam0->f_709, 7, 1);
				break;
		}
		switch (func_194(uParam0->f_4356))
		{
			case 1:
				func_1175(uParam0->f_709, 5, 1);
				break;
			
			case 2:
				func_1175(uParam0->f_709, 6, 1);
				break;
			
			case 3:
				func_1175(uParam0->f_709, 7, 1);
				break;
		}
		func_1174(uParam0, 0);
	}
	if (func_1173(uParam0) || func_1043())
	{
		iVar1 = 0;
		while (iVar1 < 17)
		{
			func_1066(uParam0->f_709, iVar1, func_1138(uParam0, iVar1));
			iVar1++;
		}
		func_1172(uParam0, 0);
	}
	if (func_1171(uParam0) || func_1043())
	{
		iVar1 = 0;
		while (iVar1 < 17)
		{
			func_710(uParam0->f_709, iVar1, func_1170(uParam0, iVar1));
			if (func_1169(uParam0, iVar1) != -1)
			{
				func_756(uParam0->f_709, iVar1, func_1169(uParam0, iVar1));
			}
			iVar1++;
		}
		func_1168(uParam0, 0);
	}
	if (func_1167(uParam0) || func_1043())
	{
		iVar1 = 0;
		while (iVar1 < 17)
		{
			if (func_1166(iVar1))
			{
				func_763(uParam0->f_709, iVar1, func_1128(uParam0, iVar1));
			}
			iVar1++;
		}
		func_1165(uParam0, 0);
	}
	if (func_1164(uParam0) || func_1043())
	{
		func_1162(uParam0->f_709, func_1163(uParam0));
		func_1161(uParam0, 0);
	}
	if (func_1160(uParam0))
	{
		iVar1 = 0;
		while (iVar1 < 17)
		{
			if (func_1159(uParam0, iVar1))
			{
				func_708(uParam0->f_709, iVar1, 1);
			}
			else
			{
				func_708(uParam0->f_709, iVar1, 0);
			}
			iVar1++;
		}
		func_1149(uParam0, 0);
	}
	if (func_1158(uParam0) || func_1043())
	{
		func_1186(uParam0->f_709, 12, func_1187(uParam0, 12));
		func_1114(uParam0, 0);
	}
	if (func_1157(uParam0) || func_1043())
	{
		func_1186(uParam0->f_709, 14, func_1187(uParam0, 14));
		func_1112(uParam0, 0);
	}
	if (func_1156(uParam0) || func_1043())
	{
		func_1186(uParam0->f_709, 16, func_1187(uParam0, 16));
		func_1110(uParam0, 0);
	}
	func_1155(0);
}

void func_1155(int iParam0)
{
	if (Global_1697302 && Global_1697303 != iParam0)
	{
		Global_1697303 = iParam0;
	}
}

bool func_1156(var uParam0)
{
	return unk_0xCE990E643CD9D0E5(uParam0->f_736, 13);
}

bool func_1157(var uParam0)
{
	return unk_0xCE990E643CD9D0E5(uParam0->f_736, 12);
}

bool func_1158(var uParam0)
{
	return unk_0xCE990E643CD9D0E5(uParam0->f_736, 11);
}

int func_1159(var uParam0, int iParam1)
{
	if (func_1136(iParam1) && func_1133(uParam0, iParam1))
	{
		return 0;
	}
	if (func_1121(uParam0, iParam1) && !func_1117(uParam0, iParam1))
	{
		return 0;
	}
	if (func_1124(uParam0, iParam1))
	{
		return 0;
	}
	return 1;
}

bool func_1160(var uParam0)
{
	return unk_0xCE990E643CD9D0E5(uParam0->f_736, 14);
}

void func_1161(var uParam0, bool bParam1)
{
	if (bParam1)
	{
		unk_0xBE20AB8238688965(&(uParam0->f_736), 9);
	}
	else
	{
		unk_0xD2459066EA58CE43(&(uParam0->f_736), 9);
	}
}

void func_1162(var uParam0, bool bParam1)
{
	unk_0x830F007E19C63E14(uParam0, "SET_BLUEPRINT_VISIBLE");
	unk_0xF1B28F753235CE2A(uParam1);
	unk_0xE1FDD153F5858534();
}

int func_1163(var uParam0)
{
	return func_265(uParam0->f_4356);
}

bool func_1164(var uParam0)
{
	return unk_0xCE990E643CD9D0E5(uParam0->f_736, 9);
}

void func_1165(var uParam0, bool bParam1)
{
	if (bParam1)
	{
		unk_0xBE20AB8238688965(&(uParam0->f_736), 7);
	}
	else
	{
		unk_0xD2459066EA58CE43(&(uParam0->f_736), 7);
	}
}

int func_1166(int iParam0)
{
	switch (iParam0)
	{
		case 5:
		case 6:
		case 7:
		case 2:
		case 4:
		case 3:
		case 9:
		case 10:
			return 1;
		
		default:
	}
	return 0;
}

bool func_1167(var uParam0)
{
	return unk_0xCE990E643CD9D0E5(uParam0->f_736, 7);
}

void func_1168(var uParam0, bool bParam1)
{
	if (bParam1)
	{
		unk_0xBE20AB8238688965(&(uParam0->f_736), 8);
	}
	else
	{
		unk_0xD2459066EA58CE43(&(uParam0->f_736), 8);
	}
}

int func_1169(var uParam0, int iParam1)
{
	switch (iParam1)
	{
		case 11:
		case 12:
		case 13:
		case 14:
		case 15:
		case 16:
			if (func_1124(uParam0, iParam1))
			{
				return 0;
			}
			else
			{
				return 2;
			}
			break;
	}
	return -1;
}

int func_1170(var uParam0, int iParam1)
{
	switch (iParam1)
	{
		case 5:
			if (!func_265(uParam0->f_4356) || !func_1129(uParam0->f_4356))
			{
				return 0;
			}
			break;
		
		case 6:
			if (!func_265(uParam0->f_4356) || !func_1129(uParam0->f_4356))
			{
				return 0;
			}
			break;
		
		case 7:
			if (!func_265(uParam0->f_4356) || !func_1129(uParam0->f_4356))
			{
				return 0;
			}
			break;
		
		case 11:
			if (!func_179(uParam0->f_4356, 0))
			{
				return 0;
			}
			break;
		
		case 12:
			if (!func_179(uParam0->f_4356, 1) && !func_179(uParam0->f_4356, 2))
			{
				return 0;
			}
			break;
		
		case 13:
			if (!func_179(uParam0->f_4356, 10))
			{
				return 0;
			}
			break;
		
		case 14:
			if (((!func_179(uParam0->f_4356, 3) && !func_179(uParam0->f_4356, 4)) && !func_179(uParam0->f_4356, 5)) && !func_179(uParam0->f_4356, 5))
			{
				return 0;
			}
			break;
		
		case 15:
			if (!func_179(uParam0->f_4356, 9))
			{
				return 0;
			}
			break;
		
		case 16:
			if (!func_179(uParam0->f_4356, 7) && !func_179(uParam0->f_4356, 8))
			{
				return 0;
			}
			break;
		
		case 2:
			if (!func_265(uParam0->f_4356))
			{
				return 0;
			}
			break;
		
		case 4:
			if (!func_265(uParam0->f_4356))
			{
				return 0;
			}
			if (!func_1122(uParam0->f_4356))
			{
				return 0;
			}
			break;
		
		case 3:
			if (!func_265(uParam0->f_4356))
			{
				return 0;
			}
			break;
		
		case 8:
			if (!func_1129(uParam0->f_4356))
			{
				return 0;
			}
			break;
	}
	return 1;
}

bool func_1171(var uParam0)
{
	return unk_0xCE990E643CD9D0E5(uParam0->f_736, 8);
}

void func_1172(var uParam0, bool bParam1)
{
	if (bParam1)
	{
		unk_0xBE20AB8238688965(&(uParam0->f_736), 6);
	}
	else
	{
		unk_0xD2459066EA58CE43(&(uParam0->f_736), 6);
	}
}

bool func_1173(var uParam0)
{
	return unk_0xCE990E643CD9D0E5(uParam0->f_736, 6);
}

void func_1174(var uParam0, bool bParam1)
{
	if (bParam1)
	{
		unk_0xBE20AB8238688965(&(uParam0->f_736), 5);
	}
	else
	{
		unk_0xD2459066EA58CE43(&(uParam0->f_736), 5);
	}
}

void func_1175(var uParam0, int iParam1, bool bParam2)
{
	unk_0x830F007E19C63E14(uParam0, "SET_EXTREME");
	unk_0xAD291B8F75D737AD(iParam1);
	unk_0xF1B28F753235CE2A(iParam2);
	unk_0xE1FDD153F5858534();
}

bool func_1176(var uParam0)
{
	return unk_0xCE990E643CD9D0E5(uParam0->f_736, 5);
}

void func_1177(var uParam0, bool bParam1)
{
	if (bParam1)
	{
		unk_0xBE20AB8238688965(&(uParam0->f_736), 4);
	}
	else
	{
		unk_0xD2459066EA58CE43(&(uParam0->f_736), 4);
	}
}

void func_1178(var uParam0, var uParam1)
{
	int iVar0;
	
	unk_0x830F007E19C63E14(uParam0, "SET_POI_IMAGES");
	iVar0 = 0;
	while (iVar0 < *uParam1)
	{
		if ((*uParam1)[iVar0] != -1)
		{
			unk_0xAD291B8F75D737AD((*uParam1)[iVar0]);
		}
		iVar0++;
	}
	unk_0xE1FDD153F5858534();
}

void func_1179(var uParam0, var uParam1)
{
	int iVar0;
	
	iVar0 = 0;
	while (iVar0 < 10)
	{
		(*uParam1)[iVar0] = func_1180(func_1182(uParam0->f_4356, iVar0), func_1181(iVar0), -1);
		iVar0++;
	}
}

int func_1180(bool bParam0, var uParam1, int iParam2)
{
	if (bParam0)
	{
		return uParam1;
	}
	return iParam2;
}

int func_1181(int iParam0)
{
	switch (iParam0)
	{
		case 0:
			return 2;
		
		case 1:
			return 4;
		
		case 2:
			return 5;
		
		case 3:
			return 9;
		
		case 4:
			return 1;
		
		case 5:
			return 10;
		
		case 6:
			return 8;
		
		case 7:
			return 3;
		
		case 8:
			return 7;
		
		case 9:
			return 6;
		
		default:
	}
	return -1;
}

int func_1182(int iParam0, int iParam1)
{
	if (iParam0 != -1)
	{
		return unk_0xCE990E643CD9D0E5(Global_1701669[iParam0 /*68*/].f_18.f_3, iParam1);
	}
	return 0;
}

bool func_1183(var uParam0)
{
	return unk_0xCE990E643CD9D0E5(uParam0->f_736, 4);
}

void func_1184(var uParam0, bool bParam1)
{
	if (bParam1)
	{
		unk_0xBE20AB8238688965(&(uParam0->f_736), 3);
	}
	else
	{
		unk_0xD2459066EA58CE43(&(uParam0->f_736), 3);
	}
}

void func_1185(var uParam0, var uParam1)
{
	unk_0x830F007E19C63E14(uParam0, "SET_TARGET_TYPE");
	func_83(uParam1);
	unk_0xE1FDD153F5858534();
}

void func_1186(var uParam0, int iParam1, int iParam2)
{
	unk_0x830F007E19C63E14(uParam0, "SET_BUTTON_IMAGE");
	unk_0xAD291B8F75D737AD(iParam1);
	unk_0xAD291B8F75D737AD(iParam2);
	unk_0xE1FDD153F5858534();
}

int func_1187(var uParam0, int iParam1)
{
	switch (iParam1)
	{
		case 2:
			return 1;
		
		case 4:
			return 2;
		
		case 3:
			return 3;
		
		case 5:
			return 1;
		
		case 6:
			return 2;
		
		case 7:
			return 3;
		
		case 8:
			switch (func_121(uParam0->f_4356))
			{
				case 0:
					return 1;
				
				case 1:
					return 2;
				
				case 2:
					return 3;
				
				case 3:
					return 4;
				
				default:
			}
			break;
		
		case 9:
			return 1;
		
		case 10:
			return 2;
		
		case 11:
			return 2;
		
		case 12:
			switch (uParam0->f_737)
			{
				case 1:
					return 11;
				
				case 2:
					return 1;
				
				default:
			}
			break;
		
		case 13:
			return 7;
		
		case 14:
			switch (uParam0->f_738)
			{
				case 3:
					return 10;
				
				case 4:
					return 5;
				
				case 5:
					return 8;
				
				case 6:
					return 3;
				
				default:
			}
			break;
		
		case 15:
			return 6;
		
		case 16:
			switch (uParam0->f_739)
			{
				case 7:
					return 9;
				
				case 8:
					return 4;
				
				default:
			}
			break;
	}
	return -1;
}

bool func_1188(var uParam0)
{
	return unk_0xCE990E643CD9D0E5(uParam0->f_736, 3);
}

int func_1189(var uParam0, int iParam1)
{
	switch (iParam1)
	{
		case 0:
			return func_1134(uParam0->f_4356);
		
		case 1:
			return func_1135(uParam0->f_4356);
		
		case 2:
			return func_1120(uParam0->f_4356);
		
		case 3:
			return func_1118(uParam0->f_4356);
		
		case 4:
			return func_1119(uParam0->f_4356);
		
		default:
	}
	return 0;
}

char* func_1190(var uParam0, int iParam1)
{
	switch (iParam1)
	{
		case 0:
			return "CH_OPT_SETUP_0";
		
		case 1:
			return "CH_OPT_SETUP_1";
		
		case 2:
			return "CH_OPT_SETUP_2";
		
		case 3:
			return "CH_OPT_SETUP_3";
		
		case 4:
			if (func_1122(uParam0->f_4356))
			{
				return "CH_OPT_SETUP_4";
			}
			else
			{
				return "CH_LIST_BLANK";
			}
			break;
	}
	return "";
}

bool func_1191(var uParam0)
{
	return unk_0xCE990E643CD9D0E5(uParam0->f_736, 2);
}

int func_1192(var uParam0, int iParam1)
{
	switch (iParam1)
	{
		case 0:
			return func_265(uParam0->f_4356);
		
		case 1:
			return func_1129(uParam0->f_4356);
		
		case 2:
			return func_1139(uParam0);
		
		default:
	}
	return 0;
}

char* func_1193(int iParam0)
{
	switch (iParam0)
	{
		case 0:
			return "CH_TODO_SETUP_0";
		
		case 1:
			return "CH_TODO_SETUP_1";
		
		case 2:
			return "CH_TODO_SETUP_2";
		
		default:
	}
	return "";
}

bool func_1194(var uParam0)
{
	return unk_0xCE990E643CD9D0E5(uParam0->f_736, 1);
}

void func_1195(var uParam0, bool bParam1)
{
	unk_0x830F007E19C63E14(uParam0, "SET_GRAPHICS_VISIBLE");
	unk_0xF1B28F753235CE2A(iParam1);
	unk_0xE1FDD153F5858534();
}

bool func_1196(var uParam0)
{
	return unk_0xCE990E643CD9D0E5(uParam0->f_736, 15);
}

void func_1197(var uParam0, bool bParam1)
{
	if (bParam1)
	{
		unk_0xBE20AB8238688965(&(uParam0->f_736), 15);
	}
	else
	{
		unk_0xD2459066EA58CE43(&(uParam0->f_736), 15);
	}
}

void func_1198(var uParam0, bool bParam1)
{
	if (bParam1)
	{
		unk_0xBE20AB8238688965(&(uParam0->f_736), 10);
	}
	else
	{
		unk_0xD2459066EA58CE43(&(uParam0->f_736), 10);
	}
}

void func_1199(var uParam0)
{
	int iVar0;
	
	func_1195(uParam0->f_709, 0);
	iVar0 = 0;
	while (iVar0 < 17)
	{
		func_710(uParam0->f_709, iVar0, 0);
		iVar0++;
	}
	iVar0 = 0;
	while (iVar0 < 2)
	{
		func_787(uParam0->f_709, func_1193(iVar0), func_1200(uParam0, iVar0));
		iVar0++;
	}
}

int func_1200(var uParam0, int iParam1)
{
	if (func_193(uParam0->f_4356, 0) && !func_267(uParam0->f_4356))
	{
		return iParam1 == 0;
	}
	return 0;
}

bool func_1201(var uParam0)
{
	return unk_0xCE990E643CD9D0E5(uParam0->f_736, 10);
}

int func_1202(var uParam0)
{
	if (!func_265(uParam0->f_4356) && !func_193(uParam0->f_4356, 0))
	{
		return 1;
	}
	else if (func_193(uParam0->f_4356, 0) && !func_267(uParam0->f_4356))
	{
		return 1;
	}
	return 0;
}

void func_1203(var uParam0)
{
	if (unk_0x460153A63B9477BC() != uParam0->f_4356)
	{
		return;
	}
	if (!func_193(uParam0->f_4356, 0))
	{
		if (!func_883(0))
		{
			switch (uParam0->f_740)
			{
				case 0:
					func_191(uParam0, 0);
					uParam0->f_740++;
					break;
				
				case 1:
					if (func_503(&(uParam0->f_4173.f_174), 6500, 0))
					{
						func_851(uParam0, 3);
						func_853(uParam0);
						uParam0->f_740++;
					}
					break;
				
				case 2:
					if (!uParam0->f_2529.f_62 && func_503(&(uParam0->f_4173.f_174), 12500, 0))
					{
						func_893(uParam0->f_709, 10);
						uParam0->f_2593 = 10;
						func_191(uParam0, 1);
						uParam0->f_740++;
					}
					break;
				
				case 3:
					if (func_882(uParam0, 1))
					{
						func_502(&(uParam0->f_4173.f_174));
						func_851(uParam0, 4);
						func_853(uParam0);
						uParam0->f_740++;
					}
					break;
				
				case 4:
					if (!uParam0->f_2529.f_62)
					{
						func_893(uParam0->f_709, 3);
						uParam0->f_2593 = 3;
						func_191(uParam0, 2);
						uParam0->f_740++;
					}
					break;
				
				case 5:
					if (func_503(&(uParam0->f_4173.f_174), 11000, 0) && uParam0->f_2593 != 2)
					{
						func_893(uParam0->f_709, 2);
						uParam0->f_2593 = 2;
						func_853(uParam0);
					}
					if (func_503(&(uParam0->f_4173.f_174), 15000, 0))
					{
						func_502(&(uParam0->f_4173.f_174));
						func_893(uParam0->f_709, 10);
						uParam0->f_2593 = 10;
						func_851(uParam0, 0);
						func_853(uParam0);
						uParam0->f_740 = 0;
						func_880(0, 1);
						func_189(uParam0, 0);
						func_189(uParam0, 1);
					}
					break;
			}
		}
		else if (!func_883(1) && func_1129(uParam0->f_4356))
		{
			switch (uParam0->f_740)
			{
				case 0:
					func_191(uParam0, 3);
					uParam0->f_740++;
					break;
				
				case 1:
					if (func_503(&(uParam0->f_4173.f_174), 4500, 0))
					{
						func_502(&(uParam0->f_4173.f_174));
						func_851(uParam0, 5);
						func_853(uParam0);
						uParam0->f_740++;
					}
					break;
				
				case 2:
					if (!uParam0->f_2529.f_62 && func_882(uParam0, 3))
					{
						func_893(uParam0->f_709, 5);
						uParam0->f_2593 = 5;
						func_191(uParam0, 4);
						uParam0->f_740++;
					}
					break;
				
				case 3:
					if (func_882(uParam0, 4))
					{
						func_853(uParam0);
						func_502(&(uParam0->f_4173.f_174));
						uParam0->f_740++;
					}
					break;
				
				case 4:
					if (func_503(&(uParam0->f_4173.f_174), 500, 0))
					{
						func_893(uParam0->f_709, 6);
						uParam0->f_2593 = 6;
						func_191(uParam0, 5);
						uParam0->f_740++;
					}
					break;
				
				case 5:
					if (func_882(uParam0, 5))
					{
						func_853(uParam0);
						func_502(&(uParam0->f_4173.f_174));
						uParam0->f_740++;
					}
					break;
				
				case 6:
					if (func_503(&(uParam0->f_4173.f_174), 500, 0))
					{
						func_893(uParam0->f_709, 7);
						uParam0->f_2593 = 7;
						func_191(uParam0, 6);
						uParam0->f_740++;
					}
					break;
				
				case 7:
					if (func_882(uParam0, 6))
					{
						func_502(&(uParam0->f_4173.f_174));
						func_851(uParam0, 0);
						func_853(uParam0);
						uParam0->f_740 = 0;
						func_880(1, 1);
						func_189(uParam0, 2);
					}
					break;
			}
		}
		else if (uParam0->f_4173.f_176)
		{
			switch (func_113(uParam0->f_4356))
			{
				case 1:
					if (func_882(uParam0, 7))
					{
						func_850(uParam0, 1);
						uParam0->f_4173.f_176 = 0;
					}
					break;
				
				case 2:
					if (func_882(uParam0, 8))
					{
						func_850(uParam0, 1);
						uParam0->f_4173.f_176 = 0;
					}
					break;
				
				case 3:
					if (func_882(uParam0, 9))
					{
						func_850(uParam0, 1);
						uParam0->f_4173.f_176 = 0;
					}
					break;
				}
		}
	}
	else if (uParam0->f_4173.f_177)
	{
		switch (func_113(uParam0->f_4356))
		{
			case 1:
				if (func_882(uParam0, 65))
				{
					func_850(uParam0, 1);
					uParam0->f_4173.f_177 = 0;
				}
				break;
			
			case 2:
				if (func_882(uParam0, 66))
				{
					func_850(uParam0, 1);
					uParam0->f_4173.f_177 = 0;
				}
				break;
			
			case 3:
				if (func_882(uParam0, 67))
				{
					func_850(uParam0, 1);
					uParam0->f_4173.f_177 = 0;
				}
				break;
			}
	}
}

void func_1204(var uParam0)
{
	if (!func_17(uParam0, 1))
	{
		func_894(uParam0, 0);
	}
	func_701(&(uParam0->f_2593), 9, 0);
	func_893(uParam0->f_709, uParam0->f_2593);
	uParam0->f_703 = 1;
	func_190(uParam0, 1);
}

void func_1205(var uParam0)
{
}

void func_1206(var uParam0)
{
	func_1154(uParam0);
	func_1152(uParam0);
	func_898(uParam0);
	if (uParam0->f_724.f_5)
	{
		func_463(unk_0x460153A63B9477BC(), 0, 0, 0);
		unk_0x6A8F948698B360B4(unk_0x9B0761B4C3EB8BC7(), true);
		uParam0->f_4360 = 1;
		func_706(&(uParam0->f_724), 0);
	}
}

void func_1207(var uParam0)
{
	if (unk_0xCE990E643CD9D0E5(Global_1701669[unk_0x1EF8FCAB83E4BE53() /*68*/].f_42, 1))
	{
		unk_0xD2459066EA58CE43(&(Global_1701669[unk_0x1EF8FCAB83E4BE53() /*68*/].f_42), 1);
	}
}

void func_1208(var uParam0)
{
	func_1150(uParam0, 1);
	func_1147(uParam0, 1);
	func_1184(uParam0, 1);
	func_1177(uParam0, 1);
	func_1174(uParam0, 1);
	func_1172(uParam0, 1);
	func_1168(uParam0, 1);
	func_1165(uParam0, 1);
	func_1161(uParam0, 1);
	func_1149(uParam0, 1);
	if (func_1202(uParam0))
	{
		func_1198(uParam0, 1);
	}
}

void func_1209(var uParam0)
{
	bool bVar0;
	
	bVar0 = true;
	if (!func_906(uParam0))
	{
		bVar0 = false;
	}
	if (!func_1092())
	{
		bVar0 = false;
	}
	if (bVar0)
	{
		func_706(&(uParam0->f_724), 0);
	}
}

void func_1210(var uParam0)
{
	func_1211(uParam0);
}

void func_1211(var uParam0)
{
	uParam0->f_709 = unk_0x5DEA4192B46CB99B("CASINO_HEIST_BOARD_SETUP");
	uParam0->f_710 = unk_0x5DEA4192B46CB99B("INSTRUCTIONAL_BUTTONS");
}

void func_1212(var uParam0)
{
	func_701(&(uParam0->f_2593), 0, 0);
	func_190(uParam0, 1);
}

void func_1213(var uParam0)
{
	func_706(&(uParam0->f_724), 0);
}

void func_1214(var uParam0)
{
}

void func_1215(var uParam0)
{
	if (!func_918(uParam0) && !func_917())
	{
		func_706(&(uParam0->f_724), 0);
	}
}

void func_1216(int iParam0)
{
	if (iLocal_255 != iParam0)
	{
		iLocal_255 = iParam0;
	}
}

int func_1217(var uParam0)
{
	if (!unk_0xFC559366953F62B3())
	{
		uParam0->f_4356 = func_270();
		if (uParam0->f_4356 != func_42())
		{
		}
	}
	if (((uParam0->f_4356 != func_42() && func_1227(uParam0)) && func_1226(uParam0)) && func_1218(uParam0))
	{
		if (!unk_0xCE990E643CD9D0E5(Global_1701669[unk_0x1EF8FCAB83E4BE53() /*68*/].f_42, 0))
		{
			unk_0xBE20AB8238688965(&(Global_1701669[unk_0x1EF8FCAB83E4BE53() /*68*/].f_42), 0);
		}
		return 1;
	}
	return 0;
}

bool func_1218(var uParam0)
{
	if (!unk_0xCE990E643CD9D0E5(uParam0->f_1497.f_1015, 0))
	{
		if (func_1221(func_1225(2), func_1224(2)))
		{
			uParam0->f_1497.f_700 = func_1220(func_1225(2));
			uParam0->f_1497.f_730 = { func_1219(0) };
			func_851(uParam0, 2);
			uParam0->f_2529.f_50 = { 1f, -2.5f, -1.2f };
			uParam0->f_2529.f_56 = { 0f, 0f, 0f };
			uParam0->f_2529.f_35 = 0f;
			uParam0->f_2529.f_35.f_1 = 0f;
			uParam0->f_2529.f_35.f_2 = -180f;
			if (!func_483(uParam0, 2))
			{
				uParam0->f_1497.f_702 = 0;
				uParam0->f_1497.f_703 = 0;
			}
			if (unk_0xFC559366953F62B3())
			{
				uParam0->f_1497.f_1017 = -1;
				uParam0->f_1497.f_1022 = -1;
				uParam0->f_1497.f_1018 = -1;
				uParam0->f_1497.f_1023 = -1;
				uParam0->f_1497.f_1019 = -1;
				uParam0->f_1497.f_1024 = -1;
				uParam0->f_1497.f_1020 = 0;
				uParam0->f_1497.f_1025 = 0;
				uParam0->f_1497.f_1021 = 5;
				uParam0->f_1497.f_1026 = 5;
			}
			else
			{
				uParam0->f_1497.f_1017 = 0;
				uParam0->f_1497.f_1022 = 0;
				uParam0->f_1497.f_1018 = 0;
				uParam0->f_1497.f_1023 = 0;
				uParam0->f_1497.f_1019 = 0;
				uParam0->f_1497.f_1024 = 0;
				if (func_173(uParam0->f_4356, 1))
				{
					uParam0->f_1497.f_1020 = 1;
					uParam0->f_1497.f_1025 = 1;
				}
				else if (func_173(uParam0->f_4356, 2))
				{
					uParam0->f_1497.f_1020 = 2;
					uParam0->f_1497.f_1025 = 2;
				}
				else if (func_173(uParam0->f_4356, 3))
				{
					uParam0->f_1497.f_1020 = 2;
					uParam0->f_1497.f_1025 = 2;
				}
				if (func_173(uParam0->f_4356, 6))
				{
					uParam0->f_1497.f_1021 = 6;
					uParam0->f_1497.f_1026 = 6;
				}
				else if (func_173(uParam0->f_4356, 7))
				{
					uParam0->f_1497.f_1021 = 7;
					uParam0->f_1497.f_1026 = 7;
				}
				uParam0->f_1497.f_736.f_276 = 1;
			}
			unk_0xBE20AB8238688965(&(uParam0->f_1497.f_1015), 0);
		}
	}
	return unk_0xCE990E643CD9D0E5(uParam0->f_1497.f_1015, 0);
}

Vector3 func_1219(int iParam0)
{
	switch (iParam0)
	{
		case 0:
			return 2712.903f, -366.1747f, -54.2342f;
		
		case 1:
			return 2716.304f, -369.5417f, -54.2342f;
		
		case 2:
			return 2712.903f, -372.6874f, -54.2342f;
		
		default:
	}
	return 2712.903f, -367.5012f, -54.2342f;
}

int func_1220(char* sParam0)
{
	if (!unk_0x7BCC91F3C1CF24E8(uParam0))
	{
		return unk_0x2BC38EC678978375(sParam0);
	}
	return -1;
}

bool func_1221(var uParam0, int iParam1)
{
	return (func_1223(uParam0) && func_1222(iParam1));
}

int func_1222(int iParam0)
{
	if (iParam0 != 0)
	{
		if (!unk_0x561503C7E871BCEC(iParam0))
		{
			unk_0xC7DE5B846819A9B2(iParam0);
		}
		return 1;
	}
	return 0;
}

int func_1223(char* sParam0)
{
	if (!unk_0x7BCC91F3C1CF24E8(uParam0))
	{
		if (!unk_0xA9A353D835A7867C(sParam0))
		{
			unk_0x3FF471D4A18A5ED5(sParam0, 0);
		}
		return 1;
	}
	return 0;
}

int func_1224(int iParam0)
{
	switch (iParam0)
	{
		case 0:
			return joaat("ch_prop_whiteboard");
		
		case 1:
			return joaat("ch_prop_whiteboard_02");
		
		case 2:
			return joaat("ch_prop_whiteboard_03");
		
		default:
	}
	return 0;
}

char* func_1225(int iParam0)
{
	switch (iParam0)
	{
		case 0:
			return "arcadeplan_01";
		
		case 1:
			return "arcadeplan_02";
		
		case 2:
			return "arcadeplan_03";
		
		default:
	}
	return "";
}

bool func_1226(var uParam0)
{
	if (!unk_0xCE990E643CD9D0E5(uParam0->f_744.f_736, 0))
	{
		if (func_1221(func_1225(1), func_1224(1)))
		{
			uParam0->f_744.f_700 = func_1220(func_1225(1));
			uParam0->f_744.f_730 = { func_1219(0) };
			func_851(uParam0, 1);
			uParam0->f_2529.f_50 = { 1f, -2.5f, -1.2f };
			uParam0->f_2529.f_56 = { 0f, 0f, 0f };
			uParam0->f_2529.f_35 = 0f;
			uParam0->f_2529.f_35.f_1 = 0f;
			uParam0->f_2529.f_35.f_2 = -180f;
			if (func_164(uParam0->f_4356, 0) == 0)
			{
				uParam0->f_744.f_738 = 1;
				uParam0->f_744.f_739 = 0;
			}
			else
			{
				uParam0->f_744.f_738 = func_164(uParam0->f_4356, 0);
				if (func_493(uParam0->f_4356))
				{
					uParam0->f_744.f_739 = func_737(uParam0->f_4356);
				}
				else
				{
					uParam0->f_744.f_739 = 0;
				}
			}
			if (func_153(uParam0->f_4356, 0) == 0)
			{
				uParam0->f_744.f_740 = 1;
				uParam0->f_744.f_741 = 0;
			}
			else
			{
				uParam0->f_744.f_740 = func_153(uParam0->f_4356, 0);
				if (func_492(uParam0->f_4356))
				{
					uParam0->f_744.f_741 = func_733(uParam0->f_4356);
				}
				else
				{
					uParam0->f_744.f_741 = 0;
				}
			}
			if (func_491(uParam0->f_4356, 0) == 0)
			{
				uParam0->f_744.f_743 = 1;
			}
			else
			{
				uParam0->f_744.f_743 = func_491(uParam0->f_4356, 0);
			}
			if (func_936(uParam0->f_4356) == 0)
			{
				uParam0->f_744.f_742 = 1;
			}
			else
			{
				uParam0->f_744.f_742 = func_936(uParam0->f_4356);
			}
			if (Global_1701669[uParam0->f_4356 /*68*/].f_64 != 0)
			{
				uParam0->f_744.f_744 = Global_1701669[uParam0->f_4356 /*68*/].f_64;
			}
			else
			{
				uParam0->f_744.f_744 = 1;
			}
			uParam0->f_744.f_745 = 6;
			if (!func_483(uParam0, 1))
			{
				uParam0->f_744.f_702 = 0;
				uParam0->f_744.f_703 = 0;
			}
			unk_0xBE20AB8238688965(&(uParam0->f_744.f_736), 0);
		}
	}
	return unk_0xCE990E643CD9D0E5(uParam0->f_744.f_736, 0);
}

bool func_1227(var uParam0)
{
	if (!unk_0xCE990E643CD9D0E5(uParam0->f_736, 0))
	{
		if (func_1221(func_1225(0), func_1224(0)))
		{
			uParam0->f_700 = func_1220(func_1225(0));
			uParam0->f_730 = { func_1219(0) };
			uParam0->f_2529.f_33 = 0;
			uParam0->f_2529.f_50 = { 1f, -2.5f, -1.2f };
			uParam0->f_2529.f_56 = { 0f, 0f, 0f };
			uParam0->f_2529.f_35 = 0f;
			uParam0->f_2529.f_35.f_1 = 0f;
			uParam0->f_2529.f_35.f_2 = -180f;
			if (func_179(uParam0->f_4356, 3))
			{
				uParam0->f_738 = 3;
			}
			else if (func_179(uParam0->f_4356, 4))
			{
				uParam0->f_738 = 4;
			}
			else if (func_179(uParam0->f_4356, 5))
			{
				uParam0->f_738 = 5;
			}
			else if (func_179(uParam0->f_4356, 6))
			{
				uParam0->f_738 = 6;
			}
			if (func_179(uParam0->f_4356, 7))
			{
				uParam0->f_739 = 7;
			}
			else if (func_179(uParam0->f_4356, 8))
			{
				uParam0->f_739 = 8;
			}
			unk_0xBE20AB8238688965(&(uParam0->f_736), 0);
		}
	}
	return unk_0xCE990E643CD9D0E5(uParam0->f_736, 0);
}

int func_1228()
{
	return iLocal_255;
}

void func_1229(var uParam0)
{
	if (func_918(uParam0) || func_917())
	{
		if (!uParam0->f_4359 && !func_917())
		{
			func_1231(uParam0);
			uParam0->f_4359 = 1;
		}
		else if (func_1230(uParam0) || func_917())
		{
			func_1216(2);
			uParam0->f_4359 = 0;
		}
	}
}

int func_1230(var uParam0)
{
	if ((uParam0->f_724 == 0 && uParam0->f_744.f_724 == 0) && uParam0->f_1497.f_724 == 0)
	{
		return 1;
	}
	return 0;
}

void func_1231(var uParam0)
{
	if (uParam0->f_724 < 5)
	{
		func_1232(&(uParam0->f_724), 5);
	}
	if (uParam0->f_744.f_724 < 5)
	{
		func_1232(&(uParam0->f_744.f_724), 5);
	}
	if (uParam0->f_1497.f_724 < 5)
	{
		func_1232(&(uParam0->f_1497.f_724), 5);
	}
}

void func_1232(var uParam0, int iParam1)
{
	uParam0->f_1 = *uParam0;
	unk_0xBE20AB8238688965(&(uParam0->f_4), uParam0->f_1);
	unk_0xD2459066EA58CE43(&(uParam0->f_3), uParam0->f_1);
	*uParam0 = iParam1;
	if (*uParam0 == 7)
	{
		*uParam0 = 5;
	}
}

void func_1233()
{
	func_685(0);
	func_480();
	func_1234();
	if (unk_0x1EF8FCAB83E4BE53() != -1 && unk_0xCE990E643CD9D0E5(Global_1701669[unk_0x1EF8FCAB83E4BE53() /*68*/].f_42, 0))
	{
		unk_0xD2459066EA58CE43(&(Global_1701669[unk_0x1EF8FCAB83E4BE53() /*68*/].f_42), 0);
	}
	if (unk_0xCE990E643CD9D0E5(Global_1701669[unk_0x1EF8FCAB83E4BE53() /*68*/].f_42, 1))
	{
		unk_0xD2459066EA58CE43(&(Global_1701669[unk_0x1EF8FCAB83E4BE53() /*68*/].f_42), 1);
	}
	if (Global_1697302)
	{
		Global_1697302 = 0;
	}
	unk_0xD39E529EBE5DB04F();
}

void func_1234()
{
	func_1235(func_1225(0));
	func_1235(func_1225(1));
	func_1235(func_1225(2));
}

void func_1235(char* sParam0)
{
	if (unk_0xA9A353D835A7867C(uParam0))
	{
		unk_0x07FF72DF7F314523(sParam0);
	}
}

int func_1236(var uParam0)
{
	if (!unk_0x25DDB354A40FFCDB())
	{
		return 0;
	}
	if (unk_0xCE990E643CD9D0E5(uParam0->f_1497.f_1015, 2))
	{
		unk_0xD2459066EA58CE43(&(uParam0->f_1497.f_1015), 2);
	}
	if (unk_0xCE990E643CD9D0E5(uParam0->f_1497.f_1015, 1))
	{
		unk_0xD2459066EA58CE43(&(uParam0->f_1497.f_1015), 1);
	}
	unk_0xDB4E5F36ED3505EE();
	if (!Global_1697302)
	{
		Global_1697302 = 1;
	}
	return 1;
}

